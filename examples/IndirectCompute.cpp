...
void initBuffers() {
    static const uint32_t indirectData[3] = {kNumParticles, 1, 1};
    indirectBuffer = utils::CreateBufferFromData(device, indirectData, sizeof(indirectData),
    ...
    modelBuffer =
        utils::CreateBufferFromData(device, model, sizeof(model), dawn::BufferUsageBit::Vertex);

    SimParams params = {0.04f, 0.1f, 0.025f, 0.025f, 0.02f, 0.05f, 0.005f, kNumParticles};
    updateParams =
        utils::CreateBufferFromData(device, &params, sizeof(params), dawn::BufferUsageBit::Uniform);
    ...
    for (size_t i = 0; i < 2; i++) {
        ...
        descriptor.usage = dawn::BufferUsageBit::TransferDst | dawn::BufferUsageBit::Vertex |
                           dawn::BufferUsageBit::Storage;
        particleBuffers[i] = device.CreateBuffer(&descriptor);

        particleBuffers[i].SetSubData(0, sizeof(Particle) * kNumParticles,
