...
//  - V: switches present modes.
//
// Closing all the windows exits the example. ^C also works.
//
// Things to test manually:
//
//  - With the triangle in a window, check it is red and pointy side up.
//  - Cycle render modes a bunch and check that the triangle background is always solid black.
//  - With the cycling colors, check that VSync on cycles reasonably fast, and has no tearing.
//    (can't check it is at the display refresh rate because VSync doesn't make the difference
//    between double and triple buffering).
//  - With the cycling colors, check that VSync off cycles much faster and can tear. (dragging
//    between two monitors can help see tearing).
//  - Check that cycling VSync and NoVSync works.
...
//  - Check rendering triangles in multiple windows works.
//  - Check that cycling colors in multiple windows cycles them as fast in VSync as a cycle
//    window.
//  - Check that cycling colors in multiple windows cycles them faster than VSync if they are
//    all NoVSync.
...
void AddWindow() {
    ...
    GLFWwindow* window = glfwCreateWindow(400, 400, "Yet to be named", nullptr, nullptr);
    ...
    descriptor.width = 0;
    descriptor.height = 0;
    descriptor.presentMode = wgpu::PresentMode::VSync;
...
std::ostream& operator<<(std::ostream& o, const wgpu::SwapChainDescriptor& desc) {
    ...
    switch (desc.presentMode) {
        case wgpu::PresentMode::VSync:
            o << "VSync";
            break;
        case wgpu::PresentMode::NoVSync:
            o << "NoVSync";
            break;
    }
    return o;
    ...
    switch (key) {
        ...
        case GLFW_KEY_V:
            switch (data->targetDesc.presentMode) {
                case wgpu::PresentMode::VSync:
                    data->targetDesc.presentMode = wgpu::PresentMode::NoVSync;
                    break;
                case wgpu::PresentMode::NoVSync:
                    data->targetDesc.presentMode = wgpu::PresentMode::VSync;
                    break;
            }
            break;
...
int main(int argc, const char* argv[]) {
    ...
    // Or hacky pipeline to render a triangle.
