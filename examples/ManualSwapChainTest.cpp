...
//    - Check the triangle is red on a black baground and with the pointy side up.
//    - Cycle render modes a bunch and check that the triangle background is always solid black.
//    - Check that rendering triangels to multiple windows works.
//
...
//    - Check latching the swapchain config and resizing the window a bunch (smaller, biffer, and
//      diagonal aspect ratio).
//
//  - Config change tests:
//    - Check that cycling between present modes works.
//    - TODO can't be tested yet: check cycking the same window over multiple devices.
//    - TODO can't be tested yet: check cycking the same window over multiple formats.
