...
namespace {
    ...
    void initTextures() {
        for (const auto& t : scene.textures) {
            ...
            dawn::BufferCopyView bufferCopyView = utils::CreateBufferCopyView(staging, 0, rowPitch, 0);
            dawn::TextureCopyView textureCopyView = utils::CreateTextureCopyView(oTexture, 0, 0, 0, 0, 0, dawn::TextureAspect::Color);
            dawn::Extent3D copySize = utils::CreateExtent3D(iImage.width, iImage.height, 1);
