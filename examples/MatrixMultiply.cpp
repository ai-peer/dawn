...
constexpr uint32_t kWorkPerThread = 4;
constexpr uint32_t kDispatchSize = (kMatrixSize + kTileSize - 1) / kTileSize;
constexpr uint32_t kNumMultiplications = 20;
...
void ProfileMultiply(dawn::Buffer A,
    ...
    std::cout << "10 trials of " << kNumMultiplications << " consecutive " << kMatrixSize << "x"
...
int main(int argc, const char* argv[]) {
    ...
    std::string tileLocalShader = R"(
        ...
        const uint TileSize = )" +
                               std::to_string(kTileSize) + R"(;

        const uint WorkPerThread = )" +
                               std::to_string(kWorkPerThread) + R"(;
