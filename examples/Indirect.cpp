...
void initBuffers() {
    ...
    static const uint32_t indirectData[5] = {6, 1, 0, 0, 0};
...
void init() {
    ...
    descriptor.cVertexInput.cAttributes[0].format = dawn::VertexFormat::Float3;
    descriptor.cVertexInput.numBuffers = 1;
    descriptor.cVertexInput.cBuffers[0].stride = 3 * sizeof(float);
...
void frame() {
    ...
    utils::ComboRenderPassDescriptor renderPass({backbuffer.CreateDefaultView()}, depthStencilView);
