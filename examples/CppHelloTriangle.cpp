...
void fillUploadData(std::array<uint8_t, kTextureRowPitchAlignment * kBlockHeightCount>* uploadData,
    ...
    constexpr std::array<uint8_t, kBCBlockWidth* kBCBlockHeight> kBC1Data = {0,  248, 224, 7,
                                                                             80, 80,  80,  80};
    ...
    constexpr std::array<uint8_t, 16> kBC6HSFloatData = {227, 30,  0, 0,   0, 224, 30, 0,
                                                         0,   255, 0, 255, 0, 255, 0,  255};
    constexpr std::array<uint8_t, 16> kBC6HUFloatData = {227, 61,  0, 0,   0, 224, 61, 0,
                                                         0,   255, 0, 255, 0, 255, 0,  255};
    ...
    for (uint32_t i = 0; i < kBlockHeightCount; ++i) {
        for (uint32_t j = 0; j < kBlockWidthCount; ++j) {
            for (uint32_t k = 0; k < compressedDataSize; ++k) {
                (*uploadData)[kTextureRowPitchAlignment * i + compressedDataSize * j + k] =
                    compressedDataPtr[k];
            }
        }
    }
