...
void init() {
    ...
    dawn::BindGroupLayout bgl = utils::MakeBindGroupLayout(device, {{0, dawn::ShaderStageBit::Vertex, dawn::BindingType::DynamicUniformBuffer}});
    ...
    bindGroup = utils::MakeBindGroup(device, bgl, {{0, ubo, 0, kNumTriangles * sizeof(ShaderData)}});
}
...
void frame() {
    ...
    ubo.SetSubData(0, kNumTriangles * sizeof(ShaderData), reinterpret_cast<const uint8_t*>(shaderData.data()));
