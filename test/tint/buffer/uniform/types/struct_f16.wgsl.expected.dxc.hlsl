...
Inner tint_symbol_1(uint offset) {
  ...
  const Inner tint_symbol_6 = {float16_t(f16tof32(((u[scalar_offset_index / 4][scalar_offset_index % 4] >> (scalar_offset_bytes % 4 == 0 ? 0 : 16)) & 0xFFFF))), vector<float16_t, 3>(ubo_load_4_xz[0], ubo_load_4_y, ubo_load_4_xz[1]), tint_symbol_4((offset + 16u))};
  return tint_symbol_6;
}

S tint_symbol(uint offset) {
  const S tint_symbol_7 = {tint_symbol_1((offset + 0u))};
  return tint_symbol_7;
}

[numthreads(1, 1, 1)]
void main() {
  const S x = tint_symbol(0u);
  return;
}
