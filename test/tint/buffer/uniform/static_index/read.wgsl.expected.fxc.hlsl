...
tint_symbol_34_ret tint_symbol_34(uint4 buffer[55], uint offset) {
  float3 arr[2] = (float3[2])0;
  {
    for(uint i = 0u; (i < 2u); i = (i + 1u)) {
      ...
      arr[i] = asfloat(buffer[scalar_offset_index_54 / 4].xyz);
    }
  }
  return arr;
}
...
Inner tint_symbol_36(uint4 buffer[55], uint offset) {
  const uint scalar_offset_bytes_55 = ((offset + 0u));
  const uint scalar_offset_index_55 = scalar_offset_bytes_55 / 4;
  const uint scalar_offset_bytes_56 = ((offset + 4u));
  const uint scalar_offset_index_56 = scalar_offset_bytes_56 / 4;
  const uint scalar_offset_bytes_57 = ((offset + 8u));
  const uint scalar_offset_index_57 = scalar_offset_bytes_57 / 4;
  const Inner tint_symbol_38 = {asint(buffer[scalar_offset_index_55 / 4][scalar_offset_index_55 % 4]), asfloat(buffer[scalar_offset_index_56 / 4][scalar_offset_index_56 % 4]), float16_t(f16tof32(((buffer[scalar_offset_index_57 / 4][scalar_offset_index_57 % 4] >> (scalar_offset_bytes_57 % 4 == 0 ? 0 : 16)) & 0xFFFF)))};
...
D:\Projects\RampUp\dawn\test\tint\buffer\Shader@0x000001DE1A9278A0(4,3-11): error X3000: unrecognized identifier 'float16_t'
