...
matrix<float16_t, 4, 2> tint_symbol_1(uint offset) {
...
void f() {
  ...
  const matrix<float16_t, 4, 2> l_a[4] = tint_symbol(0u);
  const matrix<float16_t, 4, 2> l_a_i = tint_symbol_1((16u * uint(p_a_i_save)));
