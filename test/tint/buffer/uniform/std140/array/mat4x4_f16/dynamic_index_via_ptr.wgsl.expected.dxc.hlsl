...
matrix<float16_t, 4, 4> tint_symbol_1(uint offset) {
...
tint_symbol_ret tint_symbol(uint offset) {
  matrix<float16_t, 4, 4> arr[4] = (matrix<float16_t, 4, 4>[4])0;
  {
    for(uint i_1 = 0u; (i_1 < 4u); i_1 = (i_1 + 1u)) {
      arr[i_1] = tint_symbol_1((offset + (i_1 * 32u)));
    }
  }
  return arr;
}
...
void f() {
  ...
  const matrix<float16_t, 4, 4> l_a[4] = tint_symbol(0u);
  const matrix<float16_t, 4, 4> l_a_i = tint_symbol_1((32u * uint(p_a_i_save)));
