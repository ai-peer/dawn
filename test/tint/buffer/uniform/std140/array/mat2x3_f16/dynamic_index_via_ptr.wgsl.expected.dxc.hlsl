...
matrix<float16_t, 2, 3> tint_symbol_1(uint offset) {
...
tint_symbol_ret tint_symbol(uint offset) {
  matrix<float16_t, 2, 3> arr[4] = (matrix<float16_t, 2, 3>[4])0;
  {
    for(uint i_1 = 0u; (i_1 < 4u); i_1 = (i_1 + 1u)) {
      arr[i_1] = tint_symbol_1((offset + (i_1 * 16u)));
    }
  }
  return arr;
}
...
void f() {
  ...
  const matrix<float16_t, 2, 3> l_a[4] = tint_symbol(0u);
  const matrix<float16_t, 2, 3> l_a_i = tint_symbol_1((16u * uint(p_a_i_save)));
