...
float4x4 tint_symbol_1(uint offset) {
...
tint_symbol_ret tint_symbol(uint offset) {
  float4x4 arr[4] = (float4x4[4])0;
  {
    for(uint i_1 = 0u; (i_1 < 4u); i_1 = (i_1 + 1u)) {
      arr[i_1] = tint_symbol_1((offset + (i_1 * 64u)));
    }
  }
  return arr;
}
...
void f() {
  ...
  const float4x4 l_a[4] = tint_symbol(0u);
  const float4x4 l_a_i = tint_symbol_1((64u * uint(p_a_i_save)));
