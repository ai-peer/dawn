...
void f() {
  ...
  const float2x2 l_a[4] = tint_symbol_2(a, 0u);
  const float2x2 l_a_i = tint_symbol_3(a, (16u * uint(tint_symbol)));
  const uint scalar_offset_2 = (((16u * uint(tint_symbol)) + (8u * uint(tint_symbol_1)))) / 4;
