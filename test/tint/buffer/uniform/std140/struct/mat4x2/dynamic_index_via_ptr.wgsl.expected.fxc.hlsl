...
float4x2 tint_symbol_12(uint4 buffer[32], uint offset) {
...
Inner tint_symbol_11(uint4 buffer[32], uint offset) {
  const Inner tint_symbol_15 = {tint_symbol_12(buffer, (offset + 0u))};
  return tint_symbol_15;
}

typedef Inner tint_symbol_10_ret[4];
tint_symbol_10_ret tint_symbol_10(uint4 buffer[32], uint offset) {
  Inner arr[4] = (Inner[4])0;
  {
    [loop] for(uint i_1 = 0u; (i_1 < 4u); i_1 = (i_1 + 1u)) {
      arr[i_1] = tint_symbol_11(buffer, (offset + (i_1 * 32u)));
    }
  }
  return arr;
}

Outer tint_symbol_9(uint4 buffer[32], uint offset) {
  const Outer tint_symbol_16 = {tint_symbol_10(buffer, (offset + 0u))};
  return tint_symbol_16;
}

typedef Outer tint_symbol_8_ret[4];
tint_symbol_8_ret tint_symbol_8(uint4 buffer[32], uint offset) {
  Outer arr_1[4] = (Outer[4])0;
  {
    [loop] for(uint i_2 = 0u; (i_2 < 4u); i_2 = (i_2 + 1u)) {
      arr_1[i_2] = tint_symbol_9(buffer, (offset + (i_2 * 128u)));
    }
  }
  return arr_1;
}
...
void f() {
  ...
  const Outer l_a_i = tint_symbol_9(a, (128u * uint(tint_symbol)));
  const Inner l_a_i_a[4] = tint_symbol_10(a, (128u * uint(tint_symbol)));
  const Inner l_a_i_a_i = tint_symbol_11(a, ((128u * uint(tint_symbol)) + (32u * uint(tint_symbol_1))));
  const float4x2 l_a_i_a_i_m = tint_symbol_12(a, ((128u * uint(tint_symbol)) + (32u * uint(tint_symbol_1))));
  const uint scalar_offset_4 = ((((128u * uint(tint_symbol)) + (32u * uint(tint_symbol_1))) + (8u * uint(tint_symbol_2)))) / 4;
  ...
  const uint scalar_offset_5 = (((((128u * tint_symbol_3) + (32u * tint_symbol_4)) + (8u * tint_symbol_5)) + (4u * tint_symbol_7))) / 4;
