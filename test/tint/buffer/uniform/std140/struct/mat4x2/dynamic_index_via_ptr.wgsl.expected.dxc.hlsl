...
void f() {
  ...
  const int p_a_i_a_save = ptr_index_save;
  const int ptr_index_save_1 = i();
  const int p_a_i_a_i_save = ptr_index_save;
  const int p_a_i_a_i_save_1 = ptr_index_save_1;
  const int p_a_i_a_i_m_save = ptr_index_save;
  const int p_a_i_a_i_m_save_1 = ptr_index_save_1;
  const int ptr_index_save_2 = i();
  const int p_a_i_a_i_m_i_save = ptr_index_save;
  const int p_a_i_a_i_m_i_save_1 = ptr_index_save_1;
  const int p_a_i_a_i_m_i_save_2 = ptr_index_save_2;
  const Outer l_a[4] = tint_symbol_4(a, 0u);
  const Outer l_a_i = tint_symbol_5(a, (128u * uint(ptr_index_save)));
  const Inner l_a_i_a[4] = tint_symbol_6(a, (128u * uint(ptr_index_save)));
  const Inner l_a_i_a_i = tint_symbol_7(a, ((128u * uint(ptr_index_save)) + (32u * uint(ptr_index_save_1))));
  const float4x2 l_a_i_a_i_m = tint_symbol_8(a, ((128u * uint(ptr_index_save)) + (32u * uint(ptr_index_save_1))));
  const uint scalar_offset_4 = ((((128u * uint(ptr_index_save)) + (32u * uint(ptr_index_save_1))) + (8u * uint(ptr_index_save_2)))) / 4;
  uint4 ubo_load_4 = a[scalar_offset_4 / 4];
  const float2 l_a_i_a_i_m_i = asfloat(((scalar_offset_4 & 2) ? ubo_load_4.zw : ubo_load_4.xy));
  const int tint_symbol = ptr_index_save;
  const int tint_symbol_1 = ptr_index_save_1;
  const int tint_symbol_2 = ptr_index_save_2;
