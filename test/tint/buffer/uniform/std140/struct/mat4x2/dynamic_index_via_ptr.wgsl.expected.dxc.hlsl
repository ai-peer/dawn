...
void f() {
  ...
  const uint scalar_offset_bytes_4 = ((((128u * uint(p_a_i_save)) + (32u * uint(p_a_i_a_i_save))) + (8u * uint(p_a_i_a_i_m_i_save))));
  const uint scalar_offset_index_4 = scalar_offset_bytes_4 / 4;
  uint4 ubo_load_4 = a[scalar_offset_index_4 / 4];
  const float2 l_a_i_a_i_m_i = asfloat(((scalar_offset_index_4 & 2) ? ubo_load_4.zw : ubo_load_4.xy));
  ...
  const uint scalar_offset_bytes_5 = (((((128u * uint(tint_symbol)) + (32u * uint(tint_symbol_1))) + (8u * uint(tint_symbol_2))) + (4u * uint(tint_symbol_3))));
