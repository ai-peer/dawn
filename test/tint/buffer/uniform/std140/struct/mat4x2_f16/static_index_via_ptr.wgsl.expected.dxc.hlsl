...
matrix<float16_t, 4, 2> tint_symbol_4(uint offset) {
...
Inner tint_symbol_3(uint offset) {
  const Inner tint_symbol_7 = {tint_symbol_4((offset + 0u))};
...
tint_symbol_2_ret tint_symbol_2(uint offset) {
  Inner arr[4] = (Inner[4])0;
  {
    for(uint i = 0u; (i < 4u); i = (i + 1u)) {
      arr[i] = tint_symbol_3((offset + (i * 64u)));
    }
  }
  return arr;
}

Outer tint_symbol_1(uint offset) {
  const Outer tint_symbol_8 = {tint_symbol_2((offset + 0u))};
...
tint_symbol_ret tint_symbol(uint offset) {
  Outer arr_1[4] = (Outer[4])0;
  {
    for(uint i_1 = 0u; (i_1 < 4u); i_1 = (i_1 + 1u)) {
      arr_1[i_1] = tint_symbol_1((offset + (i_1 * 256u)));
    }
  }
  return arr_1;
}
...
[numthreads(1, 1, 1)]
void f() {
  const Outer l_a[4] = tint_symbol(0u);
  const Outer l_a_3 = tint_symbol_1(768u);
  const Inner l_a_3_a[4] = tint_symbol_2(768u);
  const Inner l_a_3_a_2 = tint_symbol_3(896u);
  const matrix<float16_t, 4, 2> l_a_3_a_2_m = tint_symbol_4(896u);
