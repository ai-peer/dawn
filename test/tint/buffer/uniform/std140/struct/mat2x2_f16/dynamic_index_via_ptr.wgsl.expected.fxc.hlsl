...
void f() {
  ...
  const uint scalar_offset_bytes_2 = ((((256u * uint(p_a_i_save)) + (64u * uint(p_a_i_a_i_save))) + (4u * uint(p_a_i_a_i_m_i_save))));
  const uint scalar_offset_index_2 = scalar_offset_bytes_2 / 4;
  uint ubo_load_2 = a[scalar_offset_index_2 / 4][scalar_offset_index_2 % 4];
  ...
  const uint scalar_offset_bytes_3 = (((((256u * uint(tint_symbol)) + (64u * uint(tint_symbol_1))) + (4u * uint(tint_symbol_2))) + (2u * uint(tint_symbol_3))));
  const uint scalar_offset_index_3 = scalar_offset_bytes_3 / 4;
  const float16_t l_a_i_a_i_m_i_i = float16_t(f16tof32(((a[scalar_offset_index_3 / 4][scalar_offset_index_3 % 4] >> (scalar_offset_bytes_3 % 4 == 0 ? 0 : 16)) & 0xFFFF)));
  return;
}
FXC validation failure:
D:\Projects\RampUp\dawn\test\tint\buffer\Shader@0x000001D1FD25A080(2,10-18): error X3000: syntax error: unexpected token 'float16_t'
