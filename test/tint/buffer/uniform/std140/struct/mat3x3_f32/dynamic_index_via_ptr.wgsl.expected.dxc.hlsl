...
void f() {
  ...
  const uint scalar_offset_bytes_3 = ((((256u * uint(p_a_i_save)) + (64u * uint(p_a_i_a_i_save))) + (16u * uint(p_a_i_a_i_m_i_save))));
  const uint scalar_offset_index_3 = scalar_offset_bytes_3 / 4;
  const float3 l_a_i_a_i_m_i = asfloat(a[scalar_offset_index_3 / 4].xyz);
  ...
  const uint scalar_offset_bytes_4 = (((((256u * uint(tint_symbol)) + (64u * uint(tint_symbol_1))) + (16u * uint(tint_symbol_2))) + (4u * uint(tint_symbol_3))));
  const uint scalar_offset_index_4 = scalar_offset_bytes_4 / 4;
  const float l_a_i_a_i_m_i_i = asfloat(a[scalar_offset_index_4 / 4][scalar_offset_index_4 % 4]);
  return;
}
