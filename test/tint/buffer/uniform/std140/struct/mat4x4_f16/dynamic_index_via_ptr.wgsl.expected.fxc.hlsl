...
void f() {
  ...
  const uint scalar_offset_bytes_4 = ((((256u * uint(p_a_i_save)) + (64u * uint(p_a_i_a_i_save))) + (8u * uint(p_a_i_a_i_m_i_save))));
  const uint scalar_offset_index_4 = scalar_offset_bytes_4 / 4;
  uint4 ubo_load_9 = a[scalar_offset_index_4 / 4];
  uint2 ubo_load_8 = ((scalar_offset_index_4 & 2) ? ubo_load_9.zw : ubo_load_9.xy);
  ...
  const uint scalar_offset_bytes_5 = (((((256u * uint(tint_symbol)) + (64u * uint(tint_symbol_1))) + (8u * uint(tint_symbol_2))) + (2u * uint(tint_symbol_3))));
  const uint scalar_offset_index_5 = scalar_offset_bytes_5 / 4;
  const float16_t l_a_i_a_i_m_i_i = float16_t(f16tof32(((a[scalar_offset_index_5 / 4][scalar_offset_index_5 % 4] >> (scalar_offset_bytes_5 % 4 == 0 ? 0 : 16)) & 0xFFFF)));
  return;
}
FXC validation failure:
D:\Projects\RampUp\dawn\test\tint\buffer\Shader@0x000001D9139A49A0(2,10-18): error X3000: syntax error: unexpected token 'float16_t'
