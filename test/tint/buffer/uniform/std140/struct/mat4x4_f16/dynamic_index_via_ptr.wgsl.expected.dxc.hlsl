...
matrix<float16_t, 4, 4> tint_symbol_8(uint offset) {
...
Inner tint_symbol_7(uint offset) {
  const Inner tint_symbol_11 = {tint_symbol_8((offset + 0u))};
  return tint_symbol_11;
}

typedef Inner tint_symbol_6_ret[4];
tint_symbol_6_ret tint_symbol_6(uint offset) {
  Inner arr[4] = (Inner[4])0;
  {
    for(uint i_1 = 0u; (i_1 < 4u); i_1 = (i_1 + 1u)) {
      arr[i_1] = tint_symbol_7((offset + (i_1 * 64u)));
    }
  }
  return arr;
}

Outer tint_symbol_5(uint offset) {
  const Outer tint_symbol_12 = {tint_symbol_6((offset + 0u))};
...
tint_symbol_4_ret tint_symbol_4(uint offset) {
  Outer arr_1[4] = (Outer[4])0;
  {
    for(uint i_2 = 0u; (i_2 < 4u); i_2 = (i_2 + 1u)) {
      arr_1[i_2] = tint_symbol_5((offset + (i_2 * 256u)));
    }
  }
  return arr_1;
}
...
void f() {
  ...
  const Outer l_a[4] = tint_symbol_4(0u);
  const Outer l_a_i = tint_symbol_5((256u * uint(p_a_i_save)));
  const Inner l_a_i_a[4] = tint_symbol_6((256u * uint(p_a_i_save)));
  const Inner l_a_i_a_i = tint_symbol_7(((256u * uint(p_a_i_save)) + (64u * uint(p_a_i_a_i_save))));
  const matrix<float16_t, 4, 4> l_a_i_a_i_m = tint_symbol_8(((256u * uint(p_a_i_save)) + (64u * uint(p_a_i_a_i_save))));
