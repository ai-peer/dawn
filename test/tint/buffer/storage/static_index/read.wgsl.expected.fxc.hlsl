...
float4x3 tint_symbol_19(uint offset) {
...
float4x4 tint_symbol_20(uint offset) {
...
tint_symbol_21_ret tint_symbol_21(uint offset) {
...
Inner tint_symbol_22(uint offset) {
  const Inner tint_symbol_24 = {asint(sb.Load((offset + 0u))), asfloat(sb.Load((offset + 4u)))};
  return tint_symbol_24;
}

typedef Inner tint_symbol_23_ret[4];
tint_symbol_23_ret tint_symbol_23(uint offset) {
  Inner arr_1[4] = (Inner[4])0;
  {
    for(uint i_1 = 0u; (i_1 < 4u); i_1 = (i_1 + 1u)) {
      arr_1[i_1] = tint_symbol_22((offset + (i_1 * 8u)));
    }
  }
  return arr_1;
}
...
void main() {
  ...
  const float2x2 mat2x2_f32 = tint_symbol_12(144u);
  const float2x3 mat2x3_f32 = tint_symbol_13(160u);
  const float2x4 mat2x4_f32 = tint_symbol_14(192u);
  const float3x2 mat3x2_f32 = tint_symbol_15(224u);
  const float3x3 mat3x3_f32 = tint_symbol_16(256u);
  const float3x4 mat3x4_f32 = tint_symbol_17(304u);
  const float4x2 mat4x2_f32 = tint_symbol_18(352u);
  const float4x3 mat4x3_f32 = tint_symbol_19(384u);
  const float4x4 mat4x4_f32 = tint_symbol_20(448u);
  const float3 arr2_vec3_f32[2] = tint_symbol_21(512u);
  const Inner struct_inner = tint_symbol_22(544u);
  const Inner array_struct_inner[4] = tint_symbol_23(552u);
  return;
}
