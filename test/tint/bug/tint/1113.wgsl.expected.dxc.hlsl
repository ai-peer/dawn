...
void main_count_inner(uint3 GlobalInvocationID) {
  ...
  uint acefg = tint_atomicAdd((4u * voxelIndex), 1u);
  if ((triangleIndex == 0u)) {
...
uint tint_atomicAdd_1(uint offset, uint value) {
  uint original_value = 0;
...
void main_create_lut_inner(uint3 GlobalInvocationID) {
  ...
  uint numTriangles = tint_atomicLoad((4u * voxelIndex));
  int offset = -1;
  if ((numTriangles > 0u)) {
    const uint tint_symbol_6 = tint_atomicAdd_1(0u, numTriangles);
    offset = int(tint_symbol_6);
  }
  tint_atomicStore((4u * voxelIndex), offset);
}
...
int tint_atomicAdd_2(uint offset, int value) {
...
void main_sort_triangles_inner(uint3 GlobalInvocationID) {
  ...
  float3 voxelPos = toVoxelPos(center);
  uint voxelIndex = toIndex1D(uniforms[0].y, voxelPos);
  int triangleOffset = tint_atomicAdd_2((4u * voxelIndex), 1);
}
