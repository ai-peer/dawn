...
float4 textureLoadExternal(Texture2D<float4> plane0, Texture2D<float4> plane1, int2 coord, ExternalTextureParams params) {
  ...
  if ((params.numPlanes == 1u)) {
    ...
    color = plane0.Load(int3(clamp(coord, (0).xx, int2((tint_tmp_1.xy - (1u).xx))), int(level))).rgb;
  } else {
    int3 tint_tmp_2;
    ...
    color = mul(params.yuvToRgbConversionMatrix, float4(plane0.Load(int3(clamp(coord, (0).xx, int2((tint_tmp_4.xy - (1u).xx))), int(level_1))).r, plane1.Load(int3(clamp(coord1, (0).xx, int2((tint_tmp_5.xy - (1u).xx))), int(level_2))).rg, 1.0f));
  }
...
void main() {
  ...
  outImage[clamp((0).xx, (0).xx, int2((tint_tmp_6 - (1u).xx)))] = red;
  ...
  outImage[clamp(int2(1, 0), (0).xx, int2((tint_tmp_7 - (1u).xx)))] = green;
  return;
}
