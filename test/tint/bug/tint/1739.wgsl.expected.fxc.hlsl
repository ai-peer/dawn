...
float4 textureLoadExternal(Texture2D<float4> plane0, Texture2D<float4> plane1, int2 coord, ExternalTextureParams params) {
  ...
  if ((params.numPlanes == 1u)) {
    ...
    plane0.GetDimensions(clamp(0, 0, int((uint(tint_tmp_1.z) - 1u))), tint_tmp.x, tint_tmp.y, tint_tmp.z);
    int3 tint_tmp_2;
    plane0.GetDimensions(0, tint_tmp_2.x, tint_tmp_2.y, tint_tmp_2.z);
    color = plane0.Load(int3(clamp(coord, (0).xx, int2((uint2(tint_tmp.xy) - (1u).xx))), clamp(0, 0, int((uint(tint_tmp_2.z) - 1u))))).rgb;
  } else {
    int3 tint_tmp_3;
    int3 tint_tmp_4;
    plane0.GetDimensions(0, tint_tmp_4.x, tint_tmp_4.y, tint_tmp_4.z);
    plane0.GetDimensions(clamp(0, 0, int((uint(tint_tmp_4.z) - 1u))), tint_tmp_3.x, tint_tmp_3.y, tint_tmp_3.z);
    int3 tint_tmp_5;
    ...
    plane1.GetDimensions(clamp(0, 0, int((uint(tint_tmp_7.z) - 1u))), tint_tmp_6.x, tint_tmp_6.y, tint_tmp_6.z);
    int3 tint_tmp_8;
    plane1.GetDimensions(0, tint_tmp_8.x, tint_tmp_8.y, tint_tmp_8.z);
    color = mul(params.yuvToRgbConversionMatrix, float4(plane0.Load(int3(clamp(coord, (0).xx, int2((uint2(tint_tmp_3.xy) - (1u).xx))), clamp(0, 0, int((uint(tint_tmp_5.z) - 1u))))).r, plane1.Load(int3(clamp(coord1, (0).xx, int2((uint2(tint_tmp_6.xy) - (1u).xx))), clamp(0, 0, int((uint(tint_tmp_8.z) - 1u))))).rg, 1.0f));
  }
...
void main() {
  ...
  outImage[clamp((0).xx, (0).xx, int2((uint2(tint_tmp_9) - (1u).xx)))] = red;
  ...
  outImage[clamp(int2(1, 0), (0).xx, int2((uint2(tint_tmp_10) - (1u).xx)))] = green;
  return;
}
