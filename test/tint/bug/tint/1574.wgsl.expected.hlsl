...
void main() {
  uint value = 42u;
  const atomic_compare_exchange_weak_ret_type r1 = tint_atomicCompareExchangeWeak(a, 0u, 0u, value);
  const atomic_compare_exchange_weak_ret_type r2 = tint_atomicCompareExchangeWeak(a, 0u, 0u, value);
  const atomic_compare_exchange_weak_ret_type r3 = tint_atomicCompareExchangeWeak(a, 0u, 0u, value);
  return;
}
