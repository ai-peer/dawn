...
atomic_compare_exchange_weak_ret_type tint_atomicCompareExchangeWeak(uint offset, uint compare, uint value) {
...
void main() {
  uint value = 42u;
  const atomic_compare_exchange_weak_ret_type result = tint_atomicCompareExchangeWeak(0u, 0u, value);
  return;
}
