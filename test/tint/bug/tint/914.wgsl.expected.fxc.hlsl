...
void main_inner(uint3 local_id, uint3 global_id, uint local_invocation_index) {
  ...
  const uint tint_symbol = tint_div((uniforms[0].y - 1u), 64u);
  const uint numTiles = (tint_symbol + 1u);
        ...
        for(uint innerRow = 0u; (innerRow < 4u); innerRow = (innerRow + 1u)) {
          {
            for(uint innerCol = 0u; (innerCol < ColPerThreadA); innerCol = (innerCol + 1u)) {
              ...
              const float tint_symbol_1 = mm_readA((globalRow + innerRow), ((t * 64u) + inputCol));
        ...
        for(uint innerRow = 0u; (innerRow < RowPerThreadB); innerRow = (innerRow + 1u)) {
          {
            for(uint innerCol = 0u; (innerCol < 4u); innerCol = (innerCol + 1u)) {
              ...
              const float tint_symbol_2 = mm_readB(((t * 64u) + inputRow), (globalCol + innerCol));
...
[numthreads(16, 16, 1)]
void main(tint_symbol_4 tint_symbol_3) {
  main_inner(tint_symbol_3.local_id, tint_symbol_3.global_id, tint_symbol_3.local_invocation_index);
  return;
}
