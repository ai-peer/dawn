...
strided_arr tint_symbol_2(uint offset) {
  const strided_arr tint_symbol_10 = {asfloat(ssbo.Load2((offset + 0u)))};
  return tint_symbol_10;
}

typedef strided_arr tint_symbol_1_ret[2];
tint_symbol_1_ret tint_symbol_1(uint offset) {
  strided_arr arr_1[2] = (strided_arr[2])0;
  {
    for(uint i = 0u; (i < 2u); i = (i + 1u)) {
      arr_1[i] = tint_symbol_2((offset + (i * 16u)));
    }
  }
  return arr_1;
}

void tint_symbol_5(uint offset, strided_arr value) {
  ssbo.Store2((offset + 0u), asuint(value.el));
}

void tint_symbol_4(uint offset, strided_arr value[2]) {
  strided_arr array_1[2] = value;
  {
    for(uint i_1 = 0u; (i_1 < 2u); i_1 = (i_1 + 1u)) {
      tint_symbol_5((offset + (i_1 * 16u)), array_1[i_1]);
    }
  }
}

void f_1() {
  const float2x2 x_15 = arr_to_mat2x2_stride_16(tint_symbol_1(0u));
  const strided_arr tint_symbol[2] = mat2x2_stride_16_to_arr(x_15);
  tint_symbol_4(0u, tint_symbol);
  return;
}
