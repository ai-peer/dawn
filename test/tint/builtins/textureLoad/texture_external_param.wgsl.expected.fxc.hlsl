...
float4 textureLoadExternal(Texture2D<float4> plane0, Texture2D<float4> plane1, int2 coord, ExternalTextureParams params) {
  float2 modifiedCoords = mul(float3(float2(coord), 1.0f), params.loadTransformMatrix);
  uint2 clampedCoord0 = tint_clamp(tint_ftou(modifiedCoords), params.minVisibleCoord, params.maxVisibleCoord);
