...
struct ExternalTextureParams {
  ...
  float2 plane0VisibleRectMin;
  float2 plane0VisibleRectMax;
  float2 plane1VisibleRectMin;
  float2 plane1VisibleRectMax;
  uint2 plane0Size;
  uint2 plane1Size;
};
...
float4 textureLoadExternal(Texture2D<float4> plane0, Texture2D<float4> plane1, int2 coord, ExternalTextureParams params) {
  ...
  uint2 plane0_clamped = tint_clamp(tint_ftou(modifiedCoords), tint_ftou((params.plane0VisibleRectMin * float2(params.plane0Size))), tint_ftou((params.plane0VisibleRectMax * float2(params.plane0Size))));
  uint2 coord1 = (plane0_clamped >> (1u).xx);
  uint2 plane1_clamped = tint_clamp(uint2(coord1), tint_ftou((params.plane1VisibleRectMin * float2(params.plane1Size))), tint_ftou((params.plane1VisibleRectMax * float2(params.plane1Size))));
