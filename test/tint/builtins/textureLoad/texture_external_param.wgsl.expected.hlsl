...
float4 textureLoadExternal(Texture2D<float4> plane0, Texture2D<float4> plane1, int2 coord, ExternalTextureParams params) {
  ...
  const float tint_symbol_2 = gammaCorrection(color.r, params.gammaDecodeParams);
  color.r = tint_symbol_2;
  const float tint_symbol_3 = gammaCorrection(color.g, params.gammaDecodeParams);
  color.g = tint_symbol_3;
  const float tint_symbol_4 = gammaCorrection(color.b, params.gammaDecodeParams);
  color.b = tint_symbol_4;
  color = mul(color, params.gamutConversionMatrix);
  const float tint_symbol_5 = gammaCorrection(color.r, params.gammaEncodeParams);
  color.r = tint_symbol_5;
  const float tint_symbol_6 = gammaCorrection(color.g, params.gammaEncodeParams);
  color.g = tint_symbol_6;
  const float tint_symbol_7 = gammaCorrection(color.b, params.gammaEncodeParams);
  color.b = tint_symbol_7;
...
float3x4 tint_symbol_10(uint4 buffer[11], uint offset) {
...
GammaTransferParams tint_symbol_12(uint4 buffer[11], uint offset) {
...
float3x3 tint_symbol_14(uint4 buffer[11], uint offset) {
...
ExternalTextureParams tint_symbol_8(uint4 buffer[11], uint offset) {
  const uint scalar_offset_14 = ((offset + 0u)) / 4;
  const ExternalTextureParams tint_symbol_17 = {buffer[scalar_offset_14 / 4][scalar_offset_14 % 4], tint_symbol_10(buffer, (offset + 16u)), tint_symbol_12(buffer, (offset + 64u)), tint_symbol_12(buffer, (offset + 96u)), tint_symbol_14(buffer, (offset + 128u))};
  return tint_symbol_17;
}

void doTextureLoad() {
  float4 res = textureLoad2d(arg_0, ext_tex_plane_1, tint_symbol_8(ext_tex_params, 0u), int2(0, 0));
}
