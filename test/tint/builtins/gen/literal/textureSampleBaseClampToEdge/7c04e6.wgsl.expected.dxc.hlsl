...
float4 textureSampleExternal(Texture2D<float4> plane0, Texture2D<float4> plane1, SamplerState smp, float2 coord, ExternalTextureParams params) {
  const float2 modifiedCoords = (mul(params.coordTransformationMatrix, float3((coord - 0.5f), 1.0f)) + abs(mul(params.coordTransformationMatrix, float3(0.5f, 0.5f, 0.0f))));
  int3 tint_tmp;
