...
float4 textureSampleExternal(Texture2D<float4> plane0, Texture2D<float4> plane1, SamplerState smp, float2 coord, ExternalTextureParams params) {
  float2 modifiedCoords = ((params.visibleRectSize * coord) + params.visibleRectOffset);
  modifiedCoords = (mul(params.rotationMatrix, (modifiedCoords - 0.5f)) + 0.5f);
  int3 tint_tmp;
