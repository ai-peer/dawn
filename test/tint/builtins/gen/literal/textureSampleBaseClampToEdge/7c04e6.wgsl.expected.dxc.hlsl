...
float4 textureSampleExternal(Texture2D<float4> plane0, Texture2D<float4> plane1, SamplerState smp, float2 coord, ExternalTextureParams params) {
  ...
  float2 plane0_clamped = clamp(modifiedCoords, params.plane0VisibleRectMin, params.plane0VisibleRectMax);
  float2 plane1_clamped = clamp(modifiedCoords, params.plane1VisibleRectMin, params.plane1VisibleRectMax);
