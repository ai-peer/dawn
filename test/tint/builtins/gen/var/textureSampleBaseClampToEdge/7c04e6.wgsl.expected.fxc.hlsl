...
float4 textureSampleExternal(Texture2D<float4> plane0, Texture2D<float4> plane1, SamplerState smp, float2 coord, ExternalTextureParams params) {
  ...
  if ((params.doYuvToRgbConversionOnly == 0u)) {
    color = float4(gammaCorrection(color.rgb, params.gammaDecodeParams), 1.0f);
    color = float4(mul(color.rgb, params.gamutConversionMatrix), 1.0f);
    color = float4(gammaCorrection(color.rgb, params.gammaEncodeParams), 1.0f);
  }
  return color;
}
