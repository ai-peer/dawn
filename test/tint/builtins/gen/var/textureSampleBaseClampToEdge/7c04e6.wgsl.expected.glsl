...
vec4 textureSampleExternal(highp sampler2D plane0_1, highp sampler2D plane1_1, highp sampler2D plane0_smp, highp sampler2D plane1_smp, vec2 coord, ExternalTextureParams params) {
  ...
  if ((params.doYuvToRgbConversionOnly == 0u)) {
    color = vec4(gammaCorrection(color.rgb, params.gammaDecodeParams), 1.0f);
    color = vec4((params.gamutConversionMatrix * color.rgb), 1.0f);
    color = vec4(gammaCorrection(color.rgb, params.gammaEncodeParams), 1.0f);
  }
  return color;
}
