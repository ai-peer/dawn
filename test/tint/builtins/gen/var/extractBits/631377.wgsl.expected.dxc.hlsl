uint4 tint_extract_bits(uint4 v, uint offset, uint count) {
  ...
  const uint4 shl_result = ((shl < 32u) ? (v << uint4((shl).xxxx)) : uint4(0u, 0u, 0u, 0u));
  return ((shr < 32u) ? (shl_result >> uint4((shr).xxxx)) : ((shl_result >> uint4(31u, 31u, 31u, 31u)) >> uint4(1u, 1u, 1u, 1u)));
}
