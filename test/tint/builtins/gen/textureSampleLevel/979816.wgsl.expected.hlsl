...
float4 textureSampleExternal(Texture2D<float4> plane0, Texture2D<float4> plane1, SamplerState smp, float2 coord, ExternalTextureParams params) {
  ...
  const float tint_symbol_1 = gammaCorrection(color.r, params.gammaDecodeParams);
  color.r = tint_symbol_1;
  const float tint_symbol_2 = gammaCorrection(color.g, params.gammaDecodeParams);
  color.g = tint_symbol_2;
  const float tint_symbol_3 = gammaCorrection(color.b, params.gammaDecodeParams);
  color.b = tint_symbol_3;
  color = mul(color, params.gamutConversionMatrix);
  const float tint_symbol_4 = gammaCorrection(color.r, params.gammaEncodeParams);
  color.r = tint_symbol_4;
  const float tint_symbol_5 = gammaCorrection(color.g, params.gammaEncodeParams);
  color.g = tint_symbol_5;
  const float tint_symbol_6 = gammaCorrection(color.b, params.gammaEncodeParams);
  color.b = tint_symbol_6;
  return float4(color, 1.0f);
}

float3x4 tint_symbol_9(uint4 buffer[11], uint offset) {
...
GammaTransferParams tint_symbol_11(uint4 buffer[11], uint offset) {
...
ExternalTextureParams tint_symbol_7(uint4 buffer[11], uint offset) {
  const uint scalar_offset_14 = ((offset + 0u)) / 4;
  const ExternalTextureParams tint_symbol_16 = {buffer[scalar_offset_14 / 4][scalar_offset_14 % 4], tint_symbol_9(buffer, (offset + 16u)), tint_symbol_11(buffer, (offset + 64u)), tint_symbol_11(buffer, (offset + 96u)), tint_symbol_13(buffer, (offset + 128u))};
  return tint_symbol_16;
}

void textureSampleLevel_979816() {
  float4 res = textureSampleExternal(arg_0, ext_tex_plane_1, arg_1, float2(0.0f, 0.0f), tint_symbol_7(ext_tex_params, 0u));
}
