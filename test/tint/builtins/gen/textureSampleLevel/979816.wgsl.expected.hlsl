...
GammaTransferParams tint_symbol_5(uint4 buffer[12], uint offset) {
  ...
  const GammaTransferParams tint_symbol_9 = {asfloat(buffer[scalar_offset_3 / 4][scalar_offset_3 % 4]), asfloat(buffer[scalar_offset_4 / 4][scalar_offset_4 % 4]), asfloat(buffer[scalar_offset_5 / 4][scalar_offset_5 % 4]), asfloat(buffer[scalar_offset_6 / 4][scalar_offset_6 % 4]), asfloat(buffer[scalar_offset_7 / 4][scalar_offset_7 % 4]), asfloat(buffer[scalar_offset_8 / 4][scalar_offset_8 % 4]), asfloat(buffer[scalar_offset_9 / 4][scalar_offset_9 % 4])};
  return tint_symbol_9;
}

float3x3 tint_symbol_7(uint4 buffer[12], uint offset) {
  const uint scalar_offset_10 = ((offset + 0u)) / 4;
  const uint scalar_offset_11 = ((offset + 16u)) / 4;
  const uint scalar_offset_12 = ((offset + 32u)) / 4;
  return float3x3(asfloat(buffer[scalar_offset_10 / 4].xyz), asfloat(buffer[scalar_offset_11 / 4].xyz), asfloat(buffer[scalar_offset_12 / 4].xyz));
}

ExternalTextureParams tint_symbol_1(uint4 buffer[12], uint offset) {
  const uint scalar_offset_13 = ((offset + 0u)) / 4;
  const ExternalTextureParams tint_symbol_10 = {buffer[scalar_offset_13 / 4][scalar_offset_13 % 4], tint_symbol_3(buffer, (offset + 16u)), tint_symbol_5(buffer, (offset + 64u)), tint_symbol_5(buffer, (offset + 96u)), tint_symbol_7(buffer, (offset + 128u))};
