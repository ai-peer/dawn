...
layout(binding = 0, std140) uniform src_uniform_block_0_0_ubo {
  S inner;
} src_uniform;

layout(binding = 1, std430) buffer src_uniform_block_0_0_ssbo {
  S inner;
} src_storage;

layout(binding = 2, std430) buffer src_uniform_block_0_0_ssbo_1 {
  S inner;
} dst;

layout(binding = 3, std430) buffer dst_nested_block_0_3_ssbo {
  S_nested inner;
} dst_nested;
