...
struct src_uniform_block {
  S inner;
};

layout(binding = 0) uniform src_uniform_block_1 {
  S inner;
} src_uniform;

layout(binding = 1, std430) buffer src_uniform_block_2 {
  S inner;
} src_storage;
layout(binding = 2, std430) buffer src_uniform_block_3 {
  S inner;
} dst;
