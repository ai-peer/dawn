...
struct DAWN_NATIVE_EXPORT ExternalImageAccessDescriptorDXGISharedHandle
  ...
  public:
    // Value used for fence wait. A value of 0 is valid, but essentially a no-op
    // since the fence lifetime starts with the 0 value signaled.
    uint64_t fenceWaitValue = 0;

    // Value to signal the fence with after the texture is destroyed. A value
    // of 0 means the fence will not be signaled.
