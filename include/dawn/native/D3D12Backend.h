...
struct DAWN_NATIVE_EXPORT ExternalImageDXGIEndAccessDescriptor {
    // Value to signal the fence with after the texture is destroyed. A default value of UINT64_MAX
    // is chosen instead of 0 so that failing to specify the signal value causes future waits to
    // block indefinitely making the bug apparent instead of dropping the wait silently.
    uint64_t fenceSignalValue = UINT64_MAX;
};
