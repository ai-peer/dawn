...
namespace dawn_wire {
    namespace server {
        {% for type in by_category["object"] if type.is_builder%}
            void WireServer::Forward{{type.name.CamelCase()}}ToClient(dawnBuilderErrorStatus status, const char* message, dawnCallbackUserdata userdata1, dawnCallbackUserdata userdata2) {
                auto server = reinterpret_cast<WireServer*>(static_cast<uintptr_t>(userdata1));
        ...
        {% for type in by_category["object"] if type.is_builder%}
            ...
            void WireServer::On{{Type}}Error(dawnBuilderErrorStatus status, const char* message, uint32_t id, uint32_t serial) {
                ...
                if (status != DAWN_BUILDER_ERROR_STATUS_UNKNOWN) {
                    ...
                    cmd.builtObject = ObjectHandle{builder->builtObjectId, builder->builtObjectSerial};
                    cmd.status = status;
                    cmd.message = message;
