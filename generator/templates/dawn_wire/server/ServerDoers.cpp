            ...
            {% for type in by_category["object"] %}
                case ObjectType::{{type.name.CamelCase()}}: {
                    ...
                    if (data->device != nullptr) {
                        ...
                        auto it = device->childObjectTypesAndIds.find(PackObjectTypeAndId(objectType, objectId));
                        if (it == device->childObjectTypesAndIds.end()) {
                            return false;
                        }
                        device->childObjectTypesAndIds.erase(it);
                    }
