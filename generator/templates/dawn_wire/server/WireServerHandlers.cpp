        ...
        {% for command in cmd_records["command"] %}
            ...
            {% if Suffix not in client_side_commands %}
                ...
                bool WireServer::Handle{{Suffix}}(const char** commands, size_t* size) {
                    ...
                    {%- for member in command.members if member.is_return_value and member.target_type -%}
                        {% set Type = member.target_type.name.CamelCase() %}
                    ...
                    {%- for member in command.members if member.is_return_value and member.target_type -%}
                        ...
                        {% if member.target_type.is_builder %}
                            if ({{name}}Data->valid) {
                                ...
                                mProcs.{{as_varName(member.target_type.name, Name("set error callback"))}}({{name}}Data->handle, Forward{{Type}}ToClient, userdata1, userdata2);
                            }
                        {% endif %}
                    {% endfor %}
