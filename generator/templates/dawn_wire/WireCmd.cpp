...
{% macro write_serialization_methods(record, name, members, is_cmd=False, is_method=False, is_return_command=False) %}
...
{% macro write_serialization_definitions(command, is_return) %}
...
#define DESERIALIZE_TRY(EXPR) \
    ...
    namespace {
        ...
        {% for type in by_category["structure"] %}
            ...
            {{write_serialization_methods(type, name, type.members,
              is_cmd=False)}}
        {% endfor %}
        ...
        {% for command in cmd_records["command"] %}
            ...
            {{write_serialization_methods(command, name, command.members,
    ...
    {% for command in cmd_records["command"] %}
        {{ write_serialization_definitions(command, False) }}
    {% endfor %}

    {% for command in cmd_records["return command"] %}
        {{ write_serialization_definitions(command, True) }}
    {% endfor %}
