...
#define DESERIALIZE_TRY(EXPR) \
    ...
    DeserializeResult DeserializeBuffer::ReadN(N count, const volatile T** data) {
        ...
        *data = reinterpret_cast<const volatile T*>(mBuffer);
        mBuffer += totalSize;
        mSize -= totalSize;
    ...
    template <typename T>
    DeserializeResult DeserializeBuffer::Read(const volatile T** data) {
        if (sizeof(T) > mSize) {
            return DeserializeResult::FatalError;
        }
    ...
    template <typename T>
    DeserializeResult DeserializeBuffer::Peek(const volatile T** data) {
        if (sizeof(T) > mSize) {
            return DeserializeResult::FatalError;
        }
    ...
    namespace {
        ...
        // AssignImpl where there is no narrowing. We can assign without checking.
        template <typename Out, typename In>
        struct AssignImpl<Out, In, false /* MayNarrow */> {
            static DeserializeResult Call(Out* out, const In& in)  {
                *out = in;
                return DeserializeResult::Success;
            }
        ...
        template <typename Out, typename In>
        struct AssignImpl<Out, In, true /* MayNarrow */> {
            static DeserializeResult Call(Out* out, const In& in) {
