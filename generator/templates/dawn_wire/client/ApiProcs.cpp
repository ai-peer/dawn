    ...
    namespace {
        ...
        {% for type in by_category["object"] %}
            DAWN_DECLARE_UNUSED bool ClientMatches(const Client* client, const {{as_cType(type.name)}} obj) {
                return client == reinterpret_cast<const {{as_wireType(type)}}>(obj)->GetClient();
            }
                ...
                {% if len(method.arguments) > 0 %}
                    {
                        ...
                        if (DAWN_UNLIKELY(!sameClient)) {
                            ...
                            {% if method.return_type.category == "object" %}
                                ...
                                auto self = reinterpret_cast<{{as_wireType(type)}}>(cSelf);
                                auto* allocation = self->GetClient()->{{method.return_type.name.CamelCase()}}Allocator().New(self->GetClient());
