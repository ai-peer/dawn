...
namespace dawn_wire {
    ...
    namespace client {
        ...
        void ProxyClientQueueWait(dawnQueue cQueue, dawnFence cFence, uint64_t value) {
            Fence* fence = reinterpret_cast<Fence*>(cFence);
            if (value > fence->signaledValue) {
                fence->device->HandleError("Value greater than fence signaled value");
                return;
            }
            ClientQueueWait(cQueue, cFence, value);
        }
        ...
        {% set proxied_commands = ["BufferUnmap", "DeviceCreateFence", "QueueSignal", "QueueWait"] %}
