...
{% macro convert_object(member, in, out, in_access) -%}
    {{ out }} = reinterpret_cast<{{ as_cType(member.type.name)  }}>(
        gObjectStores[ObjectType::{{ member.type.name.CamelCase() }}].Lookup(
...
{% macro convert_objectid(member, in, out, in_access) -%}
    {{ out }} = gObjectStores[ObjectType::{{ member.id_type.name.CamelCase() }}].Lookup(
        static_cast<ObjectId>(
            {{in}}({{ in_access}})
        )
    );
...
{% macro convert_objecthandle(member, in, out, in_access) -%}
    if (gObjectStores[ObjectType::ComputePassEncoder].Size() < DawnLPMFuzzer::k{{ member.handle_type.name.CamelCase() }}Limit) {
        {{ out }} = gObjectStores[ObjectType::{{ member.handle_type.name.CamelCase() }}].ReserveHandle();
    } else {
        {{ out }} = {0, 0};
    }
{%- endmacro %}
...
{% macro convert_structure(member, in, out, in_access) -%}
    ...
    WIRE_TRY({{member_type(member)}}ProtoConvert({{in}}({{in_access}}), &{{out}}, serializeBuffer, gObjectStores));
{%- endmacro %}
...
{% macro write_record_conversion_helpers(record, name, members, is_cmd) %}
    ...
    WireResult {{WGPU}}{{name}}ProtoConvert(fuzzing::{{ name }} proto_record, {{WGPU}}{{ name }}{{ Cmd }} const *record, SerializeBuffer* serializeBuffer, PerObjectType<DawnLPMObjectStore> gObjectStores) {
        ...
        //* Pass by Value handling. Strucute mirroring WireCmd with some differences between
        ...
        //* Pass by Pointer handling. Strucute mirroring WireCmd with some divergences.
        {% for member in members if member.annotation != "value" and member.length != "strlen" and not member.skip_serialize %}
            ...
            //* Major WireCmd Divergence: We handle raw bytes uniquely
            //* as they don't lead to new coverage, lead to OOMs
            //* and are not easy to work with in protobuf.
...
//* Output command conversion first because it is used by commands.
...
WireResult SerializedData(const fuzzing::Program& program, dawn::wire::ChunkedCommandSerializer serializer) {
    ...
    PerObjectType<DawnLPMObjectStore> gObjectStores;
    ...
    for (const fuzzing::Command& command : program.commands()) {
        switch (command.command_case()) {
            ...
            case fuzzing::Command::k{{name}}: {
                ...
                WIRE_TRY({{name}}ProtoConvert(command.{{ command.name.concatcase() }}(), cmd, &serializeBuffer, gObjectStores));
