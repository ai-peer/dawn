 ...
 *
 * emit_ prefixed functions are helpers that are building blocks within the
...
{% macro lift_dawn_member_pass_by_pointer(var, access, command, member_access_pairs) -%}
    {% set member = member_access_pairs[-1][0] %}
...
{% macro proto_to_wire_helper(var, access, command, member_access_pairs) -%}
    ...
    {% elif member.annotation == 'const*' %}
        {{ lift_dawn_member_pass_by_pointer(var, access, command, member_access_pairs) }}
    {% else %}
        // If we are missing any new WebGPU types then we'd like to know at compile time
        // as it will likely cause uninteresting segfaults while fuzzing.
        {{ unreachable_code() }}
