...
{% macro write_record_serialization_helpers(record, name, members, is_cmd=False, is_return_command=False) %}
    ...
    DAWN_DECLARE_UNUSED WireResult {{Return}}{{name}}Deserialize(
        ...
        {%- set initialized_members = [] -%}
        {%- for member in members if member.skip_serialize %}
            {% set memberName = as_varName(member.name) %}
            {% if member.annotation == "*" %}
                {% do initialized_members.append(member.name.get()) %}
        ...
        {% for member in members if member.annotation != "value" and member.length != "strlen" and member.name.get() not in initialized_members %}
            {{ assert(member.annotation != "const*const*") }}
            {% set memberName = as_varName(member.name) %}
