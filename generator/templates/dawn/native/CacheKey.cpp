...
namespace {{native_namespace}} {
    ...
    {% for type in by_category["enum"] %}
        {% set typename = as_cppType(type.name) %}
        void SerializeInto(CacheKey* key, const ::{{namespace}}::{{typename}}& value) {
            detail::SerializeIntoHelper(key, static_cast<typename std::underlying_type<::{{namespace}}::{{typename}}>::type>(value));
        }
    {% endfor %}
    ...
    {% for type in by_category["structure"] %}
        {% if type.cachekey %}
            void SerializeInto(CacheKey* key, const {{as_cppType(type.name)}}& value) {
                {% set members = [] %}
