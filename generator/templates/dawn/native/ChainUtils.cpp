...
#ifdef __GNUC__
// Disable the following error because the generated code for currently unextended structs will
// generate a loop that immediately just returns the validation error.
...
{% for type in by_category["structure"] %}
    {% if type.extensible == "in" %}
        ...
        ResultOrError<{{unpackedChain}}> ValidateAndUnpackChain(const {{as_cppType(type.name)}}* chain) {
            ...
            {% endfor %}
            for (const ChainedStruct* next = chain->nextInChain; next != nullptr; next = next->nextInChain) {
                switch (next->sType) {
                    {% for extension in type.extensions %}
