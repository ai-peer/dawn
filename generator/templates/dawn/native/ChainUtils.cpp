...
struct AdditionalExtensionUnpacker<Root, Unpacked, detail::AdditionalExtensionsList<Exts...>> {
    static bool Unpack(Unpacked& unpacked, const ChainedStruct* chain) {
        ...
        ResultOrError<{{unpackedChain}}> ValidateAndUnpackChain(const {{as_cppType(type.name)}}* chain) {
            ...
            for (; next != nullptr; next = next->nextInChain) {
                ...
                switch (next->sType) {
                    ...
                    {% endfor %}
                    default: {
                      if (!AdditionalExtensionUnpacker<
                          {{as_cppType(type.name)}},
                          {{unpackedChain}},
                          detail::AdditionalExtensions<{{as_cppType(type.name)}}>::List>::Unpack(result, next)) {
                          return DAWN_VALIDATION_ERROR(
