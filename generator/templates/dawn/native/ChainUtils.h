    ...
    {% for type in by_category["structure"] %}
        {% if type.extensible == "in" %}
            ...
            {% if len(type.extensions) != 0 %}
                //* We can only get the root type when we have more than 1 extension. Otherwise the
                //* aliasing all see the same std::tuple<> type and cannot resolve the specialization.
                template <>
