...
namespace dawn_native {
    ...
    template <typename T>
    MaybeError ValidateSingletonSTypeInner(const ChainedStruct* chain, T sType) {
    ...
    MaybeError ValidateSingletonSTypeInner(const ChainedStruct* chain, T sType, Args... sTypes) {
        if (chain->sType == sType) {
            return {};
        }
        return ValidateSingletonSTypeInner(chain, sTypes...);
    }

    // Verifies that |chain| contains a single ChainedStruct of type |sType| or no ChainedStructs
    // at all.
    template <typename T>
    MaybeError ValidateSingletonSType(const ChainedStruct* chain, T sType) {
        if (chain == nullptr) {
            return {};
        }
        ...
        return ValidateSingletonSTypeInner(chain, sType);
    }
    ...
    template <typename T, typename... Args>
    MaybeError ValidateSingletonSType(const ChainedStruct* chain, T sType, Args... sTypes) {
        if (chain == nullptr) {
            return {};
        }
        if (chain->nextInChain != nullptr) {
        ...
        return ValidateSingletonSTypeInner(chain, sType, sTypes...);
    }
