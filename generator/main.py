...
def linked_record_members(json_data, types):
    ...
    for m in json_data:
        member = RecordMember(Name(m['name']), types[m['type']],
                              m.get('annotation', 'value'), a.get('optional', False))
...
=======
            RecordMember(Name('callback'), types['builder error callback'], 'value'),
            RecordMember(Name('userdata1'), types['callback userdata'], 'value'),
            RecordMember(Name('userdata2'), types['callback userdata'], 'value'),
>>>>>>> 568c1d9... generator: Unify handling of argument and member parsing
        ]))
