...
from common import BitmaskType
from common import NativeType
from common import NativelyDefined
from common import RecordMember
from common import Method
from common import ObjectType
from common import StructureType
...
def cpp_native_methods(types, typ):
    ...
    if typ.is_builder:
        methods.append(Method(Name('set error callback'), types['void'], [
            RecordMember(Name('callback'), types['builder error callback'], 'value', False, False),
            RecordMember(Name('userdata1'), types['callback userdata'], 'value', False, False),
            RecordMember(Name('userdata2'), types['callback userdata'], 'value', False, False),
        ]))
...
def get_renders_for_targets(api_params, wire_json, targets):
    ...
    if 'dawn_wire' in targets:
        additional_params = wire_cmd.add_wire_commands(api_params, wire_json)
        ...
        renders.append(FileRender('dawn_wire/server/WireServerBase.h', 'dawn_wire/server/WireServerBase_autogen.h', wire_params))
        renders.append(FileRender('dawn_wire/server/WireServerCallbacks.cpp', 'dawn_wire/server/WireServerCallbacks_autogen.cpp', wire_params))
        renders.append(FileRender('dawn_wire/server/WireServerDoers.cpp', 'dawn_wire/server/WireServerDoers_autogen.cpp', wire_params))
        renders.append(FileRender('dawn_wire/server/WireServerHandlers.cpp', 'dawn_wire/server/WireServerHandlers_autogen.cpp', wire_params))
        renders.append(FileRender('dawn_wire/server/WireServerPrototypes.inl', 'dawn_wire/server/WireServerPrototypes_autogen.inl', wire_params))
