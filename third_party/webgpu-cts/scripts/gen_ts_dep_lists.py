...
from dir_paths import webgpu_cts_dir
from tsc_ignore_errors import run_tsc_ignore_errors

webgpu_cts_src_dir = os.path.join(webgpu_cts_dir, 'src')
...
def get_ts_sources():
    ...
    stdout = run_tsc_ignore_errors([
        '--project',
        os.path.join(webgpu_cts_src_dir, 'tsconfig.json'), '--listFiles',
...
if __name__ == '__main__':
    ...
    ts_sources_txt = os.path.join(webgpu_cts_dir, 'ts_sources.txt')

    resource_files = [x + '\n' for x in get_resource_files()]
    resource_files_txt = os.path.join(webgpu_cts_dir, 'resource_files.txt')
