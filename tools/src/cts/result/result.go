...
// ReduceTags reduces the list by combining results that have redundant tags.
func (l *List) ReduceTags() {
	// Gather all the tags
	allTags := NewTags()
	uniqueTags := container.NewMap[string, Tags]()
	for _, r := range *l {
		allTags.AddAll(r.Tags.Set)
		uniqueTags.Add(r.Tags.String(), r.Tags)
	}
	...
	if len(removedTags.Set) > 0 {
		for _, r := range *l {
			r.Tags.RemoveAll(removedTags.Set)
		}
		if err := l.RemoveDuplicates(); err != nil {
			panic(err)
		}
	}
}
