	...
	/*
		...
		for _, c := range caseChildren {
			for key := range c.Query.SplitCases() {
				if s, ok := keys[key]; ok && s != c.Status {
		...
		if len(keys) > 0 {
			for _, c := range caseChildren {
				...
				n.getOrCreateChild(q).Status = c.Status
				c.Status = ""
			}
		}
	*/
...
func (t *Tree) ReduceTo(fq query.Query) {
	node := &t.Node
	fq.Walk(func(q query.Query, t query.Target, n string) error {
		if q == fq {
			node.reduce()
		}
