...
func Parse(body string) (Content, error) {
	...
	for i, l := range strings.Split(body, "\n") { // For each line...
		...
		columnIdx := 1
		// syntaxErr is a helper for returning an error diagnostic with the
		// current line and column index.
		...
		next := func() Token {
			if len(tokens) > 0 {
				tok := tokens[0]
				columnIdx = tok.end + 1 // +1 for 0-based -> 1-based
				tokens = tokens[1:]
				return tok
			}
			return Token{}
		...
		tags := func(use string) (result.Tags, error) {
			...
			for {
				...
				case "]":
					return out, nil
				case "":
					return result.Tags{}, syntaxErr("expected ']' for " + use)
				default:
					out.Add(token.str)
				}
			}
		...
		if t := peek(); t.str == "" || t.str[0] == '#' || t.str[0] == '[' {
			return Content{}, syntaxErr("expected test query")
		}
		query := next().str

		// Parse the expected status
		if t := peek(); !strings.HasPrefix(t.str, "[") {
			return Content{}, syntaxErr("expected status")
		}
