...
func (u *updater) expectation(in Expectation, keep bool) ([]Expectation, error) {
	...
	// then a diagnositic will be raised. If the expectation chunk was marked
	...
	return u.expectationsForRoot(q, unfiltered, in.Line, in.Bug, in.Comment, diag), nil
}
...
func (u *updater) addNewExpectations() error {
	...
	roots := []query.Query{}
	for _, variant := range u.qt.results.UniqueTags() {
	...
	for _, root := range roots {
		...
		expectations = append(expectations, u.expectationsForRoot(
			...
			"",                    // Comment
			func(msg string, args ...interface{}) { // Diag
				errors = append(errors, fmt.Sprintf(msg, args...))
			},
		)...)
		if len(errors) > 0 {
