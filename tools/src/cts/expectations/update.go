...
func newQueryTree(results result.List) queryTree {
	...
	for query, indices := range queryToIndices {
		if err := tree.Add(query, indices); err != nil {
			// The only error we could get is duplicate data for the same query,
...
// glob returns all the results for and under the given query.
...
// markAsConsumed marks all the results for and under the given query as
// consumed. If any results were already consumed, then an error is returned.
...
func (u *updater) chunk(in Chunk) (Chunk, error) {
	if len(in.Expectations) == 0 {
		return in, nil // Just a comment / blank line
	}
...
func (u *updater) expectation(in Expectation, keep bool) ([]Expectation, error) {
	// noResults is a helper for early return when the expectation has no test
	// results.
	// If the expectation did not have a 'Skip' result, then a diagnostic will
	// be raised.
	// If the expectation chunk was marked with 'KEEP' then the expectation
	// will be preserved, otherwise stripped from the output expectations.
	...
	// Filter the queried results to the expectation's tags.
	// If the can't find any results for this query + tag combination, then
	// bail.
	...
	// Note: We're not rebuilding for the query filtered by tag - this is
	// intentional as we want to be able to combine tags if more tests start to
	// fail.
...
func (u *updater) addNewExpectations() error {
	...
	for _, variant := range allVariants {
		// Build a tree from the results filtered by variant.
...
func (u *updater) expectationsForRoot(
	...
	results result.List, // All the results under 'root'. Must be unfiltered.
...
// • There are more than 25 child nodes with a non-pass status, and none of the
//   children are consumed.
