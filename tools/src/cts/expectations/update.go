...
func (u *updater) expectation(in Expectation, keep bool) ([]Expectation, error) {
	...
	for _, variant := range u.variants {
		...
		t, err := filtered.FilterByTags(variant).StatusTree()
		if err != nil {
			return nil, fmt.Errorf("while building tree for tags '%v': %w", variant, err)
		}
		t.ReduceTo(q, treeReducer)
		for _, qs := range t.List() {
			resultsByQuery[qs.Query] = append(resultsByQuery[qs.Query], result.Result{
				Query:  qs.Query,
				Tags:   variant,
				Status: qs.Data,
			})
	...
	// Reduce the tags
	results, err := u.reduceTags(resultsByQuery)
	if err != nil {
		return nil, err
	}
...
func (u *updater) reduceTags(resultsByQuery map[query.Query]result.List) (result.List, error) {
	...
	for q, l := range resultsByQuery {
		// Ensure that only one tag is used for each tag header set.
		l := u.filterResultTags(l)
