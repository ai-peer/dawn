...
func Generate(outputDir string, verbose, dryRun bool) error {
	staleFiles := []string{}
	...
	for _, relTmplPath := range files { // relative to project root
		...
		writeFile := func(relPath, body string) error {
			...
			sb := strings.Builder{}
			sb.WriteString(header.Get(string(existing), filepath.ToSlash(relTmplPath)))
			sb.WriteString(body)
...
func generate(tmpl string, cache *genCache, w io.Writer, writeFile WriteFile) error {
	...
	return template.Run(tmpl, w, nil, funcs)
}
