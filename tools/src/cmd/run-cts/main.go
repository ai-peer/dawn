...
func run() error {
	...
	flag.BoolVar(&genCoverage, "coverage", false, "generate coverage data. Enabled --isolated")
	flag.Parse()
...
func (r *runner) streamResults(ctx context.Context, wg *sync.WaitGroup, results chan result) error {
	...
	if covTree != nil {
		...
		if true {
			...
			fmt.Fprintln(r.stdout)
			fmt.Fprintln(r.stdout, "Serving coverage view at "+blue+url+ansiReset)
			...
			file, err := os.Create(filepath.Join(fileutils.ThisDir(), "coverage.dat"))
			if err != nil {
