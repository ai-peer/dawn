...
func (r *ResultSource) GetResults(ctx context.Context, cfg Config, auth auth.Options) (result.List, error) {
	...
	if ps.Change == 0 {
		...
		builds, ps, err = MostRecentBuildsForChange(ctx, cfg, gerrit, bb, latest.Number)
		if err != nil {
			return nil, err
		}
		log.Printf("using results from cl %v ps %v...\n", ps.Change, ps.Patchset)
	}
	...
	if builds == nil {
		if builds, err = GetOrStartBuilds(ctx, cfg, ps, bb, false); err != nil {
			return nil, err
		}
	} else {
...
func MostRecentBuildsForChange(
	ctx context.Context,
	cfg Config,
	g *gerrit.Gerrit,
	bb *buildbucket.Buildbucket,
	change int) (BuildsByName, gerrit.Patchset, error) {
