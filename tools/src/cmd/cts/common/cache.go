...
func BuildCache(ctx context.Context, ctsDir, nodePath string) (*Cache, error) {
	...
	t := tar.NewWriter(tarGz)
	defer t.Close()

	for _, relPath := range files {
		absPath := filepath.Join(ctsDir, relPath)
		...
		if err != nil {
			return nil, fmt.Errorf("failed to create tar file info header: %w", relPath, err)
		}

		header.Name = relPath // Use the nested path

		compressedFile, err := tarCompress(absPath)
		if err != nil {
			return nil, fmt.Errorf("failed to get compress '%v': %w", absPath, err)
		}

		header.Size = int64(len(compressedFile))

		if err := t.WriteHeader(header); err != nil {
			return nil, fmt.Errorf("failed to write tar header for '%v': %w", relPath, err)
		}

		if _, err := t.Write(compressedFile); err != nil {
			return nil, fmt.Errorf("failed to write tar data for '%v': %w", relPath, err)
		}
	...
	return &Cache{files, tarGz.Bytes()}, nil
}

func tarCompress(file string) ([]byte, error) {
	f, err := os.Open(file)
	if err != nil {
		return nil, fmt.Errorf("failed to open  '%v' to tar: %w", file, err)
	}
	...
	if err := gz.Close(); err != nil {
		return nil, fmt.Errorf("failed to close a gzip writer: %w", err)
	}
	if err := f.Close(); err != nil {
		return nil, fmt.Errorf("failed to close a file '%v': %w", file, err)
	}
