...
type rollerFlags struct {
	...
	abandon bool // Abandon past roll changes
}
...
func (c *cmd) RegisterFlags(ctx context.Context, cfg common.Config) ([]string, error) {
	...
	flag.BoolVar(&c.flags.abandon, "abandon", false, "do not abandon existing rolls")
...
func (r *roller) roll(ctx context.Context) error {
	...
	// Abandon existing rolls, if -abandon is true
	if r.flags.abandon && len(existingRolls) > 0 {
	...
	// Update the expectations with the re-formatted content.
	...
	for pass := 0; ; pass++ {
		...
		// Note: The new expectations are not used if the last pass didn't fail,
		// but we always want to post the diagnostics
		...
		// If all the builds passed, then we're done!
		if len(failingBuilds) == 0 {
			break
		}
