		...
		for _, cmd := range cmds {
			if cmd.name() == os.Args[1] {
				...
				flag.Usage = func() {
					...
					flagsAndArgs := args
					if hasFlags {
						flagsAndArgs = append([]string{"<flags>"}, args...)
					}
				...
				if err := flag.CommandLine.Parse(os.Args[2+len(args):]); err != nil {
					return err
				}
...
func (c *cmdUpdate) registerFlags(ctx context.Context, cfg Config) ([]string, error) {
	flag.StringVar(&c.flags.results, "results", "results.json", "path to CTS results file")
...
func (c *cmdReduce) run(ctx context.Context, cfg Config) error {
	...
	var results result.List
	if err := json.NewDecoder(input).Decode(&results); err != nil {
		return err
	}
...
func writeResults(w io.Writer, r result.List) error {
