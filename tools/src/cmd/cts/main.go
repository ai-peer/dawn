...
const ctsHashPrefix = `{chromium_git}/external/github.com/gpuweb/cts@`
...
func (c *cmdRoll) run(ctx context.Context, cfg Config) error {
	chromium, err := gitiles.New(ctx, "chromium.googlesource.com")

	ctsHash, err := chromium.Hash(ctx, "external/github.com/gpuweb/cts", "refs/heads/main")
	if err != nil {
		return err
	}

	dawn, err := gitiles.New(ctx, "dawn.googlesource.com")

	deps, err := dawn.DownloadFile(ctx, "dawn", "refs/heads/main", "DEPS")
	if err != nil {
		return err
	}
