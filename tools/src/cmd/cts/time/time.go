...
func (c *cmd) RegisterFlags(ctx context.Context, cfg common.Config) ([]string, error) {
	...
	flag.BoolVar(&c.flags.aggregate, "aggregate", false, "aggregate times by test, without reporting statuses and tags")
	return nil, nil
}
...
func (c *cmd) Run(ctx context.Context, cfg common.Config) error {
	...
	if c.flags.aggregate {
		merged := map[string]result.Result{}
		for _, r := range results {
			...
			if exists {
				...
				entry = result.Result{
					// Note: aggregation drops Status and Tags
					Query:    q,
					Duration: r.Duration,
				}
			}
