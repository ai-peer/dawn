...
func (cmd) Desc() string { return "builds the CTS case cache tar.gz file" }

func (c *cmd) RegisterFlags(ctx context.Context, cfg common.Config) ([]string, error) {
	dawnRoot := fileutils.DawnRoot()
	ctsPath := filepath.Join(dawnRoot, "third_party", "webgpu-cts")
	outPath := filepath.Join(dawnRoot, "webgpu-cts", "cache.tar.gz")
	...
	flag.StringVar(&c.flags.output, "o", outPath, "output file")
	return nil, nil
}
...
func (c *cmd) Run(ctx context.Context, cfg common.Config) error {
	...
	if err := ioutil.WriteFile(c.flags.output, cache.TarGz, 0666); err != nil {
		return fmt.Errorf("failed to write cache to '%v': %w", c.flags.output, err)
	}

	log.Println("wrote output to ", c.flags.output)
	return nil
}
