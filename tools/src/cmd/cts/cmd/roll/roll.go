...
const (
	ctsRollSubjectPrefix = "Roll third_party/webgpu-cts/ "
...
func (r *roller) roll(ctx context.Context) error {
	...
	ctsLog = ctsLog[1:] // Don't include the oldest change in the log
	...
	ps, err := r.gerrit.EditFiles(changeID, "", map[string]string{
	...
	for pass := 0; ; pass++ {
		...
		if diag, err := newExpectations.Update(results); err != nil {
			_ = diag // TODO
			return err
		}
...
func (r *roller) rollCommitMessage(
	...
	msg := &strings.Builder{}
	msg.WriteString(ctsRollSubjectPrefix)
...
func (r *roller) findExistingRolls() ([]gerrit.ChangeInfo, error) {
	...
	changes, _, err := r.gerrit.QueryChanges("owner:me",
		...
		fmt.Sprintf(`message:"%v"`, ctsRollSubjectPrefix))
	if err != nil {
