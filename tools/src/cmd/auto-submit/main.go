...
// auto-submit applies the 'Commit-Queue+2' label to Gerrit changes that are ready to be submitted
package main
...
func main() {
	flag.Usage = func() {
		...
		fmt.Fprintf(out,
			`%v applies the 'Commit-Queue+2' label to Gerrit changes that are ready to be submitted.

The tool monitors Gerrit changes, looking for changes that have the labels
...
func (a *app) submitReadyChanges() error {
	if *verboseFlag {
		fmt.Println("Scanning for changes to submit...")
	}
	...
	for _, change := range changes {
		...
		if hasLabel("Commit-Queue", 1) || hasLabel("Commit-Queue", 2) {
			// Change already in the process of dry-running or submitting
			continue
		}
		...
		case cqUnknown, cqPassed:
			if *dryrunFlag {
				fmt.Printf("Would submit %v: %v... (--dry)\n", change.ChangeID, change.Subject)
				continue
			}

			fmt.Printf("Submitting %v: %v...\n", change.ChangeID, change.Subject)
		...
		case cqFailed:
			if *verboseFlag {
				fmt.Printf("Change failed CQ: %v: %v...\n", change.ChangeID, change.Subject)
			}
		}
	}
