...
func run() error {
	...
	flag.StringVar(&tintExe, "tint", "tint", "path to tint executable")
	flag.StringVar(&tintFlags, "tint-flags", "", "comma-separated list of flags to pass to tint")
	flag.IntVar(&iterations, "iterations", 10, "number of times to benchmark tint")
	flag.Usage = func() {
		fmt.Println("tint-bench [tint-flags]")
		os.Exit(1)
	}
	...
	for alpha := alphaRange.from; alpha < alphaRange.to; alpha++ {
		...
		args := []string{wgslPath}
		if tintFlags != "" {
			args = append(args, strings.Split(tintFlags, ",")...)
		}

		durations := []time.Duration{}
		for i := 0; i < iterations; i++ {
			tint := exec.Command(tintExe, args...)
			start := time.Now()
