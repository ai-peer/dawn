...
fun createGpuTextureFromImageAlternate(device: Device, bitmap: Bitmap): android.dawn.Texture {
    val size = Extent3D(width = bitmap.width, height = bitmap.height)

    val fromBuffer = device.createBuffer(
        BufferDescriptor(
            size = bitmap.width * bitmap.height * Int.SIZE_BYTES.toLong(),
            usage = BufferUsage.CopySrc,
            mappedAtCreation = true
        )
    ).apply { bitmap.copyPixelsToBuffer(getConstMappedRange(size = this.size)); unmap() }
    return device.createTexture(
        TextureDescriptor(
            size = size,
            format = TextureFormat.RGBA8Unorm,
