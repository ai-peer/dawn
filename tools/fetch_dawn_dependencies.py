...
parser.add_argument(
    '-s', '--shallow', action='store_true', default=True,
    help="""
...
parser.add_argument(
    '-ns', '--no-shallow', action='store_false', dest='shallow',
    help="Deactivate shallow cloning."
)

parser.add_argument(
    '-t', '--use-test-deps', action='store_true', default=False,
    help="""
...
def process_dir(args, dir_path, required_submodules):
    ...
    for submodule in required_submodules:
        ...
        git = lambda *x: subprocess.run([args.git, '-C', submodule_path, *x], capture_output=True)
