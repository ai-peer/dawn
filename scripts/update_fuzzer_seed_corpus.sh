...
Then, run the following command to upload new testcases to the seed corpus:
    ...
    gsutil -m rsync -d $minimized_testcase_dir gs://clusterfuzz-corpus/libfuzzer/${fuzzer_name}/

    WARNING: This will delete all GCS files that are not also in $minimized_testcase_dir
