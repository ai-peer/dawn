...
The device lifecycle is a bit more complicated than other objects in Dawn for multiple reasons:
 ...
 - A device can become "disconnected" when a TDR or hot-unplug happens.
   In this case it destruction of the device doesn't need to wait on GPU commands to finish because they just disappeared.
...
Initialization of a device looks like the following:
 ...
 - The device is read to be used by the application!
...
All this ensures that during destruction or foreceful disconnect of the device, it properly gets to the `Disconnected` state with no commands executing on the GPU.
