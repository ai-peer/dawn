...
A **record** is a list of **record members**, each of which is a dictionary with the following schema:
 ...
 - `"is_data_only"` (default to false) a boolean that says whether it is safe to directly pass a pointer that is pointing to the memory in the transfer buffer into dawn_wire. As the prevention of TOCTOU attacks is an important feature of the wire and is done by making every single value returned to dawn_native a thing that's a copy of what's in the wire, this value is set to true only when the member is data-only and don't impact control flow.
