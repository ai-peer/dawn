...
A **record** is a list of **record members**, each of which is a dictionary with the following schema:
 ...
 - `"is_data_only"` (default to false) a boolean that says whether it is safe to directly return a pointer of this member that is pointing to a piece of memory in the transfer buffer into dawn_wire. To prevent TOCTOU attacks, by default in dawn_wire we must ensure every single value returned to dawn_native a copy of what's in the wire, so `"is_data_only"` is set to true only when the member is data-only and don't impact control flow.
