...
* list of enables
* list of static_asserts
* source Program
* list of type declarations

### Function
* Information on a given function

#### Stores
* List of blocks in function
* Name
* Attributes
* Return attributes
* Return type
* Parameter list { name, type, attributes }

### Block (FlowNode)
* Basic block. Ends  in a branch, only one branch per block.
...
### IfBlock (FlowNode)
* An if control block
** Both true and false blocks always exist, even if empty
...
## SSA
The IR is SSA. Each result has an associated origin. The Origin is either:
 * Constant
 * Temporary
 * Variable

 In order to facilitate the movement back to AST, a PHI node must have `Variable`
 origins which all point to the same originating variable.



### Scoping
Scoped blocks end up getting flattened into the IR. This means as we
return IR -> AST the shadowed variables will be renamed to make them
unique. This keeps blocks linear as just a list of instructions to
execute.
...
Alternatively could have added a `scope %block_id` instruction into the
instruction list for a block, but this has the downside that there are
now blocks which exist outside flow control. So a function ends up with
a disjoint set of flow nodes that can't all be found by walking the flow
control graph.
