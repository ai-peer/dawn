...
and `value` results. A `value` is a result, a constant result, or a
result computed by an `instruction`.

Each `block` ends in a `terminator` expression. If there is no
`terminator` then the block is considered dead code. The `terminators`
may hold `values` which are returned out of the block being terminated.
...
A free `ir::ToProgram` method is provided to convert from a
`Program` to an `ir::Module`. A similar `ToProgram` free method is
provided for the reverse converion.
...
### Naming
The instructions internal to the IR, and the values in the IR do not
...
A block instructions can be walked in linear scoped fashion. The control
instructions do not need to be entered to walk a blocks instructions.


#### Control Instructions
The control instructions provide contains for other blocks. These
...
The short-circuit expressions (e.g. `a && b`) are be convert into an
...
`constant::Value` from then `src/tint/ir/constant` system.
...
### BlockParam Values
The `BlockParam` values are used to pass information out of a block and
back to the parent instruction. This is similar to `Phi` nodes, and turn
into `Phi` nodes in the SPIR-V generator.
