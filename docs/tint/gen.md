...
* Source files with a `_fuzz` suffix before the file extension are classed as
  `fuzz` library targets. Example: `writer_fuzz.cc`.
...
* Source files with the name `main_fuzz.cc` are classed as fuzzer executable targets.
  These typically exist under `src/tint/cmd`. Example: `cmd/fuzz/wgsl/main_fuzz.cc`.
