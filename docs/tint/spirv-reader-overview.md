...
result type for an operation all have the same signedness.
...
themselves and apparently relying on WGSL's "Load Rule".
...
Glslang generates texture and sampler formal parameters as
pointer-to-UniformConstant.

WGSL models them as pointer-to-handle.

So far so good.  However, the tracing logic described in
the previous section does not know what to do when it bottoms
out on a formal parameter, e.g. OpFunctionParameter.
...
Note also that the call to textureSampleLevel should use the indirection operator
to convert the pointers back to references. That is it should read as:

    textureSampleLevel(*imparam, *sparam, ...
...
- combinatorial: think of this as an ALU operation, i.e. the effect
  ...
  as memoiry or the actions of other invocations in its subgroup.
...
Why:
- If a value has manu uses, then computing it once can save effort.
  Preserve that choice if it was made by an upstream optimizing compiler.
- If a value is consumed in a different structured construct, then it
  the site of its consumption may be inside a loop, and we don't want to
...
basic blocks that would otherwise not be visible (because they
...
- for a phi: generate an assigment to that variableat the end of
...
SPIR-V images types map to WGSL types, but SPIR-V is more flexible
in that the WGSL type is determined more by usage (what operations
are performed on it) than by declaration.

Similarly, SPIR-V only has one sampler type.  Its the use of the
sampler that determines whether it maps to a WGSL `sampler` or
`sampler_comparison` (for depth sampling).

The SPIR-V Reader scans uses of each texture and sampler
the module to infer the appopriate target WGSL type.
...
- a storage buffer without all NonWritable members is translated with
  ...
  WGSL reference type (and hnce corresponding pointer type).
...
variable. This originating varialbe may be a buffer variable which then
...
user-defined helper functions, we don't need handle this buffer type
...
structure (the pointee type of the storage buffer variabe).
