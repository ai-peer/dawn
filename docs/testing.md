...
### Terminology
 ...
 - Trial: A group of Steps run together. `kNumTrials` are run for each test. A Trial is run for approximately `kCalibrationRunTimeSeconds`. Metrics are accumlated per-trial and reported as the total time divided by `numSteps * iterationsPerStep`. `maxStepsInFlight` is passed to the `DawnPerfTestsBase` constructor to limit the number of Steps pipelined.
...
`dawn_perf_tests` measures the following metrics:
 ...
 - `recording_time`: The time to convert Dawn commands to native commands and submit work to the native API.
