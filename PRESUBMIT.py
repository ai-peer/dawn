...
def _DoClangFormat(input_api, output_api):
    ...
    original_library_path = os.environ['LD_LIBRARY_PATH'] or ''
    os.environ['LD_LIBRARY_PATH'] = os.path.join(os.getcwd(), 'build/linux/debian_sid_amd64-sysroot/usr/lib/x86_64-linux-gnu') + ':' + original_library_path

    # Call the linting script and forward the output as a notification or as an error
    result = None
    try:
        output = subprocess.check_output(lint_cmd);
        result = [output_api.PresubmitNotifyResult(output)]
    except subprocess.CalledProcessError as e:
        result = [output_api.PresubmitError(e.output)]
