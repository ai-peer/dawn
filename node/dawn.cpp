...
namespace {
    ...
    void Method(const v8::FunctionCallbackInfo<v8::Value>& args) {
        ...
        std::vector<std::vector<uint8_t>> argsStrings(args.Length());
        std::vector<const char*> argv(args.Length() + 1);
        argv[0] = "";
        for (int i = 0; i < args.Length(); ++i) {
            v8::MaybeLocal<v8::String> maybeArg = args[i]->ToString(isolate->GetCurrentContext());
            v8::Local<v8::String> arg;
            if (!maybeArg.ToLocal(&arg)) {
                isolate->ThrowException(v8::Exception::TypeError(
                    v8::String::NewFromUtf8(isolate, "Arguments must be strings",
                                            v8::NewStringType::kNormal)
                        .ToLocalChecked()));
                return;
            }
            argsStrings[i].resize(arg->Length() + 1);
            arg->WriteOneByte(argsStrings[i].data());
            argv[i + 1] = reinterpret_cast<const char*>(argsStrings[i].data());
        }
        if (!InitSample(argv.size(), argv.data())) {
            return;
        }
        init();
