...
struct BuiltinDataWithCandidates {
    ...
    BuiltinDataWithCandidates(uint32_t args_num,
    ...
    std::string GetCandidatesString() {
        // Built-ins like abs, clamp, max, min and mix may have non-default candidate lists.
        if (!candidates.empty()) {
            return candidates;
        }

        // Generate the default candidate list. Currently the order of candidate overloads may vary
        // due to input parameter types (scalar or vector), and we are adding abstract types and f16
        // type for many built-in, so the candidate list of built-ins are expected to have many
        // change. In order to reduce maintenance efforts before constant eval and f16 getting
        // completed, we don't check the details of built-in overload lists, but just check the
        // first line, i.e. "2 candidate functions:". After constant eval and f16 getting completed,
        // the result should be changed to something like :
        // 2 candidate functions:
        //   cos(T) -> T  where: T is abstract-float, f32 or f16
        //   cos(vecN<T>) -> vecN<T>  where: T is abstract-float, f32 or f16
...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::Values(
        ...
        BuiltinDataWithCandidates(2, "max", BuiltinType::kMax),
        BuiltinDataWithCandidates(2, "min", BuiltinType::kMin),
        ...
        // candidates that may change order. Note that `mix(vecN<f32>, vecN<f32>, f32)
        // -> vecN<f32>` is not tested here.
        BuiltinDataWithCandidates(3, "mix", BuiltinType::kMix, "3 candidate functions:"),
        // modf
...
struct BuiltinDataWithCandidates {
    ...
    std::string GetCandidatesString() {
        // Built-ins like abs, clamp, max and min may have non-default candidate lists.
        if (!candidates.empty()) {
            return candidates;
        }

        // Generate default candidate list. Currently we are adding abstract types and f16 type for
        // many built-in, so the candidate list of built-ins are expected to have many change. In
        // order to reduce maintenance efforts before constant eval completed, we don't check the
        // details of built-in overload lists, but just check the first line, i.e. "2 candidate
        // functions:". After constant eval and f16 getting completed, the result should be changed
        // to something like :
        // 2 candidate functions:
        //   reverseBits(T) -> T  where: T is abstract-int, i32 or u32
        //   reverseBits(vecN<T>) -> vecN<T>  where: T is abstract-int, i32 or u32
