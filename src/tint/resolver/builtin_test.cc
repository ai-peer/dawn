...
TEST_P(ResolverBuiltinTest_TwoParam, Error_NoTooManyParams) {
    ...
    EXPECT_EQ(r()->error(),
              "error: no matching call to " + std::string(param.name) +
                  ...
                  "(T, T) -> T  where: T is abstract-float, f32 or f16\n  " +
                  std::string(param.name) +
                  "(vecN<T>, vecN<T>) -> vecN<T>  where: T is abstract-float, f32 or f16\n");
}
...
TEST_P(ResolverBuiltinTest_ThreeParam_FloatOrInt, Error_NoParams) {
    ...
    EXPECT_EQ(
        ...
        "error: no matching call to " + std::string(param.name) +
            ...
            "2 candidate functions:\n  " +
            std::string(param.name) +
            "(T, T, T) -> T  where: T is abstract-float, abstract-int, f32, i32, u32 or f16\n  " +
            std::string(param.name) +
