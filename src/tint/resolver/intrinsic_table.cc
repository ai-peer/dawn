...
const sem::Struct* build_modf_result(MatchState& state, const sem::Type* el) {
    ...
    auto build_f16 = [&] {
        auto* f16 = state.builder.create<sem::F32>();
...
const sem::Struct* build_modf_result_vec(MatchState& state, Number& n, const sem::Type* el) {
    ...
    auto* vec = el->As<sem::Vector>();
    auto build_f32 = [&] {
        auto* ty =
            state.builder.create<sem::Vector>(state.builder.create<sem::F32>(), vec->Width());
        return build_struct(state.builder, prefix + "_f32", {{"fract", ty}, {"whole", ty}});
    };
    auto build_f16 = [&] {
        auto* ty =
            state.builder.create<sem::Vector>(state.builder.create<sem::F16>(), vec->Width());
        return build_struct(state.builder, prefix + "_f16", {{"fract", ty}, {"whole", ty}});
    };
    ...
    return Switch(
        ...
        [&](const sem::AbstractFloat*) {
            auto* abstract =
                build_struct(state.builder, prefix + "_abstract", {{"fract", el}, {"whole", el}});
            abstract->SetConcreteTypes(utils::Vector{build_f32(), build_f16()});
            return abstract;
        },
        [&](Default) {
