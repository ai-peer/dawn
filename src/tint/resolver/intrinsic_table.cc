...
enum class OverloadFlag {
    ...
    kIsConstructor,
    kIsConverter,
    kSupportsVertexPipeline,
    kSupportsFragmentPipeline,
    kSupportsComputePipeline,
    kIsDeprecated,
};
...
bool match_mat2x2(const sem::Type* ty, const sem::Type*& T) {
    return match_mat<2, 2>(ty, T);
}

const sem::Matrix* build_mat2x2(MatchState& state, const sem::Type* T) {
    return build_mat(state, Number(2), Number(2), T);
}

bool match_mat2x3(const sem::Type* ty, const sem::Type*& T) {
    return match_mat<2, 3>(ty, T);
}

const sem::Matrix* build_mat2x3(MatchState& state, const sem::Type* T) {
    return build_mat(state, Number(2), Number(3), T);
}

bool match_mat2x4(const sem::Type* ty, const sem::Type*& T) {
    return match_mat<2, 4>(ty, T);
}

const sem::Matrix* build_mat2x4(MatchState& state, const sem::Type* T) {
    return build_mat(state, Number(2), Number(4), T);
}

bool match_mat3x2(const sem::Type* ty, const sem::Type*& T) {
    return match_mat<3, 2>(ty, T);
}

const sem::Matrix* build_mat3x2(MatchState& state, const sem::Type* T) {
    return build_mat(state, Number(3), Number(2), T);
}

bool match_mat3x3(const sem::Type* ty, const sem::Type*& T) {
    return match_mat<3, 3>(ty, T);
}

const sem::Matrix* build_mat3x3(MatchState& state, const sem::Type* T) {
    return build_mat(state, Number(3), Number(3), T);
}

bool match_mat3x4(const sem::Type* ty, const sem::Type*& T) {
    return match_mat<3, 4>(ty, T);
}

const sem::Matrix* build_mat3x4(MatchState& state, const sem::Type* T) {
    return build_mat(state, Number(3), Number(4), T);
}

bool match_mat4x2(const sem::Type* ty, const sem::Type*& T) {
    return match_mat<4, 2>(ty, T);
}

const sem::Matrix* build_mat4x2(MatchState& state, const sem::Type* T) {
    return build_mat(state, Number(4), Number(2), T);
}

bool match_mat4x3(const sem::Type* ty, const sem::Type*& T) {
    return match_mat<4, 3>(ty, T);
}

const sem::Matrix* build_mat4x3(MatchState& state, const sem::Type* T) {
    return build_mat(state, Number(4), Number(3), T);
}

bool match_mat4x4(const sem::Type* ty, const sem::Type*& T) {
    return match_mat<4, 4>(ty, T);
}

const sem::Matrix* build_mat4x4(MatchState& state, const sem::Type* T) {
    return build_mat(state, Number(4), Number(4), T);
}
...
class Impl : public IntrinsicTable {
  public:
    ...
    const sem::CallTarget* Lookup(ConstructableType type,
  ...
  private:
    ...
    void PrintCandidates(std::ostream& ss, const Candidates& candidates, const char* name) const;
...
const sem::Builtin* Impl::Lookup(sem::BuiltinType builtin_type,
    ...
    auto match = MatchIntrinsic(kBuiltins[static_cast<uint32_t>(builtin_type)], name, args,
...
const sem::CallTarget* Impl::Lookup(ConstructableType type,
                                    const sem::Type* template_arg,
                                    const std::vector<const sem::Type*>& args,
                                    const Source& source) {
    auto name = str(type);
...
IntrinsicPrototype Impl::MatchIntrinsic(const IntrinsicInfo& intrinsic,
    ...
    for (uint8_t overload_idx = 0; overload_idx < intrinsic.num_overloads; overload_idx++) {
        auto candidate = MatchOverload(intrinsic.overloads[overload_idx], args, closed);
    ...
    switch (num_matched) {
        ...
        default: {
            std::stringstream ss;
            ss << num_matched << " overloads matched " << name;
...
void Impl::PrintCandidates(std::ostream& ss, const Candidates& candidates, const char* name) const {
    for (auto& candidate : candidates) {
        ss << "  ";
        PrintOverload(ss, candidate.overload, name);
        ss << std::endl;
