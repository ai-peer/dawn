...
TEST_F(ResolverVarLetValidationTest, LetOfPtrConstructedWithRef) {
    ...
    auto* var_b = Const(Source{{12, 34}}, "b", ty.pointer<float>(priv), Expr("a"), {});
...
TEST_F(ResolverVarLetValidationTest, VarRedeclaredInIfBlock) {
    ...
    auto* outer_body =
        Block(Decl(var_a_float), create<ast::IfStatement>(cond, body, ast::ElseStatementList{}));
