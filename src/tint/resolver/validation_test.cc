...
TEST_F(ResolverValidationTest, Expr_DontCall_Function) {
    ...
    EXPECT_FALSE(r()->Resolve());
    EXPECT_EQ(r()->error(), R"(12:34 error: missing '(' for function call)");
}
...
TEST_F(ResolverValidationTest, Expr_DontCall_Builtin) {
    ...
    EXPECT_FALSE(r()->Resolve());
    EXPECT_EQ(r()->error(), R"(12:34 error: missing '(' for builtin function call)");
}

TEST_F(ResolverValidationTest, Expr_DontCall_Alias) {
    Alias("T", ty.u32());
    WrapInFunction(Expr(Source{Source::Range{{12, 34}, {12, 35}}}, "T"));

    EXPECT_FALSE(r()->Resolve());
    EXPECT_EQ(r()->error(), R"(12:34 error: expression evaluated to type 'u32', but expected value
12:35 note: are you missing '()' for type initializer?)");
}

TEST_F(ResolverValidationTest, Expr_DontCall_Struct) {
    Structure("T", utils::Vector{Member("m", ty.u32())});
    WrapInFunction(Expr(Source{Source::Range{{12, 34}, {12, 35}}}, "T"));

    EXPECT_FALSE(r()->Resolve());
    EXPECT_EQ(r()->error(), R"(12:34 error: expression evaluated to type 'T', but expected value
12:35 note: are you missing '()' for type initializer?
note: 'T' declared here)");
}

TEST_F(ResolverValidationTest, Expr_DontCall_BuiltinType) {
    WrapInFunction(Expr(Source{Source::Range{{12, 34}, {12, 35}}}, "vec3f"));

    EXPECT_FALSE(r()->Resolve());
    EXPECT_EQ(r()->error(),
              R"(12:34 error: expression evaluated to type 'vec3<f32>', but expected value
12:35 note: are you missing '()' for type initializer?)");
}
