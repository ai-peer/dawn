...
TEST_F(ResolverValidationTest, Stmt_Else_NonBool) {
    ...
    WrapInFunction(If(Expr(true), Block(), Else(Expr(Source{{12, 34}}, 1.23f), Block())));

    EXPECT_FALSE(r()->Resolve());

    EXPECT_EQ(r()->error(), "12:34 error: else statement condition must be bool, got f32");
}
...
TEST_F(ResolverValidationTest, UsingUndefinedVariableInnerScope_Fail) {
    ...
    auto* outer_body =
        Block(create<ast::IfStatement>(cond, body, ast::ElseStatementList{}), Assign(lhs, rhs));
...
TEST_F(ResolverValidationTest, UsingUndefinedVariableOuterScope_Pass) {
    ...
    auto* outer_body =
        Block(Decl(var), create<ast::IfStatement>(cond, body, ast::ElseStatementList{}));
