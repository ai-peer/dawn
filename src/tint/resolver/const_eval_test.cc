...
struct BitValues {
    ...
    static inline const T TwoLeftMost = T{T{0b11} << (NumBits - 2u)};
    static inline const T AllButTwoLeftMost = T{~TwoLeftMost};
    static inline const T RightMost = T{1};
    static inline const T AllButRightMost = T{~RightMost};

    template <typename U, typename V>
    static constexpr T Lsh(U val, V shiftBy) {
        return T{T{val} << T{shiftBy}};
    }
};
...
std::vector<Case> ShiftLeftCases() {
    ...
    return {
        ...
        // Would overflow AInt: result cannot be represented (see
        // ResolverConstEvalBinaryOpTest_Overflow)
        C(B::AllButLeftMost, ST{1}, B::AllButRightMost, true),
        C(B::AllButLeftMost, ST{B::NumBits - 1}, B::LeftMost, true),
        C(B::LeftMost, ST{1}, T{0}, true),
        C(B::LeftMost, ST{B::NumBits - 1}, T{0}, true),

        // Would overflow AInt: sign change (see ResolverConstEvalShiftLeftSignChangeError)
        C(T{0b0001}, ST{B::NumBits - 1}, B::LeftMost, true),
        C(T{0b0010}, ST{B::NumBits - 2}, B::LeftMost, true),
        C(T{0b0100}, ST{B::NumBits - 3}, B::LeftMost, true),
        C(T{0b1000}, ST{B::NumBits - 4}, B::LeftMost, true),
        C(T{0b0011}, ST{B::NumBits - 2}, B::Lsh(0b0011, B::NumBits - 2), true),
        C(T{0b0110}, ST{B::NumBits - 3}, B::Lsh(0b0011, B::NumBits - 2), true),
        C(T{0b1100}, ST{B::NumBits - 4}, B::Lsh(0b0011, B::NumBits - 2), true),
    };
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::Values(
        ...
        OverflowCase{ast::BinaryOp::kShiftLeft,  //
                     Val(AInt{BitValues<AInt>::AllButLeftMost}), Val(1_a)},
        OverflowCase{ast::BinaryOp::kShiftLeft,  //
                     Val(AInt{BitValues<AInt>::AllButLeftMost}),
                     Val(AInt{BitValues<AInt>::NumBits - 1})},
        OverflowCase{ast::BinaryOp::kShiftLeft,  //
                     Val(AInt{BitValues<AInt>::All}), Val(AInt{BitValues<AInt>::NumBits})},
        OverflowCase{ast::BinaryOp::kShiftLeft,  //
                     Val(1_a), Val(999999999999_a)}
...
using ResolverConstEvalShiftLeftAbstractSignChangeError =
    ResolverTestWithParam<std::tuple<AInt, AInt>>;
TEST_P(ResolverConstEvalShiftLeftAbstractSignChangeError, Test) {
    GlobalConst("c",
                Shl(Source{{1, 1}}, Expr(std::get<0>(GetParam())), Expr(std::get<1>(GetParam()))));
    EXPECT_FALSE(r()->Resolve());
    EXPECT_EQ(r()->error(), "1:1 error: shift left operation results in sign change");
}
INSTANTIATE_TEST_SUITE_P(
    Test,
    ResolverConstEvalShiftLeftAbstractSignChangeError,
    testing::Values(std::make_tuple(AInt{0b0001}, (BitValues<AInt>::NumBits - 1)),
                    std::make_tuple(AInt{0b0010}, (BitValues<AInt>::NumBits - 2)),
                    std::make_tuple(AInt{0b0100}, (BitValues<AInt>::NumBits - 3)),
                    std::make_tuple(AInt{0b1000}, (BitValues<AInt>::NumBits - 4)),
                    std::make_tuple(AInt{0b0011}, (BitValues<AInt>::NumBits - 2)),
                    std::make_tuple(AInt{0b0110}, (BitValues<AInt>::NumBits - 3)),
                    std::make_tuple(AInt{0b1100}, (BitValues<AInt>::NumBits - 4))));
