...
template <typename T>
Case C(T lhs, T rhs, T expect) {
    return Case{Values<T>{lhs, rhs, expect}};
}
...
template <typename T, bool include_overflow>
std::vector<Case> OpAddCases() {
    ...
    if constexpr (IsInteger<UnwrapNumber<T>>) {
        cases = Concat(cases, std::vector<Case>{
                                  C(T::Lowest(), T::Highest(), Negate(T{1})),
                              });
        if constexpr (include_overflow) {
            cases = Concat(cases, std::vector<Case>{
                                      // Overflow
...
INSTANTIATE_TEST_SUITE_P(Add,
                         ...
                         testing::Combine(testing::Values(ast::BinaryOp::kAdd),
                                          testing::ValuesIn(Concat(  //
                                              OpAddCases<AInt, false>(),
                                              OpAddCases<AFloat, false>(),
                                              OpAddCases<i32, true>(),
                                              OpAddCases<u32, true>(),
                                              OpAddCases<f32, true>(),
                                              OpAddCases<f16, true>()  //
...
TEST_F(ResolverConstEvalTest, BinaryAbstractAddOverflow_AInt) {
    ...
    EXPECT_EQ(r()->error(), "1:1 error: Addition results in overflow/underflow");
}
