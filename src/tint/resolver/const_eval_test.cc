...
TEST_P(ResolverConstEvalBuiltinTest, Test) {
    ...
    std::visit(
        [&](auto&& values) {
            ...
            auto args = utils::Transform(values.args,
                                         [&](auto&& a) { return (const ast::Expression*)Expr(a); });

            // auto* expr = create<ast::CallExpression>(Expr(sem::str(builtin)), std::move(args));
...
template <typename T>
std::vector<Case> Atan2Cases(bool /*finite_only*/) {
    return {
...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::Combine(testing::Values(sem::BuiltinType::kAtan2),
                     ...
                     testing::ValuesIn(Concat(Atan2Cases<AFloat>(false),  //
                                              Atan2Cases<f32>(true),
                                              Atan2Cases<f16>(true)))
                     ...
                     //    // If y is +/-0 and x is negative or -0, +/-PI is returned
                     //    C({0.0_a, -0.0_a}, kPi<AFloat>, true),

                     //    // If y is +/-0 and x is positive or +0, +/-0 is returned
                     //    C({0.0_a, 0.0_a}, 0.0_a, true),

                     //    // If y is +/-INF and x is finite, +/-PI/2 is returned
                     //    C({kInf<AFloat>, 0.0_a}, kPiOver2<AFloat>, true),
                     //    C({-kInf<AFloat>, 0.0_a}, kPiOver2<AFloat>, true),

                     //    // If y is +/-INF and x is -INF, +/-3PI/4 is returned
                     //    C({kInf<AFloat>, -kInf<AFloat>}, k3PiOver4<AFloat>, true),
                     //    C({-kInf<AFloat>, -kInf<AFloat>}, k3PiOver4<AFloat>, true),

                     //    // If y is +/-INF and x is +INF, +/-PI/4 is returned
                     //    C({kInf<AFloat>, kInf<AFloat>}, kPiOver4<AFloat>, true),
                     //    C({-kInf<AFloat>, kInf<AFloat>}, kPiOver4<AFloat>, true),

                     //    // If x is +/-0 and y is negative, -PI/2 is returned
                     //    C({-1.0_a, 0.0_a}, -kPiOver2<AFloat>),
                     //    C({-1.0_a, -0.0_a}, -kPiOver2<AFloat>),
                     ...
                     //    // If x is -INF and y is finite and positive, +PI is returned
                     //    C({0.0_a, -kInf<AFloat>}, kPi<AFloat>),

                     //    // If x is -INF and y is finite and negative, -PI is returned
                     //    C({-0.0_a, -kInf<AFloat>}, -kPi<AFloat>),

                     //    // If x is +INF and y is finite and positive, +0 is returned
                     //    C({0.0_a, kInf<AFloat>}, 0.0_a),

                     //    // If x is +INF and y is finite and negative, -0 is returned
                     //    C({-0.0_a, kInf<AFloat>}, -0.0_a),

                     //    // If either x is NaN or y is NaN, NaN is returned
                     //    C({kNaN<AFloat>, 0.0_a}, kNaN<AFloat>),
                     //    C({0.0_a, kNaN<AFloat>}, kNaN<AFloat>),
                     //    C({kNaN<AFloat>, kNaN<AFloat>}, kNaN<AFloat>),
...
INSTANTIATE_TEST_SUITE_P(Clamp,
                         ...
                         testing::Combine(testing::Values(sem::BuiltinType::kClamp),
                                          testing::ValuesIn({// AFloat
                                                             C({0.0_a, 0.0_a, 0.0_a}, 0.0_a)})));
