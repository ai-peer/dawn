...
TEST_P(ResolverConstEvalBinaryOpTest, Test) {
    ...
    std::visit(
        [&](auto&& expected) {
            using T = std::decay_t<decltype(expected)>::ElementType;
            ...
            GlobalConst("E", nullptr, expected_expr);
            ...
            ForEachElemPair(value, expected_value, [&](auto* a, auto* b) {
                using T = std::decay_t<decltype(expected)>::ElementType;
                if constexpr (IsInteger<UnwrapNumber<T>>) {
