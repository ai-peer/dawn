...
// template <typename T>
Case C(std::initializer_list<Types> args, Types result, bool result_pos_or_neg = false) {
    return Case{std::move(args), std::move(result), result_pos_or_neg};
}

/// Convenience overload that creates a Case with just scalars
using ScalarTypes = std::variant<AInt, AFloat, u32, i32, f32, f16>;
Case C(std::initializer_list<ScalarTypes> sargs,
       ScalarTypes sresult,
       bool result_pos_or_neg = false) {
    utils::Vector<Types, 8> args;
    for (auto& sa : sargs) {
...
std::vector<Case> Atan2Cases() {
    std::vector<Case> cases = {
        ...
        // Vector tests
        C({Vec(T(0.0), T(0.0)), Vec(-T(0.0), T(0.0))}, Vec(kPi<T>, T(0.0)), true),
    ...
    if constexpr (!finite_only) {
        std::vector<Case> non_finite_cases = {
            ...
            C({Vec(T::Inf(), -T::Inf(), T::Inf(), -T::Inf()),  //
              ...
              Vec(kPiOver2<T>, kPiOver2<T>, k3PiOver4<T>, k3PiOver4<T>), true),
        };
