...
TEST_F(ResolverControlBlockValidationTest, SwitchWithTwoDefault_OneInCase_Fail) {
    ...
    auto* block = Block(Decl(var),                                                            //
                        Switch("a",                                                           //
                               Case(utils::Vector{CaseSelector(Expr(1_i)), CaseSelector()}),  //
                               DefaultCase(Source{{12, 34}})));
...
TEST_F(ResolverControlBlockValidationTest, SwitchWithTwoDefault_SameCase) {
    ...
    auto* block = Block(Decl(var),   //
                        Switch("a",  //
                               Case(utils::Vector{CaseSelector(), CaseSelector(Expr(1_i)),
                                                  CaseSelector(Source{{12, 34}})})));
...
TEST_F(ResolverControlBlockValidationTest, SwitchWithTwoDefault_DifferentMultiCase) {
    ...
    auto* block =
        Block(Decl(var),   //
              Switch("a",  //
                     Case(utils::Vector{CaseSelector(Expr(1_i)), CaseSelector()}),
                     Case(utils::Vector{CaseSelector(Source{{12, 34}}), CaseSelector(Expr(2_i))})));
...
TEST_F(ResolverControlBlockValidationTest, UnreachableCode_break) {
    ...
    WrapInFunction(                                                                //
        Block(Switch(1_i,                                                          //
                     Case(CaseSelector(Expr(1_i)), Block(decl_z, brk, assign_z)),  //
                     DefaultCase())));
...
TEST_F(ResolverControlBlockValidationTest, UnreachableCode_break_InBlocks) {
    ...
    WrapInFunction(Loop(Block(
        Switch(1_i,  //
               Case(CaseSelector(Expr(1_i)), Block(decl_z, Block(Block(Block(brk))), assign_z)),
               DefaultCase()),  //
        Break())));
...
TEST_F(ResolverControlBlockValidationTest, SwitchConditionTypeMustMatchSelectorType2_Fail) {
    ...
    auto* block = Block(Decl(var), Switch("a",                                              //
                                          Case(CaseSelector(Source{{12, 34}}, Expr(1_u))),  //
                                          DefaultCase()));
    WrapInFunction(block);
...
TEST_F(ResolverControlBlockValidationTest, SwitchConditionTypeMustMatchSelectorType_Fail) {
    ...
    auto* block = Block(Decl(var),                                                //
                        Switch("a",                                               //
                               Case(CaseSelector(Source{{12, 34}}, Expr(-1_i))),  //
                               DefaultCase()));
    WrapInFunction(block);
...
TEST_F(ResolverControlBlockValidationTest, NonUniqueCaseSelectorValueUint_Fail) {
    ...
    auto* block = Block(Decl(var),   //
                        Switch("a",  //
                               Case(CaseSelector(Expr(0_u))),
                               Case(utils::Vector{
                                   CaseSelector(Source{{12, 34}}, Expr(2_u)),
                                   CaseSelector(Expr(3_u)),
                                   CaseSelector(Source{{56, 78}}, Expr(2_u)),
                               }),
                               DefaultCase()));
    WrapInFunction(block);
...
TEST_F(ResolverControlBlockValidationTest, NonUniqueCaseSelectorValueSint_Fail) {
    ...
    auto* block = Block(Decl(var),   //
                        Switch("a",  //
                               Case(CaseSelector(Source{{12, 34}}, Expr(-10_i))),
                               Case(utils::Vector{
                                   CaseSelector(Expr(0_i)),
                                   CaseSelector(Expr(1_i)),
                                   CaseSelector(Expr(2_i)),
                                   CaseSelector(Source{{56, 78}}, Expr(-10_i)),
                               }),
                               DefaultCase()));
    WrapInFunction(block);
...
TEST_F(ResolverControlBlockValidationTest, NonUniqueCaseSelectorSame_Case_Expression_Fail) {
    ...
    auto* block = Block(Decl(var),   //
                        Switch("a",  //
                               Case(utils::Vector{CaseSelector(Source{{56, 78}}, Add(5_i, 5_i)),
                                                  CaseSelector(Source{{12, 34}}, Expr(10_i))}),
                               DefaultCase()));
    WrapInFunction(block);
...
TEST_F(ResolverControlBlockValidationTest, Switch_OverrideCondition_Fail) {
    ...
    auto* block = Block(Decl(var),   //
                        Switch("a",  //
                               Case(CaseSelector(Source{{12, 34}}, Expr("b"))), DefaultCase()));
    WrapInFunction(block);
