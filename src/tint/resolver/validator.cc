...
Validator::Validator(
    ...
    SemHelper& sem,
    const type::Extensions& enabled_extensions,
    ...
    diagnostic_filters_.Set(type::DiagnosticRule::kDerivativeUniformity,
                            type::DiagnosticSeverity::kError);
    diagnostic_filters_.Set(type::DiagnosticRule::kChromiumUnreachableCode,
...
bool Validator::InterpolateAttribute(const ast::InterpolateAttribute* attr,
    ...
    if (type->is_integer_scalar_or_vector() && attr->type != type::InterpolationType::kFlat) {
        AddError("interpolation type must be 'flat' for integral user-defined IO types",
                 attr->source);
        return false;
    }

    if (attr->type == type::InterpolationType::kFlat &&
        attr->sampling != type::InterpolationSampling::kUndefined) {
...
bool Validator::EntryPoint(const sem::Function* func, ast::PipelineStage stage) const {
    ...
    utils::Hashset<type::BuiltinValue, 4> builtins;
            ...
            if (invariant_attribute) {
                ...
                if (pipeline_io_attribute) {
                    if (auto* builtin = pipeline_io_attribute->As<ast::BuiltinAttribute>()) {
                        has_position = (builtin->builtin == type::BuiltinValue::kPosition);
                    }
                }
                if (!has_position) {
                    AddError(
    ...
    if (decl->PipelineStage() == ast::PipelineStage::kVertex &&
        !builtins.Contains(type::BuiltinValue::kPosition)) {
        // Check module-scope variables, as the SPIR-V sanitizer generates these.
        bool found = false;
        for (auto* global : func->TransitivelyReferencedGlobals()) {
            if (auto* builtin =
                    ast::GetAttribute<ast::BuiltinAttribute>(global->Declaration()->attributes)) {
                if (builtin->builtin == type::BuiltinValue::kPosition) {
                    found = true;
                    break;
                }
            }
...
bool Validator::Statements(utils::VectorRef<const ast::Statement*> stmts) const {
    for (auto* stmt : stmts) {
        if (!sem_.Get(stmt)->IsReachable()) {
            if (!AddDiagnostic(type::DiagnosticRule::kChromiumUnreachableCode,
...
bool Validator::RequiredExtensionForBuiltinFunction(const sem::Call* call) const {
    ...
    if (extension == type::Extension::kUndefined) {
        return true;
    }
...
bool Validator::CheckF16Enabled(const Source& source) const {
    ...
    if (!enabled_extensions_.Contains(type::Extension::kF16)) {
...
bool Validator::CheckTypeAccessAddressSpace(
    ...
    if (address_space == type::AddressSpace::kPushConstant &&
        !enabled_extensions_.Contains(type::Extension::kChromiumExperimentalPushConstant) &&
