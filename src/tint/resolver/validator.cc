...
bool Validator::LocalVariable(const sem::Variable* local) const {
    ...
    if (!ErrorIfArrayWithOverrideCount(
            local->Type(), decl->type ? decl->type->source : decl->constructor->source)) {
        return false;
    }
    return Switch(
        decl,  //
...
bool Validator::GlobalVariable(
    ...
    if (global->StorageClass() != ast::StorageClass::kWorkgroup &&
        !ErrorIfArrayWithOverrideCount(
            global->Type(), decl->type ? decl->type->source : decl->constructor->source)) {
        return false;
    }
    bool ok = Switch(
        decl,  //
...
bool Validator::Array(const sem::Array* arr, const Source& el_source) const {
    ...
    if (!ErrorIfArrayWithOverrideCount(el_ty, el_source)) {
        return false;
    }
...
bool Validator::Structure(const sem::Struct* str, ast::PipelineStage stage) const {
    ...
    for (auto* member : str->Members()) {
        if (auto* r = member->Type()->As<sem::Array>()) {
            ...
            if (!ErrorIfArrayWithOverrideCount(member->Type(),
...
bool Validator::ErrorIfArrayWithOverrideCount(const sem::Type* ty, const Source& source) const {
