    ...
    if (auto* str = store_ty->As<sem::Struct>()) {
        for (size_t i = 0; i < str->Members().size(); ++i) {
            ...
            if (m->Offset() % required_align != 0) {
                AddError(
                    utils::Join("the offset of a struct member of type",
                                ...
                                utils::Quote(member_name_of(m)), "is currently at offset",
                                utils::Post(m->Offset(), "."), "Consider setting",
                                utils::PrePost("@align(", required_align, ")"), "on this member"),
