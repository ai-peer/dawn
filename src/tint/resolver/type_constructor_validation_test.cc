...
struct ElementParams {
    ...
    builder::sem_type_func_ptr create_element_sem_type;
    builder::ast_type_func_ptr create_column_ast_type;
    builder::ast_expr_func_ptr create_column_ast_value;
    builder::sem_type_func_ptr create_column_sem_type;
};
...
constexpr MatrixParams MatrixParamsFor() {
    return MatrixParams{
        ...
        DataType<tint::resolver::builder::vec<R, T>>::Expr,
        DataType<tint::resolver::builder::vec<R, T>>::Sem,
...
TEST_P(MatrixConstructorTest, InferElementTypeFromScalars) {
    ...
    for (uint32_t i = 0; i < param.rows * param.columns; i++) {
        args.push_back(
            Construct(param.create_element_ast_type(*this), Expr(static_cast<AFloat>(i))));
    }
