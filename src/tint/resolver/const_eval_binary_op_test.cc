...
void ValidateAnd(const sem::Info& sem, const ast::BinaryExpression* binary) {
    auto* lhs = binary->lhs;
    auto* rhs = binary->rhs;
...
void ValidateOr(const sem::Info& sem, const ast::BinaryExpression* binary) {
    auto* lhs = binary->lhs;
    auto* rhs = binary->rhs;
...
// array/struct init does not fail.
...
// NOTE: Cannot demonstrate short-circuiting an invalid swizzle as const eval of swizzle
// does not fail.
