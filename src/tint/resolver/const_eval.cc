...
/// @returns the number of consecutive leading bits in `e` set to `bitValueToCount`.
template <typename T>
auto CountLeadingBits(T e, T bitValueToCount) -> std::make_unsigned_t<T> {
    using UT = std::make_unsigned_t<T>;
    constexpr UT kNumBits = sizeof(UT) * 8;
    constexpr UT kLeftMost = UT{1} << (kNumBits - 1);
    const UT b = static_cast<UT>(bitValueToCount);
...
/// @returns the number of consecutive trailing bits set to `bitValueToCount` in `e`
template <typename T>
auto CountTrailingBits(T e, T bitValueToCount) -> std::make_unsigned_t<T> {
    using UT = std::make_unsigned_t<T>;
    constexpr UT kNumBits = sizeof(UT) * 8;
    ...
    auto transform = [&](const sem::Constant* c0) {
        auto create = [&](auto e) {
            ...
            if (e == T{0}) {
                ...
                UT count = CountTrailingBits(T{e}, T{0});
                UT pos = count;
                result = NumberT(pos);
            }
...
ConstEval::Result ConstEval::insertBits(const sem::Type* ty,
                                        ...
                                        const Source&) {
    auto transform = [&](const sem::Constant* c0, const sem::Constant* c1) {
        auto create = [&](auto in_e, auto in_newbits) {
            ...
            UT offset = static_cast<UT>(in_offset);
            UT count = static_cast<UT>(in_count);

            constexpr UT w = sizeof(UT) * 8;
            UT o = std::min(offset, w);
            UT c = std::min(count, w - o);
            ...
            if (c == UT{0}) {
                ...
                auto r = e;                // Start with 'e' as result
                r = r & ~mask;             // Zero the bits in 'e' we're overwriting
                r = r | (newbits & mask);  // Now overwrite with bits from 'newbits'
                result = NumberT{r};
            }
