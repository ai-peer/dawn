    ...
    auto transform = [&](const sem::Constant* c0, const sem::Constant* c1) {
        auto create = [&](auto e1, auto e2) -> ImplResult {
            ...
            auto signed_shift_right = [&] {
                // In C++, right shift of a signed negative number is
                // implementation-defined. Although in most implementations, this performs
                // arithmetic right shift, so that the result remains negative, we do it
                // manually to ensure it works correctly on all implementations.
                const UT msb = UT{1} << (bit_width - 1);
                UT top_bits = 0;
                if (e1u & msb) {
                    // Set e2 + 1 bits to 1
                    UT num_shift_bits_mask = ((1 << e2u) - 1);
                    top_bits = (num_shift_bits_mask << (bit_width - e2u - 1)) | msb;
                }
                return static_cast<T>(e1u >> e2u | top_bits);
            };
