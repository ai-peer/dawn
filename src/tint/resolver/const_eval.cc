...
ConstEval::Result ConstEval::Bitcast(const type::Type* ty,
                                     ...
                                     const Source& source) {
    // Target type
    auto [dst_el_ty, dst_count] = ty->Elements(ty->DeepestElement(), 1u);
    // Source type
    auto [src_el_ty, src_count] = value->Type()->Elements(value->Type()->DeepestElement(), 1u);

    TINT_ASSERT(Resolver, dst_count * dst_el_ty->Size() == src_count * src_el_ty->Size());
    uint32_t totalBitWidth = dst_count * dst_el_ty->Size();
    // Buffer holding the bits from source value, result value reinterptered from it.
    utils::Vector<std::byte, 16> buffer;
    buffer.Reserve(totalBitWidth);
    ...
    // Push bits from source value into the buffer.
    auto PushSrcElementBits = [&](const constant::Value* element) {
        auto Push32Bits = [&](uint32_t v) {
            buffer.Push(static_cast<std::byte>(v & 0xffu));
            buffer.Push(static_cast<std::byte>((v >> 8) & 0xffu));
            buffer.Push(static_cast<std::byte>((v >> 16) & 0xffu));
            buffer.Push(static_cast<std::byte>((v >> 24) & 0xffu));
        };
        auto Push16Bits = [&](uint16_t v) {
            buffer.Push(static_cast<std::byte>(v & 0xffu));
            buffer.Push(static_cast<std::byte>((v >> 8) & 0xffu));
        };
        Switch(
    ...
    if (src_count == 1) {
        PushSrcElementBits(value);
    } else {
        for (size_t i = 0; i < src_count; i++) {
            PushSrcElementBits(value->Index(i));
        }
    }
    ...
    // Reinterprete the buffer bits as destination element and push the result into the vector.
    auto PushDstElement = [&](size_t offset) {
        uint32_t v;
        if (dst_el_ty->Size() == 4) {
    ...
    TINT_ASSERT(Resolver, (buffer.Length() == totalBitWidth));
    for (size_t i = 0; i < dst_count; i++) {
        PushDstElement(i * dst_el_ty->Size());
    }
