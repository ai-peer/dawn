...
ConstEval::Result ConstEval::pack2x16float(const sem::Type* ty,
    ...
    auto convert = [&](f32 val) -> utils::Result<uint32_t> {
        ...
        if (!conv) {
            AddError("Value outside range of f16", source);
            return utils::Failure;
        }
...
ConstEval::Result ConstEval::unpack2x16float(const sem::Type* ty,
    ...
    for (size_t i = 0; i < 2; ++i) {
        auto val = CheckedConvert<f32>(f16::FromBits(uint16_t((e >> (16 * i)) & 0x0000'ffff)));
        if (!val) {
            AddError("Conversion from f16 to f32 failed", source);
            return utils::Failure;
        }
