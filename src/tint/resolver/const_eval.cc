...
#define T(SEM_TYPE, NUM_TYPE)                              \
    if constexpr (traits::HasType<SEM_TYPE, TypesTuple>) { \
        if (c->Type()->Is<SEM_TYPE>()) {                   \
            return f(c->As<NUM_TYPE>());                   \
        }                                                  \
    }                                                      \
    (void)0

    T(sem::AbstractInt, AInt);
    T(sem::AbstractFloat, AFloat);
    T(sem::I32, i32);
    T(sem::U32, u32);
    T(sem::F32, f32);
    T(sem::F16, f16);
...
#undef T
    ...
    return static_cast<decltype(f(c->As<AInt>()))>(nullptr);
}
...
const sem::Constant* ConstEval::OpMinus(const sem::Type* /*ty*/,
                                        sem::Expression const* const* args,
                                        size_t /*num_args*/) {
    return TransformElements(builder, args[0]->ConstantValue(), [&](const sem::Constant* c) {
