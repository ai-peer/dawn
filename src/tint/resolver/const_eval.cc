...
template <typename T>
ImplResult scalar_convert(const Scalar<T>* scalar,
                          ProgramBuilder& builder,
                          const type::Type* target_ty,
                          const Source& source);
ImplResult splat_convert(const Splat* splat,
                         ProgramBuilder& builder,
                         const type::Type* target_ty,
                         const Source& source);
ImplResult composite_convert(const Composite* composite,
                             ProgramBuilder& builder,
                             const type::Type* target_ty,
                             const Source& source);

ImplResult convert_internal(const constant::Constant* c,
                            ProgramBuilder& builder,
                            const type::Type* target_ty,
                            const Source& source) {
    return Switch(
        ...
        [&](const Scalar<tint::AFloat>* val) {
            return scalar_convert(val, builder, target_ty, source);
        },
        ...
        [&](const Scalar<bool>* val) { return scalar_convert(val, builder, target_ty, source); },
        [&](const Splat* val) { return splat_convert(val, builder, target_ty, source); },
        [&](const Composite* val) { return composite_convert(val, builder, target_ty, source); });
}
...
ImplResult composite_convert(const Composite* composite,
    ...
    for (auto* el : composite->elements) {
        auto conv_el = convert_internal(el, builder, target_el_ty(conv_els.Length()), source);
        if (!conv_el) {
            return utils::Failure;
        }
        if (!conv_el.Get()) {
...
ConstEval::Result ConstEval::Convert(const type::Type* target_ty,
    ...
    return convert_internal(value, builder, target_ty, source);
}
