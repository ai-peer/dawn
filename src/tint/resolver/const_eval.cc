...
ConstEval::Result SplatConvert(const constant::Splat* splat,
    ...
    auto conv_el =
        ConvertInternal(splat->el, builder, type::Type::ElementOf(target_ty), source, soft_errors);
    if (!conv_el) {
        return utils::Failure;
    }
    if (!conv_el.Get()) {
...
ConstEval::Result CompositeConvert(const constant::Composite* composite,
    ...
    for (auto* el : composite->elements) {
        auto conv_el =
            ConvertInternal(el, builder, target_el_ty(conv_els.Length()), source, soft_errors);
        if (!conv_el) {
            return utils::Failure;
        }
        if (!conv_el.Get()) {
...
ConstEval::Result ConvertInternal(const constant::Value* c,
    ...
    return Switch(
        ...
        [&](const constant::Scalar<tint::AFloat>* val) {
            return ScalarConvert(val, builder, target_ty, source, soft_errors);
        },
        ...
        [&](const constant::Splat* val) {
            return SplatConvert(val, builder, target_ty, source, soft_errors);
        },
        [&](const constant::Composite* val) {
            return CompositeConvert(val, builder, target_ty, source, soft_errors);
        });
}
...
ConstEval::ConstEval(ProgramBuilder& b, bool soft_errors /* = false */)
    : builder(b), soft_errors_(soft_errors) {}
...
auto ConstEval::SqrtFunc(const Source& source, const type::Type* elem_ty) {
    return [=](auto v) -> ConstEval::Result {
        if (auto r = Sqrt(source, v)) {
            return CreateScalar(builder, source, elem_ty, r.Get(), soft_errors_);
        }
        return utils::Failure;
    };
}
...
ConstEval::Result ConstEval::Length(const Source& source,
    ...
    if (vec_ty == nullptr) {
        auto create = [&](auto e) {
            ...
            return CreateScalar(builder, source, ty, NumberT{std::abs(e)}, soft_errors_);
        };
...
ConstEval::Result ConstEval::Literal(const type::Type* ty, const ast::LiteralExpression* literal) {
    ...
    return Switch(
        ...
        [&](const ast::BoolLiteralExpression* lit) {
            return CreateScalar(builder, source, ty, lit->value, soft_errors_);
        },
        [&](const ast::IntLiteralExpression* lit) -> ConstEval::Result {
            switch (lit->suffix) {
                case ast::IntLiteralExpression::Suffix::kNone:
                    return CreateScalar(builder, source, ty, AInt(lit->value), soft_errors_);
                case ast::IntLiteralExpression::Suffix::kI:
                    return CreateScalar(builder, source, ty, i32(lit->value), soft_errors_);
                case ast::IntLiteralExpression::Suffix::kU:
                    return CreateScalar(builder, source, ty, u32(lit->value), soft_errors_);
            }
            return nullptr;
        },
        [&](const ast::FloatLiteralExpression* lit) -> ConstEval::Result {
            switch (lit->suffix) {
                case ast::FloatLiteralExpression::Suffix::kNone:
                    return CreateScalar(builder, source, ty, AFloat(lit->value), soft_errors_);
                case ast::FloatLiteralExpression::Suffix::kF:
                    return CreateScalar(builder, source, ty, f32(lit->value), soft_errors_);
                case ast::FloatLiteralExpression::Suffix::kH:
                    return CreateScalar(builder, source, ty, f16(lit->value), soft_errors_);
            }
            return nullptr;
        });
}
        ...
        auto create = [&](auto e) {
            return Switch(
                ...
                [&](const type::U32*) {  //
                    ...
                    return CreateScalar(builder, source, el_ty, r, soft_errors_);
                },
                [&](const type::I32*) {  //
...
ConstEval::Result ConstEval::OpComplement(const type::Type* ty,
    ...
    auto transform = [&](const constant::Value* c) {
        auto create = [&](auto i) {
            return CreateScalar(builder, source, c->Type(), decltype(i)(~i.value), soft_errors_);
        };
    ...
    auto transform = [&](const constant::Value* c) {
        auto create = [&](auto i) {
            ...
            if constexpr (std::is_integral_v<T>) {
                ...
                return CreateScalar(builder, source, c->Type(), decltype(i)(v), soft_errors_);
            } else {
...
ConstEval::Result ConstEval::OpNot(const type::Type* ty,
    ...
    auto transform = [&](const constant::Value* c) {
        auto create = [&](auto i) {
            return CreateScalar(builder, source, c->Type(), decltype(i)(!i), soft_errors_);
        };
...
ConstEval::Result ConstEval::OpLogicalOr(const type::Type* ty,
    ...
    // technically only return the value of the rhs.
    return CreateScalar(builder, source, ty, args[1]->ValueAs<bool>(), soft_errors_);
}
...
ConstEval::Result ConstEval::abs(const type::Type* ty,
    ...
    auto transform = [&](const constant::Value* c0) {
        auto create = [&](auto e) {
            ...
            return CreateScalar(builder, source, c0->Type(), result, soft_errors_);
        };
...
ConstEval::Result ConstEval::all(const type::Type* ty,
                                 utils::VectorRef<const constant::Value*> args,
                                 const Source& source) {
    return CreateScalar(builder, source, ty, !args[0]->AnyZero(), soft_errors_);
}

ConstEval::Result ConstEval::any(const type::Type* ty,
                                 utils::VectorRef<const constant::Value*> args,
                                 const Source& source) {
    return CreateScalar(builder, source, ty, !args[0]->AllZero(), soft_errors_);
}
    ...
    auto transform = [&](const constant::Value* c0, const constant::Value* c1) {
        auto create = [&](auto i, auto j) {
            return CreateScalar(builder, source, c0->Type(),
                                decltype(i)(std::atan2(i.value, j.value)), soft_errors_);
        };
...
ConstEval::Result ConstEval::countLeadingZeros(const type::Type* ty,
    ...
    auto transform = [&](const constant::Value* c0) {
        auto create = [&](auto e) {
            ...
            return CreateScalar(builder, source, c0->Type(), NumberT(count), soft_errors_);
        };
...
ConstEval::Result ConstEval::degrees(const type::Type* ty,
    ...
    auto transform = [&](const constant::Value* c0) {
        auto create = [&](auto e) -> ConstEval::Result {
            ...
            return CreateScalar(builder, source, c0->Type(), result.Get(), soft_errors_);
        };
        return Dispatch_fa_f32_f16(create, c0);
    };
    return TransformElements(builder, ty, transform, args[0]);
...
ConstEval::Result ConstEval::exp(const type::Type* ty,
    ...
    auto transform = [&](const constant::Value* c0) {
        auto create = [&](auto e0) -> ConstEval::Result {
            ...
            return CreateScalar(builder, source, c0->Type(), val, soft_errors_);
        };
        return Dispatch_fa_f32_f16(create, c0);
    };
    return TransformElements(builder, ty, transform, args[0]);
...
ConstEval::Result ConstEval::fma(const type::Type* ty,
    ...
    auto transform = [&](const constant::Value* c1, const constant::Value* c2,
        ...
        auto create = [&](auto e1, auto e2, auto e3) -> ConstEval::Result {
            ...
            return CreateScalar(builder, source, c1->Type(), val.Get(), soft_errors_);
        };
...
ConstEval::Result ConstEval::fract(const type::Type* ty,
    ...
    auto transform = [&](const constant::Value* c1) {
        auto create = [&](auto e) -> ConstEval::Result {
            ...
            return CreateScalar(builder, source, c1->Type(), NumberT{r}, soft_errors_);
        };
...
ConstEval::Result ConstEval::inverseSqrt(const type::Type* ty,
    ...
    auto transform = [&](const constant::Value* c0) {
        auto create = [&](auto e) -> ConstEval::Result {
            ...
            return CreateScalar(builder, source, c0->Type(), div.Get(), soft_errors_);
        };
        return Dispatch_fa_f32_f16(create, c0);
    };
...
ConstEval::Result ConstEval::ldexp(const type::Type* ty,
    ...
    auto transform = [&](const constant::Value* c1, size_t index) {
        auto create = [&](auto e1) -> ConstEval::Result {
            ...
            return CreateScalar(builder, source, target_ty, E1Type{r}, soft_errors_);
        };
        return Dispatch_fa_f32_f16(create, c1);
    };
...
ConstEval::Result ConstEval::log(const type::Type* ty,
    ...
    auto transform = [&](const constant::Value* c0) {
        auto create = [&](auto v) -> ConstEval::Result {
            ...
            return CreateScalar(builder, source, c0->Type(), NumberT(std::log(v)), soft_errors_);
        };
        return Dispatch_fa_f32_f16(create, c0);
    };
    return TransformElements(builder, ty, transform, args[0]);
...
ConstEval::Result ConstEval::log2(const type::Type* ty,
    ...
    auto transform = [&](const constant::Value* c0) {
        auto create = [&](auto v) -> ConstEval::Result {
            ...
            return CreateScalar(builder, source, c0->Type(), NumberT(std::log2(v)), soft_errors_);
        };
        return Dispatch_fa_f32_f16(create, c0);
    };
    return TransformElements(builder, ty, transform, args[0]);
...
ConstEval::Result ConstEval::mix(const type::Type* ty,
    ...
    auto transform = [&](const constant::Value* c0, const constant::Value* c1, size_t index) {
        auto create = [&](auto e1, auto e2) -> ConstEval::Result {
            ...
            return CreateScalar(builder, source, c0->Type(), r.Get(), soft_errors_);
        };
        return Dispatch_fa_f32_f16(create, c0, c1);
    };
    ...
    auto transform_fract = [&](const constant::Value* c) {
        auto create = [&](auto e) {
            return CreateScalar(builder, source, c->Type(),
                                decltype(e)(e.value - std::trunc(e.value)), soft_errors_);
        };
...
ConstEval::Result ConstEval::pack2x16float(const type::Type* ty,
    ...
    return CreateScalar(builder, source, ty, ret, soft_errors_);
}
...
ConstEval::Result ConstEval::pow(const type::Type* ty,
    ...
    auto transform = [&](const constant::Value* c0, const constant::Value* c1) {
        auto create = [&](auto e1, auto e2) -> ConstEval::Result {
            ...
            return CreateScalar(builder, source, c0->Type(), *r, soft_errors_);
        };
        return Dispatch_fa_f32_f16(create, c0, c1);
    };
    return TransformElements(builder, ty, transform, args[0], args[1]);
...
ConstEval::Result ConstEval::reflect(const type::Type* ty,
    ...
    auto calculate = [&]() -> ConstEval::Result {
        ...
        auto mul2 = [&](auto v) -> ConstEval::Result {
            using NumberT = decltype(v);
            return CreateScalar(builder, source, el_ty, NumberT{NumberT{2} * v}, soft_errors_);
        };
...
ConstEval::Result ConstEval::refract(const type::Type* ty,
    ...
    auto compute_k = [&](auto e3, auto dot_e2_e1) -> ConstEval::Result {
        ...
        return CreateScalar(builder, source, el_ty, r.Get(), soft_errors_);
    };
...
ConstEval::Result ConstEval::reverseBits(const type::Type* ty,
    ...
    auto transform = [&](const constant::Value* c0) {
        auto create = [&](auto in_e) -> ConstEval::Result {
            ...
            return CreateScalar(builder, source, c0->Type(), NumberT{r}, soft_errors_);
        };
        return Dispatch_iu32(create, c0);
    };
    return TransformElements(builder, ty, transform, args[0]);
...
ConstEval::Result ConstEval::unpack2x16float(const type::Type* ty,
    ...
    for (size_t i = 0; i < 2; ++i) {
        ...
        auto el = CreateScalar(builder, source, inner_ty, val.Get(), soft_errors_);
        if (!el) {
            return el;
        }
        els.Push(el.Get());
...
ConstEval::Result ConstEval::unpack2x16snorm(const type::Type* ty,
    ...
    for (size_t i = 0; i < 2; ++i) {
        ...
        auto el = CreateScalar(builder, source, inner_ty, val, soft_errors_);
        if (!el) {
            return el;
        }
        els.Push(el.Get());
...
ConstEval::Result ConstEval::quantizeToF16(const type::Type* ty,
    ...
    auto transform = [&](const constant::Value* c) -> ConstEval::Result {
        ...
        return CreateScalar(builder, source, c->Type(), conv.Get(), soft_errors_);
    };
    return TransformElements(builder, ty, transform, args[0]);
}
...
ConstEval::Result ConstEval::Convert(const type::Type* target_ty,
    ...
    return ConvertInternal(value, builder, target_ty, source, soft_errors_);
}
