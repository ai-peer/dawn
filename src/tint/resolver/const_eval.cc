        ...
        bool ok = Switch(
            ...
            [&](const constant::Splat* splat) {
                ...
                if (auto* str = convert->target_ty->As<type::Struct>()) {
                    ...
                    target_el_ty = convert->target_ty->Element({convert->target_ty}).type;
                }
            ...
            [&](const constant::Composite* composite) {
                ...
                if (auto* str = convert->target_ty->As<type::Struct>()) {
                    ...
                    auto* el_ty = convert->target_ty->Element({convert->target_ty}).type;
                    for (size_t i = 0; i < el_count; i++) {
...
ConstEval::Result TransformElements(ProgramBuilder& builder,
                                    ...
                                    size_t index,
                                    CONSTANTS&&... cs) {
    auto [el_ty, n] = First(cs...)->Type()->Element();
    if (!el_ty) {
    ...
    auto* composite_el_ty = composite_ty->Element({composite_ty}).type;
...
ConstEval::Result TransformBinaryElements(ProgramBuilder& builder,
    ...
    uint32_t n0 = c0->Type()->Element({nullptr, 1}).count;
    uint32_t n1 = c1->Type()->Element({nullptr, 1}).count;
    ...
    const auto* element_ty = composite_ty->Element({composite_ty}).type;
...
ConstEval::Result ConstEval::Index(const type::Type* ty,
    ...
    uint32_t el_count = obj_expr->Type()->UnwrapRef()->Element().count;
