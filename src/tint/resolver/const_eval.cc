...
utils::Result<NumberT> ConstEval::VecLength(const Source& source,
                                            ...
                                            NumberT z,
                                            NumberT w) {
    auto x2 = Mul(source, x, x);
    if (!x2) {
        return utils::Failure;
    }
    auto y2 = Mul(source, y, y);
    if (!y2) {
        return utils::Failure;
    }
    auto z2 = Mul(source, z, z);
    if (!z2) {
        return utils::Failure;
    }
    auto w2 = Mul(source, w, w);
    if (!w2) {
        return utils::Failure;
    }
    auto sum = Add(source, x2.Get(), y2.Get());
    if (!sum) {
        return utils::Failure;
    }
    sum = Add(source, sum.Get(), z2.Get());
    if (!sum) {
        return utils::Failure;
    }
    sum = Add(source, sum.Get(), w2.Get());
    if (!sum) {
        return utils::Failure;
    }
    return NumberT{std::sqrt(sum.Get())};
}
