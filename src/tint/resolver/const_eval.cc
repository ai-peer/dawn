...
ConstEval::Result ConstEval::select(const sem::Type* ty,
                                    utils::VectorRef<const sem::Constant*> args,
                                    const Source& /*source*/) {
    auto transform = [&](const sem::Constant* c0, const sem::Constant* c1,
                         const sem::Constant* c2) {
        auto create = [&](auto f, auto t, auto cond) -> ImplResult {
            if constexpr (std::is_same_v<decltype(f), decltype(t)> &&
                          std::is_same_v<decltype(cond), bool>) {
                return CreateElement(builder, sem::Type::DeepestElementOf(ty), cond ? t : f);
            } else {
                return nullptr;
            }
        };
        return Dispatch_any(create, c0, c1, c2);
    };
