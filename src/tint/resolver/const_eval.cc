...
ConstEval::Result ConstEval::Bitcast(const type::Type* ty,
    ...
    auto push_src_element_bits = [&, &src_el_ty = src_el_ty](const constant::Value* element) {
    ...
    auto push_dst_element = [&, &dst_el_ty = dst_el_ty](size_t offset) {
        uint32_t v;
        if (dst_el_ty->Size() == 4) {
    ...
    for (size_t i = 0; i < dst_count; i++) {
        push_dst_element(i * dst_el_ty->Size());
    }
