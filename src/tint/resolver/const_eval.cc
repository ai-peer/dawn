...
ConstEval::Result ConstEval::round(const sem::Type* ty,
    ...
    auto transform = [&](const sem::Constant* c0) {
        auto create = [&](auto e) {
            ...
            auto val = std::abs(std::modf(e.value, &(integral.value)));
            ...
            if (val == NumberT(0.5)) {
                ...
                // case is `x` and `x + 1` but in the ngative case is `x - 1` and `x`.
                if (std::signbit(integral)) {
                    integral = std::abs(integral - 1);
                }
                if (uint64_t(integral) % 2 == 0) {
