...
/*
namespace materialize_abstract_with_index {

using MaterializeAbstractWithIndex = resolver::ResolverTest;

TEST_F(MaterializeAbstractWithIndex, Vector) {
    // fn f() {
    //   var i = 1i;
    //   var v = vec(1, 2)[i];
    // }
    auto* i = Var("i", nullptr, Expr(1_i));
    auto* c = Var("v", nullptr, IndexAccessor(Construct(ty.vec(nullptr, 2u), 1._a, 2._a), i));
    WrapInFunction(Decl(i), Decl(c));
    ASSERT_TRUE(r()->Resolve()) << r()->error();
    auto* sem = Sem().Get(c);
    EXPECT_TRUE(sem->Is<sem::Materialize>());
}

TEST_F(MaterializeAbstractWithIndex, AIntToStructLetCtor) {
    Structure("S", {Member("a", ty.i32())});
    WrapInFunction(Decl(Let("v", ty.type_name("S"), Expr(Source{{12, 34}}, 1_a))));
    ASSERT_FALSE(r()->Resolve());
    EXPECT_THAT(
