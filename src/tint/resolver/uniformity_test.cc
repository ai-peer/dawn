...
class UniformityAnalysisTest : public UniformityAnalysisTestBase,
                               public ::testing::Test {};
...
class BasicTest : public UniformityAnalysisTestBase,
 ...
 public:
  ...
  static bool RequiredToBeUniform(Function f) {
    return f > kLastNoRestrictionFunction;
  }

  /// Convert a test parameter pair of condition+function to a string that can
  /// be used in a test name.
...
// Test the uniformity constraints for a function call inside a conditional
// statement.
...
fn foo() {
  ...
  bool should_pass =
      !(MayBeNonUniform(condition) && RequiredToBeUniform(function));
  EXPECT_EQ(RunTest(src), should_pass) << error_;
  if (!should_pass) {
    EXPECT_THAT(error_, ::testing::StartsWith("test:28:5 error: "));
...
INSTANTIATE_TEST_SUITE_P(
    ...
    BasicTest,
    ::testing::Combine(
        ::testing::Range<int>(0, BasicTest::kEndOfConditionRange),
...
TEST_F(UniformityAnalysisTest, SubsequentControlFlowMayBeNonUniform_Pass) {
  // Call a function that causes subsequent control flow to be non-uniform, and
  // then call another function that doesn't require uniformity.
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> rw : i32;
...
fn main() {
  ...
  EXPECT_TRUE(RunTest(src)) << error_;
}

TEST_F(UniformityAnalysisTest, SubsequentControlFlowMayBeNonUniform_Fail) {
  // Call a function that causes subsequent control flow to be non-uniform, and
  // then call another function that requires uniformity.
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> rw : i32;
...
TEST_F(UniformityAnalysisTest, ParameterNoRestriction_Pass) {
  // Pass a non-uniform value as an argument, and then try to use the return
  // value for control-flow guarding a barrier.
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> rw : i32;
...
TEST_F(UniformityAnalysisTest, ParameterRequiredToBeUniform_Pass) {
  // Pass a uniform value as an argument to a function that uses that parameter
  // for control-flow guarding a barrier.
  auto src = R"(
@group(0) @binding(0) var<storage, read> ro : i32;
...
TEST_F(UniformityAnalysisTest, ParameterRequiredToBeUniform_Fail) {
  // Pass a non-uniform value as an argument to a function that uses that
  // parameter for control-flow guarding a barrier.
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> rw : i32;
...
TEST_F(UniformityAnalysisTest,
       ParameterRequiredToBeUniformForReturnValue_Pass) {
  // Pass a uniform value as an argument to a function that uses that parameter
  // to produce the return value, and then use the return value for control-flow
  // guarding a barrier.
  auto src = R"(
@group(0) @binding(0) var<storage, read> ro : i32;
...
TEST_F(UniformityAnalysisTest,
  ...
  // Pass a non-uniform value as an argument to a function that uses that
  // parameter to produce the return value, and then use the return value for
...
TEST_F(UniformityAnalysisTest,
       ParameterRequiredToBeUniformForSubsequentControlFlow_Pass) {
  // Pass a uniform value as an argument to a function that uses that parameter
  // return early, and then invoke a barrier after calling that function.
  auto src = R"(
@group(0) @binding(0) var<storage, read> ro : i32;
...
TEST_F(UniformityAnalysisTest,
       ParameterRequiredToBeUniformForSubsequentControlFlow_Fail) {
  // Pass a non-uniform value as an argument to a function that uses that
  // parameter return early, and then invoke a barrier after calling that
  // function.
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> rw : i32;
...
struct BuiltinEntry {
  ...
  std::string type;
  bool uniform;
  BuiltinEntry(std::string n, std::string t, bool u)
      : name(n), type(t), uniform(u) {}
};
...
fn main(s : S) {
  ...
  bool should_pass = GetParam().uniform;
  EXPECT_EQ(RunTest(src), GetParam().uniform) << error_;
  if (!should_pass) {
    EXPECT_EQ(
        error_,
...
TEST_F(UniformityAnalysisTest, ComputeBuiltin_MixedAttributesInStruct) {
  // Mix both non-uniform and uniform shader IO attributes in the same
  // structure. Even accessing just uniform member causes non-uniformity in this
  // case.
  auto src = R"(
struct S {
...
class LoopTest : public UniformityAnalysisTestBase,
 ...
 protected:
  std::string MakeInterrupt(ControlFlowInterrupt interrupt,
                            Condition condition) {
    switch (condition) {
      case kNone:
...
TEST_P(LoopTest, CallInBody_InterruptAfter) {
  // Test control-flow interrupt in a loop after a function call that requires
...
TEST_P(LoopTest, CallInBody_InterruptBefore) {
  // Test control-flow interrupt in a loop before a function call that requires
  // uniform control flow.
  auto interrupt = static_cast<ControlFlowInterrupt>(std::get<0>(GetParam()));
  auto condition = static_cast<Condition>(std::get<1>(GetParam()));
  auto src = R"(
...
TEST_P(LoopTest, CallInContinuing_InterruptInBody) {
  // Test control-flow interrupt in a loop with a function call that requires
  // uniform control flow in the continuing statement.
  auto interrupt = static_cast<ControlFlowInterrupt>(std::get<0>(GetParam()));
  auto condition = static_cast<Condition>(std::get<1>(GetParam()));
  auto src = R"(
@group(0) @binding(0) var<storage, read> uniform_var : i32;
...
class LoopDeadCodeTest : public UniformityAnalysisTestBase,
...
TEST_P(LoopDeadCodeTest, AfterInterrupt) {
  // Dead code after a control-flow interrupt in a loop shouldn't cause
  // uniformity errors.
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> n : i32;
...
TEST_F(UniformityAnalysisTest, Loop_NonUniformBreakInBody_Reconverge) {
  // Loops reconverge at exit, so test that we can call workgroupBarrier() after
  // a loop that contains a non-uniform conditional break.
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> n : i32;
...
TEST_F(UniformityAnalysisTest, Loop_NonUniformFunctionInBody_Reconverge) {
  // Loops reconverge at exit, so test that we can call workgroupBarrier() after
  // a loop that contains a call to a function that causes non-uniform control
  // flow.
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> n : i32;
...
TEST_F(UniformityAnalysisTest, ForLoop_NonUniformCondition_Reconverge) {
  // Loops reconverge at exit, so test that we can call workgroupBarrier() after
  // a loop that has a non-uniform condition.
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> n : i32;
...
TEST_F(UniformityAnalysisTest,
       IfElse_NonUniformCondition_BarrierInElseIfBlock) {
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> non_uniform : i32;
...
TEST_F(UniformityAnalysisTest, IfElse_NonUniformCondition_Reconverge) {
  // If statements reconverge at exit, so test that we can call
  // workgroupBarrier() after an if statement with a non-uniform condition.
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> non_uniform : i32;
...
TEST_F(UniformityAnalysisTest, Switch_NonUniformCondition_Reconverge) {
  // Switch statements reconverge at exit, so test that we can call
  // workgroupBarrier() after a switch statement that contains a non-uniform
  // conditional break.
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> non_uniform : i32;
...
TEST_F(UniformityAnalysisTest, Switch_NonUniformFunctionCall_Reconverge) {
  // Switch statements reconverge at exit, so test that we can call
  // workgroupBarrier() after a switch statement that contains a call to a
  // function that causes non-uniform control flow.
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> n : i32;
...
TEST_F(UniformityAnalysisTest,
       AssignUniformThenNonUniformThroughDifferentPointer) {
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> non_uniform : i32;
...
TEST_F(UniformityAnalysisTest,
       AssignNonUniformThenUniformThroughDifferentPointer) {
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> non_uniform : i32;
...
TEST_F(UniformityAnalysisTest,
       AssignUniformThroughPointerInFunctionCallViaArg) {
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> non_uniform : i32;
...
TEST_F(UniformityAnalysisTest,
       AssignUniformThroughPointerInFunctionCallViaPointerArg) {
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> non_uniform : i32;
...
TEST_F(UniformityAnalysisTest,
       AssignNonUniformThroughPointerInFunctionCallChain) {
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> non_uniform : i32;
...
TEST_F(UniformityAnalysisTest,
       NonUniformPointerParameterBecomesUniform_AfterUse) {
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> non_uniform : i32;
...
TEST_F(UniformityAnalysisTest,
       NonUniformPointerParameterBecomesUniform_BeforeUse) {
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> non_uniform : i32;
...
TEST_F(UniformityAnalysisTest, MultiplePointerParametersBecomeNonUniform) {
  // The analysis traverses the tree for each pointer parameter, and we need to
  // make sure that we reset the "visited" state of nodes in between these
  // traversals to properly capture each of their uniformity states.
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> non_uniform : i32;
...
TEST_F(UniformityAnalysisTest, MaximumNumberOfPointerParameters) {
  ...
  // stress the quadratic nature of the analysis.
  ProgramBuilder b;
  auto& ty = b.ty;
  ...
  for (int i = 0; i < 255; i++) {
    params.push_back(
        b.Param("p" + std::to_string(i),
                ty.pointer(ty.i32(), ast::StorageClass::kFunction)));
    if (i > 0) {
      foo_body.push_back(b.Assign(b.Deref("p" + std::to_string(i)),
                                  b.Deref("p" + std::to_string(i - 1))));
    }
  }
  ...
  main_body.push_back(
      b.CallStmt(b.create<ast::CallExpression>(b.Expr("foo"), args)));
  main_body.push_back(b.If(b.Equal("v254", 0),
                           b.Block(b.CallStmt(b.Call("workgroupBarrier")))));
...
TEST_F(UniformityAnalysisTest, Var_BecomesNonUniform_BeforeCondition) {
  // Use a function-scope variable for control-flow guarding a barrier, and then
  // assign to that variable before checking the condition.
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> rw : i32;
...
TEST_F(UniformityAnalysisTest, Var_BecomesNonUniform_AfterCondition) {
  // Use a function-scope variable for control-flow guarding a barrier, and then
  // assign to that variable after checking the condition.
  auto src = R"(
@group(0) @binding(0) var<storage, read_write> rw : i32;
