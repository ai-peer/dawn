...
test:5:7 note: parameter 'i' may be non-uniform
  if (i == 0) {
      ^

test:11:7 note: possibly non-uniform value passed to 'foo' here
  foo(rw);
      ^^
...
test:4:16 note: builtin 'b' may be non-uniform
  if (all(vec3(b) == vec3(0u))) {
               ^
)");
    }
...
test:8:16 note: parameter 's' may be non-uniform
  if (all(vec3(s.b) == vec3(0u))) {
               ^
)");
    }
...
test:9:7 note: parameter 's' may be non-uniform
  if (s.num_groups.x == 0u) {
      ^
)");
}
...
test:4:7 note: user-defined input 'l' may be non-uniform
  if (l == 0.0) {
      ^
)");
}
...
test:8:7 note: parameter 's' may be non-uniform
  if (s.l == 0.0) {
      ^
)");
}
...
test:5:8 note: parameter 'p' may be non-uniform
  if (*p == 0) {
       ^

test:12:7 note: possibly non-uniform pointer contents passed to 'bar' here
  bar(&v);
      ^
...
test:7:8 note: parameter 'p' may be non-uniform
  if (*p == 0) {
       ^

test:13:7 note: possibly non-uniform pointer contents passed to 'bar' here
  bar(&non_uniform);
      ^
...
test:13:7 note: possibly non-uniform pointer contents passed to 'zoo' here
  zoo(p);
      ^
...
test:21:7 note: possibly non-uniform pointer contents passed to 'bar' here
  bar(p);
      ^
...
test:8:8 note: parameter 'p' may be non-uniform
  if (*p == 0) {
       ^

test:14:7 note: possibly non-uniform pointer contents passed to 'zoo' here
  zoo(p);
      ^
...
test:22:7 note: possibly non-uniform pointer contents passed to 'bar' here
  bar(p);
      ^
...
fn main(@builtin(local_invocation_index) idx : u32) {
  ...
  if (workgroupUniformLoad(&data[idx]) > 0) {
    ...
    RunTest(src, false);
    EXPECT_EQ(error_,
              R"(test:8:28 warning: possibly non-uniform value passed to 'workgroupUniformLoad' here
  if (workgroupUniformLoad(&data[idx]) > 0) {
                           ^

test:8:34 note: builtin 'idx' may be non-uniform
  if (workgroupUniformLoad(&data[idx]) > 0) {
                                 ^^^
)");
}
...
fn main(@builtin(local_invocation_index) idx : u32) {
  ...
  if (foo(&data[idx]) > 0) {
    ...
    RunTest(src, false);
    EXPECT_EQ(error_,
              R"(test:8:31 warning: possibly non-uniform value passed to 'workgroupUniformLoad' here
  return workgroupUniformLoad(p);
                              ^

test:8:31 note: parameter 'p' may be non-uniform
  return workgroupUniformLoad(p);
                              ^

test:14:11 note: possibly non-uniform value passed to 'foo' here
  if (foo(&data[idx]) > 0) {
          ^

test:14:17 note: builtin 'idx' may be non-uniform
  if (foo(&data[idx]) > 0) {
                ^^^
)");
}
...
test:5:7 note: parameter 'a' may be non-uniform
  if (a == 42) {
      ^

test:11:7 note: possibly non-uniform value passed to 'zoo' here
  zoo(b);
      ^

test:11:7 note: parameter 'b' may be non-uniform
  zoo(b);
      ^

test:15:7 note: possibly non-uniform value passed to 'bar' here
  bar(c);
      ^

test:15:7 note: parameter 'c' may be non-uniform
  bar(c);
      ^

test:19:7 note: possibly non-uniform value passed to 'foo' here
  foo(non_uniform);
      ^^^^^^^^^^^
