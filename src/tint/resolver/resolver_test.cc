...
TEST_F(ResolverTest, Stmt_If) {
    ...
    auto* cond = Expr(true);
    auto* stmt = create<ast::IfStatement>(cond, body, ast::ElseStatementList{else_stmt});
...
TEST_F(ResolverTest, Expr_Identifier_Function_Ptr) {
    ...
    auto* p_decl = Decl(Const("p", ty.pointer<f32>(ast::StorageClass::kFunction), AddressOf(v)));
...
TEST_F(ResolverTest, Function_NotRegisterFunctionParams) {
    auto* func = Func("my_func", {Const("var", ty.f32(), Construct(ty.f32()))}, ty.void_(), {});
    EXPECT_TRUE(r()->Resolve()) << r()->error();
...
TEST_F(ResolverTest, StorageClass_SetForSampler) {
    ...
    EXPECT_EQ(Sem().Get(var)->StorageClass(), ast::StorageClass::kUniformConstant);
}
