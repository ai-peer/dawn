...
TEST_P(Expr_Binary_Test_Invalid, All) {
    ...
    EXPECT_THAT(r()->error(), HasSubstr("12:34 error: no matching call to operator "));
}
INSTANTIATE_TEST_SUITE_P(ResolverTest,
...
TEST_P(Expr_Binary_Test_Invalid_VectorMatrixMultiply, All) {
    ...
    if (is_valid_expr) {
        ...
        ASSERT_FALSE(r()->Resolve());
        EXPECT_THAT(r()->error(), HasSubstr("no matching call to operator *"));
    }
}
...
TEST_F(ResolverTest, UnaryOp_Not) {
    ...
    EXPECT_FALSE(r()->Resolve());
    EXPECT_THAT(r()->error(), HasSubstr("error: no matching call to operator ! (vec4<f32>)"));
}
...
TEST_F(ResolverTest, UnaryOp_Negation) {
    ...
    EXPECT_FALSE(r()->Resolve());
    EXPECT_THAT(r()->error(), HasSubstr("error: no matching call to operator - (u32)"));
}
