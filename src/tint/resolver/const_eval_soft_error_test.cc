...
TEST_F(ResolverConstEvalSoftErrorTest, CreateScalar_Infinity) {
    auto result = const_eval.CreateScalar({}, create<type::F32>(), f32::Inf());
    ASSERT_TRUE(result);
...
TEST_F(ResolverConstEvalSoftErrorTest, CreateScalar_NaN) {
    auto result = const_eval.CreateScalar({}, create<type::F32>(), f32::NaN());
    ASSERT_TRUE(result);
    EXPECT_EQ(result.Get()->ValueAs<AFloat>(), 0.f);
...
TEST_F(ResolverConstEvalSoftErrorTest, Vec_Overflow_SingleComponent) {
