...
class ResolverConstEvalSoftErrorTest : public ResolverConstEvalTest {
  protected:
    /// Default constructor.
    ResolverConstEvalSoftErrorTest() : const_eval(ConstEval(*this, /* soft_errors */ true)) {}

    /// The ConstEval object used during testing (has soft-errors enabled).
...
TEST_F(ResolverConstEvalSoftErrorTest, Add_AFloat_Overflow) {
    auto* a = Scalar(AFloat(std::numeric_limits<double>::max()));
    auto* b = Scalar(AFloat(std::numeric_limits<double>::max()));
    auto result = const_eval.OpPlus(a->Type(), utils::Vector{a, b}, {});
    ASSERT_TRUE(result);
    EXPECT_EQ(result.Get()->ValueAs<AFloat>(), 0.f);
    EXPECT_EQ(
...
TEST_F(ResolverConstEvalSoftErrorTest, Add_F32_Overflow) {
    auto* a = Scalar(f32(std::numeric_limits<float>::max()));
    auto* b = Scalar(f32(std::numeric_limits<float>::max()));
    auto result = const_eval.OpPlus(a->Type(), utils::Vector{a, b}, {});
    ASSERT_TRUE(result);
...
TEST_F(ResolverConstEvalSoftErrorTest, Sub_AInt_Overflow) {
    auto* a = Scalar(AInt(std::numeric_limits<int64_t>::min()));
    auto* b = Scalar(AInt(1));
...
TEST_F(ResolverConstEvalSoftErrorTest, Sub_AFloat_Overflow) {
    auto* a = Scalar(AFloat(std::numeric_limits<double>::lowest()));
    auto* b = Scalar(AFloat(std::numeric_limits<double>::max()));
    auto result = const_eval.OpMinus(a->Type(), utils::Vector{a, b}, {});
    ASSERT_TRUE(result);
    EXPECT_EQ(result.Get()->ValueAs<AFloat>(), 0.f);
...
TEST_F(ResolverConstEvalSoftErrorTest, Sub_F32_Overflow) {
    auto* a = Scalar(f32(std::numeric_limits<float>::lowest()));
    auto* b = Scalar(f32(std::numeric_limits<float>::max()));
    auto result = const_eval.OpMinus(a->Type(), utils::Vector{a, b}, {});
    ASSERT_TRUE(result);
    EXPECT_EQ(result.Get()->ValueAs<f32>(), 0.f);
...
TEST_F(ResolverConstEvalSoftErrorTest, Mul_AInt_Overflow) {
    auto* a = Scalar(AInt(std::numeric_limits<int64_t>::max()));
...
TEST_F(ResolverConstEvalSoftErrorTest, Mul_AFloat_Overflow) {
    auto* a = Scalar(AFloat(std::numeric_limits<double>::max()));
    auto* b = Scalar(AFloat(std::numeric_limits<double>::max()));
    auto result = const_eval.OpMultiply(a->Type(), utils::Vector{a, b}, {});
    ASSERT_TRUE(result);
...
TEST_F(ResolverConstEvalSoftErrorTest, Mul_F32_Overflow) {
    auto* a = Scalar(f32(std::numeric_limits<float>::max()));
    auto* b = Scalar(f32(std::numeric_limits<float>::max()));
    auto result = const_eval.OpMultiply(a->Type(), utils::Vector{a, b}, {});
    ASSERT_TRUE(result);
...
TEST_F(ResolverConstEvalSoftErrorTest, Div_AInt_ZeroDenominator) {
...
TEST_F(ResolverConstEvalSoftErrorTest, Div_I32_ZeroDenominator) {
...
TEST_F(ResolverConstEvalSoftErrorTest, Div_U32_ZeroDenominator) {
...
TEST_F(ResolverConstEvalSoftErrorTest, Div_AFloat_ZeroDenominator) {
...
TEST_F(ResolverConstEvalSoftErrorTest, Div_F32_ZeroDenominator) {
...
TEST_F(ResolverConstEvalSoftErrorTest, Div_I32_MostNegativeByMinInt) {
    auto* a = Scalar(i32(std::numeric_limits<int32_t>::min()));
    auto* b = Scalar(i32(-1));
    auto result = const_eval.OpDivide(a->Type(), utils::Vector{a, b}, {});
    ASSERT_TRUE(result);
    EXPECT_EQ(result.Get()->ValueAs<i32>(), std::numeric_limits<int32_t>::min());
    EXPECT_EQ(error(), R"(warning: '-2147483648 / -1' cannot be represented as 'i32')");
}

TEST_F(ResolverConstEvalSoftErrorTest, Mod_AInt_ZeroDenominator) {
    auto* a = Scalar(AInt(42));
    auto* b = Scalar(AInt(0));
...
TEST_F(ResolverConstEvalSoftErrorTest, Mod_I32_ZeroDenominator) {
    auto* a = Scalar(i32(42));
    auto* b = Scalar(i32(0));
    auto result = const_eval.OpModulo(a->Type(), utils::Vector{a, b}, {});
    ASSERT_TRUE(result);
...
TEST_F(ResolverConstEvalSoftErrorTest, Mod_U32_ZeroDenominator) {
    auto* a = Scalar(u32(42));
    auto* b = Scalar(u32(0));
    auto result = const_eval.OpModulo(a->Type(), utils::Vector{a, b}, {});
    ASSERT_TRUE(result);
...
TEST_F(ResolverConstEvalSoftErrorTest, Mod_AFloat_ZeroDenominator) {
    auto* a = Scalar(AFloat(42));
    auto* b = Scalar(AFloat(0));
    auto result = const_eval.OpModulo(a->Type(), utils::Vector{a, b}, {});
    ASSERT_TRUE(result);
...
TEST_F(ResolverConstEvalSoftErrorTest, Mod_F32_ZeroDenominator) {
    auto* a = Scalar(f32(42));
    auto* b = Scalar(f32(0));
    auto result = const_eval.OpModulo(a->Type(), utils::Vector{a, b}, {});
    ASSERT_TRUE(result);
...
TEST_F(ResolverConstEvalSoftErrorTest, Mod_I32_MostNegativeByMinInt) {
    auto* a = Scalar(i32(std::numeric_limits<int32_t>::min()));
    auto* b = Scalar(i32(-1));
    auto result = const_eval.OpModulo(a->Type(), utils::Vector{a, b}, {});
    ASSERT_TRUE(result);
...
TEST_F(ResolverConstEvalSoftErrorTest, ShiftLeft_AInt_SignChange) {
...
TEST_F(ResolverConstEvalSoftErrorTest, ShiftLeft_I32_SignChange) {
...
TEST_F(ResolverConstEvalSoftErrorTest, ShiftLeft_I32_MoreThanBitWidth) {
...
TEST_F(ResolverConstEvalSoftErrorTest, ShiftLeft_U32_MoreThanBitWidth) {
...
TEST_F(ResolverConstEvalSoftErrorTest, ShiftRight_I32_MoreThanBitWidth) {
...
TEST_F(ResolverConstEvalSoftErrorTest, ShiftRight_U32_MoreThanBitWidth) {
...
TEST_F(ResolverConstEvalSoftErrorTest, Acos_F32_OutOfRange) {
    auto* a = Scalar(f32(2));
...
TEST_F(ResolverConstEvalSoftErrorTest, Acosh_F32_OutOfRange) {
    auto* a = Scalar(f32(-1));
...
TEST_F(ResolverConstEvalSoftErrorTest, Asin_F32_OutOfRange) {
    auto* a = Scalar(f32(2));
...
TEST_F(ResolverConstEvalSoftErrorTest, Atanh_F32_OutOfRange) {
    auto* a = Scalar(f32(2));
...
TEST_F(ResolverConstEvalSoftErrorTest, Exp_F32_Overflow) {
...
TEST_F(ResolverConstEvalSoftErrorTest, Exp2_F32_Overflow) {
    auto* a = Scalar(f32(1000));
...
TEST_F(ResolverConstEvalSoftErrorTest, ExtractBits_I32_TooManyBits) {
...
TEST_F(ResolverConstEvalSoftErrorTest, ExtractBits_U32_TooManyBits) {
...
TEST_F(ResolverConstEvalSoftErrorTest, InsertBits_I32_TooManyBits) {
...
TEST_F(ResolverConstEvalSoftErrorTest, InsertBits_U32_TooManyBits) {
...
TEST_F(ResolverConstEvalSoftErrorTest, InverseSqrt_F32_OutOfRange) {
    auto* a = Scalar(f32(-1));
...
TEST_F(ResolverConstEvalSoftErrorTest, LDExpr_F32_OutOfRange) {
...
TEST_F(ResolverConstEvalSoftErrorTest, Log_F32_OutOfRange) {
    auto* a = Scalar(f32(-1));
...
TEST_F(ResolverConstEvalSoftErrorTest, Log2_F32_OutOfRange) {
    auto* a = Scalar(f32(-1));
...
TEST_F(ResolverConstEvalSoftErrorTest, Normalize_ZeroLength) {
...
TEST_F(ResolverConstEvalSoftErrorTest, Pack2x16Float_OutOfRange) {
...
TEST_F(ResolverConstEvalSoftErrorTest, Pow_F32_Overflow) {
    auto* a = Scalar(f32(2));
...
TEST_F(ResolverConstEvalSoftErrorTest, Unpack2x16Float_OutOfRange) {
...
TEST_F(ResolverConstEvalSoftErrorTest, QuantizeToF16_OutOfRange) {
    auto* a = Scalar(f32(75250.f));
...
TEST_F(ResolverConstEvalSoftErrorTest, Sqrt_F32_OutOfRange) {
    auto* a = Scalar(f32(-1));
...
// TODO: convert
// TODO: test that vec stuff works component wise

TEST_F(ResolverConstEvalSoftErrorTest, Bitcast_Infinity) {
...
TEST_F(ResolverConstEvalSoftErrorTest, Bitcast_NaN) {
...
TEST_F(ResolverConstEvalSoftErrorTest, Convert_F32_TooHigh) {
...
TEST_F(ResolverConstEvalSoftErrorTest, Convert_F32_TooLow) {
...
TEST_F(ResolverConstEvalSoftErrorTest, Convert_F16_TooHigh) {
...
TEST_F(ResolverConstEvalSoftErrorTest, Convert_F16_TooLow) {
    auto* a = Scalar(f32(-1000000.0));
    auto result = const_eval.Convert(create<type::F16>(), a, {});
    ASSERT_TRUE(result);
