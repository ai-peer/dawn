...
struct Case {
    Case(utils::VectorRef<ValuePtr> in_args, utils::VectorRef<ValuePtr> expected_values)
        : args(std::move(in_args)),
          expected(Success{std::move(expected_values), CheckConstantFlags{}}) {}

    Case(utils::VectorRef<ValuePtr> in_args, std::string expected_err)
    ...
    struct Success {
        utils::Vector<ValuePtr, 2> values;
...
static Case C(std::initializer_list<ValuePtr> args, ValuePtr result) {
    return Case{utils::Vector<ValuePtr, 8>{args}, utils::Vector<ValuePtr, 2>{std::move(result)}};
}

/// Creates a Case with Values for args and result
static Case C(std::initializer_list<ValuePtr> args, std::initializer_list<ValuePtr> results) {
    return Case{utils::Vector<ValuePtr, 8>{args}, utils::Vector<ValuePtr, 2>{results}};
}
...
static Case C(std::initializer_list<ScalarTypes> sargs, ScalarTypes sresult) {
    ...
    ValuePtr result = Val(0_a);
    std::visit([&](auto&& v) { result = Val(v); }, sresult);
    return Case{std::move(args), utils::Vector<ValuePtr, 2>{std::move(result)}};
}
...
static Case C(std::initializer_list<ScalarTypes> sargs,
    ...
    utils::Vector<ValuePtr, 2> results;
...
static Case E(std::initializer_list<ValuePtr> args, std::string err) {
    return Case{utils::Vector<ValuePtr, 8>{args}, std::move(err)};
}
