...
struct Case {
    Case(utils::VectorRef<Types> in_args, Types expected_value)
        : args(std::move(in_args)),
          expected(Success{std::move(expected_value), false, false, std::nullopt}) {}
    ...
    Case& FloatComp(std::optional<double> epsilon = {}) {
        ...
        s.float_compare_epsilon = epsilon;
        expected = s;
        return *this;
    }
                    ...
                    if constexpr (std::is_same_v<bool, T>) {
                        ...
                        if (std::isnan(e)) {
                            ...
                            if (expected_case.float_compare) {
                                if (expected_case.float_compare_epsilon) {
                                    EXPECT_NEAR(vf, e, *expected_case.float_compare_epsilon);
                                } else {
        ...
        ConcatInto(  //
            r, std::vector<Case>{
                   C({Vec(T::Highest(), T::Highest()), Vec(T(1), T(1))},
                     Val(T(T::Highest() + T::Highest()))),
                   C({Vec(T::Lowest(), T::Lowest()), Vec(T(1), T(1))},
                     Val(T(T::Lowest() + T::Lowest()))),
               });
    }
    return r;
}
