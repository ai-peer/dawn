...
std::vector<Case> FaceForwardCases() {
    ...
    // A random input vector and its negation, used for e1 args to FaceForward
    auto pos_vec = Vec(T(1), T(2), T(3));
    auto neg_vec = Vec(-T(1), -T(2), -T(3));

    // A random vector in the xy plane, used for e2 and e3 args to FaceForward.
    auto fwd_xy = Vec(T(123.45), T(456.78), T(0));
