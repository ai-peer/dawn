...
TEST_F(ResolverConstantsTest, Vec3_ZeroInit_f32) {
    ...
    ASSERT_EQ(sem->ConstantValue().Elements().size(), 3u);
    EXPECT_EQ(sem->ConstantValue().Element<AFloat>(0).value, 0u);
    EXPECT_EQ(sem->ConstantValue().Element<AFloat>(1).value, 0u);
    EXPECT_EQ(sem->ConstantValue().Element<AFloat>(2).value, 0u);
}
