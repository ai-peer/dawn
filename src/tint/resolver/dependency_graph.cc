...
std::string ResolvedIdentifier::String(const SymbolTable& symbols, diag::List& diagnostics) const {
    ...
    if (auto* node = Node()) {
        return Switch(
            node,  //
            [&](const ast::TypeDecl* n) { return "type '" + symbols.NameFor(n->name) + "'"; },
            [&](const ast::Var* n) { return "var '" + symbols.NameFor(n->symbol) + "'"; },
            [&](const ast::Const* n) { return "const '" + symbols.NameFor(n->symbol) + "'"; },
            [&](const ast::Override* n) { return "override '" + symbols.NameFor(n->symbol) + "'"; },
            [&](const ast::Function* n) { return "function '" + symbols.NameFor(n->symbol) + "'"; },
            [&](Default) {
