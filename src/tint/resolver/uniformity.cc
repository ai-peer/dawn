...
class UniformityGraph {
  ...
  private:
    ...
    bool ProcessFunction(const ast::Function* func) {
        ...
        auto make_param_tag = [&](utils::UniqueVector<Node*, 4>& reachable, size_t index) {
        ...
        // Look at which nodes are reachable from "RequiredToBeUniform".
        {
            ...
            for (size_t i = 0; i < func->params.Length(); i++) {
                current_function_->parameters[i].tag_direct = make_param_tag(reachable, i);
            }
        }
        ...
        if (current_function_->value_return) {
            ...
            for (size_t i = 0; i < func->params.Length(); i++) {
                current_function_->parameters[i].tag_retval = make_param_tag(reachable, i);
            }
        }
        ...
        for (size_t i = 0; i < func->params.Length(); i++) {
            ...
            for (size_t j = 0; j < func->params.Length(); j++) {
                auto tag = make_param_tag(reachable, j);
        ...
        return Switch(
            ...
            [&](const ast::BlockStatement* b) {
                utils::Hashmap<const sem::Variable*, Node*, 8> scoped_assignments;
                {
                    ...
                    if (auto* loop = sem_.Get(b)->Parent()->As<sem::LoopStatement>()) {
    ...
    std::pair<Node*, Node*> ProcessExpression(Node* cf,
                                              ...
                                              bool load_rule = false) {
        if (auto* load = sem_.Get<sem::Load>(expr)) {
