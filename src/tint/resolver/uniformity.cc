...
/// CallSiteTag describes the uniformity requirements on the call sites of a
/// function.
enum CallSiteTag {
...
/// ParameterTag describes the uniformity requirements of values passed to a
/// function parameter.
enum ParameterTag {
...
/// ParameterInfo holds information about the uniformity requirements and
/// effects for a particular function parameter.
struct ParameterInfo {
  /// The parameter's uniformity requirements.
  ParameterTag tag = ParameterNoRestriction;
  /// Will be `true` if this function may cause the contents of this pointer
  /// parameter to become non-uniform.
  bool pointer_may_become_non_uniform = false;
  /// The parameters that are required to be uniform for the contents of this
  /// pointer parameter to be uniform at function exit.
  std::vector<const sem::Parameter*> pointer_param_output_sources;
};

/// FunctionInfo holds information about the uniformity requirements and effects
...
/// Node represents a node in the graph of control flow and value nodes within
/// the analysis of a single function.
struct Node {
  /// Constructor
  /// @param t the node tag (used for debugging)
  Node(std::string t, const ast::Node* a) : tag(t), ast(a) {}
  ...
  void AddEdge(Node* to) { edges.insert(to); }
};

/// UniformityGraph is used to analyze the uniformity requirements and effects
/// of functions in a module.
class UniformityGraph {
 public:
  /// Constructor.
  /// @param builder the program to analyze
  UniformityGraph(ProgramBuilder* builder)
      : builder_(builder),
        sem_(builder->Sem()),
  ...
  /// Build and analyze the graph to determine whether the program satisfies
  /// the uniformity constraints of WGSL.
 ...
 private:
  ...
  bool ProcessFunction(const ast::Function* func) {
    ...
    for (auto* child : sem_.Get(func)->TransitivelyCalledFunctions()) {
      if (!ProcessFunction(child->Declaration())) {
        return false;
      }
    }
    ...
    // Look at which nodes are reachable from "RequiredToBeUniform".
    {
      ...
      if (reachable.count(may_be_non_uniform_)) {
        MakeError();
        return false;
      } else if (reachable.count(cf_start)) {
        info.callsite_tag = CallSiteRequiredToBeUniform;
      }

      // Set the parameter tag to ParameterRequiredToBeUniform for each
      // parameter node that was reachable.
      for (size_t i = 0; i < func->params.size(); i++) {
        auto* param = func->params[i];
        if (reachable.count(variables_.at(sem_.Get(param)))) {
    ...
    // Look at which nodes are reachable from "CF_return"
    {
      ...
      // Set the parameter tag to
      // ParameterRequiredToBeUniformForSubsequentControlFlow for each parameter
      // node that was reachable.
      for (size_t i = 0; i < func->params.size(); i++) {
        auto* param = func->params[i];
        if (reachable.count(variables_.at(sem_.Get(param)))) {
          info.parameters[i].tag =
              ParameterRequiredToBeUniformForSubsequentControlFlow;
        }
      }
    }
    ...
    if (value_return_) {
      ...
      // Set the parameter tag to ParameterRequiredToBeUniformForReturnValue for
    ...
    for (size_t i = 0; i < func->params.size(); i++) {
      ...
      // Check every other parameter to see if they feed into this parameter's
      // final value.
      for (size_t j = 0; j < func->params.size(); j++) {
        auto* param_source = sem_.Get<sem::Parameter>(func->params[j]);
        if (reachable.count(param_init_values[j])) {
          info.parameters[i].pointer_param_output_sources.push_back(
              param_source);
        }
      }
    }
  ...
  Node* ProcessStatement(Node* cf, const ast::Statement* stmt) {
    return Switch(
        ...
        [&](const ast::BreakStatement* b) { return cf; },
        ...
        [&](const ast::ForLoopStatement* f) {
          ...
          if (sem_.Get(f)->Behaviors() ==
              sem::Behaviors{sem::Behavior::kNext}) {
            return cf;
          } else {
            return cfx;
          }
  ...
  std::pair<Node*, Node*> ProcessIdentExpression(
      Node* cf,
      const ast::IdentifierExpression* ident) {
    // Helper to check if the entry point attribute of `obj` indicates
    // non-uniformity.
    auto has_nonuniform_entry_point_attribute = [](auto* obj) {
      // Only the num_workgroups and workgroup_id builtins are uniform.
      if (auto* builtin =
              ast::GetAttribute<ast::BuiltinAttribute>(obj->attributes)) {
    ...
    return Switch(
        ...
        [&](const sem::Parameter* param) {
          ...
          if (user_func && user_func->Declaration()->IsEntryPoint()) {
            if (auto* str = param->Type()->As<sem::Struct>()) {
              // We consider the whole struct to be non-uniform if any one of
              // its members is non-uniform.
              bool uniform = true;
              for (auto* member : str->Members()) {
                if (has_nonuniform_entry_point_attribute(
                        member->Declaration())) {
                  uniform = false;
                }
              }
        ...
        [&](const sem::GlobalVariable* global) {
          if (global->Access() == ast::Access::kRead) {
            return std::make_pair(cf, cf);
          } else {
            return std::make_pair(cf, may_be_non_uniform_);
          }
        ...
        [&](Default) {
          TINT_ICE(Resolver, diagnostics_)
              << "unknown identifier expression type: "
  ...
  std::pair<Node*, Node*> ProcessExpression(Node* cf,
    ...
    return Switch(
        ...
        [&](const ast::BitcastExpression* b) {
          return ProcessExpression(cf, b->expr);
        },

        [&](const ast::CallExpression* c) { return ProcessCall(cf, c); },

        [&](const ast::IdentifierExpression* i) {
          return ProcessIdentExpression(cf, i);
        },
        ...
        [&](const ast::UnaryOpExpression* u) {
          if (u->op == ast::UnaryOp::kIndirection) {
            // Cut the analysis short, since we only need to know the
            // originating variable which is being accessed.
  ...
  std::pair<Node*, Node*> ProcessLValueExpression(Node* cf,
    ...
    return Switch(
        ...
        [&](const ast::IdentifierExpression* i) {
          ...
          if (auto* global = sem->Variable()->As<sem::GlobalVariable>()) {
            return std::make_pair(cf, may_be_non_uniform_);
        ...
        [&](const ast::UnaryOpExpression* u) {
          if (u->op == ast::UnaryOp::kIndirection) {
            // Cut the analysis short, since we only need to know the
            // originating variable that is being written to.
            auto* source_var = sem_.Get(u)->SourceVariable();
            auto name =
                builder_->Symbols().NameFor(source_var->Declaration()->symbol);
        ...
        [&](Default) {
          TINT_ICE(Resolver, diagnostics_)
              << "unknown lvalue expression type: "
              << std::string(expr->TypeInfo().name);
          return std::pair<Node*, Node*>(nullptr, nullptr);
        });
  }
  ...
  /// @returns a pair of (control flow node, value node)
  std::pair<Node*, Node*> ProcessCall(Node* cf,
    ...
    for (size_t i = 0; i < call->args.size(); i++) {
      ...
      // Note: This is an additional node that isn't described in the
      // specification, for the purpose of providing diagnostic information.
      Node* arg_node = CreateNode(name + "_arg_" + std::to_string(i), call);
      arg_node->arg_index = i;
    ...
    Switch(
        ...
        [&](const sem::Builtin* builtin) {
          // Most builtins have no restrictions. The exceptions are barriers,
          // derivatives, and some texture sampling builtins.
          if (builtin->IsBarrier()) {
            callsite_tag = CallSiteRequiredToBeUniform;
                     ...
                     builtin->Type() ==
                         sem::BuiltinType::kTextureSampleCompare) {
            callsite_tag = CallSiteRequiredToBeUniform;
        ...
        [&](const sem::Function* func) {
          // We must have already analyzed the user-defined function due to the
          // depth-first search through transitive function references.
        ...
        [&](Default) {
          TINT_ICE(Resolver, diagnostics_)
              << "unhandled function call target: " << name;
        });

    if (callsite_tag == CallSiteRequiredToBeUniform) {
      // Note: This deviates from the rules in the specification, which would
      // add the edge directly to the incoming CF node. Going through CF_after
      // instead makes it easier to produce diagnostics that can identify the
      ...
      if (func_info) {
        ...
        if (sem_arg->Type()->Is<sem::Pointer>()) {
          ...
          if (func_info->parameters[i].pointer_may_become_non_uniform) {
            ...
            // Add edges from the resulting pointer value to any other arguments
            // that feed it.
            for (auto* source :
                 func_info->parameters[i].pointer_param_output_sources) {
        ...
        // All builtin function parameters are
        // RequiredToBeUniformForReturnValue, as are parameters for type
  ...
  /// Recursively traverse a graph starting at `node`, inserting all nodes that
  /// are reached into `reachable`.
  /// @param node the starting node
  /// @param reachable the set of reachable nodes to populate
  void Traverse(Node* node, std::unordered_set<Node*>& reachable) {
  ...
  void MakeError() {
    // Trace back to find a node that is required to be uniform that was
    // reachable from a non-uniform value or control flow node.
    ...
    // TODO: Switch to warning instead of error for first release.
    if (current->arg_index != UINT32_MAX) {
