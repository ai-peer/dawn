...
sem::Constant Resolver::ConstantCast(const sem::Constant& value,
    ...
    for (size_t i = 0; i < value.Elements().size(); ++i) {
        if (target_elem_type->Is<sem::I32>()) {
            elems.emplace_back(
                value.WithScalarAt(i, [](auto&& s) { return i32(static_cast<int32_t>(s)); }));
        } else if (target_elem_type->Is<sem::U32>()) {
            elems.emplace_back(
                value.WithScalarAt(i, [](auto&& s) { return u32(static_cast<uint32_t>(s)); }));
        } else if (target_elem_type->Is<sem::F32>()) {
            elems.emplace_back(value.WithScalarAt(i, [](auto&& s) { return f32(s); }));
        } else if (target_elem_type->Is<sem::Bool>()) {
            elems.emplace_back(value.WithScalarAt(i, [](auto&& s) { return bool(s); }));
        }
    }
