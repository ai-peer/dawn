...
utils::Result<sem::Constant::Elements> MaterializeElements(const sem::Constant::Elements& in,
    ...
    auto out = Transform(in, el_ty, [&](auto& el_out, auto el_in) {
        if (auto conv = CheckedConvert<std::decay_t<decltype(el_out)>>(el_in)) {
            el_out = conv.Get();
        } else if (!failure.has_value()) {
            std::stringstream ss;
            if (conv.Failure() == ConversionFailure::kTooSmall) {
                ss << "magnitude of value " << el_in << " is too small to be represented as ";
            } else {
