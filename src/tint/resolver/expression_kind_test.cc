...
TEST_P(ResolverExpressionKindTest, Test) {
    ...
    switch (GetParam().use) {
        case Use::kAccess:
            GlobalVar("v", ty("texture_storage_2d", "rgba8unorm", Ident(kUseSource, sym)),
                      Group(0_u), Binding(0_u));
            break;
        case Use::kAddressSpace:
            Enable(ast::Extension::kChromiumExperimentalFullPtrParameters);
            Func("f", utils::Vector{Param("p", ty("ptr", Expr(kUseSource, sym), ty.f32()))},
        ...
        case Use::kMemberType:
            ...
            break;
        case Use::kTexelFormat:
            GlobalVar("v", ty("texture_storage_2d", ty(kUseSource, sym), "write"), Group(0_u),
                      Binding(0_u));
            break;
        case Use::kValueExpression:
