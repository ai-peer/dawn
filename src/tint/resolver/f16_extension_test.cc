...
using ResolverF16ExtensionCoreTypeAliasTest = ResolverTestWithParam<const char*>;

TEST_P(ResolverF16ExtensionCoreTypeAliasTest, Vec2hTypeUsedWithExtension) {
    // enable f16;
    // var<private> v : vec2h;
    Enable(ast::Extension::kF16);

    GlobalVar("v", Ident(Source{{12, 34}}, GetParam()), type::AddressSpace::kPrivate);

    EXPECT_TRUE(r()->Resolve()) << r()->error();
}

TEST_P(ResolverF16ExtensionCoreTypeAliasTest, Vec2hTypeUsedWithoutExtension) {
    // var<private> v : vec2h;
    GlobalVar("v", Ident(Source{{12, 34}}, GetParam()), type::AddressSpace::kPrivate);
...
INSTANTIATE_TEST_SUITE_P(ResolverF16ExtensionCoreTypeAliasTest,
                         ResolverF16ExtensionCoreTypeAliasTest,
