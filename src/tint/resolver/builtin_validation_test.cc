...
// experimental extension google_experimental_dp4a is not needed.
    ...
    Func("func", {}, ty.i32(),
         {
             Return(Call(Source{Source::Location{12, 34}}, "dot4I8Packed",
                         ast::ExpressionList{Expr(1u), Expr(2u)})),
         });
...
TEST_F(ResolverDP4aExtensionValidationTest, Dot4I8PackedWithoutExtension) {
    ...
    EXPECT_EQ(
        r()->error(),
        R"(error: cannot call built-in function 'dot4I8Packed' without extension google_experimental_dp4a)");
}
...
TEST_F(ResolverDP4aExtensionValidationTest, Dot4U8PackedWithoutExtension) {
    ...
    EXPECT_EQ(
        r()->error(),
        R"(error: cannot call built-in function 'dot4U8Packed' without extension google_experimental_dp4a)");
}
