...
/// Value is a base class that represents a value of type DataType<T> created with
...
ValuePtr Val(T v) {
    ...
    return std::make_unique<ConcreteValue<T>>(ScalarArgs{v});
}
...
ValuePtr Vec(T... args) {
    ...
    return std::make_unique<ConcreteValue<VT>>(utils::VectorRef<FirstT>{v});
}
...
ValuePtr Mat(const T (&m_in)[C][R]) {
    ...
    return std::make_unique<ConcreteValue<MT>>(utils::VectorRef<T>{m});
}
