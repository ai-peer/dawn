...
class Value {
  public:
    ...
    static Value Create(ScalarArgs args) {
        return Value{std::make_unique<detail::ConcreteValue<T>>(std::move(args))};
    }
  ...
  private:
    /// Private constructor
    explicit Value(std::unique_ptr<const detail::ValueBase> value) : value_(std::move(value)) {}
...
Value Vec(T... args) {
    ...
    return Value::Create<VT>(utils::VectorRef<FirstT>{v});
}
...
Value Mat(const T (&m_in)[C][R]) {
    ...
    return Value::Create<MT>(utils::VectorRef<T>{m});
}
