...
const sem::Expression* Resolver::Load(const sem::Expression* expr) {
    ...
    Switch(
        expr->RootIdentifier(),
        [&](const sem::GlobalVariable* global) {
            alias_analysis_infos_.GetOrZero(current_function_)
                ->module_scope_reads.Add(global, expr);
        },
        [&](const sem::Parameter* param) {
            alias_analysis_infos_.GetOrZero(current_function_)->parameter_reads.Add(param);
        });
