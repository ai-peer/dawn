...
sem::Expression* Resolver::Expression(const ast::Expression* root) {
    ...
    for (size_t i = 0, n = sorted.Length(); i < n; i++) {
        auto* expr = sorted[n - i - 1];
        ...
        return Switch(
            ...
            [&](const sem::Array* arr) -> sem::Call* {
                auto* call_target = utils::GetOrCreate(
                    ...
                    [&]() -> sem::TypeConstructor* {
                        sem::ParameterList<8> params(args.Length());
...
void Resolver::CollectTextureSamplerPairs(
    ...
    const utils::Vector<const sem::Expression*, 8>& args) const {
...
sem::Expression* Resolver::Binary(const ast::BinaryExpression* expr) {
    ...
    const sem::Constant* value = nullptr;
    if (op.const_eval_fn) {
        utils::Vector<const sem::Expression*, 2> args{{lhs, rhs}};
        value = (const_eval_.*op.const_eval_fn)(op.result, std::move(args));
    }
...
sem::Expression* Resolver::UnaryOp(const ast::UnaryOpExpression* unary) {
    ...
    switch (unary->op) {
        ...
        default: {
            ...
            if (stage == sem::EvaluationStage::kConstant) {
                if (op.const_eval_fn) {
                    utils::Vector<const sem::Expression*, 1> args{expr};
                    value = (const_eval_.*op.const_eval_fn)(ty, std::move(args));
                } else {
                    stage = sem::EvaluationStage::kRuntime;
                }
            }
...
bool Resolver::Mark(const ast::Node* node) {
    ...
    auto marked = marked_[node->node_id.value];
    if (!marked) {
        marked = true;
        return true;
    }
