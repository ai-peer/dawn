...
sem::Variable* Resolver::Variable(const ast::Variable* v,
    ...
    if (v->constructor) {
        ...
        if (!storage_ty) {
            if (as_var && is_global) {
                AddError("global var declaration must specify a type", v->source);
                return nullptr;
            }
        ...
        AddError((is_global) ? "module scope var declaration requires a type and initializer"
    ...
    if (!is_global && as_var && storage_class != ast::StorageClass::kFunction &&
        ...
        AddError("function var has a non-function storage class", v->source);
        return nullptr;
    }
    ...
    if (rhs && as_var &&
        !validator_.VariableConstructorOrCast(as_var, storage_class, storage_ty, rhs->Type())) {
        return nullptr;
    }
...
sem::GlobalVariable* Resolver::GlobalVariable(const ast::Variable* v) {
    auto* sem = Variable(v, /* is_global */ true)->As<sem::GlobalVariable>();
    if (!sem) {
        return nullptr;
    }

    const bool is_let = v->Is<ast::Let>();

    auto storage_class = sem->StorageClass();
    if (!is_let && storage_class == ast::StorageClass::kNone) {
        AddError("global variables must have a storage class", v->source);
        return nullptr;
    }
    if (is_let && storage_class != ast::StorageClass::kNone) {
        AddError("global constants shouldn't have a storage class", v->source);
        return nullptr;
    }
...
sem::Function* Resolver::Function(const ast::Function* decl) {
    ...
    for (auto* param : decl->params) {
        ...
        auto* p = As<sem::Parameter>(Variable(param, parameter_index++));
        if (!p) {
            return nullptr;
        }
