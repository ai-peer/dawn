...
type::Type* Resolver::Type(const ast::Type* ast) {
    if (ast == nullptr) {
    ...
    auto resolved = dependencies_.resolved_identifiers.Get(ast->name);
    if (!resolved) {
        TINT_ICE(Resolver, diagnostics_)
            << "identifier '" << builder_->Symbols().NameFor(ast->name->symbol)
            << "' was not resolved";
        return nullptr;
    }
    ...
    if (auto* ast_node = resolved->Node()) {
        ...
        if (TINT_UNLIKELY(!type)) {
            ErrorMismatchedResolvedIdentifier(ast->source, *resolved, "type");
            return nullptr;
        }

        if (TINT_UNLIKELY(ast->name->Is<ast::TemplatedIdentifier>())) {
            AddError("type '" + builder_->Symbols().NameFor(ast->name->symbol) +
        ...
        builder_->Sem().Add(ast, type);
        return type;
    }

    if (auto b = resolved->BuiltinType(); b != type::Builtin::kUndefined) {
        auto* type = BuiltinType(b, ast->name);
        if (TINT_UNLIKELY(!type)) {
            return nullptr;
        }
...
sem::Call* Resolver::Call(const ast::CallExpression* expr) {
    ...
    auto call_type = [&](const type::Type* type) {
        return Switch(
            type,  //
            ...
            [&](Default) {
                TINT_ICE(Resolver, diagnostics_) << "unhandled type " << type->TypeInfo().name;
                return nullptr;
            });
    };
    ...
    auto call = [&]() -> sem::Call* {
        ...
        if (auto* ast_node = resolved->Node()) {
            return Switch(
                sem_.Get(ast_node),  //
                [&](type::Type* ty) { return call_type(ty); },
        ...
        if (auto b = resolved->BuiltinType(); b != type::Builtin::kUndefined) {
            auto* ty = BuiltinType(b, expr->target, /* is_call_target */ true);
            if (TINT_UNLIKELY(!ty)) {
                return nullptr;
            }
            return call_type(ty);
...
type::Type* Resolver::BuiltinType(type::Builtin builtin_ty,
    ...
    auto vec = [&](type::Type* el, uint32_t n) -> type::Vector* {
        ...
        auto* out = b.create<type::Vector>(el, n);
        if (TINT_UNLIKELY(!validator_.Vector(out, ident->source))) {
            return nullptr;
        }
        return out;
    };
    auto mat = [&](type::Type* el, uint32_t num_columns, uint32_t num_rows) -> type::Matrix* {
        ...
        auto* out = b.create<type::Matrix>(vec(el, num_rows), num_columns);
        if (TINT_UNLIKELY(!validator_.Matrix(out, ident->source))) {
            return nullptr;
        }
        return out;
    };
    auto vec_t = [&](uint32_t n) -> type::Vector* {
        // If this type is used as a call-target, then permit the omission of template arguments for
        // type initializers that can infer the element type.
        auto* tmpl_ident = templated_identifier(is_call_target ? 0 : 1, 1);
        if (TINT_UNLIKELY(!tmpl_ident && !is_call_target)) {
            return nullptr;
        }
        if (tmpl_ident) {
            ...
            return vec(const_cast<type::Type*>(ty_expr->Type()), n);
        }
        return b.create<type::Vector>(/* infer */ nullptr, n);
    };
    auto mat_t = [&](uint32_t num_columns, uint32_t num_rows) -> type::Matrix* {
        ...
        if (tmpl_ident) {
            ...
            return mat(const_cast<type::Type*>(ty_expr->Type()), num_columns, num_rows);
        }
        return b.create<type::Matrix>(b.create<type::Vector>(/* infer */ nullptr, num_rows),
                                      num_columns);
    };
    auto array = [&]() -> type::Array* {
        ...
        if (tmpl_ident) {
            ...
            auto* ast_count =
                (tmpl_ident->arguments.Length() > 1) ? tmpl_ident->arguments[1] : nullptr;

            auto* el_ty_expr = TypeExpression(ast_el_ty);
            if (!el_ty_expr) {
                return nullptr;
            }
            auto* el_ty = el_ty_expr->Type();
        ...
        return b.create<type::Array>(/* infer */ nullptr);
    };
    ...
    auto ptr = [&]() -> type::Pointer* {
        ...
        auto* ty_expr = TypeExpression(tmpl_ident->arguments[1]);
        if (TINT_UNLIKELY(!ty_expr)) {
            return nullptr;
        }
        auto* ty = ty_expr->Type();
        ...
        auto* out = b.create<type::Pointer>(ty, address_space, access);
        if (!validator_.Pointer(tmpl_ident, out)) {
            return nullptr;
        }
        if (!ApplyAddressSpaceUsageToType(address_space, out, ident->source)) {
    ...
    auto sampled_texture = [&](type::TextureDimension dim) -> type::SampledTexture* {
        ...
        return b.create<type::SampledTexture>(dim, ty_expr->Type());
    };
    auto multisampled_texture = [&](type::TextureDimension dim) -> type::MultisampledTexture* {
        ...
        return b.create<type::MultisampledTexture>(dim, ty_expr->Type());
    };
