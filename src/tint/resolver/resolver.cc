...
sem::Variable* Resolver::Variable(const ast::Variable* v,
    ...
    if (v->constructor) {
        ...
        if (!storage_ty) {
            if (as_var && is_global) {
                AddError("module-scope 'var' must specify a type", v->source);
                return nullptr;
            }
        ...
        AddError("'let' must have an initializer", v->source);
        return nullptr;
    } else if (!v->type) {
        AddError((is_global) ? "module-scope 'var' requires a type and initializer"
                             : "function-scope 'var' requires a type or initializer",
                 v->source);
        return nullptr;
    }
    ...
    if (!is_global && as_var && storage_class != ast::StorageClass::kFunction &&
        ...
        AddError("function-scope 'var' must have 'function' storage class", v->source);
        return nullptr;
    }
...
sem::GlobalVariable* Resolver::GlobalVariable(const ast::Variable* v) {
    ...
    if (is_var && storage_class == ast::StorageClass::kNone) {
        AddError("module-scope 'var' must have a storage class", v->source);
        return nullptr;
    }
    if (is_let && storage_class != ast::StorageClass::kNone) {
        AddError("module-scope 'let' cannot have a storage class", v->source);
        return nullptr;
    }
