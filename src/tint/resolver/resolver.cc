...
type::Type* Resolver::Type(const ast::Type* ty) {
    ...
    auto* s = Switch(
        ...
        [&](Default) -> type::Type* {
            ...
            if (auto* type = resolved->As<type::Type, kDontErrorOnImpossibleCast>()) {
                return type;
            }
            if (auto* node = resolved->As<sem::Node>()) {
                return Switch(
                    node,  //
...
sem::Call* Resolver::Call(const ast::CallExpression* expr) {
    ...
    if (expr->target.type) {
        ...
        if (const auto* ty = resolved->As<type::Type, kDontErrorOnImpossibleCast>()) {
...
sem::Expression* Resolver::Identifier(const ast::IdentifierExpression* expr) {
    ...
    if (resolved->Is<type::Type, kDontErrorOnImpossibleCast>() || BuiltinTypeAlias(symbol)) {
