...
TEST_F(ResolverTypeInitializerValidationTest, InferredArrayArgumentTypeMismatch_Vec3i32_Vec3AF) {
    ...
    auto* t = array(Source{{12, 34}}, nullptr, nullptr, vec3<i32>(), Call("vec3", 1._a));
    WrapInFunction(t);
...
TEST_F(ResolverTypeInitializerValidationTest, InferVec2ElementTypeFromScalars) {
    ...
    auto* vec2_bool = vec2(nullptr, true, false);
    auto* vec2_i32 = vec2(nullptr, 1_i, 2_i);
    auto* vec2_u32 = vec2(nullptr, 1_u, 2_u);
    auto* vec2_f32 = vec2(nullptr, 1_f, 2_f);
    auto* vec2_f16 = vec2(nullptr, 1_h, 2_h);
    ...
    EXPECT_EQ(TypeOf(vec2_bool), TypeOf(vec2_bool->target));
    EXPECT_EQ(TypeOf(vec2_i32), TypeOf(vec2_i32->target));
    EXPECT_EQ(TypeOf(vec2_u32), TypeOf(vec2_u32->target));
    EXPECT_EQ(TypeOf(vec2_f32), TypeOf(vec2_f32->target));
    EXPECT_EQ(TypeOf(vec2_f16), TypeOf(vec2_f16->target));
}

TEST_F(ResolverTypeInitializerValidationTest, InferVec2ElementTypeFromVec2) {
    Enable(ast::Extension::kF16);

    auto* vec2_bool = vec2(nullptr, vec2<bool>(true, false));
    auto* vec2_i32 = vec2(nullptr, vec2<i32>(1_i, 2_i));
    auto* vec2_u32 = vec2(nullptr, vec2<u32>(1_u, 2_u));
    auto* vec2_f32 = vec2(nullptr, vec2<f32>(1_f, 2_f));
    auto* vec2_f16 = vec2(nullptr, vec2<f16>(1_h, 2_h));
    WrapInFunction(vec2_bool, vec2_i32, vec2_u32, vec2_f32, vec2_f16);
...
TEST_F(ResolverTypeInitializerValidationTest, InferVec3ElementTypeFromScalars) {
    ...
    auto* vec3_bool = vec(nullptr, 3u, Expr(true), Expr(false), Expr(true));
    auto* vec3_i32 = vec(nullptr, 3u, Expr(1_i), Expr(2_i), Expr(3_i));
    auto* vec3_u32 = vec(nullptr, 3u, Expr(1_u), Expr(2_u), Expr(3_u));
    auto* vec3_f32 = vec(nullptr, 3u, Expr(1_f), Expr(2_f), Expr(3_f));
    auto* vec3_f16 = vec(nullptr, 3u, Expr(1_h), Expr(2_h), Expr(3_h));
    ...
    EXPECT_EQ(TypeOf(vec3_bool), TypeOf(vec3_bool->target));
    EXPECT_EQ(TypeOf(vec3_i32), TypeOf(vec3_i32->target));
    EXPECT_EQ(TypeOf(vec3_u32), TypeOf(vec3_u32->target));
    EXPECT_EQ(TypeOf(vec3_f32), TypeOf(vec3_f32->target));
    EXPECT_EQ(TypeOf(vec3_f16), TypeOf(vec3_f16->target));
}

TEST_F(ResolverTypeInitializerValidationTest, InferVec3ElementTypeFromVec3) {
    Enable(ast::Extension::kF16);

    auto* vec3_bool = vec(nullptr, 3u, vec3<bool>(true, false, true));
    auto* vec3_i32 = vec(nullptr, 3u, vec3<i32>(1_i, 2_i, 3_i));
    auto* vec3_u32 = vec(nullptr, 3u, vec3<u32>(1_u, 2_u, 3_u));
    auto* vec3_f32 = vec(nullptr, 3u, vec3<f32>(1_f, 2_f, 3_f));
    auto* vec3_f16 = vec(nullptr, 3u, vec3<f16>(1_h, 2_h, 3_h));
    WrapInFunction(vec3_bool, vec3_i32, vec3_u32, vec3_f32, vec3_f16);
...
TEST_F(ResolverTypeInitializerValidationTest, InferVec3ElementTypeFromScalarAndVec2) {
    ...
    auto* vec3_bool = vec(nullptr, 3u, Expr(true), vec2<bool>(false, true));
    auto* vec3_i32 = vec(nullptr, 3u, Expr(1_i), vec2<i32>(2_i, 3_i));
    auto* vec3_u32 = vec(nullptr, 3u, Expr(1_u), vec2<u32>(2_u, 3_u));
    auto* vec3_f32 = vec(nullptr, 3u, Expr(1_f), vec2<f32>(2_f, 3_f));
    auto* vec3_f16 = vec(nullptr, 3u, Expr(1_h), vec2<f16>(2_h, 3_h));
    WrapInFunction(vec3_bool, vec3_i32, vec3_u32, vec3_f32, vec3_f16);
...
TEST_F(ResolverTypeInitializerValidationTest, InferVec4ElementTypeFromScalars) {
    ...
    auto* vec4_bool = vec(nullptr, 4u, Expr(true), Expr(false), Expr(true), Expr(false));
    auto* vec4_i32 = vec(nullptr, 4u, Expr(1_i), Expr(2_i), Expr(3_i), Expr(4_i));
    auto* vec4_u32 = vec(nullptr, 4u, Expr(1_u), Expr(2_u), Expr(3_u), Expr(4_u));
    auto* vec4_f32 = vec(nullptr, 4u, Expr(1_f), Expr(2_f), Expr(3_f), Expr(4_f));
    auto* vec4_f16 = vec(nullptr, 4u, Expr(1_h), Expr(2_h), Expr(3_h), Expr(4_h));
    ...
    EXPECT_EQ(TypeOf(vec4_bool), TypeOf(vec4_bool->target));
    EXPECT_EQ(TypeOf(vec4_i32), TypeOf(vec4_i32->target));
    EXPECT_EQ(TypeOf(vec4_u32), TypeOf(vec4_u32->target));
    EXPECT_EQ(TypeOf(vec4_f32), TypeOf(vec4_f32->target));
    EXPECT_EQ(TypeOf(vec4_f16), TypeOf(vec4_f16->target));
}

TEST_F(ResolverTypeInitializerValidationTest, InferVec4ElementTypeFromVec4) {
    Enable(ast::Extension::kF16);

    auto* vec4_bool = vec(nullptr, 4u, vec4<bool>(true, false, true, false));
    auto* vec4_i32 = vec(nullptr, 4u, vec4<i32>(1_i, 2_i, 3_i, 4_i));
    auto* vec4_u32 = vec(nullptr, 4u, vec4<u32>(1_u, 2_u, 3_u, 4_u));
    auto* vec4_f32 = vec(nullptr, 4u, vec4<f32>(1_f, 2_f, 3_f, 4_f));
    auto* vec4_f16 = vec(nullptr, 4u, vec4<f16>(1_h, 2_h, 3_h, 4_h));
    WrapInFunction(vec4_bool, vec4_i32, vec4_u32, vec4_f32, vec4_f16);
...
TEST_F(ResolverTypeInitializerValidationTest, InferVec4ElementTypeFromScalarAndVec3) {
    ...
    auto* vec4_bool = vec(nullptr, 4u, Expr(true), vec3<bool>(false, true, false));
    auto* vec4_i32 = vec(nullptr, 4u, Expr(1_i), vec3<i32>(2_i, 3_i, 4_i));
    auto* vec4_u32 = vec(nullptr, 4u, Expr(1_u), vec3<u32>(2_u, 3_u, 4_u));
    auto* vec4_f32 = vec(nullptr, 4u, Expr(1_f), vec3<f32>(2_f, 3_f, 4_f));
    auto* vec4_f16 = vec(nullptr, 4u, Expr(1_h), vec3<f16>(2_h, 3_h, 4_h));
    WrapInFunction(vec4_bool, vec4_i32, vec4_u32, vec4_f32, vec4_f16);
...
TEST_F(ResolverTypeInitializerValidationTest, InferVec4ElementTypeFromVec2AndVec2) {
    ...
    auto* vec4_bool = vec(nullptr, 4u, vec2<bool>(true, false), vec2<bool>(true, false));
    auto* vec4_i32 = vec(nullptr, 4u, vec2<i32>(1_i, 2_i), vec2<i32>(3_i, 4_i));
    auto* vec4_u32 = vec(nullptr, 4u, vec2<u32>(1_u, 2_u), vec2<u32>(3_u, 4_u));
    auto* vec4_f32 = vec(nullptr, 4u, vec2<f32>(1_f, 2_f), vec2<f32>(3_f, 4_f));
    auto* vec4_f16 = vec(nullptr, 4u, vec2<f16>(1_h, 2_h), vec2<f16>(3_h, 4_h));
    WrapInFunction(vec4_bool, vec4_i32, vec4_u32, vec4_f32, vec4_f16);
