...
class SymbolTable {
  public:
    ...
    /// Wrap sets this symbol table to hold symbols which point to the allocated names in `o`.
    /// The symbol table after Wrap is intended to temporarily extend the objects
    /// of an existing immutable SymbolTable
    /// As the copied objects are owned by `o`, `o` must not be destructed
  ...
  private:
    ...
    Symbol RegisterInternal(const std::string_view& name);
    ...
    utils::Hashmap<std::string_view, size_t, 0> last_prefix_to_index_;
