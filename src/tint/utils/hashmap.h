...
class Hashmap : public HashmapBase<KEY, VALUE, N, HASH, EQUAL> {
  ...
  public:
    ...
    auto GetOrZero(K&& key) {
        auto res = Add(std::forward<K>(key), Value{});
        return Reference<std::remove_reference_t<K>>(*this, key, res.value);
    }
    ...
    template <typename K>
    auto Find(K&& key) {
        return Reference<std::remove_reference_t<K>>(*this, std::forward<K>(key));
    }
    ...
    auto Find(K&& key) const {
        return ConstReference<std::remove_reference_t<K>>(*this, std::forward<K>(key));
    }
