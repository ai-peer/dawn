...
class Vector {
  ...
  private:
    ...
    struct ImplWithSmallArray {
        ...
        /// Assigns the slice fields to their initial values on Vector construction
        void Initialize() {
...
class VectorRef {
  ...
  public:
    ...
    VectorRef(const Vector<T, N>& vector)  // NOLINT(runtime/explicit)
        : slice_(const_cast<Slice&>(vector.impl_.slice)), can_move_(false) {}
