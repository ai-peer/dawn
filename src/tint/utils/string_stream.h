...
class StringStream {
  public:
    ...
    StringStream& operator<<(long long value) { return EmitValue(value); }
    /// Emit `value` to the stream
    /// @param value the value to emit
    /// @returns a reference to this
    // NOLINTNEXTLINE(runtime/int)
    StringStream& operator<<(unsigned long long value) { return EmitValue(value); }
    /// Emit `value` to the stream
    /// @param value the value to emit
    /// @returns a reference to this
    ...
    // On MSVC the type of `std::setw` and `std::setprecision` are the same.
#if !defined(_MSC_VER)
    /// @param value the value to emit
    /// @returns a reference to this
    template <typename T,
