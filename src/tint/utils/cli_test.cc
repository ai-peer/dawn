...
TEST_F(CLITest, ShowHelp_ValueWithShortName) {
    OptionSet opts;
    auto& opt = opts.Add<ValueOption<int>>();
    opt.name = "my_option";
    opt.short_name = "a";
    opt.description = "sets the awesome value";
...
TEST_F(CLITest, ShowHelp_LongName) {
    OptionSet opts;
    auto& opt = opts.Add<ValueOption<int>>();
    opt.name = "an-option-with-a-really-really-long-name";
    opt.short_name = "a";
    opt.description = "this is an option that has a silly long name";
...
--an-option-with-a-really-really-long-name <value>
 ...
 -a  alias for --an-option-with-a-really-really-long-name
)");
}
...
TEST_F(CLITest, ParseBool_Flag) {
    ...
    EXPECT_THAT(res.Get(), testing::ElementsAre("unconsumed"));
...
TEST_F(CLITest, ParseBool_ExplicitTrue) {
    ...
    ASSERT_TRUE(res) << err.str();
    EXPECT_TRUE(err.str().empty());
    EXPECT_THAT(res.Get(), testing::ElementsAre());
    EXPECT_EQ(opt.value, true);
}
...
TEST_F(CLITest, ParseUnconsumed) {
    ...
    ASSERT_TRUE(res) << err.str();
    EXPECT_TRUE(err.str().empty());
    EXPECT_THAT(res.Get(), testing::ElementsAre("abc", "def"));
