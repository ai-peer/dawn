...
struct SwitchReturnTypeImpl</*IS_CASTABLE*/ true, tint::detail::Infer, CASE_RETURN_TYPES...> {
  private:
    using InferredType =
        tint::CastableCommonBase<NullptrToIgnore<std::remove_pointer_t<CASE_RETURN_TYPES>>...>;
...
template <typename RETURN_TYPE = tint::detail::Infer,
          typename T = tint::CastableBase,
          typename... CASES>
