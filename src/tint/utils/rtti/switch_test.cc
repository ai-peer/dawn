...
TEST(Castable, SwitchInferCastableReturnTypeWithDefault) {
    ...
    }
    {
        auto* result = Switch(
            ...
            [](Default) -> tint::CastableBase* { return nullptr; });
        static_assert(std::is_same_v<decltype(result), tint::CastableBase*>);
        EXPECT_EQ(result, nullptr);
    }
}
...
TEST(Castable, SwitchExplicitCastableReturnTypeWithDefault) {
    ...
    {
        auto* result = Switch<tint::CastableBase>(
            bear.get(),                   //
            [](Mammal* p) { return p; },  //
            [](Amphibian* p) { return const_cast<const Amphibian*>(p); },
            [](Default) { return nullptr; });
        static_assert(std::is_same_v<decltype(result), const tint::CastableBase*>);
        EXPECT_EQ(result, bear.get());
    }
    {
