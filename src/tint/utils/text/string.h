...
inline tint::Vector<std::string_view, 8> Split(std::string_view str, std::string_view delimiter) {
    tint::Vector<std::string_view, 8> out;
...
template <typename T>
inline std::string Join(tint::VectorRef<T> parts, std::string_view delimiter) {
    tint::StringStream s;
    for (auto& part : parts) {
        if (part != parts.Front()) {
            s << delimiter;
...
inline std::string Join(const tint::Vector<T, N>& parts, std::string_view delimiter) {
    return Join(tint::VectorRef<T>(parts), delimiter);
}
