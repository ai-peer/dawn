...
template <size_t N>
tint::Vector<std::string, N> ToStringList(const tint::Vector<std::string_view, N>& views) {
...
TEST(StringTest, Join) {
    EXPECT_EQ(Join(tint::Vector<int, 1>{}, ","), "");
    EXPECT_EQ(Join(tint::Vector{1, 2, 3}, ","), "1,2,3");
    EXPECT_EQ(Join(tint::Vector{"cat"}, ","), "cat");
    EXPECT_EQ(Join(tint::Vector{"cat", "dog"}, ","), "cat,dog");
}
