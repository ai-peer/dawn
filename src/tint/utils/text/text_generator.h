...
class TextGenerator {
  public:
    ...
    struct LineWriter {
      public:
        ...
        /// @returns the tint::StringStream
        operator tint::StringStream&() { return os; }

        /// @param rhs the value to write to the line
        /// @returns the tint::StringStream so calls can be chained
        template <typename T>
        tint::StringStream& operator<<(T&& rhs) {
  ...
  protected:
    ...
    struct ScopedParen {
        /// Constructor
        /// @param stream the tint::StringStream that will be written to
        explicit ScopedParen(tint::StringStream& stream);
        /// Destructor
        ~ScopedParen();
