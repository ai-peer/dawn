...
/// Hasher specialization for tint::Vector
template <typename T, size_t N>
struct Hasher<tint::Vector<T, N>> {
    /// @param vector the Vector to hash
    /// @returns a hash of the Vector
    size_t operator()(const tint::Vector<T, N>& vector) const {
...
/// Hasher specialization for tint::VectorRef
template <typename T>
struct Hasher<tint::VectorRef<T>> {
    /// @param vector the VectorRef reference to hash
    /// @returns a hash of the Vector
    size_t operator()(const tint::VectorRef<T>& vector) const {
        auto hash = Hash(vector.Length());
        for (auto& el : vector) {
            hash = HashCombine(hash, el);
        }
...
class hash<tint::UnorderedKeyWrapper<T>> {
  public:
    ...
    inline std::size_t operator()(const tint::UnorderedKeyWrapper<T>& w) const {
        return w.hash;
    }
};
