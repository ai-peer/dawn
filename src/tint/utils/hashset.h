...
class Hashset {
  public:
    ...
    void Reserve(size_t new_capacity) {
        const size_t num_slots = new_capacity * kRehashFactor;
        if (slots_.Length() >= num_slots) {
            return;
        }
        Vector<Slot, N> slots;
        slots.Resize(num_slots);
        std::swap(slots, slots_);
        count_ = 0;
        for (auto& slot : slots) {
            if (slot.alive) {
  ...
  private:
    static constexpr float kRehashFactor = 1.5f;
    ...
    std::tuple<bool, size_t> IndexOf(const V& value) const {
        const auto [start, hash] = IndexAndHash(value);
    ...
    bool ShouldRehash(size_t count) const { return (count * kRehashFactor) > slots_.Length(); }
    ...
    struct Slot alignas(kAlignment) {
