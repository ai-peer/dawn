...
TEST(TintVectorTest, ConstructLength_NoSpill) {
    Vector<int, 2> vec(2);
...
TEST(TintVectorTest, ConstructLength_WithSpill) {
    Vector<int, 2> vec(3);
...
TEST(TintVectorTest, ConstructLengthValue_NoSpill) {
...
TEST(TintVectorTest, ConstructLengthValue_WithSpill) {
...
TEST(TintVectorTest, Resize_NoSpill) {
    Vector<std::string, 2> vec;
    EXPECT_EQ(vec.Length(), 0u);
    EXPECT_EQ(vec.Capacity(), 2u);
    vec.Resize(1);
...
TEST(TintVectorTest, Resize_WithSpill) {
    Vector<std::string, 1> vec;
    EXPECT_EQ(vec.Length(), 0u);
    EXPECT_EQ(vec.Capacity(), 1u);
    vec.Resize(1);
