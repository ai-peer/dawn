...
class OptionSet {
  public:
    ...
    using Unconsumed = tint::Vector<std::string_view, 8>;
    ...
    Result<Unconsumed> Parse(std::ostream& err, tint::VectorRef<std::string_view> arguments);

  private:
    /// The list of options to parse
    tint::BlockAllocator<Option, 1024> options;
};
...
class EnumOption : public Option {
  public:
    ...
    tint::Vector<EnumName<ENUM>, 8> enum_names;
    /// The default value.
    ...
    EnumOption(std::string option_name,
               ...
               tint::VectorRef<EnumName<ENUM>> names,
               SETTINGS&&... settings)
