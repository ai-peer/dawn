...
void OptionSet::ShowHelp(std::ostream& s_out) {
    tint::Vector<const Option*, 32> sorted_options;
    ...
    tint::Vector<CmdInfo, 64> cmd_infos;
...
Result<OptionSet::Unconsumed> OptionSet::Parse(std::ostream& s_err,
                                               tint::VectorRef<std::string_view> arguments_raw) {
    // Build a map of name to option, and set defaults
    tint::Hashmap<std::string, Option*, 32> options_by_name;
    for (auto* opt : options.Objects()) {
        opt->SetDefault();
    ...
    tint::Hashset<Option*, 8> options_parsed;
