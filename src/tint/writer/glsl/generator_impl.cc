...
bool GeneratorImpl::EmitBitcast(std::ostream& out, const ast::BitcastExpression* expr) {
    ...
    if (src_type->is_float_scalar_or_vector() && dst_type->is_signed_integer_scalar_or_vector()) {
        ...
        if (!EmitType(out, dst_type, type::AddressSpace::kNone, builtin::Access::kReadWrite, "")) {
            return false;
        }
    }
    ScopedParen sp(out);
...
bool GeneratorImpl::EmitBitwiseBoolOp(std::ostream& out, const ast::BinaryExpression* expr) {
    ...
    if (!EmitType(out, bool_type, type::AddressSpace::kNone, builtin::Access::kReadWrite, "")) {
        return false;
    }
    ScopedParen outerCastParen(out);
    // Cast LHS to uint scalar or vector type.
    if (!EmitType(out, uint_type, type::AddressSpace::kNone, builtin::Access::kReadWrite, "")) {
        return false;
    }
    {
...
bool GeneratorImpl::EmitTypeConversion(std::ostream& out,
    ...
    if (!EmitType(out, conv->Target(), type::AddressSpace::kNone, builtin::Access::kReadWrite,
                  "")) {
        return false;
    }
    ScopedParen sp(out);
...
bool GeneratorImpl::EmitTypeInitializer(std::ostream& out,
    ...
    if (!EmitType(out, type, type::AddressSpace::kNone, builtin::Access::kReadWrite, "")) {
        return false;
    }
    ScopedParen sp(out);
...
bool GeneratorImpl::EmitDotCall(std::ostream& out,
    ...
    if (vec_ty->type()->is_integer_scalar()) {
        ...
        fn = utils::GetOrCreate(int_dot_funcs_, vec_ty, [&]() -> std::string {
            ...
            {
                std::stringstream s;
                if (!EmitType(s, vec_ty->type(), type::AddressSpace::kNone, builtin::Access::kRead,
                              "")) {
                    return "";
                }
                v = s.str();
            ...
            {  // (u)int tint_int_dot([i|u]vecN a, [i|u]vecN b) {
                auto l = line(&b);
                if (!EmitType(l, vec_ty->type(), type::AddressSpace::kNone, builtin::Access::kRead,
                              "")) {
                    return "";
                }
                l << " " << fn_name << "(";
                if (!EmitType(l, vec_ty, type::AddressSpace::kNone, builtin::Access::kRead, "")) {
                    return "";
                }
                l << " a, ";
                if (!EmitType(l, vec_ty, type::AddressSpace::kNone, builtin::Access::kRead, "")) {
...
bool GeneratorImpl::EmitConstant(std::ostream& out, const constant::Value* constant) {
    return Switch(
        ...
        [&](const type::Vector* v) {
            if (!EmitType(out, v, type::AddressSpace::kNone, builtin::Access::kUndefined, "")) {
                return false;
            }
        ...
        [&](const type::Matrix* m) {
            if (!EmitType(out, m, type::AddressSpace::kNone, builtin::Access::kUndefined, "")) {
                return false;
            }
        ...
        [&](const type::Array* a) {
            if (!EmitType(out, a, type::AddressSpace::kNone, builtin::Access::kUndefined, "")) {
                return false;
            }
...
bool GeneratorImpl::EmitZeroValue(std::ostream& out, const type::Type* type) {
    if (type->Is<type::Bool>()) {
        ...
        if (!EmitType(out, type, type::AddressSpace::kNone, builtin::Access::kUndefined, "")) {
            return false;
        }
        bool first = true;
        ScopedParen sp(out);
...
bool GeneratorImpl::EmitStructMembers(TextBuffer* b, const sem::Struct* str) {
    ...
    for (auto* mem : str->Members()) {
        ...
        if (!EmitTypeAndName(out, ty, type::AddressSpace::kNone, builtin::Access::kReadWrite,
                             name)) {
            return false;
        }
        out << ";";
...
bool GeneratorImpl::EmitLet(const ast::Let* let) {
    ...
    if (!EmitTypeAndName(out, type, type::AddressSpace::kNone, builtin::Access::kUndefined,
