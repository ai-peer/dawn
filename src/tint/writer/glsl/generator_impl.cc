...
bool GeneratorImpl::EmitStructMembers(TextBuffer* b, const sem::Struct* str) {
    ...
    for (auto* mem : str->Members()) {
        ...
        if (host_shareable) {
            if (offset > mem->Offset()) {
                TINT_ICE(Writer, diagnostics_)
                    << "Structure member GLSL offset (" << offset << ") is ahead of WGSL offset ("
                    << mem->Offset() << ")";
                return false;
            } else if (offset < mem->Offset()) {
        ...
        if (host_shareable) {
            ...
            if (str->UsedAs(ast::StorageClass::kUniform) && ty->Is<sem::Struct>()) {
                // GLSL structs in std140 are implicitly padded out to a multiple of 16.
    ...
    if (host_shareable && offset < str->Size()) {
