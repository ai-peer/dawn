...
bool GeneratorImpl::EmitBitcast(std::ostream& out, const ast::BitcastExpression* expr) {
    ...
    if (src_type->is_float_scalar_or_vector() && dst_type->is_signed_integer_scalar_or_vector()) {
        ...
        if (!EmitType(out, dst_type, builtin::AddressSpace::kNone, builtin::Access::kReadWrite,
                      "")) {
            return false;
        }
    }
...
bool GeneratorImpl::EmitBitwiseBoolOp(std::ostream& out, const ast::BinaryExpression* expr) {
    ...
    if (!EmitType(out, bool_type, builtin::AddressSpace::kNone, builtin::Access::kReadWrite, "")) {
        return false;
    }
    ScopedParen outerCastParen(out);
    // Cast LHS to uint scalar or vector type.
    if (!EmitType(out, uint_type, builtin::AddressSpace::kNone, builtin::Access::kReadWrite, "")) {
        return false;
    }
    {
...
bool GeneratorImpl::EmitFloatModulo(std::ostream& out, const ast::BinaryExpression* expr) {
    ...
    fn = utils::GetOrCreate(float_modulo_funcs_, BinaryOperandType{{lhs_ty, rhs_ty}},
                            [&]() -> std::string {
                                ...
                                {
                                    auto decl = line(&b);
                                    if (!EmitTypeAndName(decl, ret_ty, builtin::AddressSpace::kNone,
...
bool GeneratorImpl::EmitCountOneBitsCall(std::ostream& out, const ast::CallExpression* expr) {
    ...
    // unsigned type.
    if (!EmitType(out, TypeOf(expr)->UnwrapRef(), builtin::AddressSpace::kNone,
...
bool GeneratorImpl::EmitDotCall(std::ostream& out,
    ...
    if (vec_ty->type()->is_integer_scalar()) {
        ...
        fn = utils::GetOrCreate(int_dot_funcs_, vec_ty, [&]() -> std::string {
            ...
            {
                std::stringstream s;
                if (!EmitType(s, vec_ty->type(), builtin::AddressSpace::kNone,
            ...
            {  // (u)int tint_int_dot([i|u]vecN a, [i|u]vecN b) {
                auto l = line(&b);
                if (!EmitType(l, vec_ty->type(), builtin::AddressSpace::kNone,
                              builtin::Access::kRead, "")) {
                    return "";
                }
                l << " " << fn_name << "(";
                if (!EmitType(l, vec_ty, builtin::AddressSpace::kNone, builtin::Access::kRead,
                              "")) {
                    return "";
                }
                l << " a, ";
...
bool GeneratorImpl::EmitZeroValue(std::ostream& out, const type::Type* type) {
    if (type->Is<type::Bool>()) {
        ...
        if (!EmitType(out, type, builtin::AddressSpace::kNone, builtin::Access::kUndefined, "")) {
            return false;
        }
        bool first = true;
        ScopedParen sp(out);
