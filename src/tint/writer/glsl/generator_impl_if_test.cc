...
TEST_F(GlslGeneratorImplTest_If, Emit_IfWithElseIf) {
    ...
    auto* body = Block(Return());
    auto* i =
        If(cond, body, ast::ElseStatementList{create<ast::ElseStatement>(else_cond, else_body)});
    WrapInFunction(i);
...
TEST_F(GlslGeneratorImplTest_If, Emit_IfWithElse) {
    ...
    auto* body = Block(Return());
    auto* i =
        If(cond, body, ast::ElseStatementList{create<ast::ElseStatement>(nullptr, else_body)});
    WrapInFunction(i);
