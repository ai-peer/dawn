...
TEST_F(GlslSanitizerTest, InlinePtrLetsBasic) {
    ...
    // let x : i32 = *p;
    auto* v = Var("v", ty.i32());
    auto* p = Const("p", ty.pointer<i32>(ast::StorageClass::kFunction), AddressOf(v));
...
TEST_F(GlslSanitizerTest, InlinePtrLetsComplexChain) {
    ...
    auto* ap = Const("ap", ty.pointer(ty.array(ty.mat4x4<f32>(), 4), ast::StorageClass::kFunction),
                     AddressOf(a));
    auto* mp = Const("mp", ty.pointer(ty.mat4x4<f32>(), ast::StorageClass::kFunction),
                     AddressOf(IndexAccessor(Deref(ap), 3)));
    auto* vp = Const("vp", ty.pointer(ty.vec4<f32>(), ast::StorageClass::kFunction),
