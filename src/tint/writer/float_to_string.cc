...
std::string DoubleToString(double f) {
    // Try printing the double in fixed point, with a smallish limit on the
    // precision
    std::stringstream fixed;
    fixed.flags(fixed.flags() | std::ios_base::showpoint | std::ios_base::fixed);
    fixed.precision(9);
    ...
    // If this string can be parsed without loss of information, use it
    auto double_equal_no_warning = std::equal_to<double>();
    if (double_equal_no_warning(std::stod(fixed.str()), f)) {
        auto str = fixed.str();
        while (str.length() >= 2 && str[str.size() - 1] == '0' && str[str.size() - 2] != '.') {
            str.pop_back();
        }
    ...
    // whole float
    std::stringstream sci;
    sci.precision(std::numeric_limits<double>::max_digits10);
    sci << f;
    return sci.str();
}
...
std::string DoubleToBitPreservingString(double f) {
    ...
    std::memcpy(&double_bits, &f, sizeof(double_bits));

    // Handle the sign.
    const uint64_t kSignMask = 1ull << 63;
    if (double_bits & kSignMask) {
        // If `f` is -0.0 print -0.0.
        ss << '-';
        // Strip sign bit.
        double_bits = double_bits & (~kSignMask);
    }

    switch (std::fpclassify(f)) {
        case FP_ZERO:
        case FP_NORMAL:
            std::memcpy(&f, &double_bits, sizeof(double_bits));
            ss << DoubleToString(f);
            break;
        ...
        default: {
            ...
            const uint64_t kExponentMask = 0x7ff0000000000000;
            const uint64_t kMantissaMask = 0x000fffffffffffff;
            const int kMantissaBits = 52;

            int mantissaNibbles = (kMantissaBits + 3) / 4;

            const int biased_exponent =
                static_cast<int>((double_bits & kExponentMask) >> kMantissaBits);
            int exponent = biased_exponent - kExponentBias;
            uint64_t mantissa = double_bits & kMantissaMask;
            ...
            if (exponent == 1024) {
                ...
                // Subnormal, and not zero.
                TINT_ASSERT(Writer, mantissa != 0);
                const uint64_t kTopBit = (1ull << kMantissaBits);
