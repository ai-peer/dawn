...
bool Builder::Build() {
    ...
    push_memory_model(spv::Op::OpMemoryModel, {Operand::Int(SpvAddressingModelLogical),
...
void Builder::push_capability(uint32_t cap) {
    if (capability_set_.count(cap) == 0) {
        ...
        capabilities_.push_back(Instruction{spv::Op::OpCapability, {Operand::Int(cap)}});
    }
}
...
bool Builder::GenerateBreakStatement(const ast::BreakStatement*) {
    ...
    if (!push_function_inst(spv::Op::OpBranch, {Operand::Int(merge_stack_.back())})) {
        return false;
    }
    return true;
}
...
bool Builder::GenerateContinueStatement(const ast::ContinueStatement*) {
    ...
    if (!push_function_inst(spv::Op::OpBranch, {Operand::Int(continue_stack_.back())})) {
        return false;
    }
    return true;
}
...
bool Builder::GenerateExecutionModes(const ast::Function* func, uint32_t id) {
    ...
    if (func->PipelineStage() == ast::PipelineStage::kFragment) {
        ...
        if (wgsize[0].overridable_const || wgsize[1].overridable_const ||
            ...
            push_annot(spv::Op::OpDecorate, {wgsize_result, Operand::Int(SpvDecorationBuiltIn),
    ...
    for (auto* param : func->Parameters()) {
        ...
        push_debug(spv::Op::OpName,
                   {Operand::Int(param_id),
                    Operand::String(builder_.Symbols().NameFor(param->Declaration()->symbol))});
        params.push_back(
            Instruction{spv::Op::OpFunctionParameter, {Operand::Int(param_type_id), param_op}});
...
bool Builder::GenerateFunctionVariable(const ast::Variable* var) {
    ...
    push_debug(spv::Op::OpName,
               {Operand::Int(var_id), Operand::String(builder_.Symbols().NameFor(var->symbol))});
    ...
    push_function_var({Operand::Int(type_id), result, Operand::Int(ConvertStorageClass(sc)),
                       Operand::Int(null_id)});
...
bool Builder::GenerateStore(uint32_t to, uint32_t from) {
    return push_function_inst(spv::Op::OpStore, {Operand::Int(to), Operand::Int(from)});
}
...
bool Builder::GenerateGlobalVariable(const ast::Variable* var) {
    ...
    OperandList ops = {Operand::Int(type_id), result, Operand::Int(ConvertStorageClass(sc))};
            ...
            [&](const ast::BuiltinAttribute* builtin) {
                push_annot(spv::Op::OpDecorate,
                           {Operand::Int(var_id), Operand::Int(SpvDecorationBuiltIn),
                            Operand::Int(ConvertBuiltin(builtin->builtin, sem->StorageClass()))});
                return true;
            },
...
uint32_t Builder::GenerateAccessorExpression(const ast::Expression* expr) {
    ...
    if (!info.access_chain_indices.empty()) {
        ...
        OperandList ops = {Operand::Int(result_type_id), result, Operand::Int(info.source_id)};
        for (auto id : info.access_chain_indices) {
            ops.push_back(Operand::Int(id));
        }
...
uint32_t Builder::GenerateIdentifierExpression(const ast::IdentifierExpression* expr) {
    ...
    if (val == 0) {
        error_ =
            "unable to find variable with identifier: " + builder_.Symbols().NameFor(expr->symbol);
    }
    return val;
}
...
uint32_t Builder::GenerateLoadIfNeeded(const sem::Type* type, uint32_t id) {
    ...
    auto result = result_op();
    auto result_id = result.to_i();
    if (!push_function_inst(spv::Op::OpLoad, {Operand::Int(type_id), result, Operand::Int(id)})) {
        return 0;
    }
    return result_id;
}
...
uint32_t Builder::GenerateUnaryOpExpression(const ast::UnaryOpExpression* expr) {
    ...
    if (!push_function_inst(op, {Operand::Int(type_id), result, Operand::Int(val_id)})) {
        return false;
    }
...
uint32_t Builder::GetGLSLstd450Import() {
  ...
  push_ext_import(spv::Op::OpExtInstImport, {result, Operand::String(kGLSLstd450)});
        ...
        if (auto* vec = value_type->As<sem::Vector>()) {
            ...
            for (uint32_t i = 0; i < vec->Width(); ++i) {
                ...
                if (!global_var) {
                    ...
                    if (!push_function_inst(spv::Op::OpCompositeExtract,
                                            {Operand::Int(value_type_id), extract, Operand::Int(id),
                                             Operand::Int(i)})) {
                        return false;
                    }
                    ...
                    push_type(spv::Op::OpSpecConstantOp, {Operand::Int(value_type_id), extract,
                                                          Operand::Int(SpvOpCompositeExtract),
                                                          Operand::Int(id), Operand::Int(idx_id)});
...
uint32_t Builder::GenerateCastOrCopyOrPassthrough(const sem::Type* to_type,
    ...
    if ((from_type->Is<sem::I32>() && to_type->Is<sem::F32>()) ||
        ...
        if (!push_function_inst(op, {Operand::Int(result_type_id), Operand::Int(result_id),
        ...
        op = spv::Op::OpSelect;
        if (!push_function_inst(
                op, {Operand::Int(result_type_id), Operand::Int(result_id), Operand::Int(val_id),
                     Operand::Int(one_id), Operand::Int(zero_id)})) {
            return 0;
        }
    ...
    if (!push_function_inst(op, {Operand::Int(result_type_id), result, Operand::Int(val_id)})) {
        return 0;
    }
...
uint32_t Builder::GenerateConstantIfNeeded(const ScalarConstant& constant) {
    ...
    if (constant.is_spec_op) {
        push_annot(spv::Op::OpDecorate, {Operand::Int(result_id), Operand::Int(SpvDecorationSpecId),
...
uint32_t Builder::GenerateShortCircuitBinaryExpression(const ast::BinaryExpression* expr) {
    ...
    if (!push_function_inst(
            ...
            {Operand::Int(merge_block_id), Operand::Int(SpvSelectionControlMaskNone)})) {
        return 0;
    }
    if (!push_function_inst(
            spv::Op::OpBranchConditional,
            {Operand::Int(lhs_id), Operand::Int(true_block_id), Operand::Int(false_block_id)})) {
        return 0;
    }
    ...
    if (!push_function_inst(spv::Op::OpPhi, {Operand::Int(type_id), result, Operand::Int(lhs_id),
...
uint32_t Builder::GenerateSplat(uint32_t scalar_id, const sem::Type* vec_type) {
    ...
    push_function_var({Operand::Int(GenerateTypeIfNeeded(splat_vector_type)), splat_vector,
...
uint32_t Builder::GenerateMatrixAddOrSub(uint32_t lhs_id,
    ...
    for (uint32_t i = 0; i < type->columns(); ++i) {
        ...
        if (!push_function_inst(spv::Op::OpCompositeExtract,
                                {Operand::Int(column_type_id), lhs_column_id, Operand::Int(lhs_id),
                                 Operand::Int(i)})) {
            return 0;
        }
        ...
        if (!push_function_inst(spv::Op::OpCompositeExtract,
                                {Operand::Int(column_type_id), rhs_column_id, Operand::Int(rhs_id),
                                 Operand::Int(i)})) {
            return 0;
        }
        ...
        auto result = result_op();
        if (!push_function_inst(
                op, {Operand::Int(column_type_id), result, lhs_column_id, rhs_column_id})) {
            return 0;
        }
...
uint32_t Builder::GenerateBinaryExpression(const ast::BinaryExpression* expr) {
    ...
    if (!push_function_inst(
            op, {Operand::Int(type_id), result, Operand::Int(lhs_id), Operand::Int(rhs_id)})) {
        return 0;
    }
    return result_id;
}
...
uint32_t Builder::GenerateBuiltinCall(const sem::Call* call, const sem::Builtin* builtin) {
    ...
    if (builtin->IsTexture()) {
        if (!GenerateTextureBuiltin(call, builtin, Operand::Int(result_type_id), result)) {
            return 0;
        }
        return result_id;
    }
    ...
    if (builtin->IsAtomic()) {
        if (!GenerateAtomicBuiltin(call, builtin, Operand::Int(result_type_id), result)) {
            return 0;
        }
        return result_id;
    }
    ...
    switch (builtin->Type()) {
        ...
        case BuiltinType::kArrayLength: {
            ...
            params.push_back(
                Operand::Int(uint32_t(type->As<sem::Struct>()->Declaration()->members.size() - 1)));
        ...
        case BuiltinType::kMix: {
            ...
            if (!push_function_inst(
                    spv::Op::OpExtInst,
                    {Operand::Int(result_type_id), result, std450, Operand::Int(GLSLstd450FMix),
                     Operand::Int(a_id), Operand::Int(b_id), Operand::Int(f_id)})) {
                return 0;
            }
            return result_id;
        }
            ...
            if (swizzle.size() > 1) {
                ...
                post_emission = [=] {
                    return push_function_inst(
                        spv::Op::OpCompositeExtract,
                        {result_type, result_id, spirv_result, Operand::Int(swizzle[0])});
                };
            }
            auto spirv_result_type_id = GenerateTypeIfNeeded(spirv_result_type);
            if (spirv_result_type_id == 0) {
    ...
    switch (builtin->Type()) {
        case BuiltinType::kTextureDimensions: {
            ...
            if (texture_type->IsAnyOf<sem::MultisampledTexture,       //
                ...
                op = spv::Op::OpImageQuerySizeLod;
                spirv_params.emplace_back(Operand::Int(GenerateLiteralIfNeeded(nullptr, &i32_0)));
            }
            break;
        }
        ...
        case BuiltinType::kTextureSampleLevel: {
            ...
            if (arg(Usage::kLevel)->Type()->UnwrapRef()->Is<sem::I32>()) {
                ...
                level = result_op();
                if (!push_function_inst(spv::Op::OpConvertSToF, {Operand::Int(f32_type_id), level,
                                                                 gen_arg(Usage::kLevel)})) {
                    return 0;
                }
            } else {
        ...
        case BuiltinType::kTextureSampleCompareLevel: {
            ...
            image_operands.emplace_back(ImageOperand{
                SpvImageOperandsLodMask, Operand::Int(GenerateLiteralIfNeeded(nullptr, &float_0))});
            break;
        }
        default:
...
uint32_t Builder::GenerateSampledImage(const sem::Type* texture_type,
    ...
    if (!push_function_inst(spv::Op::OpSampledImage,
                            {Operand::Int(sampled_image_type_id), sampled_image, texture_operand,
                             sampler_operand})) {
        return 0;
    }
...
bool Builder::GenerateConditionalBlock(const ast::Expression* cond,
    ...
    auto false_block_id = cur_else_idx < else_stmts.size() ? next_id() : merge_block_id;

    if (!push_function_inst(
            spv::Op::OpBranchConditional,
            {Operand::Int(cond_id), Operand::Int(true_block_id), Operand::Int(false_block_id)})) {
        return false;
    }
    ...
    if (false_block_id != merge_block_id) {
        ...
        if (!else_stmt->condition) {
            ...
            if (!GenerateConditionalBlock(else_stmt->condition, else_stmt->body, cur_else_idx + 1,
                                          else_stmts)) {
                return false;
            }
        }
...
bool Builder::GenerateIfStatement(const ast::IfStatement* stmt) {
    if (!continuing_stack_.empty() &&
        ...
        if (is_just_a_break(stmt->body) && stmt->else_statements.empty()) {
            ...
            if (es.size() == 1 && !es.back()->condition && is_just_a_break(es.back()->body)) {
                // It's a break-unless.
                TINT_ASSERT(Writer, !backedge_stack_.empty());
                const auto cond_id = GenerateExpressionWithLoadIfNeeded(stmt->condition);
                if (!cond_id) {
    ...
    if (!GenerateConditionalBlock(stmt->condition, stmt->body, 0, stmt->else_statements)) {
        return false;
    }
    return true;
}
...
bool Builder::GenerateSwitchStatement(const ast::SwitchStatement* stmt) {
    ...
    for (uint32_t i = 0; i < body.size(); i++) {
        ...
        if (LastIsFallthrough(item->body)) {
            ...
            if (!push_function_inst(spv::Op::OpBranch, {Operand::Int(case_ids[i + 1])})) {
                return false;
            }
...
bool Builder::GenerateLoopStatement(const ast::LoopStatement* stmt) {
    ...
    backedge_stack_.emplace_back(spv::Op::OpBranch, OperandList{Operand::Int(loop_header_id)});
    ...
    // manage scope outside of GenerateBlockStatement.
    {
        ...
        // We only branch if the last element of the body didn't already branch.
        if (InsideBasicBlock()) {
            if (!push_function_inst(spv::Op::OpBranch, {Operand::Int(continue_block_id)})) {
                return false;
            }
        }
...
bool Builder::GenerateTextureType(const sem::Texture* texture, const Operand& result) {
    ...
    push_type(spv::Op::OpTypeImage,
              {result, Operand::Int(type_id), Operand::Int(dim_literal),
               Operand::Int(depth_literal), Operand::Int(array_literal), Operand::Int(ms_literal),
               Operand::Int(sampled_literal), Operand::Int(format_literal)});
...
bool Builder::GenerateArrayType(const sem::Array* ary, const Operand& result) {
    ...
    if (ary->IsRuntimeSized()) {
        ...
        push_type(spv::Op::OpTypeArray, {result, Operand::Int(elem_type), Operand::Int(len_id)});
    }
...
bool Builder::GeneratePointerType(const sem::Pointer* ptr, const Operand& result) {
    ...
    push_type(spv::Op::OpTypePointer, {result, Operand::Int(stg_class), Operand::Int(subtype_id)});
...
uint32_t Builder::GenerateStructMember(uint32_t struct_id,
    ...
    push_annot(spv::Op::OpMemberDecorate,
               {Operand::Int(struct_id), Operand::Int(idx), Operand::Int(SpvDecorationOffset),
                Operand::Int(member->Offset())});

    // Infer and emit matrix layout.
    auto* matrix_type = GetNestedMatrixType(member->Type());
    if (matrix_type) {
        push_annot(spv::Op::OpMemberDecorate, {Operand::Int(struct_id), Operand::Int(idx),
        ...
        push_annot(
            ...
            {Operand::Int(struct_id), Operand::Int(idx), Operand::Int(SpvDecorationMatrixStride),
...
bool Builder::GenerateVectorType(const sem::Vector* vec, const Operand& result) {
    ...
    push_type(spv::Op::OpTypeVector, {result, Operand::Int(type_id), Operand::Int(vec->Width())});
    return true;
}
...
void Builder::AddInterpolationDecorations(uint32_t id,
    ...
    switch (type) {
        ...
        case ast::InterpolationType::kFlat:
            push_annot(spv::Op::OpDecorate, {Operand::Int(id), Operand::Int(SpvDecorationFlat)});
            break;
    ...
    switch (sampling) {
        ...
        case ast::InterpolationSampling::kSample:
            push_capability(SpvCapabilitySampleRateShading);
            push_annot(spv::Op::OpDecorate, {Operand::Int(id), Operand::Int(SpvDecorationSample)});
            break;
