...
TEST_F(SpvGeneratorImplTest, If_TrueEmpty_FalseEmpty) {
    ...
    b.Branch(i->merge.target->As<ir::Block>(), func->end_target, utils::Empty);

    b.Branch(func->start_target, i, utils::Empty);
...
TEST_F(SpvGeneratorImplTest, If_FalseEmpty) {
    ...
    b.Branch(true_block, i->merge.target, utils::Empty);
...
TEST_F(SpvGeneratorImplTest, If_TrueEmpty) {
    ...
    false_block->instructions.Push(
        b.Add(mod.types.Get<type::I32>(), b.Constant(1_i), b.Constant(1_i)));
    b.Branch(false_block, i->merge.target, utils::Empty);
...
TEST_F(SpvGeneratorImplTest, If_BothBranchesReturn) {
    ...
    auto* i = b.CreateIf(b.Constant(true));
    b.Branch(i->true_.target->As<ir::Block>(), func->end_target, utils::Empty);
    b.Branch(i->false_.target->As<ir::Block>(), func->end_target, utils::Empty);
    b.Branch(i->merge.target->As<ir::Block>(), func->end_target, utils::Empty);
