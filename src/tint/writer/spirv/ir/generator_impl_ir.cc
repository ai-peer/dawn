...
void GeneratorImplIr::EmitBlock(const ir::Block* block) {
    ...
    if (block->Instructions().IsEmpty() || !block->Instructions().Back()->Is<ir::Branch>()) {
...
void GeneratorImplIr::EmitIf(const ir::If* i) {
    ...
    if (true_block->Instructions().Length() > 1 || i->Type() != nullptr ||
        (true_block->HasBranchTarget() && !true_block->Branch()->Is<ir::ExitIf>())) {
        true_label = Label(true_block);
    }
    if (false_block->Instructions().Length() > 1 || i->Type() != nullptr ||
    ...
    // Emit Phi nodes for the exit value
    if (auto* ty = i->Type()) {
        auto emit_phi = [&](const type::Type* arg_ty, size_t arg_idx) {
            auto id = module_.NextId();

            OperandList ops{Type(arg_ty), id};
            for (auto* block : {true_block, false_block}) {
                if (block) {
                    if (auto* exit = As<ir::ExitIf>(block->Branch())) {
                        ops.push_back(Value(exit->Args()[arg_idx]));
                        ops.push_back(Label(block));
                    }
                }
            }
        ...
        if (auto* tuple = ty->As<type::Tuple>()) {
            auto id = module_.NextId();
            OperandList ops{Type(ty), id};
            for (size_t arg_idx = 0; arg_idx < tuple->Types().Length(); arg_idx++) {
                ops.push_back(emit_phi(tuple->Types()[arg_idx], arg_idx));
            }
            current_function_.push_inst(spv::Op::OpCompositeConstruct, std::move(ops));
            values_.Add(i, id);
        } else {
            values_.Add(i, emit_phi(ty, 0));
        }
    }
}
