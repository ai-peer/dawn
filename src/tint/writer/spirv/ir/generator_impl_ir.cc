...
void GeneratorImplIr::EmitAccess(const ir::Access* access) {
    ...
    if (access->Object()->Type()->Is<type::Pointer>()) {
        ...
        // For non-pointer types, we assume that the indices are all constants and use
            ...
            if (auto* constant = idx->As<ir::Constant>()) {
                ...
                ty = Switch(
                    ...
                    [&](const type::Struct* str) {
                        TINT_ASSERT(Writer, i < str->Members().Length());
                        return str->Members()[i]->Type();
                    },
                    [&](const type::Vector* vec) { return vec->type(); },
                    [&](Default) { return nullptr; });
            } else {
                // If the index is not a constant then we must have reached a vector, so we use
