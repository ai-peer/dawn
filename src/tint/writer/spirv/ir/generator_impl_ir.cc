        ...
        Switch(
            ...
            [&](const type::Reference* ref) {
                module_.PushType(spv::Op::OpTypePointer,
                                 {id, StorageClass(ref->AddressSpace()), Type(ref->StoreType())});
            },
            [&](Default) {
...
void GeneratorImplIr::EmitIf(const ir::If* i) {
    ...
    if (!true_block->instructions.IsEmpty() || i->true_.target != merge_block) {
        true_label = Label(true_block);
    }
    if (!false_block->instructions.IsEmpty() || i->false_.target != merge_block) {
...
uint32_t GeneratorImplIr::EmitVar(const ir::Var* var) {
    ...
    if (var->address_space == builtin::AddressSpace::kFunction) {
        ...
        TINT_ICE(Writer, diagnostics_)
            << "unimplemented variable address space " << var->address_space;
        return 0u;
    }
