...
class Builder {
  public:
    ...
    void GenerateMemberAccessIfNeeded(type::Type* type, uint32_t struct_id, uint32_t member_idx);
    ...
    uint32_t GenerateLoad(type::Reference* type, uint32_t id);
    ...
    bool GenerateReferenceType(type::Reference* ref, const Operand& result);
    /// Generates a vector type declaration
    /// @param struct_type the vector to generate
    /// @param result the result operand
    /// @returns true if the vector was successfully generated
    bool GenerateStructType(type::Struct* struct_type, const Operand& result);
    ...
    uint32_t GenerateStructMember(uint32_t struct_id, uint32_t idx, type::StructMember* member);
    ...
    uint32_t GenerateSplat(uint32_t scalar_id, type::Type* vec_type);
  ...
  private:
    ...
    /// @returns the ID on success or 0 on failure
    uint32_t GenerateConstantVectorSplatIfNeeded(type::Vector* type, uint32_t value_id);
    ...
    std::unordered_map<type::Type*, uint32_t> type_to_id_;
    std::unordered_map<ScalarConstant, uint32_t> const_to_id_;
    std::unordered_map<type::Type*, uint32_t> const_null_to_id_;
    std::unordered_map<uint64_t, uint32_t> const_splat_to_id_;
    std::unordered_map<type::Type*, uint32_t> texture_type_to_sampled_image_type_id_;
