...
TEST_F(AppendVectorTest, Vec2i32_u32) {
    ...
    EXPECT_EQ(Symbols().NameFor(u32_to_i32->target.type->As<ast::TypeName>()->name), "i32");
...
TEST_F(AppendVectorTest, Vec2i32FromVec2u32_u32) {
    ...
    EXPECT_EQ(Symbols().NameFor(
                  v2u32_to_v2i32->target.type->As<ast::Vector>()->type->As<ast::TypeName>()->name),
              "i32");
    ...
    ASSERT_NE(u32_to_i32, nullptr);
    ASSERT_TRUE(u32_to_i32->target.type->As<ast::Vector>()->type->Is<ast::TypeName>());
    EXPECT_EQ(Symbols().NameFor(
                  u32_to_i32->target.type->As<ast::Vector>()->type->As<ast::TypeName>()->name),
              "i32");
    ASSERT_EQ(u32_to_i32->args.Length(), 1u);
    EXPECT_EQ(u32_to_i32->args[0], scalar_3);
...
TEST_F(AppendVectorTest, Vec2i32_f32) {
    ...
    ASSERT_TRUE(f32_to_i32->target.type->As<ast::Vector>()->type->Is<ast::TypeName>());
    EXPECT_EQ(Symbols().NameFor(
                  f32_to_i32->target.type->As<ast::Vector>()->type->As<ast::TypeName>()->name),
              "i32");
