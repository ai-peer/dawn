...
const sem::Call* AppendVector(ProgramBuilder* b,
    ...
    utils::List<const sem::Expression*, 4> packed;
    ...
    if (packed_el_sem_ty != scalar_sem->Type()->UnwrapRef()) {
        ...
        auto* scalar_cast_sem = b->create<sem::Call>(
            scalar_cast_ast, scalar_cast_target, utils::List<const sem::Expression*>{scalar_sem},
    ...
    auto* constructor_ast = b->Construct(
        packed_ast_ty, utils::ToVector(utils::Transform(packed, [&](const sem::Expression* expr) {
    ...
    auto* constructor_sem =
        b->create<sem::Call>(constructor_ast, constructor_target, std::move(packed), statement,
