...
TEST_F(HlslGeneratorImplTest_Bitcast, EmitExpression_Bitcast_F16_Vec2) {
    ...
    EXPECT_EQ(gen.Result(), R"(int tint_bitcast(vector<float16_t, 2> src) {
  uint2 r = f32tof16(float2(src));
  return asint(uint((r.x & 0xffff) | ((r.y & 0xffff) << 16)));
}

vector<float16_t, 2> tint_bitcast_1(int src) {
  uint v = asuint(src);
...
float tint_bitcast_2(vector<float16_t, 2> src) {
  uint2 r = f32tof16(float2(src));
  return asfloat(uint((r.x & 0xffff) | ((r.y & 0xffff) << 16)));
}

vector<float16_t, 2> tint_bitcast_3(float src) {
  uint v = asuint(src);
  float t_low = f16tof32(v & 0xffff);
  float t_high = f16tof32((v >> 16) & 0xffff);
  return vector<float16_t, 2>(t_low.x, t_high.x);
...
uint tint_bitcast_4(vector<float16_t, 2> src) {
  uint2 r = f32tof16(float2(src));
  return asuint(uint((r.x & 0xffff) | ((r.y & 0xffff) << 16)));
}

vector<float16_t, 2> tint_bitcast_5(uint src) {
  uint v = asuint(src);
  float t_low = f16tof32(v & 0xffff);
  float t_high = f16tof32((v >> 16) & 0xffff);
  return vector<float16_t, 2>(t_low.x, t_high.x);
...
void test_function() {
  ...
  const vector<float16_t, 2> c = tint_bitcast_1(b);
  const float d = tint_bitcast_2(c);
  const vector<float16_t, 2> e = tint_bitcast_3(d);
  const uint f = tint_bitcast_4(e);
  const vector<float16_t, 2> g = tint_bitcast_5(f);
  return;
}
)");
}
...
TEST_F(HlslGeneratorImplTest_Bitcast, EmitExpression_Bitcast_F16_Vec4) {
    ...
    gen.Generate();
    EXPECT_THAT(gen.Diagnostics(), testing::IsEmpty());
    EXPECT_EQ(gen.Result(), R"(int2 tint_bitcast(vector<float16_t, 4> src) {
  uint4 r = f32tof16(float4(src));
  return asint(uint2((r.x & 0xffff) | ((r.y & 0xffff) << 16), (r.z & 0xffff) | ((r.w & 0xffff) << 16)));
}

vector<float16_t, 4> tint_bitcast_1(int2 src) {
  uint2 v = asuint(src);
...
float2 tint_bitcast_2(vector<float16_t, 4> src) {
  uint4 r = f32tof16(float4(src));
  return asfloat(uint2((r.x & 0xffff) | ((r.y & 0xffff) << 16), (r.z & 0xffff) | ((r.w & 0xffff) << 16)));
}

vector<float16_t, 4> tint_bitcast_3(float2 src) {
  uint2 v = asuint(src);
  float2 t_low = f16tof32(v & 0xffff);
  float2 t_high = f16tof32((v >> 16) & 0xffff);
  return vector<float16_t, 4>(t_low.x, t_high.x, t_low.y, t_high.y);
...
uint2 tint_bitcast_4(vector<float16_t, 4> src) {
  uint4 r = f32tof16(float4(src));
  return asuint(uint2((r.x & 0xffff) | ((r.y & 0xffff) << 16), (r.z & 0xffff) | ((r.w & 0xffff) << 16)));
}

vector<float16_t, 4> tint_bitcast_5(uint2 src) {
  uint2 v = asuint(src);
  float2 t_low = f16tof32(v & 0xffff);
  float2 t_high = f16tof32((v >> 16) & 0xffff);
  return vector<float16_t, 4>(t_low.x, t_high.x, t_low.y, t_high.y);
...
void test_function() {
  ...
  const int2 b = tint_bitcast(a);
  const vector<float16_t, 4> c = tint_bitcast_1(b);
  const float2 d = tint_bitcast_2(c);
  const vector<float16_t, 4> e = tint_bitcast_3(d);
  const uint2 f = tint_bitcast_4(e);
  const vector<float16_t, 4> g = tint_bitcast_5(f);
  return;
}
)");
}
