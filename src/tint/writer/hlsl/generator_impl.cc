...
bool GeneratorImpl::EmitUniformBufferAccess(
    ...
    switch (intrinsic->op) {
        case Op::kLoad: {
            ...
            auto load_32bits_scalar_to = [&](std::ostream& target) {
                ...
                if (scalar_offset_constant) {
                    ...
                    target << "[" << scalar_offset_index_expr << " / 4]["
                           << scalar_offset_index_expr << " % 4]";
                }
                return true;
            };
            auto load_32bits_scalar = [&] { return load_32bits_scalar_to(out); };
            // Has a minimum alignment of 8 bytes, so is either .xy or .zw
            auto load_vec2_with_32bits_elements_to = [&](std::ostream& target) {
                if (scalar_offset_constant) {
                    if (!EmitExpression(target, args[0])) {  // buffer
                        return false;
                    }
                    ...
                    std::string ubo_load = UniqueIdentifier("ubo_load");
                    {
                        ...
                        pre << "[" << scalar_offset_index_expr << " / 4];";
                    }
                    target << "((" << scalar_offset_index_expr << " & 2) ? " << ubo_load
                           << ".zw : " << ubo_load << ".xy)";
                }
                return true;
            };
            auto load_vec2_with_32bits_elements = [&] {
                return load_vec2_with_32bits_elements_to(out);
            };
            // vec4 has a minimum alignment of 16 bytes, easiest case
            auto load_vec4_with_32bits_elements = [&] {
                ...
                if (scalar_offset_constant) {
                    ...
                    out << "[" << scalar_offset_index_expr << " / 4]";
                }
                return true;
            };
            // vec3 has a minimum alignment of 16 bytes, so is just a .xyz swizzle
            auto load_vec3_with_32bits_elements = [&] {
                if (!load_vec4_with_32bits_elements()) {
                    return false;
                }
                out << ".xyz";
                return true;
            ...
            auto load_vec3_f16 = [&] {
                ...
                std::string ubo_load_y = UniqueIdentifier(ubo_load + "_y");
                {
                    ...
                    pre << "uint2 " << ubo_load << " = ";
                    if (!load_vec2_with_32bits_elements_to(pre)) {
                        return false;
                    }
                    pre << ";";
                }
            ...
            switch (intrinsic->type) {
                case DataType::kU32:
                    return load_32bits_scalar();
                case DataType::kF32:
                    return cast("asfloat", load_32bits_scalar);
                case DataType::kI32:
                    return cast("asint", load_32bits_scalar);
                case DataType::kF16:
                    return load_scalar_f16();
                case DataType::kVec2U32:
                    return load_vec2_with_32bits_elements();
                case DataType::kVec2F32:
                    return cast("asfloat", load_vec2_with_32bits_elements);
                case DataType::kVec2I32:
                    return cast("asint", load_vec2_with_32bits_elements);
                case DataType::kVec2F16:
                    return load_vec2_f16();
                case DataType::kVec3U32:
                    return load_vec3_with_32bits_elements();
                case DataType::kVec3F32:
                    return cast("asfloat", load_vec3_with_32bits_elements);
                case DataType::kVec3I32:
                    return cast("asint", load_vec3_with_32bits_elements);
                case DataType::kVec3F16:
                    return load_vec3_f16();
                case DataType::kVec4U32:
                    return load_vec4_with_32bits_elements();
                case DataType::kVec4F32:
                    return cast("asfloat", load_vec4_with_32bits_elements);
                case DataType::kVec4I32:
                    return cast("asint", load_vec4_with_32bits_elements);
