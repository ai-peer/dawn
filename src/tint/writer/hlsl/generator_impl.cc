...
bool GeneratorImpl::EmitConstantRange(std::ostream& out,
    ...
    return Switch(
        ...
        [&](const sem::Array* a) {
            // The PromoteInitializersToConstVar will have hoisted inline array constructors to a
            // let just above the expression that uses them. So, an array will only appear of the
            // RHS of an array-typed variable assignment, and takes the form: `{1, 2, 3}`
            out << "{";
            TINT_DEFER(out << "}");

            // step is the number of flattened elements we emit with each array element
            uint32_t step = 0;
            sem::Type::DeepestElementOf(a->ElemType(), &step);

            for (size_t i = start; i < end; i += step) {
                if (i > start) {
                    out << ", ";
                }
                if (!EmitConstantRange(out, constant, a->ElemType(), i, i + step)) {
                    return false;
                }
            }
            return true;
