    ...
    if (auto* lhs_access = stmt->lhs->As<ast::IndexAccessorExpression>()) {
        ...
        if (auto* lhs_sub_access = lhs_access->object->As<ast::IndexAccessorExpression>()) {
            if (auto* mat = TypeOf(lhs_sub_access->object)->UnwrapRef()->As<type::Matrix>()) {
                auto* rhs_col_idx_sem = builder_.Sem().Get<sem::ValueExpression>(lhs_access->index);
                auto* rhs_row_idx_sem =
                    builder_.Sem().Get<sem::ValueExpression>(lhs_sub_access->index);
        ...
        if (auto* mat = lhs_access_type->As<type::Matrix>()) {
            auto* lhs_index_sem = builder_.Sem().Get<sem::ValueExpression>(lhs_access->index);
        ...
        if (auto* vec = lhs_access_type->As<type::Vector>()) {
            auto* rhs_sem = builder_.Sem().Get<sem::ValueExpression>(lhs_access->index);
...
bool GeneratorImpl::EmitUniformBufferAccess(
    ...
    auto* offset_arg = builder_.Sem().Get<sem::ValueExpression>(args[1]);
...
bool GeneratorImpl::EmitDefaultOnlySwitch(const ast::SwitchStatement* stmt) {
    ...
    if (auto* sem_cond = builder_.Sem().Get<sem::ValueExpression>(stmt->condition);
        sem_cond->HasSideEffects()) {
        auto out = line();
        if (!EmitExpression(out, stmt->condition)) {
            return false;
        }
