...
bool GeneratorImpl::EmitBitcast(utils::StringStream& out, const ast::BitcastExpression* expr) {
    ...
    /*
    auto scalarTypeNameForIntrinsic = [&](const type::Type* ty) {
        return Switch(
            ty,  //
            [&](const type::I32*) { return "int"; }, [&](const type::U32*) { return "uint"; },
            [&](const type::F32*) { return "float"; }, [&](const type::F16*) { return "float16"; },
            [&](Default) {
                TINT_ICE(Writer, diagnostics_) << "unhandled scalar type";
                return "";
            });
    };
    */
    ...
    if (!src_el_type->Is<type::F16>() && !dst_el_type->Is<type::F16>()) {
        out << "as";
        if (!EmitType(out, dst_el_type, builtin::AddressSpace::kUndefined,
        ...
        out << ")";
        return true;
    } else if (src_el_type->Is<type::F16>()) {
        ...
        // Bitcast f16 types to others by converting the given f16 value to f32 and call f32tof16 to
        // get the bits. This should be safe, because the convertion is precise for finite and
        // infinite f16 value as they are exactly representable by f32, and WGSL spec allow any
        // result if f16 value is NaN.
        auto fn = utils::GetOrCreate(
            bitcast_polyfill_, BinaryType{{src_type, dst_type}}, [&]() -> std::string {
                TextBuffer b;
                TINT_DEFER(helpers_.Append(b));

                auto fn_name = UniqueIdentifier(std::string("tint_bitcast"));
                {
                    auto decl = Line(&b);
                    ...
                    {
                        ScopedParen sp(decl);
                        if (!EmitTypeAndName(decl, src_type, builtin::AddressSpace::kUndefined,
        ...
        // Bitcast other types to f16 types by reinterpreting their bits as f16 using f16tof32,
        // and convert the result f32 to f16. This should be safe, because the convertion is precise
        // for finite and infinite f16 result value as they are exactly representable by f32, and
        // WGSL spec allow any result if f16 result value would be NaN.
        auto fn = utils::GetOrCreate(
            bitcast_polyfill_, BinaryType{{src_type, dst_type}}, [&]() -> std::string {
                TextBuffer b;
                TINT_DEFER(helpers_.Append(b));
