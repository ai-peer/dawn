...
bool GeneratorImpl::EmitStructType(TextBuffer* b, const sem::Struct* str) {
    line(b) << "struct " << StructName(str) << " {";
    {
        ...
        // We need to guarantee the allocation is stable to make it consistent for vertex outputs
        ...
        // There can be at most 32 specific register numbers for DXBC/DXIL level.
        utils::Bitset<32> attribute_locations;
        attribute_locations.Resize(32);
        ...
        for (auto* mem : str->Members()) {
            ...
            // If member is a builtin, append at the end of the struct to make sure the HLSL
            // compiler assign consistent register numbers to them.
            auto out = isBuiltIn ? line(&builtIns) : line(b);
            out << pre;
        ...
        if (emit_interstage_placeholders) {
            for (uint32_t i = 0; i < highest_attribute_location_plus_one; i++) {
                if (attribute_locations[i] == false) {
                    ...
                    auto out = line(b);
                    out << "float4 tint_interstage_placeholder_" << i << " : TEXCOORD" << i << ";";
                }
            }
        }
