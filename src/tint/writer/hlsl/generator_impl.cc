...
bool GeneratorImpl::EmitTypeConstructor(std::ostream& out,
    ...
    // Single parameter matrix initializers, must be identical constructor, as single-scalar matrix
    // constructor should be handled by VectorizeScalarMatrixConstructors transformation.
    if (type->Is<sem::Matrix>() && call->Arguments().size() == 1) {
        if (!ctor->Parameters()[0]->Type()->UnwrapRef()->is_float_matrix()) {
            TINT_UNREACHABLE(Writer, diagnostics_)
                << "single-scalar matrix constructor found, which should have been vectorized by "
                   "transform VectorizeScalarMatrixConstructors";
            return false;
        }
    }
