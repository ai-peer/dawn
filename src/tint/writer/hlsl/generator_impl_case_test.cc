...
TEST_F(HlslGeneratorImplTest_Case, Emit_Case) {
    auto* s = Switch(1_i, Case(CaseSelector(Expr(5_i)), Block(create<ast::BreakStatement>())),
                     DefaultCase());
    WrapInFunction(s);
...
TEST_F(HlslGeneratorImplTest_Case, Emit_Case_BreaksByDefault) {
    auto* s = Switch(1_i, Case(CaseSelector(Expr(5_i)), Block()), DefaultCase());
    WrapInFunction(s);
...
TEST_F(HlslGeneratorImplTest_Case, Emit_Case_WithFallthrough) {
    auto* s = Switch(1_i,                                                                        //
                     Case(CaseSelector(Expr(4_i)), Block(create<ast::FallthroughStatement>())),  //
                     Case(CaseSelector(Expr(5_i)), Block(create<ast::ReturnStatement>())),       //
                     DefaultCase());
    WrapInFunction(s);
...
TEST_F(HlslGeneratorImplTest_Case, Emit_Case_MultipleSelectors) {
    auto* s = Switch(1_i,
                     Case(utils::Vector{CaseSelector(Expr(5_i)), CaseSelector(Expr(6_i))},
