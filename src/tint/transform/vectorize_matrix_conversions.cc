...
bool VectorizeMatrixConversions::ShouldRun(const Program* program, const DataMap&) const {
    for (auto* node : program->ASTNodes().Objects()) {
        if (auto* call =
                program->Sem().Get<sem::Expression>(node)->UnwrapMaterialize()->As<sem::Call>()) {
    ...
    ctx.ReplaceAll([&](const ast::CallExpression* expr) -> const ast::CallExpression* {
        ...
        auto build_vectorized_conversion_expression = [&](auto&& src_expression_builder) {
            ...
            for (uint32_t c = 0; c < dst_type->columns(); c++) {
                columns.Push(
                    ctx.dst->Construct(CreateASTTypeFor(ctx, dst_type->ColumnType()),
                                       ctx.dst->IndexAccessor(src_expression_builder(),
        ...
        if (!src->HasSideEffects()) {
            ...
            auto fn =
                utils::GetOrCreate(matrix_convs, HelperFunctionKey{{src_type, dst_type}}, [&] {
                    auto get_scalar_name = [&](const sem::Type* scalar_type) {
                        if (scalar_type->Is<sem::F32>()) {
                            return "f32";
                        }
                        if (scalar_type->Is<sem::F16>()) {
                            return "f16";
                        }
                        TINT_ICE(Transform, ctx.dst->Diagnostics())
                    ...
                    auto name = ctx.dst->Symbols().New(
                        ...
                        std::to_string(src_type->rows()) + "_" + get_scalar_name(src_type->type()) +
                        "_" + get_scalar_name(dst_type->type()));
                    ctx.dst->Func(
                        name,
                        utils::Vector{
