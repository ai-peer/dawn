...
fn f() {
  ...
  reflect(e1, e2);
    ...
    EXPECT_FALSE(ShouldRun<BuiltinPolyfill>(src));
    EXPECT_TRUE(ShouldRun<BuiltinPolyfill>(src, polyfillReflectVec2()));
}
...
fn f() {
  ...
  reflect(e1, e2);
    ...
    EXPECT_FALSE(ShouldRun<BuiltinPolyfill>(src));
    EXPECT_FALSE(ShouldRun<BuiltinPolyfill>(src, polyfillReflectVec2()));
}
...
fn f() {
  ...
  let r : vec2<f32> = tint_reflect(vec2<f32>(v), vec2<f32>(v));
    ...
    auto got = Run<BuiltinPolyfill>(src, polyfillReflectVec2());
