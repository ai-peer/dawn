...
struct CreateASTTypeForTest : public testing::Test, public Transform {
    ...
    const ast::Identifier* create(std::function<type::Type*(ProgramBuilder&)> create_sem_type) {
...
TEST_F(CreateASTTypeForTest, Basic) {
    auto check = [&](const ast::Identifier* ident, const char* expect) {
        ast::CheckIdentifier(ast_type_builder.Symbols(), ident, expect);
    };
...
TEST_F(CreateASTTypeForTest, Matrix) {
    ...
    ast::CheckIdentifier(ast_type_builder.Symbols(), mat, "mat3x2", "f32");
}
...
TEST_F(CreateASTTypeForTest, Vector) {
    ...
    ast::CheckIdentifier(ast_type_builder.Symbols(), vec, "vec2", "f32");
}
...
TEST_F(CreateASTTypeForTest, ArrayImplicitStride) {
    ...
    ast::CheckIdentifier(ast_type_builder.Symbols(), arr, "array", "f32", 2_u);
...
TEST_F(CreateASTTypeForTest, ArrayNonImplicitStride) {
    ...
    ast::CheckIdentifier(ast_type_builder.Symbols(), arr, "array", "f32", 2_u);
    ASSERT_EQ(arr->attributes.Length(), 1u);
    ASSERT_TRUE(arr->attributes[0]->Is<ast::StrideAttribute>());
    ASSERT_EQ(arr->attributes[0]->As<ast::StrideAttribute>()->stride, 64u);
}
