...
bool MergeReturn::ShouldRun(const Program* program, const DataMap&) const {
    for (auto* func : program->AST().Functions()) {
        // TODO(jrprice): We could also use the uniformity analysis to determine which functions
        // need to be transformed, if any.
...
class State {
  ...
  public:
    ...
    void ProcessStatement(const ast::Statement* stmt) {
        ...
        auto has_behavior = [&](const ast::Statement* s, sem::Behavior behavior) {
            return ctx.src->Sem().Get(s)->Behaviors().Contains(behavior);
        };
            ...
            [&](const ast::BlockStatement* block) {
                ...
                if (block == function->body) {
                    ...
                    for (auto* s : block->statements) {
                        ...
                        if (has_behavior(s, sem::Behavior::kReturn)) {
                    ...
                    if (has_behavior(s, sem::Behavior::kReturn)) {
                        if (loop_switch_depth > 0) {
                            ...
                            if (s->IsAnyOf<ast::LoopStatement, ast::ForLoopStatement,
                                ...
                                if (has_behavior(s, sem::Behavior::kNext)) {
                                    new_stmts.Back().Push(
