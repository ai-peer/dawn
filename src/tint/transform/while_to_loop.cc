...
bool ContainsWhileLoop(const Program* program) {
...
Transform::ApplyResult WhileToLoop::Apply(const Program* src, const DataMap&, DataMap&) const {
    ...
    ctx.ReplaceAll([&](const ast::WhileStatement* w) -> const ast::Statement* {
        ...
        // !condition
        auto* not_cond = b.create<ast::UnaryOpExpression>(ast::UnaryOp::kNot, ctx.Clone(cond));
        ...
        auto* loop = b.create<ast::LoopStatement>(body, continuing);
