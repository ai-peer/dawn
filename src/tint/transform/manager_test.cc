...
class IR_AddFunction final : public ir::transform::Transform {
    void Run(ir::Module* ir, const DataMap&, DataMap&) const override {
        auto* func = ir->flow_nodes.Create<ir::Function>();
        func->return_type = ir->types.Get<type::Void>();
        func->start_target = ir->flow_nodes.Create<ir::Block>();
        func->start_target->inbound_branches.Push(func);
        func->start_target->branch.target = func->end_target;
        func->end_target = ir->flow_nodes.Create<ir::FunctionTerminator>();
        func->name = ir->symbols.New("ir_func");
...
ir::Module MakeIR() {
    ...
    func->return_type = builder.ir.types.Get<type::Void>();
    func->name = builder.ir.symbols.New("main");
    func->start_target->branch.target = func->end_target;
...
// TODO(crbug.com/tint/1902): Enable this test.
TEST_F(TransformManagerTest, DISABLED_AST_MixedTransforms_AST_Before_IR) {
    Program ast = MakeAST();
    ...
    EXPECT_EQ(result.AST().Functions()[0]->name->symbol.Name(), "main");
    EXPECT_EQ(result.AST().Functions()[1]->name->symbol.Name(), "ast_func");
    EXPECT_EQ(result.AST().Functions()[2]->name->symbol.Name(), "ir_func");
}

// TODO(crbug.com/tint/1902): Enable this test.
TEST_F(TransformManagerTest, DISABLED_AST_MixedTransforms_IR_Before_AST) {
    Program ast = MakeAST();
    ...
    ASSERT_TRUE(result.IsValid()) << result.Diagnostics();
    ASSERT_EQ(result.AST().Functions().Length(), 3u);
    EXPECT_EQ(result.AST().Functions()[1]->name->symbol.Name(), "ir_func");
    EXPECT_EQ(result.AST().Functions()[2]->name->symbol.Name(), "ast_func");
}

// TODO(crbug.com/tint/1902): Enable this test.
TEST_F(TransformManagerTest, DISABLED_IR_MixedTransforms_AST_Before_IR) {
    ir::Module ir = MakeIR();
    ...
    EXPECT_EQ(ir.functions[0]->name.Name(), "main");
    EXPECT_EQ(ir.functions[1]->name.Name(), "ast_func");
    EXPECT_EQ(ir.functions[2]->name.Name(), "ir_func");
}

// TODO(crbug.com/tint/1902): Enable this test.
TEST_F(TransformManagerTest, DISABLED_IR_MixedTransforms_IR_Before_AST) {
    ir::Module ir = MakeIR();
    ...
    EXPECT_EQ(ir.functions[0]->name.Name(), "main");
    EXPECT_EQ(ir.functions[1]->name.Name(), "ir_func");
    EXPECT_EQ(ir.functions[2]->name.Name(), "ast_func");
}
#endif  // TINT_BUILD_IR
