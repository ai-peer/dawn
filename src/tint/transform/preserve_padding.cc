...
struct PreservePadding::State {
    ...
    State(const Program* src) : ctx{&b, src, /* auto_clone_symbols */ true} {}

    /// The main function for the transform.
    /// @returns the ApplyResult
    ApplyResult Run() {
        // TODO(jrprice): Skip the transform if not needed.
        ...
        ctx.ReplaceAll([&](const ast::AssignmentStatement* assign) -> const ast::Statement* {
            ...
            if (ty->As<sem::Reference>()->AddressSpace() != ast::AddressSpace::kStorage) {
                // We only care about writes to the storage address space, as nothing else is
                // host-visible.
                return nullptr;
            }
            return MakeAssignment(ty->UnwrapRef(), ctx.Clone(assign->lhs), ctx.Clone(assign->rhs));
        });
    ...
    const ast::Statement* MakeAssignment(const sem::Type* ty,
        ...
        if (auto* arr = ty->As<sem::Array>()) {
            ...
            return call_helper([&]() {
                ...
                body.Push(b.For(b.Decl(idx), b.LessThan(idx, u32(arr->ConstantCount().value())),
                                b.Assign(idx, b.Add(idx, 1_u)),
                                b.Block(MakeAssignment(
                                    arr->ElemType(), b.IndexAccessor(b.Deref(kDestParamName), idx),
    ...
    bool HasPadding(const sem::Type* ty) {
        if (auto* arr = ty->As<sem::Array>()) {
            ...
            return (current_offset % str->Align() > 0);
        } else {
            return false;
        }
    }
