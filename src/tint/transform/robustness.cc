...
struct Robustness::State {
  ...
  private:
    ...
    void PredicateBuiltin(const sem::Call* call, const sem::Builtin* builtin) {
        if (builtin->IsTexture()) {
            if (builtin->RequiresUniformControlFlow()) {
                // texture builtin cannot be predicated without introducing uniformity violations.
                // Use clamping.
                ClampTextureBuiltin(call, builtin);
            } else {
    ...
    void PredicateTextureBuiltin(const sem::Call* call, const sem::Builtin* builtin) {
        ...
        Symbol level, num_levels;
        if (level_arg_idx >= 0) {
            // let level = u32(level-arg);
            ...
            hoist.InsertBefore(stmt,
                               b.Decl(b.Let(num_levels, b.Call(sem::BuiltinType::kTextureNumLevels,
                                                               ctx.Clone(texture_arg)))));

            // predicate: level_idx < num_levels
            predicate = And(predicate, b.LessThan(level, num_levels));
        ...
        if (coords_arg_idx >= 0) {
            ...
            auto* arg_ty = builtin->Parameters()[static_cast<size_t>(coords_arg_idx)]->Type();
            if (arg_ty->is_integer_scalar_or_vector()) {
                hoist.InsertBefore(
                    stmt,
                    b.Decl(b.Let(coords, CastToUnsigned(b.Expr(ctx.Clone(arg)), WidthOf(arg_ty)))));

                // predicate: all(coords < textureDimensions(texture))
                auto* dimensions =
                    level.IsValid()
                        ? b.Call(
                              sem::BuiltinType::kTextureDimensions, ctx.Clone(texture_arg),
                              b.Call(sem::BuiltinType::kMin, b.Expr(level), b.Sub(num_levels, 1_a)))
    ...
    /// predicate.
    void PredicateCall(const sem::Call* call, const ast::Expression* predicate) {
        auto* expr = call->Declaration();
        auto* stmt = call->Stmt();
    ...
    void ClampTextureBuiltin(const sem::Call* call, const sem::Builtin* builtin) {
        ...
        if (coords_arg_idx >= 0) {
            auto* arg = expr->args[static_cast<size_t>(coords_arg_idx)];
            const auto* arg_ty = builtin->Parameters()[static_cast<size_t>(coords_arg_idx)]->Type();
            if (arg_ty->is_integer_scalar_or_vector()) {
                const auto width = WidthOf(arg_ty);
                ...
                if (arg_ty->is_signed_integer_scalar_or_vector()) {
