...
struct Robustness::State {
    ...
    const ast::CallExpression* Transform(const ast::CallExpression* expr) {
        ...
        auto cast_to_signed = [&](const ast::Expression* expr, uint32_t width) {
            return b.Construct(scalar_or_vec_ty(b.ty.i32(), width), expr);
        };
        auto cast_to_unsigned = [&](const ast::Expression* expr, uint32_t width) {
            return b.Construct(scalar_or_vec_ty(b.ty.u32(), width), expr);
        };
