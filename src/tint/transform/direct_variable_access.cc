...
struct DirectVariableAccess::State {
  ...
  private:
    ...
    /// current holds pointers to the current function, current variant and current variant's
    /// parameters.
    struct {
    ...
    void ProcessFunction(const sem::Function* fn) {
        ...
        if (fn_info->variants.IsEmpty()) {
            ...
            if (NeedsVariantToGenerate(fn)) {
                // Drop the function, as it cannot be generated, and it wasn't called.
    ...
    void ProcessStatement(const ast::Statement* stmt) {
        ...
        Switch(
            ...
            [&](const ast::LoopStatement* s) { ProcessStatement(s->body); },
            [&](const ast::VariableDeclStatement* s) {
                ProcessExpression(s->variable->constructor);
            },
            [&](const ast::AssignmentStatement* s) { ProcessExpression(s->rhs); },
    ...
    void ProcessExpression(const ast::Expression* root_expr) {
        ...
        ast::TraverseExpressions(root_expr, b.Diagnostics(), [&](const ast::Expression* ast_expr) {
            ...
            if (IsPointerToUniformStorageWorkgroup(expr->Type()) &&
                expr->SourceVariable()->Is<sem::Parameter>()) {
                // We've found an expression that originates from a pointer-parameter, of storage
                // class uniform, workgroup or uniform.
                // This needs reconstructing using a global variable and the function variant's
                ...
                ctx.Replace(ast_expr, [this, chain] {
                    ...
                    if (auto* root_param = RootParameter(chain)) {
                        if (auto* root_indices = current.variant_params->Find(root_param)) {
                            ...
                            for (auto param_access : *current.variant_params->Find(root_param)) {
                                chain_expr = BuildAccessExpr(
                                    chain_expr, param_access,
                                    [&](size_t i) { return b.IndexAccessor(param_name, AInt(i)); });
                            }
                    ...
                    for (size_t access_idx = start_idx; access_idx < chain.indices.Length();
                         access_idx++) {
                        chain_expr =
                            BuildAccessExpr(chain_expr, chain.indices[access_idx],
                                            [&](size_t i) { return chain.dynamic_indices[i](); });
                    }

                    return b.AddressOf(chain_expr);
                });
            }
    ...
    ast::TraverseAction ProcessCall(const sem::Call* call) {
        ...
        for (auto arg : call->Arguments()) {
            if (IsPointerToUniformStorageWorkgroup(arg->Type())) {
                current.function->calls.Add(arg, AccessChainFor(arg));
            } else {
                ProcessExpression(arg->Declaration());
            }
        }

        if (current.function->calls.IsEmpty()) {
        ...
        auto build_target_variant = [&](const FnVariantPtrParams& caller_variant_params,
            ...
            for (size_t i = 0; i < call->Arguments().Length(); i++) {
                ...
                if (const auto chain = current.function->calls.Find(arg)) {
        ...
        for (auto caller_variant_it : current.function->SortedVariants()) {
    ...
    const ast::CallExpression* TransformCall(const sem::Call* call) {
        ...
        for (auto* arg : call->Arguments()) {
            auto* chain = current.function->calls.Find(arg);
            if (!chain) {
        ...
        while (true) {
            ...
            Action action = Switch(
                ...
                [&](const sem::StructMemberAccess* a) {
                    ...
                    access.indices.Push(a->Member()->Name());
                    expr = a->Object();
                    return Action::kContinue;
                },
                [&](const sem::IndexAccessorExpression* a) {
                    ...
                    if (auto* val = idx->ConstantValue()) {
                        ...
                        // once, that expressions do not change between index and usage, and that
                        // variables are not shadowed between the index expression and function
                        // calls, hoist the expression to a let. As access chains can share
                        // expressions, we use the hoisted_exprs map to ensure that we only hoist
                        // the expression once.
                        bool needs_hoisting = true;
                        // There's one exception to this rule: If the index expression is a `let`
                        // and that `let` is not shadowed between use as the index and `stmt`, then
                        // we can omit the hoisting.
                        if (auto* idx_variable_user =
                                idx->UnwrapMaterialize()->As<sem::VariableUser>()) {
                            auto* idx_variable = idx_variable_user->Variable();
                            if (idx_variable->Declaration()->Is<ast::Let>()) {
                                needs_hoisting = IsShadowed(idx_variable, stmt);
                            }
                        }
                        ...
                        if (idx->UnwrapMaterialize()
                            ...
                            access.dynamic_indices.Push(idx_expr);
                        } else {
                            access.dynamic_indices.Push(
                                [this, idx_expr] { return b.Construct(b.ty.u32(), idx_expr()); });
                        }
                    }
                    expr = a->Object();
                    return Action::kContinue;
    ...
    const ast::Expression* BuildAccessExpr(
        ...
        if (auto* const* variable = std::get_if<const sem::Variable*>(&access)) {
            ...
            if (auto let = current.function->unshadowed_vars.Find(*variable)) {
