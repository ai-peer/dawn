...
/// AccessRoot describes the root of an AccessChain.
struct AccessRoot {
    /// The pointer-unwrapped type of the *transformed* originating variable.
    /// Pointers in 'private' and 'function' address space are passed to the callee via a pointer to
    /// *base object*. For these pointers, #type is of the base object type, not the pointer
    /// type in the source program.
    tint::sem::Type const* type = nullptr;
    /// The originating variable in the source program.
    tint::sem::Variable const* variable = nullptr;
    /// The access space of the variable or pointer type.
    tint::ast::StorageClass address_space = tint::ast::StorageClass::kInvalid;
};
...
/// DynamicIndex is used by DirectVariableAccess::State::AccessOp to indicate a
...
struct DirectVariableAccess::State {
    ...
    void Run() {
        ...
        // expressions are evaluated once at the correct place
        {
            ...
            for (auto* expr : chain_exprs) {
                ...
                // Chain is requires transforming.
                ...
                // Ensure the chain's root variable is accessible from this statement
                Unshadow(chain->root.variable, expr->Stmt());
                ...
                for (auto variant_it : fn_info->variants) {
                    ...
                    for (auto param_it : variant_it.key) {
                        ...
                        auto& symbols = *variant.ptr_param_symbols.Find(param_it.key);
                        if (symbols.parameters.base_ptr.IsValid() !=
                            symbols.parameters.indices.IsValid()) {
                            // Only one of param_symbols.base_ptr and param_symbols.indices is
                            // valid. In this situation, the valid symbol refers to the unaltered
                            // parameter name, so we need to ensure that this parameter is not
                            // shadowed. If both symbols were valid, then they'd both have
                            // program-unique names.
                            auto unshadowed = Unshadow(param, expr->Stmt());
                            if (symbols.parameters.base_ptr.IsValid()) {
                                symbols.unshadowed.base_ptr = unshadowed;
                            } else {
                                symbols.unshadowed.indices = unshadowed;
                            }
                        }

                        Unshadow(param_it.value.root.variable, expr->Stmt());
                    }
                }
            }
        }
  ...
  private:
    struct PtrParamSymbols {
        ...
        // The symbols used to construct the function parameters. These may be shadowed within the
        // body of the function. Do not use to construct identifier expressions.
        Symbols parameters;
        // The symbols that are guaranteed to resolve to the parameters at all places AccessChains
        ...
        Switch(
            ...
            [&](const sem::VariableUser* user) {
                ...
                auto create_new_chain = [&] {
                    ...
                    chain->root.address_space = variable->StorageClass();
                    if (auto* ptr = chain->root.type->As<sem::Pointer>()) {
                        chain->root.address_space = ptr->StorageClass();
                    }
                    access_chains.Add(expr, chain);
                };
        ...
        for (auto caller_variant_it : caller->SortedVariants()) {
            ...
            auto& target_variant = target_info->variants.GetOrCreate(target_signature, [&] {
                ...
                for (auto param_it : target_signature) {
                    ...
                    // If the access chain has dynamic indices, then we create a an array<u32, N>
                    ...
                    PtrParamSymbols::Symbols symbols;
                    ...
                    variant.ptr_param_symbols.Add(param, PtrParamSymbols{symbols, symbols});
                }
    ...
    bool AddressSpaceRequiresTransform(ast::StorageClass address_space) const {
                ...
                for (auto* param : fn->Parameters()) {
                    if (auto* incoming_shape = variant_sig.Find(param)) {
                        ...
                        if (symbols.parameters.base_ptr.IsValid()) {
                            ...
                            params.Push(b.Param(symbols.parameters.base_ptr, base_ptr_ty));
                        }
                        if (symbols.parameters.indices.IsValid()) {
                            // Variant has dynamic indices for this variant, replace it.
                            auto* dyn_idx_arr_type = DynamicIndexArrayType(*incoming_shape);
                            params.Push(b.Param(symbols.parameters.indices, dyn_idx_arr_type));
                        }
                    } else {
    ...
    void TransformCall(const sem::Call* call) {
        ...
        ctx.Replace(call->Declaration(), [this, call]() {
            ...
            for (size_t arg_idx = 0; arg_idx < call->Arguments().Length(); arg_idx++) {
                ...
                // Build the absolute AccessShape for this pointer argument using the current
                // variant of the caller.
                auto full_indices = AbsoluteAccessShape(*clone_state->current_variant_sig, *chain);

                auto* chain_root = chain->root.variable;
                ...
                if (IsPrivateOrFunction(param_ty->StorageClass())) {
                ...
                if (auto* dyn_idx_arr_ty = DynamicIndexArrayType(full_indices)) {
                    ...
                    if (auto* root_param = chain_root->As<sem::Parameter>()) {
    ...
    void TransformAccessChains() {
        ...
        ctx.ReplaceAll([this](const ast::Expression* ast_expr) -> const ast::Expression* {
            ...
            auto indices = clone_state->current_variant->ptr_param_symbols.Find(root_param)
                               ->unshadowed.indices;
    ...
    Symbol Unshadow(const sem::Variable* variable, const sem::Statement* stmt) {
        ...
        if (IsShadowed(variable, stmt)) {
            ...
            return fn_info->unshadowed_vars.GetOrCreate(variable, [&] {
    ...
    std::string AccessShapeName(const AccessShape& shape) {
        ...
        for (auto& op : shape.ops) {
            ...
            if (auto* dyn_idx = std::get_if<DynamicIndex>(&op)) {
    ...
    const ast::Expression* BuildAccessRootExpr(const AccessRoot& root, bool deref) {
        if (auto* param = root.variable->As<sem::Parameter>()) {
            if (auto symbols = clone_state->current_variant->ptr_param_symbols.Find(param)) {
                if (deref) {
                    return b.Deref(b.Expr(symbols->unshadowed.base_ptr));
                }
                return b.Expr(symbols->unshadowed.base_ptr);
            }
        }

        if (auto let = clone_state->current_function->unshadowed_vars.Find(root.variable)) {
        ...
        if (deref) {
            if (auto* ptr = root.variable->Type()->As<sem::Pointer>()) {
                expr = b.Deref(expr);
            }
        }
        return expr;
    ...
    static bool IsPrivateOrFunction(const ast::StorageClass sc) {
        return sc == ast::StorageClass::kPrivate || sc == ast::StorageClass::kFunction;
    }

#if 0
    void DebugPrint(const AccessShape& indices) const {
        bool first = true;
