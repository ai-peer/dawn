...
struct AccessRoot {
    ...
    /// The originating module-scope variable ('storage', 'uniform', 'workgroup'), function-scope
    /// variable ('function'), or pointer parameter in the source program.
...
struct DirectVariableAccess::State {
  ...
  private:
    /// PtrParamSymbols holds the symbols for the transformed pointer parameters, including symbols
    /// used to 'unshadow' the parameters.
    ...
    /// Signals to the ast::Expression clone callback that the given expression should not be
    /// replaced with the hoisted 'let' name. This is used to build the 'let' initializer.
        ...
        for (auto caller_variant_it : caller->SortedVariants()) {
            ...
            auto& target_variant = target_info->variants.GetOrCreate(target_signature, [&] {
                ...
                for (auto param_it : target_signature) {
                    ...
                    ptr_param_symbols.Add(param, PtrParamSymbols{symbols, symbols});
                }
    ...
    /// * Creates new 'let' declarations to unshadow access chain originating variables to
    ///   module-scope variables and parameters.
