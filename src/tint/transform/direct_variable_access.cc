  ...
  private:
                    ...
                    for (auto& access : chain.indices) {
                        if (auto* var = std::get_if<const sem::Variable*>(&access)) {
                            if (auto* param = (*var)->As<sem::Parameter>()) {
                                ...
                                auto param_name = ctx.Clone(param->Declaration()->symbol);
                                for (auto param_access : *current.variant_params->Find(param)) {
                        ...
                        BuildAccessExpr(expr_type, access,
    ...
    ast::TraverseAction ProcessCall(const sem::Call* call) {
        ...
        auto build_target_variant = [&](const FnVariantPtrParams& caller_variant_params,
            ...
            FnVariantPtrParams target_variant_key;
            ...
            auto& target_variant = target_info->variants.GetOrCreate(target_variant_key, [&] {
                ...
                for (auto& arg : call->Arguments()) {
                    if (const auto chain = current.function->call_access_chains.Find(arg)) {
                        name += "_" + AccessIndicesName(chain->indices);
                    }
                }
