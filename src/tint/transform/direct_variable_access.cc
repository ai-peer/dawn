...
struct DirectVariableAccess::State {
    ...
    void Run() {
        ...
        // Whenever we find a expression deriving from either a global or parameter or a `uniform`,
        // `storage` or `workgroup` address space, then start constructing an access chain. These
        // chains are passed up the expression tree until they are used. After this stage, we are
        // left with all the root expression access chains to the pointer variables.
    ...
    void AppendAccessChain(const sem::Expression* expr) {
        ...
        Switch(
            ...
            [&](const sem::Expression* e) {
                // Pass through indirection and address-of unary ops.
