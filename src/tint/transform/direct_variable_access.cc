  ...
  private:
    ...
    void MaybeHoistDynamicIndices(AccessChain* chain, const sem::Statement* usage) {
        for (auto& idx : chain->dynamic_indices) {
            // When dealing with pointer-lets, access chains may span multiple statements.
            // In order to ensure that:
            // * side-effecting expressions are only evaluated once
            // * that expressions do not change between index and usage
            // * that variables are not shadowed between the index expression and function
            // calls hoist the expression to a let. As access chains can share expressions,
            // we use the hoisted_exprs map to ensure that we only hoist the expression
            // once.
            ...
            // parameter and that variable is not shadowed between use as the index and
            // `stmt`, then we can omit the hoisting.
    ...
    const ast::Expression* BuildDynamicIndex(const sem::Expression* idx) {
        ...
        // The index may be fed to a dynamic index array<u32, N> argument, so the index expression
        // may need casting to u32.
        if (!idx->UnwrapMaterialize()->Type()->UnwrapRef()->IsAnyOf<sem::U32, sem::AbstractInt>()) {
        ...
        ctx.ReplaceAll([this](const ast::Expression* ast_expr) -> const ast::Expression* {
            ...
            for (size_t access_idx = start_idx; access_idx < chain->indices.Length();
                ...
                chain_expr = BuildAccessExpr(chain_expr, access, [&](size_t i) {
                    return BuildDynamicIndex(chain->dynamic_indices[i]);
                });
            }
            ...
            for (auto* arg : call->Arguments()) {
                ...
                if (auto* arg_ty = DynamicIndexAlias(full_indices)) {
                    ...
                    for (auto& dyn_idx : chain->dynamic_indices) {
                        dyn_idx_args.Push(BuildDynamicIndex(dyn_idx));
                    }
