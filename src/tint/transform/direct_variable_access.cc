        ...
        ast::TraverseExpressions(root_expr, b.Diagnostics(), [&](const ast::Expression* ast_expr) {
            ...
            if (NeedsTransforming(expr)) {
                ...
                ctx.Replace(ast_expr, [this, chain] {
                    ExprType expr_type = {};
                    for (auto& access : chain.indices) {
                        if (auto* var = std::get_if<const sem::Variable*>(&access)) {
                            if (auto* param = (*var)->As<sem::Parameter>()) {
                                // Reconstruct the expression using the variant's parameter chain
                                auto param_name = ctx.Clone(param->Declaration()->symbol);
                                for (auto param_access : *current_variant_key->Find(param)) {
                                    BuildAccessExpr(expr_type, param_access, [&](size_t i) {
                                        return b.IndexAccessor(param_name, u32(i));
                                    });
                                }
                                continue;
                            }
                        }
                        BuildAccessExpr(expr_type, access, [&](size_t i) {
                            return ctx.Clone(chain.dynamic_indices[i]->Declaration());
                        });
                    }
    ...
    ast::TraverseAction ProcessCall(const sem::Call* call) {
        ...
        for (auto arg : call->Arguments()) {
            if (NeedsTransforming(arg)) {
                current_function->call_access_chains.Add(arg, AccessChainFor(arg));
            } else {
                ProcessExpression(arg->Declaration());
            }
        }

        if (current_function->call_access_chains.IsEmpty()) {
        ...
        auto build_target_variant = [&](const FnVariantKey& caller_variant_key,
            ...
            for (size_t i = 0; i < call->Arguments().Length(); i++) {
                ...
                const auto& chain = *current_function->call_access_chains.Find(arg);
        ...
        for (auto caller_variant_it : current_function->variants) {
        ...
        for (auto* arg : call->Arguments()) {
            ...
            if (auto* arg_ty = DynamicIndexAlias(full_indices)) {
                ...
                for (auto* dyn_idx : chain->dynamic_indices) {
                    auto* dyn_idx_arg = ctx.Clone(dyn_idx->Declaration());
                    if (!dyn_idx->UnwrapMaterialize()
                             ->Type()
                             ->UnwrapRef()
                             ->IsAnyOf<sem::U32, sem::AbstractInt>()) {
                        dyn_idx_arg = b.Construct<u32>(dyn_idx_arg);
                    }
                    dyn_idx_args.Push(dyn_idx_arg);
                }
