...
/// FatPtr is used by DirectVariableAccess::State::AccessIndex for pointer parameter types in the
/// 'private' and 'function' scope.
struct FatPtr {
...
/// Inequality operator for FatPtr
bool operator!=(const FatPtr& a, const FatPtr& b) {
...
/// Hasher specialization for FatPtr
template <>
struct Hasher<FatPtr> {
    /// The hash function for the FatPtr
    /// @param d the FatPtr to hash
    /// @return the hash for the given FatPtr
    size_t operator()(const FatPtr& d) const { return utils::Hash(d.root_type, d.param); }
};
...
struct DirectVariableAccess::State {
  ...
  private:
    ...
    /// FatPtr          - the root ptr<function, T> parameter.
    /// Symbol               - a struct member access.
    /// DynamicIndex         - a runtime index on an array, matrix column, or vector element.
    using AccessIndex = std::variant<const sem::Variable*, FatPtr, Symbol, DynamicIndex>;
    ...
    void ProcessFunction(const sem::Function* fn, FnInfo* fn_info) {
        if (fn_info->variants.IsEmpty()) {
            ...
            if (HasTransformedPointerParameter(fn)) {
    ...
    void ProcessCall(FnInfo* caller, const sem::Call* call) {
        ...
        for (auto caller_variant_it : caller->SortedVariants()) {
            ...
            for (size_t i = 0; i < call->Arguments().Length(); i++) {
                ...
                if (ArgumentRequiresTransform(caller_signature, arg_chain)) {
                    ...
                    // If the parameter is a pointer in the 'private' or 'function' address space,
                    // then change the root to be a FatPtr
                    if (param_ty->StorageClass() == ast::StorageClass::kPrivate ||
                        param_ty->StorageClass() == ast::StorageClass::kFunction) {
                        auto* root_obj_type = RootType(arg_chain->indices);
                        indices[0] = FatPtr{root_obj_type, param};
                    }
    ...
    bool ArgumentRequiresTransform(const FnVariant::Signature& caller_signature,
                                   const AccessChain* arg_chain) const {
        if (auto* fn_ptr = RootFatPtr(arg_chain->indices)) {
        ...
        if (auto* var = RootVariable(arg_chain->indices)) {
            return Switch(
                ...
                [&](const sem::Parameter* param) {
                    ...
                    // must be a variant.
                    if (caller_signature.Contains(param)) {
                        return true;
                    }

                    if (auto* ptr = param->Type()->As<sem::Pointer>()) {
                        if (opts.transform_private &&
                            ptr->StorageClass() == ast::StorageClass::kPrivate) {
    ...
    AccessIndices AbsoluteAccessIndices(const FnVariant::Signature& signature,
        ...
        if (auto* root_param = RootParameter(indices)) {
            if (auto* incoming_chain = signature.Find(root_param)) {
                ...
                // of dynamic indices. Concatenate the signature's AccessIndices for the parameter
            ...
            for (auto variant_it : fn_info->SortedVariants()) {
                ...
                for (auto* param : fn->Parameters()) {
                    if (auto* ptr_access = variant_sig.Find(param)) {
                        ...
                        if (param_ty->StorageClass() == ast::StorageClass::kPrivate ||
                            param_ty->StorageClass() == ast::StorageClass::kFunction) {
                            // Pointers in the 'private' or 'function' address space requires
                            // passing the base object pointer.
                ...
                if (auto* dyn_idx_arr_ty = DynamicIndexArrayType(full_indices)) {
                    ...
                    if (auto* chain_root_param = chain_root->As<sem::Parameter>()) {
                        ...
                        if (auto* incoming_chain =
                            ...
                            auto N = CountDynamicIndices(*incoming_chain);
    ...
    const ast::Expression* BuildAccessExpr(
        ...
        if (auto* ptr_param = std::get_if<FatPtr>(&access)) {
            ...
            TINT_ICE(Transform, b.Diagnostics()) << "FatPtr not found in variant's fn_ptr_params";
            return nullptr;
        }
    ...
    /// @returns the originating FatPtr of the AccessIndices @p indices.
    const FatPtr* RootFatPtr(const AccessIndices& indices) const {
        auto& root = indices.Front();
        if (auto* fn_ptr = std::get_if<FatPtr>(&root)) {
            return fn_ptr;
        }
        return nullptr;
    }
    ...
    const sem::Variable* RootVariable(const AccessIndices& indices) const {
        ...
        if (auto* fn_ptr_param = RootFatPtr(indices)) {
    ...
    const sem::Type* RootType(const AccessIndices& indices) const {
        if (auto* fn_ptr = RootFatPtr(indices)) {
    ...
    bool HasTransformedPointerParameter(const sem::Function* fn) const {
        for (auto* param : fn->Parameters()) {
            if (auto* ptr = param->Type()->As<sem::Pointer>()) {
