...
uint32_t BuiltinOrder(type::BuiltinValue builtin) {
    switch (builtin) {
...
bool HasSampleMask(utils::VectorRef<const ast::Attribute*> attrs) {
    ...
    return builtin && builtin->builtin == type::BuiltinValue::kSampleMask;
}
...
struct CanonicalizeEntryPointIO::State {
    ...
    const ast::Expression* AddInput(std::string name,
        ...
        if (cfg.shader_style == ShaderStyle::kSpirv || cfg.shader_style == ShaderStyle::kGlsl) {
            ...
            if (builtin) {
                ...
                } else if (builtin->builtin == type::BuiltinValue::kSampleMask) {
    ...
    void AddFixedSampleMask() {
        ...
        AddOutput("fixed_sample_mask", ctx.dst->create<type::U32>(), std::nullopt,
                  {ctx.dst->Builtin(type::BuiltinValue::kSampleMask)},
    ...
    void AddVertexPointSize() {
        ...
        AddOutput("vertex_point_size", ctx.dst->create<type::F32>(), std::nullopt,
                  {ctx.dst->Builtin(type::BuiltinValue::kPointSize)}, ctx.dst->Expr(1_f));
    }
    ...
    const char* GLSLBuiltinToString(type::BuiltinValue builtin,
    ...
    /// WGSL expects
    const ast::Expression* FromGLSLBuiltin(type::BuiltinValue builtin,
                                           const ast::Expression* value,
                                           ast::Type& ast_type) {
        switch (builtin) {
            case type::BuiltinValue::kVertexIndex:
    ...
    const ast::Expression* ToGLSLBuiltin(type::BuiltinValue builtin,
                                         const ast::Expression* value,
                                         const type::Type*& type) {
        switch (builtin) {
            case type::BuiltinValue::kVertexIndex:
