...
class HoistToDeclBefore::State {
    ...
    void TransformWhileLoops() const {
  ...
  public:
    ...
    bool Add(const sem::Expression* before_expr,
        ...
        if (as_let) {
            auto builder = [this, expr, name] { return b.Decl(b.Let(name, ctx.Clone(expr))); };
            if (!InsertBefore(before_expr->Stmt(), &builder)) {
                return false;
            }
        } else {
            auto builder = [this, expr, name] { return b.Decl(b.Var(name, ctx.Clone(expr))); };
            if (!InsertBefore(before_expr->Stmt(), &builder)) {
                return false;
            }
        }
    ...
    bool InsertBefore(const sem::Statement* before_stmt, BUILDER* builder) {
        ...
        if (auto* block = parent->As<sem::BlockStatement>()) {
            ...
            if (builder) {
                ctx.InsertBefore(block->Declaration()->statements, ip, *builder);
            }
            return true;
        }
    ...
    bool InsertBefore(const sem::Statement* before_stmt, const ast::Statement* stmt) {
        ...
        return InsertBefore(before_stmt, &builder);
    }
