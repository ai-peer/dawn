...
TEST_F(Texture1DTo2DTest, Global1DDeclAndSampleCall) {
    auto* src = R"(
@group(0) @binding(0) var t : texture_1d<f32>;
...
fn main() -> vec4<f32> {
  return textureSample(t, s, vec2<f32>(1.0f, 0.0f));
}
)";
...
TEST_F(Texture1DTo2DTest, Global1DDeclAndSample2DCall) {
    auto* src = R"(
@group(0) @binding(0) var t : texture_2d<f32>;
...
fn main() -> vec4<f32> {
  return textureSample(t, s, vec2<f32>(1.0f, 2.0f));
}
)";
    auto* expect = R"(
@group(0) @binding(0) var t : texture_2d<f32>;
