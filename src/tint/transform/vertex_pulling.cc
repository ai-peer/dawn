...
/// Writes the BaseWGSLType to the std::ostream.
/// @param out the std::ostream to write to
/// @param format the BaseWGSLType to write
/// @returns out so calls can be chained
std::ostream& operator<<(std::ostream& out, BaseWGSLType format) {
    switch (format) {
        case BaseWGSLType::kInvalid:
            return out << "invalid";
        ...
        for (uint32_t buffer_idx = 0; buffer_idx < cfg.vertex_state.size(); ++buffer_idx) {
            ...
            for (const VertexAttributeDescriptor& attribute_desc : buffer_layout.attributes) {
                ...
                if (var_dt.width < fmt_dt.width) {
                    ...
                    const ast::Type* elem_ty = nullptr;
                    switch (var_dt.base_type) {
                        ...
                        case BaseWGSLType::kF16:
                            elem_ty = b.ty.f16();
                            for (uint32_t i = fmt_dt.width; i < var_dt.width; i++) {
                                values.Push(b.Expr((i == 3) ? 1_h : 0_h));
                            }
                            break;
                        default:
                    ...
                    value = b.Construct(b.ty.vec(elem_ty, var_dt.width), values);
                }
