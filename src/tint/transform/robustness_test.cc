...
fn f() {
  ...
  for(var i = predicated_expr; (i < 3); i++) {
    var in_loop = 42;
  }
}
)");
...
fn f() {
  ...
  var level = 1;
  let level_1 = min(u32(level), (textureNumLevels(t) - 1));
  for(var i = textureLoad(t, clamp(coords, vec2(0), vec2<i32>((textureDimensions(t, level_1) - vec2(1)))), level_1).x; (i < 3); i++) {
    var in_loop = 42;
  }
}
)",
...
fn f() {
  ...
  for(var i = texture_load.x; (i < 3); i++) {
    var in_loop = 42;
  }
}
)");
...
fn f() {
  ...
  var i = 0;
  let coords_1 = coords;
  for(if (all((u32(coords_1) < textureDimensions(t)))) {
    textureStore(t, coords_1, value);
  }; (i < 3); i++) {
    var in_loop = 42;
  }
