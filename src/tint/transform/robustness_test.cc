...
fn f() {
  ...
  let level_clamped = min(level_1, (textureNumLevels(t) - 1));
  let coords_1 = coords;
  var texture_load : vec4<i32>;
  if ((all((vec2<u32>(coords_1) < textureDimensions(t, level_clamped))) & (level_1 < textureNumLevels(t)))) {
    texture_load = textureLoad(t, coords_1, level_1);
  }
  {
    var i = texture_load.x;
    loop {
      if (!((i < 3))) {
        break;
      }
    ...
    loop {
      ...
      continuing {
        ...
        if ((all((vec2<u32>(coords) < textureDimensions(t, level_clamped))) & (level_1 < textureNumLevels(t)))) {
          texture_load = textureLoad(t, coords, level_1);
        }
        i = texture_load.x;
      }
    }
...
fn f() {
  for(var i = 0; (i < atomicXor(&(a[min(u32(i), 3u)]), 4)); i++) {
    var in_loop = 42;
                          ...
                          /* predicate */ R"(error: unresolved identifier 'predicate')");
