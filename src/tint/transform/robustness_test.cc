...
TEST_P(RobustnessTest, TextureRead_ForLoopInit) {
    auto* src = R"(
...
TEST_P(RobustnessTest, TextureRead_ForLoopCond) {
    auto* src = R"(
@group(0) @binding(0) var t : texture_2d<i32>;
...
TEST_P(RobustnessTest, TextureRead_ForLoopCont) {
    auto* src = R"(
@group(0) @binding(0) var t : texture_2d<i32>;
  ...
  var level = 1;
  {
    ...
    loop {
      ...
      continuing {
        ...
        let coords = vec2(i);
        var predicated_value : vec4<i32>;
        if ((all((vec2<u32>(coords) < textureDimensions(t, level_clamped))) & (level_1 < textureNumLevels(t)))) {
...
fn f() {
  ...
  var value = vec4(1);
  {
    ...
    loop {
      ...
      continuing {
        let coords = i;
        if (all((u32(coords) < textureDimensions(t)))) {
          textureStore(t, coords, value);
        }
      }
    }
