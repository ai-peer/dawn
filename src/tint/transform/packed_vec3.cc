...
struct PackedVec3::State {
    ...
    void Run() {
                    ...
                    for (auto* member : str->Members()) {
                        if (auto* vec = member->Type()->As<sem::Vector>()) {
                            if (vec->Width() == 3) {
                                ...
                                auto* ast_decl = member->Declaration();
                                auto name = ctx.Clone(ast_decl->symbol);
                                auto* type = ctx.Clone(ast_decl->type);
                                auto* deco =
                                    b.ASTNodes().Create<Attribute>(b.ID(), b.AllocateNodeID());
                                ctx.Replace(member->Declaration(),
                                            b.Member(name, type, utils::Vector{deco}));
                            }
                        }
                    }
                }
        ...
        for (auto* read : refs) {
            if (auto* vec_ty = read->Type()->UnwrapRef()->As<sem::Vector>()) {
                auto* expr = read->Declaration();
