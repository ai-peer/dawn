...
struct PackedVec3::State {
    ...
    /// WrapperStructKey is the hashmap key to the name of a packed vec3 wrapper struct.
    ...
    utils::Hashmap<WrapperStructKey, Symbol, 4, WrapperStructKey::Hasher>
        packed_vec3_wrapper_struct_names;
    ...
    /// Recursively rewrite an type using `__packed_vec3`, if needed.
    /// If generated, the `__packed_vec3` type will be wrapped in a structure. When used as an array
    /// element type, the wrapper struct member is given an @align() attribute to give it alignment
    /// it needs to yield the correct array element stride. For vec3 types used in structures, the
    /// @align() attribute is placed on the containing structure instead. Matrices with three rows
    /// become arrays of columns, and used the aligned wrapper struct for the column type.
        ...
        return Switch(
            ...
            [&](const type::Vector* vec) -> ast::Type {
                if (IsVec3(vec)) {
                    ...
                    return b.ty(packed_vec3_wrapper_struct_names.GetOrCreate(key, [&]() {
                        ...
                        if (array_element) {
                            // Give the struct member the same alignment as the original unpacked
                            // vec3 type when used as an array element, to avoid changing the array
                            // element stride.
            ...
            return MakePackUnpackHelper(
                ...
                [&](auto* element, const type::Type* element_type,
                    ...
                    if (element_type->Is<type::Vector>()) {
                        ...
                        return b.Call(RewriteType(element_type, array_element),
        ...
        for (auto* node : ctx.src->ASTNodes().Objects()) {
            Switch(
                ...
                [&](const sem::TypeExpression* type) {
                    ...
                    if (auto* ptr = type->Type()->As<type::Pointer>();
                        ...
                        if (IsVec3(store_type)) {
                            ...
                            new_store_type = RewriteType(store_type);
                        }
                        if (new_store_type) {
                ...
                [&](const sem::Statement* stmt) {
                    ...
                    if (auto* assign = stmt->Declaration()->As<ast::AssignmentStatement>()) {
                        ...
                        if (to_unpack.Contains(rhs)) {
                            ...
                            if (IsVec3(rhs->Type())) {
                                // If the expression produces a vec3, extract the packed vector from
                                // the wrapper struct.
                ...
                [&](const sem::ValueExpression* expr) {
                    ...
                    // space, extract the packed vector from the wrapper struct.
                    if (expr->IsAnyOf<sem::IndexAccessorExpression, sem::StructMemberAccess,
                                      sem::VariableUser>()) {
                        if (auto* ref = expr->Type()->As<type::Reference>()) {
                            if (IsVec3(ref->StoreType()) &&
                                builtin::IsHostShareable(ref->AddressSpace())) {
                                ctx.Replace(node, b.MemberAccessor(ctx.Clone(expr->Declaration()),
                                                                   kStructMemberName));
                            }
                        }
                    }
            ...
            if (IsVec3(expr->Type())) {
                if (expr->UnwrapLoad()
                        ->IsAnyOf<sem::IndexAccessorExpression, sem::StructMemberAccess,
                                  sem::VariableUser>()) {
                    // If we are unpacking a vec3, extract the vector from the wrapper struct.
