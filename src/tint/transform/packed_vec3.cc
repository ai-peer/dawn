...
struct PackedVec3::State {
    ...
    /// element type, the wrapper struct member is given an `@align()` attribute to give it
    /// alignment it needs to yield the correct array element stride. For vec3 types used in
    /// structures, the `@align()` attribute is placed on the containing structure instead. Matrices
    /// with three rows become arrays of columns, and used the aligned wrapper struct for the column
    /// type.
