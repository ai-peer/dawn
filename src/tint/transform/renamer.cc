...
Transform::ApplyResult Renamer::Apply(const Program* src,
    ...
    auto is_core_type = [&](const Symbol& symbol) {
        auto name = src->Symbols().NameFor(symbol);
        if (type::ParseCore(name) != type::Core::kUndefined) {
            // Identifier *looks* like a core type, but check that the core type isn't being
    ...
    for (auto* node : src->ASTNodes().Objects()) {
        Switch(
            ...
            [&](const ast::CallExpression* call) {
                Switch(
                    ...
                    [&](const sem::TypeConversion*) {
                        if (is_core_type(call->target->symbol)) {
            ...
            [&](const ast::Type* type) {
                if (is_core_type(type->name->symbol)) {
