...
Transform::ApplyResult TruncateInterstageVariables::Apply(const Program* src,
    ...
    for (auto* func_ast : ctx.src->AST().Functions()) {
        ...
        // utils::Hashset<const sem::StructMember*, 16u> copy_members;
        ...
        for (auto* member : struct_ty->members) {
            ...
            // } else if (ast::GetAttribute<ast::BuiltinAttribute>(member->attributes)) {
            //     auto* m = sem.Get(member);
            //     copy_members.Add(m);
            // }
        }
        ...
        auto entry =
            old_shader_io_structs_to_new_struct_and_truncate_functions.GetOrCreate(str, [&] {
                ...
                for (auto* member : struct_ty->members) {
                    ...
                    truncated_members.Push(ctx.Clone(member));
                    // const ast::Expression* arg = b.MemberAccessor("io",
                    // ctx.Clone(member->Declaration()->symbol)); initializer_exprs.Push(arg);
                    initializer_exprs.Push(b.MemberAccessor("io", ctx.Clone(member->symbol)));
                }
                ...
                //         builtin_output_members.Push(b.Member(member_name,
                //         ctx.Clone(member->type),
                ...
                //         // For user defined shader io, only preserve those with locations marked
                //         in
                //         // the input.
                //         auto* m = sem.Get(member);
                //         uint32_t location = m->Location().value();
