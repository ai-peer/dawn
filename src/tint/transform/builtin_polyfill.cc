...
struct BuiltinPolyfill::State {
    ...
    Symbol insertBits(const sem::Type* ty) {
        ...
        // `(select(Tx(), x, y < 32u) << y)`, in which `Tx` is the type of x, where y can be 32u.
        // WGSL polyfill function:
        //      fn tint_insert_bits(v : T, n : T, offset : u32, count : u32) -> T {
        //          let s = min(offset, 32u);
        //          let e = min(32u, (s + count));
        //          let mask = (
        //                        ((select(0u, 1u, s < 32u) << s) - 1u) ^
        //                        ((select(0u, 1u, e < 32u) << e) - 1u)
        //                     );
        //          return (((select(T(), n, s < 32u) << s) & mask) | (v & ~(mask)));
        //      }
        ...
        switch (polyfill.insert_bits) {
            case Level::kFull:
                ...
                body.Push(b.Decl(b.Let(
                    "mask",
                    b.Xor(  //
                        b.Sub(b.Shl(b.Call("select", 0_u, 1_u, b.LessThan("s", 32_u)), "s"), 1_u),
                        b.Sub(b.Shl(b.Call("select", 0_u, 1_u, b.LessThan("e", 32_u)), "e"),
                              1_u)  //
                        ))));

                // return (((select(T(), n, s < 32u) << s) & mask) | (v & ~(mask)));
                body.Push(b.Return(b.Or(
                    b.And(b.Shl(b.Call("select", b.Construct(T(ty)), "n", b.LessThan("s", 32_u)),
                                U("s")),
                          V("mask")),
