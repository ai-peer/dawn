...
void Mat2ToVec2::Run(CloneContext& ctx, const DataMap&, DataMap&) const {
    ...
    ctx.ReplaceAll([&](const ast::AssignmentStatement* stmt) -> const ast::Statement* {
        ...
        auto* rhs_access = sem.Get<sem::StructMemberAccess>(stmt->rhs);
        ...
        for (auto* member : it->second) {
            auto* lhs = b.MemberAccessor(
                ctx.CloneWithoutTransform(
                    static_cast<const ast::MemberAccessorExpression*>(lhs_access->Declaration())
                        ->structure),
                member->symbol);
            auto* rhs = b.MemberAccessor(
                ctx.CloneWithoutTransform(
                    static_cast<const ast::MemberAccessorExpression*>(rhs_access->Declaration())
                        ->structure),
                member->symbol);
