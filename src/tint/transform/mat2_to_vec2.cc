...
bool ShouldRun() {
    for (auto* node : program->ASTNodes().Objects()) {
        if (auto* ast_str = node->As<ast::Struct>(node)) {
            if (NeedsDecomposition(sem.Get(ast_str))) {
                return true;
            }
        }
    }
...
void Mat2ToVec2::Run(CloneContext& ctx, const DataMap&, DataMap&) const {
    ...
    ctx.ReplaceAll([&](const ast::Struct* ast_str) -> const ast::Struct* {
        ...
        for (auto* member : str->Members()) {
            auto* mat = member->Type()->As<sem::Matrix>();
            if (!mat || mat->rows() != 2) {
                members.push_back(ctx.Clone(member->Declaration()));
                continue;
            }
            ...
            for (uint32_t i = 0u; i < mat->columns(); ++i) {
                ...
                if (i == 0) {
                    attributes.push_back(d->MemberOffset(member->Offset()));
                }
                if (i == mat->columns() - 1 && padding > 0) {
                    attributes.push_back(d->MemberSize(padding + vecTySem->Size()));
                }
