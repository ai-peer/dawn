...
void VectorizeScalarMatrixConstructors::Run(CloneContext& ctx, const DataMap&, DataMap&) const {
    ...
    ctx.ReplaceAll([&](const ast::CallExpression* expr) -> const ast::CallExpression* {
        ...
        if (args.size() == 1) {
            ...
            auto isConstantExpr = [&](const ast::Expression* expr) -> bool {
                bool is_const = true;
                diag::List diagnostics;
                ast::TraverseExpressions(expr, diagnostics, [&](const ast::Expression* e) {
            ...
            if (isConstantExpr(args[0]->Declaration())) {
                ...
                // function call.
                return build_mat(
                    [&](uint32_t c, uint32_t r) { return ctx.Clone(args[0]->Declaration()); });
            }
