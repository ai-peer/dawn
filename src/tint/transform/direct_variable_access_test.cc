...
fn b() {
  a(&(P));
    ...
    // Transform must only adjust 'private' pointers *into* an object.
    auto* expect = src;
...
fn b() {
  ...
  a(&(F));
    ...
    // Transform must only adjust 'function' pointers *into* an object.
    auto* expect = src;
...
fn a_F_X(p_base_1 : ptr<function, array<i32, 4u>>, p : F_X) -> i32 {
  return (*(p_base_1))[p[0]];
}
...
TEST_F(DirectVariableAccessComplexTest, CallChaining) {
    auto* src = R"(
struct Inner {
  mat : mat3x4<f32>,
};
