...
fn b_S_X(p : S_X) -> i32 {
  return S[p[0]][u32(a(S[p[0]][0][1][2]))][u32(a(S[p[0]][u32(a(3))][4][5]))][u32(a(S[p[0]][6][u32(a(7))][8]))];
}
...
fn b_S_X_U_X(s : S_X, u : U_X) -> i32 {
  return S[s[0]][u32(a(U[u[0]][0][1].x))][u32(a(U[u[0]][u32(a(3))][4].y))];
}
...
fn b() {
  ...
  for(let p1 = &(U[u32(tint_symbol)]); true; ) {
    a_U_X_X(U_X_X(u32(tint_symbol), u32(second())));
  }
}

fn c_U() {
  let tint_symbol_1 = first();
  for(let p1 = &(U[u32(tint_symbol_1)]); true; ) {
    a_U_X_X(U_X_X(u32(tint_symbol_1), u32(second())));
  }
}
