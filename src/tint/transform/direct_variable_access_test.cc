...
fn b() {
  for (; a(10, &U[first()][second()], 20).x < 4; ) {
    let body = 1;
  }
}

fn c(p : ptr<uniform, array<array<vec4<i32>, 8>, 8>>) {
  for (; a(10, &(*p)[first()][second()], 20).x < 4; ) {
    let body = 1;
  }
}
...
fn b() {
  for(; (a_U_X_X(10, U_X_X(u32(first()), u32(second())), 20).x < 4); ) {
    let body = 1;
  }
}
...
fn b() {
  for (var i = 0; i < 3; a(10, &U[first()][second()], 20)) {
    i++;
  }
}

fn c(p : ptr<uniform, array<array<vec4<i32>, 8>, 8>>) {
  for (var i = 0; i < 3; a(10, &(*p)[first()][second()], 20)) {
    i++;
  }
}
...
fn b() {
  for(var i = 0; (i < 3); a_U_X_X(10, U_X_X(u32(first()), u32(second())), 20)) {
    i++;
  }
}
...
fn b() {
  while (a(10, &U[first()][second()], 20).x < 4) {
    let body = 1;
  }
}

fn c(p : ptr<uniform, array<array<vec4<i32>, 8>, 8>>) {
  while (a(10, &(*p)[first()][second()], 20).x < 4) {
    let body = 1;
  }
}
...
fn b() {
  while((a_U_X_X(10, U_X_X(u32(first()), u32(second())), 20).x < 4)) {
    let body = 1;
  }
}
...
fn a(pre : i32, p : ptr<private, array<i32, 4>, read_write>, post : i32) {
  *(p) = array<i32, 4>();
}
...
fn a(pre : i32, p : ptr<function, array<i32, 4>, read_write>, post : i32) {
  *(p) = array<i32, 4>();
}
  ...
  if (true) {
    ...
    if (true) {
      ...
      if (true) {
        ...
        if (true) {
          let I = 4;
          a_U_X_X_X(10, U_X_X_X(u32(I1), u32(I2), u32(I3)), 20);
        }
      }
    }
  }
...
TEST_F(DirectVariableAccessShadowTest, Globals) {
    auto* src = R"(
fn a(pre : i32, p : ptr<storage, f32>, post : i32) -> f32 {
  let S = 123;
  return *p;
}
...
fn a_S_X_X(pre : i32, p : S_X_X, post : i32) -> f32 {
  let p_1 = p;
