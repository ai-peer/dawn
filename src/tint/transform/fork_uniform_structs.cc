...
void ForkUniformStructs::Run(CloneContext& ctx, const DataMap&, DataMap&) const {
    ...
    ctx.ReplaceAll([&](const ast::CallExpression* expr) -> const ast::CallExpression* {
        utils::Vector<const ast::Expression*, 10> new_args;
        for (auto* arg : expr->args) {
            auto* str = sem.Get(arg)->Type()->UnwrapRef()->As<sem::Struct>();
            if (!str) {
            ...
            utils::Vector<const ast::Expression*, 10> struct_args;
            for (auto* member : str->Members()) {
                auto* uniform = ctx.Clone(arg);
                struct_args.Push(b.MemberAccessor(uniform, ctx.Clone(member->Name())));
            }
            auto* struct_type = CreateASTTypeFor(ctx, str);
            new_args.Push(b.Construct(struct_type, struct_args));
        }
        return b.Call(ctx.Clone(expr->target.name), new_args);
    });
