...
void FoldConstants::Run(CloneContext& ctx, const DataMap&, DataMap&) const {
    ctx.ReplaceAll([&](const ast::Expression* expr) -> const ast::Expression* {
        ...
        auto build_scalars = [&](size_t limit) {
            return Switch(
                value.ElementType(),  //
                [&](const sem::Bool*) {
                    return utils::Transform(value.IElements(), limit, [&](AInt i) {
                        return static_cast<const ast::Expression*>(ctx.dst->Expr(bool(i.value)));
                    });
                },
                [&](const sem::I32*) {
                    return utils::Transform(value.IElements(), limit, [&](AInt i) {
                ...
                [&](const sem::F32*) {
                    return utils::Transform(value.FElements(), limit, [&](AFloat f) {
        ...
        if (auto* vec = ty->As<sem::Vector>()) {
            ...
            if (call->Arguments().size() == 1) {
                ctors = build_scalars(1);
            } else {
                ctors = build_scalars(value.ElementCount());
            }
