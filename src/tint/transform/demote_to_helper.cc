...
Transform::ApplyResult DemoteToHelper::Apply(const Program* src, const DataMap&, DataMap&) const {
    ...
    ctx.ReplaceAll([&](const ast::DiscardStatement* discard) -> const ast::Statement* {
    ...
    for (auto* func : functions_to_process) {
        if (func->Declaration()->IsEntryPoint()) {
            auto& stmts = func->Declaration()->body->statements;
            if (!stmts.Back()->Is<ast::ReturnStatement>()) {
                ctx.InsertBack(stmts, b.If(flag, b.Block(b.Discard())));
            }
        }
    }
        ...
        Switch(
            ...
            [&](const ast::CallExpression* call) {
                ...
                if (builtin->Type() == sem::BuiltinType::kTextureStore) {
                    ...
                    auto& statements = stmt->Block()->Declaration()->statements;
                    if (auto* call_stmt = stmt->Declaration()->As<ast::CallStatement>();
                        ...
                        ctx.InsertBefore(statements, stmt->Declaration(), result_decl);
                        ctx.InsertBefore(statements, stmt->Declaration(), masked_call);
