...
class Transform : public Castable<Transform> {
  ...
  protected:
    ...
    /// CreateASTTypeFor constructs new ast::Identifier nodes that reconstructs the semantic type
    /// `ty`.
    /// @param ctx the clone context
    /// @param ty the semantic type to reconstruct
    /// @returns a ast::Identifier that when resolved, will produce the semantic type `ty`.
    static const ast::Identifier* CreateASTTypeFor(CloneContext& ctx, const type::Type* ty);
};
