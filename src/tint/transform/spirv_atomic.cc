...
struct SpirvAtomic::State {
    ...
    // Symbol CreateWrapperFor(const tint::ast::Function* fn,
    //                         sem::BuiltinType builtin,
    //                         const sem::Call* call) {
    //     if (builtin == sem::BuiltinType::kAtomicCompareExchangeWeak) {
    //         // SPIR-V version of atomicCompareExchangeWeak returns the old value as i32/u32, but
    //         // WGSL version returns a struct. Create a wrapper that calls the builtin and returns
    //         // the result.old_value.

    //        // TODO: only first time..., key on name + storage type (workgroup or storage) +
    //        return
    //        // type (i32 or u32)
    //        {
    //            auto* var = call->Arguments()[0]->SourceVariable();
    //            if (!var) {
    //                // TODO: emit error message
    //                return {};
    //            }

    //            // TODO: suffix with storage type and return type
    //            auto new_fn_name = ctx.Clone(fn->symbol);

    //            ast::VariableList params;
    //            params.resize(fn->params.size());
    //            // Stub's first param is by value. Make it a pointer to atomic of the value type.
    //            params[0] = b.Param(ctx.Clone(fn->params[0]->symbol),
    //                                b.ty.pointer(b.ty.atomic(ctx.Clone(fn->params[0]->type)),
    //                                             var->StorageClass(), var->Access()));
    //            // Clone the rest of the params
    //            for (size_t i = 1; i < params.size(); ++i) {
    //                params[i] = ctx.Clone(fn->params[i]);
    //            }

    //            // Create argument expressions for the builtin call
    //            ast::ExpressionList args;
    //            args.reserve(params.size());
    //            for (auto& p : params) {
    //                args.emplace_back(b.Expr(p->symbol));
    //            }

    //            // Create the wrapper
    //            b.Func(new_fn_name, std::move(params), ctx.Clone(fn->return_type),
    //                   ast::StatementList{b.Return(b.MemberAccessor(
    //                       b.Call(sem::str(builtin), std::move(args)), b.Expr("old_value")))});
