...
fn fragment_main(@builtin(sample_index) sample_index : u32,
  ...
  return mask_in;
    ...
    auto* expect = R"(
@builtin(sample_index) @interpolate(flat) @internal(disable_validation__ignore_storage_class) var<in> gl_SampleID : i32;

@builtin(sample_mask) @interpolate(flat) @internal(disable_validation__ignore_storage_class) var<in> gl_SampleMaskIn : array<i32, 1u>;
