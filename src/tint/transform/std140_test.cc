...
TEST_F(Std140Test, ShouldRunStructMat2x2F32F32Unused) {
    auto* src = R"(
struct Unused {
  m : mat2x2<f32>,
}
...
TEST_F(Std140Test, ShouldRunStructMat2x2F32F16Unused) {
    auto* src = R"(
enable f16;
...
@group(0) @binding(0) var<uniform> s : S;
    ...
    std::string expect;
    if (matrix.NotStd140Compatible()) {
        uint32_t lastSize = 128 - matrix.ColumnVectorAlign() * (matrix.columns - 1);
...
fn f() {
  let l = conv_arr3_S(a);
        ...
        uint32_t lastSize = 64 - matrix.ColumnVectorAlign() * (matrix.columns - 1);
        expect = matrix.ReplaceFieldsInString(
            expect,
            {{"${col_vectors}", matrix.ExpendedColumnVectorsWithLastSize(2, "m_", lastSize)},
             {"${col_vectors_inline}", matrix.ExpendedColumnVectorsInline("val.m_", ", ")}});
