...
class State {
 ...
 public:
  ...
  explicit State(CloneContext& context)
      : ctx(context), hoist_to_decl_before(ctx) {}
  ...
  void Expand(const ast::Statement* stmt,
    ...
    if (lhs->Is<ast::IdentifierExpression>() ||
      ...
      new_lhs = [&, lhs_ptr]() { return ctx.dst->Deref(lhs_ptr); };
    }

    // Replace the statement with a regular assignment statement.
    auto* value = ctx.dst->create<ast::BinaryExpression>(op, new_lhs(), rhs);
    ctx.Replace(stmt, ctx.dst->Assign(new_lhs(), value));
  }
