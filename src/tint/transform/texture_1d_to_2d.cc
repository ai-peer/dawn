    ...
    ApplyResult Run() {
        ...
        ctx.ReplaceAll([&](const ast::Variable* v) -> const ast::Variable* {
            const ast::Variable* r = Switch(
                ...
                [&](const type::SampledTexture* tex) -> const ast::Variable* {
                    if (tex->dim() == ast::TextureDimension::k1d) {
                        ...
                        if (v->As<ast::Parameter>()) {
                            ...
                            return ctx.dst->Var(ctx.Clone(v->symbol), type2d,
                                                ctx.Clone(v->attributes));
                        }
                    }
                    return static_cast<const ast::Var*>(nullptr);
                },
                [&](const type::StorageTexture* storageTex) -> const ast::Variable* {
                ...
                [](Default) { return static_cast<const ast::Var*>(nullptr); });
            return r;
        });
