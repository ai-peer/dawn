...
struct Texture1DTo2D::State {
    ...
    ApplyResult Run() {
        ...
        ctx.ReplaceAll([&](const ast::CallExpression* c) -> const ast::CallExpression* {
            ...
            if (texture_index == -1 || coords_index == -1) {
                return nullptr;
            }
            ...
            TINT_ASSERT(Transform, tex != nullptr);
            if (tex->dim() != ast::TextureDimension::k1d) {
                return nullptr;
            }
            auto* coords = c->args[static_cast<size_t>(coords_index)];
            auto* zero = ctx.dst->Expr(0_f);
            utils::Vector<const ast::Expression*, 8> args;
            int index = 0;
            for (auto* arg : c->args) {
                if (index == coords_index) {
                    args.Push(ctx.dst->vec(ctx.dst->ty.f32(), 2u, ctx.Clone(coords), zero));
                } else {
