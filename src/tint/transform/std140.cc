...
struct Std140::State {
  ...
  private:
    ...
    struct LoadFn {
        ...
        struct Hasher {
            uint64_t operator()(const LoadFn& fn) const {
                return utils::Hash(fn.var, fn.access_chain);
            }
        };

        /// Equality operator
        bool operator==(const LoadFn& other) const {
            return var == other.var && access_chain == other.access_chain;
        }
    };
    ...
    struct MatrixAccess {
        ...
        // The index in chain, of the access that resolves to the decomposed matrix
        std::optional<size_t> matrix_chain_idx;

        /// @returns true if the access chain is to part of (not the whole) matrix
        bool IsMatrixSubset() const {
            return matrix_chain_idx.has_value() && (matrix_chain_idx.value() + 1 != chain.Length());
        }
    };
    ...
    template <typename F>
    static void ForeachMemberNeedsDecomposing(const Program* program, F&& cb) {
    ...
    std::optional<MatrixAccess> Access(const sem::Expression* expr) {
        MatrixAccess access;
        ...
        if (access.matrix_chain_idx.has_value()) {
            access.matrix_chain_idx = access.chain.Length() - *access.matrix_chain_idx - 1;
        }
    ...
    const ast::Expression* Read(const sem::Expression* expr) {
        if (!expr) {
            return nullptr;
        }
        auto access = Access(expr);
        if (!access) {
            return nullptr;
        }
        if (!access->matrix_chain_idx.has_value()) {
            // loading a non-matrix
            return ConvertStd140(expr->Type(), ctx.Clone(expr->Declaration()));
        }
        if (!access->IsMatrixSubset() ||  // loading a whole matrix
            std::holds_alternative<DynamicIndex>(access->chain[*access->matrix_chain_idx + 1])) {
            ...
            return ReadWithHelperFn(access.value());
        }
        // Matrix column is statically indexed. Can be emitted as an inline expression.
        return ReadInline(access.value());
    }

    const ast::Expression* ConvertStd140(const sem::Type* ty, const ast::Expression* expr) {
        ...
        auto fn = decomposed.conv_fns.GetOrCreate(ty, [&] {
            ...
            auto fn_sym = b.Symbols().New("conv_" + name);
    ...
    const ast::Expression* ReadWithHelperFn(const MatrixAccess& access) {
        auto fn = decomposed.load_fns.GetOrCreate(LoadFn{access.var, access.chain}, [&] {
    ...
    const ast::Expression* ReadInline(const MatrixAccess& access) {
        ...
        for (size_t i = 0; i < access.chain.Length(); i++) {
            if (i == access.matrix_chain_idx) {
                auto mat_member_idx = std::get<u32>(access.chain[i]);
                auto* mat_member = ty->As<sem::Struct>()->Members()[mat_member_idx];
                auto mat_columns = *decomposed.mat.Get(mat_member);
                auto column_idx = std::get<u32>(access.chain[i + 1]);
                ...
                auto [new_expr, new_ty, _] =
                    BuildAccessExpr(expr, ty, access.chain[i], dynamic_index);
                expr = new_expr;
                ty = new_ty;
            }
        }
    ...
    Symbol BuildLoadMatrixFn(const MatrixAccess& access) {
        ...
        auto matrix_chain_idx = *access.matrix_chain_idx;
        for (size_t i = 0; i < matrix_chain_idx; i++) {
        ...
        auto mat_member_idx = std::get<u32>(access.chain[matrix_chain_idx]);
        auto* mat_member = ty->As<sem::Struct>()->Members()[mat_member_idx];
        auto mat_columns = *decomposed.mat.Get(mat_member);
    ...
    Symbol BuildLoadPartialMatrixFn(
        ...
        auto matrix_chain_idx = *access.matrix_chain_idx;
        auto column_param_idx = std::get<DynamicIndex>(access.chain[matrix_chain_idx + 1]).slot;
        ...
        for (uint32_t column_idx = 0; column_idx < num_columns; column_idx++) {
            ...
            const sem::Type* ty = access.var->Type()->UnwrapRef();
            // Build the expression up to, but not including the matrix member
            for (size_t i = 0; i < access.matrix_chain_idx; i++) {
                auto [new_expr, new_ty, access_name] =
                    BuildAccessExpr(expr, ty, access.chain[i], dynamic_index);
                expr = new_expr;
                ty = new_ty;
            ...
            for (size_t i = matrix_chain_idx + 2; i < access.chain.Length(); i++) {
                auto [new_expr, new_ty, access_name] =
                    BuildAccessExpr(expr, ty, access.chain[i], dynamic_index);
                expr = new_expr;
                ty = new_ty;
