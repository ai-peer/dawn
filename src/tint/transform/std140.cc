...
struct Std140::State {
  ...
  private:
        ...
        for (auto* global : ctx.src->Sem().Module()->DependencyOrderedDeclarations()) {
            ...
            if (str && str->UsedAs(ast::StorageClass::kUniform)) {
                ...
                for (auto* member : str->Members()) {
                    ...
                    // with a cloned member once we know whether we need for fork the structure or
                    // not.
                    members.Push(member->Declaration());
                }

                // Did any of the member require forking the structure?
                if (fork_std140) {
    ...
    /// Walks the globals variables, replacing the type of those that are a uniform buffer with a
    ...
    /// @returns the forked std140 type for the corresponding type. If the type is not split for
    ///          std140-layout, then nullptr is returned.
