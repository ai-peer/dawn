...
struct Std140::State {
    ...
    static bool ShouldRun(const Program* program) {
        auto decompose = [&](const sem::Type* ty) {
        ...
        for (auto* decl : program->AST().GlobalVariables()) {
            ...
            if (global->StorageClass() == ast::StorageClass::kUniform) {
                if (decompose(global->Type()->UnwrapRef())) {
                    return true;
                }
            }
        }
  ...
  private:
    ...
    /// column vectors. Populates the #std140_mat_members map and #std140_structs set.
    void ForkTypes() {
