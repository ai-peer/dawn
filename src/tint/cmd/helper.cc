...
ProgramInfo LoadProgramInfo(const std::string& filename
    ...
    auto input_format = InputFormatFromFilename(filename);
    switch (input_format) {
        ...
        case InputFormat::kWgsl: {
            ...
            if (!ReadFile<uint8_t>(filename, &data)) {
                exit(1);
            }
        ...
        case InputFormat::kSpirvBin: {
            ...
            if (!ReadFile<uint32_t>(filename, &data)) {
                exit(1);
            }
            program = std::make_unique<tint::Program>(
                tint::reader::spirv::Parse(data, spirv_reader_options));
            break;
#else
        ...
        case InputFormat::kSpirvAsm: {
            ...
            if (!ReadFile<char>(filename, &text)) {
                exit(1);
            }
    ...
    if (!program) {
        std::cerr << "Failed to parse input file: " << filename << std::endl;
        exit(1);
    }
