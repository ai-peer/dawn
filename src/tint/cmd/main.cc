...
struct Options {
    ...
    std::vector<std::string> fxc_defines;
    std::vector<std::string> dxc_defines;
};
...
${transforms}
  ...
  --xcrun                   -- Path to xcrun executable, used to validate MSL output.
                               When specified, automatically enables --validate
  --fxc-defines <defines>   -- FXC defines as NAME or NAME=VALUE, comma-separated.
  --dxc-defines <defines>   -- DXC defines as NAME or NAME=VALUE, comma-separated.)";
...
std::vector<std::string> split_csvs(std::string list) {
...
bool ParseArgs(const std::vector<std::string>& args, Options* opts) {
    for (size_t i = 1; i < args.size(); ++i) {
        ...
        if (arg == "--format") {
            ...
            opts->transforms = split_csvs(args[i]);
            ...
            opts->validate = true;
        } else if (arg == "--fxc-defines") {
            ++i;
            if (i >= args.size()) {
                std::cerr << "Missing value for " << arg << std::endl;
                return false;
            }
            opts->fxc_defines = split_csvs(args[i]);
        } else if (arg == "--dxc-defines") {
            ++i;
            if (i >= args.size()) {
                std::cerr << "Missing value for " << arg << std::endl;
                return false;
            }
            opts->dxc_defines = split_csvs(args[i]);
        } else if (!arg.empty()) {
            if (arg[0] == '-') {
...
bool GenerateHlsl(const tint::Program* program, const Options& options) {
    ...
    if (options.validate) {
        ...
        if (options.use_fxc) {
#ifdef _WIN32
            res = tint::val::HlslUsingFXC(result.hlsl, result.entry_points, options.fxc_defines);
#else
            res.failed = true;
