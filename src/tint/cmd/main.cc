...
struct Options {
    ...
    bool emit_inspector_bindings = false;
...
bool ParseArgs(tint::utils::VectorRef<std::string_view> arguments,
    ...
    SPV_WRITER_ONLY(format_enum_names.Push(EnumName(Format::kSpirv, "spirv")));
    SPV_WRITER_ONLY(format_enum_names.Push(EnumName(Format::kSpvAsm, "spvasm")));
    WGSL_WRITER_ONLY(format_enum_names.Push(EnumName(Format::kWgsl, "wgsl")));
    MSL_WRITER_ONLY(format_enum_names.Push(EnumName(Format::kMsl, "msl")));
    HLSL_WRITER_ONLY(format_enum_names.Push(EnumName(Format::kHlsl, "hlsl")));
    GLSL_WRITER_ONLY(format_enum_names.Push(EnumName(Format::kGlsl, "glsl")));
...
#if TINT_BUILD_IR
    auto& emit_ir = options.Add<BoolOption>("emit-ir", "Writes the IR to stdout", Default{false});
    TINT_DEFER(opts->emit_ir = *emit_ir.value);

    auto& emit_ir_graph = options.Add<BoolOption>(
        "emit-ir-graph", "Writes the IR graph to 'tint.dot' as a dot graph", Default{false});
    TINT_DEFER(opts->emit_ir_graph = *emit_ir_graph.value);
...
violations that may be produced)",
    ...
    auto& emit_inspector_bindings = options.Add<BoolOption>(
        "emit-inspector-bindings", "Emit reflection data about bindings to stdout", Default{false});
    TINT_DEFER(opts->emit_inspector_bindings = *emit_inspector_bindings.value);

#if TINT_BUILD_SYNTAX_TREE_WRITER
    auto& emit_ast =
        options.Add<BoolOption>("emit-ast", "Writes the AST to stdout", Default{false});
    TINT_DEFER(opts->emit_ast = *emit_ast.value);
#endif  // TINT_BUILD_SYNTAX_TREE_WRITER
...
int main(int argc, const char** argv) {
        ...
        {"substitute_override",
         [&](tint::inspector::Inspector& inspector, tint::transform::Manager& m,
             ...
             for (auto override : options.overrides) {
                 auto name = override.key;
                 auto value = override.value;
                 if (name.empty()) {
                 ...
                 if (auto num = tint::utils::ParseNumber<decltype(tint::OverrideId::value)>(name)) {
                     ...
                     auto it = override_names.find(std::string(name));
    ...
    if (options.emit_inspector_bindings) {
