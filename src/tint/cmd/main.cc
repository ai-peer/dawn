...
bool GenerateHlsl(const tint::Program* program, const Options& options) {
    ...
    // compiler.
    const bool must_validate_dxc = options.validate || !options.dxc_path.empty();
    const bool must_validate_fxc = options.validate || !options.fxc_path.empty();
    if (options.validate || must_validate_dxc || must_validate_fxc) {
        ...
        if (options.validate || must_validate_fxc) {
            ...
            if (must_validate_dxc) {
                res.failed = true;
                res.output = "FXC can only be used on Windows.";
            }
#endif  // _WIN32
        }
