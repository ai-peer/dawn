...
class Impl {
  ...
  private:
    ...
    tint::Vector<ir::ControlInstruction*, 8> control_stack_;
    ...
    void EmitFunction(const ast::Function* ast_func) {
        ...
        tint::Vector<ir::FunctionParam*, 1> params;
    ...
    void EmitSwitch(const ast::SwitchStatement* stmt) {
        ...
        for (const auto* c : sem->Cases()) {
            tint::Vector<ir::Switch::CaseSelector, 4> selectors;
    ...
    ValueOrVecElAccess EmitExpression(const ast::Expression* root) {
        struct Emitter {
          ...
          private:
            Impl& impl;
            tint::Vector<ir::Block*, 8> blocks;
            tint::Vector<std::function<void()>, 64> tasks;
            tint::Hashmap<const ast::Expression*, ValueOrVecElAccess, 64> bindings_;
