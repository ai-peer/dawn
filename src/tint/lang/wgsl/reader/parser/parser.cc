...
Maybe<Void> Parser::enable_directive() {
    return sync(Token::Type::kSemicolon, [&]() -> Maybe<Void> {
        ...
        tint::Vector<const ast::Extension*, 4> extensions;
        while (continue_parsing()) {
...
Maybe<const ast::Attribute*> Parser::attribute() {
    ...
    tint::Vector<const ast::Expression*, 2> args;
...
Expect<Void> Parser::expect_attributes_consumed(tint::VectorRef<const ast::Attribute*> in) {
    if (in.IsEmpty()) {
        return kSuccess;
    }
