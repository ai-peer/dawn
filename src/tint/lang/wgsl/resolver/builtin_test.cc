...
TEST_F(ResolverBuiltinArrayTest, ArrayLength_Vector) {
    ...
    auto* str = Structure("S", tint::Vector{Member("x", ary)});
...
TEST_P(ResolverBuiltinDerivativeTest, Scalar) {
    ...
    Func("func", tint::Empty, ty.void_(), tint::Vector{Ignore(expr)},
...
inline tint::StringStream& operator<<(tint::StringStream& out, Texture data) {
...
static std::string to_str(const std::string& func, tint::VectorRef<const sem::Parameter*> params) {
...
TEST_P(ResolverBuiltinTest_Texture, Call) {
    ...
    Func("func", tint::Empty, ty.void_(), tint::Vector{stmt},
