...
TEST_F(ResolverSubgroupsExtensionTest, UseSubgroupSizeAttribWithoutExtensionError) {
    Structure("Inputs",
              Vector{
                  Member("a", ty.u32(), Vector{Builtin(builtin::BuiltinValue::kSubgroupSize)}),
              });
...
TEST_F(ResolverSubgroupsExtensionTest, UseSubgroupInvocationIdAttribWithoutExtensionError) {
    Structure("Inputs", Vector{
                            Member("a", ty.u32(),
                                   Vector{Builtin(builtin::BuiltinValue::kSubgroupInvocationId)}),
                        });
...
TEST_F(ResolverSubgroupsExtensionTest, SubgroupSizeI32Error) {
    ...
    Structure("Inputs",
              Vector{
                  Member("a", ty.i32(), Vector{Builtin(builtin::BuiltinValue::kSubgroupSize)}),
              });
...
TEST_F(ResolverSubgroupsExtensionTest, SubgroupSizeFragmentShader) {
    Enable(builtin::Extension::kChromiumExperimentalSubgroups);
    Func("main",
         Vector{Param("size", ty.u32(), Vector{Builtin(builtin::BuiltinValue::kSubgroupSize)})},
...
TEST_F(ResolverSubgroupsExtensionTest, SubgroupInvocationIdFragmentShader) {
    ...
    Func("main",
         Vector{
             Param("id", ty.u32(), Vector{Builtin(builtin::BuiltinValue::kSubgroupInvocationId)})},
         ty.void_(), Empty, Vector{Stage(ast::PipelineStage::kFragment)});
