...
core::type::Type* IncompleteType::Clone(core::type::CloneContext& ctx) const {
    TINT_ICE() << "IncompleteType does not support cloning";
    return nullptr;
}

core::type::TypeAndCount IncompleteType::Elements(const Type* type_if_invalid /* = nullptr */,
                                                  uint32_t count_if_invalid /* = 0 */) const {
    return {};
}

const core::type::Type* IncompleteType::Element(uint32_t index) const {
    return nullptr;
}
