...
/// The tint::Vector `N` template argument value for arrays of parameters.
constexpr static const size_t kNumFixedParams = 8;

/// The tint::Vector `N` template argument value for arrays of overload candidates.
constexpr static const size_t kNumFixedCandidates = 8;

/// A special type that matches all TypeMatchers
class Any final : public tint::Castable<Any, type::Type> {
  public:
...
class TemplateState {
  public:
    ...
    const type::Type* Type(size_t idx, const type::Type* ty) {
        ...
        ty = type::Type::Common(tint::Vector{t, ty});
        if (ty) {
            t = ty;
        }
        return ty;
  ...
  private:
    tint::Vector<const type::Type*, 4> types_;
    tint::Vector<Number, 2> numbers_;
};
...
struct IntrinsicPrototype {
    ...
    struct Hasher {
        ...
        inline std::size_t operator()(const IntrinsicPrototype& i) const {
            size_t hash = tint::Hash(i.parameters.Length());
            for (auto& p : i.parameters) {
                hash = tint::HashCombine(hash, p.type, p.usage);
            }
            return tint::Hash(hash, i.overload, i.return_type);
        }
    };

    const OverloadInfo* overload = nullptr;
    type::Type const* return_type = nullptr;
    tint::Vector<Parameter, kNumFixedParams> parameters;
};
...
class Impl : public IntrinsicTable {
  public:
    ...
    Builtin Lookup(builtin::Function builtin_type,
                   tint::VectorRef<const type::Type*> args,
                   sem::EvaluationStage earliest_eval_stage,
  ...
  private:
    ...
    struct Candidate {
        ...
        tint::Vector<IntrinsicPrototype::Parameter, kNumFixedParams> parameters;
    ...
    using Candidates = tint::Vector<Candidate, kNumFixedCandidates>;

    /// Callback function when no overloads match.
    using OnNoMatch = std::function<void(tint::VectorRef<Candidate>)>;
    ...
    void PrintOverload(tint::StringStream& ss,
                       const OverloadInfo* overload,
                       const char* intrinsic_name) const;

    // Prints the list of candidates for emitting diagnostics
    void PrintCandidates(tint::StringStream& ss,
                         tint::VectorRef<Candidate> candidates,
                         const char* intrinsic_name) const;
    ...
    void ErrAmbiguousOverload(const char* intrinsic_name,
                              tint::VectorRef<const type::Type*> args,
                              TemplateState templates,
                              tint::VectorRef<Candidate> candidates) const;

    ProgramBuilder& builder;
    Matchers matchers;
    tint::Hashmap<IntrinsicPrototype, sem::Builtin*, 64, IntrinsicPrototype::Hasher> builtins;
    tint::Hashmap<IntrinsicPrototype, sem::ValueConstructor*, 16, IntrinsicPrototype::Hasher>
        constructors;
    tint::Hashmap<IntrinsicPrototype, sem::ValueConversion*, 16, IntrinsicPrototype::Hasher>
        converters;
};
...
Impl::Builtin Impl::Lookup(builtin::Function builtin_type,
    ...
    auto on_no_match = [&](tint::VectorRef<Candidate> candidates) {
        tint::StringStream ss;
    ...
    auto* sem = builtins.GetOrCreate(match, [&] {
        tint::Vector<sem::Parameter*, kNumFixedParams> params;
...
IntrinsicTable::UnaryOperator Impl::Lookup(ast::UnaryOp op,
    ...
    tint::Vector args{arg};

    // Generates an error when no overloads match the provided arguments
    auto on_no_match = [&, name = intrinsic_name](tint::VectorRef<Candidate> candidates) {
        tint::StringStream ss;
...
IntrinsicPrototype Impl::MatchIntrinsic(const IntrinsicInfo& intrinsic,
    ...
    size_t match_idx = 0;
    tint::Vector<Candidate, kNumFixedCandidates> candidates;
...
Impl::Candidate Impl::ResolveCandidate(Impl::Candidates&& candidates,
                                       ...
                                       tint::VectorRef<const type::Type*> args,
                                       TemplateState templates) const {
    tint::Vector<uint32_t, kNumFixedParams> best_ranks;
...
void Impl::PrintOverload(tint::StringStream& ss,
                         const OverloadInfo* overload,
                         const char* intrinsic_name) const {
    TemplateState templates;
...
void Impl::PrintCandidates(tint::StringStream& ss,
                           tint::VectorRef<Candidate> candidates,
                           const char* intrinsic_name) const {
    for (auto& candidate : candidates) {
        ss << "  ";
