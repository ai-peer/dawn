...
TEST_F(ResolverRootIdentifierTest, Parameter) {
    ...
    auto* expr = Expr(a);
    Func("foo", tint::Vector{a}, ty.void_(), tint::Vector{WrapInStatement(expr)});
...
TEST_F(ResolverRootIdentifierTest, PointerParameter) {
    ...
    Func("foo", tint::Vector{param}, ty.void_(),
         tint::Vector{WrapInStatement(let), WrapInStatement(expr_let)});
...
TEST_F(ResolverRootIdentifierTest, ThroughMemberAccessor) {
    ...
    //   a.f
    // }
    auto* S = Structure("S", tint::Vector{Member("f", ty.f32())});
