...
const constant::Value* ConvertInternal(const constant::Value* root_value,
    ...
    tint::Vector<Action, 8> pending{
        ActionConvert{root_value, root_target_ty},
    };

    tint::Vector<const constant::Value*, 32> value_stack;
    ...
    while (!pending.IsEmpty()) {
        ...
        if (auto* build = std::get_if<ActionBuildComposite>(&next)) {
            ...
            tint::Vector<const constant::Value*, 32> elements;
...
ConstEval::Result TransformElements(ProgramBuilder& builder,
    ...
    tint::Vector<const constant::Value*, 8> els;
    els.Reserve(n);
...
const constant::Value* ConstEval::ZeroValue(const type::Type* type) {
    return Switch(
        ...
        [&](const type::Struct* s) -> const constant::Value* {
            tint::Hashmap<const type::Type*, const constant::Value*, 8> zero_by_type;
            tint::Vector<const constant::Value*, 4> zeros;
...
template <typename NumberT>
tint::Result<NumberT> ConstEval::Clamp(const Source& source,
                                        NumberT e,
                                        NumberT low,
                                        NumberT high) {
    if (low > high) {
...
ConstEval::Result ConstEval::ArrayOrStructCtor(const type::Type* ty,
                                               tint::VectorRef<const constant::Value*> args) {
    if (args.IsEmpty()) {
        return ZeroValue(ty);
    }
...
ConstEval::Result ConstEval::Zero(const type::Type* ty,
                                  tint::VectorRef<const constant::Value*>,
                                  const Source&) {
    return ZeroValue(ty);
}
...
ConstEval::Result ConstEval::VecInitM(const type::Type* ty,
                                      tint::VectorRef<const constant::Value*> args,
                                      const Source&) {
    tint::Vector<const constant::Value*, 4> els;
    for (auto* arg : args) {
        auto* val = arg;
        if (!val) {
            return nullptr;
...
ConstEval::Result ConstEval::MatInitS(const type::Type* ty,
    ...
    for (uint32_t c = 0; c < m->columns(); c++) {
        tint::Vector<const constant::Value*, 4> column;
...
ConstEval::Result ConstEval::Swizzle(const type::Type* ty,
                                     ...
                                     tint::VectorRef<uint32_t> indices) {
...
ConstEval::Result ConstEval::Bitcast(const type::Type* ty,
    ...
    tint::Vector<std::byte, 16> buffer;
...
ConstEval::Result ConstEval::OpMultiplyMatVec(const type::Type* ty,
    ...
    tint::Vector<const constant::Value*, 4> result;
...
ConstEval::Result ConstEval::OpMultiplyMatMat(const type::Type* ty,
    ...
    tint::Vector<const constant::Value*, 4> result_mat;
    for (size_t c = 0; c < mat2_ty->columns(); ++c) {
        tint::Vector<const constant::Value*, 4> col_vec;
...
ConstEval::Result ConstEval::cross(const type::Type* ty,
    ...
    return builder.constants.Composite(
        ty, tint::Vector<const constant::Value*, 3>{x.Get(), y.Get(), z.Get()});
}
...
ConstEval::Result ConstEval::faceForward(const type::Type* ty,
    ...
    return OpUnaryMinus(ty, tint::Vector{e1}, source);
}
...
ConstEval::Result ConstEval::frexp(const type::Type* ty,
    ...
    if (auto* vec = arg->Type()->As<type::Vector>()) {
        tint::Vector<const constant::Value*, 4> fract_els;
        tint::Vector<const constant::Value*, 4> exp_els;
        ...
        return builder.constants.Composite(
            ty, tint::Vector<const constant::Value*, 2>{
        ...
        return builder.constants.Composite(ty, tint::Vector<const constant::Value*, 2>{
                                                   fe.fract.Get(),
                                                   fe.exp.Get(),
                                               });
    }
...
ConstEval::Result ConstEval::modf(const type::Type* ty,
    ...
    tint::Vector<const constant::Value*, 2> fields;
...
ConstEval::Result ConstEval::normalize(const type::Type* ty,
    ...
    return OpDivide(ty, tint::Vector{args[0], v}, source);
}
...
ConstEval::Result ConstEval::pack2x16snorm(const type::Type* ty,
    ...
    auto calc = [&](f32 val) -> u32 {
        ...
        return u32(tint::Bitcast<uint16_t>(
...
ConstEval::Result ConstEval::transpose(const type::Type* ty,
    ...
    for (size_t r = 0; r < mat_ty->rows(); ++r) {
        tint::Vector<const constant::Value*, 4> new_col_vec;
...
ConstEval::Result ConstEval::unpack2x16float(const type::Type* ty,
    ...
    tint::Vector<const constant::Value*, 2> els;
    els.Reserve(2);
