...
struct DependencyEdgeCmp {
    ...
    inline std::size_t operator()(const DependencyEdge& d) const {
        return tint::Hash(d.from, d.to);
    }
};

/// A map of DependencyEdge to DependencyInfo
using DependencyEdges =
    tint::Hashmap<DependencyEdge, DependencyInfo, 64, DependencyEdgeCmp, DependencyEdgeCmp>;
...
struct Global {
    ...
    tint::Vector<Global*, 8> deps;
};

/// A map of global name to Global
using GlobalMap = tint::Hashmap<Symbol, Global*, 16>;
...
class DependencyScanner {
  ...
  private:
    ...
    /// global dependencies.
    void TraverseStatements(tint::VectorRef<const ast::Statement*> stmts) {
        for (auto* s : stmts) {
            TraverseStatement(s);
        }
    }
    ...
    void TraverseExpression(const ast::Expression* root_expr) {
        ...
        tint::Vector<const ast::Expression*, 8> pending{root_expr};
    ...
    void TraverseAttributes(tint::VectorRef<const ast::Attribute*> attrs) {
        for (auto* attr : attrs) {
    ...
    using VariableMap = tint::Hashmap<Symbol, const ast::Variable*, 32>;
    ...
    tint::Hashmap<Symbol, BuiltinInfo, 64> builtin_info_map;
};
...
struct DependencyAnalysis {
  ...
  private:
    ...
    void TraverseDependencies(const Global* root, ENTER&& enter, EXIT&& exit) {
        ...
        tint::Vector<Entry, 16> stack{Entry{root, 0}};
        while (true) {
    ...
    void SortGlobals() {
        ...
        for (auto* global : declaration_order_) {
            ...
            tint::UniqueVector<const Global*, 8> stack;
    ...
    void CyclicDependencyFound(const Global* root, tint::VectorRef<const Global*> stack) {
    ...
    tint::BlockAllocator<Global> allocator_;
    ...
    tint::Vector<Global*, 64> declaration_order_;

    /// Globals in sorted dependency order. Populated by SortGlobals().
    tint::UniqueVector<const ast::Node*, 64> sorted_;
};
