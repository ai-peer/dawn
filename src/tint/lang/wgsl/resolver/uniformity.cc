...
struct FunctionInfo {
    ...
    Node* RequiredToBeUniform(builtin::DiagnosticSeverity severity) {
        switch (severity) {
            ...
            default:
                TINT_ASSERT(false && "unhandled severity");
                return nullptr;
        }
    }
  ...
  private:
    ...
    const ast::CallExpression* FindBuiltinThatRequiresUniformity(
        ...
        if (target->Is<sem::Builtin>()) {
            ...
            TINT_ASSERT(false && "unable to find child call with uniformity requirement");
        } else {
            TINT_ASSERT(false && "unexpected call expression type");
        }
        return nullptr;
    }
