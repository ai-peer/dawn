...
struct Node {
    ...
    tint::UniqueVector<Node*, 4> edges;
...
struct ParameterInfo {
    ...
    tint::Vector<const sem::Parameter*, 8> ptr_output_source_param_values;
    ...
    tint::Vector<const sem::Parameter*, 8> ptr_output_source_param_contents;
...
struct FunctionInfo {
    ...
    tint::Vector<ParameterInfo, 8> parameters;

    /// The control flow graph.
    tint::BlockAllocator<Node> nodes;
    ...
    tint::Hashset<const sem::Variable*, 8> local_var_decls;

    /// The set of partial pointer variables - pointers that point to a subobject (into an array or
    /// struct).
    tint::Hashset<const sem::Variable*, 4> partial_ptrs;
    ...
    struct LoopSwitchInfo {
        ...
        tint::Hashmap<const sem::Variable*, Node*, 4> var_in_nodes;
        /// The exit values for local variables at the end of this construct.
        tint::Hashmap<const sem::Variable*, Node*, 4> var_exit_nodes;
    };
  ...
  private:
    ...
    tint::Hashset<std::string, 8> tags_;

    /// Map from control flow statements to the corresponding LoopSwitchInfo structure.
    tint::Hashmap<const sem::Statement*, LoopSwitchInfo*, 8> loop_switch_infos;

    /// Allocator of LoopSwitchInfos
    tint::BlockAllocator<LoopSwitchInfo> loop_switch_info_allocator;
};
...
class UniformityGraph {
  ...
  private:
    ...
    tint::Hashmap<const ast::Function*, FunctionInfo, 8> functions_;
    ...
    bool ProcessFunction(const ast::Function* func) {
        ...
        auto get_param_tag = [&](tint::UniqueVector<Node*, 4>& reachable, size_t index) {
        ...
        {
            tint::UniqueVector<Node*, 4> reachable;
    ...
    Node* ProcessStatement(Node* cf, const ast::Statement* stmt) {
        return Switch(
            ...
            [&](const ast::BlockStatement* b) {
                tint::Hashmap<const sem::Variable*, Node*, 4> scoped_assignments;
                {
            ...
            [&](const ast::IfStatement* i) {
                ...
                tint::Hashmap<const sem::Variable*, Node*, 4> true_vars;
                tint::Hashmap<const sem::Variable*, Node*, 4> false_vars;
                ...
                auto process_in_scope =
                    [&](Node* cf_in, const ast::Statement* s,
                        tint::Hashmap<const sem::Variable*, Node*, 4>& assigned_vars) {
    ...
    std::pair<Node*, Node*> ProcessCall(Node* cf, const ast::CallExpression* call) {
        ...
        Node* cf_last_arg = cf;
        tint::Vector<Node*, 8> args;
        tint::Vector<Node*, 8> ptrarg_contents;
    ...
    void Traverse(Node* source, tint::UniqueVector<Node*, 4>* reachable = nullptr) {
        tint::Vector<Node*, 8> to_visit{source};
