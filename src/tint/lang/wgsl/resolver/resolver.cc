...
core::type::Type* Resolver::Type(const ast::Expression* ast) {
    ...
    if (auto* incomplete = type->As<IncompleteType>(); TINT_UNLIKELY(incomplete)) {
        AddError("expected '<' for '" + std::string(ToString(incomplete->builtin)) + "'", ast->source.End());
        return nullptr;
    }
    return type;
}
        ...
        [&](const sem::TypeExpression* ty_expr) {
            return Switch(
                ...
                [&](const IncompleteType* t) -> sem::Call* {
                    auto* call = incomplete_type(t);
                    if (TINT_UNLIKELY(!call)) {
                        return nullptr;
                    }
                    // Replace incomplete type with resolved type
                    const_cast<sem::TypeExpression*>(ty_expr)->SetType(call->Type());
                    return call;
                },
