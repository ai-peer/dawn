...
bool Resolver::ResolveInternal() {
    ...
    tint::Vector<const ast::DiagnosticControl*, 4> diagnostic_controls;
...
void Resolver::SetShadows() {
    for (auto it : dependencies_.shadows) {
        tint::CastableBase* b = sem_.Get(it.value);
        if (TINT_UNLIKELY(!b)) {
            TINT_ICE(Resolver, diagnostics_)
...
sem::GlobalVariable* Resolver::GlobalVariable(const ast::Variable* v) {
    tint::UniqueVector<const sem::GlobalVariable*, 4> transitively_referenced_overrides;
...
sem::Function* Resolver::Function(const ast::Function* decl) {
    ...
    tint::Hashmap<Symbol, Source, 8> parameter_names;
...
bool Resolver::Statements(tint::VectorRef<const ast::Statement*> stmts) {
...
sem::Expression* Resolver::Expression(const ast::Expression* root) {
    tint::Vector<const ast::Expression*, 64> sorted;
...
template <size_t N>
bool Resolver::MaybeMaterializeAndLoadArguments(tint::Vector<const sem::ValueExpression*, N>& args,
...
template <size_t N>
tint::Result<tint::Vector<const constant::Value*, N>> Resolver::ConvertArguments(
    const tint::Vector<const sem::ValueExpression*, N>& args,
    const sem::CallTarget* target) {
...
sem::Call* Resolver::Call(const ast::CallExpression* expr) {
    ...
    tint::Vector<const sem::ValueExpression*, 8> args;
        ...
        return Switch(
            ...
            [&](const type::Struct* str) -> sem::Call* {
                auto* call_target = struct_ctors_.GetOrCreate(
                    ...
                    [&]() -> sem::ValueConstructor* {
                        tint::Vector<sem::Parameter*, 8> params;
    ...
    auto inferred_array = [&]() -> tint::sem::Call* {
        ...
        if (!el_ty) {
            AddError("cannot infer common array element type from constructor arguments",
                     expr->source);
            tint::Hashset<const type::Type*, 8> types;
            for (size_t i = 0; i < args.Length(); i++) {
...
sem::Call* Resolver::BuiltinCall(const ast::CallExpression* expr,
                                 ...
                                 tint::Vector<const sem::ValueExpression*, N>& args) {
...
void Resolver::CollectTextureSamplerPairs(
    ...
    tint::VectorRef<const sem::ValueExpression*> args) const {
    ...
    if (auto* unresolved = resolved->Unresolved()) {
        if (identifier_resolve_hint_.expression == expr) {
            ...
            if (!identifier_resolve_hint_.suggestions.IsEmpty()) {
                ...
                tint::Vector<const char*, 8> filtered;
                for (auto* str : identifier_resolve_hint_.suggestions) {
                    if (str[0] != '_') {
                        filtered.Push(str);
                    }
                }
                tint::StringStream msg;
...
sem::ValueExpression* Resolver::MemberAccessor(const ast::MemberAccessorExpression* expr) {
    ...
    return Switch(
        ...
        [&](const type::Vector* vec) -> sem::ValueExpression* {
            ...
            auto size = s.size();
            tint::Vector<uint32_t, 4> swizzle;
...
sem::ValueExpression* Resolver::Binary(const ast::BinaryExpression* expr) {
    ...
    if (skip_const_eval_.Contains(expr)) {
        ...
        if (op.const_eval_fn) {  // Do we have a @const operator?
            ...
            tint::Vector const_args{lhs->ConstantValue(), rhs->ConstantValue()};
            ...
            if (stage == sem::EvaluationStage::kConstant) {
                if (op.const_eval_fn) {
                    if (auto r = (const_eval_.*op.const_eval_fn)(
                            ty, tint::Vector{expr->ConstantValue()},
...
tint::Result<sem::WorkgroupSize> Resolver::WorkgroupAttribute(
    const ast::WorkgroupAttribute* attr) {
    ...
    tint::Vector<const sem::ValueExpression*, 3> args;
    tint::Vector<const type::Type*, 3> arg_tys;
...
bool Resolver::ArrayAttributes(tint::VectorRef<const ast::Attribute*> attributes,
...
sem::Struct* Resolver::Structure(const ast::Struct* str) {
    ...
    tint::Vector<const sem::StructMember*, 8> sem_members;
    ...
    tint::Hashmap<Symbol, const ast::StructMember*, 8> member_map;
...
sem::SwitchStatement* Resolver::SwitchStatement(const ast::SwitchStatement* stmt) {
    ...
    return StatementScope(stmt, sem, [&] {
        ...
        tint::Vector<const type::Type*, 8> types;
        types.Push(cond_ty);
        ...
        tint::Vector<sem::CaseStatement*, 4> cases;
