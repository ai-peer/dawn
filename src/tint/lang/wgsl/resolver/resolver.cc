...
sem::Call* Resolver::BuiltinCall(const ast::CallExpression* expr,
    ...
    auto arg_tys = tint::Transform(args, [](auto* arg) { return arg->Type(); });
    auto overload = intrinsic_table_->Lookup(builtin_type, arg_tys, arg_stage, expr->source);
    if (!overload) {
        return nullptr;
    }

    // De-duplicate builtins that are identical.
    auto* target = builtins_.GetOrCreate(overload.Get(), [&] {
        ...
        return builder_->create<sem::Builtin>(
            builtin_type, overload->return_type, std::move(params), eval_stage, supported_stages,
