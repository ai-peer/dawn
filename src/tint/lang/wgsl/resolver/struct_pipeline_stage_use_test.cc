...
TEST_F(ResolverPipelineStageUseTest, StructUsedAsVertexShaderParam) {
    ...
    Func("main", tint::Vector{Param("param", ty.Of(s))}, ty.vec4<f32>(),
         tint::Vector{Return(Call<vec4<f32>>())},
...
TEST_F(ResolverPipelineStageUseTest, StructUsedAsComputeShaderParam) {
    auto* s = Structure(
        "S",
        tint::Vector{Member(
            "a", ty.u32(), tint::Vector{Builtin(builtin::BuiltinValue::kLocalInvocationIndex)})});
...
TEST_F(ResolverPipelineStageUseTest, StructUsedAsShaderReturnTypeViaAlias) {
    ...
    Func("main", tint::Empty, ty.Of(s_alias),
         tint::Vector{Return(Call(ty.Of(s_alias), Expr(0_f)))},
         tint::Vector{Stage(ast::PipelineStage::kFragment)});
