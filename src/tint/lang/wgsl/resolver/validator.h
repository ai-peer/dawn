...
class Validator {
  public:
    ...
    bool PipelineStages(tint::VectorRef<sem::Function*> entry_points) const;

    /// Validates push_constant variables
    /// @param entry_points the entry points to the module
    /// @returns true on success, false otherwise.
    bool PushConstants(tint::VectorRef<sem::Function*> entry_points) const;
    ...
    bool GlobalVariable(
        ...
        const tint::Hashmap<OverrideId, const sem::Variable*, 8>& override_id) const;
    ...
    /// @returns true on success, false otherwise
    bool Statements(tint::VectorRef<const ast::Statement*> stmts) const;
    ...
    /// @returns true on success, false otherwise.
    bool NoDuplicateAttributes(tint::VectorRef<const ast::Attribute*> attributes) const;
    ...
    /// @returns true on success, false otherwise.
    bool DiagnosticControls(tint::VectorRef<const ast::DiagnosticControl*> controls,
    ...
    bool IsValidationDisabled(tint::VectorRef<const ast::Attribute*> attributes,
    ...
    /// @param attributes the attribute list to check
    /// @param validation the validation mode to check
    bool IsValidationEnabled(tint::VectorRef<const ast::Attribute*> attributes,
                             ast::DisabledValidation validation) const;
  ...
  private:
    ...
    const tint::Hashmap<const type::Type*, const Source*, 8>& atomic_composite_info_;
...
class hash<tint::resolver::TypeAndAddressSpace> {
  public:
    ...
    inline std::size_t operator()(const tint::resolver::TypeAndAddressSpace& tas) const {
        return tint::Hash(tas.type, tas.address_space);
    }
};
