...
TEST_F(ResolverCallTest, Valid) {
    ...
    tint::Vector<const ast::Parameter*, 4> params;
    ...
    auto* func = Func("foo", std::move(params), ty.f32(), tint::Vector{Return(1.23_f)});
...
TEST_F(ResolverCallTest, OutOfOrder) {
    ...
    Func("a", tint::Empty, ty.void_(), tint::Vector{CallStmt(call_expr)});
