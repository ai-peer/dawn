...
class TestHelper : public ProgramBuilder {
  public:
    ...
    bool CheckVarUsers(const ast::Variable* var,
                       tint::VectorRef<const ast::Expression*> expected_users) {
...
struct DataType<bool> {
    ...
    static inline const ast::Expression* Expr(ProgramBuilder& b, tint::VectorRef<Scalar> args) {
    ...
    static inline const ast::Expression* ExprFromDouble(ProgramBuilder& b, double v) {
        return Expr(b, tint::Vector<Scalar, 1>{static_cast<ElementType>(v)});
    }
...
struct DataType<f32> {
    ...
    /// @return a new AST f32 literal value expression
    static inline const ast::Expression* ExprFromDouble(ProgramBuilder& b, double v) {
        return Expr(b, tint::Vector<Scalar, 1>{static_cast<f32>(v)});
    }
    /// @returns the WGSL name for the type
...
struct DataType<builtin::fluent_types::vec<N, T>> {
    ...
    static inline auto ExprArgs(ProgramBuilder& b, tint::VectorRef<Scalar> args) {
        const bool one_value = args.Length() == 1;
        tint::Vector<const ast::Expression*, N> r;
        for (size_t i = 0; i < N; ++i) {
            r.Push(DataType<T>::Expr(b, tint::Vector<Scalar, 1>{one_value ? args[0] : args[i]}));
        }
        return r;
    }
    /// @param b the ProgramBuilder
        ...
        for (uint32_t i = 0; i < N; ++i) {
            if (one_value) {
                r.Push(DataType<builtin::fluent_types::vec<M, T>>::Expr(
                    b, tint::Vector<Scalar, 1>{args[0]}));
            } else {
                tint::Vector<Scalar, M> v;
...
struct Value {
    ...
    /// @returns Value
    template <typename T>
    static Value Create(tint::VectorRef<Scalar> args) {
...
Value Val(T v) {
    ...
    return Value::Create<T>(tint::Vector<Scalar, 1>{v});
}
...
Value Vec(Ts... args) {
    ...
    tint::Vector<Scalar, sizeof...(args)> v{args...};
