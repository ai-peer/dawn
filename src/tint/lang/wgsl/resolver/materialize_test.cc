...
TEST_P(MaterializeAbstractNumericToConcreteType, Test) {
    ...
    switch (method) {
        ...
        case Method::kPhonyAssign:
            ...
            break;
        case Method::kFnArg:
            Func("F", tint::Vector{Param("P", target_ty())}, ty.void_(), tint::Empty);
        ...
        case Method::kBuiltinArg:
            ...
            break;
        case Method::kReturn:
            Func("F", tint::Empty, target_ty(), tint::Vector{Return(abstract_expr)});
            break;
        case Method::kArray:
            WrapInFunction(Call(ty.array(target_ty(), 1_i), abstract_expr));
            break;
        case Method::kStruct:
            Structure("S", tint::Vector{Member("v", target_ty())});
        ...
        case Method::kWorkgroupSize:
            Func("f", tint::Empty, ty.void_(), tint::Empty,
                 tint::Vector{WorkgroupSize(target_expr(), abstract_expr, Expr(123_a)),
...
TEST_P(MaterializeAbstractNumericToDefaultType, Test) {
    ...
    tint::Vector<const ast::Expression*, 4> abstract_exprs;
    auto abstract_expr = [&] {
    ...
    switch (method) {
        ...
        case Method::kWorkgroupSize: {
            Func(
                "f", tint::Empty, ty.void_(), tint::Empty,
                tint::Vector{WorkgroupSize(abstract_expr()), Stage(ast::PipelineStage::kCompute)});
            break;
        }
        case Method::kIndex: {
