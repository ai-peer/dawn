...
class State {
  ...
  private:
    ...
    tint::Hashmap<ir::Value*, ValueBinding, 32> bindings_;

    /// Names for values
    tint::Hashmap<ir::Value*, Symbol, 32> names_;
    ...
    using StatementList = tint::Vector<const ast::Statement*,
    ...
    tint::Hashset<ir::Value*, 64> can_inline_;

    /// Set of enable directives emitted.
    tint::Hashset<builtin::Extension, 4> enables_;

    /// Map of struct to output program name.
    tint::Hashmap<const type::Struct*, Symbol, 8> structs_;
    ...
    const ast::Function* Fn(ir::Function* fn) {
        ...
        tint::Vector<const ast::Attribute*, 1> ret_attrs{};
    ...
    void MarkInlinable(ir::Block* block) {
        ...
        tint::UniqueVector<ir::Value*, 32> pending_resolution;
    ...
    void Var(ir::Var* var) {
        ...
        tint::Vector<const ast::Attribute*, 4> attrs;
    ...
    const ast::Expression* Constant(const constant::Value* c) {
        auto composite = [&](bool can_splat) {
            ...
            tint::Vector<const ast::Expression*, 8> els;
    ...
    ast::Type Struct(const type::Struct* s) {
        auto n = structs_.GetOrCreate(s, [&] {
            auto members = tint::Transform<8>(s->Members(), [&](const type::StructMember* m) {
                ...
                tint::Vector<const ast::Attribute*, 4> ast_attrs;
