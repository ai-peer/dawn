...
void SyntaxTreePrinter::EmitAttributes(tint::VectorRef<const ast::Attribute*> attrs) {
    for (auto* attr : attrs) {
        Switch(
            attr,  //
...
void SyntaxTreePrinter::EmitStatements(tint::VectorRef<const ast::Statement*> stmts) {
    for (auto* s : stmts) {
        EmitStatement(s);
    }
}

void SyntaxTreePrinter::EmitStatementsWithIndent(tint::VectorRef<const ast::Statement*> stmts) {
