...
class Info {
  public:
    ...
    using TransitivelyReferenced = tint::UniqueVector<const GlobalVariable*, 4>;
    ...
    template <typename SEM = InferFromAST,
              typename AST = tint::CastableBase,
    ...
    void AddTransitivelyReferencedOverride(const tint::CastableBase* from,
    ...
    const TransitivelyReferenced* TransitivelyReferencedOverrides(
        const tint::CastableBase* from) const {
        if (referenced_overrides_.count(from) == 0) {
            return nullptr;
        }
  ...
  private:
    ...
    std::vector<const tint::CastableBase*> nodes_;
    // Lists transitively referenced overrides for the given item
    std::unordered_map<const tint::CastableBase*, TransitivelyReferenced> referenced_overrides_;
    // The semantic module
