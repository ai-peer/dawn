...
class Inspector {
  public:
    ...
    /// by that entry point.
    tint::VectorRef<SamplerTexturePair> GetSamplerTextureUses(const std::string& entry_point);
  ...
  private:
    ...
    std::unique_ptr<std::unordered_map<std::string, tint::UniqueVector<SamplerTexturePair, 4>>>
        sampler_targets_;
