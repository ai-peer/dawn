...
tint::VectorRef<SamplerTexturePair> Inspector::GetSamplerTextureUses(
    const std::string& entry_point) {
    auto* func = FindEntryPointByName(entry_point);
    if (!func) {
        return {};
...
bool Inspector::ContainsBuiltin(builtin::BuiltinValue builtin,
                                const type::Type* type,
                                tint::VectorRef<const ast::Attribute*> attributes) const {
    auto* unwrapped_type = type->UnwrapRef();
...
void Inspector::GenerateSamplerTargets() {
    ...
    sampler_targets_ = std::make_unique<
        std::unordered_map<std::string, tint::UniqueVector<SamplerTexturePair, 4>>>();
...
void Inspector::GetOriginatingResources(std::array<const ast::Expression*, N> exprs, F&& callback) {
    ...
    tint::UniqueVector<const ast::CallExpression*, 8> callsites;
