...
void InspectorBuilder::MakeEmptyBodyFunction(std::string name,
    ...
    Func(name, tint::Empty, ty.void_(), tint::Vector{Return()}, attributes);
}

void InspectorBuilder::MakeCallerBodyFunction(std::string caller,
                                              tint::VectorRef<std::string> callees,
                                              tint::VectorRef<const ast::Attribute*> attributes) {
    tint::Vector<const ast::Statement*, 8> body;
...
const ast::Struct* InspectorBuilder::MakeInOutStruct(std::string name,
                                                     tint::VectorRef<InOutInfo> inout_vars) {
    tint::Vector<const ast::StructMember*, 8> members;
...
const ast::Function* InspectorBuilder::MakePlainGlobalReferenceBodyFunction(
    ...
    ast::Type type,
    tint::VectorRef<const ast::Attribute*> attributes) {
    tint::Vector<const ast::Statement*, 3> stmts;
...
bool InspectorBuilder::ContainsName(tint::VectorRef<StageVariable> vec, const std::string& name) {
    for (auto& s : vec) {
        if (s.name == name) {
            return true;
        }
...
void InspectorBuilder::MakeStructVariableReferenceBodyFunction(
    ...
    tint::VectorRef<std::tuple<size_t, ast::Type>> members) {
