...
class Attribute : public tint::Castable<Attribute, Node> {
  public:
    ~Attribute() override;
...
bool HasAttribute(tint::VectorRef<const Attribute*> attributes) {
...
template <typename T>
const T* GetAttribute(tint::VectorRef<const Attribute*> attributes) {
    for (auto* attr : attributes) {
        if (attr->Is<T>()) {
            return attr->As<T>();
        }
