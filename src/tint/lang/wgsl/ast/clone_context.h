...
class Cloneable : public tint::Castable<Cloneable> {
  public:
    /// Constructor
    Cloneable();
    /// Move constructor
...
class CloneContext {
  ...
  public:
    ...
    template <typename F>
    tint::traits::EnableIf<ParamTypeIsPtrOf<F, Cloneable>, CloneContext>& ReplaceAll(
        F&& replacer) {
  ...
  private:
    ...
    struct ListTransforms {
        ...
        tint::Hashset<const Cloneable*, 4> remove_;
        ...
        tint::Hashmap<const Cloneable*, CloneableBuilderList, 4> insert_before_;

        /// A map of object in #src to the list of cloned objects in #dst.
        /// Clone(const tint::Vector<T*>& v) will use this to insert the map-value
        /// list into the target vector after cloning and inserting the map-key.
        tint::Hashmap<const Cloneable*, CloneableBuilderList, 4> insert_after_;
    };
    ...
    tint::Hashmap<const Cloneable*, std::function<const Cloneable*()>, 8> replacements_;

    /// A map of symbol in #src to their cloned equivalent in #dst
    tint::Hashmap<Symbol, Symbol, 32> cloned_symbols_;
    ...
    tint::Hashmap<const void*, ListTransforms, 4> list_transforms_;
