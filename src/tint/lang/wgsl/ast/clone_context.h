...
class CloneContext {
  ...
  public:
    ...
    template <typename F>
    utils::traits::EnableIf<ParamTypeIsPtrOf<F, ast::Node>, CloneContext>& ReplaceAll(
        F&& replacer) {
    ...
    template <typename WHAT,
              typename WITH,
              typename = utils::traits::EnableIfIsType<WITH, ast::Node>>
  ...
  private:
    struct CloneableTransform {
        ...
        // utils::TypeInfo of the Node that the transform operates on
        const utils::TypeInfo* typeinfo;
        std::function<const ast::Node*(const ast::Node*)> function;
    };

    /// A vector of const ast::Node*
    using NodeBuilderList = utils::Vector<std::function<const ast::Node*()>, 4>;
    ...
    struct ListTransforms {
        ...
        utils::Hashset<const ast::Node*, 4> remove_;
        ...
        utils::Hashmap<const ast::Node*, NodeBuilderList, 4> insert_before_;

        /// A map of object in the source program to the list of cloned objects in #dst.
        /// Clone(const utils::Vector<T*>& v) will use this to insert the map-value
        /// list into the target vector after cloning and inserting the map-key.
        utils::Hashmap<const ast::Node*, NodeBuilderList, 4> insert_after_;
    };
    ...
    void CheckedCastFailure(const ast::Node* got, const utils::TypeInfo& expected);
    ...
    utils::Hashmap<const ast::Node*, std::function<const ast::Node*()>, 8> replacements_;
