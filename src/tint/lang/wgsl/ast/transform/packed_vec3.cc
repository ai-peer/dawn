...
struct PackedVec3::State {
    ...
    tint::Hashmap<const type::Type*, Symbol, 4> packed_vec3_wrapper_struct_names;

    /// A cache of host-shareable structures that have been rewritten.
    tint::Hashmap<const type::Type*, Symbol, 4> rewritten_structs;

    /// A map from type to the name of a helper function used to pack that type.
    tint::Hashmap<const type::Type*, Symbol, 4> pack_helpers;

    /// A map from type to the name of a helper function used to unpack that type.
    tint::Hashmap<const type::Type*, Symbol, 4> unpack_helpers;
    ...
    ApplyResult Run() {
        ...
        tint::Hashset<const sem::ValueExpression*, 8> to_pack;
        tint::Hashset<const sem::ValueExpression*, 8> to_unpack;
