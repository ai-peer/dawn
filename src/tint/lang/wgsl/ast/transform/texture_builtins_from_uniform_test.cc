...
fn main() {
  var len : u32 = tint_symbol_1.texture_builtin_values[0u][0u];
    ...
    // Note: Using the following EXPECT_EQ directly on BindingPointDataInfo seems to cause compiler
    // to hang. EXPECT_EQ(
...
@compute @workgroup_size(1)
fn main() {
  var len : u32 = f(tex, tint_symbol_2.texture_builtin_values[0u][0u]);
}
)";
...
@compute @workgroup_size(1)
fn main() {
  var len : u32 = f(tex1, tex2, tex3, tint_symbol_4.texture_builtin_values[0u][0u], tint_symbol_4.texture_builtin_values[0u][1u], tint_symbol_4.texture_builtin_values[0u][2u]);
}
)";
    ...
    EXPECT_EQ(std::make_pair(TextureBuiltinsFromUniformOptions::DataType::TextureNumLevels, 0u),
              val->bindpoint_to_data.at(BindingPoint{0, 0}));
    EXPECT_EQ(std::make_pair(TextureBuiltinsFromUniformOptions::DataType::TextureNumLevels, 4u),
...
@compute @workgroup_size(1)
fn main() {
  var len : u32 = f1(tex, tint_symbol_3.texture_builtin_values[0u][0u]);
}
)";
...
fn f_nested(t1 : texture_2d<f32>, t2 : texture_2d<f32>, tint_symbol : u32, tint_symbol_2 : u32) -> u32 {
  return (tint_symbol + tint_symbol_2);
}

fn f1(a : u32, t : texture_2d<f32>, tint_symbol_1 : u32) -> u32 {
  return ((a + f_nested(t, tex1, tint_symbol_1, tint_symbol_4.texture_builtin_values[0u][1u])) + tint_symbol_4.texture_builtin_values[0u][3u]);
}

@compute @workgroup_size(1)
fn main() {
  _ = textureNumLayers(tex4);
  _ = f1(9u, tex0, tint_symbol_4.texture_builtin_values[0u][2u]);
  _ = f_nested(tex2, tex2, tint_symbol_4.texture_builtin_values[0u][0u], tint_symbol_4.texture_builtin_values[0u][0u]);
  _ = f_nested(tex1, tex0, tint_symbol_4.texture_builtin_values[0u][1u], tint_symbol_4.texture_builtin_values[0u][2u]);
}
)";
