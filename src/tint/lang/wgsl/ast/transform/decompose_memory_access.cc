...
struct Offset : tint::Castable<Offset> {
...
/// value.
struct OffsetLiteral final : tint::Castable<OffsetLiteral, Offset> {
...
struct LoadStoreKey {
    ...
    struct Hasher {
        inline std::size_t operator()(const LoadStoreKey& u) const {
            return tint::Hash(u.el_ty, u.buffer);
        }
    };
};
...
struct AtomicKey {
    ...
    struct Hasher {
        inline std::size_t operator()(const AtomicKey& u) const {
            return tint::Hash(u.el_ty, u.op, u.buffer);
        }
    };
};
...
struct DecomposeMemoryAccess::State {
    ...
    tint::BlockAllocator<Offset> offsets_;
