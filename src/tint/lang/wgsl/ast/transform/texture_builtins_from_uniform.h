...
class TextureBuiltinsFromUniform final
  ...
  public:
    ...
    struct Config final : public utils::Castable<Config, Data> {
        ...
        /// The mapping from binding point to the index for the texture builtins lookup.
        /// texture binding point to index in ubo array
        /// ?? Probably this isn't needed
        std::unordered_map<sem::BindingPoint, uint32_t> bindpoint_to_index;

        // array bindpoint to index of array length of dynamic buffer
        // texture bindpoint to index of texture num levels/samples of texture

        // std::vector<writer::TextureBuiltinsFromUniformOptions::DataType> used_data_types;
    ...
    struct Result final : public utils::Castable<Result, Data> {
        ...
        // TODO
        std::unordered_map<writer::TextureBuiltinsFromUniformOptions::DataType, uint32_t> offsets;
    };
