...
struct Hasher<AccessRoot> {
    ...
    size_t operator()(const AccessRoot& d) const { return tint::Hash(d.type, d.variable); }
};
...
struct Hasher<DynamicIndex> {
    ...
    size_t operator()(const DynamicIndex& d) const { return tint::Hash(d.slot); }
};
...
struct Hasher<AccessShape> {
    ...
    size_t operator()(const AccessShape& s) const { return tint::Hash(s.root, s.ops); }
};
...
struct DirectVariableAccess::State {
  ...
  private:
    ...
    struct FnVariant {
        ...
        using Signature = tint::Hashmap<const sem::Parameter*, AccessShape, 4>;
        ...
        tint::Hashmap<const sem::Call*, Symbol, 4> calls;

        /// A map of input program parameter to output parameter symbols.
        tint::Hashmap<const sem::Parameter*, PtrParamSymbols, 4> ptr_param_symbols;
    ...
    struct FnInfo {
        ...
        tint::Hashmap<FnVariant::Signature, FnVariant, 8> variants;
        /// A map of expressions that have been hoisted to a 'let' declaration in the function.
        tint::Hashmap<const sem::ValueExpression*, Symbol, 8> hoisted_exprs;
    ...
    tint::Hashmap<const sem::Function*, FnInfo*, 8> fns;
    ...
    /// pointer parameter.
    tint::Hashmap<AccessShape, Symbol, 8> dynamic_index_array_aliases;
    /// Map of semantic expression to AccessChain
    tint::Hashmap<const sem::ValueExpression*, AccessChain*, 32> access_chains;
    /// Allocator for FnInfo
    tint::BlockAllocator<FnInfo> fn_info_allocator;
    /// Allocator for AccessChain
    tint::BlockAllocator<AccessChain> access_chain_allocator;
    ...
    tint::Hashmap<std::string, Symbol, 8> unique_symbols;
