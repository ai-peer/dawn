...
struct TextureBuiltinsFromUniform::State {
    ...
    ApplyResult Run() {
        ...
        // std::unordered_map<const sem::Function*, std::vector<const ast::Parameter*>>
        // new_fn_to_data;

        struct CallArgInfo {
            std::optional<BindingPoint> binding;
            // const ast::Parameter* fn_param = nullptr;
            size_t extra_pass_in_arg_id;
        };
                ...
                if (global_var) {
                    ...
                    for (size_t i = 0; i < parent_fn_sem->Declaration()->params.Length(); i++) {
                        ...
                        if (p->As<Variable>() == var->Declaration()) {
                            ...
                            // ?? After function is cloned, is this var still foundable?
                    ...
                    for (auto* parent_fn_call : parent_fn_sem->CallSites()) {
                        ...
                        if (t_global_var) {
                            ...
                            it->second.push_back(CallArgInfo{std::nullopt,
                                                             // new_param
                                                             extra_param_id});
        ...
        if (!fn_to_data.empty()) {
            for (const auto& pair : fn_to_data) {
                ...
                // }
                auto* fn = pair.first;
                for (auto new_param : pair.second) {
            ...
            for (const auto& pair : call_to_data) {
                ...
                for (const auto& new_arg_info : pair.second) {
                    ...
                    if (new_arg_info.binding.has_value()) {
                        ...
                        // auto* fn = call->Target()->As<sem::Function>()->Declaration();
                        // printf("\n s.extra_pass_in_arg_id %lu, params length %lu, fn_to_data
                        // extra length %lu\n\n\n",
                        //        new_arg_info.extra_pass_in_arg_id,
                        //        fn->params.Length(),
                        //        fn_to_data[call->Target()->As<sem::Function>()].size()
                        //        );
                        //    call->Stmt()->Function()->Declaration()->params.Length());
                        // auto* param =
                        //     fn->params[new_arg_info.extra_pass_in_arg_id];
                        // ctx.InsertAfter(call->Declaration()->args,
                        // call->Declaration()->args.Back(), b.Expr(param->As<Variable>()));

                        // Unresolved tint symbol?
                        // auto* param =
                        // fn_to_data[call->Target()->As<sem::Function>()][new_arg_info.extra_pass_in_arg_id
                        // - fn->params.Length()]; ctx.InsertAfter(call->Declaration()->args,
                        // call->Declaration()->args.Back(), b.Expr(param->As<Variable>()));
                        ...
                        printf(
                            "\n s.extra_pass_in_arg_id %lu, params length %lu, fn_to_data extra "
                            "length %lu\n\n\n",
                            new_arg_info.extra_pass_in_arg_id,
                            parent_fn_sem->Declaration()->params.Length(),
                            fn_to_data[parent_fn_sem].size());
                        auto* param = fn_to_data[parent_fn_sem]
                                                [new_arg_info.extra_pass_in_arg_id -
                                                 parent_fn_sem->Declaration()->params.Length()];
                        ctx.InsertAfter(call->Declaration()->args, call->Declaration()->args.Back(),
            ...
            //     }
            //     for (const auto& new_p : iter->second) {
            //         params.Push(new_p);
            //     }
            ...
            //     auto* new_fn = ctx.dst->create<Function>(name, params, return_type, body,
            //                                              std::move(attributes),
            //                                              std::move(return_type_attributes));

            //     // new_fn_to_data.insert({new_fn, iter->seoncd})

            //     // old_fn_to_new_fn.emplace(ast_fn, new_fn);
            //     // ctx.Replace(ast_fn, new_fn);

            //     // Find all old fn callsites
            //     // Replace those callsites with new function as call target
            //     for (auto* call : fn->CallSites()) {
            //         auto it = call_to_data.find(call);
            //         TINT_ASSERT(Transform, it != call_to_data.end());
            ...
            //         // Replace old function callsites with new function and extra args passed in.
            ...
            //                 // TODO: de-dup
            //                 uint32_t index = bindpoint_to_data.at(*s.binding).second /
            //                 sizeof(uint32_t);
            //                 // auto it = bindpoint_to_data.find(*s.binding);
            //                 // if (it == bindpoint_to_data.end()) {
            //                 //     TINT_UNREACHABLE(Transform, b.Diagnostics())
            ...
            //                 args.Push(builtin_value);
            //             } else {
            //                 printf("\n s.extra_pass_in_arg_id %lu, params length %lu \n\n\n",
            //                    s.extra_pass_in_arg_id,
            //                    new_fn->params.Length());
            //                 //    call->Stmt()->Function()->Declaration()->params.Length());

            //                 auto* param =
            //                     new_fn->params[s.extra_pass_in_arg_id];
            //                 // auto* param =
            //                 //
            //                 call->Stmt()->Function()->Declaration()->params[s.extra_pass_in_arg_id];
            //                 // auto* param = call->Stmt()->Function()->Declaration()->params[0];
            //                 args.Push(b.Expr(param->As<Variable>()));
            //             }
            //         }

            //         // replace call target
            //         // ? how to get a IdentifierExpression from function
            //         ctx.Replace(call->Declaration(), ctx.dst->Call(
            //             // new_fn,
            //             // b.Expr(new_fn->name),
            //             b.Expr(new_fn->name->symbol),
            //             // b.Expr(ctx.Clone(new_fn)->name),
            //             // b.Expr(new_fn)->As<IdentifierExpression>(),
            //             std::move(args)));

            //     }

            //     // ctx.Replace(ast_fn, ctx.Clone(new_fn));
            //     ctx.Replace(ast_fn, new_fn);
            // }

            // // // //
            // // // std::unordered_map<const ast::Function*, const ast::Function*>
            // old_fn_to_new_fn;

            // // Replace function signatures with new params for the passed in texture builtin
            // values ctx.ReplaceAll([&](const Function* ast_fn) -> const Function* {
            //     auto* fn = sem.Get(ast_fn);
            //     if (fn == nullptr) {
            //         return nullptr;
            //     }
            ...
            //     // printf("\n !!!!old fn %s %lu, new fn: %s %lu\n\n\n",
            //     //            fn->Declaration()->name->symbol.Name().c_str(),
            //     //            fn->Parameters().Length(),
            //     //            new_fn->name->symbol.Name().c_str(),
            //     //            new_fn->params.Length()
            //     //            );
            ...
            //     //     printf("\n !!!!call func: %s  %lu, %lu\n\n\n",
            //     //
            //     call->Target()->As<sem::Function>()->Declaration()->name->symbol.Name().c_str(),
            //     //            call->Arguments().Length(),
            //     //            call->Target()->As<sem::Function>()->Parameters().Length());
            ...
            //     //     // Replace old function callsites with new function and extra args passed
            //     in.
            //     //     for (const auto& s : it->second) {
            //     //         if (s.binding.has_value()) {
            //     //             // This texture is a global variable with binding point.
            //     //             // Read builtin value from uniform buffer.
            //     //             printf("\n    args push global var\n");
            ...
            //     call->Stmt()->Function()->Declaration()->params[s.extra_pass_in_arg_id];
            //     //             // auto* param =
            //     call->Stmt()->Function()->Declaration()->params[0];
            //     //             args.Push(b.Expr(param->As<Variable>()));
            //     //         }
            //     //     }
            ...
            // // Replace all callsites of recorded function with new
            ...
            //     // Review: Probably still the old calls
            //     auto iter = call_to_data.find(call);
            //     if (iter == call_to_data.end()) {
            //         printf("\n\nxxx\n");
            //         return nullptr;
            //     }
            //     printf("\n\nyyy\n");
            ...
            //         // args.Push(b.Expr(u32( e.has_value() ? 1 : 0 )));
            ...
            //             args.Push(builtin_value);
            //         } else {
            //             // This texture variable is from a function parameter
            //             // Pass in the extra args for builtin values, with the same idx
            ...
            //             // auto* old_fn = call->Stmt()->Function()->Declaration();
            //             // // auto* new_fn = old_fn_to_new_fn[old_fn];
            //             // auto it = old_fn_to_new_fn.find(old_fn);
            //             // if (it == old_fn_to_new_fn.end()) {
            //             //     printf("\n\n????????????\n");
            //             //     continue;
            //             // }
            //             // auto* new_fn = it->second;

            //             // printf("\n s.extra_pass_in_arg_id %lu, params length %lu\n\n\n",
            //             // s.extra_pass_in_arg_id,
            //             //    new_fn->params.Length());
            //             // auto* param = new_fn->params[s.extra_pass_in_arg_id];
            ...
            //             // args.Push(b.Expr(s.fn_param->As<Variable>()));
            //             // args.Push(b.Expr(u32(9)));
            //         }
            //     }
