...
struct TextureBuiltinsFromUniform::State {
    ...
    ApplyResult Run() {
        ...
        std::unordered_map<const sem::Function*, std::vector<const ast::Parameter*>> new_fn_to_data;
                ...
                if (global_var) {
                    ...
                    for (auto* parent_fn_call : parent_fn_sem->CallSites()) {
                        ...
                        if (t_global_var) {
                            ...
                            it->second.push_back(
                                CallArgInfo{std::nullopt,
                                            // new_param
                                            iter->second.size() + parent_fn_params_length});
                        }
                    }
        ...
        if (!fn_to_data.empty()) {
            ...
            //     // }
            //     for (const auto& new_p : pair.second) {
            //         pair.first->AddParameter(new_p);
            //     }
            // }

            // // Temp test
            // std::unordered_map<const ast::Function*, const ast::Function*> old_fn_to_new_fn;

            // Replace function signatures with new params for the passed in texture builtin values
