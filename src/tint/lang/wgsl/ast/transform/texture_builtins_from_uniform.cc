...
struct TextureBuiltinsFromUniform::State {
  ...
  private:
    ...
    /// Get or create a ubo including u32 array for texture builtin values.
    ...
    Symbol GetUboSym() {
        ...
        auto* new_member = b.Member(
            ...
            b.ty.array(b.ty.vec4(b.ty.u32()), core::u32((bindpoint_to_data.size() + 3) / 4)));
        ...
        auto* buffer_struct = b.Structure(b.Sym(), tint::Vector{new_member});
    ...
    const ast::Expression* GetUniformValue(const BindingPoint& binding) {
        ...
        auto* vec_expr = b.IndexAccessor(b.MemberAccessor(ubo_sym, kTextureBuiltinValuesMemberName),
                                         core::u32(array_index));
        uint32_t vec_index = index % 4;

        auto* builtin_value = b.IndexAccessor(vec_expr, core::u32(vec_index));
