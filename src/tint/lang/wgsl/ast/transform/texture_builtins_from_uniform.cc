...
struct TextureBuiltinsFromUniform::State {
    ...
    ApplyResult Run() {
        ...
        struct FunctionExtraParamInfo {
            ...
            // // id of the texture param that this extra param relates to
            // // e.g. f(t0, foo, t1, e0, e1)
            // // e0 and e1 are extra params, their tex_idx are 0 (points to t0) and 2 (points to
            // t1). size_t tex_idx = 0; id of this extra param e.g. f(t0, foo, t1, e0, e1) e0 and e1
            // are extra params, their extra_idx are 0 and 1. This is to help sort extra ids in the
            // correct order.
            size_t extra_idx = 0;
        };
        ...
        Hashmap<const CallExpression*, Vector<std::variant<BindingPoint, const ast::Parameter*>, 4>,
                8>
            call_to_data;
                    ...
                    tint::Switch(
                        ...
                        [&](const sem::Builtin* builtin) {
                            ...
                            switch (builtin->Type()) {
                                ...
                                case core::Function::kTextureNumSamples:
                                    dataType =
                                        TextureBuiltinsFromUniformOptions::Field::TextureNumSamples;
                                    break;
                                default:
                                    TINT_UNREACHABLE()
                                        << "unsupported builtin function type " << builtin->Type();
                            }

                            auto* global_var = tint::As<sem::GlobalVariable>(texture_sem);
                            if (global_var) {
                                // This texture variable is a global variable.
                                auto binding = global_var->BindingPoint().value();
                                auto iter = bindpoint_to_data.find(binding);
                                if (iter == bindpoint_to_data.end()) {
                                    // First visit. Add this global texture variable
                                    uint32_t index =
                                        static_cast<uint32_t>(bindpoint_to_data.size());
                                    bindpoint_to_data.emplace(
                                        binding,
                                        Result::DataEntry{dataType, index * static_cast<uint32_t>(
                                                                                sizeof(uint32_t))});
                                }
                                ...
                                auto& param_to_info = fn_to_data.GetOrCreate(fn, [&] {
                                    return Hashmap<const ast::Parameter*, FunctionExtraParamInfo,
                                                   4>();
                                });
                                ...
                                for (auto p : fn->Declaration()->params) {
                                    if (p->As<Variable>() == var->Declaration()) {
                                        // Record a new u32 param to this function.
                                        const ast::Parameter* new_param =
                                            b.Param(b.Sym(), b.ty.u32());
                                        param_to_info.Add(
                                            p, FunctionExtraParamInfo{dataType, new_param,
                                                                      param_to_info.Count()});
                        ...
                        [&](const sem::Function* user_fn) {
                            ...
                            if (!user_param_to_info) {
                                // Uninterested function not calling texture builtins.
                                return;
                            }
                            for (size_t i = 0; i < call->Declaration()->args.Length(); i++) {
                                ...
                                if (auto info = (*user_param_to_info).Get(param)) {
                                    ...
                                    auto* global_var = texture_sem->As<sem::GlobalVariable>();
                                    if (global_var) {
                                        // This is a global texture variable.
                                        // Record replacing with builtin value from uniform.
                                        auto binding = global_var->BindingPoint().value();
                                        auto iter = bindpoint_to_data.find(binding);
                                        if (iter == bindpoint_to_data.end()) {
                                            ...
                                            bindpoint_to_data.emplace(
                                                binding,
                                                Result::DataEntry{info->field,
                                                                  index * static_cast<uint32_t>(
                                                                              sizeof(uint32_t))});
                                        }
                                        args.Push(binding);
                                    } else {
                                        // This is a texture param needs pass in extra u32 param
                                        auto* var = tint::As<sem::Variable>(texture_sem);
                                        auto& param_to_info = fn_to_data.GetOrCreate(fn, [&] {
                                            return Hashmap<const ast::Parameter*,
                                                           FunctionExtraParamInfo, 4>();
                                        });

                                        // Find which function param is the current one
                                        // size_t texture_param_id = 0;
                                        ...
                                        for (auto p : fn->Declaration()->params) {
                                            if (p->As<Variable>() == var->Declaration()) {
                                                ...
                                                const ast::Parameter* new_param =
                                                    b.Param(b.Sym(), b.ty.u32());
                                                param_to_info.Add(p, FunctionExtraParamInfo{
                                                                         info->field, new_param,
                                                                         param_to_info.Count()});

                                                // Record replacing with function parameter
                                                args.Push(new_param);
                                                break;
                                            }
                                        }
    ...
    Symbol GetUboSym() {
        if (!ubo) {
            ...
            auto* new_member = b.Member(
                ...
                b.ty.array(b.ty.vec4(b.ty.u32()), u32((bindpoint_to_data.size() + 3) / 4)));
