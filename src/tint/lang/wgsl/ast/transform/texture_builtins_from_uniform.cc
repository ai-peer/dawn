...
struct TextureBuiltinsFromUniform::State {
    ...
    ApplyResult Run() {
        ...
        // std::unordered_map<const sem::Call*, std::vector<BindingPoint>> call_to_data;
        // std::unordered_map<const sem::Call*, std::vector<CallArgInfo>> call_to_data;
        std::unordered_map<const sem::Call*, std::vector<std::optional<BindingPoint>>> call_to_data;
                ...
                if (global_var) {
                    ...
                    for (size_t i = 0; i < parent_fn_sem->Declaration()->params.Length(); i++) {
                        ...
                        if (p->As<Variable>() == var->Declaration()) {
                            ...
                            // step.
                            auto* new_param = b.Param(b.Sym(), b.ty.u32());
                    ...
                    for (auto* parent_fn_call : parent_fn_sem->CallSites()) {
                        ...
                        if (t_global_var) {
                            it->second.push_back(t_global_var->BindingPoint().value());
                        } else {
                            // temp
            ...
            ctx.ReplaceAll([&](const CallExpression* expr) -> const Expression* {
                ...
                for (const auto& e : iter->second) {
                    ...
                    if (e.has_value()) {
                        ...
                        // TODO: de-dup
                        auto it = bindpoint_to_data.find(*e);
                        if (it == bindpoint_to_data.end()) {
                            TINT_UNREACHABLE(Transform, b.Diagnostics())
                                << "binding type unfound " << *e;
                        }
