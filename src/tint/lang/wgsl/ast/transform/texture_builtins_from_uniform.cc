...
struct TextureBuiltinsFromUniform::State {
    ...
    ApplyResult Run() {
        ...
        // // TODO: need bindingpoint -> data type, a uint32_t is not enough
        // uint32_t builtin_values_count = 0;

        // temp
        IterateInterestedTextureBuiltinsCall([&](const CallExpression*,
        ...
        // }
        IterateInterestedTextureBuiltinsCall([&](const CallExpression* call_expr,
    ...
    void IterateInterestedTextureBuiltinsCall(F&& functor) {
        ...
        for (auto* node : src->ASTNodes().Objects()) {
            ...
            if (global_var) {
                ...
                // Otherwise could be a function parameter. needs to modify function to pass in the
                // builtin values
                // 1. Add new
