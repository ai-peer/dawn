...
struct TextureBuiltinsFromUniform::State {
  ...
  private:
    ...
    Symbol GetUboSym() {
        if (!ubo) {
            ...
            for (auto it : bindpoint_to_data) {
                ...
                TINT_ASSERT(i < builtin_value_syms.Length());
            ...
            ubo = b.GlobalVar(b.Sym(), b.ty.Of(buffer_size_struct), core::AddressSpace::kUniform,
                              b.Group(tint::AInt(cfg->ubo_binding.group)),
                              b.Binding(tint::AInt(cfg->ubo_binding.binding)));
        }
    ...
    const ast::Expression* GetUniformValue(const BindingPoint& binding) {
        ...
        auto* builtin_value = b.MemberAccessor(ubo_sym, builtin_value_syms[index]);
