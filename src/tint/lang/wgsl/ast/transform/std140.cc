  ...
  private:
    ...
    struct LoadFnKey {
        ...
        struct Hasher {
            ...
            size_t operator()(const LoadFnKey& fn) const { return tint::Hash(fn.var, fn.indices); }
        };
    ...
    tint::Hashmap<LoadFnKey, Symbol, 8, LoadFnKey::Hasher> load_fns;

    /// Map of std140-forked type to converter function name
    tint::Hashmap<const type::Type*, Symbol, 8> conv_fns;

    // Uniform variables that have been modified to use a std140 type
    tint::Hashset<const sem::Variable*, 8> std140_uniforms;

    // Map of original structure to 'std140' forked structure
    tint::Hashmap<const type::Struct*, Symbol, 8> std140_structs;

    // Map of structure member in src of a matrix type, to list of decomposed column
    // members in ctx.dst.
    tint::Hashmap<const type::StructMember*, tint::Vector<const StructMember*, 4>, 8>
        std140_mat_members;
    ...
    tint::Hashmap<const type::Matrix*, Std140Matrix, 8> std140_mats;
    ...
    std::string PrefixForUniqueNames(const Struct* str, Symbol unsuffixed, uint32_t count) const {
        ...
        while (true) {
            ...
            tint::Hashset<std::string, 4> strings;
