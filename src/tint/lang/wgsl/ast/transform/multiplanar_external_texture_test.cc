...
fn textureSampleExternal(plane0 : texture_2d<f32>, plane1 : texture_2d<f32>, smp : sampler, coord : vec2<f32>, params : ExternalTextureParams) -> vec4<f32> {
  ...
  if ((params.doYuvToRgbConversionOnly == 0)) {
    color = vec4<f32>(gammaCorrection(color.rgb, params.gammaDecodeParams), 1);
    color = vec4<f32>((params.gamutConversionMatrix * color.rgb), 1);
    color = vec4<f32>(gammaCorrection(color.rgb, params.gammaEncodeParams), 1);
  }
  return color;
}
