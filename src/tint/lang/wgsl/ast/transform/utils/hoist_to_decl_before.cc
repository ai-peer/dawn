...
struct HoistToDeclBefore::State {
  ...
  private:
    ...
    struct LoopInfo {
        tint::Vector<StmtBuilder, 8> init_decls;
        tint::Vector<StmtBuilder, 8> cond_decls;
        tint::Vector<StmtBuilder, 8> cont_decls;
    };
    ...
    tint::Hashmap<const sem::ForLoopStatement*, LoopInfo, 4> for_loops;

    /// Whiles that need to be decomposed to loops.
    tint::Hashmap<const sem::WhileStatement*, LoopInfo, 4> while_loops;

    /// 'else if' statements that need to be decomposed to 'else {if}'
    tint::Hashmap<const IfStatement*, ElseIfInfo, 4> else_ifs;

    template <size_t N>
    static auto Build(const tint::Vector<StmtBuilder, N>& builders) {
        ...
        ctx.ReplaceAll([&](const WhileStatement* stmt) -> const Statement* {
            ...
            if (auto* w = sem.Get(stmt)) {
                if (auto info = while_loops.Find(w)) {
                    ...
                    // expression.
                    auto body_stmts = tint::Transform(info->cond_decls,
