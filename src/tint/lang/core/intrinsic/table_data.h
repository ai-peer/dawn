...
struct TableIndex {
    ...
    auto operator+(U offset) const {
        ...
        return TableIndex<KIND, decltype(new_value)>(new_value);
    }

    /// @returns true if the datatype `T` has enough bits to index all the
    /// elements in the array @p arr.
    template <typename U, size_t N>
    static constexpr bool CanIndex(U (&arr)[N]) {
...
using TemplateTypeIndex = TableIndex<TableIndexKind::kTemplateType, uint8_t>;

/// Index type used to index TableData::template_numbers
using TemplateNumberIndex = TableIndex<TableIndexKind::kTemplateNumber, uint8_t>;

/// Index type used to index TableData::type_matchers
using TypeMatcherIndex = TableIndex<TableIndexKind::kTypeMatcher, uint8_t>;

/// Index type used to index TableData::number_matchers
using NumberMatcherIndex = TableIndex<TableIndexKind::kNumberMatcher, uint8_t>;

/// Index type used to index TableData::type_matcher_indices
using TypeMatcherIndicesIndex = TableIndex<TableIndexKind::kTypeMatcherIndices, uint8_t>;

/// Index type used to index TableData::number_matcher_indices
using NumberMatcherIndicesIndex = TableIndex<TableIndexKind::kNumberMatcherIndices, uint8_t>;

/// Index type used to index TableData::parameters
using ParameterIndex = TableIndex<TableIndexKind::kParameter, uint16_t>;
...
struct TableData {
    template <typename T>
    const TemplateTypeInfo& operator[](TableIndex<TableIndexKind::kTemplateType, T> idx) const {
        return template_types[idx.value];
    }

    template <typename T>
    const TemplateNumberInfo& operator[](TableIndex<TableIndexKind::kTemplateNumber, T> idx) const {
        return template_numbers[idx.value];
    }

    template <typename T>
    const TypeMatcherIndex* operator[](
        TableIndex<TableIndexKind::kTypeMatcherIndices, T> idx) const {
        return idx.IsValid() ? &type_matcher_indices[idx.value] : nullptr;
    }

    template <typename T>
    const NumberMatcherIndex* operator[](
        TableIndex<TableIndexKind::kNumberMatcherIndices, T> idx) const {
        return idx.IsValid() ? &number_matcher_indices[idx.value] : nullptr;
    }

    template <typename T>
    const TypeMatcher& operator[](TableIndex<TableIndexKind::kTypeMatcher, T> idx) const {
        return type_matchers[idx.value];
    }

    template <typename T>
    const NumberMatcher& operator[](TableIndex<TableIndexKind::kNumberMatcher, T> idx) const {
        return number_matchers[idx.value];
    }

    template <typename T>
    const ParameterInfo& operator[](TableIndex<TableIndexKind::kParameter, T> idx) const {
