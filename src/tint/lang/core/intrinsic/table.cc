...
const type::Type* TableData::TemplateState::Type(size_t idx, const type::Type* ty) {
...
bool TableData::TemplateState::Num(size_t idx, TableData::Number number) {
...
const type::Type* TableData::TemplateState::Type(size_t idx) const {
    if (idx >= types_.Length()) {
        return nullptr;
    }
    return types_[idx];
...
void TableData::TemplateState::SetType(size_t idx, const type::Type* ty) {
    if (idx >= types_.Length()) {
        types_.Resize(idx + 1);
    }
    types_[idx] = ty;
...
TableData::Number TableData::TemplateState::Num(size_t idx) const {
    if (idx >= numbers_.Length()) {
...
size_t TableData::TemplateState::Count() const {
    return types_.Length() + numbers_.Length();
}
