...
class Manager final {
  public:
    ...
    using TypeIterator = tint::BlockAllocator<Type>::ConstIterator;
    ...
    type::Struct* Struct(Symbol name, tint::VectorRef<StructMemberDesc> members);
  ...
  private:
    ...
    tint::UniqueAllocator<Type> types_;
    /// Unique nodes (excluding types) owned by the manager
    tint::UniqueAllocator<UniqueNode> unique_nodes_;
    /// Non-unique nodes owned by the manager
    tint::BlockAllocator<Node> nodes_;
};
