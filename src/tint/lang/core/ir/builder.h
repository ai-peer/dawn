...
class Builder {
    ...
    /// A helper used to enable overloads if the first type in `TYPES` is a tint::Vector or
    /// tint::VectorRef.
  ...
  public:
    ...
    ir::Block* Case(ir::Switch* s, tint::VectorRef<Switch::CaseSelector> selectors);
    ...
    auto Values(ARGS&&... args) {
        ...
        return tint::Vector{Value(std::forward<ARGS>(args))...};
    }
    ...
    ir::Swizzle* Swizzle(const type::Type* type, OBJ&& object, tint::VectorRef<uint32_t> indices) {
        auto* obj_val = Value(std::forward<OBJ>(object));
