...
TEST_F(IR_ValidatorTest, CallToFunctionOutsideModule) {
    ...
    auto res = ir::Validate(mod);
    ASSERT_FALSE(res);
    EXPECT_EQ(res.Failure().str(), R"(:3:20 error: call: call target is not part of the module
    %2:void = call %g
                   ^^
...
TEST_F(IR_ValidatorTest, Instruction_DeadOperand) {
    ...
    auto res = ir::Validate(mod);
    ASSERT_FALSE(res);
    EXPECT_EQ(res.Failure().str(), R"(:3:46 error: var: instruction operand 0 is not alive
    %2:ptr<function, f32, read_write> = var, %3
                                             ^^
...
TEST_F(IR_ValidatorTest, Instruction_OperandUsageRemoved) {
    ...
    auto res = ir::Validate(mod);
    ASSERT_FALSE(res);
    EXPECT_EQ(res.Failure().str(), R"(:3:46 error: var: instruction operand 0 missing usage
    %2:ptr<function, f32, read_write> = var, %3
                                             ^^
