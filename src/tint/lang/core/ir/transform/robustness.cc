...
struct State {
                ...
                tint::Switch(
                    ...
                    [&](ir::LoadVectorElement* lve) {
                        b.InsertBefore(lve, [&] {  //
                            ...
                            auto* vec = ptr->StoreType()->As<type::Vector>();
    ...
    void ClampAccessIndices(ir::Access* access) {
        auto* base_ptr = access->Object()->Type()->As<type::Pointer>();

        // Check if accesses into this object should be clamped.
        if (base_ptr) {
            if (!ShouldClamp(base_ptr->AddressSpace())) {
                return;
            }
        } else {
            ...
            auto limit = tint::Switch(
                ...
                [&](const type::Array* arr) -> ir::Value* {
                    ...
                    if (i > 0) {
                        ...
                        object = b.Access(ty.ptr(base_ptr->AddressSpace(), arr, base_ptr->Access()),
                                          object, indices[0])
                                     ->Result();
                    }
