...
struct State {
    ...
    const type::Struct* gamma_transfer_params_struct = nullptr;

    /// The external texture parameters structure.
    const type::Struct* external_texture_params_struct = nullptr;
    ...
    void Process() {
        ...
        if (ir->root_block) {
            ...
            for (auto inst : *ir->root_block) {
                ...
                if (ptr->StoreType()->Is<type::ExternalTexture>()) {
                    ReplaceVar(var);
                    to_remove.Push(var);
                }
            }
        ...
        for (auto* func : ir->functions) {
            for (uint32_t index = 0; index < func->Params().Length(); index++) {
                ...
                if (param->Type()->Is<type::ExternalTexture>()) {
    ...
    const type::SampledTexture* SampledTexture() {
        return ty.Get<type::SampledTexture>(type::TextureDimension::k2d, ty.f32());
    }
            ...
            tint::Switch(
                ...
                [&](CoreBuiltinCall* call) {
                    if (call->Func() == core::Function::kTextureDimensions) {
                        ...
                        auto* helper =
                            b.Call(ty.vec4<f32>(), TextureLoad(), plane_0, plane_1, params, coords);
                        ...
                        auto* helper = b.Call(ty.vec4<f32>(), TextureSample(), plane_0, plane_1,
    ...
    const type::Struct* GammaTransferParams() {
    ...
    const type::Struct* ExternalTextureParams() {
