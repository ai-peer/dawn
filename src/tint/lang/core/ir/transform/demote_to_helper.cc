...
struct DemoteToHelper::State {
    ...
    tint::Hashmap<Function*, bool, 4> function_discard_status;

    /// Set of functions that have been processed.
    tint::Hashset<Function*, 4> processed_functions;
    ...
    void Process() {
        ...
        tint::Vector<Function*, 4> to_process;
    ...
    void ProcessBlock(Block* block) {
        ...
        auto conditionalize = [&](Instruction* inst) {
            ...
            if (inst->HasResults() && !inst->Result()->Type()->Is<type::Void>()) {
                ...
                ifelse->SetResults(tint::Vector{b.InstructionResult(inst->Result()->Type())});
