...
struct State {
    ...
    ir::Value* ftoi(ir::Convert* convert) {
        ...
        auto* helper = helpers.GetOrCreate(res_ty, [&] {
            ...
            StringStream name;
            name << "tint_" << src_el_ty->FriendlyName() << "_to_";
            ...
            if (src_el_ty->Is<type::F32>()) {
                if (res_ty->is_signed_integer_scalar_or_vector()) {
                    ...
                    limits.high_limit_f = MatchWidth(b.Constant(f32(INT32_MAX - 127)), res_ty);
                } else {
                    limits.low_limit_f = MatchWidth(b.Constant(f32(0)), res_ty);
                    limits.high_limit_f = MatchWidth(b.Constant(f32(UINT32_MAX - 255)), res_ty);
                }
