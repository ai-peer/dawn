...
struct BuiltinPolyfillSpirv::State {
    ...
    void Process() {
        ...
        tint::Vector<CoreBuiltinCall*, 4> worklist;
    ...
    Value* ArrayLength(CoreBuiltinCall* builtin) {
        ...
        auto* call = b.Call(
            ...
            tint::Vector{access->Object(), Literal(u32(const_idx->Value()->ValueAs<uint32_t>()))});
        ...
        switch (builtin->Func()) {
            ...
            case builtin::Function::kAtomicCompareExchangeWeak: {
                ...
                call = b.Construct(type::CreateAtomicCompareExchangeResult(ty, ir->symbols, int_ty),
                                   tint::Vector{original, compare->Result()});
                break;
            }
            case builtin::Function::kAtomicExchange:
    ...
    Value* Dot(CoreBuiltinCall* builtin) {
        ...
        auto args = tint::Vector<Value*, 4>(builtin->Args());
        auto* call =
    ...
    Value* AppendArrayIndex(Value* coords, Value* array_idx, Instruction* insertion_point) {
        ...
        auto* construct = b.Construct(coord_ty, tint::Vector{coords, array_idx});
    ...
    Value* TextureSample(CoreBuiltinCall* builtin) {
        ...
        tint::Vector<Value*, 8> intrinsic_args;
    ...
    Value* TextureNumLayers(CoreBuiltinCall* builtin) {
        ...
        tint::Vector<Value*, 2> intrinsic_args;
        intrinsic_args.Push(texture);
...
BuiltinPolyfillSpirv::SampledImage::SampledImage(const type::Type* image)
    : Base(static_cast<size_t>(
               tint::Hash(tint::TypeInfo::Of<BuiltinPolyfillSpirv::SampledImage>().full_hashcode,
                           image)),
           type::Flags{}),
      image_(image) {}
