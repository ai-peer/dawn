...
class DirectVariableAccessTest : public TransformTestBase<testing::Test> {
  public:
    std::string Run(std::string in) {
        ...
        auto res = DirectVariableAccess(&module.Get(), DirectVariableAccessOptions{});
        if (!res) {
...
using IR_DirectVariableAccessTest_RemoveUncalled = DirectVariableAccessTest;
...
var<private> keep_me = 42;
    ...
    auto got = Run(src, EnablePrivate());
...
var<private> keep_me = 42;
    ...
    auto got = Run(src, EnableFunction());
...
using IR_DirectVariableAccessTest_PtrChains = DirectVariableAccessTest;
...
fn d() {
  c(&U);
    ...
    auto* expect = R"(@group(0) @binding(0) var<uniform> U : array<array<array<vec4<i32>, 8u>, 8u>, 8u>;

fn a(pre : i32, v : array<u32, 3u>, post : i32) -> vec4<i32> {
  return U[v[0u]][v[1u]][v[2u]];
}
...
#if 0
TEST_F(IR_DirectVariableAccessTest_PtrChains, HoistIndices) {
    auto* src = R"(
enable chromium_experimental_full_ptr_parameters;
...
TEST_F(IR_DirectVariableAccessTest_PtrChains, HoistInForLoopInit) {
    auto* src = R"(
enable chromium_experimental_full_ptr_parameters;
...
TEST_F(IR_DirectVariableAccessTest_PtrChains, HoistInForLoopCond) {
    auto* src = R"(
enable chromium_experimental_full_ptr_parameters;
...
TEST_F(IR_DirectVariableAccessTest_PtrChains, HoistInForLoopCont) {
    auto* src = R"(
enable chromium_experimental_full_ptr_parameters;
...
TEST_F(IR_DirectVariableAccessTest_PtrChains, HoistInWhileCond) {
    auto* src = R"(
enable chromium_experimental_full_ptr_parameters;
...
using IR_DirectVariableAccessTest_UniformAS = DirectVariableAccessTest;
...
using IR_DirectVariableAccessTest_StorageAS = DirectVariableAccessTest;
...
using IR_DirectVariableAccessTest_WorkgroupAS = DirectVariableAccessTest;
...
using IR_DirectVariableAccessTest_PrivateAS = DirectVariableAccessTest;
...
using IR_DirectVariableAccessTest_FunctionAS = DirectVariableAccessTest;
...
using IR_DirectVariableAccessTest_Complex = DirectVariableAccessTest;
