...
TEST_F(IR_Bgra8UnormPolyfillTest, TextureLoad) {
    ...
    b.Append(func->Block(), [&] {
        auto* load = b.Load(var->Result());
        auto* result = b.Call(ty.vec4<f32>(), core::Function::kTextureLoad, load);
...
TEST_F(IR_Bgra8UnormPolyfillTest, TextureLoadAndStore) {
    ...
    b.Append(func->Block(), [&] {
        auto* load = b.Load(var->Result());
        auto* result = b.Call(ty.vec2<u32>(), core::Function::kTextureLoad, load);
...
%foo = func(%coords:vec2<u32>):void -> %b2 {
  %b2 = block {
    ...
    %result:vec2<u32> = textureLoad %4
    %6:vec2<u32> = swizzle %result, zyxw
    %7:vec2<u32> = swizzle %6, zyxw
