...
struct State {
    ...
    utils::Hashmap<const type::Type*, const type::Type*, 4> rewritten_types{};

    /// Map from struct member to its new index.
    utils::Hashmap<const type::StructMember*, uint32_t, 4> member_index_map{};

    /// Map from a type to a helper function that will convert its rewritten form back to it.
    utils::Hashmap<const type::Struct*, Function*, 4> convert_helpers{};
        ...
        return rewritten_types.GetOrCreate(type, [&]() -> const type::Type* {
            return tint::Switch(
                ...
                [&](const type::Struct* str) -> const type::Type* {
                    ...
                    utils::Vector<const type::StructMember*, 4> new_members;
    ...
    Value* LoadMatrix(const type::Matrix* mat, Value* root, utils::Vector<Value*, 4> indices) {
        ...
        return tint::Switch(
            ...
            [&](const type::Array* arr) -> Value* {
                ...
                b.Append(loop->Continuing(), [&] {
                    // Increment idx.
                    auto* inc = b.Add(ty.i32(), idx, b.Constant(1_i));
        ...
        b.InsertBefore(inst, [&] {
            tint::Switch(
                ...
                [&](LoadVectorElement* load) {
                    ...
                    TINT_ASSERT(Transform, !replacement->Type()->Is<type::Pointer>());
