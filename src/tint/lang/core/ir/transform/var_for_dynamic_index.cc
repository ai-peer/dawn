...
struct PartialAccess {
    ...
    tint::Vector<Value*, 4> indices;

    // A specialization of tint::Hasher for PartialAccess.
    struct Hasher {
        inline std::size_t operator()(const PartialAccess& src) const {
            return tint::Hash(src.base, src.indices);
        }
    };
...
void VarForDynamicIndex::Run(ir::Module* ir) const {
    ...
    tint::Vector<AccessToReplace, 4> worklist;
    ...
    tint::Hashmap<Value*, Value*, 4> object_to_local;
    tint::Hashmap<PartialAccess, Value*, 4, PartialAccess::Hasher> source_object_to_value;
    for (const auto& to_replace : worklist) {
        ...
        tint::Vector<Value*, 4> indices{access->Indices().Offset(to_replace.first_dynamic_index)};
