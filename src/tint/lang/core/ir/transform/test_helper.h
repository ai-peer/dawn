...
using TransformFunc = std::function<utils::Result<utils::SuccessType, diag::List>(Module*)>;
...
class TransformTestBase : public BASE {
  public:
    ...
    void Run(std::function<utils::Result<utils::SuccessType, diag::List>(Module*)> transform_func) {
        // Run the transform.
        auto result = transform_func(&mod);
        EXPECT_TRUE(result) << result.Failure().str();
        if (!result) {
            return;
        }
