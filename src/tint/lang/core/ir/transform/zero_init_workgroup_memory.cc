...
struct State {
    ...
    using StoreList = Vector<Store, 4>;

    /// StoreMap is a map from iteration count to a list of `Store` descriptors.
    using StoreMap = std::map<uint32_t, StoreList>;
                ...
                if (entry.first == 1u) {
                    ...
                    b.Append(ifelse->True(), [&] {
                        for (auto& store : entry.second) {
                            GenerateStore(store, entry.first, b.Constant(0_u));
                        }
                        b.ExitIf(ifelse);
                    });
                } else {
                    // Use a loop for arrayed stores.
                    GenerateZeroingLoop(local_index, entry.first, wgsize, entry.second);
                }
            }
