...
struct State {
    ...
    void ProcessEntryPoint(Function* func) {
        ...
        auto build = [&](const Store& store, uint32_t total_count, Value* linear_index) {
            ...
            if (!store.indices.IsEmpty()) {
                ...
                for (auto idx = store.indices.rbegin(); idx != store.indices.rend(); idx++) {
                ...
                if (entry.first == 1u) {
                    ...
                    b.Append(ifelse->True(), [&] {
                        for (auto& store : entry.second) {
                            build(store, entry.first, b.Constant(0_u));
                        }
                        b.ExitIf(ifelse);
                    });
                } else {
                    ...
                    b.Append(loop->Body(), [&] {
                        auto* gt_max = b.GreaterThan(ty.bool_(), index, u32(entry.first - 1u));
                        auto* ifelse = b.If(gt_max);
                        b.Append(ifelse->True(), [&] {  //
                            b.ExitLoop(loop);
                        });
                        for (auto& store : entry.second) {
                            build(store, entry.first, index);
                        }
                        b.Continue(loop);
                    });
