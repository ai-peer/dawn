...
class Swizzle : public tint::Castable<Swizzle, OperandInstruction<1, 1>> {
  public:
    ...
    Swizzle(InstructionResult* result, Value* object, tint::VectorRef<uint32_t> indices);
    ~Swizzle() override;
    ...
    tint::VectorRef<uint32_t> Indices() { return indices_; }

    /// @returns the friendly name for the instruction
    std::string_view FriendlyName() override { return "swizzle"; }

  private:
    tint::Vector<uint32_t, 4> indices_;
};
