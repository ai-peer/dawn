...
/// ControlInstruction.
class ControlInstruction : public tint::Castable<ControlInstruction, OperandInstruction<1, 1>> {
  public:
    /// Constructor
    ControlInstruction();
    ...
    void SetResults(tint::VectorRef<InstructionResult*> values) {
    ...
    void SetResults(ARGS&&... values) {
        SetResults(tint::Vector{std::forward<ARGS>(values)...});
    }

    /// @return All the exits for the flow control instruction
    const tint::Hashset<Exit*, 2>& Exits() const { return exits_; }
