...
utils::Result<utils::SuccessType, diag::List> ValidateAndDumpIfNeeded(
    [[maybe_unused]] Module& ir,
    ...
    if (!result) {
        ...
        ss << "validating input to " << msg << " failed";
        errors.add_error(diag::System::Transform, ss.str());
        errors.add(std::move(result.Failure()));
        return errors;
    }
#endif
