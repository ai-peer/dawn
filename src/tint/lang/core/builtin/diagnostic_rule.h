...
/// @param CoreDiagnosticRule the enum value
...
auto& operator<<(STREAM& o, CoreDiagnosticRule value) {
...
/// @param ChromiumDiagnosticRule the enum value
/// @returns the string for the given enum value
...
/// @returns `out` so calls can be chained
template <typename STREAM, typename = utils::traits::EnableIfIsOStream<STREAM>>
auto& operator<<(STREAM& o, ChromiumDiagnosticRule value) {
    return o << ToString(value);
}
