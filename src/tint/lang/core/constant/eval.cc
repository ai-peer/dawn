...
TransformElements(Manager& mgr,
    ...
    for (uint32_t i = 0; i < n; i++) {
        if (auto el = TransformElements(mgr, composite_el_ty, std::forward<F>(f), index + i,
                                        cs->Index(i)...)) {
            els.Push(el.Get());
...
Eval::Result TransformUnaryElements(Manager& mgr,
    ...
    els.Reserve(n);
    for (uint32_t i = 0; i < n; i++) {
        if (auto el = TransformUnaryElements(mgr, composite_el_ty, std::forward<UnaryTransform>(f),
                                             c0->Index(i))) {
            els.Push(el.Get());
...
Eval::Result TransformBinaryElements(Manager& mgr,
    ...
    for (uint32_t i = 0; i < n; i++) {
        if (auto el =
                TransformBinaryElements(mgr, composite_el_ty, std::forward<BinaryTransform>(f),
                                        c0->Index(i), c1->Index(i))) {
            els.Push(el.Get());
...
Eval::Result TransformTernaryElements(Manager& mgr,
    ...
    for (uint32_t i = 0; i < n; i++) {
        if (auto el =
                TransformTernaryElements(mgr, composite_el_ty, std::forward<TernaryTransform>(f),
                                         c0->Index(i), c1->Index(i), c2->Index(i))) {
            els.Push(el.Get());
