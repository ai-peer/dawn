...
class Manager final {
  public:
    ...
    using TypeIterator = tint::BlockAllocator<Value>::ConstIterator;
    ...
    const constant::Value* Composite(const type::Type* type,
                                     tint::VectorRef<const constant::Value*> elements);
  ...
  private:
    /// A specialization of tint::Hasher for constant::Value
    struct Hasher {
    ...
    tint::UniqueAllocator<Value, Hasher, Equal> values_;
};
