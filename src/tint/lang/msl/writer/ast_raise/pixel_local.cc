        ...
        for (auto* global : src->AST().GlobalVariables()) {
            ...
            for (auto* ep : entry_points) {
                ...
                Vector<const ast::Statement*, 3> body{
                    b.Assign(ctx.Clone(var->name), pl_param),
                };
                ...
                ast::Type ret_ty;
                if (fn->return_type) {
                    // Create a structure to hold the result of the entry point and the pixel local
                    // structure.
                    auto str_name = b.Symbols().New(fn_name + "_res");
                    b.Structure(str_name, Vector{
                                              b.Member("output", ctx.Clone(fn->return_type)),
                                              b.Member("pixel_local", b.ty(pixel_local_str_name)),
                                          });
                    auto result = b.Symbols().New("result");
                    body.Push(b.Decl(b.Let(result, b.Call(inner_name, std::move(call_args)))));
                    body.Push(b.Return(b.Call(str_name, "result", ctx.Clone(var->name))));
                    ret_ty = b.ty(str_name);
                } else {
                    body.Push(b.CallStmt(b.Call(inner_name, std::move(call_args))));
                    body.Push(b.Return(ctx.Clone(var->name)));
                    ret_ty = b.ty(pixel_local_str_name);
                }
