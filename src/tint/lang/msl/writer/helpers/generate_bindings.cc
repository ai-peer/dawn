    ...
    for (auto* var : program.AST().GlobalVariables()) {
        if (auto* sem_var = program.Sem().Get(var)->As<sem::GlobalVariable>()) {
            if (auto bp = sem_var->BindingPoint()) {
                ...
                auto* type = sem_var->Type()->UnwrapRef();

                // Store up the external textures, we'll add them in the next step
                if (type->Is<core::type::ExternalTexture>()) {
                    ext_tex_bps.emplace_back(*bp);
                    continue;
                }

                binding::ResourceType resource_type;
                Switch(
                    type,  //
                    [&](const core::type::Texture*) {
                        resource_type = binding::ResourceType::kTexture;
                    },
                    [&](const core::type::Sampler*) {
                        resource_type = binding::ResourceType::kSampler;
                    },
                    [&](const core::type::Pointer* ptr) {
                        switch (ptr->AddressSpace()) {
                            case core::AddressSpace::kWorkgroup: {
                                resource_type = binding::ResourceType::kThreadGroup;
                                break;
                            }
                            case core::AddressSpace::kStorage:
                            case core::AddressSpace::kUniform: {
                                resource_type = binding::ResourceType::kBuffer;
                                break;
                            }
                            default: {
                                TINT_UNREACHABLE() << "invalid pointer address space";
                            }
                        }
                    },
                    [&](Default) { TINT_UNREACHABLE(); });

                binding::BindingInfo info{resource_type, bp->binding};
