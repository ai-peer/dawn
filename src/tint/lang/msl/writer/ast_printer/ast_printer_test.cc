...
TEST_F(MslASTPrinterTest, HasInvariantAttribute_True) {
    auto* out = Structure("Out", tint::Vector{
    ...
    Func("vert_main", tint::Empty, ty.Of(out), tint::Vector{Return(Call(ty.Of(out)))},
         tint::Vector{
...
TEST_F(MslASTPrinterTest, WorkgroupMatrix) {
    ...
    Func("comp_main", tint::Empty, ty.void_(), tint::Vector{Decl(Let("x", Expr("m")))},
         tint::Vector{
             Stage(ast::PipelineStage::kCompute),
             WorkgroupSize(1_i),
         });
...
TEST_F(MslASTPrinterTest, WorkgroupMatrixInStruct) {
    ...
    Structure("S2", tint::Vector{
                        Member("s", ty("S1")),
                    });
    GlobalVar("s", ty("S2"), builtin::AddressSpace::kWorkgroup);
    Func("comp_main", tint::Empty, ty.void_(), tint::Vector{Decl(Let("x", Expr("s")))},
         tint::Vector{
             Stage(ast::PipelineStage::kCompute),
             WorkgroupSize(1_i),
         });
