...
class TypeManager {
  public:
    ...
    /// the same pointer.
    const Pointer* Pointer(builtin::AddressSpace address_space,
                           const Type* ty,
    ...
    const Reference* Reference(builtin::AddressSpace address_space,
                               const Type* ty,
                               builtin::Access access = builtin::Access::kUndefined);
    /// @param ty the element type
    /// @param sz the number of elements in the vector
    ...
    /// the same pointer.
    const Vector* Vector(const Type* ty, uint32_t sz);
    /// @param ty the matrix element type
    /// @param c the number of columns in the matrix
    /// @param r the number of rows in the matrix
    /// @return a Matrix type. Repeated calls with the same arguments will return
    /// the same pointer.
    const Matrix* Matrix(const Type* ty, uint32_t c, uint32_t r);
    /// @param el the element type
    /// @param sz the number of elements in the array. 0 represents runtime-sized
    /// array.
