...
using namespace tint::core::number_suffixes;  // NOLINT
                                              //
using IR_SpirvIntrinsicCallTest = core::ir::IRTestHelper;

TEST_F(IR_SpirvIntrinsicCallTest, Clone) {
    auto* builtin = b.Call<IntrinsicCall>(mod.Types().f32(), Intrinsic::kImageWrite, 1_u, 2_u);

    auto* new_b = clone_ctx.Clone(builtin);
    EXPECT_NE(builtin->Result(), new_b->Result());
    EXPECT_EQ(mod.Types().f32(), new_b->Result()->Type());

    EXPECT_EQ(Intrinsic::kImageWrite, new_b->Kind());
...
TEST_F(IR_SpirvIntrinsicCallTest, CloneNoArgs) {
    auto* builtin = b.Call<IntrinsicCall>(mod.Types().f32(), Intrinsic::kImageWrite);
