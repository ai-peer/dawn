...
bool ValidateBindingOptions(const Options& options) {
    tint::Hashset<tint::BindingPoint, 8> seen_wgsl_bindings{};
    tint::Hashset<binding::BindingInfo, 8> seen_spirv_bindings{};

    auto wgsl_seen = [&seen_wgsl_bindings](const tint::BindingPoint& info) -> bool {
    ...
    auto spirv_seen = [&seen_spirv_bindings](const binding::BindingInfo& info) -> bool {
