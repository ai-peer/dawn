...
class Printer {
  ...
  private:
    ...
    struct FunctionType {
        ...
        tint::Vector<uint32_t, 4> param_type_ids;
        ...
        struct Hasher {
            ...
            inline std::size_t operator()(const FunctionType& ft) const {
                size_t hash = tint::Hash(ft.return_type_id);
                for (auto& p : ft.param_type_ids) {
                    hash = tint::HashCombine(hash, p);
                }
                return hash;
            }
        };
    ...
    tint::Hashmap<const type::Type*, uint32_t, 8> types_;

    /// The map of function types to their result IDs.
    tint::Hashmap<FunctionType, uint32_t, 8, FunctionType::Hasher> function_types_;

    /// The map of constants to their result IDs.
    tint::Hashmap<const constant::Value*, uint32_t, 16> constants_;

    /// The map of types to the result IDs of their OpConstantNull instructions.
    tint::Hashmap<const type::Type*, uint32_t, 4> constant_nulls_;

    /// The map of types to the result IDs of their OpUndef instructions.
    tint::Hashmap<const type::Type*, uint32_t, 4> undef_values_;

    /// The map of non-constant values to their result IDs.
    tint::Hashmap<ir::Value*, uint32_t, 8> values_;

    /// The map of blocks to the IDs of their label instructions.
    tint::Hashmap<ir::Block*, uint32_t, 8> block_labels_;

    /// The map of extended instruction set names to their result IDs.
    tint::Hashmap<std::string_view, uint32_t, 2> imports_;
