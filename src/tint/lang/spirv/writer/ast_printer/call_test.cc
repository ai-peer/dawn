...
TEST_F(SpirvASTPrinterTest, Expression_Call) {
    auto* a_func = Func("a_func",
                        ...
                        ty.f32(), tint::Vector{Return(Add("a", "b"))});
    auto* func = Func("main", tint::Empty, ty.void_(),
                      tint::Vector{Assign(Phony(), Call("a_func", 1_f, 1_f))});
...
TEST_F(SpirvASTPrinterTest, Statement_Call) {
    ...
    auto* func =
        Func("main", tint::Empty, ty.void_(), tint::Vector{CallStmt(Call("a_func", 1_f, 1_f))});
