...
TEST_P(Attribute_StageTest, Emit) {
    ...
    ast::Type ret_type;
    tint::Vector<const ast::Attribute*, 2> ret_type_attrs;
    tint::Vector<const ast::Statement*, 2> body;
    ...
    tint::Vector<const ast::Attribute*, 2> deco_list{Stage(params.stage)};
...
TEST_F(SpirvASTPrinterTest, Decoration_ExecutionMode_WorkgroupSize_Default) {
    auto* func = Func("main", tint::Empty, ty.void_(), tint::Empty,
                      tint::Vector{Stage(ast::PipelineStage::kCompute), WorkgroupSize(1_i)});
