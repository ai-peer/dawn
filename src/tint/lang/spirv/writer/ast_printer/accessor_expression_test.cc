...
TEST_F(SpirvASTPrinterTest, MemberAccessor_Nested) {
    ...
    // var ident : my_struct
    // ident.inner.a
    auto* inner_struct = Structure("Inner", tint::Vector{
                                                Member("a", ty.f32()),
                                                Member("b", ty.f32()),
                                            });

    auto* s_type = Structure("my_struct", tint::Vector{Member("inner", ty.Of(inner_struct))});
...
TEST_F(SpirvASTPrinterTest, MemberAccessor_Nested_WithAlias) {
    ...
    auto* s_type = Structure("Outer", tint::Vector{Member("inner", ty.Of(alias))});
...
TEST_F(SpirvASTPrinterTest, IndexAccessor_Mixed_ArrayAndMember) {
    ...
    auto* c_type = Structure("C", tint::Vector{Member("baz", ty.vec3<f32>())});

    auto* b_type = Structure("B", tint::Vector{Member("bar", ty.Of(c_type))});
    auto b_ary_type = ty.array(ty.Of(b_type), 3_u);
    auto* a_type = Structure("A", tint::Vector{Member("foo", b_ary_type)});
