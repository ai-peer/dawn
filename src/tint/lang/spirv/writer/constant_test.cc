...
TEST_F(SpirvWriterTest, Constant_Mat4x2h) {
    ...
    auto* f16 = ty.f16();
    auto* v = mod.constant_values.Composite(
        ty.mat4x2(f16), tint::Vector{
                            mod.constant_values.Composite(
                                ty.vec2(f16), tint::Vector{const_f16(42), const_f16(-1)}),
                            mod.constant_values.Composite(
                                ty.vec2(f16), tint::Vector{const_f16(0), const_f16(0.25)}),
                            mod.constant_values.Composite(
                                ty.vec2(f16), tint::Vector{const_f16(-42), const_f16(1)}),
                            mod.constant_values.Composite(
                                ty.vec2(f16), tint::Vector{const_f16(0.5), const_f16(-0)}),
                        });
    writer_.Constant(b.Constant(v));
    ASSERT_TRUE(Generate()) << Error() << output_;
    EXPECT_INST(R"(
