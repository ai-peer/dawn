...
struct TextureBuiltinTestCase {
    ...
    tint::Vector<NameAndType, 4> args;
    /// The result type.
    NameAndType result;
    /// The expected SPIR-V instruction strings.
    tint::Vector<const char*, 2> instructions;
};

inline tint::StringStream& operator<<(tint::StringStream& out, TextureType type) {
    switch (type) {
...
class TextureBuiltinTest : public SpirvWriterTestWithParam<TextureBuiltinTestCase> {
  protected:
    ...
    void Run(enum builtin::Function function, SamplerUsage sampler) {
        ...
        tint::Vector<ir::FunctionParam*, 4> func_params;
