...
TEST_F(SpirvWriterTest, TextureSampleBaseClampToEdge_2d_f32) {
    ...
    b.Append(func->Block(), [&] {
        auto* result =
            b.Call(ty.vec4<f32>(), builtin::Function::kTextureSampleBaseClampToEdge, args);
        b.Return(func, result);
        mod.SetName(result, "result");
    });
...
TEST_F(SpirvWriterTest, Bgra8Unorm_textureStore) {
    ...
    auto* texture_ty =
        ty.Get<type::StorageTexture>(type::TextureDimension::k2d, format, builtin::Access::kWrite,
                                     type::StorageTexture::SubtypeFor(format, ty));
