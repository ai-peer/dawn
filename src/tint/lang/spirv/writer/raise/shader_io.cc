...
struct StateImpl : ir::transform::ShaderIOBackendState {
    ...
    /// Option to clamp frag_depth builtin outputs.
    bool clamp_frag_depth;
    /// The frag_depth clamp arguments.
    ir::Value* frag_depth_clamp_args = nullptr;

    /// Constructor
    StateImpl(ir::Module* mod, ir::Function* f, bool clamp_depth)
        : ShaderIOBackendState(mod, f), clamp_frag_depth(clamp_depth) {}
...
Result<SuccessType, std::string> ShaderIO(ir::Module* ir, bool clamp_frag_depth) {
    ...
    ir::transform::RunShaderIOBase(ir, [&](ir::Module* mod, ir::Function* func) {
        return std::make_unique<StateImpl>(mod, func, clamp_frag_depth);
    });
