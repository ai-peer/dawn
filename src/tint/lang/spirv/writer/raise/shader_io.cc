...
#include "src/tint/lang/spirv/ir/io_var.h"
...
struct StateImpl : core::ir::transform::ShaderIOBackendState {
    /// The input variables.
    Vector<spirv::ir::IOVar*, 4> input_vars;
    /// The output variables.
    Vector<spirv::ir::IOVar*, 4> output_vars;
    ...
    void MakeVars(Vector<spirv::ir::IOVar*, 4>& vars,
        ...
        for (auto io : entries) {
            ...
            auto* var = ir->instructions.Create<spirv::ir::IOVar>(b.InstructionResult(ptr),
                                                                  spirv::ir::IOVarAttributes{
            ...
            ir->SetName(var->Result(), name.str());
