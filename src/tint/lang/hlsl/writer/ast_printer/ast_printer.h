...
class ASTPrinter : public tint::TextGenerator {
  ...
  private:
    ...
    /// @return true if ty is a struct with a matrix member (recursively), false otherwise.
    /// @param ty the type that will be queried.
    bool IsStructWithMatrixMember(const core::type::Type* ty);
    ...
    std::unordered_map<const core::type::Struct*, bool> is_struct_with_matrix_member_;
};
