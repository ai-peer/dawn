...
class GlslASTPrinterTest_MemberAccessorBase : public BASE {
  public:
    void SetupStorageBuffer(tint::VectorRef<const ast::StructMember*> members) {
    ...
    void SetupFunction(tint::VectorRef<const ast::Statement*> statements) {
        ProgramBuilder& b = *this;
        b.Func("main", tint::Empty, b.ty.void_(), statements,
               tint::Vector<const ast::Attribute*, 1>{
...
TEST_F(GlslASTPrinterTest_MemberAccessor, EmitExpression_MemberAccessor) {
    auto* s = Structure("Data", tint::Vector{Member("mem", ty.f32())});
...
TEST_P(GlslASTPrinterTest_MemberAccessor_StorageBufferLoad, Test) {
    ...
    SetupStorageBuffer(tint::Vector{
        Member("a", ty.i32()),
        Member("b", p.member_type(ty)),
    });

    SetupFunction(tint::Vector{
