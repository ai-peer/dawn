...
TEST_F(GlslASTPrinterTest_Switch, Emit_Switch) {
    ...
    auto* def = create<ast::CaseStatement>(tint::Vector{DefaultCaseSelector()}, def_body);

    auto* case_body = Block(create<ast::BreakStatement>());
    auto* case_stmt = create<ast::CaseStatement>(tint::Vector{CaseSelector(5_i)}, case_body);

    auto* cond = Expr("cond");
    auto* s = Switch(cond, tint::Vector{case_stmt, def});
    WrapInFunction(s);
...
TEST_F(GlslASTPrinterTest_Switch, Emit_Switch_MixedDefault) {
    ...
    auto* def_body = Block(create<ast::BreakStatement>());
    auto* def = create<ast::CaseStatement>(tint::Vector{CaseSelector(5_i), DefaultCaseSelector()},
                                           def_body);
