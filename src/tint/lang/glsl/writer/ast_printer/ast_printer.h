...
class ASTPrinter : public tint::TextGenerator {
  public:
    ...
    void EmitIndexAccessor(tint::StringStream& out, const ast::IndexAccessorExpression* expr);
    ...
    void EmitBitwiseBoolOp(tint::StringStream& out, const ast::BinaryExpression* expr);
    /// Handles generating a binary expression
    /// @param out the output of the expression stream
    /// @param expr the binary expression
    void EmitFloatModulo(tint::StringStream& out, const ast::BinaryExpression* expr);
    /// Handles generating the modulo operator on float vector operands
    /// @param out the output of the expression stream
    /// @param expr the binary expression
    void EmitBinary(tint::StringStream& out, const ast::BinaryExpression* expr);
    /// Handles generating a bitcast expression
    /// @param out the output of the expression stream
    /// @param expr the expression
    void EmitVectorRelational(tint::StringStream& out, const ast::BinaryExpression* expr);
    /// Handles generating a vector relational expression
    /// @param out the output of the expression stream
    /// @param expr the expression
    void EmitBitcast(tint::StringStream& out, const ast::BitcastExpression* expr);
    /// Emits a list of statements
    /// @param stmts the statement list
    void EmitStatements(tint::VectorRef<const ast::Statement*> stmts);
    /// Emits a list of statements with an indentation
    /// @param stmts the statement list
    void EmitStatementsWithIndent(tint::VectorRef<const ast::Statement*> stmts);
    ...
    void EmitCall(tint::StringStream& out, const ast::CallExpression* expr);
    /// Handles generating a function call expression
    /// @param out the output of the expression stream
    /// @param call the call expression
    /// @param fn the function being called
    void EmitFunctionCall(tint::StringStream& out, const sem::Call* call, const sem::Function* fn);
    /// Handles generating a builtin call expression
    /// @param out the output of the expression stream
    /// @param call the call expression
    /// @param builtin the builtin being called
    void EmitBuiltinCall(tint::StringStream& out,
                         const sem::Call* call,
    ...
    void EmitValueConversion(tint::StringStream& out,
                             const sem::Call* call,
    ...
    void EmitValueConstructor(tint::StringStream& out,
                              const sem::Call* call,
                              const sem::ValueConstructor* ctor);
    /// Handles generating a barrier builtin call
    /// @param out the output of the expression stream
    /// @param builtin the semantic information for the barrier builtin
    void EmitBarrierCall(tint::StringStream& out, const sem::Builtin* builtin);
    ...
    void EmitWorkgroupAtomicCall(tint::StringStream& out,
                                 const ast::CallExpression* expr,
                                 const sem::Builtin* builtin);
    /// Handles generating an array.length() call
    /// @param out the output of the expression stream
    /// @param expr the call expression
    void EmitArrayLength(tint::StringStream& out, const ast::CallExpression* expr);
    /// Handles generating a call to `bitfieldExtract`
    /// @param out the output of the expression stream
    /// @param expr the call expression
    void EmitExtractBits(tint::StringStream& out, const ast::CallExpression* expr);
    /// Handles generating a call to `bitfieldInsert`
    /// @param out the output of the expression stream
    /// @param expr the call expression
    void EmitInsertBits(tint::StringStream& out, const ast::CallExpression* expr);
    /// Emulates 'fma' on GLSL ES, where it is unsupported.
    /// @param out the output of the expression stream
    /// @param expr the fma() expression
    void EmitEmulatedFMA(tint::StringStream& out, const ast::CallExpression* expr);
    ...
    void EmitTextureCall(tint::StringStream& out,
                         const sem::Call* call,
                         const sem::Builtin* builtin);
    /// Handles generating a call to the `select()` builtin
    /// @param out the output of the expression stream
    /// @param expr the call expression
    void EmitCountOneBitsCall(tint::StringStream& out, const ast::CallExpression* expr);
    /// Handles generating a call to the `countOneBits()` builtin
    /// @param out the output of the expression stream
    /// @param expr the call expression
    /// @param builtin the semantic information for the builtin
    void EmitSelectCall(tint::StringStream& out,
                        const ast::CallExpression* expr,
                        const sem::Builtin* builtin);
    ...
    /// @param expr the call expression
    /// @param builtin the semantic information for the builtin
    void EmitDotCall(tint::StringStream& out,
                     const ast::CallExpression* expr,
                     const sem::Builtin* builtin);
    ...
    /// @param expr the call expression
    /// @param builtin the semantic information for the builtin
    void EmitModfCall(tint::StringStream& out,
                      const ast::CallExpression* expr,
                      const sem::Builtin* builtin);
    ...
    /// @param expr the call expression
    /// @param builtin the semantic information for the builtin
    void EmitFrexpCall(tint::StringStream& out,
                       const ast::CallExpression* expr,
                       const sem::Builtin* builtin);
    ...
    /// @param expr the call expression
    /// @param builtin the semantic information for the builtin
    void EmitDegreesCall(tint::StringStream& out,
                         const ast::CallExpression* expr,
                         const sem::Builtin* builtin);
    ...
    /// @param expr the call expression
    /// @param builtin the semantic information for the builtin
    void EmitRadiansCall(tint::StringStream& out,
                         const ast::CallExpression* expr,
                         const sem::Builtin* builtin);
    ...
    /// @param expr the call expression
    /// @param builtin the semantic information for the builtin
    void EmitQuantizeToF16Call(tint::StringStream& out,
                               const ast::CallExpression* expr,
                               const sem::Builtin* builtin);
    ...
    /// @param out the output of the expression stream
    /// @param expr the expression
    void EmitExpression(tint::StringStream& out, const ast::Expression* expr);
    ...
    void EmitInterpolationQualifiers(tint::StringStream& out,
                                     tint::VectorRef<const ast::Attribute*> attrs);
    /// Handles emitting attributes
    /// @param out the output of the expression stream
    /// @param var the global variable semantics
    /// @param attrs the attributes
    void EmitAttributes(tint::StringStream& out,
    ...
    void EmitConstant(tint::StringStream& out, const constant::Value* constant);
    /// Handles a literal
    /// @param out the output stream
    /// @param lit the literal to emit
    void EmitLiteral(tint::StringStream& out, const ast::LiteralExpression* lit);
    ...
    void EmitIdentifier(tint::StringStream& out, const ast::IdentifierExpression* expr);
    /// Handles a member accessor expression
    /// @param out the output of the expression stream
    /// @param expr the member accessor expression
    void EmitMemberAccessor(tint::StringStream& out, const ast::MemberAccessorExpression* expr);
    ...
    void EmitType(tint::StringStream& out,
    ...
    void EmitTypeAndName(tint::StringStream& out,
                         const type::Type* type,
                         builtin::AddressSpace address_space,
                         builtin::Access access,
                         const std::string& name);
    ...
    /// @param out the output of the expression stream
    /// @param expr the expression to emit
    void EmitUnaryOp(tint::StringStream& out, const ast::UnaryOpExpression* expr);
    /// Emits the zero value for the given type
    /// @param out the output stream
    /// @param type the type to emit the value for
    void EmitZeroValue(tint::StringStream& out, const type::Type* type);
  ...
  private:
    ...
    template <typename F>
    void CallBuiltinHelper(tint::StringStream& out,
