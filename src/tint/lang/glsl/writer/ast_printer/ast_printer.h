...
struct SanitizedResult {
    ...
    /// True if the shader needs a UBO of buffer.
    /// It is now only used for texture builtin values, but can be applied to others in the future.
    bool needs_internal_uniform_buffer = false;

    /// Store a map of globl texture variable binding point to the byte offset and data type to push
