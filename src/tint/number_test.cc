...
TEST_P(CheckedAddTest_Float, Test) {
    ...
    std::visit(
        [&](auto&& lhs) {
            ...
            auto* rhs = std::get_if<T>(&std::get<2>(p));
            auto* expect = std::get_if<std::optional<T>>(&std::get<0>(p));
            ASSERT_NE(rhs, nullptr);
            ASSERT_NE(expect, nullptr);
            EXPECT_EQ(CheckedAdd(lhs, *rhs), *expect) << std::hex << "0x" << lhs << " + 0x" << *rhs;
            EXPECT_EQ(CheckedAdd(*rhs, lhs), *expect) << std::hex << "0x" << lhs << " + 0x" << *rhs;
        },
        std::get<1>(p));
}
template <typename T>
...
TEST_P(CheckedSubTest_Float, Test) {
    ...
    std::visit(
        [&](auto&& lhs) {
            ...
            ASSERT_NE(rhs, nullptr);
            ASSERT_NE(expect, nullptr);
            EXPECT_EQ(CheckedSub(lhs, *rhs), *expect) << std::hex << "0x" << lhs << " - 0x" << *rhs;
        },
        std::get<1>(p));
}
template <typename T>
...
TEST_P(CheckedMulTest_Float, Test) {
    ...
    std::visit(
        [&](auto&& lhs) {
            ...
            ASSERT_NE(rhs, nullptr);
            ASSERT_NE(expect, nullptr);
            EXPECT_EQ(CheckedMul(lhs, *rhs), *expect) << std::hex << "0x" << lhs << " * 0x" << *rhs;
            EXPECT_EQ(CheckedMul(*rhs, lhs), *expect) << std::hex << "0x" << lhs << " * 0x" << *rhs;
        },
        std::get<1>(p));
}
template <typename T>
...
TEST_P(CheckedDivTest_Float, Test) {
    ...
    std::visit(
        [&](auto&& lhs) {
            ...
            ASSERT_NE(rhs, nullptr);
            ASSERT_NE(expect, nullptr);
            EXPECT_EQ(CheckedDiv(lhs, *rhs), *expect) << std::hex << "0x" << lhs << " / 0x" << *rhs;
        },
        std::get<1>(p));
}
template <typename T>
