...
static constexpr bool IsCastable = ((traits::IsTypeOrDerived<TYPES, CastableBase> ||
    ...
    if constexpr (kNumCases == 0) {
        ...
        if (type->Is(&TypeInfo::Of<CaseType>())) {
            ...
            if constexpr (kHasReturnType) {
                result = new (result) RETURN_TYPE(std::get<0>(cases)(ptr));
            } else {
                std::get<0>(cases)(ptr);
            }
            return true;
        ...
        if (object) {
            ...
            if constexpr (kHasDefaultCase) {
                ...
                if (!detail::NonDefaultCases<T>(object, type, result,
                    ...
                    if constexpr (kHasReturnType) {
                        new (result) RETURN_TYPE(std::get<kDefaultIndex>(cases)({}));
                    } else {
...
inline auto Switch(T* object, CASES&&... cases) {
    ...
    if constexpr (kHasReturnType) {
        ...
        auto* res = utils::UnsafeReinterpretCast<ReturnType*>(&storage);
