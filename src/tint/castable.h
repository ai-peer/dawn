...
struct TypeInfo {
    ...
    /// @param type the object type to test from, which must be, or derive from type `FROM`.
    /// @see CastFlags
    ...
    static inline bool Is(const tint::TypeInfo* type) {
        ...
        return upcast || nocast || type->Is<TO>();
    }
    ...
    /// @returns true if the class with this TypeInfo is of, or derives from the class with the
    /// given TypeInfo.
