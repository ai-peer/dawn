...
class BuiltinStructsTest : public testing::TestWithParam<T> {
  protected:
    ...
    const type::Type* Type(ElementType t) {
        switch (t) {
            case kAFloat:
                return ty.AFloat();
            case kAInt:
...
class BuiltinFrexpResultStructTest : public BuiltinStructsTest<FrexpCase> {
  protected:
    void Run(const type::Type* in_type, const type::Type* fract_type, const type::Type* exp_type) {
        auto* result = CreateFrexpResult(ty, symbols, in_type);

        auto* str = As<type::Struct>(result);
...
TEST_P(BuiltinFrexpResultStructTest, Scalar) {
    ...
    Run(Type(params.in), Type(params.fract), Type(params.exp));
}
TEST_P(BuiltinFrexpResultStructTest, Vec2) {
    auto params = GetParam();
    Run(ty.vec2(Type(params.in)), ty.vec2(Type(params.fract)), ty.vec2(Type(params.exp)));
}
TEST_P(BuiltinFrexpResultStructTest, Vec3) {
    auto params = GetParam();
    Run(ty.vec3(Type(params.in)), ty.vec3(Type(params.fract)), ty.vec3(Type(params.exp)));
}
TEST_P(BuiltinFrexpResultStructTest, Vec4) {
    auto params = GetParam();
    Run(ty.vec4(Type(params.in)), ty.vec4(Type(params.fract)), ty.vec4(Type(params.exp)));
}
