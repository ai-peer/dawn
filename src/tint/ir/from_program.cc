...
class Impl {
  ...
  private:
    ...
    utils::Result<Value*> EmitShortCircuit(const ast::BinaryExpression* expr) {
        ...
        // If returns a tuple containing a single 'bool'
        if_inst->SetType(builder_.ir.Types().tuple<bool>());  // NOLINT: mistaken for std::tuple
        ...
        auto* access = builder_.Access(builder_.ir.Types().bool_(), if_inst,
                                       utils::Vector{builder_.Constant(0_u)});
