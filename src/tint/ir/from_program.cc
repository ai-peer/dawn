...
class Impl {
  ...
  private:
    ...
    utils::Result<Value*> EmitAccess(const ast::AccessorExpression* expr) {
        auto* expr_sem = program_->Sem().Get(expr);

        if (auto* load = expr_sem->As<sem::Load>()) {
            // Load instruction emission is handled by the caller
            return EmitAccess(load->Reference());
        }

        return EmitAccess(expr_sem);
    }
    ...
    utils::Result<Value*> EmitAccess(const sem::ValueExpression* expr) {
        ...
        auto end_chain = [&](ir::Value* root) -> Result {
            ...
            indices.Reverse();
            auto* access = builder_.Access(access_ty, root, std::move(indices));
            ...
            if (auto vec_access = AsVectorRefElementAccess(expr)) {
                ...
                res = tint::Switch(
                    ...
                    [&](const sem::IndexAccessorExpression* access) -> Result {
                        auto idx = EmitExpression(access->Index()->Declaration());
                        if (!idx) {
                            return kFailure;
                        }
                        indices.Push(idx.Get());
                        expr = access->Object();
                        return kNext;
                    },
                    [&](const sem::StructMemberAccess* access) -> Result {
                        indices.Push(builder_.Constant(u32(access->Member()->Index())));
                        expr = access->Object();
                        return kNext;
                    },
                    [&](const sem::Swizzle* access) -> Result {
                        ...
                        if (access->Indices().Length() == 1) {
                            indices.Push(builder_.Constant(u32(access->Indices()[0])));
                            expr = access->Object();
                            return kNext;
                        }
