...
TEST_F(IRBuilderImplTest, Loop_WithBreak) {
    ...
    EXPECT_TRUE(ir_loop->Is<ir::LoopFlowNode>());

    auto* flow = ir_loop->As<ir::LoopFlowNode>();
...
TEST_F(IRBuilderImplTest, Loop_WithContinue) {
    ...
    auto* loop_flow = ir_loop->As<ir::LoopFlowNode>();
    ...
    auto* ir_if = b.FlowNodeForAstNode(ast_if);
    ASSERT_NE(ir_if, nullptr);
    ASSERT_TRUE(ir_if->Is<ir::IfFlowNode>());

    auto* if_flow = ir_if->As<ir::IfFlowNode>();
...
TEST_F(IRBuilderImplTest, Loop_WithContinuing_BreakIf) {
    ...
    ASSERT_TRUE(ir_break_if->Is<ir::IfFlowNode>());

    auto* break_if_flow = ir_break_if->As<ir::IfFlowNode>();
...
TEST_F(IRBuilderImplTest, Loop_Nested) {
    ...
    EXPECT_TRUE(ir_loop_a->Is<ir::LoopFlowNode>());
    auto* loop_flow_a = ir_loop_a->As<ir::LoopFlowNode>();
    ...
    EXPECT_TRUE(ir_loop_b->Is<ir::LoopFlowNode>());
    auto* loop_flow_b = ir_loop_b->As<ir::LoopFlowNode>();
    ...
    EXPECT_TRUE(ir_loop_c->Is<ir::LoopFlowNode>());
    auto* loop_flow_c = ir_loop_c->As<ir::LoopFlowNode>();
    ...
    EXPECT_TRUE(ir_loop_d->Is<ir::LoopFlowNode>());
    auto* loop_flow_d = ir_loop_d->As<ir::LoopFlowNode>();
    ...
    EXPECT_TRUE(ir_if_a->Is<ir::IfFlowNode>());
    auto* if_flow_a = ir_if_a->As<ir::IfFlowNode>();
    ...
    EXPECT_TRUE(ir_if_b->Is<ir::IfFlowNode>());
    auto* if_flow_b = ir_if_b->As<ir::IfFlowNode>();
    ...
    EXPECT_TRUE(ir_if_c->Is<ir::IfFlowNode>());
    auto* if_flow_c = ir_if_c->As<ir::IfFlowNode>();
    ...
    EXPECT_TRUE(ir_if_d->Is<ir::IfFlowNode>());
    auto* if_flow_d = ir_if_d->As<ir::IfFlowNode>();
