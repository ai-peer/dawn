...
class Builder {
  ...
  public:
    ...
    ir::Binary* Binary(enum Binary::Kind kind, const type::Type* type, LHS&& lhs, RHS&& rhs) {
        return Append(ir.instructions.Create<ir::Binary>(RuntimeValue(type), kind,
    ...
    ir::Unary* Unary(enum Unary::Kind kind, const type::Type* type, VAL&& val) {
        return Append(ir.instructions.Create<ir::Unary>(RuntimeValue(type), kind,
    ...
    ir::Bitcast* Bitcast(const type::Type* type, VAL&& val) {
        return Append(
            ir.instructions.Create<ir::Bitcast>(RuntimeValue(type), Value(std::forward<VAL>(val))));
    }
    ...
    ir::UserCall* Call(const type::Type* type, ir::Function* func, ARGS&&... args) {
        return Append(ir.instructions.Create<ir::UserCall>(RuntimeValue(type), func,
    ...
    ir::BuiltinCall* Call(const type::Type* type, builtin::Function func, ARGS&&... args) {
        return Append(ir.instructions.Create<ir::BuiltinCall>(RuntimeValue(type), func,
                                                              Values(std::forward<ARGS>(args)...)));
    }
    ...
    ir::Convert* Convert(const type::Type* to, VAL&& val) {
        return Append(
            ir.instructions.Create<ir::Convert>(RuntimeValue(to), Value(std::forward<VAL>(val))));
    }
    ...
    ir::Construct* Construct(const type::Type* type, ARGS&&... args) {
        return Append(ir.instructions.Create<ir::Construct>(RuntimeValue(type),
                                                            Values(std::forward<ARGS>(args)...)));
    }
    ...
    ir::Load* Load(VAL&& from) {
        ...
        return Append(
            ir.instructions.Create<ir::Load>(RuntimeValue(val->Type()->UnwrapPtr()), val));
    }
    ...
    ir::Access* Access(const type::Type* type, OBJ&& object, ARGS&&... indices) {
        return Append(ir.instructions.Create<ir::Access>(RuntimeValue(type),
    ...
    ir::Swizzle* Swizzle(const type::Type* type, OBJ&& object, utils::VectorRef<uint32_t> indices) {
        return Append(ir.instructions.Create<ir::Swizzle>(
            RuntimeValue(type), Value(std::forward<OBJ>(object)), std::move(indices)));
    }
    ...
    ir::Swizzle* Swizzle(const type::Type* type,
        ...
        return Append(ir.instructions.Create<ir::Swizzle>(RuntimeValue(type),
                                                          Value(std::forward<OBJ>(object)),
    ...
    /// @param type the return type
    /// @returns the value
    ir::RuntimeValue* RuntimeValue(const type::Type* type) {
        return ir.values.Create<ir::RuntimeValue>(type);
    }
