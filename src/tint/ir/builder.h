...
class Builder {
  ...
  public:
    ...
    utils::Vector<ir::Value*, sizeof...(ARGS)> Values(ARGS&&... args) {
    ...
    /// @returns the instruction
    ir::Store* Store(ir::Value* to, ir::Value* from);
    ...
    /// @returns the instruction
    template <typename... ARGS>
    ir::BreakIf* BreakIf(ir::Value* condition, ir::Loop* loop, ARGS&&... args) {
        return ir.values.Create<ir::BreakIf>(condition, loop, Values(std::forward<ARGS>(args)...));
    }
    ...
    /// @returns the instruction
    template <typename... ARGS>
    ir::Swizzle* Swizzle(const type::Type* type, ir::Value* object, ARGS&&... indices) {
        return ir.values.Create<ir::Swizzle>(type, object, Values(std::forward<ARGS>(indices)...));
    }
