        ...
        if constexpr (is_null) {
            ...
            if constexpr (is_value) {
                ...
                if constexpr (!std::is_reference_v<T>) {
                    // ir::Instruction* was passed as a builder function argument without being an
                    // l-value or r-value reference. This can only occur when being returned from
                    // another function. C++ argument evaluation order is non-deterministic, but the
                    // order in which Value() is called is deterministic by the methods in this
                    // function. To prevent non-deterministic ordering of instructions created with
                    // calls like:
                    //   `b.Add(b.Call(ty.i32(), fn_a), b.Call(ty.i32(), fn_b))`
                    // We move these instructions to the back of the current block, ensuring
                    // deterministic ordering.
                    if (current_block_ && current_block_ == in->Block() &&
                        current_block_->Back() != in) {
                        in->Remove();
                        Append(in);
                    }
                }
