...
TINT_INSTANTIATE_TYPEINFO(tint::ir::transform::BlockDecoratedStructs::BlockStruct);
...
void BlockDecoratedStructs::Run(ir::Module* ir, const DataMap&, DataMap&) const {
    ...
    for (auto inst : *ir->root_block) {
        ...
        auto* block_struct = ir->Types().Get<BlockStruct>(
            /* name */ ir->symbols.New(),
            /* members */ members,
            /* align */ store_ty->Align(),
        ...
        auto* new_var = builder.Declare(
            ir->Types().Get<type::Pointer>(block_struct, ptr->AddressSpace(), ptr->Access()));
