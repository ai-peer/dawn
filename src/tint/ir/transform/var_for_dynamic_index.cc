...
void VarForDynamicIndex::Run(ir::Module* ir, const DataMap&, DataMap&) const {
    ...
    struct AccessToReplace {
        ...
        uint32_t first_dynamic_index;
        // The object type the corresponds to the source of the first dynamic index.
        const type::Type* dynamic_index_source_type;
    };
    ...
    for (auto* inst : ir->values.Objects()) {
        ...
        if (access->Type()->Is<type::Pointer>()) {
            // No need to modify accesses into pointers.
            continue;
        }
        ...
        // This access instruction will need replacing.
        worklist.Push(to_replace);
    }
    ...
    for (auto to_replace : worklist) {
        ...
        if (to_replace.first_dynamic_index > 0) {
            PartialAccess partial_access = {access->Object(), {}};
            for (uint32_t i = 0; i < to_replace.first_dynamic_index; i++) {
                partial_access.indices.Push(access->Indices()[i]);
            }
        ...
        utils::Vector<Value*, 4> indices;
        for (uint32_t i = to_replace.first_dynamic_index; i < access->Indices().Length(); i++) {
            indices.Push(access->Indices()[i]);
        }
        auto* new_access =
