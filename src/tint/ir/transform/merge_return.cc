...
struct MergeReturn::State {
    ...
    void ProcessBlock(Block* block) {
        // The stack of 'if(continue_execution)' instructions emitted after each control instruction
        // holding a return instruction. If non-empty, instructions will moved from 'block' to the
        // true-block of the 'if' at the back of this stack.
    ...
    void ProcessNestedReturn(Return* ret, If* cond) {
        ...
        auto exit_args = utils::Transform<8>(ctrl->Results(),  //
                                             [&](auto* v) { return b.Undef(v->Type()); });
    ...
    void SanitizeIfStack(utils::VectorRef<If*> if_stack) {
        ...
        // new_value_with_type returns a new ir::RuntimeValue with the same type as 'v'
        auto new_value_with_type = [&](ir::Value* v) { return b.RuntimeValue(v->Type()); };
        ...
        if (inner_if->True()->HasBranchTarget()) {
            ...
            if (exit_if && !exit_if->Args().IsEmpty()) {
                ...
                inner_if->SetResults(utils::Transform<8>(exit_if->Args(), new_value_with_type));
            }
        } else {
        ...
        for (auto* i : utils::Reverse(if_stack)) {
            ...
            utils::Vector<Value*, 8> exit_args = i->Results();
            if (!i->HasResults()) {
