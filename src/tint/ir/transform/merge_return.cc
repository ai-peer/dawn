...
struct MergeReturn::State {
    ...
    void Process(Function* func) {
        ...
        if (continue_execution->Usages().All(
                [](const Usage& u) { u.instruction->Is<ir::Store>(); })) {
            while (!continue_execution->Usages().IsEmpty()) {
                auto& usage = *continue_execution->Usages().begin();
                usage.instruction->Destroy();
            }
            continue_execution->Destroy();
        }
    }
