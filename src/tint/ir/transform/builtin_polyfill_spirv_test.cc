...
%foo = func(%t:texture_2d_array<f32>, %s:sampler, %coords:vec2<f32>, %array_idx:i32):vec4<f32> -> %b1 {
  %b1 = block {
    ...
    %7:f32 = access %coords, 0u
    %8:f32 = access %coords, 1u
    %9:f32 = convert %array_idx
    %10:vec3<f32> = construct %7, %8, %9
    %11:vec4<f32> = spirv.image_sample_implicit_lod %6, %10, 8u, vec2<i32>(1i)
    ret %11
  }
}
)";
...
%foo = func(%t:texture_2d_array<f32>, %s:sampler, %coords:vec2<f32>, %array_idx:i32, %bias:f32):vec4<f32> -> %b1 {
  %b1 = block {
    ...
    %8:f32 = access %coords, 0u
    %9:f32 = access %coords, 1u
    %10:f32 = convert %array_idx
    %11:vec3<f32> = construct %8, %9, %10
    %12:vec4<f32> = spirv.image_sample_implicit_lod %7, %11, 9u, %bias, vec2<i32>(1i)
    ret %12
  }
}
)";
...
TEST_F(IR_BuiltinPolyfillSpirvTest, TextureSampleCompare_2D_Offset) {
    ...
    b.With(func->Block(), [&] {
        auto* result = b.Call(
            ty.vec4<f32>(), builtin::Function::kTextureSampleCompare, t, s, coords, dref,
            b.Constant(mod.constant_values.Splat(ty.vec2<i32>(), mod.constant_values.Get(1_i), 2)));
        b.Return(func, result);
    });
...
%foo = func(%t:texture_depth_2d, %s:sampler, %coords:vec2<f32>, %dref:f32):f32 -> %b1 {
  %b1 = block {
    %6:vec4<f32> = textureSampleCompare %t, %s, %coords, %dref, vec2<i32>(1i)
    ret %6
  }
}
)";
...
TEST_F(IR_BuiltinPolyfillSpirvTest, TextureSampleCompare_2DArray_Offset) {
    ...
    b.With(func->Block(), [&] {
        auto* result = b.Call(
            ty.vec4<f32>(), builtin::Function::kTextureSampleCompare, t, s, coords, array_idx, bias,
            b.Constant(mod.constant_values.Splat(ty.vec2<i32>(), mod.constant_values.Get(1_i), 2)));
        b.Return(func, result);
    });
...
%foo = func(%t:texture_depth_2d_array, %s:sampler, %coords:vec2<f32>, %array_idx:i32, %bias:f32):f32 -> %b1 {
  %b1 = block {
    %7:vec4<f32> = textureSampleCompare %t, %s, %coords, %array_idx, %bias, vec2<i32>(1i)
    ret %7
  }
}
)";
...
%foo = func(%t:texture_depth_2d_array, %s:sampler, %coords:vec2<f32>, %array_idx:i32, %bias:f32):f32 -> %b1 {
  %b1 = block {
    ...
    %10:f32 = convert %array_idx
    %11:vec3<f32> = construct %8, %9, %10
    %12:f32 = spirv.image_sample_dref_implicit_lod %7, %11, %bias, 8u, vec2<i32>(1i)
    ret %12
  }
}
)";
...
TEST_F(IR_BuiltinPolyfillSpirvTest, TextureSampleCompareLevel_2D_Offset) {
    ...
    b.With(func->Block(), [&] {
        auto* result = b.Call(
            ty.vec4<f32>(), builtin::Function::kTextureSampleCompareLevel, t, s, coords, dref,
            b.Constant(mod.constant_values.Splat(ty.vec2<i32>(), mod.constant_values.Get(1_i), 2)));
        b.Return(func, result);
    });
...
%foo = func(%t:texture_depth_2d, %s:sampler, %coords:vec2<f32>, %dref:f32):f32 -> %b1 {
  %b1 = block {
    %6:vec4<f32> = textureSampleCompareLevel %t, %s, %coords, %dref, vec2<i32>(1i)
    ret %6
  }
}
)";
...
TEST_F(IR_BuiltinPolyfillSpirvTest, TextureSampleCompareLevel_2DArray_Offset) {
    ...
    b.With(func->Block(), [&] {
        auto* result = b.Call(
            ty.vec4<f32>(), builtin::Function::kTextureSampleCompareLevel, t, s, coords, array_idx,
            bias,
            b.Constant(mod.constant_values.Splat(ty.vec2<i32>(), mod.constant_values.Get(1_i), 2)));
        b.Return(func, result);
    });
...
%foo = func(%t:texture_depth_2d_array, %s:sampler, %coords:vec2<f32>, %array_idx:i32, %bias:f32):f32 -> %b1 {
  %b1 = block {
    %7:vec4<f32> = textureSampleCompareLevel %t, %s, %coords, %array_idx, %bias, vec2<i32>(1i)
    ret %7
  }
}
)";
...
%foo = func(%t:texture_depth_2d_array, %s:sampler, %coords:vec2<f32>, %array_idx:i32, %bias:f32):f32 -> %b1 {
  %b1 = block {
    ...
    %10:f32 = convert %array_idx
    %11:vec3<f32> = construct %8, %9, %10
    %12:f32 = spirv.image_sample_dref_implicit_lod %7, %11, %bias, 10u, 0.0f, vec2<i32>(1i)
    ret %12
  }
}
)";
...
%foo = func(%t:texture_2d_array<f32>, %s:sampler, %coords:vec2<f32>, %array_idx:i32, %ddx:vec2<f32>, %ddy:vec2<f32>):vec4<f32> -> %b1 {
  %b1 = block {
    ...
    %9:f32 = access %coords, 0u
    %10:f32 = access %coords, 1u
    %11:f32 = convert %array_idx
    %12:vec3<f32> = construct %9, %10, %11
    %13:vec4<f32> = spirv.image_sample_implicit_lod %8, %12, 9u, %ddx, %ddy
    ret %13
  }
}
)";
...
%foo = func(%t:texture_2d_array<f32>, %s:sampler, %coords:vec2<f32>, %array_idx:i32, %lod:f32):vec4<f32> -> %b1 {
  %b1 = block {
    ...
    %10:f32 = convert %array_idx
    %11:vec3<f32> = construct %8, %9, %10
    %12:vec4<f32> = spirv.image_sample_explicit_lod %7, %11, 10u, %lod, vec2<i32>(1i)
    ret %12
  }
}
)";
