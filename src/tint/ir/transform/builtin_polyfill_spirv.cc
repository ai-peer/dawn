...
struct BuiltinPolyfillSpirv::State {
    ...
    void Process() {
        ...
        for (auto* builtin : worklist) {
            ...
            TINT_ASSERT(Transform, replacement);

            // Replace the old builtin with the new instruction.
            if (auto name = ir->NameOf(builtin->Result())) {
                ir->SetName(replacement->Result(), name);
            }
            builtin->Result()->ReplaceAllUsesWith(replacement->Result());
            builtin->ReplaceWith(replacement);
            builtin->Destroy();
        }
    }
    ...
    /// @returns the replacement instruction
    Instruction* Select(BuiltinCall* builtin) {
        ...
        return b.Call(builtin->Result()->Type(), IntrinsicCall::Kind::kSpirvSelect,
                      std::move(args));
    }
};
