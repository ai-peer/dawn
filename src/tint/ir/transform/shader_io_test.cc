...
TEST_F(IR_ShaderIOTest, Struct_SharedByVertexAndFragment_Spirv) {
    ...
    auto* str_ty = ty.Struct(
        mod.symbols.New("Interface"),
        {
            {mod.symbols.New("position"), vec4f, {.builtin = builtin::BuiltinValue::kPosition}},
            {mod.symbols.New("color"), vec4f, {.location = 0u}},
        });
    ...
    // Vertex shader.
    {
        ...
        b.With(ep->Block(), [&] {  //
            b.Return(ep, b.Construct(str_ty, b.Construct(vec4f, 0_f), b.Construct(vec4f, 1_f)));
        });
    }
    ...
    // Fragment shader.
    {
        ...
        b.With(ep->Block(), [&] {  //
            b.Return(ep, b.Add(vec4f, b.Access(vec4f, inputs, 0_u), b.Access(vec4f, inputs, 1_u)));
        });
    }
