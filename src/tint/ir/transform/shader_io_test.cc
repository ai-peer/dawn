...
%foo = @fragment func(%front_facing:bool [@front_facing], %position:vec4<f32> [@invariant, @position], %color1:f32 [@location(0)], %color2:f32 [@location(1), @interpolate(linear, sample)]):void -> %b1 {
  %b1 = block {
    ...
    auto* expect = R"(
fooBuiltinInputsStruct = struct @align(16), @block {
...
%b1 = block {  # root
  %foo_BuiltinInputs:ptr<__in, fooBuiltinInputsStruct, read> = var
    ...
    auto* str_ty = ty.Struct(
        mod.symbols.New("Inputs"),
        {
            ...
            },
            {
                ...
                ty.vec4<f32>(),
                {.builtin = builtin::BuiltinValue::kPosition, .invariant = true},
            },
            {
...
%foo = @vertex func():vec4<f32> [@invariant, @position] -> %b1 {
  %b1 = block {
    ...
    auto* expect = R"(
fooBuiltinOutputsStruct = struct @align(16), @block {
  tint_symbol:vec4<f32> @offset(0), @invariant, @builtin(position)
}

%b1 = block {  # root
  %foo_BuiltinOutputs:ptr<__out, fooBuiltinOutputsStruct, write> = var
}
...
// TODO(jrprice): struct shared by vertex and fragment
// TODO(jrprice): struct used for other things
