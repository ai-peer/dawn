  ...
  %b2 = block {
    ...
    if %20 [t: %b11]
      ...
      %b11 = block {
        %21:i32 = access %8, 0i
        %22:i32 = add %21, 1i
        store %return_value, %22
        exit_if
      }

    %23:i32 = load %return_value
    ret %23
  }
}
)";
...
TEST_F(IR_MergeReturnTest, Switch_WithBasicBlockArgumentsOnMerge) {
    ...
    auto* sw = sb.Switch(cond);
    sw->SetType(ty.tuple<i32>());  // NOLINT: false detection of std::tuple
    auto caseA = b.With(b.Case(sw, {Switch::CaseSelector{b.Constant(1_i)}}));
    caseA.Return(func, 42_i);
