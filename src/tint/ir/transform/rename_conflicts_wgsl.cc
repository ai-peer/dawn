    ...
    void EnsureResolvesTo(std::string_view identifier, const CastableBase* thing) {
        for (auto& scope : utils::Reverse(scopes)) {
            if (auto decl = scope.Get(identifier)) {
                ...
                // Operand is shadowed `decl` which prevents the `identifier` resolving to `thing`.
                // Rename `decl` to a new, unique symbol. As this new name is guaranteed to be
                // unique, we can simply remove the identifier from the scope as it's impossible for
                // it to shadow anything else.
