...
struct ShaderIO::State {
    ...
    utils::Vector<type::StructMember*, 4> input_struct_members;

    /// The list of struct members used as shader outputs.
    utils::Vector<type::StructMember*, 4> output_struct_members;
    ...
    void Process() {
        ...
        // TODO(jrprice): return value if not SPIR-V
        wrapper.Return(ep);
    }
    ...
    void PushStructMember(utils::Vector<type::StructMember*, 4>& member_list,
                          type::Type* type,
                          type::StructMemberAttributes attributes) {
        uint32_t offset = 0;
    ...
    type::Struct* MakeStruct(utils::VectorRef<type::StructMember*> members) {
        uint32_t align = 0;
        uint32_t size = 0;
    ...
    utils::Vector<Value*, 4> BuildInnerCallArgs(Builder& builder) {
        ...
        std::function<Value*(type::Type*, uint32_t)> get_input;
        switch (config.backend) {
            case Backend::kSpirv: {
                // TODO(jrprice): needs separate structs for builtin vs location
                ...
                get_input = [&, input_var](type::Type* type, uint32_t idx) {
    ...
    void SetOutputs(Builder& builder, Value* inner_result) {
        ...
        for (auto* member : output_struct->Members()) {
            ...
            if (from->Type()->Is<type::Struct>()) {
                // TODO(jrprice): index might not be the same if sorted
