        ...
        for (auto* param : func->Params()) {
            if (auto* str = param->Type()->As<type::Struct>()) {
                for (auto* member : str->Members()) {
                    ...
                    // TODO(jrprice): Can't do this here, could be used by another entry point.
...
struct ShaderIO::StateSpirv : State {
    ...
    void MakeStructs(Var** builtin_var,
        ...
        if (!builtin_members.IsEmpty()) {
            auto name = ir->NameOf(func).Name() + "Builtin" + name_suffix + "Struct";
    ...
    Value* GetInput(Builder& builder, uint32_t idx) override {
        ...
        auto* source = inputs[idx].attributes.builtin ? builtin_input_var : location_input_var;
        auto* from = builder.Access(ptr, source, u32(input_indices[idx]));
    ...
    void SetOutput(Builder& builder, uint32_t idx, Value* value) override {
        ...
        auto* source = outputs[idx].attributes.builtin ? builtin_output_var : location_output_var;
        auto* to = builder.Access(ptr, source, u32(output_indices[idx]));
