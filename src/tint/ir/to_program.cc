...
class State {
  ...
  private:
    const Module& mod;
    ProgramBuilder b;
    utils::Hashmap<const ir::Value*, Symbol, 16> var_names;
    ...
    const ast::Statement* Stmt(const ir::Instruction* inst) {
        return Switch(
            ...
            [&](const ir::Var* i) { return Var(i); }, [&](const ir::Store* i) { return Store(i); },
            [&](Default) {
                TINT_UNIMPLEMENTED(IR, b.Diagnostics())
    ...
    const ast::VariableDeclStatement* Var(const ir::Var* var) {
        Symbol name =  // var->metadata ? Sym(var->metadata->name) :
            b.Symbols().Register("var_" + std::to_string(var->id));
        auto ty = Type(var->Type());
        var_names.Add(var, name);
        switch (var->address_space) {
            case builtin::AddressSpace::kFunction:
                return b.Decl(b.Var(name, ty));
            case builtin::AddressSpace::kStorage:
                return b.Decl(b.Var(name, ty, var->access, var->address_space));
            default:
                return b.Decl(b.Var(name, ty, var->address_space));
        }
    }

    const ast::AssignmentStatement* Store(const ir::Store* store) {
        auto* expr = Expr(store->from);
        auto var = var_names.Get(store->to);
        if (!var) {
            Err("store to unknown variable");  // TODO: Describe variable
            return nullptr;
        }
        return b.Assign(*var, expr);
    }
