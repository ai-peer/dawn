...
class State {
  ...
  private:
    ...
    std::tuple<const ast::Statement*, bool> AsShortCircuit(ir::If* i) {
        ...
        utils::Vector<ir::Access*, 4> accesses;
        for (auto usage : i->Usages()) {
            auto* access = usage.instruction->As<ir::Access>();
            if (!access || access->Operands().Length() != 1) {
        ...
        if (accesses.Length() == 1 && CanInline(i)) {
            // Replace the single access with the binary op
            Bind(accesses.Front(), bin_op);
            return {nullptr, true};
        }

        // Multiple uses.
        // Declare a let for the binary op, and replace the accesses with the let.
        auto name = BindName(i);
        for (auto access : accesses) {
            access->ReplaceWith(i);
        }
