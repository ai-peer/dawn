...
class State {
  ...
  private:
    ...
    const ast::BlockStatement* FlowNodeGraph(ir::Block* start_node) {
        ...
        ir::FlowNode* block = start_node->As<Block>();
        ...
        while (block) {
            ...
            Status status = tint::Switch(
                ...
                [&](ir::FunctionTerminator*) { return kStop; },
    ...
    bool IsEmpty(ir::Block* node, const ir::FlowNode* stop_at) {
    ...
    utils::Result<const ast::Statement*> Stmt(ir::Instruction* inst) {
    ...
    utils::Result<ast::Type> Type(const type::Type* ty) {
        return tint::Switch<utils::Result<ast::Type>>(
            ...
            [&](const type::Pointer* p) -> utils::Result<ast::Type> {
                // Note: type::Pointer always has an inferred access, but WGSL
                // only allows an explicit access in the 'storage' address space.
