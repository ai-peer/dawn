...
class State {
  ...
  private:
    ...
    const ir::Block* current_switch_case_ = nullptr;
    ...
    const ast::IfStatement* If(const ir::If* i) {
        ...
        if (auto* false_blk = i->False(); false_blk && !false_blk->Instructions().IsEmpty()) {
            bool maybe_elseif = (false_blk->Instructions().Length() == 1) ||
                                (false_blk->Instructions().Length() == 2 &&
                                 false_blk->Instructions()[1]->Is<ir::Branch>());
            if (maybe_elseif) {
                if (auto* else_if = false_blk->Instructions()[0]->As<ir::If>()) {
                    auto* f = If(else_if);
                    if (!f) {
                        return nullptr;
                    }
    ...
    utils::Result<const ast::Statement*> Stmt(const ir::Instruction* inst) {
        return tint::Switch<utils::Result<const ast::Statement*>>(
            ...
            [&](const ir::ExitSwitch* e) { return ExitSwitch(e); },      //
            [&](const ir::ExitIf*) { return nullptr; },                  //
            [&](Default) {
                UNHANDLED_CASE(inst);
                return utils::Failure;
            });
