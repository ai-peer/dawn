  ...
  private:
    ...
    void MarkInlinable(ir::Block* block) {
        ...
        for (auto* inst : *block) {
            ...
            switch (inst->Results().Length()) {
                case 0: {
                    if (sequenced) {
                        // A sequenced instruction with no return values cannot be inlined (as it
                        // has no values) All preceding sequenced instructions cannot be inlined
                        // past this point.
                        pending.Clear();
                    }
                    break;
