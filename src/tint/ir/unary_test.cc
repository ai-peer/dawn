...
TEST_F(IR_InstructionTest, CreateAddressOf) {
    ...
    const auto* inst = b.builder.AddressOf(
        b.builder.ir.types.Get<type::Pointer>(b.builder.I32(), builtin::AddressSpace::kPrivate,
...
TEST_F(IR_InstructionTest, CreateComplement) {
    auto& b = CreateEmptyBuilder();
    const auto* inst = b.builder.Complement(b.builder.I32(), b.builder.Constant(4_i));
...
TEST_F(IR_InstructionTest, CreateIndirection) {
    ...
    // TODO(dsinclair): This would be better as an identifier, but works for now.
    const auto* inst = b.builder.Indirection(b.builder.I32(), b.builder.Constant(4_i));
...
TEST_F(IR_InstructionTest, CreateNegation) {
    auto& b = CreateEmptyBuilder();
    const auto* inst = b.builder.Negation(b.builder.I32(), b.builder.Constant(4_i));
