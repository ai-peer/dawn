...
TEST_F(IR_FromProgramAccessorTest, Accessor_Let_SingleIndex) {
    ...
    EXPECT_EQ(Disassemble(m.Get()),
              R"(%test_function = @compute @workgroup_size(1, 1, 1) func():void -> %b1 {
  %b1 = block {
    %a:vec3<u32> = access vec3<u32>(0u)  # named value (no-op)
    %b:u32 = access %a, 2u
    ret
  }
}
...
TEST_F(IR_FromProgramAccessorTest, Accessor_Let_MultiIndex) {
    ...
    EXPECT_EQ(Disassemble(m.Get()),
              R"(%test_function = @compute @workgroup_size(1, 1, 1) func():void -> %b1 {
  %b1 = block {
    %a:mat3x4<f32> = access mat3x4<f32>(vec4<f32>(0.0f))  # named value (no-op)
...
%test_function = @compute @workgroup_size(1, 1, 1) func():void -> %b1 {
  %b1 = block {
    %a:MyStruct = access MyStruct(0i)  # named value (no-op)
    %b:i32 = access %a, 0u
    ret
  }
}
...
%test_function = @compute @workgroup_size(1, 1, 1) func():void -> %b1 {
  %b1 = block {
    %a:Outer = access Outer(0i, Inner(0.0f))  # named value (no-op)
...
%test_function = @compute @workgroup_size(1, 1, 1) func():void -> %b1 {
  %b1 = block {
    %a:array<Outer, 4> = access array<Outer, 4>(Outer(0i, array<Inner, 4>(Inner(0i, 0.0f, vec4<f32>(0.0f)))))  # named value (no-op)
...
TEST_F(IR_FromProgramAccessorTest, Accessor_Let_SingleElement) {
    ...
    EXPECT_EQ(Disassemble(m.Get()),
              R"(%test_function = @compute @workgroup_size(1, 1, 1) func():void -> %b1 {
  %b1 = block {
    %a:vec2<f32> = access vec2<f32>(0.0f)  # named value (no-op)
    %b:f32 = access %a, 1u
    ret
  }
}
...
TEST_F(IR_FromProgramAccessorTest, Accessor_Let_MultiElementSwizzle) {
    ...
    EXPECT_EQ(Disassemble(m.Get()),
              R"(%test_function = @compute @workgroup_size(1, 1, 1) func():void -> %b1 {
  %b1 = block {
    %a:vec3<f32> = access vec3<f32>(0.0f)  # named value (no-op)
...
%test_function = @compute @workgroup_size(1, 1, 1) func():void -> %b1 {
  %b1 = block {
    %a:MyStruct = access MyStruct(0i, vec4<f32>(0.0f))  # named value (no-op)
...
TEST_F(IR_FromProgramAccessorTest, Accessor_Const_AbstractVectorWithIndex) {
    ...
    EXPECT_EQ(Disassemble(m.Get()),
              R"(%test_function = @compute @workgroup_size(1, 1, 1) func():void -> %b1 {
  %b1 = block {
    %i:i32 = access 1i  # named value (no-op)
