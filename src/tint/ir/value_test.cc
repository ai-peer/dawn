...
using IR_Value = IRTestHelper;

TEST_F(IR_Value, ReplaceAllUsesWith_Value) {
    auto* val_old = b.RuntimeValue(ty.i32());
    auto* val_new = b.RuntimeValue(ty.i32());
...
TEST_F(IR_Value, ReplaceAllUsesWith_Fn) {
    auto* val_old = b.RuntimeValue(ty.i32());
    auto* val_new = b.RuntimeValue(ty.i32());
    auto* inst = b.Add(mod.Types().i32(), val_old, 1_i);
    EXPECT_EQ(inst->LHS(), val_old);
...
TEST_F(IR_Value, Destroy) {
    auto* val = b.RuntimeValue(ty.i32());
    EXPECT_TRUE(val->Alive());
    val->Destroy();
    EXPECT_FALSE(val->Alive());
}

TEST_F(IR_Value, Destroy_HasUsage) {
    EXPECT_FATAL_FAILURE(
        {
            Module mod;
            Builder b{mod};
            auto* val = b.RuntimeValue(mod.Types().i32());
...
TEST_F(IR_Value, Destroy_HasSource) {
    EXPECT_FATAL_FAILURE(
        {
            Module mod;
            Builder b{mod};
