...
TEST_F(IR_FromProgramLetTest, Constant) {
  ...
  %b1 = block {
    %a:i32 = access 42i  # named value (no-op)
    ret
  }
}
)");
...
TEST_F(IR_FromProgramLetTest, BinaryOp) {
    ...
    EXPECT_EQ(Disassemble(m.Get()),
              R"(%test_function = @compute @workgroup_size(1, 1, 1) func():void -> %b1 {
  %b1 = block {
    %a:i32 = access 3i  # named value (no-op)
    ret
  }
}
)");
...
TEST_F(IR_FromProgramLetTest, Chain) {
    ...
    EXPECT_EQ(Disassemble(m.Get()),
              R"(%test_function = @compute @workgroup_size(1, 1, 1) func():void -> %b1 {
  %b1 = block {
    %a:i32 = access 1i  # named value (no-op)
    %b:i32 = access %a  # named value (no-op)
    %c:i32 = access %b  # named value (no-op)
    ret
  }
}
)");
