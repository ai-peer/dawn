  ...
  private:
    ...
    void CheckFunction(const Function* func) {
        for (const auto* param : func->Params()) {
            if (param == nullptr) {
                AddError("null parameter in function " + Name(func));
                continue;
            }

            utils::Vector<const Instruction*, 4> v(param->Usage());
            if (!v.Any([func](const Value* inst) { return inst->As<ir::Function>() == func; })) {
                AddError("function " + Name(func) + " param does not have function in usage");
            }
        }

        if (!func->StartTarget()) {
            AddError("function " + Name(func) + " start target is null");
        }
    ...
    void CheckBlock(const Block* blk) {
        ...
        for (const auto* inst : *blk) {
            if (!inst) {
                AddError("block with nullptr instruction");
                continue;
            }
        ...
        tint::Switch(
            ...
            [&](const ir::If* if_) {
                if (if_->Condition() == nullptr) {
                    AddError("if instruction with a null condition");
                }
            },
            [&](Default) {
