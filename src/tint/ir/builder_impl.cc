...
#include "src/tint/ir/switch_flow_node.h"
...
IfFlowNode* BuilderImpl::CreateIf(const ast::Statement* stmt) {
...
LoopFlowNode* BuilderImpl::CreateLoop(const ast::LoopStatement* stmt) {
    auto* ir_loop = ir_.flow_nodes.Create<ir::LoopFlowNode>(stmt);
...
void BuilderImpl::BranchTo(const ir::FlowNode* node) {
...
void BuilderImpl::BranchToIfNeeded(const ir::FlowNode* node) {
...
FlowNode* BuilderImpl::FindEnclosingControl(ControlFlags flags) {
    for (auto it = flow_stack.rbegin(); it != flow_stack.rend(); ++it) {
        if ((*it)->Is<ir::LoopFlowNode>()) {
            return *it;
        }
        if (flags == ControlFlags::kExcludeSwitch) {
            continue;
        }
        if ((*it)->Is<ir::SwitchFlowNode>()) {
            return *it;
        }
    }
    return nullptr;
...
bool BuilderImpl::EmitLoop(const ast::LoopStatement* stmt) {
    auto* loop_node = CreateLoop(stmt);
...
bool BuilderImpl::EmitBreak(const ast::BreakStatement*) {
    ...
    if (auto* c = current_control->As<ir::LoopFlowNode>()) {
        BranchTo(c->merge_target);
    } else if (auto* s = current_control->As<ir::SwitchFlowNode>()) {
...
bool BuilderImpl::EmitBreakIf(const ast::BreakIfStatement* stmt) {
    ...
    auto* current_control = FindEnclosingControl(ControlFlags::kExcludeSwitch);
    TINT_ASSERT(IR, current_control);
    TINT_ASSERT(IR, current_control->Is<ir::LoopFlowNode>());

    auto* loop = current_control->As<ir::LoopFlowNode>();
