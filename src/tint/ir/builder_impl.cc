...
bool IsConnected(const FlowNode* b) {
    ...
    // Getting hear means all the including branches are disconnected.
    return false;
}
...
bool BuilderImpl::EmitLoop(const ast::LoopStatement* stmt) {
    ...
    // The loop merge is only the current flow block if there is actual control flow which goes to
    // that node. There are three casese:
    //  * 0 inbound branches in which case it is outside of control flow
    //  * 1 inbound branch, that branch is the continuing block and the continuing block has no
    //    branches targeting it in which case it's outside of control flow
    //  * > 1 branches targeting it, in which case it's in control flow
    //
