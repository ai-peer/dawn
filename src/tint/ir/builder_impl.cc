...
bool BuilderImpl::EmitIf(const ast::IfStatement* stmt) {
    ...
    // Emit the if condition generation into the end of the preceeding block
...
bool BuilderImpl::EmitSwitch(const ast::SwitchStatement* stmt) {
...

    {
        ...
        for (const auto* c : stmt->body) {
            auto* ir_case = builder.CreateCase(switch_node, c->selectors);
            current_flow_block = ir_case->start_target;
...
utils::Result<Register> BuilderImpl::EmitLiteral(const ast::LiteralExpression* lit) {
    auto op = tint::Switch(
        ...
        [&](const ast::BoolLiteralExpression* l) {
            return utils::Result<Op>{builder.CreateLoad(l)};
        },
    ...
    auto result = op.Get().result;
    current_flow_block->ops.Push(op.Get());
    return result;
}
