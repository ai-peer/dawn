    ...
    for (auto* decl : sem->DependencyOrderedDeclarations()) {
        tint::Switch(
            ...
            [&](const ast::Variable* var) {
                ...
                current_flow_block = builder.CreateRootBlockIfNeeded();
                EmitVariable(var);
                current_flow_block = nullptr;
            },
    ...
    if (ast_func->IsEntryPoint()) {
        ...
        switch (ast_func->PipelineStage()) {
            case ast::PipelineStage::kVertex:
                ir_func->pipeline_stage = builtin::Attribute::kVertex;
                break;
            case ast::PipelineStage::kFragment:
                ir_func->pipeline_stage = builtin::Attribute::kFragment;
                break;
            case ast::PipelineStage::kCompute: {
                ir_func->pipeline_stage = builtin::Attribute::kCompute;
