...
Block* BuilderImpl::CreateBlock() {
    return ir_.flow_nodes.Create<Block>();
}

Terminator* BuilderImpl::CreateTerminator() {
    return ir_.flow_nodes.Create<Terminator>();
}

Function* BuilderImpl::CreateFunction(const ast::Function* ast_func) {
    current_function_ = ir_.flow_nodes.Create<Function>(ast_func);
    current_function_->start_target = CreateBlock();
    current_function_->end_target = CreateTerminator();
...
If* BuilderImpl::CreateIf(const ast::Statement* stmt) {
    auto* ir_if = ir_.flow_nodes.Create<If>(stmt);
    ir_if->false_target = CreateBlock();
    ir_if->true_target = CreateBlock();
    return ir_if;
}

Loop* BuilderImpl::CreateLoop(const ast::LoopStatement* stmt) {
    auto* ir_loop = ir_.flow_nodes.Create<Loop>(stmt);
