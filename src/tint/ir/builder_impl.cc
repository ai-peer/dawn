...
utils::Result<Value*> BuilderImpl::EmitShortCircuit(const ast::BinaryExpression* expr) {
    ...
    // TODO(dsinclair): This needs to emit something before branching to the if_node which is the
    // var that will be assigned too with the rhs value.
    ...
    // TODO(dsinclair): This isn't needed. In the pre-amble create a declaration, assign that the
    // lhs, set the condition to the declaration value and then assign at the end of the rhs with
    // the rhs value. This then all goes away and the declaration is returned.
