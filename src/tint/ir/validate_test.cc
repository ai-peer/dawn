...
TEST_F(IR_ValidateTest, Access_DynamicallyUnindexableType_Value) {
    auto* str_ty = ty.Struct(mod.symbols.New(), {
    ...
    ASSERT_FALSE(res);
    EXPECT_EQ(res.Failure().str(),
              R"(:8:25 error: access: type tint_symbol cannot be dynamically indexed
    %4:i32 = access %2, %3
                        ^^
...
%my_func = func(%2:tint_symbol, %3:i32):void -> %b1 {
  %b1 = block {
    %4:i32 = access %2, %3
    ret
  }
...
TEST_F(IR_ValidateTest, Access_DynamicallyUnindexableType_Ptr) {
    ...
    ASSERT_FALSE(res);
    EXPECT_EQ(res.Failure().str(),
              R"(:8:25 error: access: type ptr<tint_symbol> cannot be dynamically indexed
    %4:i32 = access %2, %3
                        ^^
...
%my_func = func(%2:ptr<private, tint_symbol, read_write>, %3:i32):void -> %b1 {
  %b1 = block {
    %4:i32 = access %2, %3
    ret
  }
