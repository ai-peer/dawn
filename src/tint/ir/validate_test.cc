...
TEST_F(IR_ValidateTest, Valid_Access) {
    auto* f = b.CreateFunction("my_func", ty.void_());
    auto* vec = b.FunctionParam(ty.mat3x2(ty.f32()));
    f->SetParams(utils::Vector{vec});
    mod.functions.Push(f);

    f->StartTarget()->Append(
        b.Access(ty.f32(), vec, utils::Vector{b.Constant(1_u), b.Constant(0_u)}));
...
TEST_F(IR_ValidateTest, Valid_Access_OOB_Index_Ptr) {
    auto* f = b.CreateFunction("my_func", ty.void_());
    auto* vec = b.FunctionParam(ty.pointer(ty.mat3x2(ty.f32()), builtin::AddressSpace::kPrivate,
    ...
    f->StartTarget()->Append(
        b.Access(ty.pointer(ty.f32(), builtin::AddressSpace::kPrivate, builtin::Access::kReadWrite),
                 vec, utils::Vector{b.Constant(1_u), b.Constant(0_u)}));
    f->StartTarget()->Append(b.Return(f));
...
TEST_F(IR_ValidateTest, Access_NegativeIndex) {
    auto* f = b.CreateFunction("my_func", ty.void_());
    auto* vec = b.FunctionParam(ty.vec3(ty.f32()));
    f->SetParams(utils::Vector{vec});
    mod.functions.Push(f);

    f->StartTarget()->Append(b.Access(ty.f32(), vec, utils::Vector{b.Constant(-1_i)}));
    f->StartTarget()->Append(b.Return(f));

    auto res = ir::Validate(mod);
    ASSERT_FALSE(res);
    EXPECT_EQ(res.Failure().str(), R"( error: access: constant index must be positive, got -1
:2:1 note: In block
  %b1 = block {
^^^^^^^^^^^^^^^
    %3:f32 = access %2, -1i
...
TEST_F(IR_ValidateTest, Access_OOB_Index_Value) {
    ...
    f->StartTarget()->Append(
        b.Access(ty.f32(), vec, utils::Vector{b.Constant(1_u), b.Constant(3_u)}));
    f->StartTarget()->Append(b.Return(f));

    auto res = ir::Validate(mod);
    ASSERT_FALSE(res);
    EXPECT_EQ(res.Failure().str(), R"( error: access: index out of bounds for type vec2<f32>
:2:1 note: In block
  %b1 = block {
^^^^^^^^^^^^^^^
...
TEST_F(IR_ValidateTest, Access_OOB_Index_Ptr) {
    ...
    f->StartTarget()->Append(
        b.Access(ty.pointer(ty.f32(), builtin::AddressSpace::kPrivate, builtin::Access::kReadWrite),
                 vec, utils::Vector{b.Constant(1_u), b.Constant(3_u)}));
    f->StartTarget()->Append(b.Return(f));

    auto res = ir::Validate(mod);
    ASSERT_FALSE(res);
    EXPECT_EQ(res.Failure().str(), R"( error: access: index out of bounds for type ptr<vec2<f32>>
:2:1 note: In block
  %b1 = block {
^^^^^^^^^^^^^^^
...
TEST_F(IR_ValidateTest, Access_StaticallyUnindexableType_Value) {
    auto* f = b.CreateFunction("my_func", ty.void_());
    auto* vec = b.FunctionParam(ty.f32());
    f->SetParams(utils::Vector{vec});
    mod.functions.Push(f);

    f->StartTarget()->Append(b.Access(ty.f32(), vec, utils::Vector{b.Constant(1_u)}));
    f->StartTarget()->Append(b.Return(f));

    auto res = ir::Validate(mod);
    ASSERT_FALSE(res);
    EXPECT_EQ(res.Failure().str(), R"( error: access: type f32 cannot be indexed
:2:1 note: In block
  %b1 = block {
^^^^^^^^^^^^^^^
    %3:f32 = access %2, 1u
...
TEST_F(IR_ValidateTest, Access_StaticallyUnindexableType_Ptr) {
    ...
    f->StartTarget()->Append(
        b.Access(ty.pointer(ty.f32(), builtin::AddressSpace::kPrivate, builtin::Access::kReadWrite),
                 vec, utils::Vector{b.Constant(1_u)}));
    f->StartTarget()->Append(b.Return(f));

    auto res = ir::Validate(mod);
    ASSERT_FALSE(res);
    EXPECT_EQ(res.Failure().str(), R"( error: access: type ptr<f32> cannot be indexed
:2:1 note: In block
  %b1 = block {
^^^^^^^^^^^^^^^
...
TEST_F(IR_ValidateTest, Access_DynamicallyUnindexableType_Value) {
    ...
    auto* f = b.CreateFunction("my_func", ty.void_());
    auto* str = b.FunctionParam(str_ty);
    auto* idx = b.FunctionParam(ty.i32());
    f->SetParams(utils::Vector{str, idx});
    mod.functions.Push(f);

    f->StartTarget()->Append(b.Access(ty.i32(), str, utils::Vector{idx}));
    f->StartTarget()->Append(b.Return(f));

    auto res = ir::Validate(mod);
    ASSERT_FALSE(res);
    EXPECT_EQ(res.Failure().str(),
              R"( error: access: type tint_symbol_2 cannot be dynamically indexed
:7:1 note: In block
  %b1 = block {
^^^^^^^^^^^^^^^
    %4:i32 = access %2, %3
...
TEST_F(IR_ValidateTest, Access_DynamicallyUnindexableType_Ptr) {
    ...
    ASSERT_FALSE(res);
    EXPECT_EQ(res.Failure().str(),
              R"( error: access: type ptr<tint_symbol_2> cannot be dynamically indexed
:7:1 note: In block
  %b1 = block {
^^^^^^^^^^^^^^^
    %4:i32 = access %2, %3
...
TEST_F(IR_ValidateTest, Access_Incorrect_Type_Value_Value) {
    ...
    f->StartTarget()->Append(
        b.Access(ty.i32(), vec, utils::Vector{b.Constant(1_u), b.Constant(1_u)}));
    f->StartTarget()->Append(b.Return(f));
...
TEST_F(IR_ValidateTest, Access_Incorrect_Type_Ptr_Ptr) {
    ...
    f->StartTarget()->Append(
        b.Access(ty.pointer(ty.i32(), builtin::AddressSpace::kPrivate, builtin::Access::kReadWrite),
                 vec, utils::Vector{b.Constant(1_u), b.Constant(1_u)}));
    f->StartTarget()->Append(b.Return(f));
...
TEST_F(IR_ValidateTest, Access_Incorrect_Type_Ptr_Value) {
    ...
    f->StartTarget()->Append(
        b.Access(ty.f32(), vec, utils::Vector{b.Constant(1_u), b.Constant(1_u)}));
    f->StartTarget()->Append(b.Return(f));
