...
TEST_F(IR_FromProgramFunctionTest, EmitFunction_UnreachableEnd_NoReturn) {
    Func("test", utils::Empty, ty.void_(),
         utils::Vector{If(true, Block(Return()), Else(Block(Return())))}, utils::Empty);

    auto m = Build();
    ASSERT_TRUE(m) << (!m ? m.Failure() : "");

    EXPECT_EQ(Disassemble(m.Get()), R"(%test = func():void -> %b1 {
  %b1 = block {
