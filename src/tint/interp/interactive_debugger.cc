...
#include "tint/ast/expression.h"
#include "tint/ast/statement.h"
...
#define SCOPED_RICH_TEXT(code) ScopedRichText scoped_rich_text(use_rich_text_, code)
}  // namespace

InteractiveDebugger::InteractiveDebugger(const Source::File& source,
                                         ShaderExecutor& executor,
                                         std::istream& input)
    : source_(source), executor_(executor), input_(input) {
    diag_printer_ = diag::Printer::create(stderr, true);
    interactive_ = isatty(fileno(stdout));
    use_rich_text_ = interactive_;

    // Build a map of available breakpoints.
    // Each line has at most one available breakpoint, which is the first non-identifier expression
    ...
    for (auto* node : executor.Program().ASTNodes().Objects()) {
        ...
        if (auto* stmt = node->As<ast::Statement>()) {
            if (!stmt->Is<ast::BlockStatement>()) {
                ...
                available_breakpoints_.insert({loc.line, stmt});
            }
        } else if (auto* expr = node->As<ast::Expression>()) {
            auto* sem = executor.Sem().GetVal(expr);
            if (!expr->Is<ast::IdentifierExpression>() && sem &&
                sem->Stage() == sem::EvaluationStage::kRuntime) {
                // Add the expression if there is no other expression in the map for this line.
                auto prev = available_breakpoints_.find(loc.line);
                if (prev == available_breakpoints_.end() || prev->second->Is<ast::Statement>()) {
                    available_breakpoints_[loc.line] = expr;
                }
            }
        }
    }
...
void InteractiveDebugger::Interact(bool force_break) {
    ...
    while (true) {
        // Some commands can change the current invocation, so fetch this each on every iteration.
        auto* current_invocation = executor_.CurrentInvocation();
        ...
        if (tokens[0] == "h" || tokens[0] == "help") {
            // TODO: Show more detailed command usage, e.g. with "help command".
            std::cout << R"(
...
void InteractiveDebugger::ShowBacktrace(uint32_t max_depth) const {
    ...
    if (current_invocation) {
        ...
        for (uint32_t i = 0; i < max_depth && i < call_stack.size(); i++) {
            ...
            std::cout << "frame #" << i << ": " << executor_.Symbols().NameFor(func->symbol)
                      << "() at ";
...
namespace {
bool ParseU32(const std::string& str, const char* use, uint32_t& result) {
    char* end;
    result = static_cast<uint32_t>(std::strtoull(str.c_str(), &end, 10));
    if (strlen(end) != 0) {
...
void InteractiveDebugger::Backtrace(const std::vector<std::string>& tokens) {
    ...
    if (tokens.size() > 1) {
        ...
        if (!ParseU32(tokens[1], "maximum depth", max_depth)) {
            return;
        }
    }
...
void InteractiveDebugger::Break(const std::vector<std::string>& tokens) {
    ...
    uint32_t line_num;
    if (!ParseU32(tokens[1], "line number", line_num)) {
        return;
    }

    // Check that the line contains a statement or expression that we can break on.
    auto itr = available_breakpoints_.find(line_num);
    if (itr == available_breakpoints_.end()) {
    ...
    if (!breakpoints_.insert(itr->second).second) {
        std::cerr << "Breakpoint already exists at line " << line_num << std::endl;
    } else {
        std::cout << "Breakpoint added at " << source_.path << ":" << line_num << std::endl;
        ShowLines(static_cast<int64_t>(line_num), 1, itr->second->source.range);
    }
}
...
void InteractiveDebugger::Breakpoint(const std::vector<std::string>& tokens) {
    ...
    if (tokens[1] == "list") {
        ...
        // Parse the line number as an integer.
        uint32_t line_num;
        if (!ParseU32(tokens[2], "line number", line_num)) {
            return;
        }

        // Get the breakpoint location for this line.
        auto itr = available_breakpoints_.find(line_num);
        if (itr == available_breakpoints_.end() || !breakpoints_.count(itr->second)) {
        ...
        if (breakpoints_.erase(itr->second)) {
            std::cout << "Breakpoint removed at " << source_.path << ":" << line_num << std::endl;
        }
    } else {
...
void InteractiveDebugger::Invocation(const std::vector<std::string>& tokens) {
    ...
    // Parse the group ID.
    UVec3 local_id;
    if (!ParseU32(tokens[1], "local_id.x", local_id[0])) {
        return;
    }
    if (tokens.size() > 2 && !ParseU32(tokens[2], "local_id.y", local_id[1])) {
        return;
    }
    if (tokens.size() > 3 && !ParseU32(tokens[3], "local_id.z", local_id[2])) {
        return;
    }

    // Check the ID is within the workgroup size.
    if (local_id[0] >= workgroup->Size()[0] || local_id[1] >= workgroup->Size()[1] ||
        local_id[2] >= workgroup->Size()[2]) {
...
void InteractiveDebugger::Workgroup(const std::vector<std::string>& tokens) {
    ...
    // Parse the group ID.
    UVec3 group_id;
    if (!ParseU32(tokens[1], "group_id.x", group_id[0])) {
        return;
    }
    if (tokens.size() > 2 && !ParseU32(tokens[2], "group_id.y", group_id[1])) {
        return;
    }
    if (tokens.size() > 3 && !ParseU32(tokens[3], "group_id.z", group_id[2])) {
        return;
    }

    // Check the ID is inside the dispatch.
    if (group_id[0] >= executor_.WorkgroupCount()[0] ||
        group_id[1] >= executor_.WorkgroupCount()[1] ||
        group_id[2] >= executor_.WorkgroupCount()[2]) {
