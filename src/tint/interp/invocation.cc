...
#include "tint/builtin/builtin_value.h"
...
#include "tint/sem/value_constructor.h"
#include "tint/sem/value_conversion.h"
...
Invocation::Invocation(ShaderExecutor& executor,
    ...
    auto make_u32 = [&](uint32_t value) { return executor_.Builder().constants.Get(u32(value)); };
    auto make_uvec3 = [&](const Source& source, uint32_t a, uint32_t b, uint32_t c) {
        utils::Vector<const constant::Value*, 3> els;
        els.Push(make_u32(a));
        els.Push(make_u32(b));
        els.Push(make_u32(c));
        ...
        return Switch(
            ...
            [&](const sem::ValueConstructor*) {
                ...
                auto mat_vec = [&](const type::Type* type,
                                   tint::resolver::CtorConvIntrinsic intrinsic) {
                ...
                auto result = Switch(
                    ...
                    [&](const type::Vector* vec) {
                        return mat_vec(vec->type(),
                                       resolver::VectorCtorConvIntrinsic(vec->Width()));
                    },
                    [&](const type::Matrix* mat) {
                        return mat_vec(mat->type(), resolver::MatrixCtorConvIntrinsic(
                                                        mat->columns(), mat->rows()));
                    },
                    [&](Default) {
                        if (!result_ty->Is<type::Scalar>()) {
                            executor_.ReportFatalError("unhandled type constructor", call->source);
                            return resolver::ConstEval::Result(nullptr);
                        }
...
ExprResult Invocation::EvaluateBuiltin(const sem::Builtin* builtin,
    ...
    if (builtin->Type() == builtin::Function::kArrayLength) {
        ...
        auto* result =
            executor_.Builder().constants.Get(u32(ptr.Pointer()->Size() / arr->Stride()));
        return ExprResult::MakeValue(result);
    }
    if (builtin->IsAtomic()) {
        return EvaluateBuiltinAtomic(builtin, call);
    }
    if (builtin->Type() == builtin::Function::kStorageBarrier ||
        builtin->Type() == builtin::Function::kWorkgroupBarrier) {
        barrier_ = call;
        return ExprResult();
    }
    if (builtin->Type() == builtin::Function::kWorkgroupUniformLoad) {
...
Invocation::ExprEvaluator Invocation::EnqueueIndexAccessor(
    ...
    return [&, accessor]() {
        ...
        if (auto* arr = obj_ty->As<type::Array>()) {
            stride = arr->Stride();
        } else if (auto* mat = obj_ty->As<type::Matrix>()) {
...
Invocation::StmtExecutor Invocation::IncrementDecrement(
    ...
    return [&, inc_dec]() {
        ...
        auto* one = executor_.Builder().constants.Get(AInt(1));
