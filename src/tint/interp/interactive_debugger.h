...
class InteractiveDebugger {
  public:
    ...
    /// @param source the source file of the shader module
    /// @param executor the shader executor to attach to
    /// @param input the command input stream
    InteractiveDebugger(const Source::File& source,
  ...
  private:
    ...
    std::istream& input_;
    std::unique_ptr<diag::Printer> diag_printer_;
    ...
    std::unordered_map<uint64_t, const ast::Node*> available_breakpoints_;
