...
using BindingList = std::unordered_map<sem::BindingPoint, Binding>;
...
class ShaderExecutor {
  public:
    /// Empty structure for use by `Result` for methods which need to signal failures.
    struct Success {};

    /// The result of a method which may fail with a reason string.
    using Result = utils::Result<Success, std::string>;
    ...
    static utils::Result<std::unique_ptr<ShaderExecutor>, std::string>
    ...
    resolver::ConstEval& ConstEval() { return *const_eval_; }

    /// @returns the IntrinsicTable object to use for expression evaluation
    resolver::IntrinsicTable& IntrinsicTable() { return *intrinsic_table_; }

    /// @returns the semantic info used for this execution
    const sem::Info& Sem() const { return builder_.Sem(); }

    /// @returns the symbol table used for this execution
    const SymbolTable& Symbols() const { return builder_.Symbols(); }

    /// @returns the memory view allocator used for this execution
    utils::BlockAllocator<MemoryView>& MemoryViews() { return memory_views_; }
    ...
    const constant::Value* GetNamedOverride(const sem::Variable* named_override) const {
  ...
  private:
    /// Constructor
    ShaderExecutor(const tint::Program& program);
    ...
    Result Init(const tint::Program& program,
    ...
    utils::BlockAllocator<MemoryView> memory_views_;

    std::unique_ptr<resolver::ConstEval> const_eval_;
    std::unique_ptr<resolver::IntrinsicTable> intrinsic_table_;
