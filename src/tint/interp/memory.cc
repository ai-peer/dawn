...
const core::constant::Value* MemoryView::Load(const core::type::Type* type, uint64_t offset) {
    ...
    auto check_fp_value = [&](float& value) {
        if (!std::isfinite(value)) {
            ...
            list.AddWarning(diag::System::Interpreter, ss.str(), source);
...
void MemoryView::ReportOutOfBounds(std::string msg) {
    ...
    // Report the error on the expression that caused it.
    {
        ...
        list.AddWarning(diag::System::Interpreter, msg, source);
    }
    ...
    // Show the base allocation that we are accessing.
    {
        ...
        ss << "accessing " << root->Size() << " byte allocation in the " << root->AddressSpace()
           << " address space";
        list.AddNote(diag::System::Interpreter, ss.str(), root->source_);
    }
    ...
    // Show where the invalid view was created.
    {
        ...
        list.AddNote(diag::System::Interpreter, ss.str(), first_invalid->source_);
    }
