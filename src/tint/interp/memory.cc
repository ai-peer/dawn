...
const constant::Value* MemoryView::Load(const type::Type* type, uint64_t offset) {
    ...
    auto check_fp_value = [&](float& value) {
        if (!std::isfinite(value)) {
            ...
            ss << "loading a non-finite " << type->FriendlyName(executor_->Symbols()) << " value ("
               << value << ")";
    ...
    return Switch(
        ...
        [&](const type::F16*) {
            ...
            uint16_t bits;
            memory_->Load(&bits, offset, sizeof(uint16_t));
            f16 value = f16::FromBits(bits);
            check_fp_value(value.value);
            return executor_->Builder().create<constant::Scalar<f16>>(type, value);
        },
        [&](const type::I32*) {
            i32 value;
