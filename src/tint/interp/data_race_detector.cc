...
DataRaceDetector::DataRaceDetector(ShaderExecutor& executor) : executor_(executor) {
    executor_.AddBarrierCallback(
        [&](auto* workgroup, auto addrspace) { Barrier(workgroup, addrspace); });
    executor_.AddMemoryLoadCallback([&](auto* view) { MemoryLoad(view); });
    executor_.AddMemoryStoreCallback([&](auto* view) { MemoryStore(view); });
    executor_.AddWorkgroupBeginCallback([&](auto* workgroup) { WorkgroupBegin(workgroup); });
    executor_.AddWorkgroupCompleteCallback([&](auto* workgroup) { WorkgroupComplete(workgroup); });
}

void DataRaceDetector::Barrier(const Workgroup* workgroup, const ast::CallExpression* call) {
    auto* builtin = executor_.Sem().Get<sem::Call>(call)->Target()->As<sem::Builtin>();
    TINT_ASSERT(Interpreter, builtin != nullptr);
    ...
    switch (builtin->Type()) {
        case sem::BuiltinType::kStorageBarrier:
            SyncWorkgroup(workgroup, type::AddressSpace::kStorage);
            break;
        case sem::BuiltinType::kWorkgroupBarrier:
        case sem::BuiltinType::kWorkgroupUniformLoad:
            SyncWorkgroup(workgroup, type::AddressSpace::kWorkgroup);
            break;
        default:
            TINT_ASSERT(Interpreter, false && "unhandled barrier builtin");
            break;
    }
}

void DataRaceDetector::MemoryLoad(const MemoryView* view) {
    RegisterAccess(view, kLoad);
}

void DataRaceDetector::MemoryStore(const MemoryView* view) {
    RegisterAccess(view, kStore);
}
...
void DataRaceDetector::WorkgroupBegin(const Workgroup* workgroup) {
    ...
    per_invocation_accesses_[workgroup->GroupId()].resize(wgsize[0] * wgsize[1] * wgsize[2]);
}
...
void DataRaceDetector::RegisterAccess(const MemoryView* view, MemoryAccessKind kind) {
    ...
    if (addrspace != type::AddressSpace::kStorage && addrspace != type::AddressSpace::kWorkgroup) {
        // We only care about shared resources.
        // TODO: Check the access mode of the variable and skip read-only storage buffers too.
        return;
    }
    ...
    TINT_ASSERT(Interpreter, workgroup != nullptr);
    ...
    TINT_ASSERT(Interpreter, node != nullptr);
    ...
    if (kind == kStore && type->is_scalar()) {
        auto* parent = view->Parent();
        if (parent && parent->Type()->is_scalar_vector()) {
            access.offset = parent->Offset();
            access.size = parent->Size();
    ...
    for (uint32_t i = 0; i < access.size; i++) {
        MemoryAccessMap* map;
        if (addrspace == type::AddressSpace::kStorage) {
        ...
        MergeAccess({root_view, access.offset + i}, access, *map, /* check_for_races */ false);
    }
}
...
void DataRaceDetector::MergeAccess(MemoryAccessKey access_key,
    ...
    if (itr != access_map.end()) {
        ...
        if (check_for_races &&
            ...
            (access.kind == kStore || itr->second.kind == kStore)) {
            RecordRace(access_key.view, access, itr->second);
        }

        if (itr->second.kind == kStore) {
...
void DataRaceDetector::SyncWorkgroup(const Workgroup* workgroup, type::AddressSpace addrspace) {
    ...
    for (auto& access_map : per_invocation_accesses_[workgroup->GroupId()]) {
        if (addrspace == type::AddressSpace::kWorkgroup) {
        ...
        } else if (addrspace == type::AddressSpace::kStorage) {
...
void DataRaceDetector::EmitRaces() {
    for (auto& itr : races_) {
        ...
        // Show the workgroup or storage buffer declaration that is being accessed.
        {
            ...
            ss << "data race detected on accesses to "
               << (race.root_view->AddressSpace() == type::AddressSpace::kStorage
                       ? "storage buffer"
                       : "workgroup variable");
            error.add_warning(diag::System::Interpreter, ss.str(), race.root_view->Source());
        }

        // Show the two accesses as diagnostic notes.
        auto add_note = [&](auto& a) {
            std::ostringstream ss;
            ss << (a.kind == kLoad ? "loaded " : "stored ") << a.size << " bytes at offset "
               << a.offset << std::endl
               << "while running local_invocation_id" << a.invocation.Str() << " workgroup_id"
               << a.workgroup.Str();
            error.add_note(diag::System::Interpreter, ss.str(), a.cause->source);
        };
        add_note(race.a);
        add_note(race.b);
