...
void DataRaceDetector::Barrier(const Workgroup* workgroup, const ast::CallExpression* call) {
    ...
    switch (builtin->Type()) {
        case sem::BuiltinType::kStorageBarrier:
            SyncWorkgroup(workgroup, builtin::AddressSpace::kStorage);
            break;
        case sem::BuiltinType::kWorkgroupBarrier:
        case sem::BuiltinType::kWorkgroupUniformLoad:
            SyncWorkgroup(workgroup, builtin::AddressSpace::kWorkgroup);
            break;
        default:
...
void DataRaceDetector::RegisterAccess(const MemoryView* view, MemoryAccessKind kind) {
    ...
    if (addrspace != builtin::AddressSpace::kStorage &&
        addrspace != builtin::AddressSpace::kWorkgroup) {
...
void DataRaceDetector::SyncWorkgroup(const Workgroup* workgroup, builtin::AddressSpace addrspace) {
    ...
    for (auto& access_map : per_invocation_accesses_[workgroup->GroupId()]) {
        if (addrspace == builtin::AddressSpace::kWorkgroup) {
        ...
        } else if (addrspace == builtin::AddressSpace::kStorage) {
    ...
    for (auto& itr : races_) {
        ...
        // Show the workgroup or storage buffer declaration that is being accessed.
        {
            ...
            ss << "data race detected on accesses to "
               << (race.root_view->AddressSpace() == builtin::AddressSpace::kStorage
                       ? "storage buffer"
                       : "workgroup variable");
