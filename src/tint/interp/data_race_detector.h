...
/// The DataRaceDetector class implements dynamic data race detector that works by tracking memory
...
class DataRaceDetector {
  ...
  private:
    ...
    enum MemoryAccessKind { kLoad, kStore };
    ...
    struct MemoryAccess {
        ...
        uint64_t offset;         // The offset in bytes from the root of the memory allocation.
        uint64_t size;           // The size in bytes of the access.
    ...
    struct MemoryAccessHasher {
        size_t operator()(const MemoryAccessKey& key) const {
            return utils::Hash(key.view, key.offset);
        }
    };
    ...
    /// Merge an memory access into an access map, optionally check for data races.
    ...
    void SyncWorkgroup(const Workgroup* workgroup, builtin::AddressSpace addrspace);
    ...
    struct RaceHasher {
        size_t operator()(const RaceKey& key) const {
            return utils::Hash(key.decl.line, key.decl.column, key.cause_a, key.cause_b);
        }
    };
