...
utils::Result<std::unique_ptr<ShaderExecutor>, std::string> ShaderExecutor::Create(
    ...
    auto result = executor->Init(program, entry_point, std::move(overrides));
    if (!result) {
...
ShaderExecutor::Result ShaderExecutor::Init(const tint::Program& program,
                                            std::string entry_point,
                                            NamedOverrideList&& overrides) {
...
ShaderExecutor::Result ShaderExecutor::EvaluateNamedOverride(Invocation& override_evaluator,
    ...
    if (overrides.count(key)) {
        ...
        if (result == nullptr) {
            // TODO: This should be non-fatal and done at pipeline creation time so that we
