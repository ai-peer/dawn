...
ShaderExecutor::Result ShaderExecutor::Init(std::string entry_point,
    ...
    auto get_wgsize_dim = [&](const ast::Expression* ast) {
        ...
        if (expr->Stage() == sem::EvaluationStage::kOverride) {
...
ShaderExecutor::Result ShaderExecutor::EvaluateNamedOverride(Invocation& override_evaluator,
    ...
    if (overrides.count(key)) {
        ...
        auto* result = Switch(
            ...
            [&](const type::I32*) { return builder_.constants.Get(i32(value)); },
            [&](const type::U32*) { return builder_.constants.Get(u32(value)); },
            [&](const type::F32*) { return builder_.constants.Get(f32(value)); });
        if (result == nullptr) {
