...
TEST_F(InvocationTest, Memory_ArrayStoreThroughView) {
    ...
    utils::Vector<const constant::Value*, 4> elements = {
        executor_->Builder().constants.Get(i32(42)),
        executor_->Builder().constants.Get(i32(-1007361)),
        executor_->Builder().constants.Get(i32(20222022)),
        executor_->Builder().constants.Get(i32(-1)),
    };
...
TEST_F(InvocationTest, Memory_MatrixStoreThroughView) {
    ...
    for (uint32_t j = 0; j < 3; j++) {
        ...
        for (uint32_t i = 0; i < 3; i++) {
            values.Push(executor_->Builder().constants.Get(f32(j * 3 + i)));
        }
