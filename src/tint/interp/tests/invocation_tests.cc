...
fn main() {
  ...
  let v_f = bitcast<vec2<f32>>(v);
    ...
    ContinueToEnd();
    CHECK_VALUE("a_i", std::to_string(i32(0x40000042)));
    CHECK_VALUE("a_u", std::to_string(u32(0x40000042)));
    CHECK_VALUE("a_f", "2.000016");
    CHECK_VALUE("b_i", std::to_string(i32(0xc2478000)));
    CHECK_VALUE("b_u", std::to_string(u32(0xc2478000)));
    CHECK_VALUE("b_f", "-49.875000");
    CHECK_VALUE("c_i", std::to_string(i32(0xc4479000)));
    CHECK_VALUE("c_u", std::to_string(u32(0xc4479000)));
...
// TODO: Need to execute initializer in its own scope.
...
TEST_F(InvocationTest, Memory_LoadThroughView) {
    ...
    alloc.Store(&value1, 0, 4);
    alloc.Store(&value2, 4, 4);

    auto* i32 = executor_->Builder().create<type::I32>();
    auto* view1 = alloc.CreateView(executor_.get(), type::AddressSpace::kPrivate, i32, 0, 4, {});
    auto* view2 = alloc.CreateView(executor_.get(), type::AddressSpace::kPrivate, i32, 4, 4, {});
...
TEST_F(InvocationTest, Memory_StoreThroughView) {
    ...
    auto* i32 = executor_->Builder().create<type::I32>();
    constant::Scalar value1(i32, AInt(42));
    constant::Scalar value2(i32, AInt(-1007361));
    auto* view1 = alloc.CreateView(executor_.get(), type::AddressSpace::kPrivate, i32, 0, 4, {});
    auto* view2 = alloc.CreateView(executor_.get(), type::AddressSpace::kPrivate, i32, 4, 4, {});
    view1->Store(&value1);
    view2->Store(&value2);
...
TEST_F(InvocationTest, Memory_ArrayStoreThroughView) {
    ...
    auto* i32_ty = executor_->Builder().create<type::I32>();
    type::ConstantArrayCount count(4);
    auto* arr_ty = executor_->Builder().create<type::Array>(i32_ty, &count, 4u, 16u, 4u, 4u);
    utils::Vector<constant::Value*, 4> elements = {
        executor_->Builder().create<constant::Scalar<i32>>(i32_ty, i32(42)),
        executor_->Builder().create<constant::Scalar<i32>>(i32_ty, i32(-1007361)),
        executor_->Builder().create<constant::Scalar<i32>>(i32_ty, i32(20222022)),
        executor_->Builder().create<constant::Scalar<i32>>(i32_ty, i32(-1)),
    };
    auto* view = alloc.CreateView(executor_.get(), type::AddressSpace::kPrivate, arr_ty, {});
    view->Store(executor_->ConstEval().ArrayOrStructInit(arr_ty, elements).Get());
...
TEST_F(InvocationTest, Memory_ArrayStoreThroughView_Strided) {
    ...
    auto* i32_ty = executor_->Builder().create<type::I32>();
    type::ConstantArrayCount count(4);
    auto* arr_ty = executor_->Builder().create<type::Array>(i32_ty, &count, 4u, 32u, 8u, 4u);
    utils::Vector<constant::Value*, 4> elements = {
        executor_->Builder().create<constant::Scalar<i32>>(i32_ty, i32(42)),
        executor_->Builder().create<constant::Scalar<i32>>(i32_ty, i32(-1007361)),
        executor_->Builder().create<constant::Scalar<i32>>(i32_ty, i32(20222022)),
...
TEST_F(InvocationTest, Memory_MatrixLoadThroughView) {
    ...
    auto* mat3x3 = executor_->Builder().create<type::Matrix>(
        executor_->Builder().create<type::Vector>(executor_->Builder().create<type::F32>(), 3u),
        3u);
    auto* view = alloc.CreateView(executor_.get(), type::AddressSpace::kPrivate, mat3x3, 0, 48, {});
...
TEST_F(InvocationTest, Memory_MatrixStoreThroughView) {
    ...
    auto* f32_ty = executor_->Builder().create<type::F32>();
    auto* vec3 = executor_->Builder().create<type::Vector>(f32_ty, 3u);
    auto* mat3x3 = executor_->Builder().create<type::Matrix>(vec3, 3u);
    utils::Vector<const constant::Value*, 4> columns;
    for (uint32_t j = 0; j < 3; j++) {
        utils::Vector<const constant::Value*, 4> values;
        for (uint32_t i = 0; i < 3; i++) {
            values.Push(executor_->Builder().create<constant::Scalar<f32>>(f32_ty, f32(j * 3 + i)));
        }
        columns.Push(executor_->Builder().create<constant::Composite>(vec3, values));
    }
    auto* matrix = executor_->Builder().create<constant::Composite>(mat3x3, columns);
    ...
    for (uint32_t j = 0; j < 3; j++) {
        for (uint32_t i = 0; i < 3; i++) {
            // std::cout << loaded[j * 4 + i] << std::endl;
