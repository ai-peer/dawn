...
fn main(@builtin(local_invocation_index) idx : u32) {
  buffer[idx] = buffer[idx] + 1e38;
    ...
    EXPECT_EQ(
        errors_,
        R"(test.wgsl:6:29 warning: '250000007218949514365393469883371487232.0 + 99999996802856924650656260769173209088.0' cannot be represented as 'f32'
  buffer[idx] = buffer[idx] + 1e38;
                            ^
...
fn main(@builtin(local_invocation_index) idx : u32) {
  buffer[idx] = 1.f / buffer[idx];
    ...
    EXPECT_EQ(errors_,
              R"(test.wgsl:6:21 warning: '1.0 / 0.0' cannot be represented as 'f32'
  buffer[idx] = 1.f / buffer[idx];
                    ^
...
fn main() {
  ...
  var f = sqrt(c);
    ...
    EXPECT_EQ(errors_,
              R"(test.wgsl:6:13 warning: '1.0 / 0.0' cannot be represented as 'f32'
  var c = a / b;
            ^
