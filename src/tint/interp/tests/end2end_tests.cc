...
class ComputeEndToEndTestBase {
  protected:
    ...
    void Init(std::string source, NamedOverrideList&& overrides = {}) {
        ...
        auto diag_printer = diag::Printer::create(stderr, true);
        diag::Formatter diag_formatter;
        ASSERT_TRUE(program_) << "Failed to run WGSL parser.";
        if (program_->Diagnostics().count() > 0) {
            diag_formatter.format(program_->Diagnostics(), diag_printer.get());
        }
    ...
    bool RunShader(UVec3 group_count, BindingList&& bindings, bool expect_errors = false) {
        if (!executor_->Run(group_count, std::move(bindings))) {
            return false;
        }
        if (expect_errors) {
            if (errors_.empty()) {
    ...
    std::unique_ptr<Memory> MakeBuffer(std::array<T, N> values) {
        ...
        for (size_t i = 0; i < N; i++) {
            buffer->Store(&values[i], i * sizeof(T), sizeof(T));
        }
        return buffer;
    }
    ...
    bool CheckEqual(Memory& buffer, uint64_t offset, T value) {
        T result;
        buffer.Load(&result, offset, sizeof(T));
...
fn foo() {
  _ = f;
    ...
    ASSERT_TRUE(executor_->Run(UVec3(1, 1, 1), std::move(bindings)));
...
fn main(@builtin(local_invocation_index) idx : u32) {
  output[idx] = values[idx];
    ...
    EXPECT_EQ(errors_,
              R"(test.wgsl:8:23 warning: index 3 is out of bounds
  output[idx] = values[idx];
                      ^
...
fn main(@builtin(local_invocation_index) idx : u32) {
  output[idx] = input[idx].x;
    ...
    EXPECT_EQ(errors_,
              R"(test.wgsl:7:28 warning: loading from an out-of-bounds memory view
  output[idx] = input[idx].x;
                           ^
...
test.wgsl:7:22 note: created a 16 byte memory view at an offset of 48 bytes
  output[idx] = input[idx].x;
                     ^
...
fn main(@builtin(local_invocation_index) idx : u32) {
  output[idx] = input[idx];
    ...
    EXPECT_EQ(errors_,
              R"(test.wgsl:7:22 warning: loading from an out-of-bounds memory view
  output[idx] = input[idx];
                     ^
...
test.wgsl:7:22 note: created a 4 byte memory view at an offset of 12 bytes
  output[idx] = input[idx];
                     ^
...
test.wgsl:7:9 note: created a 4 byte memory view at an offset of 12 bytes
  output[idx] = input[idx];
        ^
...
fn main(@builtin(local_invocation_index) idx : u32) {
  ...
  output[idx] = input[idx];
    ...
    EXPECT_EQ(errors_,
              R"(test.wgsl:8:22 warning: loading from an out-of-bounds memory view
  output[idx] = input[idx];
                     ^
...
test.wgsl:8:22 note: created a 4 byte memory view at an offset of 12 bytes
  output[idx] = input[idx];
                     ^
...
test.wgsl:6:8 note: created a 4 byte memory view at an offset of 12 bytes
  input[idx] = idx;
       ^
...
test.wgsl:5:8 note: created a 4 byte memory view at an offset of 12 bytes
  input[idx] = idx;
       ^
...
fn main(@builtin(local_invocation_index) idx : u32) {
  ...
  output[idx] = input[idx];
    ...
    EXPECT_EQ(errors_,
              R"(test.wgsl:11:22 warning: loading from an out-of-bounds memory view
  output[idx] = input[idx];
                     ^
...
fn main(@builtin(local_invocation_index) idx : u32) {
  ...
  output[idx] = foo(&input[idx]);
    ...
    EXPECT_EQ(errors_,
              R"(test.wgsl:7:14 warning: loading from an out-of-bounds memory view
  return 1 + *p;
             ^

test.wgsl:12:3 note: accessing 12 byte allocation in the function address space
  var input = array(1, 2, 3);
  ^^^^^^^^^

test.wgsl:13:27 note: created a 4 byte memory view at an offset of 12 bytes
  output[idx] = foo(&input[idx]);
                          ^
...
fn main(@builtin(local_invocation_index) idx : u32) {
  ...
  output[idx] = input[idx][0][0];
    ...
    EXPECT_EQ(errors_,
              R"(test.wgsl:7:30 warning: loading from an out-of-bounds memory view
  output[idx] = input[idx][0][0];
                             ^
...
test.wgsl:7:22 note: created a 24 byte memory view at an offset of 48 bytes
  output[idx] = input[idx][0][0];
                     ^
...
test.wgsl:7:28 note: created a 4 byte memory view at an offset of 12 bytes
  output[idx] = input[0][0][idx];
                           ^
...
test.wgsl:6:20 note: created a 4 byte memory view at an offset of 12 bytes
  atomicAdd(&output[idx], 1);
                   ^
...
fn main(@builtin(local_invocation_index) idx : u32) {
  var f : f32 = buffer[idx];
    ...
    EXPECT_EQ(errors_,
              R"(test.wgsl:6:23 warning: loading a non-finite f32 value (inf)
  var f : f32 = buffer[idx];
                      ^

test.wgsl:6:23 warning: loading a non-finite f32 value (nan)
  var f : f32 = buffer[idx];
                      ^
...
fn main(@builtin(local_invocation_index) idx : u32) {
  var f : f16 = buffer[idx];
    ...
    EXPECT_EQ(errors_,
              R"(test.wgsl:8:23 warning: loading a non-finite f16 value (inf)
  var f : f16 = buffer[idx];
                      ^

test.wgsl:8:23 warning: loading a non-finite f16 value (nan)
  var f : f16 = buffer[idx];
                      ^
...
fn main(@builtin(local_invocation_index) idx : u32) {
  buffer[idx] = buffer[idx] + 1e38;
    ...
    EXPECT_EQ(
        errors_,
        R"(test.wgsl:6:29 warning: '2.5000000721894951437e+38 + 9.9999996802856924651e+37' cannot be represented as 'f32'
  buffer[idx] = buffer[idx] + 1e38;
                            ^
...
fn main(@builtin(local_invocation_index) idx : u32) {
  buffer[idx] = 1.f / buffer[idx];
    ...
    EXPECT_EQ(errors_,
              R"(test.wgsl:6:21 warning: '1 / 0' cannot be represented as 'f32'
  buffer[idx] = 1.f / buffer[idx];
                    ^
...
fn main(@builtin(local_invocation_index) idx : u32) {
  buffer[idx] = 100 / buffer[idx];
    ...
    EXPECT_EQ(errors_,
              R"(test.wgsl:6:21 warning: '100 / 0' cannot be represented as 'u32'
  buffer[idx] = 100 / buffer[idx];
                    ^
...
fn main(@builtin(local_invocation_index) idx : u32) {
  buffer[idx] = quantizeToF16(buffer[idx]);
    ...
    EXPECT_EQ(errors_,
              R"(test.wgsl:6:17 warning: value 65505 cannot be represented as 'f16'
  buffer[idx] = quantizeToF16(buffer[idx]);
                ^^^^^^^^^^^^^^^^^^^^^^^^^^
...
fn main() {
  ...
  var f = sqrt(c);
    ...
    EXPECT_EQ(errors_,
              R"(test.wgsl:6:13 warning: '1 / 0' cannot be represented as 'f32'
  var c = a / b;
            ^
