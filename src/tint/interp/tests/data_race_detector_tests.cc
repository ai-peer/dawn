...
test.wgsl:11:19 note: loaded 4 bytes at offset 4
while running local_invocation_id(2,0,0) workgroup_id(0,0,0)
    let x = wgvar.b;
                  ^
)");
...
test.wgsl:14:19 note: loaded 4 bytes at offset 4
while running local_invocation_id(1,0,0) workgroup_id(0,0,0)
    let x = wgvar.b;
                  ^
)");
...
test.wgsl:12:19 note: loaded 4 bytes at offset)"));
    EXPECT_THAT(errors_, testing::HasSubstr(R"(workgroup_id(0,0,0)
      sum += wgvar[j];
                  ^
)"));
...
test.wgsl:10:18 note: loaded 4 bytes at offset)"));
    EXPECT_THAT(errors_, testing::HasSubstr(R"(workgroup_id(0,0,0)
    let x = wgvar[i] + 10;
                 ^
)"));
...
fn main(@builtin(local_invocation_index) idx : u32) {
  ...
  if (idx == 1) {
    ...
    EXPECT_EQ(errors_,
              R"(
test.wgsl:4:48 warning: data race detected on accesses to storage buffer
@group(0) @binding(0) var<storage, read_write> buffer : u32;
                                               ^^^^^^
...
test.wgsl:12:21 note: loaded 2 bytes at offset 4
while running local_invocation_id(0,0,0) workgroup_id(0,0,0)
      let x = wgvar1[2];
                    ^

test.wgsl:4:16 warning: data race detected on accesses to workgroup variable
var<workgroup> wgvar2 : array<bool, 4>;
               ^^^^^^
...
test.wgsl:11:16 note: loaded 4 bytes at offset 4
while running local_invocation_id(0,0,0) workgroup_id(0,0,0)
    if (!wgvar2[1]) {
               ^
)");
