...
Workgroup::Workgroup(ShaderExecutor& executor, UVec3 group_id, UVec3 size)
    ...
    TINT_ASSERT(Interpreter, result);
        ...
        if (first_barrier_count != invocations_.size()) {
            ...
            error.add_note(diag::System::Interpreter,
                           "invocation" + barrier_.front()->LocalInvocationId().Str() + " and " +
                               std::to_string(first_barrier_count - 1) +
            ...
            if (second_barrier != nullptr) {
                ...
                error.add_note(diag::System::Interpreter,
                               "invocation" + second_barrier_example->LocalInvocationId().Str() +
                                   " and " + std::to_string(second_barrier_count - 1) +
                                   " other invocations waiting here",
