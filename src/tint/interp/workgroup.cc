...
Workgroup::Workgroup(ShaderExecutor& executor, UVec3 group_id, UVec3 size)
    ...
    for (auto* global : func->TransitivelyReferencedGlobals()) {
        if (global->AddressSpace() == builtin::AddressSpace::kWorkgroup) {
            ...
            auto* view = alloc->CreateView(&executor_, builtin::AddressSpace::kWorkgroup,
    ...
    for (uint32_t iz = 0; iz < size[2]; iz++) {
        for (uint32_t iy = 0; iy < size[1]; iy++) {
            for (uint32_t ix = 0; ix < size[0]; ix++) {
    ...
    TINT_ASSERT(Interpreter, SelectInvocation({0, 0, 0}));
...
void Workgroup::Step() {
    TINT_ASSERT(Interpreter, current_invocation_ != nullptr);
    ...
    if (state == Invocation::kFinished) {
        current_invocation_ = nullptr;
    } else if (state == Invocation::kBarrier) {
        barrier_.push_back(current_invocation_);
        current_invocation_ = nullptr;
    } else if (state != Invocation::kReady) {
    ...
    if (current_invocation_ == nullptr) {
        if (ready_.empty() && !barrier_.empty()) {
            ...
            // Clear barriers for every invocation that is waiting, and move them to the ready
            // queue. Track the number of invocations that hit the same barrier while we do this.
            ...
            if (first_barrier_count != invocations_.size()) {
                diag::List error;
                error.add_error(diag::System::Interpreter,
                                "barrier not reached by all invocations in the workgroup\n", {});
                error.add_note(diag::System::Interpreter,
                               "invocation" + barrier_.front()->LocalInvocationId().Str() +
                                   " and " + std::to_string(first_barrier_count - 1) +
                                   " other invocations waiting here",
                               first_barrier->source);
                if (second_barrier != nullptr) {
                    // Show an example of an invocation that is waiting at a different barrier.
                    error.add_note(diag::System::Interpreter,
                                   "invocation" +
                                       second_barrier_example->LocalInvocationId().Str() + " and " +
                                       std::to_string(second_barrier_count - 1) +
                                       " other invocations waiting here",
                                   second_barrier->source);
                }
                auto other_barrier_count =
                    barrier_.size() - first_barrier_count - second_barrier_count;
                if (other_barrier_count > 0) {
                    // Note how many invocations are waiting are other barriers.
                    error.add_note(diag::System::Interpreter,
                                   std::to_string(other_barrier_count) +
                ...
                if (barrier_.size() != invocations_.size()) {
                    ...
                    error.add_note(diag::System::Interpreter,
                                   std::to_string(invocations_.size() - barrier_.size()) +
        ...
        if (!ready_.empty()) {
            ...
            TINT_ASSERT(Interpreter, SelectInvocation(ready_.begin()->first));
        } else {
