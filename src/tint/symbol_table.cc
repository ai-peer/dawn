...
Symbol SymbolTable::Register(const std::string_view& name) {
...
Symbol SymbolTable::RegisterInternal(const std::string_view& name) {
    ...
    ++next_symbol_;
    name_to_symbol_.Add(nv, sym);

    return sym;
}

Symbol SymbolTable::Get(const std::string_view& name) const {
    auto it = name_to_symbol_.Find(name);
    return it ? *it : Symbol();
}

Symbol SymbolTable::New(std::string_view prefix /* = "" */) {
    if (prefix.empty()) {
        prefix = std::string_view("tint_symbol");
    }
    ...
    std::string name;
    std::string prefix_name(prefix);
    do {
        ++i;
        name = prefix_name + "_" + std::to_string(i);
    ...
    if (last_prefix) {
        *last_prefix = i;
        ...
        last_prefix_to_index_.Add(sym.Name(), i);
    }
    return sym;
}
