...
class ProgramBuilder {
    ...
    struct OverrideOptions {
        ...
        const ast::Identifier* type = nullptr;
        const ast::Expression* initializer = nullptr;
        utils::Vector<const ast::Attribute*, 4> attributes;

      private:
        void Set(const ast::Identifier* t) { type = t; }
  ...
  public:
      ...
      public:
        ...
        const ast::TemplatedIdentifier* array(
            ...
            if constexpr (std::is_same_v<traits::Decay<COUNT>, std::nullptr_t>) {
                return builder->Ident(source, "array", subtype);
            } else {
                if (n) {
                    return builder->Ident(source, "array", subtype);
                } else {
                    return builder->Ident(source, "array", subtype, std::forward<COUNT>(n));
                }
            }
        }
        ...
        const ast::TemplatedIdentifier* array(const ast::Identifier* subtype,
                                              EXPR&& n,
        ...
        /// @return the identifier for an array of size `n` of type `T`
        template <typename EXPR>
        const ast::TemplatedIdentifier* array(const Source& source,
                                              const ast::Identifier* subtype,
                                              EXPR&& n,
                                              uint32_t stride) const {
            utils::Vector<const ast::Attribute*, 2> attrs;
        ...
        const ast::Identifier* array() const {
            return array(Of<T>(), builder->Expr(builder->source_, tint::u32(N)));
        }

        /// @param stride the array stride
        /// @return the identifier for a runtime-sized array of type `T`
        template <typename T>
        const ast::Identifier* array(uint32_t stride) const {
        ...
        template <typename T, int N>
        const ast::Identifier* array(uint32_t stride) const {
            return array(Of<T>(), builder->Expr(builder->source_, tint::u32(N)), stride);
        }
        ...
        const ast::Alias* alias(NAME&& name, const ast::Identifier* type) const {
        ...
        const ast::Identifier* pointer(const ast::Identifier* type,
        ...
        const ast::Identifier* pointer(const Source& source,
            ...
            if (access != type::Access::kUndefined) {
                return builder->Ident(source, "ptr", type, address_space, access);
            } else {
                return builder->Ident(source, "ptr", type, address_space);
            }
        }
        ...
        /// @param type the type
        /// @return either type or a pointer to a new ast::Type
        const ast::Type* Of(const ast::TypeDecl* type) const;
    ...
    /// @return an ast::Type with the given identifier
    const ast::Type* Type(const ast::Identifier* ident) {
        return create<ast::Type>(ident->source, ident);
    }
    ...
    const ast::IdentifierExpression* Expr(const Source& source, const IDENTIFIER* ident) {
        static_assert(!traits::IsType<IDENTIFIER, ast::TemplatedIdentifier>,
                      ...
                      "an identifier expression, as this should parse as an ast::Type");
    ...
    /// `expr`.
    template <typename EXPR>
    const ast::BitcastExpression* Bitcast(const ast::Identifier* type, EXPR&& expr) {
    ...
    template <typename... ARGS>
    const ast::CallExpression* vec(const ast::Identifier* type, uint32_t size, ARGS&&... args) {
    ...
    const ast::Function* Func(
        ...
        auto* func = create<ast::Function>(
            source, Sym(std::forward<NAME>(name)), std::move(params),
    ...
    const ast::Function* Func(
        ...
        utils::VectorRef<const ast::Attribute*> body_attributes = utils::Empty) {
        auto* func = create<ast::Function>(
            Sym(std::forward<NAME>(name)), std::move(params), Type(std::forward<TYPE>(type)),
            create<ast::BlockStatement>(std::move(body), std::move(body_attributes)),
            std::move(attributes), std::move(return_type_attributes));
        AST().AddFunction(func);
        return func;
    ...
    template <typename NAME>
    const ast::Alias* Alias(const Source& source, NAME&& name, const ast::Identifier* type) {
    ...
    /// @returns the alias type
    template <typename NAME>
    const ast::Alias* Alias(NAME&& name, const ast::Identifier* type) {
