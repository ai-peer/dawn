...
class ProgramBuilder {
    ...
    /// Evaluates to true if T is a scalar.
    template <typename T>
    ...
    /// avoid ambiguities in overloads that take a Source as the first parameter and those that
    /// perfectly-forward the first argument.
    template <typename... TYPES>
  ...
  public:
    ...
    class TypesBuilder {
      public:
        ...
        const ast::Alias* alias(const Source& source, NAME&& name, TYPE&& type) const {
            return builder->create<ast::Alias>(source, builder->Sym(std::forward<NAME>(name)),
    ...
    const ast::Parameter* Param(const Source& source,
                                ...
                                TYPE&& type,
                                utils::VectorRef<const ast::Attribute*> attributes = utils::Empty) {
        return create<ast::Parameter>(source, Sym(std::forward<NAME>(name)),
    ...
    const ast::Function* Func(
        ...
        utils::VectorRef<const ast::Attribute*> return_type_attributes = utils::Empty) {
        auto* func =
            create<ast::Function>(source, Sym(std::forward<NAME>(name)), std::move(params),
                                  Type(std::forward<TYPE>(type)), Block(std::forward<BODY>(body)),
    ...
    const ast::Alias* Alias(NAME&& name, TYPE&& type) {
        auto* out = ty.alias(std::forward<NAME>(name), std::forward<TYPE>(type));
        AST().AddTypeDecl(out);
        return out;
    }
