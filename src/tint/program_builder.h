  ...
  public:
    ...
    class TypesBuilder {
      public:
        ...
        /// @return the identifier for a `n`-element vector of `type`.
        template <typename T>
        const ast::TemplatedIdentifier* vec(uint32_t n) const {
            return vec(Of<T>(), n);
        }

        /// @return the identifier for a 2-element vector of the type `T`.
        template <typename T>
        const ast::TemplatedIdentifier* vec2() const {
            return vec2(Of<T>());
        }
        ...
        /// @return the identifier for a 2-element vector of the type `T`.
        template <typename T>
        const ast::TemplatedIdentifier* vec2(const Source& source) const {
            return vec2(source, Of<T>());
        }

        /// @return the identifier for a 3-element vector of the type `T`.
        template <typename T>
        const ast::TemplatedIdentifier* vec3() const {
            return vec3(Of<T>());
        }
        ...
        /// @return the identifier for a 3-element vector of the type `T`.
        template <typename T>
        const ast::TemplatedIdentifier* vec3(const Source& source) const {
            return vec3(source, Of<T>());
        }

        /// @return the identifier for a 4-element vector of the type `T`.
        template <typename T>
        const ast::TemplatedIdentifier* vec4() const {
            return vec4(Of<T>());
        }
        ...
        /// @return the identifier for a 4-element vector of the type `T`.
        template <typename T>
        const ast::TemplatedIdentifier* vec4(const Source& source) const {
        ...
        /// @return the identifier for a matrix of `type`
        template <typename T>
        const ast::TemplatedIdentifier* mat(uint32_t columns, uint32_t rows) const {
            return mat(Of<T>(), columns, rows);
        }

        /// @return the identifier for a 2x3 matrix of the type `T`.
        template <typename T>
        const ast::TemplatedIdentifier* mat2x2() const {
            return mat2x2(Of<T>());
        }

        /// @return the identifier for a 2x3 matrix of the type `T`.
        template <typename T>
        const ast::TemplatedIdentifier* mat2x3() const {
            return mat2x3(Of<T>());
        }

        /// @return the identifier for a 2x4 matrix of the type `T`.
        template <typename T>
        const ast::TemplatedIdentifier* mat2x4() const {
            return mat2x4(Of<T>());
        }

        /// @return the identifier for a 3x2 matrix of the type `T`.
        template <typename T>
        const ast::TemplatedIdentifier* mat3x2() const {
            return mat3x2(Of<T>());
        }

        /// @return the identifier for a 3x3 matrix of the type `T`.
        template <typename T>
        const ast::TemplatedIdentifier* mat3x3() const {
            return mat3x3(Of<T>());
        }

        /// @return the identifier for a 3x4 matrix of the type `T`.
        template <typename T>
        const ast::TemplatedIdentifier* mat3x4() const {
            return mat3x4(Of<T>());
        }

        /// @return the identifier for a 4x2 matrix of the type `T`.
        template <typename T>
        const ast::TemplatedIdentifier* mat4x2() const {
            return mat4x2(Of<T>());
        }

        /// @return the identifier for a 4x3 matrix of the type `T`.
        template <typename T>
        const ast::TemplatedIdentifier* mat4x3() const {
            return mat4x3(Of<T>());
        }

        /// @return the identifier for a 4x4 matrix of the type `T`.
        template <typename T>
        const ast::TemplatedIdentifier* mat4x4() const {
            return mat4x4(Of<T>());
        }
        ...
        const ast::TemplatedIdentifier* array(
            ...
            utils::VectorRef<const ast::Attribute*> attrs = utils::Empty) const {
            if (!attrs.IsEmpty()) {
                TINT_UNIMPLEMENTED(ProgramBuilder, builder->Diagnostics())
                    << "TODO(crbug.com/tint/1810)";
            }
            if constexpr (std::is_same_v<traits::Decay<COUNT>, std::nullptr_t>) {
                return builder->Ident(source, "array", std::forward<TYPE>(subtype));
            } else {
                if (n) {
                    return builder->Ident(source, "array", std::forward<TYPE>(subtype),
                                          std::forward<COUNT>(n));
        ...
        /// @param attrs the optional attributes for the array
        /// @return the identifier for a runtime-sized array of type `T`
        template <typename T>
    ...
    const ast::Type* Type(NAME&& name) {
        ...
        return ident ? create<ast::Type>(ident) : nullptr;
    }
    ...
    const ast::IdentifierExpression* Expr(const Source& source, const IDENTIFIER* ident) {
        return create<ast::IdentifierExpression>(source, ident);
    }
    ...
    const ast::CallExpression* vec2(const Source& source, ARGS&&... args) {
        if constexpr (std::is_same_v<T, Infer>) {
            return Call(source, "vec2", std::forward<ARGS>(args)...);
        } else {
            return vec2(source, ty.Of<T>(), std::forward<ARGS>(args)...);
        }
    }
    ...
    const ast::CallExpression* vec3(const Source& source, ARGS&&... args) {
        if constexpr (std::is_same_v<T, Infer>) {
            return Call(source, "vec3", std::forward<ARGS>(args)...);
        } else {
            return vec3(source, ty.Of<T>(), std::forward<ARGS>(args)...);
        }
    }
    ...
    const ast::CallExpression* vec4(const Source& source, ARGS&&... args) {
        if constexpr (std::is_same_v<T, Infer>) {
            return Call(source, "vec4", std::forward<ARGS>(args)...);
        } else {
            return vec4(source, ty.Of<T>(), std::forward<ARGS>(args)...);
        }
    }
    ...
    /// `T` and size `N`, constructed with the values @p args.
    ...
    const ast::CaseStatement* Case(const Source& source,
        ...
        return create<ast::CaseStatement>(source, std::move(selectors), body);
    }
    ...
    const ast::CaseStatement* Case(utils::VectorRef<const ast::CaseSelector*> selectors,
                                   const ast::BlockStatement* body = nullptr) {
        return create<ast::CaseStatement>(std::move(selectors), body);
    }
...
template <>
struct ProgramBuilder::TypesBuilder::CToAST<Infer> {
    static const ast::Identifier* get(const ProgramBuilder::TypesBuilder*) { return nullptr; }
};
template <>
