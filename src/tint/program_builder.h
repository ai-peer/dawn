  ...
  public:
    ...
    class TypesBuilder {
      public:
        ...
        const ast::TemplatedIdentifier* vec(const ast::Identifier* type, uint32_t n) const {
        ...
        /// @param n vector width in elements
        /// @return the identifier for a `n`-element vector of `type`.
        const ast::TemplatedIdentifier* vec(const Source& source,
                                            const ast::Identifier* type,
                                            uint32_t n) const {
            return builder->Ident(source, "vec", type, AInt(n));
        }

        /// @param type vector subtype
        /// @return the identifier for a 2-element vector of `type`.
        const ast::TemplatedIdentifier* vec2(const ast::Identifier* type) const {
            return vec(type, 2u);
        }
        ...
        /// @param type vector subtype
        /// @return the identifier for a 2-element vector of `type`.
        const ast::TemplatedIdentifier* vec2(const Source& source,
                                             const ast::Identifier* type) const {
            return vec(source, type, 2u);
        }

        /// @param type vector subtype
        /// @return the identifier for a 3-element vector of `type`.
        const ast::TemplatedIdentifier* vec3(const ast::Identifier* type) const {
            return vec(type, 3u);
        }
        ...
        /// @param type vector subtype
        /// @return the identifier for a 3-element vector of `type`.
        const ast::TemplatedIdentifier* vec3(const Source& source,
                                             const ast::Identifier* type) const {
        ...
        const ast::TemplatedIdentifier* vec4(const ast::Identifier* type) const {
            return vec(type, 4u);
        }
        ...
        /// @param type vector subtype
        /// @return the identifier for a 4-element vector of `type`.
        const ast::TemplatedIdentifier* vec4(const Source& source,
                                             const ast::Identifier* type) const {
        ...
        /// @return the identifier for a 2-element vector of the C type `T`.
        template <typename T>
        ...
        /// @return the identifier for a 3-element vector of the C type `T`.
        template <typename T>
        ...
        /// @return the identifier for a 4-element vector of the C type `T`.
        template <typename T>
        ...
        /// @return the identifier for a 2x3 matrix of the C type `T`.
        template <typename T>
        ...
        /// @return the identifier for a 2x4 matrix of the C type `T`.
        template <typename T>
        const ast::TemplatedIdentifier* mat2x4() const {
            return mat2x4(Of<T>());
        }

        /// @return the identifier for a 3x2 matrix of the C type `T`.
        template <typename T>
        const ast::TemplatedIdentifier* mat3x2() const {
            return mat3x2(Of<T>());
        }

        /// @return the identifier for a 3x3 matrix of the C type `T`.
        template <typename T>
        const ast::TemplatedIdentifier* mat3x3() const {
            return mat3x3(Of<T>());
        }

        /// @return the identifier for a 3x4 matrix of the C type `T`.
        template <typename T>
        const ast::TemplatedIdentifier* mat3x4() const {
            return mat3x4(Of<T>());
        }

        /// @return the identifier for a 4x2 matrix of the C type `T`.
        template <typename T>
        const ast::TemplatedIdentifier* mat4x2() const {
            return mat4x2(Of<T>());
        }

        /// @return the identifier for a 4x3 matrix of the C type `T`.
        template <typename T>
        const ast::TemplatedIdentifier* mat4x3() const {
            return mat4x3(Of<T>());
        }

        /// @return the identifier for a 4x4 matrix of the C type `T`.
        template <typename T>
        ...
        const ast::Identifier* pointer(const Source& source,
            ...
            if (access != type::Access::kUndefined) {
                return builder->Ident(source, "ptr", builder->Ident(std::forward<TYPE>(type)),
        ...
        const ast::Identifier* pointer(const Source& source,
                                       ...
                                       type::Access access = type::Access::kUndefined) const {
            if (access != type::Access::kUndefined) {
                return builder->Ident(source, "ptr", Of<T>(), address_space, access);
            } else {
                return builder->Ident(source, "ptr", Of<T>(), address_space);
            }
        }
...
#if 0
        const ast::Identifier* storage_texture_format(type::TexelFormat format) {
            return storage_texture_format(builder->source_, format);
        }

        const ast::Identifier* storage_texture_format(const Source& source,
                                                      type::TexelFormat format) {
            switch (format) {
                ...
                default:
                    TINT_ICE(ProgramBuilder, builder->Diagnostics())
                        << "invalid storage_texture_format format: " << format;
                    return nullptr;
            }
        }
#endif
        ...
        const ast::Identifier* Of(const ast::TypeDecl* type) const {
            return builder->Ident(type->name);
        }
    ...
    template <typename IDENTIFIER, typename = DisableIfSource<IDENTIFIER>>
    ...
    /// @param source the source information
    /// @param identifier the identifier symbol
    /// @param args templated identifier arguments
    ...
    const ast::TemplatedIdentifier* Ident(const Source& source,
        ...
        return create<ast::TemplatedIdentifier>(source, sym, ExprList(std::forward<ARGS>(args)...));
    }
    ...
    const ast::Type* Type(NAME&& name) {
        return create<ast::Type>(Ident(std::forward<NAME>(name)));
    }
    ...
    const ast::CallExpression* vec2(const Source& source, ARGS&&... args) {
        return vec2(source, ty.vec2<T>(), std::forward<ARGS>(args)...);
    }
    ...
    const ast::CallExpression* vec2(const Source& source, TYPE&& type, ARGS&&... args) {
        return Call(source, std::forward<TYPE>(type), std::forward<ARGS>(args)...);
    }
    ...
    const ast::CallExpression* vec3(const Source& source, ARGS&&... args) {
        return vec3(source, ty.vec3<T>(), std::forward<ARGS>(args)...);
    }
    ...
    const ast::CallExpression* vec3(const Source& source, TYPE&& type, ARGS&&... args) {
        return Call(source, Type(std::forward<TYPE>(type)), std::forward<ARGS>(args)...);
    }
    ...
    const ast::CallExpression* vec4(const Source& source, ARGS&&... args) {
        return vec4(source, ty.vec4<T>(), std::forward<ARGS>(args)...);
    }
    ...
    /// @param source the source information
    /// @param name the call target name
    /// @param args the function call arguments
    /// @returns an ast::CallExpression to the target @p name, with the arguments of @p args
    /// converted to `ast::Expression`s using `Expr()`.
    template <typename NAME, typename... ARGS>
    const ast::CallExpression* Call(const Source& source, NAME&& name, ARGS&&... args) {
        return Call(source, std::forward<NAME>(name), std::forward<ARGS>(args)...);
    }
    ...
    const ast::CallExpression* Call(NAME&& name, ARGS&&... args) {
        return create<ast::CallExpression>(Ident(std::forward<NAME>(name)),
