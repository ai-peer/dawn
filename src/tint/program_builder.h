...
class ProgramBuilder {
    ...
    /// A helper used to disable overloads if the first type in `TYPES` is not identifier-like.
    template <typename... TYPES>
      ...
      public:
        ...
        const auto* vec2(const Source& source) const {
            if constexpr (IsInferOrAbstract<T>) {
                ...
                return builder->Ident(source, "vec2", Of<T>());
            }
        }
        ...
        const auto* vec3(const Source& source) const {
            if constexpr (IsInferOrAbstract<T>) {
                ...
                return builder->Ident(source, "vec3", Of<T>());
            }
        }
        ...
        const auto* vec4(const Source& source) const {
            if constexpr (IsInferOrAbstract<T>) {
                ...
                return builder->Ident(source, "vec4", Of<T>());
            }
        }
        ...
        /// @return the identifier for a `n`-element vector of `type`.
        template <typename T>
        const auto* vec(const Source& source, uint32_t n) const {
            switch (n) {
                case 2:
        ...
        /// @return the identifier for a matrix of `type`
        template <typename T>
        const auto* mat(const Source& source, uint32_t columns, uint32_t rows) const {
        ...
        const ast::Identifier* array(const Source& source) const {
            return builder->create<ast::Identifier>(source, builder->Sym("array"));
        }

        /// @return the identifier for a inferred-size or runtime-sized array of type `T`
        template <typename T, typename = EnableIfInferOrAbstract<T>>
        const ast::Identifier* array() const {
        ...
        const ast::Identifier* array(
            ...
            utils::VectorRef<const ast::Attribute*> attrs = utils::Empty) const {
            return builder->create<ast::TemplatedIdentifier>(
                source, builder->Sym("array"), builder->ExprList(Of<T>()), std::move(attrs));
        }
        ...
        const ast::Identifier* sampler(const Source& source, type::SamplerKind kind) const {
            switch (kind) {
                ...
                case type::SamplerKind::kComparisonSampler:
                    return builder->Ident(source, "sampler_comparison");
            }
