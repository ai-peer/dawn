...
class UtilsHashmapPrinter(Printer):
    ...
    def initialize(self):
        self.hash_set = self.member('set_')
        self.hash_set_printer = UtilsHashsetPrinter(self.hash_set)
        # Replace the lookup function so we can extract the key and value out of the std::optionals in the Hashset
        self.hash_set_printer.try_read_std_optional_func = self.try_read_std_optional

    def update(self):
        self.hash_set_printer.update()

    def get_summary(self):
        return self.hash_set_printer.get_summary()

    def num_children(self):
        return self.hash_set_printer.num_children()

    def has_children(self):
        return self.hash_set_printer.has_children()

    def get_child_at_index(self, index):
        return self.hash_set_printer.get_child_at_index(index)
    ...
    def try_read_std_optional(self, slot, value):
        ...
        try:
            ...
            pass
        # Failed, fall back on hash_set_printer
        return self.hash_set_printer.try_read_std_optional(slot, value)
