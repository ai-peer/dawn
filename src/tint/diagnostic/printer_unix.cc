...
class PrinterUnix : public Printer {
  public:
    PrinterUnix(FILE* f, bool colors) : file(f), use_colors(colors && supports_colors(f)) {}
...
std::unique_ptr<Printer> Printer::create(FILE* out, bool use_colors) {
    return std::make_unique<PrinterUnix>(out, use_colors);
}
