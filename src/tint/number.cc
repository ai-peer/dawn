...
f16::type f16::Quantize(f16::type value) {
    ...
    // is 0x0.FFCp-14 (0_00000_1111111111, 2^-14*2^-10*1023=1023*2^-24). The smallest positive
    ...
    if (value >= kSmallest || value <= -kSmallest) {
        ...
        if (value >= 0x1p-24f || value <= -0x1p-24f) {
            ...
            // For quantizing into denormal f16 number, more mantissa bits will be discarded.
            // E.g., If value is 0x1.ABCDp-15, 10 mantissa bits of the corresponding denormal f16
            ...
            // mantissa bits will be: leading 5 bits of 0, 1 bit of 1, and 4 bits from the
            // significant part of ABCD. 23 - 4 = 19 mantissa bits in f32 should be discarded. If
            // value is 0x1.ABCDp-24, the 10 mantissa bits will be: leading 9 bits of 0, 1 bit of
