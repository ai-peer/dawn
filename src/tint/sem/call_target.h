...
struct CallTargetSignature {
    ...
    CallTargetSignature(type::Type* ret_ty, utils::VectorRef<const Parameter*> params);
...
class CallTarget : public utils::Castable<CallTarget, Node> {
  public:
    ...
    /// @param must_use the result of the call target must be used, i.e. it cannot be used as a call
    /// statement.
    CallTarget(type::Type* return_type,
    ...
    void SetReturnType(type::Type* ty) { signature_.return_type = ty; }

    /// @return the return type of the call target
    type::Type* ReturnType() const { return signature_.return_type; }
