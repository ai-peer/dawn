...
bool Value::Equal(const constant::Value* b) const {
    ...
    return Switch(
        Type(),  //
        [&](type::Vector* vec) { return elements_equal(vec->Width()); },
        [&](type::Matrix* mat) { return elements_equal(mat->columns()); },
        [&](type::Struct* str) { return elements_equal(str->Members().Length()); },
        [&](type::Array* arr) {
