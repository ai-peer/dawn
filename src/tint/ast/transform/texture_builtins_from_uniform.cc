...
struct TextureBuiltinsFromUniform::State {
    ...
    ApplyResult Run() {
        ...
        IterateArrayLengthOnStorageVar([&](const CallExpression*, const sem::GlobalVariable* var) {
                ...
                for (auto* var : src->AST().Globals<Var>()) {
                    ...
                    if (var->HasBindingPoint()) {
                        ...
                        if (binding_point == cfg->ubo_binding) {
                            // Existing struct, add new member and replace
                            ...
                            // auto* str = src->Sem().Get<sem::Struct>(var);
                            // utils::Vector<const StructMember*, 4>
                            // new_members(str->Declaration()->members);
                            utils::Vector<const StructMember*, 4> new_members(
                                ctx.Clone(str->Declaration()->members));
                            // utils::Vector<const StructMember*, 4>
                            // new_members(var->As<Struct>()->members); utils::Vector<const
                            // StructMember*, 4> new_members(str->Members());
                            new_members.Push(new_member);

                            auto new_str_sym = b.Symbols().New();
                            auto* new_str = b.Structure(new_str_sym, new_members);

                            // auto* new_str_sem = src->Sem().Get<sem::Struct>(new_str);

                            // b.Structure(new_str_sym, new_members);

                            // ctx.Replace(buffer_size_ubo, b.Expr(new_str_sym));
                            // ctx.Replace(str, new_str->Declaration());

                            // ctx.Replace(str->Declaration(), new_str);

                            // ctx.Replace(global_sem->Type(), new_str->name);
                            // ctx.Replace(global_sem->Type(), b.Expr(new_str_sem));

                            // ctx.Replace(str->Name(), b.Expr(new_str->name->symbol));
        ...
        IterateArrayLengthOnStorageVar([&](const CallExpression* call_expr,
            ...
            auto* vec_expr = b.IndexAccessor(
                b.MemberAccessor(get_ubo()->name->symbol, kTextureNumLevelsMemberName),
                u32(array_index));
...
TextureBuiltinsFromUniform::Config::Config(sem::BindingPoint ubo_bp) : ubo_binding(ubo_bp) {}
