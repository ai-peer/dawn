  ...
  private:
    ...
    const Expression* IntDivMod(const BinaryExpression* bin_op) {
        ...
        auto fn = binary_op_polyfills.GetOrCreate(sig, [&] {
            ...
            const auto [lhs_el_ty, lhs_width] = lhs_ty->Element({lhs_ty, 1});
            const auto [rhs_el_ty, rhs_width] = rhs_ty->Element({rhs_ty, 1});
        ...
        Symbol fn = Switch(
            ...
            [&](const sem::ValueConversion* conv) {
                if (cfg.builtins.conv_f32_to_iu32) {
                    ...
                    if (tint::Is<type::F32>(src_ty->Element({src_ty}).type)) {
                        auto* dst_ty = conv->Target();
                        if (tint::utils::IsAnyOf<type::I32, type::U32>(
                                dst_ty->Element({dst_ty}).type)) {
