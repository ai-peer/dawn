...
struct ArrayLengthFromUniform::State {
    ...
    ApplyResult Run() {
                ...
                for (auto* var : ctx.src->AST().Globals<Var>()) {
                    if (var->HasBindingPoint()) {
                        ...
                        if (binding_point == cfg->ubo_binding) {
                            ...
                            ctx.Replace(var->As<Variable>()->type.expr,
                                        b.Expr(new_str->name->symbol));

                            // ctx.Remove(ctx.src->AST().GlobalVariables(),
                            // global_sem->Declaration()); buffer_size_ubo = b.GlobalVar(b.Sym(),
                            // b.ty.Of(new_str),
        ...
        IterateArrayLengthOnStorageVar([&](const CallExpression* call_expr,
            ...
            auto* vec_expr = b.IndexAccessor(b.MemberAccessor(get_ubo_sym(), kBufferSizeMemberName),
                                             u32(array_index));
