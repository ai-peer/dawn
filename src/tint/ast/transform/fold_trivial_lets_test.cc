...
TEST_F(FoldTrivialLetsTest, Fold_ComplexInitializer_ChainedSingleUse) {
    auto* src = R"(
fn f(idx : i32) {
  var v = array<vec4i, 4>();
  let x = v[idx].y;
...
TEST_F(FoldTrivialLetsTest, Fold_ComplexInitializer_ChainedMultiUse) {
    auto* src = R"(
fn f(idx : i32) {
  var v = array<vec4i, 4>();
  let x = v[idx].y;
...
TEST_F(FoldTrivialLetsTest, NoFold_IdentInitializer_AssignmentHasSideEffects) {
    auto* src = R"(
var<private> v = 42;
