...
TEST_F(AstMultisampledTextureTest, IsTexture) {
    Texture* t = create<MultisampledTexture>(TextureDimension::kCube, ty.f32());
...
TEST_F(AstMultisampledTextureTest, Dim) {
    auto* s = create<MultisampledTexture>(TextureDimension::k3d, ty.f32());
...
TEST_F(AstMultisampledTextureTest, Type) {
    ...
    EXPECT_EQ(Symbols().NameFor(s->type->As<ast::TypeName>()->name), "f32");
}
