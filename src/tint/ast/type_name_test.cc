...
TEST_F(TypeNameTest, Creation_Templated) {
    ...
    auto* name = As<ast::TemplatedIdentifierExpression>(t->name);
...
TEST_F(TypeNameTest, Creation_WithSource) {
    auto* t = ty.type_name(Source{{20, 2}}, "ty", 1_a, 2._a, false);
    auto* name = As<ast::TemplatedIdentifierExpression>(t->name);
    ASSERT_NE(name, nullptr);
    EXPECT_EQ(name->symbol, Symbols().Get("ty"));
    ASSERT_EQ(name->arguments.Length(), 3u);
...
TEST_F(TypeNameTest, Assert_DifferentProgramID_TemplateArg) {
    EXPECT_FATAL_FAILURE(
        {
            ProgramBuilder b1;
            ProgramBuilder b2;
            b1.ty.type_name("b1", b2.Expr("b2"));
        },
        "internal compiler error");
}
