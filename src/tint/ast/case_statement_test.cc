...
TEST_F(CaseStatementTest, Creation_i32) {
    auto* selector = CaseSelector(Expr(2_i));
...
TEST_F(CaseStatementTest, Creation_u32) {
    auto* selector = CaseSelector(Expr(2_u));
    utils::Vector b{selector};
...
TEST_F(CaseStatementTest, ContainsDefault_WithDefault) {
    utils::Vector b{CaseSelector(Expr(2_u)), CaseSelector()};
...
TEST_F(CaseStatementTest, ContainsDefault_WithOutDefault) {
    utils::Vector b{CaseSelector(Expr(2_u)), CaseSelector(Expr(3_u))};
    auto* c = create<CaseStatement>(b, create<BlockStatement>(utils::Empty));
    EXPECT_FALSE(c->ContainsDefault());
}

TEST_F(CaseStatementTest, Creation_WithSource) {
    utils::Vector b{CaseSelector(Expr(2_i))};
...
TEST_F(CaseStatementTest, IsCase) {
    auto* c =
        create<CaseStatement>(utils::Vector{CaseSelector()}, create<BlockStatement>(utils::Empty));
...
TEST_F(CaseStatementTest, Assert_Null_Body) {
    EXPECT_FATAL_FAILURE(
        {
            ProgramBuilder b;
            b.create<CaseStatement>(utils::Vector{b.CaseSelector()}, nullptr);
        },
    ...
    EXPECT_FATAL_FAILURE(
        {
            ProgramBuilder b1;
            ProgramBuilder b2;
            b1.create<CaseStatement>(utils::Vector{b1.CaseSelector()},
