...
Extension::Type Extension::NameToType(const std::string& name) {
  /*
  TODO(Zhaoming): Deal with each supported extension, convert its name to
  corresponding type. For example:
  ```
      if (name == "f16") {
        return Extension::Type::kF16;
      }
  ```
  */

  return Extension::Type::kNotAnExtension;
}

std::string Extension::TypeToName(Type type) {
  switch (type) {
    /*
    TODO(Zhaoming): Deal with each supported extension, convert its type to
...
Extension::Extension(ProgramID pid, const Source& src, const std::string& name)
    : Base(pid, src), name(name), type(NameToType(name)) {}
