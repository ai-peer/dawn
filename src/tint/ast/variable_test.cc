...
TEST_F(VariableTest, Creation) {
    ...
    EXPECT_EQ(v->declared_address_space, builtin::AddressSpace::kFunction);
...
TEST_F(VariableTest, CreationWithSource) {
    ...
    EXPECT_EQ(v->declared_address_space, builtin::AddressSpace::kPrivate);
...
TEST_F(VariableTest, CreationEmpty) {
    ...
    EXPECT_EQ(v->declared_address_space, builtin::AddressSpace::kWorkgroup);
    CheckIdentifier(Symbols(), v->type, "i32");
    EXPECT_EQ(v->source.range.begin.line, 27u);
    EXPECT_EQ(v->source.range.begin.column, 4u);
    EXPECT_EQ(v->source.range.end.line, 27u);
