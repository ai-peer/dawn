            ...
            switch (static_cast<Writer>(thread_idx % static_cast<size_t>(Writer::kCount))) {
#if TINT_BUILD_WGSL_WRITER
                case Writer::kWGSL: {
                    tint::wgsl::writer::Generate(&program, {}).Move();
                    break;
                }
...
#if TINT_BUILD_HLSL_WRITER
                case Writer::kHLSL: {
                    tint::hlsl::writer::Generate(&program, {}).Move();
                    break;
                }
...
#if TINT_BUILD_GLSL_WRITER
                case Writer::kGLSL: {
                    tint::glsl::writer::Generate(&program, {}, entry_point).Move();
                    break;
                }
                ...
                case Writer::kMSL: {
                    ...
                    if (auto flattened = tint::writer::FlattenBindings(&program)) {
                        tint::msl::writer::Generate(&flattened.value(), {}).Move();
                    }
                    break;
                }
