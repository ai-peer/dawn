...
bool MutationChangeUnaryOperator::IsApplicable(
  ...
  const auto* unary_expr_node = tint::As<ast::UnaryOpExpression>(
      node_id_map.GetNode(message_.unary_expr_id()));
  ...
  const auto* basic_type = type->Is<sem::Reference>()
                               ? type->As<sem::Reference>()->StoreType()
                               : type;
...
void MutationChangeUnaryOperator::Apply(const NodeIdMap& node_id_map,
  ...
  switch (static_cast<ast::UnaryOp>(message_.new_operator())) {
    case ast::UnaryOp::kComplement:
      cloned_replacement = clone_context->dst->Complement(
          clone_context->Clone(unary_expr_node->expr));
      break;
    case ast::UnaryOp::kNegation:
      cloned_replacement = clone_context->dst->Negation(
          clone_context->Clone(unary_expr_node->expr));
      break;
    default:
...
ast::UnaryOp MutationChangeUnaryOperator::ToggleOperator(
