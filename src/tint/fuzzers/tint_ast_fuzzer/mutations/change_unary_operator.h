...
class MutationChangeUnaryOperator : public Mutation {
 public:
  ...
  explicit MutationChangeUnaryOperator(
      protobufs::MutationChangeUnaryOperator message);

  /// @brief Constructor.
  /// @param expression_id - the id of the `ast::UnaryOpExpression` instance
  /// to change its operator.
  /// @param new_unary_op - A new unary operator for the unary expression
  /// specified by `expression_id`.
  MutationChangeUnaryOperator(uint32_t expression_id,
  ...
  /// Return a list of all valid unary op expression operators that are
  /// allowed given the type of the expression.
  /// @param original_op - the unary operator of the expression.
  /// @return the allowed for the given expr type.
  static ast::UnaryOp GetValidUnaryOpForSignedInt(
