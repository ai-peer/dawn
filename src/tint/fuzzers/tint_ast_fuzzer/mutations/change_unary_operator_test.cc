...
TEST(ChangeUnaryOperatorTest, Operator_Not_Applicable) {
  ...
  const auto* neg_a_var =
      main_fn_statements[4]->As<ast::VariableDeclStatement>()->variable;
  ASSERT_NE(neg_a_var, nullptr);

  const auto* not_b_var =
      main_fn_statements[5]->As<ast::VariableDeclStatement>()->variable;
  ASSERT_NE(not_b_var, nullptr);

  const auto* not_c_var =
      main_fn_statements[6]->As<ast::VariableDeclStatement>()->variable;
  ASSERT_NE(not_c_var, nullptr);

  const auto* neg_d_var =
      main_fn_statements[7]->As<ast::VariableDeclStatement>()->variable;
  ...
  ASSERT_FALSE(MaybeApplyMutation(
      program, MutationChangeUnaryOperator(neg_a_id, ast::UnaryOp::kComplement),
      node_id_map, &program, &node_id_map, nullptr));
  ASSERT_FALSE(MaybeApplyMutation(
      program, MutationChangeUnaryOperator(neg_a_id, ast::UnaryOp::kNot),
      node_id_map, &program, &node_id_map, nullptr));
  ASSERT_FALSE(MaybeApplyMutation(
      program, MutationChangeUnaryOperator(neg_a_id, ast::UnaryOp::kNegation),
      node_id_map, &program, &node_id_map, nullptr));

  auto not_b_id = node_id_map.GetId(not_b_expr);
  // Only complement and negation is allowed for signed integer type.
  ASSERT_FALSE(MaybeApplyMutation(
      program, MutationChangeUnaryOperator(not_b_id, ast::UnaryOp::kNot),
      node_id_map, &program, &node_id_map, nullptr));
  // Cannot change to the same unary operator.
  ASSERT_FALSE(MaybeApplyMutation(
      program, MutationChangeUnaryOperator(not_b_id, ast::UnaryOp::kComplement),
      node_id_map, &program, &node_id_map, nullptr));
  ...
  //  // the operator of float types to any other.
  ASSERT_FALSE(MaybeApplyMutation(
      program, MutationChangeUnaryOperator(not_c_id, ast::UnaryOp::kNot),
      node_id_map, &program, &node_id_map, nullptr));
  ASSERT_FALSE(MaybeApplyMutation(
      program, MutationChangeUnaryOperator(not_c_id, ast::UnaryOp::kNegation),
      node_id_map, &program, &node_id_map, nullptr));
  ASSERT_FALSE(MaybeApplyMutation(
      program, MutationChangeUnaryOperator(not_c_id, ast::UnaryOp::kComplement),
      node_id_map, &program, &node_id_map, nullptr));
  ...
  // the operator of float types to any other.
  ASSERT_FALSE(MaybeApplyMutation(
      program, MutationChangeUnaryOperator(neg_d_id, ast::UnaryOp::kComplement),
      node_id_map, &program, &node_id_map, nullptr));
  ASSERT_FALSE(MaybeApplyMutation(
      program, MutationChangeUnaryOperator(neg_d_id, ast::UnaryOp::kNegation),
      node_id_map, &program, &node_id_map, nullptr));
  ASSERT_FALSE(MaybeApplyMutation(
      program, MutationChangeUnaryOperator(neg_d_id, ast::UnaryOp::kNot),
      node_id_map, &program, &node_id_map, nullptr));
}
...
TEST(ChangeUnaryOperatorTest, Signed_Integer_Types_Applicable1) {
  ...
  // Get variable from statements.
  const auto* comp_a_var =
      main_fn_statements[1]->As<ast::VariableDeclStatement>()->variable;
  ASSERT_NE(comp_a_var, nullptr);

  // Get the expression from variable declaration.
  const auto* comp_a_expr =
      comp_a_var->constructor->As<ast::UnaryOpExpression>();
  ...
  ASSERT_TRUE(MaybeApplyMutation(
      program, MutationChangeUnaryOperator(comp_a_id, ast::UnaryOp::kNegation),
      node_id_map, &program, &node_id_map, nullptr));
  ASSERT_TRUE(program.IsValid()) << program.Diagnostics().str();
...
TEST(ChangeUnaryOperatorTest, Signed_Integer_Types_Applicable2) {
  ...
  // Get variable from statements.
  const auto* comp_b_var =
      main_fn_statements[1]->As<ast::VariableDeclStatement>()->variable;
  ASSERT_NE(comp_b_var, nullptr);

  // Get the expression from variable declaration.
  const auto* comp_b_expr =
      comp_b_var->constructor->As<ast::UnaryOpExpression>();
  ASSERT_NE(comp_b_expr, nullptr);

  // Assert mutation to be applicable and apply mutation.
  auto comp_b_id = node_id_map.GetId(comp_b_expr);
  ASSERT_TRUE(MaybeApplyMutation(
      program, MutationChangeUnaryOperator(comp_b_id, ast::UnaryOp::kNegation),
      node_id_map, &program, &node_id_map, nullptr));
  ASSERT_TRUE(program.IsValid()) << program.Diagnostics().str();
...
TEST(ChangeUnaryOperatorTest, Signed_Integer_Types_Applicable4) {
  ...
  ASSERT_TRUE(MaybeApplyMutation(
      program, MutationChangeUnaryOperator(neg_b_id, ast::UnaryOp::kComplement),
      node_id_map, &program, &node_id_map, nullptr));
  ASSERT_TRUE(program.IsValid()) << program.Diagnostics().str();
