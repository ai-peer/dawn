...
TypeAndCount Tuple::Elements(const Type* type_if_invalid /* = nullptr */,
                             uint32_t /* count_if_invalid = 0 */) const {
    return {type_if_invalid, static_cast<uint32_t>(types_.Length())};
}

const Type* Tuple::Element(uint32_t index) const {
    return index < types_.Length() ? types_[index] : nullptr;
}
