...
TEST_F(TypeTest, Element) {
    EXPECT_EQ(f32->Element(), (TypeAndCount{nullptr, 0u}));
    EXPECT_EQ(f16->Element(), (TypeAndCount{nullptr, 0u}));
    EXPECT_EQ(i32->Element(), (TypeAndCount{nullptr, 0u}));
    EXPECT_EQ(u32->Element(), (TypeAndCount{nullptr, 0u}));
    EXPECT_EQ(vec2_f32->Element(), (TypeAndCount{f32, 2u}));
    EXPECT_EQ(vec3_f16->Element(), (TypeAndCount{f16, 3u}));
    EXPECT_EQ(vec4_f32->Element(), (TypeAndCount{f32, 4u}));
    EXPECT_EQ(vec3_u32->Element(), (TypeAndCount{u32, 3u}));
    EXPECT_EQ(vec3_i32->Element(), (TypeAndCount{i32, 3u}));
    EXPECT_EQ(mat2x4_f32->Element(), (TypeAndCount{vec4_f32, 2u}));
    EXPECT_EQ(mat4x2_f32->Element(), (TypeAndCount{vec2_f32, 4u}));
    EXPECT_EQ(mat4x3_f16->Element(), (TypeAndCount{vec3_f16, 4u}));
    EXPECT_EQ(str_f16->Element(), (TypeAndCount{nullptr, 1u}));
    EXPECT_EQ(arr_i32->Element(), (TypeAndCount{i32, 5u}));
    EXPECT_EQ(arr_vec3_i32->Element(), (TypeAndCount{vec3_i32, 5u}));
    EXPECT_EQ(arr_mat4x3_f16->Element(), (TypeAndCount{mat4x3_f16, 5u}));
    EXPECT_EQ(arr_mat4x3_af->Element(), (TypeAndCount{mat4x3_af, 5u}));
    EXPECT_EQ(arr_str_f16->Element(), (TypeAndCount{str_f16, 5u}));
}

TEST_F(TypeTest, ElementWithCustomInvalid) {
    EXPECT_EQ(f32->Element({f32, 42}), (TypeAndCount{f32, 42}));
    EXPECT_EQ(f16->Element({f16, 42}), (TypeAndCount{f16, 42}));
    EXPECT_EQ(i32->Element({i32, 42}), (TypeAndCount{i32, 42}));
    EXPECT_EQ(u32->Element({u32, 42}), (TypeAndCount{u32, 42}));
    EXPECT_EQ(vec2_f32->Element({vec2_f32, 42}), (TypeAndCount{f32, 2u}));
    EXPECT_EQ(vec3_f16->Element({vec3_f16, 42}), (TypeAndCount{f16, 3u}));
    EXPECT_EQ(vec4_f32->Element({vec4_f32, 42}), (TypeAndCount{f32, 4u}));
    EXPECT_EQ(vec3_u32->Element({vec3_u32, 42}), (TypeAndCount{u32, 3u}));
    EXPECT_EQ(vec3_i32->Element({vec3_i32, 42}), (TypeAndCount{i32, 3u}));
    EXPECT_EQ(mat2x4_f32->Element({mat2x4_f32, 42}), (TypeAndCount{vec4_f32, 2u}));
    EXPECT_EQ(mat4x2_f32->Element({mat4x2_f32, 42}), (TypeAndCount{vec2_f32, 4u}));
    EXPECT_EQ(mat4x3_f16->Element({mat4x3_f16, 42}), (TypeAndCount{vec3_f16, 4u}));
    EXPECT_EQ(str_f16->Element({str_f16, 42}), (TypeAndCount{str_f16, 1}));
    EXPECT_EQ(arr_i32->Element({arr_i32, 42}), (TypeAndCount{i32, 5u}));
    EXPECT_EQ(arr_vec3_i32->Element({arr_vec3_i32, 42}), (TypeAndCount{vec3_i32, 5u}));
    EXPECT_EQ(arr_mat4x3_f16->Element({arr_mat4x3_f16, 42}), (TypeAndCount{mat4x3_f16, 5u}));
    EXPECT_EQ(arr_mat4x3_af->Element({arr_mat4x3_af, 42}), (TypeAndCount{mat4x3_af, 5u}));
    EXPECT_EQ(arr_str_f16->Element({arr_str_f16, 42}), (TypeAndCount{str_f16, 5u}));
}
