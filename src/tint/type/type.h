...
class Type : public utils::Castable<Type, UniqueNode> {
  public:
    ...
    /// @param invalid_type the type to return if this type has no child elements.
    /// @param invalid_count the count to return if this type has no child elements, or the number
    /// is unbounded.
    ...
    /// `[invalid_type, N]` is returned, where `N` is the number of elements.
    ...
    ///  * Elements() of `struct S { a : f32, b : i32 }` returns `[invalid_count, 2]`.
    ///  * Elements() of `struct S { a : i32, b : i32 }` also returns `[invalid_count, 2]`.
    virtual TypeAndCount Elements(const Type* invalid_type = nullptr,
