...
class Type : public utils::Castable<Type, UniqueNode> {
  public:
    ...
    /// @param invalid the type and count to return if this type has no child elements.
    /// Defaults to `[nullptr, 0]`
    ...
    /// `[invalid.type, N]` is returned, where `N` is the number of elements.
    ///
    /// Examples:
    ///  * Element() of `array<vec3<f32>, 5>` returns `[vec3<f32>, 5]`.
    ///  * Element() of `struct S { a : f32, b : i32 }` returns `[invalid.type, 2]`.
    ///  * Element() of `struct S { a : i32, b : i32 }` also returns `[invalid.type, 2]`.
    virtual TypeAndCount Element(TypeAndCount invalid = {}) const;
