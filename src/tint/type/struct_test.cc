...
TEST_F(TypeStructTest, Clone) {
    ...
    EXPECT_EQ(members[0]->Index(), 0);
    EXPECT_EQ(members[0]->Offset(), 0);
    EXPECT_EQ(members[0]->Align(), 16);
    EXPECT_EQ(members[0]->Size(), 12);

    EXPECT_EQ(new_st.NameFor(members[1]->Name()), "a");
    EXPECT_TRUE(members[1]->Type()->Is<I32>());
    EXPECT_EQ(members[1]->Index(), 1);
    EXPECT_EQ(members[1]->Offset(), 16);
    EXPECT_EQ(members[1]->Align(), 4);
}
