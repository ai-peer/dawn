...
/// @tparam CALLBACK a function with the signature `void(FIELD)`
template <typename OBJECT, typename CALLBACK>
void ForeachField(OBJECT&& object, CALLBACK&& callback) {
...
#define TINT_REFLECT(...)                                          \
    struct Reflection {                                            \
        template <typename OBJECT, typename CALLBACK>              \
        static void Fields(OBJECT&& object, CALLBACK&& callback) { \
