...
bool Inspector::CheckExtensionAllowlist(
  ...
  for (const std::string& name : extension_allowlist) {
    set.erase(ast::Extension::NameToType(name));
  }

  /*
  TODO(Zhaoming):
  If a disallowed extension is found, should we add an error into
  inspector's diagnostic list?
  E.g.:
  for (const ast::Extension::Type type : set) {
    diagnostics_.add_error(Inspector, "Disallowed extension is used: " +
                                          ast::Extension::TypeToName(type));
  }
  */
...
std::vector<std::string> Inspector::GetUsedExtensionNames() {
  ...
  for (auto type : set) {
    std::string name = ast::Extension::TypeToName(type);
