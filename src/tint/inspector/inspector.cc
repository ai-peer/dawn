...
std::vector<std::pair<std::string, tint::Source::Range>>
Inspector::GetEnableDirectives() {
  std::vector<std::pair<std::string, tint::Source::Range>> result;
  ...
  for (auto node : global_decls) {
    if (auto ext = node->As<ast::Enable>()) {
      result.push_back({ext->name, ext->source.range});
    }
  }
