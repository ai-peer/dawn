...
const ast::Identifier* Void::Build(ProgramBuilder& b) const {
    return nullptr;
}

const ast::Identifier* Bool::Build(ProgramBuilder& b) const {
    return b.ty.bool_();
}

const ast::Identifier* U32::Build(ProgramBuilder& b) const {
    return b.ty.u32();
}

const ast::Identifier* F32::Build(ProgramBuilder& b) const {
    return b.ty.f32();
}

const ast::Identifier* I32::Build(ProgramBuilder& b) const {
    return b.ty.i32();
}
...
const ast::Identifier* Pointer::Build(ProgramBuilder& b) const {
...
const ast::Identifier* Reference::Build(ProgramBuilder& b) const {
    return type->Build(b);
}
...
const ast::Identifier* Vector::Build(ProgramBuilder& b) const {
...
const ast::Identifier* Matrix::Build(ProgramBuilder& b) const {
    auto* ty = type->Build(b);

    // TODO(crbug.com/tint/1810): Remove the cast to ast::IdentifierName when finished.
    auto* type_name = ty->As<ast::IdentifierName>();
    TINT_ASSERT(Reader, type_name);

    return b.ty.mat(type_name, columns, rows);
}

Array::Array(const Type* t, uint32_t sz, uint32_t st) : type(t), size(sz), stride(st) {}
Array::Array(const Array&) = default;

const ast::Identifier* Array::Build(ProgramBuilder& b) const {
    if (size > 0) {
...
const ast::Identifier* Sampler::Build(ProgramBuilder& b) const {
...
const ast::Identifier* DepthTexture::Build(ProgramBuilder& b) const {
...
const ast::Identifier* DepthMultisampledTexture::Build(ProgramBuilder& b) const {
...
const ast::Identifier* MultisampledTexture::Build(ProgramBuilder& b) const {
...
const ast::Identifier* SampledTexture::Build(ProgramBuilder& b) const {
...
const ast::Identifier* StorageTexture::Build(ProgramBuilder& b) const {
...
const ast::Identifier* Alias::Build(ProgramBuilder& b) const {
    return b.ty(name);
}
...
const ast::Identifier* Struct::Build(ProgramBuilder& b) const {
    return b.ty(name);
}
