...
bool FunctionEmitter::EmitNormalTerminator(const BlockInfo& block_info) {
    ...
    switch (opcode(terminator)) {
        ...
        case spv::Op::OpBranchConditional: {
            ...
            // that this is a continue construct and the parent construct has a continue (i.e. the
            // parent is not `continue is entire loop`.
            ...
            if ((true_kind == EdgeKind::kLoopBreak || false_kind == EdgeKind::kLoopBreak) &&
                ...
                auto* parent = GetBlockInfo(block_info.construct->begin_id);
                if (parent->construct && parent->construct->kind == Construct::Kind::kContinue &&
                    !parent->is_continue_entire_loop) {
                    needs_break_if = true;
                }
            }
