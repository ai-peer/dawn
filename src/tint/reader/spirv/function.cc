...
TypedExpression FunctionEmitter::EmitGlslStd450MatrixInverse(
    ...
    switch (mat_ty->columns) {
        ...
        case 3: {
            ...
            auto r = pb.mat3x3<f32>(  //
                pb.vec3<f32>(
                    // s * (e * i - f * h)
                    pb.Mul(s, sub_mul2(e, i, f, h)),
                    // s * (c * h - b * i)
                    pb.Mul(s, sub_mul2(c, h, b, i)),
                    // s * (b * f - c * e)
                    pb.Mul(s, sub_mul2(b, f, c, e))),

                pb.vec3<f32>(
                    // s * (f * g - d * i)
                    pb.Mul(s, sub_mul2(f, g, d, i)),
                    // s * (a * i - c * g)
                    pb.Mul(s, sub_mul2(a, i, c, g)),
                    // s * (c * d - a * f)
                    pb.Mul(s, sub_mul2(c, d, a, f))),
                pb.vec3<f32>(
                    // s * (d * h - e * g)
                    pb.Mul(s, sub_mul2(d, h, e, g)),
                    // s * (b * g - a * h)
                    pb.Mul(s, sub_mul2(b, g, a, h)),
                    // s * (a * e - b * d)
                    pb.Mul(s, sub_mul2(a, e, b, d))));
            return {mat.type, r};
        }
        ...
        case 4: {
            ...
            auto r = pb.mat4x4<f32>(  //
                pb.vec4<f32>(
                    // s * (f * kplo - g * jpln + h * jokn)
                    pb.Mul(s, sub_add_mul3(f, kplo, g, jpln, h, jokn)),

                    // s * (-b * kplo + c * jpln - d * jokn)
                    pb.Mul(s, add_sub_mul3(neg(b), kplo, c, jpln, d, jokn)),

                    // s * (b * gpho - c * fphn + d * fogn)
                    pb.Mul(s, sub_add_mul3(b, gpho, c, fphn, d, fogn)),

                    // s * (-b * glhk + c * flhj - d * fkgj)
                    pb.Mul(s, add_sub_mul3(neg(b), glhk, c, flhj, d, fkgj))),

                pb.vec4<f32>(
                    // s * (-e * kplo + g * iplm - h * iokm)
                    pb.Mul(s, add_sub_mul3(neg(e), kplo, g, iplm, h, iokm)),

                    // s * (a * kplo - c * iplm + d * iokm)
                    pb.Mul(s, sub_add_mul3(a, kplo, c, iplm, d, iokm)),

                    // s * (-a * gpho + c * ephm - d * eogm)
                    pb.Mul(s, add_sub_mul3(neg(a), gpho, c, ephm, d, eogm)),

                    // s * (a * glhk - c * elhi + d * ekgi)
                    pb.Mul(s, sub_add_mul3(a, glhk, c, elhi, d, ekgi))),

                pb.vec4<f32>(
                    // s * (e * jpln - f * iplm + h * injm)
                    pb.Mul(s, sub_add_mul3(e, jpln, f, iplm, h, injm)),

                    // s * (-a * jpln + b * iplm - d * injm)
                    pb.Mul(s, add_sub_mul3(neg(a), jpln, b, iplm, d, injm)),

                    // s * (a * fphn - b * ephm + d * enfm)
                    pb.Mul(s, sub_add_mul3(a, fphn, b, ephm, d, enfm)),

                    // s * (-a * flhj + b * elhi - d * ejfi)
                    pb.Mul(s, add_sub_mul3(neg(a), flhj, b, elhi, d, ejfi))),

                pb.vec4<f32>(
                    // s * (-e * jokn + f * iokm - g * injm)
                    pb.Mul(s, add_sub_mul3(neg(e), jokn, f, iokm, g, injm)),

                    // s * (a * jokn - b * iokm + c * injm)
                    pb.Mul(s, sub_add_mul3(a, jokn, b, iokm, c, injm)),

                    // s * (-a * fogn + b * eogm - c * enfm)
                    pb.Mul(s, add_sub_mul3(neg(a), fogn, b, eogm, c, enfm)),

                    // s * (a * fkgj - b * ekgi + c * ejfi)
                    pb.Mul(s, sub_add_mul3(a, fkgj, b, ekgi, c, ejfi))));
    ...
    const auto ext_opcode = inst.GetSingleWordInOperand(1);
    Fail() << "Error generating " << GetGlslStd450FuncName(ext_opcode);
    return {};
}
