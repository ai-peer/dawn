...
bool ParserImpl::EmitScalarSpecConstants() {
    ...
    for (auto& inst : module_->types_values()) {
        ...
        if (ast_type && ast_expr) {
            ...
            auto* ast_var = MakeLet(inst.result_id(), ast_type, ast_expr, std::move(spec_id_decos));
            if (ast_var) {
...
ast::Let* ParserImpl::MakeLet(uint32_t id,
                              const Type* type,
                              const ast::Expression* constructor,
                              ast::AttributeList decorations) {
    ...
    // Note: we're constructing the const here with the *storage* type,
    ...
    auto sym = builder_.Symbols().Register(namer_.Name(id));
    return create<ast::Let>(Source{}, sym, type->Build(builder_), constructor, decorations);
}
