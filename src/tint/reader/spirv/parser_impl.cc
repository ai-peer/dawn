...
bool ParserImpl::EmitModuleScopeVariables() {
    ...
    for (const auto& type_or_value : module_->types_values()) {
        ...
        if (spirv_storage_class == spv::StorageClass::UniformConstant) {
            ...
            // ast_storage_class should remain kNone becuase handle variables
...
const spvtools::opt::Instruction* ParserImpl::GetSpirvTypeForHandleOrHandleMemoryObjectDeclaration(
    ...
    if (!type) {
        Fail() << "Invalid type for image, sampler, variable or function parameter to image or sampler " << obj.PrettyPrint();
        return nullptr;
    }
    switch (opcode(type)) {
        ...
        case spv::Op::OpTypePointer:
            ...
            break;
        default:
            Fail() << "Invalid type for image, sampler, variable or function parameter to image or sampler "
                   << obj.PrettyPrint();
            return nullptr;
    }
