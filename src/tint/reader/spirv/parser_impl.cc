...
const Pointer* ParserImpl::GetTypeForHandleVar(const spvtools::opt::Instruction& var) {
  ...
  if (!usage.IsComplete()) {
      ...
      if (raw_handle_type->opcode() == SpvOpTypeSampler) {
          ...
          if ((format_param != SpvImageFormatUnknown) || sampled_param == 2 /* without sampler */) {
  ...
  if (usage.IsSampler()) {
      ...
      if (!image_type) {
          Fail() << "internal error: Couldn't look up image type" << raw_handle_type->PrettyPrint();
          return nullptr;
      }
  ...
  auto* result = ty_.Pointer(ast_store_type, ast::StorageClass::kUniformConstant);
    ...
    for (const auto* f : topologically_ordered_functions_) {
        for (const auto& bb : *f) {
            for (const auto& inst : bb) {
              ...
              const auto* operand_as_image = GetMemoryObjectDeclarationForHandle(operand_id, true);
