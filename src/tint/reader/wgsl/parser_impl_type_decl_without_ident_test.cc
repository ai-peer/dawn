...
TEST_P(TypeDeclWithoutIdent_VecTest, Parse) {
    ...
    CheckIdentifier(p->builder().Symbols(), t.value, "vec" + std::to_string(params.count), "f32");
...
TEST_F(ParserImplTest, TypeDeclWithoutIdent_Ptr_ToVec) {
    ...
    CheckIdentifier(p->builder().Symbols(), t.value, "vec2", "f32");
...
TEST_P(TypeDeclWithoutIdent_MatrixTest, Parse) {
    ...
    CheckIdentifier(p->builder().Symbols(), t.value, expected_name, "f32");
