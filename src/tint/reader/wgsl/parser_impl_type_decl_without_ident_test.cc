...
TEST_F(ParserImplTest, TypeDeclWithoutIdent_Ptr_MissingCommaAfterAccess) {
    ...
    ASSERT_EQ(t.value, nullptr);
    ASSERT_TRUE(p->has_error());
    ASSERT_EQ(p->error(), "1:19: expected '> (closing template argument list)' for ptr declaration");
}
...
TEST_F(ParserImplTest, TypeDeclWithoutIdent_Array_MissingComma) {
    ...
    ASSERT_EQ(t.value, nullptr);
    ASSERT_TRUE(p->has_error());
    ASSERT_EQ(p->error(), "1:11: expected '> (closing template argument list)' for array declaration");
}
