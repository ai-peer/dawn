...
TEST_F(ParserImplTest, BitwiseExpr_NoOp_ReturnsLHS) {
...
TEST_F(ParserImplTest, BitwiseExpr_InclusiveOrExpression_Parses) {
    auto p = parser("a | true");
    auto lhs = p->unary_expression();
    auto e = p->bitwise_expression_post_unary_expression(lhs.value);
    EXPECT_TRUE(e.matched);
...
TEST_F(ParserImplTest, BitwiseExpr_InclusiveOrExpression_InvalidRHS) {
...
TEST_F(ParserImplTest, BitwiseExpr_ExclusiveOrExpression_Parses) {
    auto p = parser("a ^ true");
    auto lhs = p->unary_expression();
    auto e = p->bitwise_expression_post_unary_expression(lhs.value);
    EXPECT_TRUE(e.matched);
...
TEST_F(ParserImplTest, BitwiseExpr_ExclusiveOrExpression_InvalidRHS) {
    auto p = parser("true ^ if (a) {}");
    auto lhs = p->unary_expression();
    auto e = p->bitwise_expression_post_unary_expression(lhs.value);
    EXPECT_FALSE(e.matched);
...
TEST_F(ParserImplTest, BitwiseExpr_AndExpression_Parses) {
    auto p = parser("a & true");
    auto lhs = p->unary_expression();
    auto e = p->bitwise_expression_post_unary_expression(lhs.value);
    EXPECT_TRUE(e.matched);
...
TEST_F(ParserImplTest, BitwiseExpr_AndExpression_InvalidRHS) {
    auto p = parser("true & if (a) {}");
    auto lhs = p->unary_expression();
    auto e = p->bitwise_expression_post_unary_expression(lhs.value);
    EXPECT_FALSE(e.matched);
...
TEST_F(ParserImplTest, BitwiseExpr_OrderOfOperations) {
    // Should combine as:
    // (((a | true) & false) ^ b) | c
    //
    auto p = parser("a | true & false ^ b | c");
    auto lhs = p->unary_expression();
    auto e = p->bitwise_expression_post_unary_expression(lhs.value);
    EXPECT_TRUE(e.matched);
    EXPECT_FALSE(e.errored);
    ...
    ASSERT_TRUE(rel->rhs->Is<ast::BoolLiteralExpression>());
    ASSERT_FALSE(rel->rhs->As<ast::BoolLiteralExpression>()->value);
