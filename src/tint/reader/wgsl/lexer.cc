...
Lexer::Lexer(const Source::File* file)
    : file_(file), location_{1, 1}, len_(line().size()) {}
...
const char& Lexer::at(size_t pos) const {
  ...
  // std::string_view. We know that our string_view points at a null-terminated
  // string, though, so we access the data directly.
  return l.data()[pos];
}

std::string Lexer::substr(size_t offset, size_t count) {
  return std::string{line().substr(offset, count)};
}

std::string_view Lexer::substr_view(size_t offset, size_t count) {
...
bool Lexer::is_eof() const {
  return location_.line >= file_->content.lines.size() && pos_ >= len_;
}
...
bool Lexer::matches(size_t pos, std::string_view substr) {
  if (pos >= len_)
    return false;
  return substr_view(pos, substr.size()) == substr;
}
...
Token Lexer::skip_comment() {
  if (matches(pos_, "//")) {
    ...
    while (!is_eol() && !(is_blankspace(at(pos_)) && !matches(pos_, " ") &&
                          !matches(pos_, "\t"))) {
      if (is_null()) {
...
Token Lexer::try_float() {
  ...
  while (end < len_ && is_digit(at(end))) {
  ...
  if (end < len_ && (matches(end, "e") || matches(end, "E"))) {
    ...
    if (!has_exponent) {
      const auto str = substr(start, end - start);
...
Token Lexer::build_token_from_int_if_possible(Source source,
  ...
  if (matches(pos_, "u")) {
    if (static_cast<uint64_t>(res) >
      ...
      return {Token::Type::kError, source,
              "u32 (" + substr(start, end - start) + ") too large"};
    }
    advance(1);
    end_source(source);
  ...
  if (res < static_cast<int64_t>(std::numeric_limits<int32_t>::min())) {
    return {Token::Type::kError, source,
            "i32 (" + substr(start, end - start) + ") too small"};
  }
  if (res > static_cast<int64_t>(std::numeric_limits<int32_t>::max())) {
    return {Token::Type::kError, source,
            "i32 (" + substr(start, end - start) + ") too large"};
  }
  end_source(source);
...
Token Lexer::try_hex_integer() {
  ...
  while (!is_eol() && is_hex(at(end))) {
    ...
    if (digits > kMaxDigits) {
      return {Token::Type::kError, source,
              "integer literal (" + substr(start, end - 1 - start) +
                  "...) has too many digits"};
    }
  }
  if (first == end) {
...
Token Lexer::try_ident() {
  ...
  {
    auto* utf8 = reinterpret_cast<const uint8_t*>(&at(pos_));
    auto [code_point, n] = text::utf8::Decode(utf8, line().size() - pos_);
    if (n == 0) {
  ...
  if (at(start) == '_') {
    ...
    if ((pos_ == start + 1) || (at(start + 1) == '_')) {
      set_pos(start);
      return {};
    }
  }

  auto str = substr_view(start, pos_ - start);
  end_source(source);
