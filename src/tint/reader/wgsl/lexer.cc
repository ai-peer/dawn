...
Lexer::Lexer(const Source::File* file)
    : file_(file), len_(static_cast<uint32_t>(file->content.data.size())), location_{1, 1} {}
...
Token Lexer::skip_comment() {
    if (matches(pos_, "//")) {
        ...
        while (!is_eof() && !(is_blankspace(file_->content.data[pos_]) && !matches(pos_, " ") &&
                              !matches(pos_, "\t"))) {
            if (is_null()) {
  ...
  // -?0[xX]([0-9a-fA-F]*.?[0-9a-fA-F]+ | [0-9a-fA-F]+.[0-9a-fA-F]*)(p|P)(+|-)?[0-9]+  // NOLINT
    ...
    if (has_exponent) {
        ...
        while (end < len_ && isdigit(file_->content.data[end])) {
            ...
            input_exponent = (input_exponent * 10) + dec_value(file_->content.data[end]);
...
Token Lexer::build_token_from_int_if_possible(Source source,
    ...
    if (matches(pos_, "u")) {
        if (static_cast<uint64_t>(res) >
            ...
            return {Token::Type::kError, source,
                    "u32 (" + file_->content.data.substr(start, end - start) + ") too large"};
        }
        pos_ += 1;
        location_.column += 1;
        end_source(source);
    ...
    if (res < static_cast<int64_t>(std::numeric_limits<int32_t>::min())) {
        return {Token::Type::kError, source,
                "i32 (" + file_->content.data.substr(start, end - start) + ") too small"};
    }
    if (res > static_cast<int64_t>(std::numeric_limits<int32_t>::max())) {
        return {Token::Type::kError, source,
                "i32 (" + file_->content.data.substr(start, end - start) + ") too large"};
    }
    end_source(source);
...
Token Lexer::try_hex_integer() {
    ...
    while (!is_eof() && is_hex(file_->content.data[end])) {
        ...
        if (digits > kMaxDigits) {
            return {Token::Type::kError, source,
                    "integer literal (" + file_->content.data.substr(start, end - 1 - start) +
                        "...) has too many digits"};
        }
    }
    if (first == end) {
...
Token Lexer::try_integer() {
    ...
    auto next = first + 1;
    if (next < len_) {
        if (file_->content.data[first] == '0' && is_digit(file_->content.data[next])) {
            return {Token::Type::kError, source,
                    "integer literal (" + file_->content.data.substr(start, end - 1 - start) +
                        "...) has leading 0s"};
        }
...
Token Lexer::try_ident() {
    ...
    // Must begin with an XID_Source unicode character, or underscore
    {
        ...
        auto [code_point, n] = text::utf8::Decode(utf8, file_->content.data.size() - pos_);
        if (n == 0) {
