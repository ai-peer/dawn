...
TEST_F(ParserImplTest, TypeDecl_Ptr_MissingCommaAfterAddressSpace) {
    ...
    ASSERT_EQ(t.value, nullptr);
    ASSERT_TRUE(p->has_error());
    ASSERT_EQ(p->error(), "1:14: expected '>' for type template arguments");
}
...
TEST_F(ParserImplTest, TypeDecl_Ptr_MissingCommaAfterAccess) {
    ...
    ASSERT_EQ(t.value, nullptr);
    ASSERT_TRUE(p->has_error());
    ASSERT_EQ(p->error(), "1:19: expected '>' for type template arguments");
}
...
TEST_F(ParserImplTest, TypeDecl_Array_MissingComma) {
    ...
    ASSERT_EQ(t.value, nullptr);
    ASSERT_TRUE(p->has_error());
    ASSERT_EQ(p->error(), "1:11: expected '>' for type template arguments");
}
