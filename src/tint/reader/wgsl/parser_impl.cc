...
Maybe<const ast::Expression*> ParserImpl::expression() {
    ...
    if (expr.matched) {
        // Check that the next token isn't a binary operator.
        // If it is, then the user probably wants to use parenthesis to make the expression parse.
        if (auto* lhs_op = expr->As<ast::BinaryExpression>()) {
            if (auto& n = peek(); n.IsBinaryOperator()) {
                auto source = Source::Combine(expr->source, n.source());
                add_error(source, std::string("mixing '") + ast::Operator(lhs_op->op) + "' and '" +
