...
Maybe<bool> ParserImpl::enable_directive() {
  auto decl = sync(Token::Type::kSemicolon, [&]() -> Maybe<bool> {
    ...
    be added before the first else to deal with the "f16" extension name.
    ```
        else if (t.Is(Token::ExtensionKind::kF16)) {
          synchronized_ = true;
          next();
          name = {"f16", t.source()};
        }
    ...
    */
    if (t.IsIdentifier()) {
      ...
      next();
      // name = t.to_str();
      // source_of_extension_name = t.source();
      ...
      synchronized_ = false;
      return add_error(t.source(), "expect a valid extension name");
    }
    ...
    if (ast::Enable::NameToKind(name.value) !=
      ...
      builder_.AST().AddExtension(extension);
    } else {
