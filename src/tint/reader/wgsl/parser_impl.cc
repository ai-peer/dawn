...
void ParserImpl::set_next_placeholder(Token::Type type) {
    ...
    tokens_[next_token_idx_].SetType(type);
}
...
void ParserImpl::InitializeTokenList() {
    Lexer l{file_};
    tokens_ = l.TokenList();
}
...
bool ParserImpl::expect(std::string_view use, Token::Type tok) {
    ...
    if (tok == Token::Type::kGreaterThan &&
        ...
        if (t.Is(Token::Type::kShiftRight)) {
            set_next_placeholder(Token::Type::kGreaterThan);
        } else if (t.Is(Token::Type::kGreaterThanEqual)) {
            set_next_placeholder(Token::Type::kEqual);
        }
