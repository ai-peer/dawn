...
ParserImpl::TemplateOrLessThan ParserImpl::disambiguate_less_than(size_t& lookahead) {
    // Progresses 'lookahead' until it points to either a balanced end token, or the end of the
    // current statement. Returns true if end_tok was found.
    ...
    for (;; lookahead++) {
        switch (peek(lookahead).type()) {
            case Token::Type::kIdentifier: {
                if (peek(lookahead + 1).type() == Token::Type::kLessThan) {  // ident '<'
                    // Skip identifier
                    lookahead++;
                    // And disambiguate the inner '<'. This progresses 'lookahead'.
            ...
            case Token::Type::kGreaterThan: {  // '>'
                ...
                if (t.type() == Token::Type::kPeriod ||           // '>' '.'
                    t.type() == Token::Type::kParenLeft ||        // '>' '('
                    t.type() == Token::Type::kGreaterThan ||      // '>' '>'
                    t.type() == Token::Type::kShiftRightEqual) {  // '>' '>' '='
            ...
            case Token::Type::kParenLeft:                      // '('
                if (!skip_nested(Token::Type::kParenRight)) {  // Skip to closing ')'
                    return kLessThan;
                }
                break;

            case Token::Type::kBracketLeft:                      // '['
                if (!skip_nested(Token::Type::kBracketRight)) {  // Skip to closing ']'
                    return kLessThan;
                }
                break;

            // Hit any of theses, and we can immediately rule out a template list.
