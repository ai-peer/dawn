...
class ParserImpl {
  ...
  public:
    ...
    struct TypedIdentifier {
        ...
        TypedIdentifier(const ast::Identifier* type_in, std::string name_in, Source source_in);
        /// Destructor
        ~TypedIdentifier();
    ...
    struct FunctionHeader {
        ...
        const ast::Identifier* return_type = nullptr;
    ...
    Maybe<const ast::Identifier*> callable();
    ...
    Maybe<const ast::Identifier*> type_specifier_without_ident();
    /// Parses a `type_specifier` grammar element
    /// @returns the parsed Type or nullptr if none matched.
    Maybe<const ast::Identifier*> type_specifier();
    ...
    /// @returns the parsed Type or nullptr if none matched.
    Maybe<const ast::Identifier*> texture_and_sampler_types();
    /// Parses a `sampler_type` grammar element
    /// @returns the parsed Type or nullptr if none matched.
    Maybe<const ast::Identifier*> sampler_type();
    ...
    /// @returns the parsed Type or nullptr if none matched.
    Maybe<const ast::Identifier*> depth_texture_type();
    /// Parses a 'texture_external_type' grammar element
    /// @returns the parsed Type or nullptr if none matched
    Maybe<const ast::Identifier*> external_texture();
  ...
  private:
    ...
    Expect<const ast::Identifier*> expect_type_specifier_pointer(const Source& s);
    Expect<const ast::Identifier*> expect_type_specifier_atomic(const Source& s);
    Expect<const ast::Identifier*> expect_type_specifier_vector(const Source& s, uint32_t count);
    Expect<const ast::Identifier*> expect_type_specifier_array(const Source& s);
    Expect<const ast::Identifier*> expect_type_specifier_matrix(const Source& s,
    ...
    Expect<const ast::Identifier*> expect_type(std::string_view use);
