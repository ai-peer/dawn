...
TEST_F(ParserImplTest, ParamList_Single) {
    ...
    EXPECT_EQ(Symbols().NameFor(e.value[0]->type->As<ast::TypeName>()->name), "f32");
...
TEST_F(ParserImplTest, ParamList_Multiple) {
    ...
    EXPECT_EQ(Symbols().NameFor(e.value[1]->type->As<ast::TypeName>()->name), "f32");
    ...
    EXPECT_EQ(
        Symbols().NameFor(e.value[2]->type->As<ast::Vector>()->type->As<ast::TypeName>()->name),
        "f32");
