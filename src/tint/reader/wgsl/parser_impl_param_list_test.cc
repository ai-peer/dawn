...
TEST_F(ParserImplTest, ParamList_Single) {
    ...
    CheckIdentifier(p->builder().Symbols(), e.value[0]->type->name, "i32");
...
TEST_F(ParserImplTest, ParamList_Multiple) {
    ...
    CheckIdentifier(p->builder().Symbols(), e.value[1]->type->name, "f32");
    ...
    CheckIdentifier(p->builder().Symbols(), e.value[1]->type->name, "vec2", "f32");
...
TEST_F(ParserImplTest, ParamList_Attributes) {
    ...
    CheckIdentifier(p->builder().Symbols(), e.value[0]->type->name, "vec4", "f32");
    EXPECT_TRUE(e.value[0]->Is<ast::Parameter>());
