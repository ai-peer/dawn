...
TEST_F(ParserImplTest, ParamList_Single) {
    ...
    ast::CheckIdentifier(p->builder().Symbols(), e.value[0]->type->name, "i32");
...
TEST_F(ParserImplTest, ParamList_Multiple) {
    ...
    ast::CheckIdentifier(p->builder().Symbols(), e.value[1]->type->name, "f32");
    ...
    ast::CheckIdentifier(p->builder().Symbols(), e.value[1]->type->name,
                         ast::Template("vec2", "f32"));
...
TEST_F(ParserImplTest, ParamList_Attributes) {
    ...
    ast::CheckIdentifier(p->builder().Symbols(), e.value[0]->type->name,
                         ast::Template("vec4", "f32"));
    EXPECT_TRUE(e.value[0]->Is<ast::Parameter>());
