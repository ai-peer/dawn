...
TEST_F(ParserImplTest, TextureSamplerTypes_SampledTexture_MissingLessThan) {
    ...
    EXPECT_FALSE(t.matched);
    EXPECT_TRUE(t.errored);
    EXPECT_EQ(p->error(), "1:11: expected '< (opening template argument list)' for sampled texture type");
}
...
TEST_F(ParserImplTest, TextureSamplerTypes_MultisampledTexture_MissingLessThan) {
    ...
    EXPECT_FALSE(t.matched);
    EXPECT_TRUE(t.errored);
    EXPECT_EQ(p->error(), "1:24: expected '< (opening template argument list)' for multisampled texture type");
}
...
TEST_F(ParserImplTest, TextureSamplerTypes_StorageTexture_MissingLessThan) {
    ...
    EXPECT_FALSE(t.matched);
    EXPECT_TRUE(t.errored);
    EXPECT_EQ(p->error(), "1:19: expected '< (opening template argument list)' for storage texture type");
}
