...
TEST_F(EnableDirectiveTest, Valid) {
  ...
  EXPECT_EQ(
      ast.Extensions(),
      ast::ExtensionSet{ast::Extension::Kind::kInternalExtensionForTesting});
  EXPECT_EQ(ast.GlobalDeclarations().size(), 1u);
  auto node = ast.GlobalDeclarations()[0]->As<ast::Extension>();
  EXPECT_TRUE(node != nullptr);
  EXPECT_EQ(node->name, "InternalExtensionForTesting");
  EXPECT_EQ(node->kind, ast::Extension::Kind::kInternalExtensionForTesting);
}
...
enable InternalExtensionForTesting;
  ...
  auto node1 = ast.GlobalDeclarations()[0]->As<ast::Extension>();
  EXPECT_TRUE(node1 != nullptr);
  EXPECT_EQ(node1->name, "InternalExtensionForTesting");
  EXPECT_EQ(node1->kind, ast::Extension::Kind::kInternalExtensionForTesting);
  auto node2 = ast.GlobalDeclarations()[1]->As<ast::Extension>();
  EXPECT_TRUE(node2 != nullptr);
  EXPECT_EQ(node2->name, "InternalExtensionForTesting");
  EXPECT_EQ(node2->kind, ast::Extension::Kind::kInternalExtensionForTesting);
}

// Test an invalid extension identifier.
TEST_F(EnableDirectiveTest, InvalidIdentifier) {
  auto p = parser("enable InvalidExtensionName;");
  p->enable_directive();
  // Emit warning when invalid extension found
  EXPECT_FALSE(p->has_error()) << p->error();
  auto program = p->program();
  auto& ast = program.AST();
...
TEST_F(EnableDirectiveTest, InvalidTokens) {
  ...
  }
  {
    ...
    p->translation_unit();
    EXPECT_TRUE(p->has_error());
    EXPECT_EQ(p->error(), "1:8: expect a valid extension identifier");
    auto program = p->program();
    auto& ast = program.AST();
    EXPECT_EQ(ast.Extensions().size(), 0u);
    EXPECT_EQ(ast.GlobalDeclarations().size(), 0u);
...
enable InternalExtensionForTesting;
  ...
  EXPECT_EQ(
      p->error(),
      "3:1: enable directive must go before any other global decleration");
  auto program = p->program();
  auto& ast = program.AST();
...
enable InternalExtensionForTesting;
  ...
  p->translation_unit();
  // An empty semiclon is treated as a global decleration
  EXPECT_TRUE(p->has_error());
  EXPECT_EQ(
      p->error(),
      "3:1: enable directive must go before any other global decleration");
