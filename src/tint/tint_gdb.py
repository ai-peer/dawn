...
class UtilsHashsetPrinter(Printer):
    ...
    def children(self):
        for slot in range(0, self.slice.length()):
            ...
            if v['hash'] != 0:
                value = v['value']
                kvp = self.try_get_key_value(value)
                if kvp is not None:
                    yield kvp
                else:
                    # Failed to get key-value, so we emit the slot number and the value
                    #  which will rely on the default printer for std::optional
                    yield 'slot {}'.format(slot), value

    def display_hint(self):
        return 'array'

    def try_get_key_value(self, v):
        # v is an HashSet<T,...>::optional<T>
        # It's not easy to get the key, value pair out of this generically, so we
        # attempt to get it based on specific implementations.
        try:
            # libstdc++ v12
            kvp = v['_M_payload']['_M_payload']['_M_value']
