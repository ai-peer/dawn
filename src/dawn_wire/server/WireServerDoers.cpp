...
namespace dawn_wire { namespace server {
    ...
    bool WireServer::DoBufferMapAsync(ObjectId bufferId,
    ...
    bool WireServer::DoBufferUpdateMappedData(ObjectId bufferId,
                                              uint32_t count,
                                              const uint8_t* data) {
        // The null object isn't valid as `self`
        if (bufferId == 0) {
    ...
    bool WireServer::DoQueueSignal(dawnQueue cSelf, dawnFence cFence, uint64_t signalValue) {
        if (cFence == nullptr) {
            return false;
        }
