...
namespace dawn_wire { namespace server {
    ...
    bool Server::DoDevicePopErrorScope(DawnDevice cDevice, uint32_t requestSerial) {
        ErrorScopeUserdata* userdata = new ErrorScopeUserdata;
        userdata->server = this;
        userdata->requestSerial = requestSerial;

        mProcs.devicePopErrorScope(cDevice, ForwardPopErrorScope, userdata);
        return true;
    }
    ...
    void Server::OnDevicePopErrorScope(DawnErrorType type,
        ...
        auto data = std::unique_ptr<ErrorScopeUserdata>(userdata);
