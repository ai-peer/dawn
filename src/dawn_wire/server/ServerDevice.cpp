...
namespace dawn_wire { namespace server {
    ...
    void Server::ForwardCreateReadyComputePipeline(WGPUCreateReadyPipelineStatus status,
        ...
        if (status == WGPUCreateReadyPipelineStatus_DeviceDestroyed) {
            // Because WireClient and WireServer are both removed before the removal of backend
            // device (e.g. in DawnTestBase::~DawnTestBase()), when the callback is called inside
            // device::ShutDown(), createReadyPipelineUserData->server will point to an invalid
            // memory and we should not call any functions on this invalid object.
            return;
        }
