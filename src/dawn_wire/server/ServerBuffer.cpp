...
namespace dawn_wire { namespace server {
    ...
    bool Server::DoBufferMapAsync(ObjectId bufferId,
        ...
        if (isWrite) {
            userdata->writeHandle = std::unique_ptr<MemoryTransfer::WriteHandle>(
                mMemoryTransfer->DeserializeWriteHandle(handleCreateInfo, handleCreateInfoLength));
            mProcs.bufferMapWriteAsync(buffer->handle, ForwardBufferMapWriteAsync, userdata);
        } else {
            userdata->readHandle = std::unique_ptr<MemoryTransfer::ReadHandle>(
                mMemoryTransfer->DeserializeReadHandle(handleCreateInfo, handleCreateInfoLength));
    ...
    bool Server::DoDeviceCreateBufferMapped(DawnDevice device,
        ...
        resultData->writeHandle = std::unique_ptr<MemoryTransfer::WriteHandle>(
            mMemoryTransfer->DeserializeWriteHandle(handleCreateInfo, handleCreateInfoLength));
    ...
    bool Server::DoBufferUpdateMappedData(ObjectId bufferId,
        ...
        return buffer->writeHandle->DeserializeClose(writeFlushInfo, writeFlushInfoLength);
    }
    ...
    void Server::OnBufferMapReadAsyncCallback(DawnBufferMapAsyncStatus status,
        ...
        uint32_t handleInitializeInfoLength =
            bufferData->readHandle->SerializeInitialize(ptr, dataLength);

        ReturnBufferMapReadAsyncCallbackCmd cmd;
        cmd.buffer = data->buffer;
        cmd.requestSerial = data->requestSerial;
        cmd.status = status;
        cmd.handleInitializeInfoLength = static_cast<uint32_t>(handleInitializeInfoLength);
        cmd.handleInitializeInfo = nullptr;

        size_t requiredSize = cmd.GetRequiredSize();
        char* allocatedBuffer = static_cast<char*>(GetCmdSpace(requiredSize));
        cmd.Serialize(allocatedBuffer);
        bufferData->readHandle->SerializeInitialize(
            ptr, dataLength, allocatedBuffer + requiredSize - handleInitializeInfoLength);
    }
