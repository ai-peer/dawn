...
namespace dawn_wire { namespace server {
    ...
    bool Server::PreHandleBufferUnmap(const BufferUnmapCmd& cmd) {
        ...
        if (buffer->mappedAtCreation && !(buffer->usage & WGPUMapMode_Write)) {
            // This indicates the writeHandle is for mappedAtCreation only. Destory on unmap
    ...
    bool Server::DoDeviceCreateBuffer(ObjectId deviceId,
        ...
        if (readHandleCreateInfoLength + writeHandleCreateInfoLength >
        ...
        if (isWriteMode) {
            ...
            if (descriptor->mappedAtCreation) {
                ...
                // writeHandle->SetTarget(mapping, descriptor->size, 0);
    ...
    bool Server::DoBufferUpdateMappedData(ObjectId bufferId,
        ...
        return buffer->writeHandle->DeserializeDataUpdate(
            writeFlushInfo, static_cast<size_t>(writeFlushInfoLength), offset, size);
    }
    ...
    void Server::OnBufferMapAsyncCallback(WGPUBufferMapAsyncStatus status, MapUserdata* data) {
        ...
        if (isSuccess && isRead) {
            ...
            //     bufferData->readHandle->SerializeInitialDataSize(readData, data->size);
            cmd.readInitialDataInfoLength =
            ...
            if (isSuccess) {
                if (isRead) {
                    ...
                    bufferData->writeHandle->SetTarget(
                        static_cast<uint8_t*>(mProcs.bufferGetMappedRange(
