...
namespace dawn_wire { namespace client {
    ...
    void ClientHandwrittenBufferMapReadAsync(WGPUBuffer cBuffer,
        ...
        if (buffer->size > std::numeric_limits<size_t>::max()) {
            // Dawn returns OOM and makes an error buffer on creation if the size is larger than
            // size_t. We can't make this large of a read handle, so call the error callback
            // immediately, and inject a validation error.
            callback(WGPUBufferMapAsyncStatus_Error, nullptr, 0, buffer);
            ClientDeviceInjectError(reinterpret_cast<WGPUDevice>(buffer->device),
                                    WGPUErrorType_Validation, "Buffer is an error");
            return;
        }
    ...
    void ClientHandwrittenBufferMapWriteAsync(WGPUBuffer cBuffer,
        ...
        if (buffer->size > std::numeric_limits<size_t>::max()) {
            // Dawn returns OOM and makes an error buffer on creation if the size is larger than
            // size_t. We can't make this large of a write handle, so call the error callback
            // immediately, and inject a validation error.
    ...
    WGPUCreateBufferMappedResult ClientHandwrittenDeviceCreateBufferMapped(
        ...
        if (descriptor->size > std::numeric_limits<size_t>::max()) {
            ...
            if ((descriptor->usage & (WGPUBufferUsage_MapRead | WGPUBufferUsage_MapWrite)) != 0) {
                // If the buffer has map usage, use the normal CreateBuffer command. Dawn will
                // generate an OOM error for this buffer.
                result.buffer = ClientHandwrittenDeviceCreateBuffer(cDevice, descriptor);
            } else {
                // Otherwise, add a map usage so that Dawn generates an OOM error.
                WGPUBufferDescriptor descriptorWithMapUsage = *descriptor;
                descriptorWithMapUsage.usage |= WGPUBufferUsage_MapWrite;
                result.buffer =
                    ClientHandwrittenDeviceCreateBuffer(cDevice, &descriptorWithMapUsage);
            }
            result.data = nullptr;
            result.dataLength = 0;
            return result;
        ...
        std::unique_ptr<MemoryTransferService::WriteHandle> writeHandle =
            std::unique_ptr<MemoryTransferService::WriteHandle>(
                wireClient->GetMemoryTransferService()->CreateWriteHandle(buffer->size));
