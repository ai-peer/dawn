...
namespace dawn_wire { namespace client {
    ...
    void ClientBufferMapReadAsync(dawnBuffer cBuffer,
        ...
        char* allocatedBuffer = static_cast<char*>(buffer->GetClient()->GetCmdSpace(requiredSize));
    ...
    void ClientBufferUnmap(dawnBuffer cBuffer) {
        ...
        if (buffer->mappedData) {
            ...
            if (buffer->isWriteMapped) {
                ...
                char* allocatedBuffer =
                    static_cast<char*>(buffer->GetClient()->GetCmdSpace(requiredSize));
                cmd.Serialize(allocatedBuffer);
            }
        ...
        size_t requiredSize = cmd.GetRequiredSize();
        char* allocatedBuffer = static_cast<char*>(buffer->GetClient()->GetCmdSpace(requiredSize));
        cmd.Serialize(allocatedBuffer, *buffer->GetClient());
    }
    ...
    void ClientQueueSignal(dawnQueue cQueue, dawnFence cFence, uint64_t signalValue) {
        ...
        size_t requiredSize = cmd.GetRequiredSize();
        char* allocatedBuffer = static_cast<char*>(fence->GetClient()->GetCmdSpace(requiredSize));
        cmd.Serialize(allocatedBuffer, *fence->GetClient());
    }
    ...
    void ClientDeviceSetErrorCallback(dawnDevice cSelf,
        ...
        self->SetErrorCallback(callback, userdata);
    }
