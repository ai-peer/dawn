...
namespace dawn_wire { namespace client {
    bool ClientImpl::DoDeviceErrorCallback(const char* message) {
        mDevice->HandleError(message);
        return true;
    }

    bool ClientImpl::DoBufferMapReadAsyncCallback(Buffer* buffer,
    ...
    bool ClientImpl::DoBufferMapWriteAsyncCallback(Buffer* buffer,
                                                   uint32_t requestSerial,
                                                   uint32_t status) {
        // The buffer might have been deleted or recreated so this isn't an error.
        if (!buffer) {
    ...
    bool ClientImpl::DoFenceUpdateCompletedValue(Fence* fence, uint64_t value) {
