...
namespace dawn_wire { namespace client {
        ...
        if (mappable) {
            ...
            if ((descriptor->usage & WGPUBufferUsage_MapWrite) != 0 ||
                ...
                if (descriptor->mappedAtCreation) {
                    // Call GetMapData on buffer creation, it may fail by returning a nullptr in
                    // writeData.
    ...
    void Buffer::MapAsync(WGPUMapModeFlags mode,
        ...
        // Step 2a: Check the existence of mReadHandle/mWriteHandle which indicates if the buffer
        // usage contains MapRead/MapWrite on creation
        if (isReadMode) {
    ...
    bool Buffer::OnMapAsyncCallback(uint32_t requestSerial,
        ...
        if (status == WGPUBufferMapAsyncStatus_Success) {
            ...
            if (isRead) {
                ...
                // Call GetMapData for WriteHandle. This returns a pointer and size of mapped
