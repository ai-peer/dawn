            ...
            if ((descriptor->usage & WGPUBufferUsage_MapWrite) != 0 ||
                ...
                if (descriptor->mappedAtCreation) {
                    ...
                    std::tie(writeData, writeDataLength) =
                        writeHandle->GetMapData(descriptor->size, 0);
    ...
    bool Buffer::OnMapAsyncCallback(uint32_t requestSerial,
        ...
        if (status == WGPUBufferMapAsyncStatus_Success) {
            ...
            if (isRead) {
                ...
                //     return FailRequest();
                // }
                if (!request.readHandle->UpdateMapData(
                        readInitialDataInfo, static_cast<size_t>(readInitialDataInfoLength),
                        request.size, request.offset, &mappedData, &mappedDataLength)) {
                    return FailRequest();
                }
                ...
                std::tie(mappedData, mappedDataLength) =
                    request.writeHandle->GetMapData(request.size, request.offset);
