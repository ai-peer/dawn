...
namespace dawn_wire { namespace client {
    ...
    ShaderModule::~ShaderModule() {
        ...
        for (auto& it : mCompilationInfoRequests) {
            if (it.second.callback) {
                // TODO: Would it be better to return a compilation info with 0 messages or 1 error
                // message?
                it.second.callback(nullptr, it.second.userdata);
            }
        }
    ...
    bool ShaderModule::GetCompilationInfoCallback(uint64_t requestSerial,
        ...
        // bool ShaderModule::GetCompilationInfoCallback(uint64_t requestSerial, uint64_t info) {
        ...
        request.callback(info, request.userdata);
        // request.callback(nullptr, request.userdata);
        return true;
    }
