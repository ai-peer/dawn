...
namespace dawn_wire { namespace client {
    ...
    class InlineMemoryTransferService : public MemoryTransferService {
        class ReadHandleImpl : public ReadHandle {
          public:
            explicit ReadHandleImpl(std::unique_ptr<uint8_t[]> stagingData)
        ...
        class WriteHandleImpl : public WriteHandle {
          public:
            explicit WriteHandleImpl(std::unique_ptr<uint8_t[]> stagingData)
                : mStagingData(std::move(stagingData)) {
            }
      ...
      public:
        ...
        ReadHandle* CreateReadHandle(size_t size) override {
            ...
            if (stagingData) {
                return new ReadHandleImpl(std::move(stagingData));
            }
            return nullptr;
        }
        ...
        WriteHandle* CreateWriteHandle(size_t size) override {
            ...
            if (stagingData) {
                ...
                return new WriteHandleImpl(std::move(stagingData));
            }
            return nullptr;
        }
    };
