...
namespace dawn_wire { namespace client {
    bool WireClient::DoDeviceErrorCallback(const char* message) {
        mDevice->HandleError(message);
        return true;
    }

    bool WireClient::DoBufferMapReadAsyncCallback(Buffer* buffer,
    ...
    bool WireClient::DoBufferMapWriteAsyncCallback(Buffer* buffer,
                                                   uint32_t requestSerial,
                                                   uint32_t status) {
        // The buffer might have been deleted or recreated so this isn't an error.
        if (!buffer) {
    ...
    bool WireClient::DoFenceUpdateCompletedValue(Fence* fence, uint64_t value) {
