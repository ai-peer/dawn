...
namespace utils {
    ...
    uint32_t GetBytesInBufferTextureCopy(wgpu::TextureFormat format,
        ...
        const uint32_t bytesPerTexel = utils::GetTexelBlockSizeInBytes(format);
        const uint32_t bytesAtLastImage = bytesPerRow * (height - 1) + bytesPerTexel * width;
    ...
    TextureDataCopyLayout GetTextureDataCopyLayoutForTexture2DAtLevel(
        ...
        layout.byteLength = GetBytesInBufferTextureCopy(
            format, layout.mipSize.width, layout.mipSize.height, layout.bytesPerRow,
