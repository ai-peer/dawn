    ...
    class WindowsDebugLogger : public PlatformDebugLogger {
      public:
        WindowsDebugLogger() : PlatformDebugLogger() {
            ...
            ASSERT(mShouldExitHandle != INVALID_HANDLE_VALUE);
            ...
            mThread = std::thread(
                [](HANDLE shouldExit) {
                    ...
                    dbWinBuffer = reinterpret_cast<decltype(dbWinBuffer)>(
                        MapViewOfFile(file, SECTION_MAP_READ, 0, 0, 0));
                    ASSERT(dbWinBuffer && dbWinBuffer != INVALID_HANDLE_VALUE);

                    HANDLE dbWinBufferReady =
                        CreateEventA(nullptr, FALSE, FALSE, "DBWIN_BUFFER_READY");
                    ASSERT(dbWinBufferReady && dbWinBufferReady != INVALID_HANDLE_VALUE);

                    HANDLE dbWinDataReady = CreateEventA(nullptr, FALSE, FALSE, "DBWIN_DATA_READY");
                    ASSERT(dbWinDataReady && dbWinDataReady != INVALID_HANDLE_VALUE);
        ...
        ~WindowsDebugLogger() override {
            if (mShouldExitHandle != INVALID_HANDLE_VALUE) {
