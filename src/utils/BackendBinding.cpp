...
namespace utils {
    ...
    void DiscoverAdapter(dawn_native::Instance* instance, dawn_native::BackendType type) {
        if (type == dawn_native::BackendType::OpenGL) {
#if defined(DAWN_ENABLE_BACKEND_OPENGL)
    ...
    BackendBinding* CreateBinding(dawn_native::BackendType type, GLFWwindow* window, dawnDevice device) {
        switch (type) {
#if defined(DAWN_ENABLE_BACKEND_D3D12)
