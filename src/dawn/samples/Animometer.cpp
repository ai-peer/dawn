...
void frame() {
    ...
    for (int i = 0; i < kNumWaits; ++i) {
        ...
        bool done = false;
        if (kUseFd) {
            std::vector<struct pollfd> pfds(futures.size());
            for (int j = 0; j < kNumFuturesPerWait; ++j) {
                pfds[j] = {futures[j].GetFd(), POLLIN, 0};
            }
            while (pfds.size()) {
            ...
            auto waited =
                wgpu::WaitAnyFutures(&count, reinterpret_cast<wgpu::Future*>(futures.data()), 0);
            ...
            futures[0].Then(
                ...
                &done);
            size_t count = futures.size();
            auto waited = wgpu::WaitAnyFutures(
