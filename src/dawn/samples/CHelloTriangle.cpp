...
void init() {
    device = CreateCppDawnDevice().MoveIntoCHandle();
    queue = wgpuDeviceGetQueue(device);
    swapchain = GetSwapChain().MoveIntoCHandle();
    ...
    WGPUShaderModule vsModule = utils::CreateShaderModule(device, vs).MoveIntoCHandle();

    const char* fs = R"(
        @fragment fn main() -> @location(0) vec4f {
            return vec4f(1.0, 0.0, 0.0, 1.0);
        })";
    WGPUShaderModule fsModule = utils::CreateShaderModule(device, fs).MoveIntoCHandle();
