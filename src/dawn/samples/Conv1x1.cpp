...
void TestWebGPU() {
    ...
    if (!adapters.empty()) {
        ...
        for (uint32_t i = 0; i < absl::GetFlag(FLAGS_trials); ++i) {
            ...
            wgpu::ComputePassTimestampWrite timestampWrites[] = {
                {querySet, 2 * i, wgpu::ComputePassTimestampLocation::Beginning},
                {querySet, 2 * i + 1, wgpu::ComputePassTimestampLocation::End},
            };
            computePassDesc.timestampWrites = timestampWrites;
            computePassDesc.timestampWriteCount = 2u;
