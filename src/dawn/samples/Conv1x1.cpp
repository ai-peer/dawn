...
fn main(
  ...
  while(true) {
    ...
    var src_w0_h0 : vec4<foptype>;
    src_w0_h0 = vec4<foptype>(textureLoad(src_tensor_image2d, vec2<i32>((DST_X), ((DST_Y) * U.i0.z + (s))), 0));
    s += 1;
  ...
  int4 shared_int4_1)
{
  ...
  while(true) {
    ...
    FLTOP4 src_w0_h0 = convert_FLTOP4(READIMG(src_tensor_image2d, smp_zero, (int2)((DST_X), ((DST_Y) * shared_int4_0.z + (s)))));
    r_w0_h0_s0 += load_weight(0) * src_w0_h0.x;
    r_w0_h0_s0 += load_weight(1) * src_w0_h0.y;
    r_w0_h0_s0 += load_weight(2) * src_w0_h0.z;
    r_w0_h0_s0 += load_weight(3) * src_w0_h0.w;
    ...
    src_w0_h0 = convert_FLTOP4(READIMG(src_tensor_image2d, smp_zero, (int2)((DST_X), ((DST_Y) * shared_int4_0.z + (s)))));
    r_w0_h0_s0 += load_weight(16) * src_w0_h0.x;
    r_w0_h0_s0 += load_weight(17) * src_w0_h0.y;
    r_w0_h0_s0 += load_weight(18) * src_w0_h0.z;
    r_w0_h0_s0 += load_weight(19) * src_w0_h0.w;
...
void TestOpenCL() {
    ...
    cl_uint vendorID = 0;
    devices[0].getInfo(CL_DEVICE_VENDOR_ID, &vendorID);
    std::cout << "Testing OpenCL vendor id 0x" << std::hex << vendorID << std::endl;

    cl::Context context(devices[0]);

    uint32_t wgSize = absl::GetFlag(FLAGS_global) && vendorID == 0x8086 ? 32 : 64;
    ...
    if (!absl::GetFlag(FLAGS_global)) {
      ...
      shader = std::regex_replace(shader, std::regex("decl_weights_cache;"), "__local FLTD4 weights_cache[32];");

      std::regex re("load_weight\\((.+?)\\)");
      while (std::regex_search(shader, re)) {
        shader = std::regex_replace(shader, re, "convert_FLTOP4(weights_cache[$1])");
      }
    } else {
      ...
      std::regex re("load_weight\\((.+?)\\)");
      while (std::regex_search(shader, re)) {
        shader = std::regex_replace(shader, re, "convert_FLTOP4(weights_buffer[$1])");
      }
    }
...
void TestWebGPU() {
    ...
    if (!adapters.empty()) {
        ...
        uint32_t wgSize = absl::GetFlag(FLAGS_global) && properties.vendorID == 0x8086 ? 32 : 64;
        ...
        if (!absl::GetFlag(FLAGS_global)) {
          shader = "var<workgroup> weights_cache : array<vec4<fdtype>, 32>;\n" + shader;
          ...
          std::regex re("load_weight\\((.+?)\\)");
          while (std::regex_search(shader, re)) {
            shader = std::regex_replace(shader, re, "vec4<foptype>(weights_cache[$1])");
          }
        } else {
          shader = std::regex_replace(shader, std::regex("load_workgroup_weights\\(\\);"), "");
          ...
          std::regex re("load_weight\\((.+?)\\)");
          while (std::regex_search(shader, re)) {
            shader = std::regex_replace(shader, re, "vec4<foptype>(weights_buffer.data[$1])");
          }
        }
        if (absl::GetFlag(FLAGS_f16_data)) {
