...
bool InitSample(int argc, const char** argv) {
    for (int i = 1; i < argc; i++) {
        ...
        if (opt == "-b") {
            static constexpr struct NameAndBackendType {
                const char* name;
                wgpu::BackendType type;
            } backends[] = {
                {"vulkan", wgpu::BackendType::Vulkan}, {"d3d11", wgpu::BackendType::D3D11},
                {"d3d12", wgpu::BackendType::D3D12},   {"metal", wgpu::BackendType::Metal},
                {"opengl", wgpu::BackendType::OpenGL}, {"opengles", wgpu::BackendType::OpenGLES},
            };
            backendType = wgpu::BackendType::Null;
            for (const auto& backend : backends) {
                if (value == backend.name) {
                    backendType = backend.type;
                    break;
                }
            }
            if (backendType == wgpu::BackendType::Null) {
                fprintf(stderr,
                        "--backend expects a backend name (opengl, opengles, metal, d3d12, d3d11, "
                        "null, vulkan)\n");
                return false;
            }
            continue;
