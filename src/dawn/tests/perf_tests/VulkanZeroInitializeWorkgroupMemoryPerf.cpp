...
void VulkanZeroInitializeWorkgroupMemoryExtensionTest::SetUp() {
    ...
    uint32_t workgroupSize = GetParam().workgroupSize;
    uint32_t loopLength = kWorkgroupArraySize / workgroupSize;

    std::ostringstream ostream;
    ostream << R"(
        @group(0) @binding(0) var<storage, read_write> dst : array<f32>;

        var<workgroup> mm_Asub : array<f32, )"
        ...
        fn main(@builtin(local_invocation_id) LocalInvocationID : vec3u) {
            var loopLength = )"
            << loopLength << R"(u;
            for (var k = 0u; k < loopLength; k = k + 1u) {
                var index = loopLength * LocalInvocationID.x + k;
