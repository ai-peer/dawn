...
// bindGroup[0] contains 12 uniform buffer bindings, 4 storage buffer bindings and 4 storage texture
// bindings. bindGroup[1] contains 4 storage buffer bindings with dynamic offsets. In Step() it
// creates a compute pass and dispatch, which will validate the bindings. If DirtyAspectTestType ==
// BusyValidation, it creates another compute pass and dispatch to avoid only validating the bind
// group as a lazy aspect once.
...
void BindingsValidationPerf::SetUp() {
    ...
    for (uint32_t i = 0; i < numUniformBufferBindings; i++) {
        bindingGroupsDescriptors[0][0].push_back(
            BindingDescriptor{{i, uniformBuffer, 0, 64}, BindingType::UniformBuffer});
        bindingGroupsDescriptors[1][0].push_back(
            BindingDescriptor{{i, uniformBuffer, 0, 1024}, BindingType::UniformBuffer});
    }
    ...
    dynamicOffsets[0].resize(bindingGroupsDescriptors[0][1].size());
    dynamicOffsets[1].resize(bindingGroupsDescriptors[1][1].size());
    for (size_t i = 0; i < numStorageBufferBindingsWithDynamicOffset; ++i) {
        ...
        dynamicOffsets[1][i] = 256 * (i + numStorageBufferBindingsWithStaticOffset);
    }
...
void BindingsValidationPerf::Step() {
    ...
    //     computePassEncoder.SetBindGroup(1, bindGroups[p][1], dynamicOffsets[p].size(),
    //                                     dynamicOffsets[p].data());
