...
TEST(WeakRefTests, Basic) {
    ...
    EXPECT_EQ(weak.Get().Get(), base.Get());
...
TEST(WeakRefTests, Derived) {
    ...
    WeakRef<WeakRefCountedDerivedA> weak = GetWeakRef<WeakRefCountedDerivedA>(base);
    WeakRef<WeakRefCountedBaseA> weak2 = weak;
    WeakRef<WeakRefCountedBaseA> weak3 = GetWeakRef<WeakRefCountedBaseA>(base);
    EXPECT_TRUE(weak.IsValid());
    EXPECT_TRUE(weak.Get().Get() == base.Get());
    EXPECT_TRUE(weak2.IsValid());
    EXPECT_TRUE(weak3.IsValid());
