...
TEST_F(FeatureTests, AdapterWithRequiredFeatureDisabled) {
    ...
    for (size_t i = 0; i < kTotalFeaturesCount; ++i) {
        ...
        // Test that creating device with or without DisallowUnsafeApis toggle disabled will failed
        // if required an unsupported feature. Without disabling DisallowUnsafeApis toggle
        {
...
TEST_F(FeatureTests, RequireAndGetEnabledFeatures) {
    ...
    for (size_t i = 0; i < kTotalFeaturesCount; ++i) {
        ...
        // Test with DisallowUnsafeApis not disabled.
        {
            ...
            // Devicec creation should fail if requiring experimental features without disabling
            // DisallowUnsafeApis
