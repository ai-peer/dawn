  ...
  public:
    FeatureTests()
        : testing::Test(),
          ...
          // toggle enabled by default, and record a empty RequiredTogglesSet, but it is fine for this test.
          mUnsafeAdapterBase(mInstanceBase.Get(),
                             dawn::native::TogglesState::CreateFromInitializerForTesting(
                                 dawn::native::ToggleStage::Adapter,
                                 {},
                                 {dawn::native::Toggle::DisallowUnsafeAPIs}),
                             dawn::native::RequiredTogglesSet(dawn::native::ToggleStage::Adapter)) {
    }
