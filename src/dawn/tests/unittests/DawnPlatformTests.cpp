...
class TestPlatform : public dawn::platform::Platform {
  public:
    MOCK_METHOD(void, ReportError, (WGPUErrorType type, std::string_view message), (override));
};
...
TEST(DawnPlatformTests, ReportError) {
    ...
    EXPECT_CALL(platform, ReportError(WGPUErrorType_OutOfMemory, StrEq("fake oom error"))).Times(1);
    EXPECT_CALL(platform, ReportError(WGPUErrorType_Validation, _)).Times(0);
