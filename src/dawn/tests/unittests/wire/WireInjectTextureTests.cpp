...
class WireInjectTextureTests : public WireTest {
  public:
    ...
    // A dummy texture format for ReserveTexture. The data in it doesn't matter as long as we don't call texture reflection methods.
...
TEST_F(WireInjectTextureTests, CallAfterReserveInject) {
    ReservedTexture reservation = GetWireClient()->ReserveTexture(device, &texDesc);
...
TEST_F(WireInjectTextureTests, ReserveDifferentIDs) {
    ReservedTexture reservation1 = GetWireClient()->ReserveTexture(device, &texDesc);
    ReservedTexture reservation2 = GetWireClient()->ReserveTexture(device, &texDesc);
