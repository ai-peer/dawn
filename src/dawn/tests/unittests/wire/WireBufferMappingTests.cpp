...
namespace dawn::wire {
    ...
    TEST_F(WireBufferMappingReadTests, MappingForReadSuccessBuffer) {
        ...
        EXPECT_CALL(api, BufferGetConstMappedRange(apiBuffer, 0, kBufferSize))
            .WillOnce(testing::Return(&bufferContent));
    ...
    TEST_F(WireBufferMappingReadTests, UnmapInsideMapReadCallback) {
        ...
        EXPECT_CALL(*mockBufferMapCallback, Call(WGPUBufferMapAsyncStatus_Success, testing::_))
            .WillOnce(testing::InvokeWithoutArgs([&]() { wgpuBufferUnmap(buffer); }));
    ...
    TEST_F(WireBufferMappingReadTests, DestroyInsideMapReadCallback) {
        ...
        EXPECT_CALL(*mockBufferMapCallback, Call(WGPUBufferMapAsyncStatus_Success, testing::_))
            .WillOnce(testing::InvokeWithoutArgs([&]() { wgpuBufferRelease(buffer); }));
    ...
    TEST_F(WireBufferMappingWriteTests, MappingForWriteSuccessBuffer) {
        ...
        EXPECT_CALL(api, BufferGetMappedRange(apiBuffer, 0, kBufferSize))
            .WillOnce(testing::Return(&serverBufferContent));
    ...
    TEST_F(WireBufferMappingTests, MappedAtCreationSuccess) {
        ...
        EXPECT_CALL(api, DeviceCreateBuffer(apiDevice, testing::_))
            .WillOnce(testing::Return(apiBuffer));
        EXPECT_CALL(api, BufferGetMappedRange(apiBuffer, 0, 4))
            .WillOnce(testing::Return(&apiBufferData));
    ...
    TEST_F(WireBufferMappingTests, MaxSizeMappableBufferOOMDirectly) {
        ...
        // Check for CreateBufferMapped.
        {
            ...
            EXPECT_CALL(api, DeviceInjectError(apiDevice, WGPUErrorType_OutOfMemory, testing::_));
            EXPECT_CALL(api, DeviceCreateErrorBuffer(apiDevice))
                .WillOnce(testing::Return(apiBuffer));
            FlushClient();
        }
