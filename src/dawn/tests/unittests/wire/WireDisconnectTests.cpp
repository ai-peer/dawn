...
namespace dawn::wire {
    ...
    TEST_F(WireDisconnectTests, CommandsAfterDisconnect) {
        ...
        wgpuDeviceCreateCommandEncoder(device, nullptr);
        EXPECT_CALL(api, DeviceCreateCommandEncoder(testing::_, testing::_))
    ...
    TEST_F(WireDisconnectTests, CallsDeviceLostCallback) {
        testing::MockCallback<WGPUDeviceLostCallback> mockDeviceLostCallback;
    ...
    TEST_F(WireDisconnectTests, ServerLostThenDisconnect) {
        ...
        EXPECT_CALL(mockDeviceLostCallback,
                    Call(WGPUDeviceLostReason_Undefined, testing::StrEq("some reason"), this))
            .Times(testing::Exactly(1));
        FlushServer();

        // Disconnect the client. We shouldn't see the lost callback again.
        EXPECT_CALL(mockDeviceLostCallback, Call(testing::_, testing::_, testing::_))
            .Times(testing::Exactly(0));
        GetWireClient()->Disconnect();
    }
    ...
    TEST_F(WireDisconnectTests, ServerLostThenDisconnectInCallback) {
        ...
        // only once.
        EXPECT_CALL(mockDeviceLostCallback,
                    Call(WGPUDeviceLostReason_Undefined, testing::StrEq("lost reason"), this))
    ...
    TEST_F(WireDisconnectTests, DeleteClientDestroysObjects) {
        ...
        EXPECT_CALL(api, DeviceCreateCommandEncoder(apiDevice, nullptr))
            .WillOnce(testing::Return(apiCommandEncoder));

        WGPUSampler apiSampler = api.GetNewSampler();
        EXPECT_CALL(api, DeviceCreateSampler(apiDevice, testing::_))
            .WillOnce(testing::Return(apiSampler));
