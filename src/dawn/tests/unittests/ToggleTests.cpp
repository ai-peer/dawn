...
TEST_F(AdapterToggleTest, AdapterTogglesSetAndInheritToDevice) {
    ...
    // Request an adapter with required toggles using APIRequestAdapter.
    auto requestAdapterWithTogglesDescriptor =
        [&instance, this](const WGPUDawnTogglesDescriptor* requiredAdapterToggles) {
            native::RequestAdapterOptions options = {};
            wgpu::DawnTogglesDescriptor adapterTogglesDesc = {};

            if (requiredAdapterToggles) {
                adapterTogglesDesc.enabledTogglesCount =
                    requiredAdapterToggles->enabledTogglesCount;
                adapterTogglesDesc.enabledToggles = requiredAdapterToggles->enabledToggles;
                adapterTogglesDesc.disabledTogglesCount =
                    requiredAdapterToggles->disabledTogglesCount;
                adapterTogglesDesc.disabledToggles = requiredAdapterToggles->disabledToggles;
            ...
            instanceBase->APIRequestAdapter(&options, cb.Callback(), cb.MakeUserdata(this));
    ...
    auto validateAdapterTogglesInheritedToDevice = [](native::AdapterBase* adapter) {
        ...
        native::DeviceBase* device = adapter->APICreateDevice();
    ...
    auto CreateAdapterAndValidateToggles =
        [createNullAdapterWithTogglesDescriptor, requestAdapterWithTogglesDescriptor,
         validateAdapterToggles, validateAdapterTogglesInheritedToDevice](
            ...
            // Test creating adapter by GetAdapters
            {
            ...
            // Test creating adapter by APIRequestAdapter
            {
                wgpu::Adapter adapter = requestAdapterWithTogglesDescriptor(requiredAdapterToggles);
                validateAdapterToggles(native::FromAPI(adapter.Get()), expectedEnabledToggles,
                                       expectedDisabledToggles);
                validateAdapterTogglesInheritedToDevice(native::FromAPI(adapter.Get()));
            }
