...
template <size_t N>
std::bitset<N> BitsetFromBitString(const char (&str)[N]) {
    return std::bitset<N>(str, N);
}
...
static auto kSerdeValueVectorParams = std::make_tuple(
    ...
    std::vector<std::bitset<57>>{
        BitsetFromBitString("10011001010101100110011010101100110010101011001100101101"),
        BitsetFromBitString("00011001010101100010011010101100110010101001001100101010"),
        BitsetFromBitString("11111111111111111111111111111111111111111111111111111111"), 0},
    // Test vectors.
