...
DeviceMock::DeviceMock(Ref<QueueBase> defaultQueue) {
    ...
    // Initialize the device and limits.
    if (defaultQueue == nullptr) {
        QueueDescriptor desc = {};
        defaultQueue = AcquireRef(new NiceMock<QueueMock>(this, &desc));
    }

    EXPECT_FALSE(Initialize(defaultQueue).IsError());
}
