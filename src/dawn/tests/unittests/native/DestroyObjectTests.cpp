...
class DestroyObjectTests : public DawnMockTest {
  public:
    DestroyObjectTests() : DawnMockTest() {
        ...
        mDeviceMock->SetToggle(Toggle::SkipValidation, true);
    }

    BindGroupLayoutMock* GetBindGroupLayout() { return mDeviceMock->GetEmptyBindGroupLayoutMock(); }

    TextureMock* GetTexture() {
        if (mTexture != nullptr) {
            return mTexture.Get();
        }
        ...
        mTexture = AcquireRef(new NiceMock<TextureMock>(mDeviceMock, &desc));
        return mTexture.Get();
    }

    TextureViewMock* GetTextureView() {
        ...
        mTextureView = AcquireRef(new NiceMock<TextureViewMock>(GetTexture(), &desc));
        return mTextureView.Get();
    }

    ShaderModuleMock* GetVertexShaderModule() {
        if (mVsModule != nullptr) {
            return mVsModule.Get();
        }
        mVsModule = ShaderModuleMock::Create(mDeviceMock, kVertexShader.data());
        return mVsModule.Get();
    }

    ShaderModuleMock* GetComputeShaderModule() {
        if (mCsModule != nullptr) {
            return mCsModule.Get();
        }
        mCsModule = ShaderModuleMock::Create(mDeviceMock, kComputeShader.data());
        return mCsModule.Get();
    }
  ...
  protected:
    ...
    Ref<TextureViewMock> mTextureView;
...
TEST_F(DestroyObjectTests, BindGroupNativeExplicit) {
    ...
    desc.layout = GetBindGroupLayout();
    desc.entryCount = 0;
...
TEST_F(DestroyObjectTests, BindGroupImplicit) {
    ...
    EXPECT_CALL(*bindGroupMock.Get(), DestroyImpl).Times(1);
    {
        ...
        wgpu::BindGroup bindGroup = device.CreateBindGroup(wgpu::ToAPI(&desc));
...
TEST_F(DestroyObjectTests, BindGroupLayoutImplicit) {
    ...
    EXPECT_CALL(*bindGroupLayoutMock.Get(), DestroyImpl).Times(1);
    {
        ...
        wgpu::BindGroupLayout bindGroupLayout = device.CreateBindGroupLayout(wgpu::ToAPI(&desc));
...
TEST_F(DestroyObjectTests, BufferApiExplicit) {
    ...
    wgpu::Buffer buffer = device.CreateBuffer(wgpu::ToAPI(&desc));
...
TEST_F(DestroyObjectTests, CommandBufferImplicit) {
    ...
    wgpu::CommandEncoder commandEncoder =
        device.CreateCommandEncoder(wgpu::ToAPI(&commandEncoderDesc));
    ...
    EXPECT_CALL(*commandBufferMock.Get(), DestroyImpl).Times(1);
    {
        ...
        wgpu::CommandBuffer commandBuffer = commandEncoder.Finish(wgpu::ToAPI(&commandBufferDesc));
...
TEST_F(DestroyObjectTests, ComputePipelineNativeExplicit) {
    ...
    desc.compute.module = GetComputeShaderModule();
...
TEST_F(DestroyObjectTests, ComputePipelineImplicit) {
...

    {
        ...
        wgpu::ComputePipeline computePipeline = device.CreateComputePipeline(wgpu::ToAPI(&desc));
...
TEST_F(DestroyObjectTests, ExternalTextureApiExplicit) {
    ...
    wgpu::ExternalTexture externalTexture = device.CreateExternalTexture(wgpu::ToAPI(&desc));
...
TEST_F(DestroyObjectTests, PipelineLayoutNativeExplicit) {
    ...
    bindGroupLayouts.push_back(GetBindGroupLayout());
...
TEST_F(DestroyObjectTests, PipelineLayoutImplicit) {
    ...
    EXPECT_CALL(*pipelineLayoutMock.Get(), DestroyImpl).Times(1);
    {
        ...
        wgpu::PipelineLayout pipelineLayout = device.CreatePipelineLayout(wgpu::ToAPI(&desc));
...
TEST_F(DestroyObjectTests, QuerySetApiExplicit) {
    ...
    wgpu::QuerySet querySet = device.CreateQuerySet(wgpu::ToAPI(&desc));
...
TEST_F(DestroyObjectTests, RenderPipelineNativeExplicit) {
    ...
    desc.vertex.module = GetVertexShaderModule();
...
TEST_F(DestroyObjectTests, RenderPipelineImplicit) {
...

    {
        ...
        wgpu::RenderPipeline renderPipeline = device.CreateRenderPipeline(wgpu::ToAPI(&desc));
...
TEST_F(DestroyObjectTests, SamplerImplicit) {
    ...
    EXPECT_CALL(*samplerMock.Get(), DestroyImpl).Times(1);
    {
        ...
        wgpu::Sampler sampler = device.CreateSampler(wgpu::ToAPI(&desc));
...
TEST_F(DestroyObjectTests, ShaderModuleNativeExplicit) {
    Ref<ShaderModuleMock> shaderModuleMock = GetVertexShaderModule();
...
TEST_F(DestroyObjectTests, ShaderModuleImplicit) {
    ShaderModuleMock* shaderModuleMock = GetVertexShaderModule();
    EXPECT_CALL(*shaderModuleMock, DestroyImpl).Times(1);
    {
        ScopedRawPtrExpectation scoped(shaderModuleMock);

        EXPECT_CALL(*mDeviceMock, CreateShaderModuleImpl)
            .WillOnce(Return(ByMove(std::move(mVsModule))));
        wgpu::ShaderModule shaderModule =
            device.CreateShaderModule(wgpu::ToAPI(shaderModuleMock->GetDescriptor()));
...
TEST_F(DestroyObjectTests, TextureNativeExplicit) {
    Ref<TextureMock> textureMock = GetTexture();
...
TEST_F(DestroyObjectTests, TextureApiExplicit) {
    ...
    wgpu::Texture texture = device.CreateTexture(wgpu::ToAPI(textureMock->GetDescriptor()));
...
TEST_F(DestroyObjectTests, TextureImplicit) {
    TextureMock* textureMock = GetTexture();

    EXPECT_CALL(*textureMock, DestroyImpl).Times(1);
    {
        ScopedRawPtrExpectation scoped(textureMock);

        EXPECT_CALL(*mDeviceMock, CreateTextureImpl).WillOnce(Return(ByMove(std::move(mTexture))));
        wgpu::Texture texture = device.CreateTexture(wgpu::ToAPI(textureMock->GetDescriptor()));
...
TEST_F(DestroyObjectTests, TextureViewNativeExplicit) {
...

    {
        ...
        Ref<TextureViewMock> textureViewMock = AcquireRef(new TextureViewMock(GetTexture(), &desc));
...
TEST_F(DestroyObjectTests, TextureViewApiExplicit) {
    ...
    Ref<TextureViewMock> textureViewMock = AcquireRef(new TextureViewMock(GetTexture(), &viewDesc));
    EXPECT_CALL(*textureViewMock.Get(), DestroyImpl).Times(1);

    EXPECT_CALL(*mDeviceMock, CreateTextureImpl).WillOnce(Return<Ref<TextureBase>>(GetTexture()));
    wgpu::Texture texture = device.CreateTexture(wgpu::ToAPI(GetTexture()->GetDescriptor()));

    EXPECT_CALL(*mDeviceMock, CreateTextureViewImpl(GetTexture(), _))
        .WillOnce(Return(ByMove(std::move(textureViewMock))));
    wgpu::TextureView textureView = texture.CreateView(wgpu::ToAPI(&viewDesc));
...
TEST_F(DestroyObjectTests, TextureViewImplicit) {
    ...
    EXPECT_CALL(*textureViewMock.Get(), DestroyImpl).Times(1);
    {
        ...
        EXPECT_CALL(*mDeviceMock, CreateTextureImpl)
            .WillOnce(Return<Ref<TextureBase>>(GetTexture()));
        wgpu::Texture texture = device.CreateTexture(wgpu::ToAPI(GetTexture()->GetDescriptor()));
...
TEST_F(DestroyObjectTests, DestroyObjectsApiExplicit) {
    ...
    wgpu::BindGroup bindGroup;
    {
        ...
        bindGroup = device.CreateBindGroup(wgpu::ToAPI(&desc));
    }
    ...
    wgpu::BindGroupLayout bindGroupLayout;
    {
        ...
        bindGroupLayout = device.CreateBindGroupLayout(wgpu::ToAPI(&desc));
    }
    ...
    wgpu::Buffer buffer;
    {
        ...
        buffer = device.CreateBuffer(wgpu::ToAPI(&desc));
    }
    ...
    wgpu::CommandBuffer commandBuffer;
    {
        ...
        commandBuffer = commandEncoder.Finish(wgpu::ToAPI(&commandBufferDesc));
    }
    ...
    wgpu::ComputePipeline computePipeline;
    {
        ...
        computePipeline = device.CreateComputePipeline(wgpu::ToAPI(&desc));
    }
    ...
    wgpu::ExternalTexture externalTexture;
    {
        ...
        externalTexture = device.CreateExternalTexture(wgpu::ToAPI(&desc));
    }
    ...
    wgpu::PipelineLayout pipelineLayout;
    {
        ...
        pipelineLayout = device.CreatePipelineLayout(wgpu::ToAPI(&desc));
    }
    ...
    wgpu::QuerySet querySet;
    {
        ...
        querySet = device.CreateQuerySet(wgpu::ToAPI(&desc));
    }
    ...
    wgpu::RenderPipeline renderPipeline;
    {
        ...
        renderPipeline = device.CreateRenderPipeline(wgpu::ToAPI(&desc));
    }
    ...
    wgpu::Sampler sampler;
    {
        ...
        sampler = device.CreateSampler(wgpu::ToAPI(&desc));
    }
    ...
    {
        ScopedRawPtrExpectation scoped(mDeviceMock);
        shaderModuleMock = mVsModule;
        EXPECT_CALL(*mDeviceMock, CreateShaderModuleImpl).WillOnce(Return(mVsModule));
        shaderModule = device.CreateShaderModule(wgpu::ToAPI(shaderModuleMock->GetDescriptor()));
    }
    ...
    wgpu::Texture texture;
    {
        ScopedRawPtrExpectation scoped(mDeviceMock);
        ASSERT(GetTexture());
        textureMock = mTexture;
        EXPECT_CALL(*mDeviceMock, CreateTextureImpl).WillOnce(Return(textureMock));
        texture = device.CreateTexture(wgpu::ToAPI(mTexture->GetDescriptor()));
    }
    ...
    wgpu::TextureView textureView;
    {
        ...
        textureView = texture.CreateView(wgpu::ToAPI(&desc));
    }
    ...
    EXPECT_FALSE(FromAPI(shaderModule.Get())->IsAlive());
    EXPECT_FALSE(FromAPI(texture.Get())->IsAlive());
    EXPECT_FALSE(FromAPI(textureView.Get())->IsAlive());
}
