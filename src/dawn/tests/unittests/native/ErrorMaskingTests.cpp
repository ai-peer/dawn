...
class ErrorMaskingTests : public Test {
  public:
    ErrorMaskingTests() : Test() {
  ...
  protected:
    ...
    // Mock underlying the API device.
    DeviceMock* mDeviceMock = new DeviceMock();
    wgpu::Device device = wgpu::Device::Acquire(ToAPI(mDeviceMock));
};
