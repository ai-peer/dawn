...
TEST_F(ErrorMaskingTests, QueueCopyTextureForBrowserOomBuffer) {
...
// Even though OOM is allowed in buffer creation, when creating a buffer in internal workaround the
// OOM should be masked as a device loss.
TEST_F(ErrorMaskingTests, QueueCopyExternalTextureForBrowserOomBuffer) {
...
TEST_F(ErrorMaskingTests, CreateComputePipeline) {
    ...
    EXPECT_CALL(mDeviceLostCb,
                Call(WGPUDeviceLostReason_Undefined, HasSubstr(kOomErrorMessage), this))
        .Times(1);
    device.CreateComputePipeline(wgpu::ToAPI(&desc));
}
...
TEST_F(ErrorMaskingTests, CreateRenderPipeline) {
    ...
    EXPECT_CALL(mDeviceLostCb,
                Call(WGPUDeviceLostReason_Undefined, HasSubstr(kOomErrorMessage), this))
        .Times(1);
    device.CreateRenderPipeline(wgpu::ToAPI(&desc));
}
...
TEST_F(ErrorMaskingTests, CreateComputePipelineAsync) {
    ...
    EXPECT_CALL(cb, Call(WGPUCreatePipelineAsyncStatus_Error, _, HasSubstr(kOomErrorMessage), this))
        .Times(1);

    device.CreateComputePipelineAsync(wgpu::ToAPI(&desc), cb.Callback(), cb.MakeUserdata(this));
    device.Tick();
...
TEST_F(ErrorMaskingTests, CreateRenderPipelineAsync) {
    ...
    device.CreateRenderPipelineAsync(wgpu::ToAPI(&desc), cb.Callback(), cb.MakeUserdata(this));
    device.Tick();
