...
TEST(BlobTests, AlignTo) {
    ...
    for (size_t alignment : {1, 2, 4, 8, 16, 32}) {
        for (size_t offset = 0; offset < alignment; ++offset) {
            ...
            // If the Blob is not aligned, its data will be deleted and replaced with a new backing.
            if (!alreadyAligned) {
                EXPECT_CALL(mockDeleter, Call());
            }
            b.AlignTo(alignment);
            if (!alreadyAligned) {
                // Verify now instead of at the end of the scope.
                testing::Mock::VerifyAndClearExpectations(&mockDeleter);
            }
            // The blob should not be empty or have changed in size.
            EXPECT_FALSE(b.Empty());
            ...
            // If the blob was already aligned, it will not have been deleted before. It will be
            // deleted when the scope exits.
            if (alreadyAligned) {
                EXPECT_CALL(mockDeleter, Call());
            }
        }
