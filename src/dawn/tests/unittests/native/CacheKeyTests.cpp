...
namespace dawn::native {
    ...
    namespace {
        ...
        TEST(CacheKeyTest, Strings) {
            ...
            EXPECT_THAT(GetCacheKey("string0"), CacheKeyEq("{0:\"string0\"}"));
            EXPECT_THAT(GetCacheKey(str0), CacheKeyEq("{0:\"string0\"}"));
            EXPECT_THAT(GetCacheKey("string0", str1), CacheKeyEq("{0:\"string0\",1:\"string1\"}"));
        }

        TEST(CacheKeyTest, NestedCacheKey) {
            EXPECT_THAT(GetCacheKey(GetCacheKey((int)-1)), CacheKeyEq("{0:{0:-1}}"));
            EXPECT_THAT(GetCacheKey(GetCacheKey("string")), CacheKeyEq("{0:{0:\"string\"}}"));
