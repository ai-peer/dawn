...
namespace dawn::native {
    ...
    namespace {
        ...
        TEST(CacheKeySerializerTests, FloatingTypes) {
            ...
            EXPECT_THAT(CacheKey().Record(static_cast<float>(0)),
                        CacheKeyEq(CacheKey(sizeof(float), 0)));
            EXPECT_THAT(CacheKey().Record(static_cast<double>(0)),
                        CacheKeyEq(CacheKey(sizeof(double), 0)));
        }
