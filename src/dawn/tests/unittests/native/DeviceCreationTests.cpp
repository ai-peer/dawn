...
TEST_F(DeviceCreationTest,
    ...
    for (size_t i = 0; i < kTotalFeaturesCount; i++) {
        ...
        // disallow_unsafe_apis is instance stage toggle and will be ignored when required as device
        // toggle.
        {
...
TEST_F(DeviceCreationTest,
    ...
    for (size_t i = 0; i < kTotalFeaturesCount; i++) {
        ...
        // Test creating device with DisallowUnsafeApis disabled in device toggle descriptor would
        // also succeed, as disallow_unsafe_apis is instance stage toggle and will be ignored when
        // required as device toggle.
        {
            const char* const disableToggles[] = {"disallow_unsafe_apis"};
