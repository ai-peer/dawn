...
class DeviceCreationTest : public testing::Test {
  protected:
    void SetUp() override {
        ...
        WGPUDawnTogglesDescriptor unsafeAdapterTogglesDesc = {};
        // unsafeAdapterTogglesDesc.enabledTogglesCount = 0;
        unsafeAdapterTogglesDesc.disabledTogglesCount = 1;
        unsafeAdapterTogglesDesc.disabledToggles = &disallowUnsafeApisToggle;

        instance = std::make_unique<dawn::native::Instance>();
        // Discover adapters with no toggles
        instance->DiscoverDefaultAdapters();
        // Discover adapters with toggle DisallowUnsafeApis disabled
        instance->DiscoverDefaultAdapters(&unsafeAdapterTogglesDesc);
        ...
        for (dawn::native::Adapter& nativeAdapter : instance->GetAdapters()) {
            ...
            if (properties.backendType == wgpu::BackendType::Null) {
                if (nativeAdapter.IsCreatedWithRequiredToggles(&unsafeAdapterTogglesDesc)) {
