...
TEST_F(InstanceToggleTest, InstanceTogglesSet) {
    auto validateInstanceToggles = [](const dawn::native::Instance* nativeInstance,
        ...
        auto* instance = dawn::native::FromAPI(nativeInstance->Get());
        auto& instanceTogglesState = instance->GetTogglesState();
...
TEST_F(InstanceToggleTest, InstanceTogglesInheritToAdapterAndDevice) {
    auto validateInstanceTogglesInheritedToAdapter =
        [&](const dawn::native::Instance* nativeInstance) {
            auto* instance = dawn::native::FromAPI(nativeInstance->Get());
            auto& instaneTogglesState = instance->GetTogglesState();
            ...
            auto& adapterTogglesState = nullAdapter->GetAdapterTogglesState();
            ...
            for (int i = 0; i < static_cast<int>(dawn::native::Toggle::EnumCount); i++) {
                ...
                if (togglesInfo.GetToggleInfo(toggle)->stage !=
                    dawn::native::ToggleStage::Instance) {
                    continue;
                }
                EXPECT_EQ(instaneTogglesState.IsSet(toggle), adapterTogglesState.IsSet(toggle));
                EXPECT_EQ(instaneTogglesState.IsEnabled(toggle),
                          adapterTogglesState.IsEnabled(toggle));
                EXPECT_EQ(instaneTogglesState.IsEnabled(toggle),
                          nullDevice->IsToggleEnabled(toggle));
            }
...
// TODO: Add adapter toggles and their inheriteation test.
