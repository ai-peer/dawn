...
// Tests that for various deprecated path, with Toggle::DisallowDeprecatedAPIs off, it emits
...
TEST_F(DeprecatedPathToggleTests, MultisampledTextureSampleType) {
    ...
    {
        wgpu::Device deviceDisallowDeprecatedAPIs = CreateDeviceWithDeprecatedPathDisallowed();

        ASSERT_DEVICE_ERROR(utils::MakeBindGroupLayout(
            deviceDisallowDeprecatedAPIs,
            {
                {0, wgpu::ShaderStage::Compute, wgpu::TextureSampleType::Float,
                 wgpu::TextureViewDimension::e2D, true},
            }));
