...
// using BindingDescriptorGroups = std::vector<BindingDescriptor>;
...
std::string GenerateBindingString(const BindingDescriptorGroups& descriptors) {
    ...
    for (const auto& entries : descriptors) {
        ...
        //     b.textureView. format?
        //     ostream << "@group(" << groupIndex << ") @binding(" << bindingIndex << ") "
        //         << "var b" << index << " : texture_storage_2d<rgba8unorm, write>;\n";
...
std::string GenerateReferenceString(const BindingDescriptorGroups& descriptors,
    ...
    std::ostringstream ostream;
    size_t index = 0;
    if (stage == wgpu::ShaderStage::Compute || stage == wgpu::ShaderStage::Fragment) {
        for (const auto& entries : descriptors) {
            for (uint32_t bindingIndex = 0; bindingIndex < entries.size(); bindingIndex++) {
                ostream << "_ = textureDimensions(b" << index << ");\n";
                // ostream << "textureStore(b" << index
                //         << ", vec2<i32>(0, 0), 0, vec4<f32>(1.0, 1.0, 1.0, 1.0));\n";
                index++;
            }
        }
    }
  ...
  public:
    ...
    wgpu::BindGroupLayout CreateBindGroupLayout(
        ...
        for (size_t i = 0; i < bindings.size(); ++i) {
            ...
            // TODO: get texture format somewhere for depth stencil
...
TEST_F(WritableTextureBindingAliasingValidationTests, BasicTest) {
    ...
    // view2 and view3 intersects in mip levels
    ...
    // view4 and view5 intersects in array layers
    ...
    // view52 is created by another texture, so no aliasing at all.
    wgpu::TextureView view52 = textureStorage2.CreateView(&viewDescriptor5);
    ...
    std::vector<TestSet> testSet = {
        ...
        // same texture, subresources intersects in mip level
        {false,
         {{
             {0, view2},
             {1, view3},
         }}},
        // same texture, subresources don't intersect, in different bind group
        {true,
         {{
              {0, view0},
          },
        ...
        // same texture, subresources intersects in mip level, in different bind group
        {false,
         {{
              {0, view2},
          },
        ...
        // same texture, subresources intersects in array layers
        {false,
         {{
             {0, view4},
             {1, view5},
         }}},
        // same texture, subresources intersects in array layers, reverse to test overlap logic
        {false,
         {{
             {0, view5},
             {1, view4},
