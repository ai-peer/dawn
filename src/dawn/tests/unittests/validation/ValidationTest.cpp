...
void ValidationTest::RequestAdapterResolve(ValidationTest* self, wgpu::Adapter adapter) {
    self->adapter = adapter;
}

void ValidationTest::RequestDeviceResolve(ValidationTest* self, wgpu::Device device) {
    self->device = device;
    self->device.SetUncapturedErrorCallback(ValidationTest::OnDeviceError, self);
    self->device.SetDeviceLostCallback(ValidationTest::OnDeviceLost, self);
    self->mRequestDeviceResolved = true;
}
...
void ValidationTest::SetUp() {
    ...
    wgpuInstanceRequestAdapter(instance, nullptr, [](WGPURequestAdapterStatus status, WGPUAdapter adapter, const char* message, void* userdata) {
        ...
        ValidationTest::RequestAdapterResolve(reinterpret_cast<ValidationTest*>(userdata), wgpu::Adapter::Acquire(adapter));
        ...
        wgpuAdapterRequestDevice(adapter, nullptr, [](WGPURequestDeviceStatus status, WGPUDevice dev, const char* message, void* userdata) {
            ...
            ValidationTest::RequestDeviceResolve(reinterpret_cast<ValidationTest*>(userdata), device);
        }, userdata);
