...
TEST_F(CommandBufferValidationTest, PassEndedAgainMidAnotherPass) {
    // Error case, render pass ended again in another render pass
    {
        ...
        ASSERT_DEVICE_ERROR(
            pass0.End(), HasSubstr("Recording in an error or already ended [RenderPassEncoder]."));
        pass1.End();
        encoder.Finish();
    }
    ...
    // Error case, compute pass ended again in another compute pass
    {
        ...
        ASSERT_DEVICE_ERROR(
            pass0.End(), HasSubstr("Recording in an error or already ended [ComputePassEncoder]."));
        pass1.End();
        encoder.Finish();
    }
}
