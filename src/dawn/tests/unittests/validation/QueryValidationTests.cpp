...
TEST_F(ResolveQuerySetValidationTest, ResolveToInvalidBufferAndOffset) {
    ...
    constexpr uint64_t kBufferSize =
        (kQueryCount - 1) * sizeof(uint64_t) +
        kResolveQuerySetDestinationBufferOffsetAlignment /*destinationOffset*/;
    ...
    {
        wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
        encoder.ResolveQuerySet(querySet, 1, kQueryCount - 1, destination,
                                kResolveQuerySetDestinationBufferOffsetAlignment);
        wgpu::CommandBuffer commands = encoder.Finish();
    ...
    {
        wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
        encoder.ResolveQuerySet(querySet, 0, kQueryCount, destination,
                                kResolveQuerySetDestinationBufferOffsetAlignment / 2);
        ASSERT_DEVICE_ERROR(encoder.Finish());
    }
