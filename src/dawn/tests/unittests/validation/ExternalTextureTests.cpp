...
TEST_F(ExternalTextureTest, CreateExternalTextureWithErrorVisibleRect) {
    // Control case should succeed.
    {
        ...
        wgpu::ExternalTextureDescriptor externalDesc = CreateDefaultExternalTextureDescriptor();
        externalDesc.plane0 = texture.CreateView();
        wgpu::Extent3D visibleRect = {texture.GetWidth(), texture.GetHeight(),
                                      texture.GetDepthOrArrayLayers()};
        externalDesc.visibleRect = &visibleRect;
        device.CreateExternalTexture(&externalDesc);
    }
    ...
    // VisibleRect is OOB on width
    {
        ...
        wgpu::ExternalTextureDescriptor externalDesc = CreateDefaultExternalTextureDescriptor();
        wgpu::Extent3D visibleRectOOBOnWidth = {texture.GetWidth() + 1, texture.GetHeight(),
                                                texture.GetDepthOrArrayLayers()};
        externalDesc.plane0 = texture.CreateView();
        externalDesc.visibleRect = &visibleRectOOBOnWidth;
        ASSERT_DEVICE_ERROR(device.CreateExternalTexture(&externalDesc));
    }
    ...
    // VisibleRect is OOB on height
    {
        ...
        wgpu::ExternalTextureDescriptor externalDesc = CreateDefaultExternalTextureDescriptor();
        wgpu::Extent3D visibleRectOOBOnHeight = {texture.GetWidth(), texture.GetHeight() + 1,
                                                 texture.GetDepthOrArrayLayers()};
        externalDesc.plane0 = texture.CreateView();
        externalDesc.visibleRect = &visibleRectOOBOnHeight;
        ASSERT_DEVICE_ERROR(device.CreateExternalTexture(&externalDesc));
    }

    // VisibleRect is OOB on depthOrArrayLayers
    {
        wgpu::TextureDescriptor textureDescriptor = CreateTextureDescriptor();
        wgpu::Texture texture = device.CreateTexture(&textureDescriptor);

        wgpu::ExternalTextureDescriptor externalDesc = CreateDefaultExternalTextureDescriptor();
        wgpu::Extent3D visibleRectOOBOnDepthOrArrayLayers = {texture.GetWidth(),
                                                             texture.GetHeight(), 2};
        externalDesc.plane0 = texture.CreateView();
        externalDesc.visibleRect = &visibleRectOOBOnDepthOrArrayLayers;
        ASSERT_DEVICE_ERROR(device.CreateExternalTexture(&externalDesc));
    }
}

// Test submit external texture with plane is not submittable results in error.
