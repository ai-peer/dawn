...
// Creating a render pass with DawnRenderPassDescriptorRenderToSingleSampled chained struct without
...
TEST_F(MultisampledRenderPassDescriptorValidationTest,
    ...
    wgpu::DawnRenderPassDescriptorRenderToSingleSampled renderToSingleSampledDesc;
    renderToSingleSampledDesc.sampleCount = 4;
    utils::ComboRenderPassDescriptor renderPass({colorTextureView});
    renderPass.nextInChain = &renderToSingleSampledDesc;
...
class MSAARenderToSingleSampledRenderPassDescriptorValidationTest
  ...
  protected:
    void SetUp() override {
        ...
        mRenderToSingleSampledDesc.sampleCount = kSampleCount;
    }
    ...
    utils::ComboRenderPassDescriptor CreateMultisampledRenderToSingleSampledRenderPass(
        ...
        renderPass.nextInChain = &mRenderToSingleSampledDesc;
    ...
    wgpu::TextureView CreateCompatibleColorTextureView() {
        wgpu::Texture colorTexture =
            CreateTexture(device, wgpu::TextureDimension::e2D, kColorFormat, kSize, kSize,
                          kArrayLayers, kLevelCount, /*sampleCount=*/1,
  ...
  private:
    wgpu::DawnRenderPassDescriptorRenderToSingleSampled mRenderToSingleSampledDesc;
};

// When MSAARenderToSingleSampled is enabled in a render pass, all color attachments must be created
// with MSAARenderToSingleSampledAttachment usage.
...
// When MSAARenderToSingleSampled is enabled in a render pass, there must be no explicit resolve
// target in the descriptor.
TEST_F(MSAARenderToSingleSampledRenderPassDescriptorValidationTest,
       ValidateThereIsNoResolveTarget) {
    // Create a texture with sample count = 1.
...
// Using unsupported sample count in DawnRenderPassDescriptorRenderToSingleSampled chained struct
// should result in error.
TEST_F(MSAARenderToSingleSampledRenderPassDescriptorValidationTest, ValidateSupportedSampleCount) {
    // Create a texture with sample count = 1.
    auto textureView = CreateCompatibleColorTextureView();

    wgpu::DawnRenderPassDescriptorRenderToSingleSampled renderToSingleSampledDesc;
    renderToSingleSampledDesc.sampleCount = 3;
    utils::ComboRenderPassDescriptor renderPass({textureView});
    renderPass.nextInChain = &renderToSingleSampledDesc;

    AssertBeginRenderPassError(&renderPass, testing::HasSubstr("sampleCount (3) is not supported"));
}

// Depth stencil attachment's sample count must match the one specified in
// DawnRenderPassDescriptorRenderToSingleSampled.sampleCount.
TEST_F(MSAARenderToSingleSampledRenderPassDescriptorValidationTest,
       ValidateDepthStencilSampleCount) {
    // Create a color texture with sample count = 1.
    auto colorTextureView = CreateCompatibleColorTextureView();

    // Create depth stencil texture with sample count = 1. Which doesn't match sampleCount=4
