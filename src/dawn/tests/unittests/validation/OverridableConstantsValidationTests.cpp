...
// Test overrides validation with valid default values.
...
// Test overrides validation with zero as the default values.
...
// Test overrides validation with out-of-limits default values.
...
// Test overrides validation without default values specified.
...
// Test overrides validation after being overrided with invalid values.
TEST_F(ComputePipelineWorkgroupSizeOverridableConstantsValidationTest, ValidationAfterOverride) {
    SetUpShadersWithUninitializedConstants();
    {
        // Error: exceed maxComputeWorkgroupSizeZ
        std::vector<wgpu::ConstantEntry> constants{
            {nullptr, "x", 1}, {nullptr, "y", 1}, {nullptr, "z", 256}};
        ASSERT_DEVICE_ERROR(TestCreatePipeline(constants),
                            testing::HasSubstr("Entry-point uses workgroup_size("));
    }
    {
        // Error: exceed maxComputeInvocations
        std::vector<wgpu::ConstantEntry> constants{
            {nullptr, "x", 128}, {nullptr, "y", 128}, {nullptr, "z", 1}};
        ASSERT_DEVICE_ERROR(TestCreatePipeline(constants), testing::HasSubstr("invocations"));
    }
}
