...
class PixelLocalStorageTest : public ValidationTest {
  protected:
    ...
    void RecordRenderPass(const wgpu::RenderPassDescriptor* desc, wgpu::RenderPipeline pipeline = {}) {
        wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
...
// Check that the render pass and the pipeline must have both a PLS state, or both no PLS (even if empty)
