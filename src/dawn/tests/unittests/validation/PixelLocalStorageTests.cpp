...
// -- BeginRenderPass
// Check that same texture subresource cannot be used twice in BeginRenderPass (either render attachment or storage attachment).
// Check that using a storage attachment is a writable state (and cannot be used with usage as texture binding)
// Check that the storage attachment view's size must match the size of other attachments.
// Check that the storage attachment view must be from a StorageAttachment texture.
// Check that the load and store ops must not be undefined.
// Check that if the load op is clear, the clear value doesn't have nans.
// Check that PLS + RenderToSingleSampled cannot be used together.
