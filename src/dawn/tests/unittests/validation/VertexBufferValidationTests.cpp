...
// Check that vertex buffer stride requirements for draw command
...
// Check that instance buffer stride requirements with instanced attributes for draw command
...
// Check that vertex buffer stride requirements with instanced attributes for draw indexed command
...
// Check that last stride is computed correctly for vertex buffer with multiple attributes
TEST_F(VertexBufferValidationTest, DrawStrideLimitsVertexMultipleAttributes) {
    ...
    // Invalid: draw 3 elements, last stride is 12, 3 * 12 + 12 = 48 > 44
    encoder = device.CreateCommandEncoder();
    {
        wgpu::RenderPassEncoder pass = encoder.BeginRenderPass(&renderPass);
        pass.SetPipeline(pipeline2);
