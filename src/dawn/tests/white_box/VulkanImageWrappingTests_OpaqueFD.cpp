...
ExternalSemaphoreOpaqueFD::ExternalSemaphoreOpaqueFD(int handle) : mHandle(handle) {}

ExternalSemaphoreOpaqueFD::~ExternalSemaphoreOpaqueFD() {
    if (mHandle != -1) {
        close(mHandle);
    }
}

int ExternalSemaphoreOpaqueFD::AcquireHandle() {
    int handle = mHandle;
    mHandle = -1;
    return handle;
}

ExternalTextureOpaqueFD::ExternalTextureOpaqueFD(dawn::native::vulkan::Device* device,
                                                 int fd,
...
ExternalTextureOpaqueFD::~ExternalTextureOpaqueFD() {
    if (mFd != -1) {
        close(mFd);
    }
...
int ExternalTextureOpaqueFD::Dup() const {
    return dup(mFd);
}

VulkanImageWrappingTestBackendOpaqueFD::VulkanImageWrappingTestBackendOpaqueFD(
    const wgpu::Device& device)
    : mDevice(device) {
    mDeviceVk = dawn::native::vulkan::ToBackend(dawn::native::FromAPI(device.Get()));
}

bool VulkanImageWrappingTestBackendOpaqueFD::SupportsTestParams(const TestParams& params) const {
...
bool VulkanImageWrappingTestBackendOpaqueFD::Supported() const {
...
VulkanImageWrappingTestBackendOpaqueFD::CreateTexture(uint32_t width,
                                                      uint32_t height,
...
wgpu::Texture VulkanImageWrappingTestBackendOpaqueFD::WrapImage(
    const wgpu::Device& device,
...
bool VulkanImageWrappingTestBackendOpaqueFD::ExportImage(
...
::VkResult VulkanImageWrappingTestBackendOpaqueFD::CreateImage(
...
::VkResult VulkanImageWrappingTestBackendOpaqueFD::AllocateMemory(
    dawn::native::vulkan::Device* deviceVk,
    VkImage handle,
    VkDeviceMemory* allocation,
    VkDeviceSize* allocationSize,
    ...
    int bestType =
        deviceVk->GetResourceMemoryAllocator()->FindBestTypeIndex(requirements, MemoryKind::Opaque);
...
::VkResult VulkanImageWrappingTestBackendOpaqueFD::BindMemory(
    dawn::native::vulkan::Device* deviceVk,
    VkImage handle,
...
int VulkanImageWrappingTestBackendOpaqueFD::GetMemoryFd(dawn::native::vulkan::Device* deviceVk,
                                                        VkDeviceMemory memory) {
...
void VulkanImageWrappingTestBackendOpaqueFD::CreateBindExportImage(
    dawn::native::vulkan::Device* deviceVk,
    uint32_t width,
    uint32_t height,
    VkFormat format,
