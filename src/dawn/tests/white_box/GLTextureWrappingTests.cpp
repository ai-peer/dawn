...
class GLTextureTestBase : public DawnTest {
  protected:
    ...
    void SetUp() override {
        // Only ANGLE has the EGL_ANGLE_display_texture_share_group extension.
...
TEST_P(GLTextureValidationTests, Success) {
    ...
    wgpu::Texture texture = WrapGLTexture(&descriptor, glTexture.getTexture());
...
TEST_P(GLTextureValidationNoANGLETextureSharingTests, Failure) {
    DAWN_TEST_UNSUPPORTED_IF(UsesWire());
    ScopedGLTexture glTexture = CreateDefaultGLTexture();
    ASSERT_DEVICE_ERROR(wgpu::Texture texture = WrapGLTexture(&descriptor, glTexture.getTexture()));
...
TEST_P(GLTextureValidationTests, InvalidTextureUsage) {
    ...
    ASSERT_DEVICE_ERROR(texture = WrapGLTexture(&descriptor, glTexture.getTexture()));
...
TEST_P(GLTextureUsageTests, ClearR8GLTexture) {
    DAWN_TEST_UNSUPPORTED_IF(UsesWire());
    ScopedGLTexture eglImage = CreateGLTexture(1, 1, GL_R8, GL_RED, GL_UNSIGNED_BYTE, nullptr, 0);

    uint8_t data = 0x01;
    DoClearTest(eglImage.getTexture(), wgpu::TextureFormat::R8Unorm, GL_RED, GL_UNSIGNED_BYTE,
                &data, sizeof(data));
}
...
TEST_P(GLTextureUsageTests, ClearRG8GLTexture) {
    DAWN_TEST_UNSUPPORTED_IF(UsesWire());
    ScopedGLTexture eglImage = CreateGLTexture(1, 1, GL_RG8, GL_RG, GL_UNSIGNED_BYTE, nullptr, 0);

    uint16_t data = 0x0201;
    DoClearTest(eglImage.getTexture(), wgpu::TextureFormat::RG8Unorm, GL_RG, GL_UNSIGNED_BYTE,
                &data, sizeof(data));
}
...
TEST_P(GLTextureUsageTests, ClearRGBA8GLTexture) {
    DAWN_TEST_UNSUPPORTED_IF(UsesWire());
    ScopedGLTexture eglImage =
        CreateGLTexture(1, 1, GL_RGBA8, GL_RGBA, GL_UNSIGNED_BYTE, nullptr, 0);

    uint32_t data = 0x04030201;
    DoClearTest(eglImage.getTexture(), wgpu::TextureFormat::RGBA8Unorm, GL_RGBA, GL_UNSIGNED_BYTE,
                &data, sizeof(data));
}
