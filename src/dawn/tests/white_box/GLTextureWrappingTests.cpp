...
TEST_P(GLTextureUsageTests, ClearRGBA8GLTexture) {
    ScopedGLTexture glTexture =
        CreateGLTexture(1, 1, GL_RGBA8, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);

    uint32_t data = 0x04030201;
    DoClearTest(glTexture.Get(), wgpu::TextureFormat::RGBA8Unorm, GL_RGBA, GL_UNSIGNED_BYTE, &data, sizeof(data));
}

// Test sampling an imported RGBA8 GL texture
TEST_P(GLTextureUsageTests, SampleRGBA8GLTexture) {
    GLuint pixels[1] = { 0x40302010 };
    ScopedGLTexture glTexture =
        CreateGLTexture(1, 1, GL_RGBA8, GL_RGBA, GL_UNSIGNED_BYTE, pixels);

    // Get a texture view for the GL texture.
    wgpu::TextureDescriptor textureDescriptor;
    // FIXME: don't need CopySrc
    textureDescriptor.usage = wgpu::TextureUsage::TextureBinding | wgpu::TextureUsage::CopySrc;
    textureDescriptor.size = {1, 1, 1};
    textureDescriptor.format = wgpu::TextureFormat::RGBA8Unorm;

    wgpu::Texture wrappedTexture = WrapGLTexture(&textureDescriptor, glTexture.Get());
    ASSERT_NE(wrappedTexture, nullptr);

    // Create a color attachment texture.
    wgpu::TextureDescriptor attachmentDescriptor;
    attachmentDescriptor.usage = wgpu::TextureUsage::RenderAttachment | wgpu::TextureUsage::CopySrc;
...
//            output.color = vec4(0.0, 1.0, 0.0, 1.0);
    ...
    // Check the correct data was written
//    EXPECT_TEXTURE_EQ(pixels, wrappedTexture, {0, 0}, {1, 1});
    EXPECT_TEXTURE_EQ(pixels, attachment, {0, 0}, {1, 1});
//    ASSERT_EQ(0, memcmp(result.data(), data, dataSize));
}
