...
DAWN_TEST_PARAM_STRUCT(GPUTimestampCalibrationTestParams, FeatureName);
...
class ExpectBetweenTimestamps : public ::detail::Expectation {
  public:
    ...
    // Expect the actual results are between mValue0 and mValue1 with error tolerance ratio.
    testing::AssertionResult Check(const void* data, size_t size) override {
        const uint64_t* actual = static_cast<const uint64_t*>(data);

        if (mErrorToleranceRatio != 0.0f) {
            mMinValue = mMinValue - static_cast<uint64_t>(
                                        static_cast<double>(mMinValue * mErrorToleranceRatio));
            mMaxValue = mMaxValue + static_cast<uint64_t>(
                                        static_cast<double>(mMaxValue * mErrorToleranceRatio));
        }
...
class D3D12GPUTimestampCalibrationTests
  ...
  protected:
    ...
    void RunTest(EncoderType encoderType) {
        ...
        switch (encoderType) {
            case EncoderType::NonPass: {
                // The "timestamp-query-inside-passes" is not supported on command encoder
...
// GetClockCalibration() with the timestamp conversion disabled or enabled.
TEST_P(D3D12GPUTimestampCalibrationTests, TimestampsCalibration) {
    // The "timestamp-query-inside-passes" is not supported on command encoder
    if (GetParam().mFeatureName == wgpu::FeatureName::TimestampQuery) {
        RunTest(EncoderType::NonPass);
    }

    RunTest(EncoderType::ComputePass);

    RunTest(EncoderType::RenderPass);
}
...
DAWN_INSTANTIATE_TEST_P(
    ...
    {D3D12Backend({"disable_timestamp_query_conversion"}, {}),
     D3D12Backend({}, {"disable_timestamp_query_conversion"})},
    std::vector<wgpu::FeatureName>({wgpu::FeatureName::TimestampQuery,
