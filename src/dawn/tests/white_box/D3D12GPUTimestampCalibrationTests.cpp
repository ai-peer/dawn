...
namespace {
    class ExpectGPUTimestampsBetween : public detail::Expectation {
      public:
        ~ExpectGPUTimestampsBetween() override = default;

        ExpectGPUTimestampsBetween(uint64_t value0, uint64_t value1) {
            mValue0 = value0;
            mValue1 = value1;
        }
...
TEST_P(D3D12GPUTimestampCalibrationTests, TimestampsInOrder) {
    ...
    EXPECT_BUFFER(destination, 0, kQueryCount * sizeof(uint64_t),
                  new ExpectGPUTimestampsBetween(gpuTimestamp0, gpuTimestamp1));
}
