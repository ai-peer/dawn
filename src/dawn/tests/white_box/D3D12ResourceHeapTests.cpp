...
// Verify the MSAA textures are 64KB alignment if it is supported on the current platform.
// Otherwise the alignment will be 4MB.
TEST_P(D3D12ResourceHeapTests, MSAATexture) {
    ...
    uint64_t expectedAlignment = HasToggleEnabled("d3d12_support_msaa_64kb_alignment_texture")
...
DAWN_INSTANTIATE_TEST(D3D12ResourceHeapTests,
                      D3D12Backend(),
                      D3D12Backend({}, {"d3d12_support_msaa_64kb_alignment_texture"}));
