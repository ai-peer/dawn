...
class QueryInternalShaderTests : public DawnTest {
  protected:
    ...
    void RunTest(uint32_t firstQuery,
                 ...
                 float period) {
        ASSERT(destinationOffset % kResolveQuerySetDestinationBufferOffsetAlignment == 0);
...
TEST_P(QueryInternalShaderTests, TimestampComputeShader) {
    ...
    for (float period : kPeriodsToTest) {
        ...
        RunTest(1, kQueryCount - 1, kResolveQuerySetDestinationBufferOffsetAlignment, period);
        ...
        RunTest(1, 4, kResolveQuerySetDestinationBufferOffsetAlignment, period);
    }
}
