...
ExternalSemaphoreDmaBuf::ExternalSemaphoreDmaBuf(int handle) : mHandle(handle) {}

ExternalSemaphoreDmaBuf::~ExternalSemaphoreDmaBuf() {
    if (mHandle != -1) {
        close(mHandle);
    }
}

int ExternalSemaphoreDmaBuf::AcquireHandle() {
    int handle = mHandle;
    mHandle = -1;
    return handle;
}
...
int ExternalTextureDmaBuf::Dup() const {
    return dup(mFd);
}
...
bool VulkanImageWrappingTestBackendDmaBuf::SupportsTestParams(const TestParams& params) const {
...
bool VulkanImageWrappingTestBackendDmaBuf::Supported() const {
    return mDevice->GetDeviceInfo().HasExt(DeviceExt::ExternalMemoryFD) &&
           mDevice->GetDeviceInfo().HasExt(DeviceExt::ImageDrmFormatModifier);
}

std::unique_ptr<VulkanImageWrappingTestBackend::ExternalTexture>
VulkanImageWrappingTestBackendDmaBuf::CreateTexture(uint32_t width,
                                                    uint32_t height,
...
wgpu::Texture VulkanImageWrappingTestBackendDmaBuf::WrapImage(
    const wgpu::Device& device,
    ...
    const ExternalTextureDmaBuf* textureDmaBuf = static_cast<const ExternalTextureDmaBuf*>(texture);
    std::vector<int> waitFDs;
    for (auto& semaphore : semaphores) {
        waitFDs.push_back(static_cast<ExternalSemaphoreDmaBuf*>(semaphore.get())->AcquireHandle());
    }
...
bool VulkanImageWrappingTestBackendDmaBuf::ExportImage(
...
void VulkanImageWrappingTestBackendDmaBuf::CreateGbmDevice() {
...
gbm_bo* VulkanImageWrappingTestBackendDmaBuf::CreateGbmBo(uint32_t width,
                                                          uint32_t height,
                                                          bool linear) {
