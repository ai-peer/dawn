    ...
    for (uint32_t level = 0; level < mipmapLevels; ++level) {
        for (uint32_t layer = 0; layer < arrayLayers; ++layer) {
            ...
            if ([dsTex pixelFormat] == MTLPixelFormatDepth16Unorm) {
                ...
                uint16_t depthValue = rand() % 0xFFFF;
...
void WriteDepthWithLoadOp(id<MTLDevice> device,
                          ...
                          TextureData* data) {
    // Randomize initial value to mitigate reading results from a previous test iteration.
    uint16_t depthStartValue = rand() % 0xFFFF;
    ...
    for (uint32_t level = 0; level < mipmapLevels; ++level) {
        for (uint32_t layer = 0; layer < arrayLayers; ++layer) {
            ...
            [commandBuffer addCompletedHandler:^(id<MTLCommandBuffer>) {
                if ([dsTex pixelFormat] == MTLPixelFormatDepth16Unorm) {
                    auto* contents = static_cast<uint16_t*>([buffer contents]);
                    if (memcmp(contents, data[level][layer].Get<U16>().data(),
                               data[level][layer].Get<U16>().size()) != 0) {
                        failed = true;
                        if (level == 0 && layer == 0) {
                            failedFor0thSubresource = true;
                        }
        ...
        for (uint32_t layer = 0; layer < arrayLayers; ++layer) {
                                                        ...
                                                        options:MTLResourceStorageModeShared];
            {
                ...
                for (size_t i = 0; i < data[level][layer].Get<F32>().size(); ++i) {
                    ...
                    contents0[i] = data[level][layer].Get<F32>()[i] - 0.002;
                    contents1[i] = data[level][layer].Get<F32>()[i] + 0.002;
                }
            }
...
uint32_t GetEntryProperty(io_registry_entry_t entry, CFStringRef name) {
    ...
    auto data = static_cast<CFDataRef>(IORegistryEntrySearchCFProperty(
            ...
            for (ReadMethod rm : readMethods) {
                ...
                for (bool copyT2T : {false, true}) {
                    for (MTLPixelFormat pixelFormat : {
                        ...
                        if (copyT2T) {
                            LOG(@"%@, %@ (%04x:%04x), %s, %s, CopyT2T-%s", osVersion, [device name], vendorId, deviceId, ToString(pixelFormat),
                                ToString(wm), ToString(rm));
                        } else {
                            LOG(@"%@, %@ (%04x:%04x), %s, %s, %s", osVersion, [device name], vendorId, deviceId, ToString(pixelFormat),
                                ToString(wm), ToString(rm));
                        }
