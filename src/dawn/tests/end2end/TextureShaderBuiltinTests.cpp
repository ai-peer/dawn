...
class TextureShaderBuiltinTests : public DawnTest {
  protected:
    ...
    // void DoTest(
    //     std::map<uint32_t, wgpu::BindGroup> bindGroups,
    ...
    //     bufferDesc.size = expected.size() * sizeof(uint32_t);
...
@compute @workgroup_size(1, 1, 1) fn main() {
    ...
    dst_buf[3] = textureNumLevels(tex3);
    }
    )");
    pipelineDescriptor.compute.entryPoint = "main";
    wgpu::ComputePipeline pipeline = device.CreateComputePipeline(&pipelineDescriptor);
...
// // Test calling textureNumLevels & textureNumSamples with an indirect texture argument.
// TEST_P(TextureShaderBuiltinTests, IndirectTextureArgument) {
//     constexpr uint32_t kLayers = 3;
//     constexpr uint32_t kMipLevels1 = 2;
//     wgpu::Texture tex1 = CreateTexture(kLayers, kMipLevels1, 1);
//     wgpu::TextureView texView1 = CreateTextureView(tex1, wgpu::TextureViewDimension::e2DArray);
...
// // not constructable
// fn f(selector: u32) -> texture_2d_array<f32> {
//     switch selector {
//         case 1u: {      // The colon is optional
//             return tex1;
//         }
...
// @compute @workgroup_size(1, 1, 1) fn main() {
//     dst_buf[0] = textureNumLayers(tex1); // control case
//     dst_buf[1] = textureNumLevels(f(1u));
//     dst_buf[2] = textureNumLevels(f(2u));
// }
//     )");
//     pipelineDescriptor.compute.entryPoint = "main";
//     wgpu::ComputePipeline pipeline = device.CreateComputePipeline(&pipelineDescriptor);
...
@compute @workgroup_size(1, 1, 1) fn main2() {
    ...
    wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
    {
        ...
        // pass.SetBindGroup(1, utils::MakeBindGroup(device, pipeline_2.GetBindGroupLayout(1),
        //                                         {
        //                                             {3, texView3_2},
        //                                         }));
        pass.DispatchWorkgroups(1);
    ...
    // constexpr uint32_t kExpectedByteSize = 3 * sizeof(uint32_t);

    // auto test = [&](const uint32_t* expected, const char* entryPoint,
    //     const wgpu::TextureView& texViewSlot1,
    //     const wgpu::TextureView& texViewSlot2,
    //     const wgpu::TextureView& texViewSlot3) {
    //     wgpu::BufferDescriptor bufferDesc;
    //     bufferDesc.size = kExpectedByteSize;
    //     bufferDesc.usage = wgpu::BufferUsage::Storage | wgpu::BufferUsage::CopySrc;
    //     wgpu::Buffer buffer = device.CreateBuffer(&bufferDesc);

    //     wgpu::ComputePipelineDescriptor pipelineDescriptor;
    //     pipelineDescriptor.compute.module = utils::CreateShaderModule(device, shader);
    //     pipelineDescriptor.compute.entryPoint = entryPoint;
    //     wgpu::ComputePipeline pipeline = device.CreateComputePipeline(&pipelineDescriptor);
    ...
    //     EXPECT_BUFFER_U32_RANGE_EQ(expected, buffer, 0, kExpectedByteSize / sizeof(uint32_t));
    // };

    // constexpr uint32_t e1[] = {
    //     kLayers,
    //     kMipLevels,
    //     kSampleCount,
    // };
    // ASSERT(sizeof(e1) == kExpectedByteSize);
    // test(e1, "main1", texView1, texView2, texView3);

    // constexpr uint32_t e2[] = {
    //     kLayers,
    //     kMipLevelsView,
    //     kSampleCount,
    // };
    // ASSERT(sizeof(e2) == kExpectedByteSize);
    // test(e2, "main2", texView3, texView2, texView1);
}

//
// TEST_P(TextureShaderBuiltinTests, TexturesWithSamplers) {
//     constexpr uint32_t kLayers = 3;
//     constexpr uint32_t kMipLevels = 2;
//     wgpu::Texture tex1 = CreateTexture(kLayers, kMipLevels, 1);
//     wgpu::TextureView texView1 = CreateTextureView(tex1, wgpu::TextureViewDimension::e2DArray);
...
//     // textureSample must be used in fragment stage
//     wgpu::ComputePipelineDescriptor pipelineDescriptor;
//     pipelineDescriptor.compute.module = utils::CreateShaderModule(device, R"(
// @group(0) @binding(0) var<storage, read_write> dst_buf : array<u32>;
// @group(0) @binding(3) var tex1 : texture_2d_array<f32>;
// @group(0) @binding(2) var sampler1 : sampler;
// @group(1) @binding(4) var sampler2 : sampler;

// @compute @workgroup_size(1, 1, 1) fn main() {
//     dst_buf[0] = textureNumLayers(tex1); // control case
//     // _ = textureSample(tex1, sampler1, vec2f(0.5, 0.5), 0);
//     // _ = textureSample(tex1, sampler2, vec2f(0.5, 0.5), 0);
//     dst_buf[1] = textureNumLevels(tex1);
//     }
//     )");
//     pipelineDescriptor.compute.entryPoint = "main";
...
// TODO: other texture type (storage, )
