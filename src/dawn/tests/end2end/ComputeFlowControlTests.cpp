...
void ComputeFlowControlTests::RunTest(const char* shader,
    ...
    queue.WriteBuffer(dst, 0, dst_init_values.data(), dst_init_values.size() * sizeof(uint32_t));
    EXPECT_BUFFER_U32_RANGE_EQ(dst_init_values.data(), dst, 0, dst_init_values.size());
...
fn main() {
  ...
  //push_output(0xA3);
  {
    // UHD 630 bug: outputs.count should be 1 here, but it's actually 0! Looks like a miscompilation due to the
    // conditional block `if (inputs[0] != 0)` above that depends on the input storage buffer read.
    var i = outputs.count; 
