            ...
            switch (operation.mType) {
                ...
                case OperationType::Padding: {
                    for (size_t i = 0; i < operation.mOperand; i++) {
                        buffer.push_back((operation.mType == OperationType::Data)
                                             ? NextDataByte()
                                             : NextPaddingByte());
                    }
                    break;
...
class Field {
  public:
    Field(std::string name, size_t align, size_t size) : mName(name), mAlign(align), mSize(size) {}

    const std::string& Name() const { return mName; }
    size_t Align() const { return mAlign; }
    // The natural size of this field type, i.e. the size without @size attribute
    size_t UnpaddedSize() const { return mSize; }
    // The padded size determined by @size attribute if existed, otherwise the natural size
    size_t PaddedSize() const { return mHasSizeAttribute ? mPaddedSize : mSize; }
    ...
    void MatchData(DataMatcherCallback callback) const {
        if (mIsStrided) {
            // Field is strided, calls `callback` with the strided intervals of length
            // mStrideDataBytes, skipping mStridePaddingBytes. For example: for a field of mSize =
            // 18, mStrideDataBytes = 2, and mStridePaddingBytes = 4, calls callback with the
            // intervals: [0, 2), [6, 8), [12, 14)
            size_t offset = 0;
            while (offset < mSize) {
            ...
            // Field is not strided and data bytes are placed continious, call the call back on the
            // whole data part.
            callback(0, mSize);
        }
    }
  ...
  private:
    std::string mName;  // Friendly name of the type of the field
    size_t mAlign;      // Natural alignment of the type in bytes
    size_t mSize;       // Natural size of the type in bytes
    ...
    bool mIsStrided = false;
    size_t mStrideDataBytes = 0;
    size_t mStridePaddingBytes = 0;
};

std::ostream& operator<<(std::ostream& o, Field field) {
    o << "@align(" << field.Align() << ") @size(" << field.PaddedSize() << ") " << field.Name();
    return o;
}
...
fn main() {
    ...
    shader = ReplaceAll(shader, "{field_size}", std::to_string(field.PaddedSize()));
    ...
    inputDataBuilder.ApplyOperationsToBuffer(inputData, dataKeyForInputAndExpectation,
                                             paddingKeyForInput);
    expectedDataBuilder.ApplyOperationsToBuffer(expectedData, dataKeyForInputAndExpectation,
                                                paddingKeyForDstInitAndExpectation);
    expectedDataBuilder.ApplyOperationsToBuffer(initData, dataKeyForDstInit,
                                                paddingKeyForDstInitAndExpectation);
    ...
    // Check the data
    field.MatchData([&](uint32_t offset, uint32_t size) {
...
fn main() {
    ...
    dataBuilder.ApplyOperationsToBuffer(inputData, dataKeyForInputAndExpectation,
                                        paddingKeyForInput);
    dataBuilder.ApplyOperationsToBuffer(expectedData, dataKeyForInputAndExpectation,
                                        paddingKeyForDstInitAndExpectation);
    dataBuilder.ApplyOperationsToBuffer(initData, dataKeyForDstInit,
                                        paddingKeyForDstInitAndExpectation);
