...
struct Field {
    ...
    size_t paddedSize = 0;  // Decorated size of the type in bytes indicated by @size attribute, 0
                            // if no @size attribute applied and natural size should be used
    ...
    // Applies a @size attribute, sets the paddedSize to value.
    ...
    // Sets the matcher to StridedDataMatcher, and record given strideDataBytes and
