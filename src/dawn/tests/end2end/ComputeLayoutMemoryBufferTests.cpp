...
// Set the following macro to 1 to print into stderr whether each test case that require f16 is run
// or skipped.
#define SHOW_F16_CASE_RUN_OR_SKIP 1
...
class MemoryDataBuilder {
  public:
    ...
    void ApplyOperationsToBuffer(std::vector<uint8_t>& buffer,
        ...
        // Get a data byte with the second most significant bit set to 0. By setting the second most
        // significant bit to 0, we avoid generating float point datas that is NaN or Inf.
        auto NextDataByte = [&]() {
            dataByte += 0x11u;
            return static_cast<uint8_t>((dataByte ^ dataXorKey) & 0xbfu);
        };
        // Get a padding byte with the second most significant bit set to 1, distinguished from data
        // bytes.
        auto NextPaddingByte = [&]() {
            paddingByte += 0x13u;
            return static_cast<uint8_t>((paddingByte ^ paddingXorKey) | 0x40u);
        };
...
TEST_P(ComputeLayoutMemoryBufferTests, StructMember) {
    ...
    if (field.IsRequireF16Feature()) {
        if (!device.HasFeature(wgpu::FeatureName::ShaderF16)) {
#if SHOW_F16_CASE_RUN_OR_SKIP
            fprintf(stderr,
                    ">>>>>>>>>>>>>>>>>>Skip the testcase as device not support shader-f16 "
                    "feature<<<<<<<<<<<<<<<<<<\n");
#endif
            return;
        }
#if SHOW_F16_CASE_RUN_OR_SKIP
        fprintf(stderr,
                "==================Run the testcase as device support shader-f16 "
                "feature==================\n");
#endif
    }
