...
std::vector<wgpu::FeatureName> BufferHostMappedPointerTests::GetRequiredFeatures() {
    if (!SupportsFeatures({wgpu::FeatureName::BufferHostMappedPointer})) {
        return {};
    }
    return {wgpu::FeatureName::BufferHostMappedPointer};
}

void BufferHostMappedPointerTests::SetUp() {
    DawnTestWithParams<BufferHostMappedPointerTestParams>::SetUp();
    DAWN_TEST_UNSUPPORTED_IF(!SupportsFeatures({wgpu::FeatureName::BufferHostMappedPointer}));
}
...
TEST_P(BufferHostMappedPointerNoFeatureTests, Creation) {
    ...
    ASSERT_DEVICE_ERROR_MSG(
        ...
        testing::HasSubstr(
            "SType::BufferHostMappedPointer requires FeatureName::BufferHostMappedPointer"));
}

DAWN_INSTANTIATE_TEST(BufferHostMappedPointerNoFeatureTests, MetalBackend());

// Test that memory allocations must be aligned to 4096.
TEST_P(BufferHostMappedPointerTests, Alignment) {
    ASSERT_DEVICE_ERROR(
        GetParam().mBackend->CreateHostMappedBuffer(device, wgpu::BufferUsage::CopySrc, 1024));

    ASSERT_DEVICE_ERROR(
        GetParam().mBackend->CreateHostMappedBuffer(device, wgpu::BufferUsage::CopySrc, 2048));

    GetParam().mBackend->CreateHostMappedBuffer(device, wgpu::BufferUsage::CopySrc, 4096);

    // Invalid: just below 4096
    ASSERT_DEVICE_ERROR(
        GetParam().mBackend->CreateHostMappedBuffer(device, wgpu::BufferUsage::CopySrc, 4095));

    // Invalid: just over 4096
    ASSERT_DEVICE_ERROR(
        GetParam().mBackend->CreateHostMappedBuffer(device, wgpu::BufferUsage::CopySrc, 4097));

    // Valid: multiple of 4096
    GetParam().mBackend->CreateHostMappedBuffer(device, wgpu::BufferUsage::CopySrc, 2 * 4096);
}
...
TEST_P(BufferHostMappedPointerTests, InitialDataAndCopySrc) {
    ...
    std::vector<uint32_t> expected(kBufferSize / sizeof(uint32_t));
    ...
    auto [buffer, ptr] = GetParam().mBackend->CreateHostMappedBuffer(
        device, wgpu::BufferUsage::CopySrc, kBufferSize,
        [&](void* initialPtr) { memcpy(initialPtr, expected.data(), kBufferSize); });
...
TEST_P(BufferHostMappedPointerTests, CopyDst) {
    ...
    // Create the buffer.
    auto [buffer, ptr] = GetParam().mBackend->CreateHostMappedBuffer(
        device, wgpu::BufferUsage::CopyDst, kBufferSize);
    ...
    memcpy(bufferSrc.GetMappedRange(), expected.data(), kBufferSize);
    bufferSrc.Unmap();

    // Do a GPU-GPU copy from the source buffer into the host-mapped buffer.
    wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
    encoder.CopyBufferToBuffer(bufferSrc, 0, buffer, 0, kBufferSize);
    ...
    EXPECT_EQ(memcmp(ptr, expected.data(), kBufferSize), 0);
}
