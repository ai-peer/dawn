...
struct ExpectationFloat {
    double y;
    double u;
    double v;
    utils::RGBA8 rgba;
    std::array<float, 4> rgba32Float;
};

using Expectation = ExpectationFloat;

std::ostream& operator<<(std::ostream& o, ExpectationFloat expectation) {
    o << expectation.rgba;
    return o;
}

DAWN_TEST_PARAM_STRUCT(FloatExpecation, Expectation);

class ExternalTextureFloatTests : public DawnTestWithParams<FloatExpecation> {
  protected:
    wgpu::ExternalTextureDescriptor CreateDefaultExternalTextureDescriptor() {
        wgpu::ExternalTextureDescriptor desc;
        desc.yuvToRgbConversionMatrix = kYuvToRGBMatrixBT709.data();
...
TEST_P(ExternalTextureTests, SampleMultiplanarExternalTexture) {
    ...
    std::array<ConversionExpectation, 7> expectations = {
        {{0.0, .5, .5, utils::RGBA8::kBlack, {0.0, 0.0, 0.0, 1.0}},
         {0.2126, 0.4172, 1.0, utils::RGBA8::kRed, {1.0, 0.0, 0.0, 1.0}},
         {0.7152, 0.1402, 0.0175, utils::RGBA8::kGreen, {0.0, 1.0, 0.0, 1.0}},
         {0.0722, 1.0, 0.4937, utils::RGBA8::kBlue, {0.0, 0.0, 1.0, 1.0}},
         {0.6382, 0.3232, 0.6644, {246, 169, 90, 255}, {0.9647, 0.6627, 0.3529, 1.0}},
         {0.5423, 0.5323, 0.4222, {120, 162, 169, 255}, {0.4706, 0.6353, 0.6627, 1.0}},
         {0.2345, 0.4383, 0.6342, {126, 53, 33, 255}, {0.4941, 0.2078, 0.1294, 1.0}}}};
...
TEST_P(ExternalTextureFloatTests, SampleMultiplanarExternalTextureFloat) {
    // TODO(crbug.com/dawn/1263): SPIR-V has an issue compiling the output from Tint's external
    // texture transform. Re-enable this test for OpenGL when the switch to Tint is complete.
    DAWN_SUPPRESS_TEST_IF(IsOpenGL() || IsOpenGLES());
    ...
    wgpu::Texture renderTexture =
        Create2DTexture(device, kWidth, kHeight, wgpu::TextureFormat::RGBA32Float,
                        wgpu::TextureUsage::CopySrc | wgpu::TextureUsage::RenderAttachment);
    wgpu::Texture renderTextureFloat =
        Create2DTexture(device, kWidth, kHeight, wgpu::TextureFormat::RGBA32Float,
                        wgpu::TextureUsage::CopySrc | wgpu::TextureUsage::RenderAttachment);
    ...
    ExpectationFloat expectation = GetParam().mExpectation;

    // Initialize the texture planes with YUV data
    {
        utils::ComboRenderPassDescriptor renderPass({externalViewPlane0, externalViewPlane1},
                                                    nullptr);
        renderPass.cColorAttachments[0].clearValue = {expectation.y, 0.0f, 0.0f, 1.0f};
        renderPass.cColorAttachments[1].clearValue = {expectation.u, expectation.v, 0.0f, 1.0f};
        wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
        wgpu::RenderPassEncoder pass = encoder.BeginRenderPass(&renderPass);
        pass.End();
    ...
    EXPECT_TEXTURE_EQ(expectation.rgba32Float.data(), renderTexture, {0, 0}, {1, 1},
                      wgpu::TextureFormat::RGBA32Float, tolerance);
}

DAWN_INSTANTIATE_TEST_P(
    ExternalTextureFloatTests,
    {D3D12Backend(), MetalBackend(), OpenGLBackend(), OpenGLESBackend(), VulkanBackend()},
    std::vector<ExpectationFloat>(
        {{0.0, .5, .5, utils::RGBA8::kBlack, {0.0, 0.0, 0.0, 1.0}},
         {0.2126, 0.4172, 1.0, utils::RGBA8::kRed, {1.0, 0.0, 0.0, 1.0}},
         {0.7152, 0.1402, 0.0175, utils::RGBA8::kGreen, {0.0, 1.0, 0.0, 1.0}},
         {0.0722, 1.0, 0.4937, utils::RGBA8::kBlue, {0.0, 0.0, 1.0, 1.0}},
