...
namespace {
    ...
    class ExternalTextureTests : public DawnTest {
      protected:
        ...
        void MultiplanarYUVConversionTest(ConversionExpectation expectation,
                                          wgpu::VideoSourceColorSpace sourceColorSpace) {
        ...
        @stage(fragment) fn main(@builtin(position) FragCoord : vec4<f32>)
            ...
            // Initialize the texture planes with YUV data
            {
                ...
                renderPass.cColorAttachments[1].clearValue = {expectation.u, expectation.v, 0.0f,
                                                              0.0f};
            ...
            externalDesc.videoSourceColorSpace = sourceColorSpace;
            ...
            // Run the shader, which should sample from the external texture and draw a triangle
            // into the upper left corner of the render texture.
...
TEST_P(ExternalTextureTests, SampleSinglePlaneExternalTexture) {
...
TEST_P(ExternalTextureTests, SampleMultiplanarExternalTextureBt601Limited) {
    // TODO(crbug.com/dawn/1263): SPIR-V has an issue compiling the output from Tint's external
    // texture transform. Re-enable this test for OpenGL when the switch to Tint is complete.
    DAWN_SUPPRESS_TEST_IF(IsOpenGL() || IsOpenGLES());

    // Y values are recommendation-defined constants Kr, Kg, Kb. UV values calculated by hand from
    // ITU-R Rec.601.
    std::array<ConversionExpectation, 4> expectations = {{{0.0, .5, .5, RGBA8::kBlack},
                                                          {0.299, 0.329, 1.0, RGBA8::kRed},
                                                          {0.587, 0.1402, 0.0175, RGBA8::kGreen},
                                                          {0.114, 1.0, 0.419, RGBA8::kBlue}}};

    for (ConversionExpectation expectation : expectations) {
        MultiplanarYUVConversionTest(expectation, wgpu::VideoSourceColorSpace::Bt601Limited);
    }
}

TEST_P(ExternalTextureTests, SampleMultiplanarExternalTextureBt709Limited) {
    // TODO(crbug.com/dawn/1263): SPIR-V has an issue compiling the output from Tint's external
    // texture transform. Re-enable this test for OpenGL when the switch to Tint is complete.
    DAWN_SUPPRESS_TEST_IF(IsOpenGL() || IsOpenGLES());
    ...
    for (ConversionExpectation expectation : expectations) {
        MultiplanarYUVConversionTest(expectation, wgpu::VideoSourceColorSpace::Bt709Limited);
    }
}
