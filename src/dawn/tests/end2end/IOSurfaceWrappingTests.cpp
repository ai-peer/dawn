...
TEST_P(IOSurfaceUsageTests, UninitializedOnEndAccess) {
    ...
    // Unitinialize the teuxture with a render pass.
...
TEST_P(IOSurfaceUsageTests, WriteThenConcurrentReadThenWrite) {
    ...
    writeExternDesc.waitEvents = {
        {endReadAccessDesc1.sharedEvent, endReadAccessDesc1.signaledValue},
        {endReadAccessDesc2.sharedEvent, endReadAccessDesc2.signaledValue}};
    writeExternDesc.isInitialized = true;
    ...
    encoder = writeDevice.CreateCommandEncoder();
    encoder.BeginRenderPass(&renderPassDescriptor).End();
