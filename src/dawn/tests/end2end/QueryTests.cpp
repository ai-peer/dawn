...
class TimestampQueryTests : public QueryTests {
  protected:
    ...
    void EncodeTimestampWrites(const wgpu::CommandEncoder& encoder,
    ...
    void TestTimestampWritesOnComputePass(
        ...
        wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
        EncodeTimestampWrites(encoder, timestampWrites, hasPipeline);

        // Begin another compute pass if the timestampWritesOnAnotherPass is set.
        if (!timestampWritesOnAnotherPass.empty()) {
            EncodeTimestampWrites(encoder, timestampWritesOnAnotherPass, hasPipeline);
        }
    ...
    void TestTimestampWritesOnRenderPass(
        ...
        wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
        EncodeTimestampWrites(encoder, timestampWrites, hasPipeline, hasFragmentStage);
