...
class TimestampQueryTests : public QueryTests {
  protected:
    ...
    void TestTimestampWritesOnComputePass(
        ...
        // The timestampWrites may contain different query set at the beginning and end of a pass,
        // we need to resolve them to buffer seperately. But it fails to resolve them to same buffer
        // with offset on Intel Windows Vulkan due to a driver issue that vkCmdFillBuffer and
        // vkCmdCopyQueryPoolResults are not executed in order when resolving queries. As a
        // workaround, we resolve each query set in timestampWrites to different buffers, and always
        // check the first value of each buffer.
        size_t queryCount = timestampWrites.size() + timestampWritesOnAnotherPass.size();
        std::vector<wgpu::Buffer> destinations;
        for (size_t i = 0; i < queryCount; i++) {
            destinations.push_back(CreateResolveBuffer(sizeof(uint64_t)));
        }
        ...
        if (!timestampWritesOnAnotherPass.empty()) {
            ...
            for (size_t i = 0; i < timestampWritesOnAnotherPass.size(); i++) {
                ...
                encoder.ResolveQuerySet(timestampWritesOnAnotherPass[i].querySet,
                                        timestampWritesOnAnotherPass[i].queryIndex, 1,
                                        destinations[timestampWrites.size() + i], 0);
            }
        }
        ...
        for (size_t i = 0; i < queryCount; i++) {
            EXPECT_BUFFER(destinations[i], 0, sizeof(uint64_t), new TimestampExpectation);
        }
    }
...
TEST_P(TimestampQueryTests, TimestampWritesOnComputePass) {
    // TODO (dawn:1473): Metal bug: sampleBufferAttachments cannot sample GPU counters on compute
    // pass.
...
TEST_P(TimestampQueryTests, TimestampWritesOnRenderPass) {
    // TODO (dawn:1473): Metal bug: MTLCounterDontSample cannot stop counter sampling.
    DAWN_SUPPRESS_TEST_IF(IsMacOS() && IsMetal() && IsApple());
