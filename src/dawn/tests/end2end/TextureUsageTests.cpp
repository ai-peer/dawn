...
TEST_P(MemorylessTexturesNotSupportedTest, TransientAttachmentCausesError) {
    ...
    sampleTextureDesc.usage = wgpu::TextureUsage::TransientAttachment;
    sampleTextureDesc.size = {1024, 1, 1};
    sampleTextureDesc.format = wgpu::TextureFormat::R8Unorm;
    ASSERT_DEVICE_ERROR(device.CreateTexture(&sampleTextureDesc));
}
