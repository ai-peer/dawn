...
TEST_P(D3D12SharedHandleMultithreadTests, ClearInD3D12ReadbackInD3D11_TwoThreads) {
    ...
    // KeyedMutex doesn't guaranteed the order of commands so skip it.
    ...
    std::thread d3d12Thread([=, &fenceMutex, &d3d12SignalFence] {
        wgpu::Texture dawnTexture;
        std::unique_ptr<dawn::native::d3d12::ExternalImageDXGI> externalImage;
        ...
        {
            std::lock_guard<std::mutex> lg(fenceMutex);
