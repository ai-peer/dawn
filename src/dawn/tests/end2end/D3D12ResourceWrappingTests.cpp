...
namespace {
    ...
    using UseKeyedMutex = bool;

    DAWN_TEST_PARAM_STRUCT(D3D12ResourceTestParams, UseKeyedMutex);
      ...
      protected:
        ...
        void WrapSharedHandle(const wgpu::TextureDescriptor* dawnDesc,
            ...
            if (!GetParam().mUseKeyedMutex) {
                ...
                hr = d3d11Device5->CreateFence(0, D3D11_FENCE_FLAG_SHARED,
            ...
            if (d3d11Fence != nullptr) {
                ...
                externalAccessDesc.fenceSignalValue = 1;
            }
...
class D3D12SharedHandleUsageTests : public D3D12ResourceTestBase {
  protected:
    ...
    void WrapAndClearD3D11Texture(const wgpu::TextureDescriptor* dawnDescriptor,
                                  ...
                                  uint64_t* fenceSignalValue = nullptr) const {
        ComPtr<ID3D11Texture2D> d3d11Texture;
        ...
        *d3d11TextureOut = d3d11Texture.Detach();
        ...
        if (fenceSignalValue != nullptr) {
            *fenceSignalValue = externalAccessDesc.fenceSignalValue;
        }
    }

    void ExpectPixelRGBA8EQ(ComPtr<ID3D11Texture2D> d3d11Texture,
                            ComPtr<ID3D11Fence> d3d11Fence,
        ...
        if (SUCCEEDED(hr)) {
            ...
            hr = d3d11DeviceContext4->Wait(d3d11Fence.Get(), fenceWaitValue);
            ASSERT_EQ(hr, S_OK);
        }
...
TEST_P(D3D12SharedHandleUsageTests, ClearInD3D12ReadbackInD3D11) {
    ...
    WrapAndClearD3D11Texture(&baseDawnDescriptor, &baseD3dDescriptor, &dawnTexture, d3d11ClearColor,
                             &d3d11Texture, true, &d3d11Fence, &fenceSignalValue);
    ASSERT_NE(dawnTexture.Get(), nullptr);
    ...
    ExpectPixelRGBA8EQ(d3d11Texture.Get(), d3d11Fence.Get(), fenceSignalValue, d3d12ClearColor);
}
...
TEST_P(D3D12SharedHandleUsageTests, ClearTwiceInD3D12ReadbackInD3D11) {
    ...
    WrapAndClearD3D11Texture(&baseDawnDescriptor, &baseD3dDescriptor, &dawnTexture, d3d11ClearColor,
                             &d3d11Texture, true, &d3d11Fence, &fenceCounter);
    ASSERT_NE(dawnTexture.Get(), nullptr);
    ...
    ExpectPixelRGBA8EQ(d3d11Texture.Get(), d3d11Fence.Get(), fenceCounter, d3d12ClearColor2);
}
...
TEST_P(D3D12SharedHandleUsageTests, ConcurrentExternalImageReadAccess) {
    ...
    externalAccessDesc.fenceWaitValue = 1;
    externalAccessDesc.fenceSignalValue = 0;  // do not signal
...
DAWN_INSTANTIATE_TEST_P(D3D12SharedHandleValidation, {D3D12Backend()}, {false, true});
DAWN_INSTANTIATE_TEST_P(D3D12SharedHandleUsageTests, {D3D12Backend()}, {false, true});
