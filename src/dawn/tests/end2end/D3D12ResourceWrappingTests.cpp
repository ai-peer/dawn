  ...
  protected:
    void WrapSharedHandle(
        ...
        if (GetParam().mSyncMode == SyncMode::kFence) {
            ...
            externalAccessDesc.waitFences.push_back({fenceSharedHandle, 0});
        }
  ...
  protected:
    ...
    void ExpectPixelRGBA8EQ(
        ...
        if (waitFence != nullptr && waitFence->fenceHandle != nullptr) {
            ...
            mD3d11Device.As(&d3d11Device1);

            ComPtr<ID3D11Fence> d3d11Fence;
            d3d11Device1->OpenSharedResource1(waitFence->fenceHandle, IID_PPV_ARGS(&d3d11Fence));
