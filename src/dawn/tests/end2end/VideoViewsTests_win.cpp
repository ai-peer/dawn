...
class VideoViewsTestBackendWin : public VideoViewsTestBackend {
  public:
    ...
    void OnSetUp(WGPUDevice device) override {
        ...
        ComPtr<ID3D12Device> d3d12Device = dawn::native::d3d::d3d12::GetD3D12Device(device);
  ...
  protected:
    ...
    std::unique_ptr<VideoViewsTestBackend::PlatformTexture> CreateVideoTextureForTest(
        ...
        std::unique_ptr<dawn::native::d3d::d3d12::ExternalImageDXGI> externalImage =
            dawn::native::d3d::d3d12::ExternalImageDXGI::Create(mWGPUDevice, &externalImageDesc);

        // Handle is no longer needed once resources are created.
        ::CloseHandle(sharedHandle);

        dawn::native::d3d::d3d12::ExternalImageAccessDescriptorDXGIKeyedMutex externalAccessDesc;
