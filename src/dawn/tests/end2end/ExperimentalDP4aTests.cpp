...
class ExperimentalDP4aTests : public DawnTest {
  protected:
    std::vector<wgpu::FeatureName> GetRequiredFeatures() override {
        mIsDP4aSupported = SupportsFeatures({wgpu::FeatureName::ChromiumExperimentalDp4a});
        if (!mIsDP4aSupported) {
            return {};
        }
    ...
    bool IsDP4aSupported() const { return mIsDP4aSupported; }

    bool mIsDP4aSupported = false;
};

TEST_P(ExperimentalDP4aTests, BasicDP4aFeaturesTest) {
    DAWN_TEST_UNSUPPORTED_IF(!IsDP4aSupported());
    ...
    if (!HasToggleEnabled("enable_chromium_experimental_dp4a")) {
...
DAWN_INSTANTIATE_TEST(ExperimentalDP4aTests,
                      D3D12Backend(),
                      D3D12Backend({"enable_chromium_experimental_dp4a"}));
