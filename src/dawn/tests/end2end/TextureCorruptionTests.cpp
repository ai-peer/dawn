...
enum class WriteType {
    ...
    Rendering,     // Write the tested texture via rendering the whole rectangle with solid color
                   // (0xFFFFFFFF)
    SamplingAndWrite,    // Write the tested texture via sampling from a temp texture and write the
                         // sampled data
    TextureLoadAndWrite  // Write the tested texture via textureLoad() from a temp texture and write
                         // the loaded data
};
...
std::ostream& operator<<(std::ostream& o, WriteType writeType) {
    switch (writeType) {
        ...
        case WriteType::Rendering:
            o << "Rendering";
            break;
        case WriteType::SamplingAndWrite:
            o << "SamplingAndWrite";
            break;
        case WriteType::TextureLoadAndWrite:
            o << "TextureLoadAndWrite";
            break;
    }
    return o;
}
...
class TextureCorruptionTests : public DawnTestWithParams<TextureCorruptionTestsParams> {
  protected:
    std::ostringstream& DoTest(wgpu::Texture texture,
        ...
        for (uint32_t i = 0; i < copySize.height; ++i) {
            for (uint32_t j = 0; j < copySize.width; ++j) {
                if (type == WriteType::SamplingAndWrite || type == WriteType::Rendering) {
    ...
    void CreatePipelineAndRender(wgpu::TextureView renderView,
        ...
        } else if (type == WriteType::SamplingAndWrite) {
            fragmentShader << R"(
        ...
        if (type != WriteType::Rendering) {
            ...
            if (type == WriteType::TextureLoadAndWrite) {
...
DAWN_INSTANTIATE_TEST_P(TextureCorruptionTests,
                        ...
                        {WriteType::WriteTexture, WriteType::B2TCopy, WriteType::Rendering,
                         WriteType::SamplingAndWrite, WriteType::TextureLoadAndWrite});
