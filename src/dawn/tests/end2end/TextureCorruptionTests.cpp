...
using DepthOrArrayLayerCount = uint32_t;
...
class TextureCorruptionTests : public DawnTestWithParams<TextureCorruptionTestsParams> {
  protected:
    void DoTest(wgpu::Texture texture,
                const wgpu::Extent3D textureSize,
                uint32_t depthOrArrayLayer,
                uint32_t copyValue,
                uint32_t bytesPerTexel) {
        uint32_t bytesPerRow = AlignUp(textureSize.width * bytesPerTexel, 256);
        ...
        uint32_t totalTexelNum = bufferSize / bytesPerTexel;
        ...
        std::vector<uint32_t> data(totalTexelNum * elementNumPerTexel, 0);
        ...
        switch (GetParam().mWriteType) {
            case WriteType::B2TCopy: {
                wgpu::CommandEncoder verifier = this->device.CreateCommandEncoder();
                verifier.CopyTextureToBuffer(&imageCopyTexture, &imageCopyResult, &copySize);
                wgpu::CommandBuffer commands = verifier.Finish();
                queue.Submit(1, &commands);
                EXPECT_BUFFER_U32_RANGE_EQ(data.data(), resultBuffer, 0,
...
TEST_P(TextureCorruptionTests, CopyTests) {
    ...
    uint32_t depthOrArrayLayerCount = GetParam().mDepthOrArrayLayerCount;
    wgpu::TextureFormat format = GetParam().mTextureFormat;
    wgpu::Extent3D textureSize = {width, height, depthOrArrayLayerCount};

    // Pre-allocate textures. The incorrect copy may corrupt neighboring textures, or depth slices
    // or layers.
    ...
    // Copy data and verify the result one by one for every depth slice or layer of every texture
