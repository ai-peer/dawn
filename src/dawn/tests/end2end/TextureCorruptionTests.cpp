...
class TextureCorruptionTests : public DawnTestWithParams<TextureCorruptionTestsParams> {
  protected:
    ...
    void DoTest() {
        ...
        // Multisample textures have only 1 layer.
        std::vector<uint32_t> testedLayers = {0};

        if (sampleCount == 1) {
            // Most 2d-array textures being tested have only 2 layers. But if the texture has a lot
            // of layers, select a few layers to test.
