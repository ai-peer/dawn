...
namespace {
enum class WriteType { WriteTexture, B2TCopy, Rendering, Sampling, TextureLoad };
...
std::ostream& operator<<(std::ostream& o, WriteType writeType) {
    switch (writeType) {
        ...
        case WriteType::B2TCopy:
            o << "B2TCopy";
            break;
        case WriteType::Rendering:
            o << "Rendering";
            break;
        case WriteType::Sampling:
            o << "Sampling";
            break;
        case WriteType::TextureLoad:
            o << "TextureLoad";
            break;
    }
    return o;
...
class TextureCorruptionTests : public DawnTestWithParams<TextureCorruptionTestsParams> {
  protected:
    std::ostringstream& DoTest(wgpu::Texture texture,
        ...
        for (uint32_t i = 0; i < copySize.height; ++i) {
            for (uint32_t j = 0; j < copySize.width; ++j) {
                if (type == WriteType::Sampling || type == WriteType::Rendering) {
        ...
        switch (type) {
            ...
            case WriteType::Sampling:
            case WriteType::TextureLoad: {
                // Write data into a single layer texture and read from this texture if needed
                wgpu::TextureView samplerView;
                if (type != WriteType::Rendering) {
                    wgpu::Texture singleLayerTexture = Create2DTexture(copySize);
                    wgpu::ImageCopyTexture copyToSingleLayerTexture =
                        utils::CreateImageCopyTexture(singleLayerTexture, 0, {0, 0, 0});
                    wgpu::TextureDataLayout textureDataLayout =
                        utils::CreateTextureDataLayout(0, bytesPerRow);
                    queue.WriteTexture(&copyToSingleLayerTexture, data.data(), bufferSize,
                                       &textureDataLayout, &copySize);
                    samplerView = singleLayerTexture.CreateView();
                }
                ...
                CreatePipelineAndRender(texture.CreateView(&viewDesc), samplerView, encoder, type);
                break;
            }
            default:
                break;
    ...
    void CreatePipelineAndRender(wgpu::TextureView renderView,
        ...
        } else if (type == WriteType::Sampling) {
            fragmentShader << R"(
        ...
        pipelineDescriptor.cFragment.module =
            utils::CreateShaderModule(device, fragmentShader.str().c_str());
...
DAWN_INSTANTIATE_TEST_P(TextureCorruptionTests,
                        ...
                        {100u, 200u},
                        {WriteType::WriteTexture, WriteType::B2TCopy, WriteType::Rendering,
                         WriteType::Sampling, WriteType::TextureLoad});
