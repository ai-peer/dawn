...
@stage(fragment) fn main(input : FragInputs) {
    ...
    // Per the specification, if indirect-first-instance is not enabled, Draw[Indexed]Indirect with
    // firstInstance > 0 will be a no-op. The buffer should still have the values from the first
    // draw.
    if (firstInstance > 0 &&
        (mode == DrawMode::NonIndexedIndirect || mode == DrawMode::IndexedIndirect) &&
