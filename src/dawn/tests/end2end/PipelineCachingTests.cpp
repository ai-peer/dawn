...
// Tests that pipeline creation on the same device uses frontend cache properly with special
// constant values.
TEST_P(SinglePipelineCachingTests, ComputePipelineFrontedCacheSpecialConstantValues) {
    wgpu::ComputePipelineDescriptor desc;
    std::vector<wgpu::ConstantEntry> constants{{nullptr, "a", 0.0}};
    ...
    // Creation with -0 value should still hit the cache.
    EXPECT_CALL(mMockCache, LoadData).Times(0);
    wgpu::ComputePipeline samePipelineDifferent;
    std::vector<wgpu::ConstantEntry> differentConstants{{nullptr, "a", -0.0}};
    desc.compute.constants = differentConstants.data();
    desc.compute.constantCount = differentConstants.size();
    EXPECT_CACHE_STATS(mMockCache, Hit(0), Add(0),
                       samePipelineDifferent = device.CreateComputePipeline(&desc));
    EXPECT_EQ(pipeline.Get() == samePipelineDifferent.Get(), !UsesWire());
}
