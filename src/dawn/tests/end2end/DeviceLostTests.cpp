...
TEST_P(DeviceLostTest, LoseDeviceForTestingOnce) {
    ...
    // Second LoseDeviceForTesting call should result in no callbacks. The LoseForTesting will
    // return without doing anything when it sees that device has already been lost.
    LoseDeviceForTesting();
}
