  ...
  protected:
    ...
    void RunTest(TestParams params, const utils::RGBA8& expectation) {
        wgpu::ShaderModule fsModule = utils::CreateShaderModule(device, R"(
                ...
                @group(0) @binding(0) var<uniform> fragIn : FragIn;
                ...
                @fragment fn main() -> FragOut {
                  var output : FragOut;
                  output.color = fragIn.color;
                  output.blend = fragIn.blend;
                  return output;
                }
            )");
        ...
        wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
        {
            ...
            wgpu::BindGroup testColors = MakeBindGroupForColors(
                std::array<utils::RGBA8, 2>({{params.testColorIndex0}, {params.testColorIndex1}}));
            pass.SetBindGroup(0, testColors);
            pass.Draw(3);
            pass.End();
        }
        ...
        EXPECT_PIXEL_RGBA8_EQ(expectation, renderPass.color, kRTSize / 2, kRTSize / 2);
    }
    ...
    wgpu::BindGroup MakeBindGroupForColors(std::array<utils::RGBA8, 2> colors) {
        ...
        uint32_t bufferSize = static_cast<uint32_t>(8 * sizeof(float));
