...
TEST_P(SharedTextureMemoryTests, ImportSharedFenceNoChain) {
    ...
    ASSERT_DEVICE_ERROR(wgpu::SharedFence fence = device.ImportSharedFence(&desc));
    ...
    for (wgpu::SharedTextureMemory memory :
        ...
        if (textureDesc.usage & wgpu::TextureUsage::RenderAttachment) {
            ...
            if (textureDesc.usage & wgpu::TextureUsage::CopyDst) {
                ...
                uint64_t data[2];
                ASSERT_DEVICE_ERROR_MSG(device.GetQueue().WriteTexture(&dest, &data, sizeof(data),
...
TEST_P(SharedTextureMemoryTests, AccessStack) {
    ...
    for (size_t i = 0; i < memories.size(); ++i) {
        ...
        ASSERT_EQ(endState.fenceCount, ti2BeginDesc.fenceCount);
        for (size_t j = 0; j < endState.fenceCount; ++j) {
            EXPECT_EQ(endState.fences[j].Get(), ti2BeginDesc.fences[j].Get());
            EXPECT_EQ(endState.signaledValues[j], ti2BeginDesc.signaledValues[j]);
        }
        ...
        if (i + 1 < memories.size()) {
            ...
            ASSERT_EQ(endState.fenceCount, tj3BeginDesc.fenceCount);
            for (size_t j = 0; j < endState.fenceCount; ++j) {
                EXPECT_EQ(endState.fences[j].Get(), tj3BeginDesc.fences[j].Get());
                EXPECT_EQ(endState.signaledValues[j], tj3BeginDesc.signaledValues[j]);
            }
        }
        ...
        if (i + 1 < memories.size()) {
            ...
            ASSERT_EQ(endState.fenceCount, tj5BeginDesc.fenceCount);
            for (size_t j = 0; j < endState.fenceCount; ++j) {
                EXPECT_EQ(endState.fences[j].Get(), tj5BeginDesc.fences[j].Get());
                EXPECT_EQ(endState.signaledValues[j], tj5BeginDesc.signaledValues[j]);
            }
        }
    }
}
