...
class MultithreadCachingTests : public MultithreadTests {
  protected:
    ...
    void RunInParallel(const std::function<void(uint32_t)>& workerFunc) {
        ...
        utils::RunInParallel(numThreads, workerFunc);
    }
};
...
// resources creation and texture to buffer copy for texture expectations).
// This test is needed since since CopyTextureForBrowser() command might internally allocate
// resources and we need to make sure that it won't race with other threads' works.
