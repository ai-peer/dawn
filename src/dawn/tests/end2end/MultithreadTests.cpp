...
TEST_P(MultithreadTests, Buffers_MapInParallel) {
    ...
    RunInParallel(10, [=, &myData = std::as_const(myData)](uint32_t) {
        wgpu::Buffer buffer;
...
TEST_P(MultithreadTests, CreateComputePipelineAsyncInParallel) {
    ...
    RunInParallel(static_cast<uint32_t>(pipelines.size()),
                  [&pipelines, CreatePipelineFunc](uint32_t index) {
                      pipelines[index] = CreatePipelineFunc();
                  });

    for (auto& pipeline : pipelines) {
        VerifyPipelineFunc(pipeline.Get());
    }
}
...
TEST_P(MultithreadTests, RenderPassEncoders_ResolveToMipLevelOne_InParallel) {
