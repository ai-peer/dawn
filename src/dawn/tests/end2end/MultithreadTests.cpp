...
TEST_P(MultithreadTests, Device_DroppedOnAnotherThread) {
    ...
    std::thread thread([&lockStep, device2] {
        device2.PushErrorScope(wgpu::ErrorFilter::Validation);

        bool done = false;
        device2.PopErrorScope(
            [](WGPUErrorType type, const char*, void* userdataPtr) {
                auto userdata = static_cast<bool*>(userdataPtr);
                *userdata = true;
            },
            &done);
        device2.Tick();
