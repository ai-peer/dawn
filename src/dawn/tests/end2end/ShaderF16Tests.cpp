...
fn FSMain(@location(0) color : vec4<f32>) -> @location(0) vec4<f32> {
    ...
    ASSERT(colors.size() == pointNumber);
    // Color (devided by 4.0) for each point
    ...
    wgpu::Buffer vertexBufferPos = utils::CreateBufferFromData(
        device, positionData.data(), 2 * pointNumber * sizeof(float), wgpu::BufferUsage::Vertex);
    // Buffer for color_quarter
    wgpu::Buffer vertexBufferColor = utils::CreateBufferFromData(
        device, colorData.data(), 4 * pointNumber * sizeof(float), wgpu::BufferUsage::Vertex);
    ...
    wgpu::Texture renderTarget;
    {
        wgpu::TextureDescriptor descriptor;
        descriptor.dimension = wgpu::TextureDimension::e2D;
        descriptor.size.width = pointNumber;
