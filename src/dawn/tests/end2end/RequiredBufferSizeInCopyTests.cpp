...
namespace {
    ...
    using NumOfRowsPerImagePadding = uint64_t;
    DAWN_TEST_PARAM_STRUCT(RequiredBufferSizeInCopyTestsParams,
                           Type,
                           TextureDimension,
                           CopySize,
                           NumOfRowsPerImagePadding);
}  // namespace
...
TEST_P(RequiredBufferSizeInCopyTests, AbundantBufferSize) {
    ...
    const uint64_t numOfRowsPerImagePadding = GetParam().mNumOfRowsPerImagePadding;
    const uint64_t rowsPerImage = numOfRowsPerImagePadding + copySize.height;
...
TEST_P(RequiredBufferSizeInCopyTests, BufferSizeOnBoundary) {
    ...
    if (numOfRowsPerImagePadding > 0) {
        ...
        bool mayNeedWorkaround = GetParam().mTextureDimension == wgpu::TextureDimension::e3D &&
                                 numOfRowsPerImagePadding > 0 && copySize.depthOrArrayLayers > 1;
...
DAWN_INSTANTIATE_TEST_P(
    ...
    std::vector<Type>({Type::T2BCopy, Type::B2TCopy}),
    std::vector<wgpu::TextureDimension>({wgpu::TextureDimension::e3D, wgpu::TextureDimension::e2D}),
    std::vector<wgpu::Extent3D>({{1, 1, 2}, {1, 1, 1}}),
