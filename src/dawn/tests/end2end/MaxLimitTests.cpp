...
TEST_P(MaxLimitTests, WriteToMaxFragmentCombinedOutputResources) {
    // Currently does not work on GL and GLES.
    ...
    for (uint32_t i = 0; i < storageTextures; i++) {
        ...
        textureViews.push_back(textures[i].CreateView());
        textureEntries.push_back(
            utils::BindingInitializationHelper(i, textureViews[i]).GetAsBinding());
    }
    ...
    // Verify the results.
    for (uint32_t i = 0; i < storageBuffers; i++) {
        EXPECT_BUFFER_U32_EQ(i, buffers[i], 0);
    }
    for (uint32_t i = 0; i < storageTextures; i++) {
        EXPECT_PIXEL_RGBA8_EQ(utils::RGBA8(i, i, i, i), textures[i], 0, 0);
    }
    for (uint32_t i = 0; i < attachmentCount; i++) {
        EXPECT_PIXEL_RGBA8_EQ(utils::RGBA8(i, 0, 0, 0), attachments[i], 0, 0);
    }
}
