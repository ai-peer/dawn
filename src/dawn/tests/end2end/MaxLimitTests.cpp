...
// #9: 31x vec4<f32> + position in fragment shader
TEST_P(MaxInterStageShaderComponentsTests, vec4x31_position_fragment) {
    DAWN_TEST_UNSUPPORTED_IF(IsSwiftshader() || IsWARP() || IsANGLE());

    uint32_t maxInterStageComponentCount =
        GetSupportedLimits().limits.maxInterStageShaderComponents;
    DAWN_TEST_UNSUPPORTED_IF(maxInterStageComponentCount != 128 &&
                             maxInterStageComponentCount != 124);
    ...
    std::string fragmentShader = R"(
        ...
        @stage(fragment)
        fn main(input: VertexOut) -> @location(0) vec4<f32> {
            return input.color0 + input.color1 + input.color2 + input.color3 + input.color4 +
                   input.color5 + input.color6 + input.color7 + input.color8 + input.color9 +
                   input.color10 + input.color11 + input.color12 + input.color13 + input.color14 +
                   input.color15 + input.color16 + input.color17 + input.color18 + input.color19 +
                   input.color20 + input.color21 + input.color22 + input.color23 + input.color24 +
                   input.color25 + input.color26 + input.color27 + input.color28 + input.color29 +
                   input.color30 +input.frag_pos;
        })";
...
// #10: 31x vec4<f32> in fragment shader (+position in vertex shader)
TEST_P(MaxInterStageShaderComponentsTests, vec4x31_fragment) {
    DAWN_TEST_UNSUPPORTED_IF(IsSwiftshader() || IsWARP() || IsANGLE());
...
// #11: 30x vec4<f32> in fragment shader (+position in vertex shader)
TEST_P(MaxInterStageShaderComponentsTests, vec4x30_fragment) {
    DAWN_TEST_UNSUPPORTED_IF(IsSwiftshader() || IsWARP() || IsANGLE());
