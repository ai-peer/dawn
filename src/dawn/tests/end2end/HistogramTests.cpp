...
#include "dawn/platform/metrics/histogram_macros.h"
...
class DawnHistogramMockPlatform : public dawn::platform::Platform {
  public:
    explicit DawnHistogramMockPlatform() {
        // Enable by default
        Enable();
        SetTime(1.0);
    }
    ...
    // Returning a time of 0 implies disabling histograms.
    // This is the default in Dawn standalone builds.
...
TEST_P(HistogramTests, Times_Disabled) {
    InSequence seq;
    EXPECT_CALL(*mMockPlatform, HistogramCustomCounts).Times(0);

    mMockPlatform->Disable();
    DAWN_HISTOGRAM_TIMES(mMockPlatform, "times", 1);
    DAWN_HISTOGRAM_MEDIUM_TIMES(mMockPlatform, "medium_times", 2);
    DAWN_HISTOGRAM_LONG_TIMES(mMockPlatform, "long_times", 3);
    DAWN_HISTOGRAM_LONG_TIMES_100(mMockPlatform, "long_times_100", 4);
...
TEST_P(HistogramTests, Percentage_Disabled) {
    InSequence seq;
    EXPECT_CALL(*mMockPlatform, HistogramEnumeration).Times(0);
...
TEST_P(HistogramTests, Boolean_Disabled) {
    InSequence seq;
    EXPECT_CALL(*mMockPlatform, HistogramBoolean).Times(0);
...
TEST_P(HistogramTests, Enumeration_Disabled) {
    enum Animal { Dog, Cat, Bear, Count };
...
TEST_P(HistogramTests, Memory_Disabled) {
    InSequence seq;
    EXPECT_CALL(*mMockPlatform, HistogramCustomCounts).Times(0);
...
TEST_P(HistogramTests, Sparse) {
    ...
    DAWN_HISTOGRAM_SPARSE_SLOWLY(mMockPlatform, "sparse", 1);
    DAWN_HISTOGRAM_SPARSE_SLOWLY(mMockPlatform, "sparse", 2);
}

TEST_P(HistogramTests, Sparse_Disable) {
    InSequence seq;
    EXPECT_CALL(*mMockPlatform, HistogramSparse).Times(0);
...
TEST_P(HistogramTests, ScopedTimer_Disabled) {
    InSequence seq;
    EXPECT_CALL(*mMockPlatform, HistogramCustomCounts).Times(0);
