...
TEST_P(EventCompletionTests, WorkDone_Simple) {
    TrivialSubmit();
...
TEST_P(EventCompletionTests, WorkDone_AcrossDeviceLoss) {
    TrivialSubmit();
    TrackForTest(OnSubmittedWorkDone(WGPUQueueWorkDoneStatus_Success));
    TestWaitAll();
}
...
TEST_P(EventCompletionTests, WorkDone_AfterDeviceLoss) {
    TrivialSubmit();
    LoseTestDevice();
...
TEST_P(EventCompletionTests, WorkDone_Twice) {
    TrivialSubmit();
    TrackForTest(OnSubmittedWorkDone(WGPUQueueWorkDoneStatus_Success));
    TrackForTest(OnSubmittedWorkDone(WGPUQueueWorkDoneStatus_Success));
    TestWaitAll();
...
TEST_P(EventCompletionTests, WorkDone_NoWork) {
    TrackForTest(OnSubmittedWorkDone(WGPUQueueWorkDoneStatus_Success));
    TestWaitAll();
    TrackForTest(OnSubmittedWorkDone(WGPUQueueWorkDoneStatus_Success));
    TrackForTest(OnSubmittedWorkDone(WGPUQueueWorkDoneStatus_Success));
...
TEST_P(EventCompletionTests, WorkDone_AlreadyCompleted) {
    TrivialSubmit();
    TrackForTest(OnSubmittedWorkDone(WGPUQueueWorkDoneStatus_Success));
    TestWaitAll();
    TrackForTest(OnSubmittedWorkDone(WGPUQueueWorkDoneStatus_Success));
...
TEST_P(EventCompletionTests, WorkDone_OutOfOrder) {
...
TEST_P(EventCompletionTests, WorkDone_DropInstanceBeforeEvent) {
...
TEST_P(EventCompletionTests, WorkDone_DropInstanceAfterEvent) {
    // TODO(crbug.com/dawn/1987): Wire does not implement instance destruction correctly yet.
    DAWN_TEST_UNSUPPORTED_IF(UsesWire());
