...
TEST(AdapterEnumerationTests, MatchingDXGIAdapterD3D11) {
    ...
    for (uint32_t adapterIndex = 0;; ++adapterIndex) {
        ...
        wgpu::RequestAdapterOptionsLUID luidOptions = {};
        uint32_t high = static_cast<uint32_t>(adapterDesc.AdapterLuid.HighPart);
        uint32_t low = static_cast<uint32_t>(adapterDesc.AdapterLuid.LowPart);
        luidOptions.luid = (static_cast<uint64_t>(high) << 32) + low;
