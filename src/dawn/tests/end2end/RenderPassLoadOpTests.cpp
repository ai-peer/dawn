...
TEST_P(RenderPassLoadOpTests, LoadOpClearWithBig32BitIntegralValuesOnMultipleColorAttachments) {
    ...
    struct AttachmentCase {
        AttachmentCase(wgpu::TextureFormat format,
                       wgpu::Color clearValue,
                       size_t expSize,
                       const uint8_t* expData)
            : mFormat(format), mClearValue(clearValue), mExpSize(expSize) {
            memcpy(mExpData, expData, sizeof(mExpData));
        }
        static AttachmentCase Int(wgpu::TextureFormat format,
                                  wgpu::Color clearValue,
                                  const std::array<int32_t, 4>& expData) {
            return AttachmentCase(format, clearValue, sizeof(int32_t) * 4,
        ...
        static AttachmentCase Uint(wgpu::TextureFormat format,
            ...
            return AttachmentCase(format, clearValue, sizeof(uint32_t) * 4,
                                  reinterpret_cast<const uint8_t*>(expData.data()));
        }
