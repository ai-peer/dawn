...
TEST_P(RenderPassLoadOpTests, LoadOpClearWithBig32BitIntegralValuesOnMultipleColorAttachments) {
    ...
    struct AttachmentCase {
        AttachmentCase(wgpu::TextureFormat format, wgpu::Color clearValue)
            : mFormat(format), mClearValue(clearValue) {}
        wgpu::TextureFormat mFormat;
        wgpu::Color mClearValue;
        size_t mExpSize = 0;
        const uint8_t* mExpData = nullptr;
    };
    struct AttachmentCaseInt : public AttachmentCase {
        AttachmentCaseInt(wgpu::TextureFormat format,
                          wgpu::Color clearValue,
                          const std::array<int32_t, 4>& expected)
            : AttachmentCase(format, clearValue), mData(expected) {
            mExpSize = sizeof(mData);
            mExpData = reinterpret_cast<const uint8_t*>(mData.data());
        }
        std::array<int32_t, 4> mData;
    };
    struct AttachmentCaseUint : public AttachmentCase {
        AttachmentCaseUint(wgpu::TextureFormat format,
                           wgpu::Color clearValue,
    ...
    std::vector<TestCase> kTestCases = {
        // Full 8 attachment case (Signed 1 and 2 components).
        {AttachmentCaseInt(wgpu::TextureFormat::R32Sint, {kMaxInt32RepresentableInFloat, 0, 0, 0},
                           {kMaxInt32RepresentableInFloat, 0, 0, 0}),
         AttachmentCaseInt(wgpu::TextureFormat::R32Sint,
                           {kMaxInt32RepresentableInFloat + 1, 0, 0, 0},
                           {kMaxInt32RepresentableInFloat + 1, 0, 0, 0}),
         AttachmentCaseInt(wgpu::TextureFormat::R32Sint, {kMinInt32RepresentableInFloat, 0, 0, 0},
        ...
        // Signed 4 components.
        {AttachmentCaseInt(wgpu::TextureFormat::RGBA32Sint,
        ...
        {AttachmentCaseUint(wgpu::TextureFormat::R32Uint, {kMaxUInt32RepresentableInFloat, 0, 0, 0},
                            {kMaxUInt32RepresentableInFloat, 0, 0, 0}),
         AttachmentCaseUint(wgpu::TextureFormat::R32Uint,
                            {kMaxUInt32RepresentableInFloat + 1, 0, 0, 0},
                            {kMaxUInt32RepresentableInFloat + 1, 0, 0, 0})},

        // Unsigned 2 components.
        {AttachmentCaseUint(wgpu::TextureFormat::RG32Uint,
    ...
    for (const TestCase& testCase : kTestCases) {
        ...
        for (const AttachmentCase& attachmentCase : testCase) {
            ...
            wgpu::BufferDescriptor bufferDescriptor = {};
            bufferDescriptor.size = attachmentCase.mExpSize;
            bufferDescriptor.usage = wgpu::BufferUsage::CopySrc | wgpu::BufferUsage::CopyDst;
