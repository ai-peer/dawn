...
TEST_P(CopyTests_T2B, TextureMipUnaligned) {
    // TODO(dawn:1877): suppress
    for (auto* enabledToggle : GetParam().forceEnabledWorkarounds) {
        if (strcmp(enabledToggle, "use_blit_for_snorm_texture_to_buffer_copy") == 0) {
            DAWN_SUPPRESS_TEST_IF(IsSnorm(GetParam().mTextureFormat) && IsVulkan() && IsIntel() &&
                                  IsWindows());
            break;
        }
    }
    ...
    DAWN_SUPPRESS_TEST_IF(IsSnorm(GetParam().mTextureFormat) &&
                          (GetParam().forceEnabledWorkarounds || IsOpenGLES()));
    constexpr uint32_t kWidth = 259;
    constexpr uint32_t kHeight = 127;
