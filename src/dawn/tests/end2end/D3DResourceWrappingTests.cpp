  ...
  public:
    void SetUp() override {
        ...
        switch(GetParam().mExternalImageType) {
            case ExternalImageType::kSharedHandleFence:
                baseD3dDescriptor.MiscFlags = D3D11_RESOURCE_MISC_SHARED_NTHANDLE | D3D11_RESOURCE_MISC_SHARED;
                break;
            case ExternalImageType::kSharedHandleKeyedMutex:
                baseD3dDescriptor.MiscFlags = D3D11_RESOURCE_MISC_SHARED_NTHANDLE | D3D11_RESOURCE_MISC_SHARED_KEYEDMUTEX;
                break;
            case ExternalImageType::kD3D11Texture:
...
class D3DExternalImageUsageTests : public D3DResourceTestBase {
  protected:
    ...
    void ClearD3D11Texture(const wgpu::Color& clearColor,
        ...
        if (IsSharedHandleKeyedMutex()) {
            hr = dxgiResource.As(&dxgiKeyedMutex);
            ASSERT_EQ(hr, S_OK);
