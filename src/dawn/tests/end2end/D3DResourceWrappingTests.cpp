...
class D3DExternalImageUsageTests : public D3DResourceTestBase {
  protected:
    ...
    void CreateSharedD3D11Texture(const D3D11_TEXTURE2D_DESC& d3dDescriptor,
        ...
        ComPtr<ID3D11Device> d3d11Device =
            IsSharedHandle() ? mD3d11Device : native::d3d11::GetD3D11Device(device.Get());
    ...
    void ClearD3D11Texture(const wgpu::Color& clearColor,
        ...
        ComPtr<ID3D11Device> d3d11Device;
        d3d11Texture->GetDevice(d3d11Device.GetAddressOf());
        ...
        hr = d3d11Device->CreateRenderTargetView(d3d11Texture.Get(), nullptr, &d3d11RTV);
        ASSERT_EQ(hr, S_OK);
        ...
        d3d11DeviceContext->ClearRenderTargetView(d3d11RTV.Get(), colorRGBA);
    ...
    void WrapAndClearD3D11Texture(const wgpu::TextureDescriptor& dawnDescriptor,
        ...
        HANDLE fenceSharedHandle = nullptr;
        CreateSharedD3D11Texture(d3dDescriptor, d3d11Texture.GetAddressOf(),
                                 d3d11Fence.GetAddressOf(), &sharedHandle, &fenceSharedHandle);
    ...
    void ExpectPixelRGBA8EQ(ID3D11Texture2D* d3d11Texture,
        ...
        HRESULT hr =
            d3d11Device->CreateTexture2D(&texture2DStagingDesc, nullptr, &spD3DTextureStaging);
        ASSERT_EQ(hr, S_OK);
        ...
        d3d11DeviceContext->CopySubresourceRegion(spD3DTextureStaging.Get(),  // pDstResource
                                                  0,                          // DstSubresource
                                                  0,                          // DstX
                                                  0,                          // DstY
                                                  0,                          // DstZ
        ...
        hr = d3d11DeviceContext->Map(spD3DTextureStaging.Get(), 0, D3D11_MAP_READ_WRITE, 0,
                                     &mappedResource);
        ASSERT_EQ(hr, S_OK);
        ...
        d3d11DeviceContext->Unmap(spD3DTextureStaging.Get(), 0);
    }
};
...
TEST_P(D3DExternalImageMultithreadTests, ClearInD3D12ReadbackInD3D11_TwoThreads) {
    ...
    HANDLE sharedHandle = nullptr;
    HANDLE fenceSharedHandle = nullptr;
    CreateSharedD3D11Texture(baseD3dDescriptor, d3d11Texture.GetAddressOf(),
                             d3d11Fence.GetAddressOf(), &sharedHandle, &fenceSharedHandle);
...
class D3D11TextureValidation : public D3DResourceTestBase {};

// Test a successful wrapping of an D3D11Texture2D in a texture
TEST_P(D3D11TextureValidation, Success) {
    wgpu::Texture texture;
    ComPtr<ID3D11Texture2D> d3d11Texture;
    std::unique_ptr<native::d3d::ExternalImageDXGI> externalImage;
...
TEST_P(D3D11TextureValidation, InvalidD3D11Texture) {
    ComPtr<ID3D11Texture2D> d3d11Texture;
...
DAWN_INSTANTIATE_TEST_P(D3DExternalImageMultithreadTests,
                        {D3D11Backend(), D3D12Backend()},
                        {ExternalImageType::kSharedHandle, ExternalImageType::kD3D11Texture});
DAWN_INSTANTIATE_TEST_P(D3D11TextureValidation,
                        {D3D11Backend()},
