...
@compute @workgroup_size(x, 1, 1) fn main() {
    ...
    void TestInitializedWithOutOfLimitValue(const wgpu::Limits& limits) {
        std::vector<wgpu::ConstantEntry> constants{
            {nullptr, "x", static_cast<double>(limits.maxComputeWorkgroupSizeX + 1)},
            {nullptr, "y", 1},
            {nullptr, "z", 1}};
        TestCreatePipeline(constants);
    }
...
TEST_P(WorkgroupSizeValidationTest, OverridesWithValidDefault) {
    ...
    }
    {
        ...
        ASSERT_DEVICE_ERROR(TestInitializedWithOutOfLimitValue(GetSupportedLimits().limits));
    }
    {
...
TEST_P(WorkgroupSizeValidationTest, ValidationAfterOverrideStorageSize) {
    DAWN_SKIP_TEST_IF_BASE(true, "unsupported", "TODO(tint:1660)");
