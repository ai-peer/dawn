...
void DawnTestBase::LoseDeviceForTesting(wgpu::Device device) {
    ...
    // If the device isn't lost yet, then it should call the callback. Otherwise it should not call
    // it even if we reset it right before.
    testing::Mock::VerifyAndClearExpectations(&mDeviceLostCallback);
    if (dawn::native::FromAPI(resolvedDevice.Get())->IsLost()) {
        resolvedDevice.SetDeviceLostCallback(
            mDeviceLostCallback.Callback(), mDeviceLostCallback.MakeUserdata(resolvedDevice.Get()));
        EXPECT_CALL(mDeviceLostCallback, Call).Times(0);
    } else {
        EXPECT_CALL(mDeviceLostCallback,
