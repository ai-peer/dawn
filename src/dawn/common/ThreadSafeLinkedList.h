  ...
  public:
    ...
    class Iterator {
      public:
        class Cursor {
          public:
            explicit Cursor(LinkNode<T>* node) : mCurrent(node), mNext(node->next()) {}

            Cursor const& operator++() {
                mCurrent = mNext;
                mNext = mCurrent->next();
                return *this;
            }

            bool operator!=(const Cursor& other) const { return mCurrent != other.mCurrent; }

            T* operator*() const { return mCurrent->value(); }
        ...
        Iterator(Lock lock, const LinkedList<T>& list) : mLock(std::move(lock)), mList(list) {}

        Cursor begin() { return Cursor(mList.head()); }

        Cursor end() { return Cursor(mList.tail()->next()); }
    ...
    Iterator Iterate() const { return Iterator(Lock(mMutex), mList); }
