...
// Tagged-type to force special paths for EqualityFunc when dealing with Erase. When erasing, we
// only care about pointer equality, not value equality. This is also particularly important because
// otherwise, trying to promote on the Erase path can cause failures because the object's last ref
// could've been dropped already.
template <typename RefCountedT>
struct ForErase {
    ForErase(RefCountedT* value) : mValue(value) {}
    RefCountedT* mValue;
};
...
// The ContentLessObjectCache stores WeakRefs to living Refs without adding to their refcounts. This
// means that any RefCountedT that is inserted into the cache needs to make sure
// that their DeleteThis function erases itself from the cache. Otherwise, the cache can grow
// indefinitely via leaked pointers to deleted Refs.
template <typename RefCountedT>
class ContentLessObjectCache : public RefCounted,
  ...
  public:
    ...
    void Erase(RefCountedT* object) {
        std::lock_guard<std::mutex> lock(mMutex);
        auto it = mCache.find(detail::ForErase<RefCountedT>(object));
        if (it == mCache.end()) {
            return;
        }
        mCache.erase(it);
