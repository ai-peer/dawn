...
class ContentLessObjectCache {
  ...
  private:
    ...
    auto WithLockAndCleanup(F func) {
        ...
        RetType result;
        std::vector<Ref<RefCountedT>> temps;
        {
            std::lock_guard<std::mutex> lock(mMutex);
            result = func();
            temps.swap(mTemporaryRefs);
        }
        return result;
    }
    ...
    // The cache has a list of temporary Refs that are by-products of Promotes inside the
    // EqualityFunc. These Refs need to outlive the EqualityFunc calls because otherwise, they could
    // be the last living Ref of the object resulting in a re-entrant Erase call that deadlocks on
    // the mutex. See dawn:1993 for more details.
    std::vector<Ref<RefCountedT>> mTemporaryRefs;
};
