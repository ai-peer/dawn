...
#if DAWN_OS_IS(WINDOWS)
#include <Windows.h>
#include <vector>
#elif DAWN_OS_IS(LINUX)
#include <dlfcn.h>
#include <limits.h>
#include <unistd.h>
#include <cstdlib>
#elif DAWN_OS_IS(MACOS) || DAWN_OS_IS(IOS)
#include <dlfcn.h>
...
#elif DAWN_OS_IS(FUCHSIA)
std::optional<std::string> GetExecutablePath() {
    // UNIMPLEMENTED
    return {};
}
#elif DAWN_OS_IS(EMSCRIPTEN)
std::optional<std::string> GetExecutablePath() {
    return {};
}
#else
...
#if DAWN_OS_IS(LINUX) || DAWN_OS_IS(MACOS) || DAWN_OS_IS(IOS)
