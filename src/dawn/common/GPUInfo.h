...
class DriverVersion {
  public:
    ...
    DriverVersion(const std::initializer_list<uint16_t>& version) {
        mDriverVersion->resize(version.size());
        mDriverVersion->assign(version.begin(), version.end());
    }

    uint16_t& operator[](size_t i) { return mDriverVersion->operator[](i); }
    const uint16_t& operator[](size_t i) const { return mDriverVersion->operator[](i); }

    uint32_t size() const { return mDriverVersion->size(); }
