...
// is also assumed that the type already extends RefCounted and WeakRefSupport in some way.
// Otherwise, this helper class does not work.
template <typename RefCountedT>
class ContentLessObjectCacheable : public detail::ContentLessObjectCacheableBase {
  ...
  protected:
    ...
    virtual ~ContentLessObjectCacheable() { ASSERT(mCache == nullptr); }
