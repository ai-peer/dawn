...
GPUDevice::createComputePipelineAsync(Napi::Env env,
    ...
    auto ctx = std::make_unique<Context>(Context{env, Promise(env, PROMISE_INFO), AsyncTask(async_), {}});
    auto promise = ctx->promise;

    Converter conv(env);
    if (!conv(ctx->desc, descriptor)) {
...
void GPUDevice::setOnuncapturederror(Napi::Env env, interop::Interface<interop::EventHandler> value) {
    // TODO(dawn:1348): Implement support for the "unhandlederror" event.
    UNIMPLEMENTED(env);
}
