...
bool EventTarget::DispatchEvent(Napi::Env env, interop::Interface<interop::Event> event) {
    ...
    for (const Listener& listener : listeners) {
        if (const auto* callback =
            ...
            Napi::Object obj = std::get<interop::Interface<interop::EventListenerInterface>>(listener.callback);

            // TODO: does the dispatchEvent spec require any special behavior when failing to call handleEvent?
