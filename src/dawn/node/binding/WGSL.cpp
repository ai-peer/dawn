...
uint32_t parseVectorElement(std::string_view value) {
    if (value == "x" || value == "r") {
        return 0;
    }
    if (value == "y" || value == "g") {
...
struct TypeAndOffset {
    const tint::type::Type* type = nullptr;
    size_t offset = 0;
};

TypeAndOffset indexType(Napi::Env env,
                        const tint::Program* program,
                        const tint::type::Type* type,
                        uint32_t index) {
    return tint::Switch(
        ...
        [&](const tint::type::Vector* ty) {
            if (index >= ty->Width()) {
                return TypeAndOffset{};
            }
            auto* elTy = ty->type();
            return TypeAndOffset{elTy, elTy->Size() * index};
        },
        [&](const tint::type::Matrix* ty) {
            if (index >= ty->columns()) {
                return TypeAndOffset{};
            }
            return TypeAndOffset{ty->ColumnType(), ty->ColumnStride() * index};
        },
        [&](const tint::type::Array* ty) {
            if (auto count = ty->ConstantCount()) {
                if (index < *count) {
                    auto* elTy = ty->ElemType();
                    auto stride = align(elTy->Size(), elTy->Align());
                    return TypeAndOffset{elTy, stride * index};
                }
            }
            return TypeAndOffset{};
        },
        [&](tint::Default) { return TypeAndOffset{}; });
}
...
TypeAndOffset indexType(Napi::Env env,
    ...
    return tint::Switch(
        type,  //
        [&](const tint::type::Vector* ty) {
            auto index = parseVectorElement(name);
            if (index >= ty->Width()) {
                return TypeAndOffset{};
            }
            auto* elTy = ty->type();
        ...
        [&](const tint::type::Struct* ty) {
            if (auto sym = program->Symbols().Get(std::string(name)); sym.IsValid()) {
                if (auto* member = ty->FindMember(sym)) {
                    return TypeAndOffset{member->Type(), member->Offset()};
                }
            }
            return TypeAndOffset{};
        },
...
class View : public interop::WGSLView {
  public:
    ...
    interop::Interface<WGSLView> index(Napi::Env env, interop::WGSLViewIndex value) override {
        auto [type, offset] = std::visit(  //
            [&](auto&& idx) { return indexType(env, program_, type_, idx); }, value);
        if (type == nullptr) {
            return {};
        }
        return interop::WGSLView::Create<View>(env, program_, type, buffer_.Value(),
                                               offset_ + offset, missingValue_);
    }
    ...
    interop::Interface<interop::WGSLSizedType> getType(Napi::Env env) override {
        return createSizedType(env, program_, type_);
    }
...
interop::Interface<interop::WGSLType> createType(Napi::Env env,
                                                 const tint::Program* program,
                                                 const tint::type::Type* type) {
    return tint::Switch<interop::Interface<interop::WGSLType>>(
        type,  //
        [&](const tint::type::I32* ty) {
        ...
        [&](const tint::type::Sampler* ty) -> interop::Interface<interop::WGSLType> {
            if (ty->IsComparison()) {
...
interop::Interface<interop::WGSLType> WGSLBindPoint::getType(Napi::Env env) {
