...
It may be helpful to write a bash script like `use-clang.sh` that sets up your build environment, for example:

```sh
#!/bin/bash
PATH=$HOME/clang+llvm-15.0.6-x86_64-linux-gnu-ubuntu-18.04/bin:$PATH
CC=clang
CXX=clang++
```

Then you can setup your build for coverage as follows:
