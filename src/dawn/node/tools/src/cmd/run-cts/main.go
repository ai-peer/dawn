...
func defaultNodePath() string {
	if dir := thisDir(); dir != "" {
		if projectDir := getPathOfFileInParentDirs(dir, "DEPS"); projectDir != "" {
			node := filepath.Join(projectDir, "third_party/node")
...
func defaultCtsPath() string {
	if dir := thisDir(); dir != "" {
		if projectDir := getPathOfFileInParentDirs(dir, "DEPS"); projectDir != "" {
			cts := filepath.Join(projectDir, "third_party/webgpu-cts")
