...
ObjectBaseStore::ObjectBaseStore() {
    // ID 0 is nullptr
    mObjects.emplace_back(nullptr);
    mCurrentId = 1;
}

ObjectHandle ObjectBaseStore::ReserveHandle() {
...
void ObjectBaseStore::Insert(std::unique_ptr<ObjectBase> obj) {
...
void ObjectBaseStore::Free(ObjectBase* obj) {
    ASSERT(obj->IsInList());
...
ObjectBase* ObjectBaseStore::Get(ObjectId id) const {
    if (id >= mObjects.size()) {
        return nullptr;
    }
...
ObjectAllocator::ObjectAllocator(Client* client) : mClient(client) {}

void ObjectAllocator::Free(ObjectBase* obj, ObjectType type) {
    mPerTypeStores[type].Free(obj);
}

void ObjectAllocator::Track(std::unique_ptr<ObjectBase> obj, ObjectType type) {
    mClient->TrackObject(obj.get(), type);
    mPerTypeStores[type].Insert(std::move(obj));
}
