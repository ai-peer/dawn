...
class EventManager final : NonMovable {
  public:
    ...
    // TODO(crbug.com/dawn/1987): Clean up any leftover callbacks in the destructor, which currently
    // happens on client destructor (but should happen on instance destructor). See RequestTracker.
  ...
  private:
    struct TrackedEvent {  // FIXME: NonCopyable?
        TrackedEvent(WGPUCallbackModeFlags mode, std::function<void()>&& callback);
        // FIXME: Add a destructor which asserts mCallback is falsey?
