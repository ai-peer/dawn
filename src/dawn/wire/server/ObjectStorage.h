...
struct Known {
    ...
    const ObjectData<T>* operator->() const { return data; }
    ObjectData<T>* operator->() { return data; }

    ObjectHandle AsHandle() const { return {id, data->generation}; }
};
...
class KnownObjects<WGPUDevice> : public KnownObjectsBase<WGPUDevice> {
  public:
    ...
    bool Allocate(Known<WGPUDevice>* result,
                  ObjectHandle handle,
                  AllocationState state = AllocationState::Allocated) {
        bool success = KnownObjectsBase<WGPUDevice>::Allocate(result, handle, state);
  ...
  private:
    void AddToKnownSet(Known<WGPUDevice> device) {
        if (device.data != nullptr && device->state == AllocationState::Allocated &&
