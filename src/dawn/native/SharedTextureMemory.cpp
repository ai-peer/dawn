...
ResultOrError<Ref<TextureBase>> SharedTextureMemoryBase::CreateTexture(
    ...
    // Validate that there is a one 2D, single-sampled subresource
    ...
    // Validate that the texture size matches the shared texture memory's size, exactly.
    DAWN_INVALID_IF(
    ...
    // Validate that the texture format matches the shared texture memory's format, exactly.
    ...
    // Validate that the rest of the texture descriptor, and require its usage to be a subset of the
...
// Helper for use in AcquireBeginFences / PopAccessFences. Finds the latest AccessScope for
// `texture`.
