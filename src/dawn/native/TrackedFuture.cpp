...
PosixFd TrackedFuture::GetFd() const {
    ASSERT(mFd != PosixFd(-1));
    return mFd;
}
...
void TrackedFuture::Complete() {
    ASSERT(!mCompleted);
    CompleteImpl();
    mInstance->UntrackFuture(mFutureID);
    mCompleted = true;
}
...
ResultOrError<Ref<WorkDoneFuture>> WorkDoneFuture::Create(DeviceBase* device,
    ...
    DAWN_TRY_ASSIGN(future->mFd, device->CreateWorkDoneEventFd(future->mSerial));
    return future;
}
...
void WorkDoneFuture::CompleteImpl() {
    ...
    if (mDevice->IsLost()) {
        ...
        printf("%llu < %llu\n", uint64_t(mDevice->GetCompletedCommandSerial()), uint64_t(mSerial));
