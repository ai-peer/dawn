...
MaybeError InitializeDebugLayerFilters(ComPtr<ID3D11Device> d3d11Device) {
    ...
    static constexpr D3D11_MESSAGE_ID kDenyIds[] = {
    ...
    static constexpr D3D11_MESSAGE_SEVERITY kDenySeverities[] = {
        D3D11_MESSAGE_SEVERITY_INFO,
        D3D11_MESSAGE_SEVERITY_MESSAGE,
    };

    static constexpr D3D11_INFO_QUEUE_FILTER filter = {
        .AllowList = {},
        .DenyList =
            {
                .NumCategories = 0,
                .pCategoryList = nullptr,
                .NumSeverities = std::size(kDenySeverities),
                .pSeverityList = const_cast<D3D11_MESSAGE_SEVERITY*>(kDenySeverities),
                .NumIDs = std::size(kDenyIds),
                .pIDList = const_cast<D3D11_MESSAGE_ID*>(kDenyIds),
            },
    };

    return CheckHRESULT(infoQueue->PushStorageFilter(const_cast<D3D11_INFO_QUEUE_FILTER*>(&filter)),
