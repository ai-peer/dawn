...
ResultOrError<Ref<BindGroupLayout>> BindGroupLayout::Create(
    ...
    Ref<BindGroupLayout> bindGroupLayout =
        AcquireRef(new BindGroupLayout(device, descriptor, pipelineCompatibilityToken));
    DAWN_TRY(bindGroupLayout->Initialize(descriptor));
...
MaybeError BindGroupLayout::Initialize(const BindGroupLayoutDescriptor* descriptor) {
    return {};
}
