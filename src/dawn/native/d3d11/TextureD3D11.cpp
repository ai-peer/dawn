...
MaybeError ValidateTextureCanBeWrapped(ID3D11Resource* d3d12Resource,
                                       const TextureDescriptor* dawnDescriptor) {
    ComPtr<ID3D11Texture2D> d3d11Texture;
    DAWN_TRY(
        CheckHRESULT(d3d12Resource->QueryInterface(IID_PPV_ARGS(&d3d11Texture)), "QueryInterface"));
...
MaybeError Texture::InitializeAsExternalTexture(ComPtr<ID3D11Resource> d3d11Texture,
    ...
    for (Ref<Fence>& fence : waitFences) {
        DAWN_TRY(
            CheckHRESULT(d3d11DeviceContext4->Wait(fence->GetD3D11Fence(), fence->GetFenceValue()),
