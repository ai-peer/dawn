...
T Texture::GetD3D11TextureDesc() const {
    ...
    if constexpr (std::is_same<T, D3D11_TEXTURE1D_DESC>::value) {
        ...
        desc.Width = GetIntendedSize().width;
        desc.Height = GetIntendedSize().height;
        desc.Depth = GetIntendedSize().depthOrArrayLayers;
        desc.MiscFlags = 0;
    }
...
MaybeError Texture::CopyInternal(CommandRecordingContext* commandContext,
    ...
    bool isWholeSubresource =
        src.texture->CoverFullSubresource(src.mipLevel, src.aspect, copy->copySize) &&
        dst.texture->CoverFullSubresource(dst.mipLevel, dst.aspect, copy->copySize);
    // Partial update subresource of a depth/stencil texture is not allowed.
