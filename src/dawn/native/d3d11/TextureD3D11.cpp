...
MaybeError Texture::Clear(CommandRecordingContext* commandContext,
    ...
    if (!isRenderable) {
        ...
        if (GetFormat().isCompressed) {
            ...
            ASSERT(desc.dimension != wgpu::TextureDimension::e3D);
            // The interim texture size has to be that of the compressed texture in blocks.
            // Otherwise the validation layer would complain:
            //      ID3D11DeviceContext::CopySubresourceRegion: When offset by the destination
            //      coordinates and converted to block dimensions, pSrcBox does not fit on the
            //      destination subresource. OffsetSrcBox = { left:0, top:0, front:0, right:128,
            //      bottom:128, back:1 } (in blocks). DstSubresource = { left:0, top:0, front:0,
            //      right:32, bottom:32, back:1 } (in blocks). [ RESOURCE_MANIPULATION ERROR #280:
            ...
            DAWN_TRY(interimTexture->Clear(
                commandContext, SubresourceRange::MakeFull(Aspect::Color, 1, 1), clearValue));
            ...
            for (uint32_t layer = range.baseArrayLayer;
                ...
                for (uint32_t mipLevel = range.baseMipLevel;
                    ...
                    uint32_t dstSubresource =
                        GetSubresourceIndex(mipLevel, layer, D3D11Aspect(range.aspects));
                    auto physicalSize = GetMipLevelSingleSubresourcePhysicalSize(mipLevel);
                    ...
                    commandContext->GetD3D11DeviceContext1()->CopySubresourceRegion(
                        GetD3D11Resource(), dstSubresource, 0, 0, 0,
                        interimTexture->GetD3D11Resource(), srcSubresource, &srcBox);
                }
            }
        } else {
                ...
                if (GetFormat().HasDepthOrStencil()) {
                    ...
                    if (mIsInterim) {
                        const TexelBlockInfo& blockInfo =
                            GetFormat().GetAspectInfo(range.aspects).block;
                        // Ensure value 1 per byte rather than per component.
                        for (uint32_t i = 0;
                             i < blockInfo.byteSize / GetFormat().componentCount - 1; ++i) {
