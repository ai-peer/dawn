...
struct DepthStencilAspectLayout DepthStencilAspectLayout(DXGI_FORMAT format, Aspect aspect) {
...
MaybeError Texture::Clear(CommandRecordingContext* commandContext,
    ...
    if (isRenderable) {
        DAWN_TRY(ClearRenderable(commandContext, range, clearValue));
...
MaybeError Texture::ClearCompressed(CommandRecordingContext* commandContext,
    ...
    DAWN_TRY(interimTexture->ClearRenderable(
...
MaybeError Texture::WriteInternal(CommandRecordingContext* commandContext,
    ...
    if (d3d::IsDepthStencil(d3d::DXGITextureFormat(GetFormat().format))) {
        ...
        DAWN_TRY_ASSIGN(stagingTexture,
                        CreateInternal(ToBackend(GetDevice()), &desc, Kind::Staging));
        ...
        for (uint32_t layer = subresources.baseArrayLayer;
            ...
            DAWN_TRY(
                CheckHRESULT(d3d11DeviceContext1->Map(stagingTexture->GetD3D11Resource(), layer,
