...
MaybeError Texture::WriteDepthStencilInternal(CommandRecordingContext* commandContext,
    ...
    bool shouldCacheExistingDataFirst = true;
    if (GetFormat().format == wgpu::TextureFormat::Stencil8) {
        // Not needed if the texture only has a single aspect.
        shouldCacheExistingDataFirst = false;
    } else {
        SubresourceRange otherRange = subresources;
        otherRange.aspects = otherRange.aspects == Aspect::Depth ? Aspect::Stencil : Aspect::Depth;
        // We should do that if the other aspect has valid data.
        shouldCacheExistingDataFirst = IsSubresourceContentInitialized(otherRange);
    }

    if (shouldCacheExistingDataFirst) {
    ...
    for (uint32_t layer = subresources.baseArrayLayer;
         layer < subresources.baseArrayLayer + subresources.layerCount; ++layer) {
        ...
        stagingTexture->SetIsSubresourceContentInitialized(
            true, SubresourceRange::MakeSingle(subresources.aspects, layer, /*baseMipLevel*/ 0));
    }
