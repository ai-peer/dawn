            ...
            for (uint32_t layer = range.baseArrayLayer;
                ...
                for (uint32_t level = range.baseMipLevel;
                    ...
                    uint32_t dstSubresource =
                        GetSubresourceIndex(level, layer, D3D11Aspect(range.aspects));
            ...
            for (uint32_t mipLevel = range.baseMipLevel;
                ...
                if (GetFormat().HasDepthOrStencil()) {
                    ...
                    if (mIsInterim) {
                        ...
                        const uint32_t bytesPerComponent =
                            blockInfo.byteSize / GetFormat().componentCount;
                        // Ensure value 1 per byte rather than per component.
                        for (uint32_t i = 0; i < bytesPerComponent - 1; ++i) {
