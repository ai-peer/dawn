...
MaybeError Texture::Clear(CommandRecordingContext* commandContext,
    ...
    if (!isRenderable) {
        ...
        Extent3D writeSize = GetMipLevelSingleSubresourcePhysicalSize(range.baseMipLevel);
        const uint32_t bytesPerRow = blockInfo.byteSize * writeSize.width;
        ...
        for (uint32_t level = range.baseMipLevel; level < range.baseMipLevel + range.levelCount;
            ...
            writeSize = GetMipLevelSingleSubresourcePhysicalSize(level);
