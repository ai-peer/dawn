...
MaybeError Texture::Clear(CommandRecordingContext* commandContext,
    ...
    if (!isRenderable) {
        ...
        for (uint32_t level = range.baseMipLevel; level < range.baseMipLevel + range.levelCount;
             ++level) {
            wgpu::Extent3D writeSize = {std::max(GetSize().width >> level, 1u),
                                        std::max(GetSize().height >> level, 1u),
                                        GetSize().depthOrArrayLayers};

            if (GetDimension() == wgpu::TextureDimension::e3D) {
                writeSize.depthOrArrayLayers = std::max(GetSize().depthOrArrayLayers >> level, 1u);
            }
            const uint32_t bytesPerRow =
                Align(blockInfo.byteSize * writeSize.width, kTextureBytesPerRowAlignment);
            const uint32_t rowsPerImage = writeSize.height;
            const uint64_t byteLength =
                ComputeRequiredBytesInCopy(
                    blockInfo, {writeSize.width, writeSize.height, writeSize.depthOrArrayLayers},
                    bytesPerRow, rowsPerImage)
                    .AcquireSuccess();
            clearData.resize(byteLength, clearValue == ClearValue::Zero ? 0 : 1);
            writeRange.baseMipLevel = level;
            writeRange.levelCount = 1;
            DAWN_TRY(
                WriteInternal(commandContext, writeRange, {0, 0, 0},
                              {writeSize.width, writeSize.height, writeSize.depthOrArrayLayers},
