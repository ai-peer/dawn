...
ResultOrError<ID3D11DepthStencilView*> TextureView::GetD3D11DepthStencilView(
    bool depthReadOnly,
    bool stencilReadOnly) const {
    auto& view = mDepthStencilViews[depthReadOnly ? 1 : 0][stencilReadOnly ? 1 : 0];
    if (!view.has_value()) {
        D3D11_DEPTH_STENCIL_VIEW_DESC dsvDesc =
            ToBackend(GetTexture())
        ...
        DAWN_TRY(
            CheckHRESULT(ToBackend(GetDevice())
                             ->GetD3D11Device()
                             ->CreateDepthStencilView(ToBackend(GetTexture())->GetD3D11Resource(),
                                                      &dsvDesc, view->GetAddressOf()),
