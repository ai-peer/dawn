...
class ScopedRenderPassState : public NonCopyable {
  public:
    ScopedRenderPassState() = default;

    ~ScopedRenderPassState() {
        if (mCommandContext) {
            // Clear render targets after the render pass.
            ID3D11DeviceContext1* d3d11DeviceContext1 = mCommandContext->GetD3D11DeviceContext1();
            d3d11DeviceContext1->OMSetRenderTargets(0, nullptr, nullptr);
        }
    }

    MaybeError Begin(BeginRenderPassCmd* renderPass, CommandRecordingContext* commandContext) {
...
MaybeError CommandBuffer::ExecuteRenderPass(BeginRenderPassCmd* renderPass,
    ...
    ScopedRenderPassState ScopedRenderPassState;
    DAWN_TRY(ScopedRenderPassState.Begin(renderPass, commandContext));
