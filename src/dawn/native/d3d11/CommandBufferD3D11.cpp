    ...
    while (mCommands.NextCommandId(&type)) {
        switch (type) {
            ...
            case Command::ResolveQuerySet: {
                ...
                size_t size = queryCount * sizeof(uint64_t);

                DAWN_TRY(destination->EnsureDataInitializedAsDestination(commandContext,
                                                                         destinationOffset, size));
                std::vector<uint64_t> resolved(queryCount);
                DAWN_TRY(querySet->Resolve(commandContext->GetD3D11DeviceContext(), firstQuery,
                                           queryCount, resolved.data()));
                DAWN_TRY(
                    destination->Write(commandContext, destinationOffset, resolved.data(), size));
                destination->MarkUsedInPendingCommands();
                break;
            }
