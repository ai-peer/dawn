        ...
        switch (type) {
            ...
            case Command::CopyTextureToBuffer: {
                ...
                if (scopedDstMap.GetMappedData()) {
                    uint8_t* dstData = scopedDstMap.GetMappedData() + dst.offset;
                    callback = [dstData](const uint8_t* data, uint64_t offset,
                                         uint64_t size) -> MaybeError {
                        memcpy(dstData + offset, data, size);
                        return {};
                    };
                } else {
                    callback = [commandContext, buffer = ToBackend(dst.buffer),
                                dstOffset = dst.offset](const uint8_t* data, uint64_t offset,
                                                        uint64_t size) -> MaybeError {
                        DAWN_TRY(buffer->Write(commandContext, dstOffset + offset, data, size));
                        return {};
                    };
                }
