        ...
        for (BindingIndex bindingIndex{0}; bindingIndex < group->GetLayout()->GetBindingCount();
            ...
            switch (bindingInfo.bindingType) {
                ...
                case BindingInfoType::Texture: {
                    ...
                    commandContext->GetD3D11DeviceContext1()->PSSetShaderResources(bindingSlot, 1,
                                                                                   &srv);
                    break;
                }
    ...
    while (mCommands.NextCommandId(&type)) {
        switch (type) {
            ...
            case Command::ClearBuffer: {
                ...
                DAWN_TRY(buffer->ClearBuffer(commandContext, 0, cmd->offset, cmd->size));
                break;
            }
            ...
            case Command::WriteBuffer: {
                ...
                DAWN_TRY(dstBuffer->WriteBuffer(commandContext, cmd->offset, data, cmd->size));
...
MaybeError CommandBuffer::RecordFirstIndexOffset(RenderPipeline* renderPipeline,
    ...
    DAWN_TRY(commandContext->GetUniformBuffer()->WriteBuffer(commandContext, 0, offsets.data(),
...
MaybeError CommandBuffer::RecordNumWorkgroupsForDispatch(ComputePipeline* computePipeline,
    ...
    // TODO: only update the uniform buffer when the value changes.
    DAWN_TRY(commandContext->GetUniformBuffer()->WriteBuffer(commandContext, 0, dispatch.data(),
                                                             sizeof(uint32_t) * dispatch.size()));
