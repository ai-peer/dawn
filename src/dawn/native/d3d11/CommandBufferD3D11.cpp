    ...
    while (mCommands.NextCommandId(&type)) {
        switch (type) {
            ...
            case Command::CopyBufferToTexture: {
                ...
                const uint8_t* pSrcData = buffer->GetMappedData() + src.offset;

                d3d11DeviceContext1->UpdateSubresource(texture->GetD3D11Resource(), subresource,
                                                       &dstBox, pSrcData, src.bytesPerRow,
