...
class QuerySet final : public QuerySetBase {
  public:
    ...
    MaybeError Resolve(ID3D11DeviceContext* d3d11DeviceContext,
                       uint32_t firstQuery,
                       uint32_t queryCount,
                       uint8_t* destination);
  ...
  private:
    ...
    MaybeError CreateD3D11OcclusionQuery(ID3D11Query** query);

    // Dawn API
    void DestroyImpl() override;
    void SetLabelImpl() override;

    std::vector<ComPtr<ID3D11Query>> mQuerySet;
    enum class State { Begin, End, GetData };
    std::vector<State> mQueryState;
};
