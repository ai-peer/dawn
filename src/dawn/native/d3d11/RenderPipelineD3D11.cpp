...
MaybeError RenderPipeline::InitializeInputLayout(const D3D11_SHADER_BYTECODE& vertexShader) {
...
MaybeError RenderPipeline::InitializeShaders() {
    ...
    if (GetStageMask() & wgpu::ShaderStage::Vertex) {
        ...
        auto shaderBytecode = compiledShader[SingleShaderStage::Vertex].GetD3D11ShaderBytecode();
    ...
    if (GetStageMask() & wgpu::ShaderStage::Fragment) {
        ...
        DAWN_TRY(CheckHRESULT(
            device->GetD3D11Device()->CreatePixelShader(
                compiledShader[SingleShaderStage::Fragment]
                    .GetD3D11ShaderBytecode()
                    .pShaderBytecode,
                compiledShader[SingleShaderStage::Fragment].GetD3D11ShaderBytecode().BytecodeLength,
