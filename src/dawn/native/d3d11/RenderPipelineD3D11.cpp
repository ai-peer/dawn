...
MaybeError RenderPipeline::InitializeInputLayout(const D3D12_SHADER_BYTECODE& vertexShader) {
    ...
    if (GetAttributeLocationsUsed().any()) {
        ...
        Device* device = ToBackend(GetDevice());
        DAWN_TRY(CheckHRESULT(
            device->GetD3D11Device()->CreateInputLayout(inputElementDescriptors.data(), count,
                                                        vertexShader.pShaderBytecode,
                                                        vertexShader.BytecodeLength, &mInputLayout),
...
MaybeError RenderPipeline::InitializeShaders() {
    ...
    if (GetStageMask() & wgpu::ShaderStage::Vertex) {
        ...
        auto shaderBytecode = compiledShader[SingleShaderStage::Vertex].GetD3D12ShaderBytecode();
        DAWN_TRY(CheckHRESULT(device->GetD3D11Device()->CreateVertexShader(
                                  shaderBytecode.pShaderBytecode, shaderBytecode.BytecodeLength,
                                  nullptr, &mVertexShader),
                              "D3D11 create vertex shader"));
        DAWN_TRY(InitializeInputLayout(shaderBytecode));
    }
    ...
    if (GetStageMask() & wgpu::ShaderStage::Fragment) {
        ...
        DAWN_TRY(CheckHRESULT(
            device->GetD3D11Device()->CreatePixelShader(
                compiledShader[SingleShaderStage::Fragment]
                    .GetD3D12ShaderBytecode()
                    .pShaderBytecode,
                compiledShader[SingleShaderStage::Fragment].GetD3D12ShaderBytecode().BytecodeLength,
