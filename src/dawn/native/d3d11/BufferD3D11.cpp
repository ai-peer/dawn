...
void Buffer::UnmapInternal() {
    ...
    CommandRecordingContext* commandContext;
    if (!GetDevice()->ConsumedError(ToBackend(GetDevice())->GetPendingCommandContext(),
                                    &commandContext)) {
...
MaybeError Buffer::WriteInternal(CommandRecordingContext* commandContext,
    ...
    if (offset == 0 && size == GetAllocatedSize()) {
        d3d11DeviceContext1->UpdateSubresource1(GetD3D11Buffer(), /*DstSubresource=*/0,
                                                /*pDstBox=*/nullptr, data,
                                                /*SrcRowPitch=*/0,
                                                /*SrcDepthPitch*/ 0, /*copyFlags=*/0);
        return {};
    }
    ...
    d3d11DeviceContext1->UpdateSubresource1(GetD3D11Buffer(), /*DstSubresource=*/0, &dstBox, data,
                                            /*SrcRowPitch=*/0,
                                            /*SrcDepthPitch*/ 0, copyFlags);
...
ResultOrError<Buffer::ScopedMap> Buffer::ScopedMap::Create(Buffer* buffer) {
    ...
    if (buffer->mMappedData) {
        return ScopedMap(nullptr, buffer->mMappedData);
    }

    DAWN_TRY(buffer->MapInternal());
    return ScopedMap(buffer, buffer->mMappedData);
}

Buffer::ScopedMap::ScopedMap() = default;

Buffer::ScopedMap::ScopedMap(Buffer* buffer, uint8_t* mappedData)
    : mBuffer(buffer), mMappedData(mappedData) {}
