...
// - Can only invoke UpdateSubresource when the destination Resource was created with
//   D3D11_USAGE_DEFAULT.
// - Cannot invoke CopySubresourceRegion while a Subresource of the source is mapped.
...
Buffer::ScopedMap::ScopedMap(Buffer* buffer, const char* contextInfo) {
    if (IsMapUsage(buffer->GetUsage()) && !buffer->GetMappedData()) {
