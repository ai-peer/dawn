...
MaybeError Buffer::MapInternal() {
    ...
    DAWN_TRY(CheckHRESULT(commandContext->GetD3D11DeviceContext()->Map(
                              mD3d11NonConstantBuffer.Get(), /*Subresource=*/0, D3D11_MAP_READ_WRITE,
...
void Buffer::UnmapInternal() {
    ...
    CommandRecordingContext* commandContext = ToBackend(GetDevice())->GetPendingCommandContext();
    commandContext->GetD3D11DeviceContext()->Unmap(mD3d11NonConstantBuffer.Get(), /*Subresource=*/0);
    mMappedData = nullptr;
}
...
void Buffer::SetLabelImpl() {
    ...
    SetDebugName(ToBackend(GetDevice()), mD3d11ConstantBuffer.Get(), "Dawn_Buffer", GetLabel());
}
...
ResultOrError<ComPtr<ID3D11UnorderedAccessView1>> Buffer::CreateD3D11UnorderedAccessView1(
    ...
    // UAV is created, we assume the mD3d11NonConstantBuffer content will be modified, mark
    //  mConstantBufferIsUpdated is false, so the content buffer will be updated when it is about to
    //  be used.
...
MaybeError Buffer::WriteInternal(CommandRecordingContext* commandContext,
    ...
    if (mD3d11NonConstantBuffer) {
        ...
        box.front = 0;
        box.back = 1;
        d3d11DeviceContext1->UpdateSubresource(mD3d11NonConstantBuffer.Get(), /*DstSubresource=*/0, &box, data,
                                               /*SrcRowPitch=*/0,
                                               /*SrcDepthPitch*/ 0);
    ...
    // If the mD3d11buffer is null,  we have to create a staging buffer for transfer the data to
    // mD3d11ConstantBuffer, since UpdateSubresource() has many restrictions. For example, the
    // size of the data has to be a multiple of 16, etc
...
MaybeError Buffer::CopyInternal(CommandRecordingContext* commandContext,
    ...
    ID3D11Buffer* d3d11SourceBuffer =
        source->mD3d11NonConstantBuffer ? source->mD3d11NonConstantBuffer.Get() : source->mD3d11ConstantBuffer.Get();
    ASSERT(d3d11SourceBuffer);

    if (destination->mD3d11NonConstantBuffer) {
        commandContext->GetD3D11DeviceContext()->CopySubresourceRegion(
            destination->mD3d11NonConstantBuffer.Get(), /*DstSubresource=*/0, /*DstX=*/destinationOffset,
            /*DstY=*/0,
