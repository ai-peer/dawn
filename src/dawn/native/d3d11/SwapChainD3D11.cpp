...
OldSwapChain::OldSwapChain(Device* device, const SwapChainDescriptor* descriptor)
    ...
    DawnWSIContextD3D11 wsiContext = {};
...
TextureBase* OldSwapChain::GetNextTextureImpl(const TextureDescriptor* descriptor) {
    ...
    ComPtr<ID3D11Resource> d3d11Texture = static_cast<ID3D11Resource*>(next.texture.ptr);
    Ref<Texture> dawnTexture;
    if (device->ConsumedError(
            Texture::Create(ToBackend(GetDevice()), descriptor, std::move(d3d11Texture)),
            &dawnTexture)) {
        return nullptr;
    }
...
MaybeError SwapChain::Initialize(NewSwapChainBase* previousSwapChain) {
    ...
    mConfig.format = D3D11TextureFormat(GetFormat());
