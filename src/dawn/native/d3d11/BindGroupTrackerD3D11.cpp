...
bool BindGroupTracker::IsRenderPass() const {
    ...
    return mPipeline->HasStage(SingleShaderStage::Fragment);
}

bool BindGroupTracker::IsComputePass() const {
    DAWN_ASSERT(mPipeline != nullptr);
    return mPipeline->HasStage(SingleShaderStage::Compute);
}
...
MaybeError BindGroupTracker::ApplyBindGroup(BindGroupIndex index) {
    ...
    // time.
    std::map<uint32_t, ComPtr<ID3D11UnorderedAccessView>> sortedUAVs;
            ...
            case BindingInfoType::Buffer: {
                ...
                switch (bindingInfo.buffer.type) {
                    ...
                    case kInternalStorageBufferBinding: {
                        ...
                        if (bindingInfo.visibility & wgpu::ShaderStage::Fragment &&
                            IsRenderPass()) {
                            sortedUAVs[bindingSlot] = d3d11UAV;
                        }
                        if (bindingInfo.visibility & wgpu::ShaderStage::Compute &&
                            IsComputePass()) {
