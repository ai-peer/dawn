...
BindGroupTracker::BindGroupTracker(CommandRecordingContext* commandContext, bool isRenderPass)
    : mCommandContext(commandContext),
      ...
      mVisiableStages(isRenderPass ? wgpu::ShaderStage::Vertex | wgpu::ShaderStage::Fragment
...
MaybeError BindGroupTracker::Apply() {
    ...
    // Force to re-apply all UAV bind groups so that all UAV slots can be always bound at the same
    // time.
    // TODO(dawn:1816): Avoid re-binding CBVs, and SRVs in these bind groups.
    if (mIsRenderPass) {
        const auto uavBindGroups = ToBackend(mPipelineLayout)->GetUAVBindGroupLayoutsMask();
...
MaybeError BindGroupTracker::ApplyBindGroup(BindGroupIndex index) {
    ...
    for (BindingIndex bindingIndex{0}; bindingIndex < group->GetLayout()->GetBindingCount();
        ...
        const auto bindingVisibility = bindingInfo.visibility & mVisiableStages;
