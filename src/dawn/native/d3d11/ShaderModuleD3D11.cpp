...
ResultOrError<d3d::CompiledShader> ShaderModule::Compile(
    ...
    uint32_t maxOpenBindingNumber = kMaxBindingsPerBindGroup;
    ...
    for (BindGroupIndex group : IterateBitSet(layout->GetBindGroupLayoutsMask())) {
        ...
        for (const auto& [_, expansion] : groupLayout->GetExternalTextureBindingExpansionMap()) {
            maxOpenBindingNumber =
                std::max(maxOpenBindingNumber, static_cast<uint32_t>(expansion.plane1) + 1u);
            maxOpenBindingNumber =
                std::max(maxOpenBindingNumber, static_cast<uint32_t>(expansion.params) + 1u);
    ...
    // Use 'maxOpenBindingNumber' to avoid conflicting with any existing bindings.
    req.hlsl.firstIndexOffsetShaderRegister = maxOpenBindingNumber;
    // Remap from the interim 'maxOpenBindingNumber' to the actual
    // 'kFirstIndexOffsetConstantBufferSlot'.
    {
        tint::writer::BindingPoint srcBindingPoint{0u, maxOpenBindingNumber};
