...
ResultOrError<CompiledShader> ShaderModule::Compile(
    ...
    for (BindGroupIndex group : IterateBitSet(layout->GetBindGroupLayoutsMask())) {
        ...
        // d3d11::BindGroupLayout packs the bindings per HLSL register-space. We modify
        // the Tint AST to make the "bindings" decoration match the offset chosen by
        // d3d11::BindGroupLayout so that Tint produces HLSL with the correct registers
        // assigned to each interface variable.
        for (const auto& [binding, bindingInfo] : groupBindingInfo) {
            BindingIndex bindingIndex = bgl->GetBindingIndex(binding);
            BindingPoint srcBindingPoint{static_cast<uint32_t>(group),
                                         static_cast<uint32_t>(binding)};
            BindingPoint dstBindingPoint{static_cast<uint32_t>(group), 0};
            // TODO(penghuang):
    ...
    if (device->IsToggleEnabled(Toggle::DumpShaders)) {
        ...
        if (device->IsToggleEnabled(Toggle::UseDXC)) {
            ...
            D3D11_SHADER_BYTECODE code = compiledShader->GetD3D11ShaderBytecode();
...
D3D11_SHADER_BYTECODE CompiledShader::GetD3D11ShaderBytecode() const {
