...
void CreateComputePipelineAsyncTask::Run() {
    ...
    {
        SCOPED_DAWN_HISTOGRAM_TIMER_MICROS(device->GetPlatform(),
                                           "GPU.Dawn.CreateComputePipelineUS");
        maybeError = mComputePipeline->Initialize();
    }
    DAWN_HISTOGRAM_BOOLEAN(device->GetPlatform(), "GPU.Dawn.CreateComputePipelineSuccess",
...
void CreateRenderPipelineAsyncTask::Run() {
    ...
    {
        SCOPED_DAWN_HISTOGRAM_TIMER_MICROS(device->GetPlatform(),
                                           "GPU.Dawn.CreateRenderPipelineUS");
        maybeError = mRenderPipeline->Initialize();
    }
    DAWN_HISTOGRAM_BOOLEAN(device->GetPlatform(), "GPU.Dawn.CreateRenderPipelineSuccess",
                           maybeError.IsSuccess());
