...
namespace dawn::native {
    ...
    namespace {
        ...
        void GetExpandedBglEntries(const BindGroupLayoutDescriptor* descriptor,
                                   BindingCounts& bindingCounts,
                                   std::map<BindingNumber, ExternalTextureBindingExpansion>&
                                       externalTextureBindingExpansions,
            ...
            for (uint32_t i = 0; i < descriptor->entryCount; i++) {
                ...
                if (externalTextureBindingLayout != nullptr) {
                    for (SingleShaderStage stage : IterateStages(entry.visibility)) {
                        ...
                        // increment the binding counts for an additional sampled texture and a
                        ...
                        bindingCounts.perStage[stage].sampledTextureCount += 2;
            ...
            // for the first sampled texture. For every external texture we must find two open
            // binding locations to assign the second sampled texture and uniform buffer.
            for (BindGroupLayoutEntry entry : externalTextureBindingEntries) {
                ...
                externalTextureBindingExpansions[BindingNumber(entry.binding)] = bindingExpansion;
            }
    ...
    BindGroupLayoutBase::BindGroupLayoutBase(DeviceBase* device,
        ...
        GetExpandedBglEntries(descriptor, mBindingCounts, mExternalTextureBindingExpansionMap,
                              sortedBindings);
    ...
    ExternalTextureBindingExpansionMap BindGroupLayoutBase::GetExternalTextureBindingExpansionMap()
        const {
