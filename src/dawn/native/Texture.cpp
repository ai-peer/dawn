...
const Extent3D& TextureBase::GetIntendedSize() const {
    DAWN_ASSERT(!IsError());
...
uint32_t TextureBase::GetArrayLayers() const {
    ...
    return mIntendedSize.depthOrArrayLayers;
}
...
bool TextureBase::CoverFullSubresource(uint32_t mipLevel,
                                       Aspect aspect,
...
Extent3D TextureBase::GetMipLevelSingleSubresourceVirtualSize(uint32_t level, Aspect aspect) const {
    ...
    extent.depthOrArrayLayers = std::max(mIntendedSize.depthOrArrayLayers >> level, 1u);
    return extent;
}
...
Extent3D TextureBase::GetMipLevelSubresourceVirtualSize(uint32_t level, Aspect aspect) const {
    Extent3D extent = GetMipLevelSingleSubresourceVirtualSize(level, aspect);
    if (mDimension == wgpu::TextureDimension::e2D) {
        extent.depthOrArrayLayers = mIntendedSize.depthOrArrayLayers;
    }
    return extent;
}
...
uint32_t TextureBase::APIGetWidth() const {
    return mIntendedSize.width;
}

uint32_t TextureBase::APIGetHeight() const {
    return mIntendedSize.height;
}
...
Extent3D TextureViewBase::GetSize() const {
    DAWN_ASSERT(!IsError());
    return GetTexture()->GetSize(GetAspects());
}
uint32_t TextureViewBase::GetWidth() const {
    DAWN_ASSERT(!IsError());
    return GetTexture()->GetWidth(GetAspects());
}
uint32_t TextureViewBase::GetHeight() const {
    DAWN_ASSERT(!IsError());
    return GetTexture()->GetHeight(GetAspects());
}
uint32_t TextureViewBase::GetDepth() const {
    DAWN_ASSERT(!IsError());
    return GetTexture()->GetDepth(GetAspects());
}
