...
bool NeedsInternalRenderAttachmentUsageAsCopyDst(const DeviceBase* device, const Format& format) {
    // Depth
    if (format.HasDepth() &&
...
bool NeedsInternalTextureBindingUsageAsCopySrc(const DeviceBase* device, const Format& format) {
    // Snorm
...
TextureBase::TextureBase(DeviceBase* device,
    ...
    if (mInternalUsage & wgpu::TextureUsage::CopyDst) {
        if (NeedsInternalRenderAttachmentUsageAsCopyDst(device, mFormat)) {
            AddInternalUsage(wgpu::TextureUsage::RenderAttachment);
        }
    }
    if (mInternalUsage & wgpu::TextureUsage::CopySrc) {
        if (NeedsInternalTextureBindingUsageAsCopySrc(device, mFormat)) {
