...
namespace dawn::native {
    namespace {
        ...
        MaybeError ValidateCanViewTextureAs(const DeviceBase* device,
            ...
            if (aspect != wgpu::TextureAspect::All) {
                if (format.GetAspectInfo(aspect).format == viewFormat.format) {
                    return {};
                }
            }
            ...
            if (std::find(compatibleViewFormats.begin(), compatibleViewFormats.end(),
                          viewFormat.format) == compatibleViewFormats.end()) {
    ...
    TextureBase::TextureBase(DeviceBase* device,
        ...
        if (descriptor->viewFormatCount > 0) {
            // Make a set to remove duplicates, and remove self format.
            std::set<wgpu::TextureFormat> viewFormats(
                descriptor->viewFormats, descriptor->viewFormats + descriptor->viewFormatCount);
            viewFormats.erase(descriptor->format);
            mViewFormats = {viewFormats.begin(), viewFormats.end()};
        }
