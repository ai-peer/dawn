...
MaybeError ValidateTextureUsage(const DeviceBase* device,
    ...
    if (!allowedSharedTextureMemoryUsage) {
        ...
        // Only allows simple readonly texture usages and RenderAttachment.
        wgpu::TextureUsage validMultiPlanarUsages = wgpu::TextureUsage::TextureBinding |
                                                    wgpu::TextureUsage::CopySrc |
...
//     // TODO(buglink): Possibly add support for multiplanar formats with plane configs different
//     than
//     // Y_UV and subsampling other than 4:2:0 in which case the size changes for different
//     aspects. if (aspect & Aspect::Plane1) {
//         size.width /= 2;
//         size.height /= 2;
//     }
//     return size;
