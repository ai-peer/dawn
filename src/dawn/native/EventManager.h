...
// wait (in case it `Spontaneous`ly completes during the wait). In some cases it might also end up
// being useful to hold a ref somewhere else (like on the thread doing async pipeline creation, so
// we can signal it directly instead of through an OS event, as an optimization).
