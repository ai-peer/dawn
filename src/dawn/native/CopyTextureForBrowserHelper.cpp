...
MaybeError ValidateCopyExternalTextureForBrowser(DeviceBase* device,
    ...
    const Extent3D& sourceVisibleRect = source->externalTexture->GetVisibleRect();
    ...
    DAWN_INVALID_IF(
        static_cast<uint64_t>(source->origin.x) + static_cast<uint64_t>(copySize->width) >
            ...
            static_cast<uint64_t>(source->origin.z) > sourceVisibleRect.depthOrArrayLayers,
...
MaybeError DoCopyExternalTextureForBrowser(DeviceBase* device,
    ...
    info.size = source->externalTexture->GetVisibleRect();
