...
#define FOOTER \
    R"(
            ...
            fn fs_main(
                ...
                // Support both copy from external texture and 2d texture, controlled by step mask.
                var color = sampleSourceTexture(mySourceTexture, mySampler, texcoord);
...
static const char sCopyTextureForBrowserShader[] = HEADER R"(
            @binding(1) @group(0) var mySampler: sampler;
            @binding(2) @group(0) var mySourceTexture: texture_2d<f32>;

            fn sampleSourceTexture(sourceTexture: texture_2d<f32>,
                                   mySampler: sampler,
                                   texcoord: vec2<f32>) -> vec4<f32> {
                return textureSample(sourceTexture, mySampler, texcoord);
            }

        )" FOOTER;

static const char sCopyExternalTextureForBrowserShader[] = HEADER R"(
            @binding(1) @group(0) var mySampler: sampler;
            @binding(2) @group(0) var mySourceTexture: texture_external;

            fn sampleSourceTexture(sourceTexture: texture_external,
                                   mySampler: sampler,
                                   texcoord: vec2<f32>) -> vec4<f32> {
                return textureSampleBaseClampToEdge(sourceTexture, mySampler, texcoord);
            }
        )" FOOTER;
...
ResultOrError<RenderPipelineBase*> GetOrCreateCopyExternalTextureForBrowserPipeline(
    ...
    if (GetCachedCopyExternalTexturePipeline(store, dstFormat) == nullptr) {
        if (store->copyExternalTextureForBrowser == nullptr) {
            DAWN_TRY_ASSIGN(
                store->copyExternalTextureForBrowser,
                utils::CreateShaderModule(device, sCopyExternalTextureForBrowserShader));
        }

        ShaderModuleBase* shaderModule = store->copyExternalTextureForBrowser.Get();
        Ref<RenderPipelineBase> pipeline;
        DAWN_TRY_ASSIGN(pipeline, CreateCopyForBrowserPipeline(device, dstFormat, shaderModule));
...
MaybeError ValidateImageCopyExternalTexture(
    DeviceBase* device,
    const ImageCopyExternalTexture& imageCopyExternalTexture,
    const Extent3D& copySize) {
    Extent3D visibleSize = {imageCopyExternalTexture.visibleWidth,
                            imageCopyExternalTexture.visibleHeight, 1u};

    // All texture dimensions are in uint32_t so by doing checks in uint64_t we avoid
    // overflows.
    DAWN_INVALID_IF(static_cast<uint64_t>(imageCopyExternalTexture.origin.x) +
                                static_cast<uint64_t>(copySize.width) >
                            static_cast<uint64_t>(visibleSize.width) ||
                        static_cast<uint64_t>(imageCopyExternalTexture.origin.y) +
                                static_cast<uint64_t>(copySize.height) >
                            static_cast<uint64_t>(visibleSize.height) ||
                        static_cast<uint64_t>(imageCopyExternalTexture.origin.z) >
                            visibleSize.depthOrArrayLayers,
                    "Texture copy range (origin: %s, copySize: %s) touches outside of %s "
                    "visible size (%s).",
                    &imageCopyExternalTexture.origin, &copySize,
                    imageCopyExternalTexture.externalTexture, &visibleSize);

    return {};
}

MaybeError ValidateExternalTextureToTextureCopyDestinationRestrictions(
    const ImageCopyTexture& destination) {
    // ExternalTexture always have sample count 1.
    const uint32_t srcSamples = 1;
    const uint32_t dstSamples = destination.texture->GetSampleCount();

    DAWN_INVALID_IF(srcSamples != dstSamples,
                    "Source external texture sample count is 1 and destination %s sample count "
                    "(%u) does not match.",
                    destination.texture, dstSamples);

    // Metal cannot select a single aspect for texture-to-texture copies.
    const Format& format = destination.texture->GetFormat();

    DAWN_INVALID_IF(
        SelectFormatAspects(format, destination.aspect) != format.aspects,
        "Destination %s aspect (%s) doesn't select all the aspects of the destination format "
        "(%s).",
        destination.texture, destination.aspect, format.format);
...
MaybeError ValidateCopyExternalTextureForBrowser(DeviceBase* device,
    ...
    DAWN_TRY_CONTEXT(ValidateImageCopyExternalTexture(device, *source, *copySize),
                     "validating that the copy fits in the source");

    DAWN_TRY(ValidateExternalTextureToTextureCopyDestinationRestrictions(*destination));
...
MaybeError DoCopyExternalTextureForBrowser(DeviceBase* device,
    ...
    info.size = {source->visibleWidth, source->visibleHeight};
