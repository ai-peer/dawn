...
void BufferBase::UnmapInternal(BufferState targetState, WGPUBufferMapAsyncStatus callbackStatus) {
    if (targetState == BufferState::PendingMap) {
        UnmapImpl();
        CallMapCallback(mLastMapID, callbackStatus);
    } else if (targetState == BufferState::Mapped) {
        UnmapImpl();
    } else if (targetState == BufferState::MappedAtCreation) {
        if (mStagingBuffer != nullptr) {
