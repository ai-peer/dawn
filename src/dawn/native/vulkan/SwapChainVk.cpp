...
MaybeError SwapChain::PresentImpl() {
    ...
    if (mConfig.needsBlit) {
        ...
        region.srcOffsets[1] = {static_cast<int32_t>(mBlitTexture->GetIntendedSize().width),
                                static_cast<int32_t>(mBlitTexture->GetIntendedSize().height), 1};

        region.dstSubresource = region.srcSubresource;
        region.dstOffsets[0] = {0, 0, 0};
        region.dstOffsets[1] = {static_cast<int32_t>(mTexture->GetIntendedSize().width),
                                static_cast<int32_t>(mTexture->GetIntendedSize().height), 1};
