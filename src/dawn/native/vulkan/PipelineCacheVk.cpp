...
namespace dawn::native::vulkan {
    ...
    Ref<PipelineCache> PipelineCache::Create(DeviceBase* device, const PipelineBase* pipeline) {
        ...
        if (pipeline != nullptr) {
            cache = AcquireRef(new PipelineCache(device, pipeline->GetCacheKey()));
        } else {
            // Monolithic-like caches are not implemented yet.
            UNREACHABLE();
        }
        cache->Initialize();
        return cache;
    ...
    bool PipelineCache::CacheHit() const {
        return mCacheHit;
    }
