...
ResultOrError<Ref<Queue>> Queue::Create(Device* device,
                                        ...
                                        uint32_t family,
                                        VkQueue vkQueue) {
    Ref<Queue> queue = AcquireRef(new Queue(device, descriptor, family, vkQueue));
    DAWN_TRY(queue->Initialize());
    return queue;
}

Queue::Queue(Device* device, const QueueDescriptor* descriptor, uint32_t family, VkQueue vkQueue)
    : QueueBase(device, descriptor), mQueueFamily(family), mQueue(vkQueue) {}
...
void Queue::RecycleCompletedCommands() {
...
MaybeError Queue::SubmitPendingCommands() {
    ...
    ScopedSignalSemaphore externalTextureSemaphore(device, VK_NULL_HANDLE);
        ...
        for (auto* texture : mRecordingContext.externalTexturesForEagerTransition) {
            ExternalSemaphoreHandle handle =
                (first ? semaphoreHandle
                       : device->GetExternalSemaphoreService()->DuplicateHandle(semaphoreHandle));
            first = false;
