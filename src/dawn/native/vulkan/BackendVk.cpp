...
OnDebugUtilsCallback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
    ...
    if (ShouldReportDebugMessage(pCallbackData->pMessageIdName, pCallbackData->pMessage)) {
        ...
        if (pUserData != nullptr) {
            // Look through all the object labels attached to the debug message and try to parse a
            // device debug prefix out of one of them. If a debug prefix is found and matches a
            // registered device, forward the message on to it.
            for (uint32_t i = 0; i < pCallbackData->objectCount; ++i) {
                ...
                Device* device = instance->GetDeviceForDebugPrefix(std::move(deviceDebugPrefix));
                if (device != nullptr) {
                    device->OnDebugMessage(pCallbackData->pMessage);
                    break;
                }
            }
        }
...
Device* VulkanInstance::GetDeviceForDebugPrefix(std::string deviceDebugPrefix) {
