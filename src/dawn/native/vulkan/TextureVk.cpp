...
Texture::Texture(Device* device, const TextureDescriptor* descriptor, TextureState state)
    : TextureBase(device, descriptor, state),
      mCombinedAspect(ComputeCombinedAspectIfNeeded()),
...
MaybeError Texture::InitializeFromExternal(const ExternalImageDescriptorVk* descriptor,
    ...
    // TODO(dawn:XXX): Support multi-planar images with the DISJOINT feature and potentially allow
    // acting on planes individually? Always using Color is valid even for disjoint images.
    DAWN_UNUSED(supportsDisjoint);
    ASSERT(mCombinedAspect == Aspect::Color);
...
Aspect Texture::ComputeCombinedAspectIfNeeded() const {
    ...
    // internally.
    if (GetFormat().aspects == (Aspect::Depth | Aspect::Stencil)) {
    ...
    if (GetFormat().aspects == (Aspect::Plane0 | Aspect::Plane1)) {
        return Aspect::Color;
    }
...
bool Texture::AspectRequiresCombining() const {
