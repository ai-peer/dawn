...
VkImageLayout VulkanImageLayout(const Texture* texture, wgpu::TextureUsage usage) {
    ...
    switch (usage) {
        case wgpu::TextureUsage::CopyDst:
            ...
            // The bindgroup's layout must match the runtime layout of the image when it is used via
            // the bindgroup, but we don't know exactly what it will be yet. So we have to prepare
