...
MaybeError Texture::ExportToIOSurface(VkImageLayout desiredLayout,
    ...
    ASSERT(GetNumMipLevels() == 1 && GetArrayLayers() == 1);
    wgpu::TextureUsage currentUsage = mSubresourceLastUsages.Get(GetDisjointVulkanAspects(), 0, 0);

    VkImageLayout currentLayout = VulkanImageLayout(this, currentUsage);

    // Write out the layouts. Note this is not important for IOSurface external image.
    *releasedOldLayout = currentLayout;
    *releasedNewLayout = currentLayout;
