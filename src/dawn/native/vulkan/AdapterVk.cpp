...
#define CHECK_AND_SET_V1_MIN_LIMIT(vulkanName, webgpuName) \
    ...
    if (vkLimits.maxFragmentCombinedOutputResources < minFragmentCombinedOutputResources) {
        return DAWN_INTERNAL_ERROR(
            "Insufficient Vulkan limits for the combination of maxStorageBuffersPerShaderStage, "
            "maxStorageTexturesPerShaderStage, and maxColorAttachments");
    }
    ...
    if (uint64_t(vkLimits.maxFragmentCombinedOutputResources) <
