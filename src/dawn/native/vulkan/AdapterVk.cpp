...
gpu_info::DriverVersion DecodeVulkanDriverVersion(uint32_t vendorID, uint32_t versionRaw) {
    ...
    switch (vendorID) {
        ...
        case gpu_info::kVendorID_Intel:
            ...
            driverVersion.resize(2);
            driverVersion[0] = versionRaw >> 14;
            driverVersion[1] = versionRaw & 0x3FFF;
            break;
#endif
        default:
            // Use Vulkan driver conversions for other vendors
            driverVersion.resize(3);
...
#if DAWN_PLATFORM_IS(WINDOWS)
    // Disabling Vulkan adapter on Intel Windows driver < 30.0.101.2111 due to flaky
    // issues.
    const gpu_info::DriverVersion kFixedDriverVersion({30, 0, 101, 2111});
    if (gpu_info::IsIntel(GetVendorId()) &&
        gpu_info::CompareWindowsDriverVersion(GetVendorId(), mDriverVersion, kFixedDriverVersion) ==
            -1) {
        return DAWN_INTERNAL_ERROR(
            "Disabling Windows Intel Vulkan adapter. See https://crbug.com/1338622.");
    }
#endif
