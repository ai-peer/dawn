...
void RecordWriteTimestampCmd(CommandRecordingContext* recordingContext,
    ...
    // The queries must be reset between uses, and the reset command must be called outside
    // render pass.
    if (!isRenderPass) {
        ...
        switch (type) {
            ...
            case Command::BeginComputePass: {
                ...
                if (it != cmd->timestampWrites.end()) {
                    RecordWriteTimestampCmd(recordingContext, device, it->second.querySet.Get(),
                                            it->second.queryIndex, false);
                }
...
MaybeError CommandBuffer::RecordRenderPass(CommandRecordingContext* recordingContext,
    ...
    // Write timestamp at the beginning of render pass
    auto it = renderPassCmd->timestampWrites.find(wgpu::RenderPassTimestampLocation::Beginning);
    if (it != renderPassCmd->timestampWrites.end()) {
        RecordWriteTimestampCmd(recordingContext, device, it->second.querySet.Get(),
                                it->second.queryIndex, false);
    }
