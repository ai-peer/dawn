...
MaybeError TransitionAndClearForSyncScope(Device* device,
    ...
    const VkPipelineStageFlags vertexStages =
    ...
    for (size_t i = 0; i < scope.textures.size(); ++i) {
        ...
        DAWN_TRY(scope.textureSyncInfos[i].Iterate(
            [&](const SubresourceRange& range, TextureSyncInfo syncInfo) -> MaybeError {
    ...
    auto issueBarriers = [device, recordingContext](const Barriers& barriers) {
    ...
    issueBarriers(vertexBarriers);
    issueBarriers(nonVertexBarriers);
