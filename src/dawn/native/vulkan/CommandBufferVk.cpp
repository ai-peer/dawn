...
MaybeError CommandBuffer::RecordComputePass(CommandRecordingContext* recordingContext,
    ...
    // TODO(dawn:1564): Clearing a depth/stencil buffer in a render pass and then sampling it in a
    ...
    if (ToBackend(device->GetAdapter())->IsAndroidQualcomm() &&
        ...
        for (auto texture : resourceUsages.referencedTextures) {
            if (texture->GetFormat().HasDepthOrStencil() &&
                ...
                ASSERT(recordingContext == device->GetPendingRecordingContext());
                DAWN_TRY(device->SplitPendingRecordingContext());
...
MaybeError CommandBuffer::RecordRenderPass(CommandRecordingContext* recordingContext,
    ...
    if (ToBackend(device->GetAdapter())->IsAndroidQualcomm() &&
        ...
        // TODO(dawn:1564): Need to track depth/stencil textures used as render pass attachments to
