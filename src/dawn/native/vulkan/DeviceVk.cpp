...
ResultOrError<Ref<SharedTextureMemoryBase>> Device::ImportSharedTextureMemoryImpl(
    ...
    DAWN_INVALID_IF(baseDescriptor->nextInChain == nullptr,
                    "SharedTextureMemoryDmaBufDescriptor must be chained.");
    {
...
ResultOrError<Ref<SharedFenceBase>> Device::ImportSharedFenceImpl(
    ...
    DAWN_INVALID_IF(
        baseDescriptor->nextInChain == nullptr,
        "SharedFenceVkSemaphoreZirconHandleDescriptor, SharedFenceVkSemaphoreSyncFDDescriptor, or "
        "SharedFenceVkSemaphoreOpaqueFDDescriptor must be chained.");
    {
...
MaybeError Device::SubmitPendingCommands() {
    ...
    for (auto* texture : mRecordingContext.externalTexturesForEagerTransition) {
        ...
        Ref<SharedTextureMemory> memory = ToBackend(texture->TryGetSharedTextureMemory());
        if (memory != nullptr) {
