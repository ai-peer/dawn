...
MaybeError Device::Initialize(const DeviceDescriptor* descriptor) {
    ...
    *functions = ToBackend(GetPhysicalDevice())->GetVulkanInstance()->GetFunctions();
    ...
    VkQueue vkQueue;
    fn.GetDeviceQueue(mVkDevice, mMainQueueFamily, 0, &vkQueue);
    DAWN_TRY_ASSIGN(mQueue,
                    Queue::Create(this, &descriptor->defaultQueue, mMainQueueFamily, vkQueue));
...
MaybeError Device::TickImpl() {
    mQueue->RecycleCompletedCommands();

    ExecutionSerial completedSerial = mQueue->GetCompletedCommandSerial();
    ...
    DAWN_TRY(mQueue->SubmitPendingCommands());
...
CommandRecordingContext* Device::GetPendingRecordingContext(Device::SubmitMode submitMode) {
    return mQueue->GetPendingRecordingContext(submitMode);
}

MaybeError Device::SubmitPendingCommands() {
    return mQueue->SubmitPendingCommands();
}
