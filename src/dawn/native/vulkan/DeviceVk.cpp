...
bool Device::CreateAndSubmitSignalSemaphoreForExport(uint64_t executionSerial,
    ...
    return !ConsumedError([&]() -> MaybeError {
        ...
        DAWN_TRY(SubmitPendingCommands());
        // Get the fence of the submit.
        fence = mFencesInFlight.back().first;
        DAWN_TRY_ASSIGN(*semaphoreHandle, mExternalSemaphoreService->ExportSemaphore(semaphore));
        // Add to the in-flight external semaphores.
        mExternalSemaphoresInFlight.emplace_back(
            std::make_tuple(GetLastSubmittedCommandSerial(), *semaphoreHandle, fence));
        return {};
    }());
}
...
bool Device::IsExternalSemaphoreSignaled(uint64_t executionSerial) {
    ...
    // Peek the fences of the in-flight external semaphores.
    for (const auto& tuple : mExternalSemaphoresInFlight) {
        if (executionSerial == static_cast<uint64_t>(std::get<0>(tuple))) {
            VkFence fence = std::get<2>(tuple);
            VkResult result = VkResult::WrapUnsafe(
                INJECT_ERROR_OR_RUN(fn.GetFenceStatus(mVkDevice, fence), VK_ERROR_DEVICE_LOST));
