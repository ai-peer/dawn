...
class ScopedSignalSemaphore : public NonCopyable {
  public:
    ...
    ScopedSignalSemaphore(ScopedSignalSemaphore&& rhs)
        : mDevice(rhs.mDevice), mSemaphore(rhs.mSemaphore) {
        rhs.mDevice = nullptr;
        rhs.mSemaphore = VK_NULL_HANDLE;
    }

    ScopedSignalSemaphore& operator=(ScopedSignalSemaphore&& rhs) {
...
MaybeError Device::SubmitPendingCommands() {
    ...
    for (const auto& type : kSemaphoreTypes) {
        if (mRecordingContext.externalTexturesForEagerTransition[type].size() > 0) {
            ...
            DAWN_TRY_ASSIGN(semaphore, mExternalSemaphoreService->CreateExportableSemaphore(type));
            externalTextureSemaphores[type] = ScopedSignalSemaphore(this, semaphore);
