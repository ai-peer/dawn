...
// static
void Buffer::TransitionMappableBuffersEagerly(Device* device, std::set<Buffer*> buffers) {
    ASSERT(!buffers.empty());
    ASSERT(device->GetPendingRecordingContext()->mappableBuffersForEagerTransition.empty());
    ...
    for (Buffer* buffer : buffers) {
        ...
        if (buffer->TrackUsageAndGetResourceBarrier(device->GetPendingRecordingContext(), mapUsage,
