...
constexpr wgpu::BufferUsage kShaderUsages = wgpu::BufferUsage::Uniform |
                                            wgpu::BufferUsage::Storage | kInternalStorageBuffer |
                                            kReadOnlyStorageBuffer;
constexpr wgpu::BufferUsage kReadOnlyShaderUsages = kShaderUsages & kReadOnlyBufferUsages;
...
bool Buffer::TrackUsageAndGetResourceBarrier(CommandRecordingContext* recordingContext,
    ...
    const bool isMapUsage = usage & kMapUsages;
    ...
    if (IsSubset(GetUsage() & ~kMapUsages, kReadOnlyBufferUsages)) {
        // Buffer can't be written to by device. There is no need for any pipeline barriers.
        // Execution dependencies are enough for write-after-read hazards, and vkQueueSubmit
        // does an implicit domain and visibility operation, so read-after-write hazards
        // are covered for HOST_COHERENT memory.
        return false;
    }

    if (!isMapUsage && (GetUsage() & kMapUsages)) {
    ...
    if (readOnly) {
        if ((shaderStage & wgpu::ShaderStage::Fragment) &&
            ...
            // waited, there is no need for fragment to wait.
        ...
        if (mLastWriteUsage == wgpu::BufferUsage::None) {
            // Read -> read dependency. No barrier needed.
            return false;
        }
        ...
        *srcStages |= VulkanPipelineStage(mLastWriteUsage, mLastWriteShaderStage);
        ...
        if ((mLastWriteUsage == wgpu::BufferUsage::None && mReadUsage == wgpu::BufferUsage::None) ||
            IsSubset(usage | mLastWriteUsage | mReadUsage, kMapUsages)) {
