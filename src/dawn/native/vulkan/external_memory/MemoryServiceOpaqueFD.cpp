...
ResultOrError<MemoryImportParams> Service::GetMemoryImportParams(
    ...
    // Choose whether to use a dedicated allocation depending on what both the application and the
    // driver tell us.
    switch (opaqueFDDescriptor->dedicatedAllocation) {
        case NeedsDedicatedAllocation::Yes:
            params.dedicatedAllocation = true;
            break;

        case NeedsDedicatedAllocation::No:
            params.dedicatedAllocation = false;
            break;
        ...
        case NeedsDedicatedAllocation::Detect:
            if (mDevice->GetDeviceInfo().HasExt(DeviceExt::DedicatedAllocation)) {
                ...
                // The Vulkan spec requires that requiresDA => prefersDA so we can just check for
                // prefersDA
                params.dedicatedAllocation = dedicatedRequirements.prefersDedicatedAllocation;
            } else {
                params.dedicatedAllocation = false;
            }
            break;
...
ResultOrError<VkDeviceMemory> Service::ImportMemory(ExternalMemoryHandle handle,
    ...
    // Tag the memory as dedicated for this texture. This is sometimes necessary for imported
    // texture. We could query whether it is really needed but since the memory is only for that one
    // texture we might as well always use a dedicated allocation for it when the feature is
    // available.
