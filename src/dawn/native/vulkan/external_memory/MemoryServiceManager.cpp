...
#if DAWN_PLATFORM_IS(ANDROID) || DAWN_PLATFORM_IS(POSIX)
#include "dawn/native/vulkan/external_memory/MemoryServiceAHardwareBuffer.h"
#include "dawn/native/vulkan/external_memory/MemoryServiceDmaBuffer.h"
#include "dawn/native/vulkan/external_memory/MemoryServiceOpaqueFD.h"
#endif  // DAWN_PLATFORM_IS(ANDROID) || DAWN_PLATFORM_IS(POSIX)
...
// Linux and ChromeOS
    ...
    if (memoryServiceDmaBuffer->Supported()) {
        services_.emplace(
            std::make_pair(ExternalImageType::DmaBuf, std::move(memoryServiceDmaBuf)));
    }
#endif  // DAWN_PLATFORM_IS(POSIX)
}
