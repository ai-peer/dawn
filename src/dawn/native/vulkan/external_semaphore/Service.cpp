...
#if DAWN_PLATFORM_IS(FUCHSIA)
    serviceImpl_ = std::make_unique<ServiceImplementationZirconHandle>(mDevice);
#elif DAWN_PLATFORM_IS(LINUX)  // Android, ChromeOS and Linux
    serviceImpl_ = std::make_unique<ServiceImplementationFD>(mDevice);
#else
    serviceImpl_ = std::make_unique<ServiceImplementationNull>(mDevice);
#endif
}
