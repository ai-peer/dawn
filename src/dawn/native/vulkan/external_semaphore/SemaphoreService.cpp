...
#elif DAWN_PLATFORM_IS(LINUX)  // Android, ChromeOS and Linux
    mServiceImpl = CreateFDSerivce(device);
#endif
}
...
ResultOrError<VkSemaphore> Service::ImportSemaphore(ExternalSemaphoreHandle handle) {
    if (!mServiceImpl) {
        return DAWN_INTERNAL_ERROR(
            "No valid semaphore service to interop inside Vulkan for importing semaphore.");
    }
...
ResultOrError<VkSemaphore> Service::CreateExportableSemaphore() {
    if (!mServiceImpl) {
        return DAWN_INTERNAL_ERROR(
            "No valid semaphore service to interop inside Vulkan for creating exportable "
            "semaphore.");
    }
...
ResultOrError<ExternalSemaphoreHandle> Service::ExportSemaphore(VkSemaphore semaphore) {
    if (!mServiceImpl) {
        return DAWN_INTERNAL_ERROR(
            "No valid semaphore service to interop inside Vulkan for exporting semaphore.");
    }
