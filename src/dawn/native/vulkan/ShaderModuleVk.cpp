...
ResultOrError<ShaderModule::ModuleAndSpirv> ShaderModule::GetHandleAndSpirv(
    ...
    // Transform external textures into the binding locations specified in the bgl. Note, this has
    // to happen before the general binding loop below as external textures seem to show up as
    // `Texture` below and having them entered here allows us to skip adding to the texture hash and
    // ending up with a duplicate binding.
        ...
        for (const auto& [binding, _] : groupBindingInfo) {
            ...
            switch (info.bindingType) {
                ...
                case BindingInfoType::StorageTexture:
                    // If this texture has already been found as an external texture above, we don't
                    // want to re-add it here.
                    if (bindings.external_texture.find(srcBindingPoint) !=
                        bindings.external_texture.end()) {
                        continue;
                    }
