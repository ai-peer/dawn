...
#define SPIRV_COMPILATION_REQUEST_MEMBERS(X)                                                     \
    ...
    X(tint::spirv::writer::Options, tint_writer_options)                                         \
...
ResultOrError<ShaderModule::ModuleAndSpirv> ShaderModule::GetHandleAndSpirv(
    ...
    req.tint_writer_options.clamp_frag_depth = clampFragDepth;
    req.tint_writer_options.disable_robustness = !GetDevice()->IsRobustnessEnabled();
    req.tint_writer_options.emit_vertex_point_size = true;
    req.tint_writer_options.disable_workgroup_init =
        GetDevice()->IsToggleEnabled(Toggle::DisableWorkgroupInit);
    req.tint_writer_options.use_zero_initialize_workgroup_memory_extension =
        GetDevice()->IsToggleEnabled(Toggle::VulkanUseZeroInitializeWorkgroupMemoryExtension);
    req.tint_writer_options.bindings = std::move(bindings);
    req.tint_writer_options.disable_image_robustness =
    ...
    req.tint_writer_options.disable_runtime_sized_array_index_clamping =
        GetDevice()->IsToggleEnabled(Toggle::VulkanUseBufferRobustAccess2);
    req.tint_writer_options.use_tint_ir = GetDevice()->IsToggleEnabled(Toggle::UseTintIR);
    ...
    if (GetDevice()->HasFeature(Feature::ChromiumExperimentalSubgroupUniformControlFlow)) {
        req.tint_writer_options.experimental_require_subgroup_uniform_control_flow = true;
    } else {
        req.tint_writer_options.experimental_require_subgroup_uniform_control_flow = false;
    }
    ...
    DAWN_TRY_LOAD_OR_RUN(
        ...
        [](SpirvCompilationRequest r) -> ResultOrError<CompiledSpirv> {
            ...
            auto tintResult = tint::spirv::writer::Generate(program, r.tint_writer_options);
