...
template <>
void CacheResult<vulkan::ShaderModule::Spirv>::Store(BlobCache* cache,
                                                     const CacheKey& key,
                                                     const vulkan::ShaderModule::Spirv& spirv) {
    ASSERT(cache != nullptr);
    cache->Store(key, spirv.WordCount() * sizeof(uint32_t), spirv.Code());
}
...
ResultOrError<ShaderModule::ModuleAndSpirv> ShaderModule::GetHandleAndSpirv(
    ...
    DAWN_TRY_ASSIGN(
        spirv,
        LoadOrRun(
            GetDevice(), std::move(req), Spirv::FromBlob,
            [](SpirvCompilationRequest r) -> ResultOrError<Spirv> {
                ...
                return Spirv::FromBlob(CreateBlob(std::move(result.spirv)));
            }));
    ...
    if (newHandle != VK_NULL_HANDLE) {
        spirv.EnsureStored(GetDevice()->GetBlobCache());
        moduleAndSpirv =
