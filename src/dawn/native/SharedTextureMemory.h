...
class SharedTextureMemoryBase : public ApiObjectBase,
  ...
  public:
    ...
    // Register a texture with this SharedTextureMemory, and return a
    // `State` object to track accesses using the texture.
    SharedTextureMemoryState* RegisterTexture(TextureBase* texture);

    // Unregister a texture with this SharedTextureMemory.
    void DeregisterTexture(TextureBase* texture);
