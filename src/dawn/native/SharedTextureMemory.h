...
class SharedTextureMemoryBase : public ApiObjectBase,
  ...
  private:
    ...
    // use.
    void BeginAccessScope(TextureBase* texture,
    ...
    void EndAccessScope(TextureBase* texture, PendingFenceList* fences);

    struct AccessScope {
        TextureBase* texture;
        PendingFenceList pendingBeginFences;
    };
    // Associative list mapping texture -> PendingFenceList.
