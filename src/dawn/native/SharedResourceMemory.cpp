...
SharedResourceMemory::SharedResourceMemory(DeviceBase* device, ObjectBase::ErrorTag tag, const char* label)
...
bool SharedResourceMemory::APIBeginAccess(TextureBase* texture,
                                          const SharedTextureMemoryBeginAccessDescriptor* descriptor) {
    return !GetDevice()->ConsumedError(BeginAccess(texture, descriptor), "calling %s.BeginAccess(%s).",
                                   this, texture);
}
...
MaybeError SharedResourceMemory::BeginAccess(Resource* resource,
    ...
    if (resource->IsReadOnly()) {
        ...
        DAWN_INVALID_IF(descriptor->concurrentRead, "Concurrent reading read-write %s.", resource);
...
bool SharedResourceMemory::APIEndAccess(TextureBase* texture, SharedTextureMemoryEndAccessState* state) {
    bool didEnd = false;
...
const WeakRef<SharedResourceMemory>& SharedResourceMemoryContents::GetSharedResourceMemory()
    const {
