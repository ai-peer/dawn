...
// pass.
static constexpr wgpu::TextureUsage kResolveTextureLoadingUsages =
    wgpu::TextureUsage::RenderAttachment | kResolveAttachmentLoading;
...
class TextureBase : public ApiObjectBase {
  ...
  private:
    ...
    // Implicit MSAA textures to be used as attachment during MSAA render to single sampled.
    // This is a table to map from sample count to the respective MSAA texture.
    std::map<uint32_t, Ref<CachedMultisampleAttachment>> mCachedImplicitMSAttachments;
};
