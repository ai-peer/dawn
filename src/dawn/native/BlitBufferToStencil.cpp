...
MaybeError BlitR8ToStencil(DeviceBase* device,
    ...
    Ref<BindGroupLayoutBase> bgl0;
    {
        ...
        DAWN_TRY_ASSIGN(bgl0, device->CreateBindGroupLayout(&bglDesc));
    }
    ...
    Ref<PipelineLayoutBase> pipelineLayout;
    {
        ...
        BindGroupLayoutBase* bgl = bgl0.Get();
    ...
    Ref<RenderPipelineBase> clearRenderPipeline;
    fragmentState.entryPoint = "frag_noop";
    DAWN_TRY_ASSIGN(clearRenderPipeline, device->CreateRenderPipeline(&renderPipelineDesc));

    // Build 8 pipelines masked to replace each bit of the stencil.
    std::array<Ref<RenderPipelineBase>, 8> renderPipelines;
    fragmentState.entryPoint = "frag_check_src_stencil";
    for (uint32_t bit = 0; bit < 8; ++bit) {
        dsState.stencilWriteMask = 1u << bit;
        DAWN_TRY_ASSIGN(renderPipelines[bit], device->CreateRenderPipeline(&renderPipelineDesc));
    }
    ...
    for (uint32_t z = 0; z < copyExtent.depthOrArrayLayers; ++z) {
        ...
        pass->APISetPipeline(clearRenderPipeline.Get());
        ...
        for (uint32_t bit = 0; bit < 8; ++bit) {
            pass->APISetPipeline(renderPipelines[bit].Get());
