...
MaybeError BlitDepthToBuffer(DeviceBase* device,
    ...
    if (format.format == wgpu::TextureFormat::Depth16Unorm && dst.buffer->GetSize() % 4 != 0 &&
        ...
        // width, to a compact buffer. When we copy the last texel, we inevitablly needs to access
        ...
        // OpenGL/GLES, the size of storage buffer binding for the shader is not. Thus we make an
        // intermediate buffer aligned with 4-byte for the compute shader to safely access, and
        // perform an additional buffer to buffer copy at the end. This path should be hit rarely.
