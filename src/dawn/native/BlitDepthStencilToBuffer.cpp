...
@compute @workgroup_size(8, 8, 1) fn blit_depth_to_buffer(@builtin(global_invocation_id) id : vec3u) {
    ...
    // Sampling doesn't work as intended? translate to glsl texelFetch
    dst_buf[dstOffset] = textureLoad(src_tex, coord, 0);
    // dst_buf[dstOffset] = textureLoad(src_tex, vec2u(0, 0), 0);
    // dst_buf[dstOffset] = textureLoad(src_tex, coord, 0).r;

    // dst_buf[0] = textureLoad(src_tex, coord, 0);
    // dst_buf[0] = 0.20000000298023224;
    // dst_buf[0] = 0.9999;
    // dst_buf[dstOffset] = 0.9999;
}
...
MaybeError BlitDepthToBuffer(DeviceBase* device,
    ...
    Ref<BufferBase> uniformBuffer;
    {
        ...
        params[4] = dst.bytesPerRow / 4;  // f32
    ...
    // 1D?
    pass->APIDispatchWorkgroups((copyExtent.width + 7) / 8, (copyExtent.height + 7) / 8, 1);
