...
MaybeError BlitDepthToBuffer(DeviceBase* device,
    ...
    if (format.format == wgpu::TextureFormat::Depth16Unorm && dst.buffer->GetSize() % 4 != 0 &&
        ...
        // This path is made for OpenGL/GLES depth16unorm bliting a texture with an odd number of
        // width, to a compact buffer. When we copy the last texel, we inevitablly need to access
        ...
        // OpenGL/GLES backend, the size of storage buffer binding for the shader is not. Thus we
        // make an intermediate buffer aligned with 4-byte for the compute shader to safely access,
        // and perform an additional buffer to buffer copy at the end. This path should be hit
        // rarely.
        useIntermediateCopyBuffer = true;
        BufferDescriptor descriptor = {};
        descriptor.size = Align(dst.buffer->GetSize(), 4);
        descriptor.usage = wgpu::BufferUsage::CopySrc | kInternalStorageBuffer;
        DAWN_TRY_ASSIGN(destinationBuffer,
                        device->CreateBuffer(&descriptor, UsageValidationMode::Internal));
    }
