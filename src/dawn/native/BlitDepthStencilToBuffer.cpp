...
@group(1) @binding(0) var src_tex : texture_depth_2d;
@group(0) @binding(0) var<storage, read_write> dst_buf : array<f32>;
...
// so it should not use it generally for the emulation.
...
@compute @workgroup_size(workgroupSizeX, workgroupSizeY, 1) fn blit_depth_to_buffer(@builtin(global_invocation_id) id : vec3u) {
    ...
    if (coord1.x < srcBoundary.x) {
        ...
        // Skip reading and packing the value at coord1.
...
ResultOrError<Ref<ComputePipelineBase>> CreateDepthBlitComputePipeline(DeviceBase* device,
    ...
    Ref<BindGroupLayoutBase> srcTextureBindGroupLayout;
    DAWN_TRY_ASSIGN(srcTextureBindGroupLayout,
                    utils::MakeBindGroupLayout(
                        device,
                        {
...
MaybeError BlitDepthToBuffer(DeviceBase* device,
    ...
    Ref<BindGroupLayoutBase> srcTextureBindGroupLayout;
    DAWN_TRY_ASSIGN(srcTextureBindGroupLayout, pipeline->GetBindGroupLayout(1));
    ...
    for (uint32_t z = 0; z < copyExtent.depthOrArrayLayers; z++) {
        ...
        Ref<BindGroupBase> srcTextureBindGroup;
        DAWN_TRY_ASSIGN(srcTextureBindGroup, utils::MakeBindGroup(device, srcTextureBindGroupLayout,
                                                                  {
                                                                      {0, srcView},
                                                                  },
                                                                  UsageValidationMode::Internal));
        pass->APISetBindGroup(1, srcTextureBindGroup.Get());
