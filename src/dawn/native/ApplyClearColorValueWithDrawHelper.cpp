...
ResultOrError<RenderPipelineBase*> GetOrCreateApplyClearValueWithDrawPipeline(
    ...
    fragment.targetCount = colorAttachmentCount;
...
bool ShouldApplyClearBigIntegerColorValueWithDraw(
    ...
    switch (format.GetAspectInfo(Aspect::Color).baseType) {
        ...
        // We don't need this workaround when the clear color values are representable in float.
        case wgpu::TextureComponentType::Float:
            return false;
...
MaybeError ApplyClearBigIntegerColorValueWithDraw(
    ...
    DAWN_TRY_ASSIGN(pipeline, GetOrCreateApplyClearValueWithDrawPipeline(
                                  device, key, renderPassDescriptor->colorAttachmentCount));
