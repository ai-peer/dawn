...
MaybeError PipelineBase::RunTintProgramTransformForOverrides(SingleShaderStage singleShaderStage,
    ...
    if (!metadata.overrides.empty()) {
        ...
        DAWN_TRY_ASSIGN(program,
                        RunTransforms(&transformManager, stage->module->GetRawTintProgram(),
    ...
    if (singleShaderStage == SingleShaderStage::Compute) {
        // Validate workgroup size after the transform as it could possibly be updated.
        const CombinedLimits& limits = GetDevice()->GetLimits();
        tint::inspector::Inspector inspector(stage->GetTintProgram());
        ...
        const size_t workgroupStorageSize = inspector.GetWorkgroupStorageSize(stage->entryPoint);
...
const tint::Program* ProgrammableStage::GetTintProgram() const {
    ...
    return module->GetRawTintProgram();
}
