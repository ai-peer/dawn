...
FutureID EventManager::Track(WGPUCallbackModeFlags mode, TrackedEvent* future) {
    ...
    if (isFuture) {
        ...
        mTrackedFutures.Use([&](auto trackedFutures) {
            futureID = mNextFutureID++;
            // mNextFutureID must be incremented inside the lock. Otherwise, there's a gap (where
            // this comment is) during which WaitAny will incorrectly assume this future is
            // Completed.
            trackedFutures->emplace(futureID, future);
        });
        return futureID;
    } else if (isProcessEvents) {
        mTrackedPollEvents.Use([&](auto trackedPollEvents) {
            FutureID futureID = mNextFutureID++;
            trackedPollEvents->emplace(futureID, future);
        });
