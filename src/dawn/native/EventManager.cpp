    ...
    for (size_t sliceStart = 0; sliceStart < futures.size();) {
...

        {
            ...
            if (waitDevice) {
                ...
                success = OSEventReceiver::WaitAny(sliceLength, &futures[sliceStart], timeout);
            }
            anySuccess |= success;
        }
...
FutureID EventManager::TrackEvent(WGPUCallbackModeFlags mode, Ref<TrackedEvent>&& future) {
    switch (ValidateAndFlattenCallbackMode(mode)) {
        ...
        case CallbackMode::FutureOrSpontaneous: {
            ...
            mTrackedFutures->emplace(futureID, std::move(future));
            return futureID;
        }
        case CallbackMode::ProcessEvents:
        case CallbackMode::ProcessEventsOrSpontaneous: {
            FutureID futureID = mNextFutureID++;
            mTrackedPollEvents->emplace(futureID, std::move(future));
...
void EventManager::ProcessPollEvents() {
    ...
    mTrackedPollEvents.Use([&](auto trackedPollEvents) {
...
wgpu::WaitStatus EventManager::WaitAny(size_t count, FutureWaitInfo* infos, Nanoseconds timeout) {
    ...
    mTrackedFutures.Use([&](auto trackedFutures) {
...
EventManager::TrackedEvent::TrackedEvent(InstanceBase* instance,
...
OSEventPrimitive::T EventManager::TrackedEvent::GetPrimitive() const {
    return mReceiver.Get();
}

void EventManager::TrackedEvent::EnsureComplete() {
