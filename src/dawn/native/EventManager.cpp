...
EventManager::~EventManager() {
    // Ensure all events are completed, and leave mTrackedFutures and mTrackedPollEvents empty.
    mTrackedFutures.Use([&](auto trackedFutures) {
        for (auto& [futureID, trackedEvent] : std::move(*trackedFutures)) {
            trackedEvent->EnsureComplete(EventCompletionType::Shutdown);
        }
    });
    mTrackedPollEvents.Use([&](auto trackedPollEvents) {
        for (auto& [futureID, trackedEvent] : std::move(*trackedPollEvents)) {
            trackedEvent->EnsureComplete(EventCompletionType::Shutdown);
        }
    });
}
