...
bool DeviceBase::IsDeviceIdle() {
    ...
    if (mCompletedSerial == mLastSubmittedSerial && mCompletedSerial >= mLastForceFlushSerial) {
        return true;
    }
    return false;
}
...
MaybeError DeviceBase::Tick() {
    // Skip tick while still in middle of 'DummySubmit()'.
    if (mDummySubmit) {
        return {};
    }
    ...
    // 2. or there are any forcibly flushed commands to be executed.
    if (mLastSubmittedSerial > mCompletedSerial || mLastForceFlushSerial > mCompletedSerial) {
        // There is no GPU work in flight, we need to move the serials forward so that CPU
        // operations waiting on GPU completion can know they don't have to wait.
...
void DeviceBase::ForceEventualFlushOfCommands() {
    mLastForceFlushSerial = GetPendingCommandSerial();
}

void DeviceBase::DummySubmit() {
    mDummySubmit = true;
    CommandEncoder* encoder = APICreateCommandEncoder(nullptr);
    CommandBufferBase* commandBuffer = encoder->APIFinish();
    mQueue->APISubmit(1, &commandBuffer);
