...
BufferBase* DeviceBase::APICreateErrorBuffer(const BufferDescriptor* desc) {
    ...
    MaybeError maybeError = DoValidateBufferDescriptor(desc);
...
ResultOrError<Ref<BufferBase>> DeviceBase::CreateBuffer(const BufferDescriptor* descriptor) {
    DAWN_TRY(ValidateIsAlive());
    if (IsValidationEnabled()) {
        DAWN_TRY(DoValidateBufferDescriptor(descriptor));
    }
...
MaybeError DeviceBase::DoValidateBufferDescriptor(const BufferDescriptor* descriptor) {
