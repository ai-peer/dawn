...
struct GenericFunctionTask : CallbackTask {
  public:
    GenericFunctionTask(std::function<void()> func) : mFunction(func) {}
...
void DeviceBase::Destroy() {
    ...
    if (mState != State::BeingCreated) {
        ...
        for (std::unique_ptr<CallbackTask>& callbackTask : callbackTasks) {
            CallbackTask::HandleShutDownInNextTick(*this, std::move(callbackTask));
        }
    }
...
void DeviceBase::HandleError(InternalErrorType type,
    ...
    if (type == InternalErrorType::DeviceLost) {
        ...
        auto callbackTasks = mCallbackTaskManager->AcquireCallbackTasks();
        for (std::unique_ptr<CallbackTask>& callbackTask : callbackTasks) {
            CallbackTask::HandleDeviceLossInNextTick(*this, std::move(callbackTask));
        }
