...
void DeviceBase::HandleError(std::unique_ptr<ErrorData> error,
    ...
    DawnErrorType type = error->GetType();
    if (type == DawnErrorType::DeviceLost) {
        mState = State::Disconnected;
...
void DeviceBase::ConsumeError(std::unique_ptr<ErrorData> error, DawnErrorType allowedErrors) {
    ASSERT(error != nullptr);
    AppendDebugLayerMessages(error.get());
    HandleError(std::move(error), allowedErrors);
}
...
void DeviceBase::APIForceLoss(wgpu::DeviceLostReason reason, const char* message) {
    ...
    HandleError(DAWN_INTERNAL_ERROR(message), DawnErrorType::None, ToAPI(reason));
}
...
BufferBase* DeviceBase::APICreateBuffer(const BufferDescriptor* descriptor,
    ...
    if (ConsumedError(CreateBuffer(descriptor), &result, allowedErrors,
...
QuerySetBase* DeviceBase::APICreateQuerySet(const QuerySetDescriptor* descriptor,
    ...
    if (ConsumedError(CreateQuerySet(descriptor), &result, allowedErrors,
...
TextureBase* DeviceBase::APICreateTexture(const TextureDescriptor* descriptor,
    ...
    if (ConsumedError(CreateTexture(descriptor), &result, allowedErrors,
...
BufferBase* DeviceBase::APICreateErrorBuffer(const BufferDescriptor* desc,
    ...
    if (maybeError.IsError()) {
        CONSUME_ERROR_WITH_CONTEXT_AND_RETURN(this, maybeError.AcquireError(), allowedErrors,
        ...
        if (clientErrorInfo != nullptr && clientErrorInfo->outOfMemory) {
            CONSUME_ERROR_AND_RETURN(
                ...
                allowedErrors, BufferBase::MakeError(this, &fakeDescriptor));
        }
    }
...
void DeviceBase::APIInjectError(wgpu::ErrorType type, const char* message) {
    ...
    HandleError(DAWN_MAKE_ERROR(FromWGPUErrorType(type), message),
                DawnErrorType::Validation | DawnErrorType::OutOfMemory);
}

void DeviceBase::APIValidateTextureDescriptor(const TextureDescriptor* desc) {
    CONSUME_IF_ERROR(this, ValidateTextureDescriptor(this, desc), DawnErrorType::Validation);
}
