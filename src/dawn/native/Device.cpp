...
void AddCallbackTask(DeviceBase& device,
...
void AddCallbackTask(DeviceBase& device,
    ...
    std::string messageStr = message;
    device.GetCallbackTaskManager()->AddCallbackTask(
        [=] { callback(arg1, arg2, messageStr.c_str(), userdata); });
}
...
void DeviceBase::Destroy() {
    ...
    if (mState != State::BeingCreated) {
        ...
        if (mDeviceLostCallback != nullptr) {
            AddCallbackTask(*this, mDeviceLostCallback, WGPUDeviceLostReason_Destroyed,
...
void DeviceBase::HandleError(std::unique_ptr<ErrorData> error,
    ...
    if (type == InternalErrorType::DeviceLost) {
        ...
        // possible deadlock.
        if (mDeviceLostCallback != nullptr) {
            AddCallbackTask(*this, mDeviceLostCallback, lost_reason, message, mDeviceLostUserdata);
            mDeviceLostCallback = nullptr;
        }
        ...
        if (!captured && mUncapturedErrorCallback != nullptr) {
            AddCallbackTask(*this, mUncapturedErrorCallback,
...
bool DeviceBase::APIPopErrorScope(wgpu::ErrorCallback callback, void* userdata) {
    ...
    if (IsLost()) {
        AddCallbackTask(*this, callback, WGPUErrorType_DeviceLost, "GPU device disconnected",
                        userdata);
        return returnValue;
    }
    if (mErrorScopeStack->Empty()) {
        AddCallbackTask(*this, callback, WGPUErrorType_Unknown, "No error scopes to pop", userdata);
        return returnValue;
    }
    ErrorScope scope = mErrorScopeStack->Pop();
    AddCallbackTask(*this, callback, static_cast<WGPUErrorType>(scope.GetErrorType()),
...
void DeviceBase::APICreateComputePipelineAsync(const ComputePipelineDescriptor* descriptor,
    ...
    if (maybeResult.IsError()) {
        ...
        // TODO(crbug.com/dawn/1122): Call callbacks only on wgpuInstanceProcessEvents
        AddCallbackTask(*this, callback, status, static_cast<WGPUComputePipeline>(nullptr),
...
void DeviceBase::APICreateRenderPipelineAsync(const RenderPipelineDescriptor* descriptor,
    ...
    if (maybeResult.IsError()) {
        ...
        WGPUCreatePipelineAsyncStatus status =
            CreatePipelineAsyncStatusFromErrorType(error->GetType());
        // TODO(crbug.com/dawn/1122): Call callbacks only on wgpuInstanceProcessEvents
        AddCallbackTask(*this, callback, status, static_cast<WGPURenderPipeline>(nullptr),
                        error->GetMessage().c_str(), userdata);
    }
}
...
MaybeError DeviceBase::CreateComputePipelineAsync(const ComputePipelineDescriptor* descriptor,
    ...
    if (cachedComputePipeline.Get() != nullptr) {
        // TODO(crbug.com/dawn/1122): Call callbacks only on wgpuInstanceProcessEvents
        AddCallbackTask(*this, callback, WGPUCreatePipelineAsyncStatus_Success,
