...
MaybeError DeviceBase::Tick() {
    ...
    if (mQueue != nullptr && mQueue->HasPendingTrackTask()) {
        mTicksSinceLastForceFlush++;
        if (mTicksSinceLastForceFlush > 3 && !mDidDummySubmit) {
            DummySubmit();
        }
    }
...
void DeviceBase::ForceEventualFlushOfCommands() {
    mForceNextTick = true;
    mTicksSinceLastForceFlush = 0;
    mDidDummySubmit = false;
}

bool DeviceBase::CheckCommandsNeedFlush() {
    return mForceNextTick;
}

void DeviceBase::DummySubmit() {
    mDidDummySubmit = true;
