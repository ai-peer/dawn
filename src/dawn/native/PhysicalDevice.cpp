...
bool PhysicalDeviceBase::HasFeature(wgpu::FeatureName feature, const TogglesState& toggles) const {
...
size_t PhysicalDeviceBase::EnumerateFeatures(wgpu::FeatureName* features,
                                             const TogglesState& toggles) const {
    return GetSupportedFeatures(toggles).EnumerateFeatures(features);
}
...
FeaturesSet PhysicalDeviceBase::GetSupportedFeatures(const TogglesState& toggles) const {
    ...
    for (uint32_t i : IterateBitSet(mSupportedFeatures.featuresBitSet)) {
        ...
        if (HasFeature(featureName, toggles)) {
