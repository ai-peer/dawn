...
bool PhysicalDeviceBase::HasFeatureWithToggles(wgpu::FeatureName feature,
...
size_t PhysicalDeviceBase::EnumerateFeaturesWithToggles(wgpu::FeatureName* features,
                                                        const TogglesState& toggles) const {
    return GetSupportedFeaturesWithToggles(toggles).EnumerateFeatures(features);
}
...
FeaturesSet PhysicalDeviceBase::GetSupportedFeaturesWithToggles(const TogglesState& toggles) const {
    ...
    for (uint32_t i : IterateBitSet(mSupportedFeatures.featuresBitSet)) {
        ...
        if (HasFeatureWithToggles(featureName, toggles)) {
...
bool PhysicalDeviceBase::SupportsAllRequiredFeaturesWithToggles(
    const ityp::span<size_t, const wgpu::FeatureName>& features,
    const TogglesState& toggles) const {
    for (wgpu::FeatureName f : features) {
        if (!HasFeatureWithToggles(f, toggles)) {
            return false;
        }
    }
    return true;
