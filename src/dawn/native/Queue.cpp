...
void QueueBase::APISubmit(uint32_t commandCount, CommandBufferBase* const* commands) {
    ...
    // Destroy the command buffers even if SubmitInternal failed.
    // TODO(crbug.com/dawn/1863): Is this right?
    for (uint32_t i = 0; i < commandCount; ++i) {
        commands[i]->Destroy();
    }

    if (GetDevice()->ConsumedError(result)) {
        return;
    }
}
...
MaybeError QueueBase::SubmitInternal(uint32_t commandCount, CommandBufferBase* const* commands) {
    ...
    if (device->IsValidationEnabled()) {
        DAWN_TRY(ValidateSubmit(commandCount, commands), "calling %s.Submit(%s)", this,
