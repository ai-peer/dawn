...
void TrackTaskCallback::SetFinisnedSerial(ExecutionSerial serial) {
    mSerial = serial;
}
...
void QueueBase::TrackTask(std::unique_ptr<TrackTaskCallback> task) {
    ...
    // No pending commands after the above forced eventual flush means all previously submitted
    // works are done. So we can move the task to the callback task manager, as it's ready to be
    // called.
    if (!GetDevice()->HasPendingCommands()) {
        task->SetFinisnedSerial(GetDevice()->GetCompletedCommandSerial());
...
void QueueBase::Tick(ExecutionSerial finishedSerial) {
    ...
    for (auto& task : tasks) {
        task->SetFinisnedSerial(finishedSerial);
        GetDevice()->GetCallbackTaskManager()->AddCallbackTask(std::move(task));
    }
}
