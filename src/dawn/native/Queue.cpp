...
MaybeError QueueBase::IterateResourceForSubmit(uint32_t commandCount,
...
void QueueBase::SubmitInternal(uint32_t commandCount, CommandBufferBase* const* commands) {
    ...
    if (device->IsValidationEnabled()) {
        if (device->ConsumedError(
                IterateResourceForSubmit</*validation=*/true>(commandCount, commands))) {
            return;
        }
    } else {
        if (device->ConsumedError(
                IterateResourceForSubmit</*validation=*/false>(commandCount, commands))) {
            return;
        }
    }
    ASSERT(!IsError());
