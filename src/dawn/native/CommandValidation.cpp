...
MaybeError ValidateTextureCopyRange(DeviceBase const* device,
    ...
    DAWN_INVALID_IF(texture->GetFormat().IsMultiPlanar() && !HasOneBit(aspect),
                    "Texture copy aspect (%s) is not supported for multiplanar texture %s",
                    textureCopy.aspect, texture);
