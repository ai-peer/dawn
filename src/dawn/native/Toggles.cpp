...
bool RequiredTogglesSet::operator==(const RequiredTogglesSet& rhs) const {
    return (togglesProvided == rhs.togglesProvided) &&
           (togglesEnabled == rhs.togglesEnabled);
}
...
void TogglesState::InheritToggles(const TogglesState& inheritedToggles) {
    ...
    for (uint32_t i : inheritedToggles.mTogglesSet.Iterate()) {
        ...
        Inherit(toggle, inheritedToggles.mEnabledToggles.Has(toggle), inheritedToggles.mForcedToggles.Has(toggle));
    }
}
