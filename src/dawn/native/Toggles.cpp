...
const TogglesSet& TogglesState::GetEnabledTogglesSet() const {
    return mEnabledToggles;
}

// Allowing TogglesState to be used in cache key.
template <>
void stream::Stream<TogglesState>::Write(stream::Sink* s, const TogglesState& togglesState) {
    StreamIn(s, togglesState.GetEnabledTogglesSet().bitset);
}
