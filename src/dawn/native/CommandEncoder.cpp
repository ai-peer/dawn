...
namespace dawn::native {
    ...
    namespace {
        ...
        MaybeError ValidateRenderPassDescriptor(DeviceBase* device,
            ...
            if (descriptor->timestampWriteCount > 0 && descriptor->timestampWrites != nullptr) {
                ...
                for (uint32_t i = 0; i < descriptor->timestampWriteCount; ++i) {
                    ...
                    DAWN_TRY_CONTEXT(ValidateRenderPassTimestampLocation(
                                     ...
                                     "validating location of loctimestampWrites[%u].", i);

                    auto checkIt = tempMap.find(querySet);
                    DAWN_INVALID_IF(checkIt != tempMap.end() && checkIt->second[queryIndex],
                    ...
                    auto addIt = tempMap.emplace(querySet, querySet->GetQueryCount()).first;
    ...
    ComputePassEncoder* CommandEncoder::APIBeginComputePass(
        ...
        bool success = mEncodingContext.TryEncode(
            ...
            [&](CommandAllocator* allocator) -> MaybeError {
                ...
                for (uint32_t i = 0; i < descriptor->timestampWriteCount; i++) {
                    Ref<QuerySetBase> querySet = descriptor->timestampWrites[i].querySet;
                    uint32_t queryIndex = descriptor->timestampWrites[i].queryIndex;
                    ...
                    TrackQueryAvailability(querySet.Get(), queryIndex);
                }

                cmd->timestampWrites = timestampWritesAtBeginning;
    ...
    RenderPassEncoder* CommandEncoder::APIBeginRenderPass(const RenderPassDescriptor* descriptor) {
        ...
        bool success = mEncodingContext.TryEncode(
            ...
            [&](CommandAllocator* allocator) -> MaybeError {
                ...
                for (uint32_t i = 0; i < descriptor->timestampWriteCount; i++) {
                    ...
                    usageTracker.TrackQueryAvailability(querySet.Get(), queryIndex);
                }
        ...
        if (success) {
            RenderPassEncoder* passEncoder = new RenderPassEncoder(
                ...
                std::move(attachmentState), timestampWritesAtEnd, width, height, depthReadOnly,
                stencilReadOnly);
            mEncodingContext.EnterPass(passEncoder);
            return passEncoder;
        }
