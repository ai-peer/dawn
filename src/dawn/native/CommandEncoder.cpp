...
MaybeError ValidateOrSetColorAttachmentSampleCount(const TextureViewBase* colorAttachment,
    ...
    if (implicitSampleCount > 1) {
        ...
        if (*sampleCount == 0) {
            *sampleCount = implicitSampleCount;
        }
    } else {
        uint32_t attachmentSampleCount = colorAttachment->GetTexture()->GetSampleCount();
        if (*sampleCount == 0) {
...
MaybeError ValidateRenderPassColorAttachment(DeviceBase* device,
    ...
    if (implicitSampleCount > 1) {
        DAWN_INVALID_IF(
            ...
            "render pass with implicit sample count (%u). The format does not support resolve.",
...
MaybeError ValidateRenderPassDescriptor(DeviceBase* device,
    ...
    if (msaaRenderToSingleSampledDesc) {
        DAWN_TRY(ValidateMSAARenderToSingleSampledDescriptor(device, descriptor,
