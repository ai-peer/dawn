...
MaybeError ValidateOrSetAttachmentSize(const TextureViewBase* attachment,
    ...
    if (attachment->GetFormat().IsMultiPlanar()) {
        attachmentSize =
            attachment->GetFormat().GetAspectSize(attachment->GetAspects(), attachmentSize);
    }
...
MaybeError ValidateRenderPassColorAttachment(DeviceBase* device,
    ...
    Aspect kRenderableAspects = Aspect::Color;
    ...
    if (device->HasFeature(Feature::MultiPlanarRenderTargets)) {
        kRenderableAspects |= Aspect::Plane0 | Aspect::Plane1;
    }
    DAWN_INVALID_IF(
