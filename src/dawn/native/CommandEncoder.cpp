...
Ref<RenderPassEncoder> CommandEncoder::BeginRenderPass(const RenderPassDescriptor* descriptor) {
    ...
    bool success = mEncodingContext.TryEncode(
        ...
        [&](CommandAllocator* allocator) -> MaybeError {
            ...
            std::set<TextureViewBase*> tracked3DColorAttachments;
            for (ColorAttachmentIndex index :
                ...
                bool canUsedMoreThanOnce = false;
                if (colorTarget->GetDimension() == wgpu::TextureViewDimension::e3D) {
                    if (tracked3DColorAttachments.find(colorTarget) !=
                        tracked3DColorAttachments.end()) {
                        canUsedMoreThanOnce = true;
                    } else {
                        tracked3DColorAttachments.insert(colorTarget);
                    }
                }

                if (!canUsedMoreThanOnce) {
