...
MaybeError ValidateOrSetColorAttachmentSampleCount(const TextureViewBase* colorAttachment,
    ...
    if (implicitSampleCount > 1) {
        DAWN_INVALID_IF(colorAttachment->GetTexture()->GetSampleCount() != 1,
                        ...
                        "msaaRenderToSingleSampledDesc.sampleCount (%u) > 1.",
        ...
        DAWN_INVALID_IF(!colorAttachment->GetTexture()->IsImplicitMSAARenderTextureViewSupported(),
                        "Color attachment %s was not created with "
                        "%s usage, which is required for render pass with "
                        "msaaRenderToSingleSampledDesc.sampleCount (%u) > 1.",
...
MaybeError ValidateRenderPassColorAttachment(DeviceBase* device,
    ...
    if (implicitSampleCount > 1) {
        DAWN_INVALID_IF(colorAttachment.resolveTarget != nullptr,
                        "Cannot set %s as a resolve target when render pass' "
                        "msaaRenderToSingleSampledDesc.sampleCount (%u) is > 1.",
...
MaybeError ValidateMSAARenderToSingleSampledDescriptor(
    ...
    if (msaaRenderToSingleSampledDesc != nullptr) {
        ...
        DAWN_INVALID_IF(!IsValidSampleCount(msaaRenderToSingleSampledDesc->sampleCount),
                        "msaaRenderToSingleSampledDesc.sampleCount (%u) is not supported.",
                        msaaRenderToSingleSampledDesc->sampleCount);
        ...
        *implicitSampleCount = msaaRenderToSingleSampledDesc->sampleCount;
    } else {
...
Ref<RenderPassEncoder> CommandEncoder::BeginRenderPass(const RenderPassDescriptor* descriptor) {
    ...
    if (success) {
        Ref<RenderPassEncoder> passEncoder = RenderPassEncoder::Create(
            ...
            std::move(attachmentState), width, height, implicitSampleCount, depthReadOnly,
            stencilReadOnly, passEndCallback);
