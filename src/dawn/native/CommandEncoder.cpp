...
MaybeError ValidateRenderPassDescriptor(DeviceBase* device,
    ...
    if (descriptor->timestampWrites != nullptr) {
        // The TrackQueryAvailability of RenderPassResourceUsageTracker is not used here because the
        // timestampWrites are not validated and encoded one by one, but encoded together after
        ...
        DAWN_INVALID_IF(
            beginningOfPassWriteIndex == wgpu::kQueryIndexUndefined &&
                endOfPassWriteIndex == wgpu::kQueryIndexUndefined,
            "At least one of beginningOfPassWriteIndex or endOfPassWriteIndex must be given");
        DAWN_TRY_CONTEXT(ValidateTimestampQuery(device, querySet, beginningOfPassWriteIndex),
                         "validating querySet and beginningOfPassWriteIndex of timestampWrites.");
        DAWN_TRY_CONTEXT(ValidateTimestampQuery(device, querySet, endOfPassWriteIndex),
                         "validating querySet and endOfPassWriteIndex of timestampWrites.");
        DAWN_INVALID_IF(beginningOfPassWriteIndex == endOfPassWriteIndex,
                        "beginningOfPassWriteIndex is equal to endOfPassWriteIndex in %s.",
                        querySet);
    }
    ...
    bool success = mEncodingContext.TryEncode(
        ...
        [&](CommandAllocator* allocator) -> MaybeError {
            ...
            if (descriptor->timestampWrites != nullptr) {
                ...
                if (beginningOfPassWriteIndex != wgpu::kQueryIndexUndefined) {
                    cmd->beginTimestamp.querySet = querySet;
                ...
                if (endOfPassWriteIndex != wgpu::kQueryIndexUndefined) {
                    cmd->endTimestamp.querySet = querySet;
