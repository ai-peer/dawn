    ...
    mEncodingContext.TryEncode(
        ...
        [&](CommandAllocator* allocator) -> MaybeError {
            if (GetDevice()->IsValidationEnabled()) {
                ...
                DAWN_TRY_CONTEXT(ValidateCopySizeFitsInBuffer(source, sourceOffset, size, true),
                                 "validating source %s copy size against allocated size.", source);
                DAWN_TRY_CONTEXT(
                    ValidateCopySizeFitsInBuffer(destination, destinationOffset, size, true),
                    "validating destination %s copy size against allocated size.", destination);
                DAWN_TRY(ValidateB2BCopyAlignment(size, sourceOffset, destinationOffset));
