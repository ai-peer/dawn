...
MaybeError ValidateOrSetColorAttachmentSampleCount(const TextureViewBase* colorAttachment,
    ...
    if (implicitSampleCount > 1) {
        DAWN_INVALID_IF(colorAttachment->GetTexture()->GetSampleCount() != 1,
                        "Color attachment %s sample count (%u) is not 1 for render pass with "
                        "implicit sample count (%u).",
                        colorAttachment, colorAttachment->GetTexture()->GetSampleCount(),
                        implicitSampleCount);

        DAWN_INVALID_IF(!colorAttachment->GetTexture()->IsImplicitMSAARenderTextureViewSupported(),
                        "Color attachment %s was not created with %s usage, which is required for "
                        "render pass with implicit sample count (%u).",
                        colorAttachment, wgpu::TextureUsage::MSAARenderToSingleSampledAttachment,
                        implicitSampleCount);
    ...
    if (*sampleCount == 0) {
        *sampleCount = attachmentSampleCount;
        ...
        // For implicitSampleCount > 1 case, attachmentSampleCount & *sampleCount should already
        // have the same value as implicitSampleCount, thus we don't need to check for their
        // equality.
        DAWN_INVALID_IF(
            implicitSampleCount <= 1 && *sampleCount != attachmentSampleCount,
            "Color attachment %s sample count (%u) does not match the sample count of the "
            "other attachments (%u).",
            colorAttachment, attachmentSampleCount, *sampleCount);
    }
...
MaybeError ValidateRenderPassColorAttachment(DeviceBase* device,
    ...
    DAWN_TRY(ValidateOrSetColorAttachmentSampleCount(attachment, implicitSampleCount, sampleCount));

    if (implicitSampleCount > 1) {
        DAWN_INVALID_IF(
            !attachment->GetFormat().supportsResolveTarget,
            "The color attachment %s format (%s) does not support being used in a "
            "render pass having implicit sample count (%u). The format does not support resolve.",
            attachment, attachment->GetFormat().format, implicitSampleCount);
        ...
        DAWN_INVALID_IF(colorAttachment.resolveTarget != nullptr,
                        ...
                        "when render pass has implicit sample count (%u).",
                        colorAttachment.resolveTarget, implicitSampleCount);
    } else {
...
MaybeError ValidateMSAARenderToSingleSampledDescriptor(
    const DeviceBase* device,
    const RenderPassDescriptor* descriptor,
    const DawnRenderPassDescriptorRenderToSingleSampled* msaaRenderToSingleSampledDesc,
    uint32_t* implicitSampleCount) {
    if (msaaRenderToSingleSampledDesc != nullptr) {
        ...
        DAWN_INVALID_IF(!IsValidSampleCount(msaaRenderToSingleSampledDesc->implicitSampleCount),
                        "msaaRenderToSingleSampledDesc.implicitSampleCount (%u) is not supported.",
                        msaaRenderToSingleSampledDesc->implicitSampleCount);

        DAWN_INVALID_IF(
            descriptor->colorAttachmentCount != 1,
            "colorAttachmentCount (%u) is not supported when msaaRenderToSingleSampledDesc is not "
            "empty. (Currently) colorAttachmentCount = 1 is supported.",
...
MaybeError ValidateRenderPassDescriptor(DeviceBase* device,
    ...
    DAWN_TRY(ValidateSTypes(descriptor->nextInChain,
                            {{wgpu::SType::RenderPassDescriptorMaxDrawCount,
                              wgpu::SType::DawnRenderPassDescriptorRenderToSingleSampled}}));
    ...
    DAWN_TRY(ValidateMSAARenderToSingleSampledDescriptor(
        device, descriptor, msaaRenderToSingleSampledDesc, implicitSampleCount));
    ...
    for (uint32_t i = 0; i < descriptor->colorAttachmentCount; ++i) {
        DAWN_TRY_CONTEXT(ValidateRenderPassColorAttachment(device, descriptor->colorAttachments[i],
                                                           *implicitSampleCount, width, height,
...
MaybeError ApplyMSAARenderToSingleSampledLoadOp(DeviceBase* device,
    ...
    // Read resolve texture in fragment shader and copy to the implicit MSAA attachment.
    // Use special usage flag kResolveAttachmentLoading for the resolve texture to tell the
    // validation that we will only read from the texture once at the begin of the render pass. So
    // there shoudn't be any read-write hazard.
    return BlitColorToColorWithDraw(device, renderPassEncoder, renderPassDescriptor,
                                    implicitSampleCount,
                                    /*src=*/renderPassDescriptor->colorAttachments[0].view,
                                    /*srcTextureUsage=*/kResolveAttachmentLoading);
}
        ...
        [&](CommandAllocator* allocator) -> MaybeError {
            ...
            for (ColorAttachmentIndex index :
                ...
                if (implicitSampleCount <= 1) {
                    ...
                    DAWN_TRY_ASSIGN(colorTarget,
                                    resolveTarget->GetTexture()->GetImplicitMSAARenderTextureView(
                                        implicitSampleCount));
