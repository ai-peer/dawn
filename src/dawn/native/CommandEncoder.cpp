...
MaybeError ValidateRenderPassDepthStencilAttachment(
    ...
    if (depthStencilAttachment->depthReadOnly ||
        ...
        DAWN_INVALID_IF(
            depthStencilAttachment->depthLoadOp != wgpu::LoadOp::Undefined ||
                depthStencilAttachment->depthStoreOp != wgpu::StoreOp::Undefined,
            "depthLoadOp (%s) and depthStoreOp (%s) must not be set if the attachment (%s) has "
            "no depth aspect or depthReadOnly (%u) is true.",
            depthStencilAttachment->depthLoadOp, depthStencilAttachment->depthStoreOp, attachment,
    ...
    if (depthStencilAttachment->stencilReadOnly ||
        ...
        DAWN_INVALID_IF(
            ...
            "stencilLoadOp (%s) and stencilStoreOp (%s) must not be set if the attachment (%s) has "
            "no stencil aspect or stencilReadOnly (%u) is true.",
