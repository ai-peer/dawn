...
struct MemoResultTraits {
    // Implementations should define this static method for converting |T| into |CachedBlob|.
...
//
// The key is generated from every argument, as well as a CacheKey::Type to uniquely indentify
// this function signature. It would be better if the type enum could be autogenerated.
...
auto Memoize(BlobCache* cache, CacheKey::Type type, Fn fn, const Args&... args) {
...
struct MemoizeImpl<Fn, ResultOrError<R>, Args...> {
    ResultOrError<R> operator()(BlobCache* cache, CacheKey key, Fn fn, Args... args) {
        // Load the result from the cache.
        auto blob = cache->Load(key);
        if (!blob.Empty()) {
            // If we hit the cache, return it immediately.
            auto result = detail::MemoResult<R>(std::move(blob));
        ...
        // Store the result.
        cache->Store(key, result.AsBlob());
        return std::move(result).IntoView();
    }
};

// Specialization for MemoResultTraits where T is a vector of pod elements.
template <typename T>
struct detail::MemoResultTraits<std::vector<T>, std::enable_if_t<std::is_pod_v<T>>> {
    static CachedBlob AsBlob(const std::vector<T>& value) {
        CachedBlob blob(sizeof(T) * value.size());
        memcpy(blob.Data(), value.data(), blob.Size());
        return blob;
    }

    static std::vector<T> View(std::vector<T> value) { return value; }

    static std::vector<T> View(CachedBlob blob) {
        T* start = reinterpret_cast<T*>(blob.Data());
        T* end = start + blob.Size() / sizeof(T);
...
//           return a + b;
//       })
#define DAWN_MEMOIZE(cache, type, ...)                                                            \
