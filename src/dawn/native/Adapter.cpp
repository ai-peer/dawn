...
AdapterBase::AdapterBase(InstanceBase* instance,
    ...
    // TODO: Actually record the reqquired set
    mRequiredTogglesSet(ToggleStage::Adapter) {
...
bool AdapterBase::IsCreatedWithRequiredToggles(
    ...
    ASSERT(requiredAdapterToggles.requiredStage == ToggleStage::Adapter);
    return requiredAdapterToggles == mRequiredTogglesSet;
}
    ...
    if (deprecatedTogglesDeviceDesc) {
        ...
        if (workaroundInstanceToggles.togglesProvided.Has(Toggle::DisallowUnsafeAPIs)) {
            ...
            deviceToggles.Inherit(Toggle::DisallowUnsafeAPIs,
                workaroundInstanceToggles.togglesEnabled.Has(Toggle::DisallowUnsafeAPIs),
                false);
        }
    }
