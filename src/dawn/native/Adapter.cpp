...
void AdapterBase::InitializeSupportedFeatures() {
    // Backend-specific features checking.
...
ResultOrError<Ref<DeviceBase>> AdapterBase::CreateDeviceInternal(
    ...
    // these behaviors works, set the device DisallowUnsafeApis toggle if required in
    // DawnTogglesDeviceDescriptor, even though that may break the coherence between device toggles
    // state and instance/adapter toggles state.
    if (deprecatedTogglesDeviceDesc) {
        RequiredTogglesSet workaroundInstanceToggles =
            RequiredTogglesSet::CreateFromTogglesDescriptor(deviceTogglesDesc,
                                                            ToggleInfo::ToggleStage::Instance);
        if (workaroundInstanceToggles.IsRequired(Toggle::DisallowUnsafeAPIs)) {
            ...
            deviceToggles.ForceSet(Toggle::DisallowUnsafeAPIs,
