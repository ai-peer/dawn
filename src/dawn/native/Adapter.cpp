...
ResultOrError<Ref<DeviceBase>> AdapterBase::CreateDeviceInternal(
    ...
    for (uint32_t i = 0; i < descriptor->requiredFeaturesCount; ++i) {
        ...
        if (f == wgpu::FeatureName::ShaderF16 || f == wgpu::FeatureName::ChromiumExperimentalDp4a) {
            // If the adapter is of D3D12 backend and support ShaderF16 or ChromiumExperimentalDp4a
            // feature, d3d12::Adapter::InitializeSupportedFeaturesImpl ensure that DXC is
            // avaliable, so if use_dxc toggle is provided, the device will use DXC.
            ...
            DAWN_INVALID_IF((f == wgpu::FeatureName::ChromiumExperimentalDp4a) &&
                                !(isToggleDisabledByUser(Toggle::DisallowUnsafeAPIs) &&
                                  nonD3D12BackendOrUseDXC),
