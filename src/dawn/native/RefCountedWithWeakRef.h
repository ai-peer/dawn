...
// RecCountedWithWeakRef is a version of reference counted object which supports weak reference.
...
class RefCountedWithWeakRef {
  public:
    ...
    uint64_t GetRefCountForTesting() const {
        return mWeakRef->StrongReferenceGetRefCountForTesting();
    }
  ...
  private:
    class WeakReferenceWithStrongControl : public WeakReference<Derived> {
      public:
        ...
        uint64_t StrongReferenceGetRefCountForTesting() const {
            return this->mStrongRefCount.GetValueForTesting();
        }
