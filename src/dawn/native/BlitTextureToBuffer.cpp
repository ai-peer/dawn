...
fn textureLoadGeneral(tex: texture_2d<u32>, coords: vec3u, level: u32) -> u32 {
    return textureLoad(tex, coords.xy, level);
}
...
fn textureLoadGeneral(tex: texture_2d_array<u32>, coords: vec3u, level: u32) -> u32 {
    return textureLoad(tex, coords.xy, coords.z, level);
}
...
#if defined(DAWN_ENABLE_BACKEND_OPENGL) || defined(DAWN_ENABLE_BACKEND_OPENGLES)
                wgpu::BackendType backendType = device->GetPhysicalDevice()->GetBackendType();
                if (backendType == wgpu::BackendType::OpenGL ||
                    backendType == wgpu::BackendType::OpenGLES) {
                    // OpenGL/GLES backend has no texture view and require view dimension matches
                    // the underlying texture. Otherwise an intermediate CopyImageSubData is needed.
                    // We don't want to trigger that copy path when possible. So for OpenGL/GLES
                    // backend we need to match shader texture view binding with copy src texture
                    // target specifically.
                    switch (opengl::ToBackend(src.texture)->GetGLTarget()) {
                        case GL_TEXTURE_2D:
