...
// In this format float is repsented as
...
// e.g.
// 0x0a090807 and 0x0412100e both unpacks to
// [8.344650268554688e-7, 0.000015735626220703125, 0.000015497207641601562]
// So the copied bytes via blit could be different.
constexpr std::string_view kPackRGB9E5UfloatToU32 = R"(
    ...
    const sharedexp_max: f32 = (f32((1 << N) - 1) / f32(1 << N)) * (1 << (Emax - B));
