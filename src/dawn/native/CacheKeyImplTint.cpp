...
// static
template <>
void CacheKeySerializer<tint::Program>::Serialize(serde::Sink* sink, const tint::Program& p) {
#if TINT_BUILD_WGSL_WRITER
    tint::writer::wgsl::Options options{};
    CacheKeyRecorder(sink).Record(tint::writer::wgsl::Generate(&p, options).wgsl);
#else
...
void CacheKeySerializer<tint::transform::BindingPoints>::Serialize(
    ...
    static_assert(sizeof(tint::transform::BindingPoints) == 16,
                  "Please update serialization for tint::transform::BindingPoints");
    CacheKeyRecorder(sink).Record(points.plane_1, points.params);
}

// static
template <>
void CacheKeySerializer<tint::sem::BindingPoint>::Serialize(serde::Sink* sink,
    ...
    static_assert(sizeof(tint::sem::BindingPoint) == 8,
                  "Please update serialization for tint::sem::BindingPoint");
    CacheKeyRecorder(sink).Record(p.group, p.binding);
}
