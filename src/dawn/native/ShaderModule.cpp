...
#define DelayedInvalidIf(invalid, ...)                                              \
    ...
    if (metadata->stage == SingleShaderStage::Compute) {
        metadata->usesNumWorkgroups = true;
    }
...
// MaybeError ValidateComputeStageWorkgroupSize(DeviceBase* device, const tint::Program& program,
...
ResultOrError<tint::inspector::WorkgroupSize> ValidateComputeStageWorkgroupSize(
    ...
    tint::inspector::Inspector inspector(&program);
    tint::inspector::WorkgroupSize workgroup_size = inspector.GetWorkgroupSize(entryPointName);
...
const tint::Program* ShaderModuleBase::GetRawTintProgram() const {
    ASSERT(mTintProgram);
