...
const tint::Program* ShaderModuleBase::GetTintProgram() const {
    // const tint::Program* ShaderModuleBase::GetOriginalTintProgram() const {
    ASSERT(mTintProgram);
    return mTintProgram.get();
}

// const tint::Program* ShaderModuleBase::GetTintProgram(const std::string& entryPoint) const {
//     ASSERT(HasEntryPoint(entryPoint));
//     const auto metadata = mEntryPoints.at(entryPoint);
//     if (metadata->transformedProgram != nullptr) {
//         return metadata->transformedProgram.get();
//     }
//     return mTintProgram.get();
// }
