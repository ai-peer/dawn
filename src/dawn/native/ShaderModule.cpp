...
namespace dawn::native {
    ...
    namespace {
        ...
        ResultOrError<std::unique_ptr<EntryPointMetadata>> ReflectEntryPointUsingTint(
            ...
            // Returns the satisfied argument, and if it is false additionally store the formatted
            // error in metadata.infringedLimits. This is to delay the emission of these validation
            // errors until the entry point is used.
            auto CheckLimit = [&metadata](bool satisfied, const char* formatStr,
                                          auto... args) -> bool {
                if (!satisfied) {
                    std::string out;
                    absl::UntypedFormatSpec format(formatStr);
                    if (absl::FormatUntyped(&out, format, {absl::FormatArg(args)...})) {
                        metadata->infringedLimits.push_back(out);
                    } else {
                        metadata->infringedLimits.push_back(absl::StrFormat(
            ...
            if (metadata->stage == SingleShaderStage::Compute) {
                CheckLimit(entryPoint.workgroup_size_x <= limits.v1.maxComputeWorkgroupSizeX &&
                ...
                CheckLimit(numInvocations <= limits.v1.maxComputeInvocationsPerWorkgroup,
                ...
                CheckLimit(workgroupStorageSize <= limits.v1.maxComputeWorkgroupStorageSize,
            ...
            if (metadata->stage == SingleShaderStage::Vertex) {
                for (const auto& inputVar : entryPoint.input_variables) {
                    ...
                    if (CheckLimit(unsanitizedLocation < kMaxVertexAttributes,
                ...
                for (const auto& outputVar : entryPoint.output_variables) {
                    ...
                    if (CheckLimit(location <= kMaxInterStageShaderLocation,
                ...
                CheckLimit(totalInterStageShaderComponents <= kMaxInterStageShaderComponents,
            ...
            if (metadata->stage == SingleShaderStage::Fragment) {
                ...
                for (const auto& outputVar : entryPoint.output_variables) {
                    ...
                    if (CheckLimit(unsanitizedAttachment < kMaxColorAttachments,
