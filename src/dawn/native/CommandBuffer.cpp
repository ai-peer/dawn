...
LeftoverClearsNeeded LazyClearRenderPassAttachments(
    ...
    if (renderPass->attachmentState->HasDepthStencilAttachment()) {
        ...
        // If the depth stencil texture has not been initialized, we want to use loadop clear to init the contents to 0's. That is unless the attachments are read-only and the backend doesn't support lazy initialization for them.
