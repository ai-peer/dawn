...
class TrackedFuture : public RefCounted {
  protected:
    TrackedFuture(InstanceBase* instance, WGPUCallbackModeFlags callbackMode);
...
class WorkDoneFuture final : public TrackedFuture {
  public:
    ...
    WorkDoneFuture(QueueBase* queue, const WGPUQueueWorkDoneCallbackInfo& callbackInfo);
    ~WorkDoneFuture() override = default;

    // Mark the future as having become ready early, and return a new
    // already-completed FutureID (if needed by the callback mode).
    void FailEarly(WGPUQueueWorkDoneStatus status);
  ...
  private:
    ...
    // The early-ready status (used iff TrackedFuture::mState is EarlyReady).
