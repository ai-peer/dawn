...
//   T OnCacheMiss(Request r) { ... }
//   // ...
//   Request r = { ... };
//   auto result = LoadOrRun(device, std::move(r), OnCacheHit, OnCacheMiss);
//
// LoadOrRun generates a CacheKey from the request and loads from the BlobCache. On cache hit,
// calls CacheHitFn and returns a CacheResult<T>. On cache miss or if CacheHitFn returned an Error,
// calls CacheMissFn -> ResultOrError<T> with the request data and returns a
  ...
  public:
    ...
    friend auto LoadOrRun(DeviceBase* device,
        ...
        static_assert(std::is_same_v<typename detail::UnwrapResultOrError<CacheHitReturnType>::type,
                                     UnwrappedReturnType>,
                      "CacheHitFn must return the same type as CacheMissFn.");
