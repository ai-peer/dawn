...
// calls CreateFn -> ResultOrError<T> with the request data and returns a
// ResultOrError<CacheResult<T>>. CacheHitFn must return the same unwrapped type as CreateFn. i.e.
// it doesn't need to be wrapped in ResultOrError.
//
// CreateFn may not have any additional data bound to it. It may not be a lambda or std::function
...
class CacheRequestImpl {
  public:
    ...
    friend auto LoadOrCreate(DeviceBase* device,
        ...
        // Get return types and check that CreateReturnType can be cast to a raw function pointer.
        ...
        using CreateReturnType = decltype(createFn(std::declval<Request>()));
        static_assert(std::is_convertible_v<CreateFn, CreateReturnType (*)(Request)>,
                      "CreateFn function signature does not match, or it is not a free function.");

        static_assert(detail::IsResultOrError<CreateReturnType>::value,
                      "CreateFn should return a ResultOrError.");
        using UnwrappedReturnType = typename detail::UnwrapResultOrError<CreateReturnType>::type;

        static_assert(std::is_same_v<typename detail::UnwrapResultOrError<CacheHitReturnType>::type,
                                     UnwrappedReturnType>,
                      "CacheHitFn must return the same type as CreateFn.");
        ...
        auto result = createFn(std::move(r));
        if (DAWN_LIKELY(result.IsSuccess())) {
