...
MaybeError Queue::Dispatch(Ref<PipelineBase> pipeline,
    ...
    // least one global declaration since there is a valid pipeline. We should just expose the Tint
    // source file from ShaderModuleBase instead.
    const tint::Source::File* source = program->AST().GlobalDeclarations()[0]->source.file;
    DAWN_ASSERT(source != nullptr);
    ...
    if (GetDevice()->IsToggleEnabled(Toggle::Interactive)) {
        debugger = std::make_unique<tint::interp::InteractiveDebugger>(*source, *shader_executor);
    }
