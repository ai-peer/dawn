...
#define DAWN_TRY_CONTEXT(EXPR, ...) \
    ...
    // DAWN_TRY_WITH_CLEANUP is overloaded with 2 version so that users can override the return
    // value of the macro when necessary. This is particularly useful if the function calling the
    // macro may want to return void instead of the error, i.e. in a test where we may just want to
    // assert and fail if the assign cannot go through. In both the cleanup and return
    ...
    //              GetMaybeErrorFunction(), { AddAdditionalErrorInformation(error.get()); }
    //          );
    //
    //     4 Argument Case:
    //          bool FunctionThatReturnsBool() {
    //              DAWN_TRY_WITH_CLEANUP(
    //                  GetMaybeErrorFunction(),
