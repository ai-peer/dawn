...
void OwnedCompilationMessages::AddMessage(const tint::diag::Diagnostic& diagnostic) {
    ...
    if (lineNum && linePos && diagnostic.source.file) {
        ...
        // source file. So to get the offset in bytes of a line we just need to substract it's start
        // pointer with the start of the file's content.
        const char* fileStart = content.data.data();
        const char* lineStart = content.lines[lineNum].data();
        offset = static_cast<uint64_t>(fileStart - lineStart) + linePos - 1;
        ...
        const char* endLineStart = content.lines[endLineNum].data();
        uint64_t endOffset = static_cast<uint64_t>(fileStart - endLineStart) + endLinePos - 1;
