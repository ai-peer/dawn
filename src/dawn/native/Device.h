...
class DeviceBase : public RefCountedWithExternalCount {
  public:
    ...
    void HandleError(std::unique_ptr<ErrorData> error,
                     InternalErrorType allowedErrors = kDefaultAllowedErrors,
    ...
    [[nodiscard]] bool ConsumedError(MaybeError maybeError,
                                     const char* formatStr,
                                     const Args&... args) {
        return ConsumedError(std::move(maybeError), kDefaultAllowedErrors, formatStr, args...);
    }
    ...
    // For Dawn Wire
    BufferBase* APICreateErrorBuffer(const BufferDescriptor* desc, InternalErrorType allowedErrors);
