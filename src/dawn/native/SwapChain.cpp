...
class ErrorSwapChain final : public SwapChainBase {
  ...
  private:
    void APIConfigure(wgpu::TextureFormat format,
        ...
        CONSUME_ERROR(GetDevice(), DAWN_VALIDATION_ERROR("%s is an error swapchain.", this),
                      kDefaultAllowedErrors);
    }

    TextureViewBase* APIGetCurrentTextureView() override {
        CONSUME_ERROR_AND_RETURN(GetDevice(),
                                 DAWN_VALIDATION_ERROR("%s is an error swapchain.", this),
                                 kDefaultAllowedErrors, TextureViewBase::MakeError(GetDevice()));
    }
