...
// RequiredTogglesSet track each toggle with three posible states, i.e. "Not required" (default),
// "Required as enabled", and "Required as disabled". This struct is used to record the
...
// is of one of these states: set to enabled/disabled, force set to enabled/disabled, or left unset.
// It also keep a record of the required toggles set when creating the instance/adapter/device.
class TogglesState {
  public:
    ...
    // Helper function to create a TogglesState according to required toggles set.
    static TogglesState CreateFromRequiredTogglesSet(const RequiredTogglesSet& requiredTogglesSet);

    // Helper function to create a TogglesState with required toggles set and inheriting from
    // TogglesState of earlier stage.
