...
MaybeError BlitDepthToDepth(DeviceBase* device,
    ...
    // Metal Intel drivers incorrectly create a texture view starting at a
    // nonzero array layer. They also don't textureLoad in the shader at a
    // non-zero array index correctly.
    // Workaround this issue by copying the non-zero array
    // slices to a single-layer texture. That texture will be be sampled as the
    // source instead.
    ...
    for (uint32_t z = 0; z < copyExtent.depthOrArrayLayers; ++z) {
        ...
        if (src.origin.z + z == 0u) {
            ...
            // Copy from the original texture source into the intermediate.
            {
                ...
                intermediateSrc.origin = {0, 0, src.origin.z + z};
