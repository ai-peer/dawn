...
MaybeError InstanceBase::ValidateCallbackModeDisallowFuture(WGPUCallbackModeFlags mode) {
    if (mode & WGPUCallbackMode_Future) {
        return DAWN_VALIDATION_ERROR("WGPUCallbackMode_Future is not allowed on this callback");
    }
    return ValidateCallbackModeDisallowFuture(mode);
}

MaybeError InstanceBase::ValidateCallbackModeAllowFuture(WGPUCallbackModeFlags mode) {
    if ((mode & WGPUCallbackMode_Future) && (mode & WGPUCallbackMode_ProcessEvents)) {
        return DAWN_VALIDATION_ERROR(
            "WGPUCallbackMode_Future and WGPUCallbackMode_ProcessEvents are not allowed on the "
            "same callback");
    }
    return {};
}
...
// FIXME
// - need to track id atomically with creating id.
// - need to createID before starting error checking
// - need to have receiver (and do error checking) before trackID
// thus must hold a lock all the way from create id, error checks, track id?
// - but there IS no receiver if there was an error creating it

FutureID InstanceBase::TrackFuture(WGPUCallbackModeFlags mode, TrackedFuture* future) {
    if (!(mode & WGPUCallbackMode_Future)) {
        return 0;
    } else {
        ...
        // mNextFutureID must be incremented inside the lock.
        // Otherwise, there's a gap (here) where WaitAny will incorrectly assume this future is
        // Completed.
        future->SetID(futureID);
...
void InstanceBase::APIProcessEvents() {
    ...
    // FIXME: Handle WGPUCallbackMode_ProcessEvents callbacks.
    // FIXME: And make sure it's thread-safe with itself (but can still be nested).
}
...
ResultOrError<wgpu::WaitStatus> InstanceBase::WaitAnyImpl(size_t count,
    ...
    bool anyEarlyReady = false;
    {
        ...
        for (size_t i = 0; i < count; ++i) {
            ...
            if (it == mTrackedFutures.end()) {
                ...
                futures.push_back(TrackedFutureWaitInfo{it->second->TakeWaitRef(), i, false});
                if (it->second->IsEarlyReady()) {
                    anyEarlyReady = true;
                }
            }
        }
    ...
    // If any were early-ready, complete them and return immediately.
    // We have to do this in case they don't have a working OSEventReceiver.
    if (anyEarlyReady) {
        for (TrackedFutureWaitInfo& future : futures) {
            if (future.future->IsEarlyReady()) {
    ...
    for (size_t sliceStart = 0; sliceStart < count;) {
        ...
        ASSERT(sliceStart < sliceEnd && sliceEnd <= count);
    ...
    // something actually isn't tracked anymore (because it completed elsewhere while waiting.)
    {
        ...
        for (const TrackedFutureWaitInfo& future : futures) {
            if (future.ready) {
                mTrackedFutures.erase(future.future->GetID());
            }
        }
    }
