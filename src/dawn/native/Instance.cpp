...
void InstanceBase::UntrackFuture(FutureID id) {
    printf("%s\n", __FUNCTION__);
    std::lock_guard lock{mTrackedFuturesMutex};
    auto it = mTrackedFutures.find(uint64_t(id));
    ASSERT(it != mTrackedFutures.end());
        ...
        for (size_t i = 0; i < count; ++i) {
            if (extras[i].alreadyCompleted) {
                ...
                if (ready) {
                    ...
                    if (it != mTrackedFutures.end()) {
                        it->second->Complete();  // This also untracks the future.
                    }
                }
