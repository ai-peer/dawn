...
ResultOrError<Ref<AdapterBase>> InstanceBase::RequestAdapterInternal(
    ...
    TogglesState requiredAdapterToggles =
        TogglesState::CreateFromTogglesDescriptor(adapterTogglesDesc, ToggleStage::Adapter);

    // Adapter toggles inheritance and default/forced toggles set up handled in AdapterBase
    // constructor.
    return AcquireRef(
        new AdapterBase(std::move(selectedPhysicalDevice), featureLevel, requiredAdapterToggles));
}
...
std::vector<Ref<AdapterBase>> InstanceBase::GetAdapters(
    const DawnTogglesDescriptor* requiredAdapterTogglesDesc) const {
    TogglesState requiredAdapterToggles =
        TogglesState::CreateFromTogglesDescriptor(requiredAdapterTogglesDesc, ToggleStage::Adapter);
    ...
    for (const auto& physicalDevice : mPhysicalDevices) {
        for (FeatureLevel featureLevel : {FeatureLevel::Compatibility, FeatureLevel::Core}) {
            if (physicalDevice->SupportsFeatureLevel(featureLevel)) {
                // Adapter toggles inheritance and default/forced toggles set up handled in
                // AdapterBase constructor.
                adapters.push_back(AcquireRef(
                    new AdapterBase(physicalDevice, featureLevel, requiredAdapterToggles)));
            }
        }
    }
    return adapters;
