...
bool InstanceBase::DiscoverPhysicalDevices(
    ...
    RequestAdapterOptionsLUID adapterOptionsLUID = {};
        ...
        case wgpu::BackendType::D3D12: {
            if (IDXGIAdapter* dxgiAdapter =
                    static_cast<const d3d::PhysicalDeviceDiscoveryOptions*>(deprecatedOptions)
                        ->dxgiAdapter.Get()) {
                d3d::LUID luid;
                if (ConsumedErrorAndWarnOnce(d3d::GetAdapterLUID(dxgiAdapter), &luid)) {
                    return false;
                }
                adapterOptionsLUID.luid = static_cast<uint64_t>(luid);
