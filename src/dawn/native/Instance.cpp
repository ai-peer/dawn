...
namespace d3d {
struct RequestAdapterOptionsIDXGIAdapter;
}
...
void InstanceBase::APIRequestAdapter(const RequestAdapterOptions* options,
    ...
    for (const auto& adapter : adapters) {
        ...
        adapter->APIGetProperties(&properties);
        DAWN_DEBUG() << properties.name << " " << static_cast<uint32_t>(properties.backendType)
...
void InstanceBase::DiscoverDefaultPhysicalDevices() {
    ...
    for (auto physicalDevice : DiscoverPhysicalDevicesInternal(&defaultOptions)) {
        bool found = false;
...
bool InstanceBase::DiscoverPhysicalDevices(
...
#if defined(DAWN_ENABLE_BACKEND_D3D11) || defined(DAWN_ENABLE_BACKEND_D3D12)
    d3d::RequestAdapterOptionsIDXGIAdapter dxgiAdapterOptions = {};
#endif  // defined(DAWN_ENABLE_BACKEND_D3D11) || defined(DAWN_ENABLE_BACKEND_D3D12)
    ...
    switch (backendTypeOptions.backendType) {
        ...
        case wgpu::BackendType::D3D12:
            dxgiAdapterOptions.dxgiAdapter =
    ...
    for (auto physicalDevice : DiscoverPhysicalDevicesInternal(&adapterOptions)) {
        bool found = false;
        for (const auto& other : mDeprecatedPhysicalDevices) {
            if (other.Get() == physicalDevice.Get()) {
                found = true;
...
std::vector<Ref<AdapterBase>> InstanceBase::EnumerateAdapters(
    ...
    if (options == nullptr) {
        ...
        // Discover the defaults, both compat, and non-compat adapters.
        RequestAdapterOptions defaultOptions = {};
        defaultOptions.compatibilityMode = true;
...
std::vector<Ref<PhysicalDeviceBase>> InstanceBase::DiscoverPhysicalDevicesInternal(
    ...
    const d3d::RequestAdapterOptionsIDXGIAdapter* dxgiAdapterOptions = nullptr;
    ...
    for (wgpu::BackendType b : IterateBitSet(backendsToFind)) {
        ...
        std::vector<Ref<PhysicalDeviceBase>> physicalDevices =
            mBackends[b]->DiscoverPhysicalDevices(options, getGLProcOptions, dxgiAdapterOptions);
