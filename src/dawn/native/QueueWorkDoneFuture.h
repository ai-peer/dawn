...
class QueueWorkDoneFutureBase : public FutureBase {
  public:
    QueueWorkDoneFutureBase(QueueBase*, ExecutionSerial, bool requestedFd);
    ~QueueWorkDoneFutureBase() override = default;

    ObjectType GetType() const override;
    void DestroyImpl() override;

    std::optional<std::pair<QueueBase*, ExecutionSerial>> GetQueueSerial() override;
    PosixFd GetFd() const override;

    // Dawn API
    virtual void APIGetResult(QueueWorkDoneResult*) const = 0;
    void APIThen(wgpu::CallbackMode, WGPUQueueWorkDoneFutureCallback, void* userdata);

  protected:
    Ref<QueueBase> mQueue;
    ExecutionSerial mSerial;
    const bool mRequestedFd = false;
