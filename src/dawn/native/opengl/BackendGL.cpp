...
EGLContext CreateEGLContext(PFNEGLGETPROCADDRESSPROC getProc) {
    EGLFunctions egl(getProc);
...
std::vector<Ref<AdapterBase>> Backend::DiscoverDefaultAdapters() {
    ...
    if (GetType() == wgpu::BackendType::OpenGLES) {
        ...
        EGLContext context =
            CreateEGLContext(reinterpret_cast<PFNEGLGETPROCADDRESSPROC>(options.getProc));
        if (!context) {
            return {};
        }
