...
std::vector<Ref<AdapterBase>> Backend::DiscoverDefaultAdapters() {
    ...
    if (GetType() == wgpu::BackendType::OpenGLES) {
        ...
        std::unique_ptr<Device::Context> context = ContextEGL::Create(egl);
    ...
    if (GetType() == wgpu::BackendType::OpenGL) {
        const char* glXLib = "libGLX.so";
        if (!mLibGLX.Valid() && !mLibGLX.Open(glXLib)) {
            return {};
        }

        AdapterDiscoveryOptions options;
        options.getProc =
            reinterpret_cast<void* (*)(const char*)>(mLibGLX.GetProc("glXGetProcAddress"));
        if (!options.getProc) {
            return {};
        }
        glX.Init(reinterpret_cast<PFNGLXGETPROCADDRESSPROC>(options.getProc));
        std::unique_ptr<Device::Context> context = ContextGLX::Create(glX);
        if (!context) {
            return {};
        }
