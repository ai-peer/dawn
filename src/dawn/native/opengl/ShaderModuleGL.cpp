...
ResultOrError<GLuint> ShaderModule::CompileShader(
    ...
    for (BindGroupIndex group : IterateBitSet(layout->GetBindGroupLayoutsMask())) {
        ...
        for (const auto& [_, expansion] : bgl->GetExternalTextureBindingExpansionMap()) {
            ...
            BindingPoint plane0(static_cast<uint32_t>(group),
                                static_cast<uint32_t>(expansion.plane0));
            BindingPoint plane1(static_cast<uint32_t>(group),
                                static_cast<uint32_t>(expansion.plane1));
            BindingPoint params(static_cast<uint32_t>(group),
                                static_cast<uint32_t>(expansion.params));
            glBindings.emplace(plane0, BindingPoint{0u, plane1Slot});
