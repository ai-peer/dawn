...
void StreamIn(stream::Sink* s, const BindingLocation& bindingLocation) {
    return bindingLocation.VisitAll([&](auto&... members) { return StreamIn(s, members...); });
}

MaybeError StreamOut(stream::Source* s, BindingLocation* bindingLocation) {
    return bindingLocation->VisitAll([&](auto&... members) { return StreamOut(s, &members...); });
}

void StreamIn(stream::Sink* s, const CombinedSampler& combinedSampler) {
    return combinedSampler.VisitAll([&](auto&... members) { return StreamIn(s, members...); });
}

MaybeError StreamOut(stream::Source* s, CombinedSampler* combinedSampler) {
    ...
    DAWN_TRY_LOAD_OR_RUN(
        ...
        [](GLSLCompilationRequest r) -> ResultOrError<GLSLCompilation> {
            ...
            tintOptions.version = tint::writer::glsl::Version(ToTintGLStandard(r.glVersionStandard),
                                                              r.glVersionMajor, r.glVersionMajor);
