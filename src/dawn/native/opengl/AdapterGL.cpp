...
MaybeError Adapter::InitializeGLFunctions(void* (*getProc)(const char*)) {
    ...
    if (GetBackendType() == wgpu::BackendType::OpenGLES) {
        mEGLFunctions.Init(reinterpret_cast<PFNEGLGETPROCADDRESSPROC>(getProc));
    } else if (GetBackendType() == wgpu::BackendType::OpenGL) {
        mGLXFunctions.Init(reinterpret_cast<PFNGLXGETPROCADDRESSPROC>(getProc));
    }
...
ResultOrError<Ref<DeviceBase>> Adapter::CreateDeviceImpl(const DeviceDescriptor* descriptor) {
    ...
    if (GetBackendType() == wgpu::BackendType::OpenGLES) {
        context = ContextEGL::Create(mEGLFunctions);
    } else if (GetBackendType() == wgpu::BackendType::OpenGL) {
        context = ContextGLX::Create(mGLXFunctions);
    }
