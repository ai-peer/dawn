...
namespace dawn::native::opengl {
    ...
    void TextureView::CopyIfNeeded() {
        ...
        uint32_t srcLayer = GetBaseArrayLayer();
        uint32_t numLayers = GetLayerCount();
        ...
        if (mHandle == 0) {
            ...
            AllocateTexture(gl, mTarget, texture->GetSampleCount(), numLevels,
                            glFormat.internalFormat, width, height, numLayers);
            mOwnsHandle = true;
        }

        Origin3D src{0, 0, srcLayer};
        Origin3D dst{0, 0, 0};
        Extent3D size{width, height, numLayers};
