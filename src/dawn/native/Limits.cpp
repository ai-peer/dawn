...
#define X_CHECK_BETTER_AND_CLAMP(Class, limitName, ...)                                   \
    ...
    // After tiering all limit values, enforce additional restriction.
#define X_ENSURE_NO_BETTER_THAN_RESTRICT_LIMIT(Class, limitName, ...)                              \
    {                                                                                              \
        if (CheckLimit<LimitClass::Class>::IsBetter(limits.limitName, limits.restrictLimitName)) { \
            constexpr decltype(Limits::limitName) kTiers[] = {__VA_ARGS__};                        \
            constexpr uint32_t kTierCount = sizeof(kTiers) / sizeof(decltype(Limits::limitName));  \
            for (uint32_t i = kTierCount; i != 0; --i) {                                           \
                /* Find the best tier that is no better than limitName or restrictLimitName. */    \
                if (CheckLimit<LimitClass::Class>::IsBetter(kTiers[i - 1], limits.limitName) ||    \
                    CheckLimit<LimitClass::Class>::IsBetter(kTiers[i - 1],                         \
                                                            limits.restrictLimitName)) {           \
                    /* The tier is better. Go to the next tier. */                                 \
                    continue;                                                                      \
                }                                                                                  \
                limits.limitName = kTiers[i - 1];                                                  \
                break;                                                                             \
            }                                                                                      \
            /* If all tiers are not suitable, just clamp the limitName to restrictLimitName. */    \
            /* This should not happen as long as both limits are clamped to a valid tier,    */    \
            /* and the worst tier of limitName is not better than that of restrictLimitName. */    \
            if (CheckLimit<LimitClass::Class>::IsBetter(limits.limitName,                          \
                                                        limits.restrictLimitName)) {               \
                limits.limitName = limits.restrictLimitName;                                       \
            }                                                                                      \
        }                                                                                          \
    }

    // Ensure that tiered maxStorageBufferBindingSize and maxUniformBufferBindingSize is not better
    // than maxBufferSize.
#define restrictLimitName maxBufferSize
    LIMITS_STORAGE_BUFFER_BINDING_SIZE(X_ENSURE_NO_BETTER_THAN_RESTRICT_LIMIT)
    X_ENSURE_NO_BETTER_THAN_RESTRICT_LIMIT(Maximum, maxUniformBufferBindingSize, 65536)
#undef restrictLimitName
