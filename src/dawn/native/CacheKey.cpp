...
CacheKey::CacheKey(std::initializer_list<uint8_t> init) : std::vector<uint8_t>(init), CacheKeyRecorder(this) {}
CacheKey::CacheKey(size_t count, uint8_t value) : std::vector<uint8_t>(count, value), CacheKeyRecorder(this) {}
...
template <>
void CacheKeySerializer<CacheKeyRecorder>::Serialize(serde::Sink* sink, const CacheKeyRecorder& t) {
    CacheKeySerializer<CacheKey>::Serialize(sink, static_cast<const CacheKey&>(t));
}
