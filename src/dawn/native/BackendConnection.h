...
class BackendConnection {
  public:
    ...
    // options (such as debug adapters, custom driver libraries, user-provided toggles, etc.)
    ...
    TogglesState GenerateInstanceInheritedAdapterToggles(
        const TogglesState& requiredAdapterToggle) const;

    virtual TogglesState MakeAdapterToggles(const TogglesState& requiredAdapterToggle) const = 0;
