...
MaybeError ValidateDepthStencilState(const DeviceBase* device,
    ...
    DAWN_INVALID_IF(
        ...
        "Depth stencil format (%s) must provide depthCompare.", descriptor->format);

    DAWN_INVALID_IF(format->HasDepth() && (depthWriteDefined == nullptr),
                    "Depth stencil format (%s) must provide depthWriteEnabled.",
                    descriptor->format);

    DAWN_INVALID_IF(
        !format->HasDepth() && (descriptor->depthCompare != wgpu::CompareFunction::Always ||
                                depthWriteDefined->depthWriteEnabled),
