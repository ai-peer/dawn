...
MaybeError CheckHRESULTImpl(platform::Platform* platform, HRESULT result, const char* context);

// Uses CheckRESULT but returns OOM specific error when recoverable.
MaybeError CheckOutOfMemoryHRESULTImpl(platform::Platform* platform,
                                       HRESULT result,
                                       const char* context);

#define CheckHRESULT(platform, resultIn, contextIn)                                             \
    ::dawn::native::d3d::CheckHRESULTImpl(                                                      \
        platform, INJECT_ERROR_OR_RUN(resultIn, ::dawn::native::d3d::E_FAKE_ERROR_FOR_TESTING), \
        contextIn)

#define CheckOutOfMemoryHRESULT(platform, resultIn, contextIn)                                   \
