...
MaybeError TranslateToHLSL(d3d::HlslCompilationRequest r,
    ...
    auto result = [&, p = platform]() {
        SCOPED_DAWN_CACHE_MISS_TIMER(p, "D3D.CompileShader.TintHLSLGenerate");
...
ResultOrError<CompiledShader> CompileShader(d3d::D3DCompilationRequest r) {
    ...
    DAWN_TRY(TranslateToHLSL(std::move(r.hlsl), platform, &remappedEntryPoint, &compiledShader));
