...
uint32_t GetColumnPitch(uint32_t baseHeight, uint32_t mipLevelCount) {
    ...
    // example, if the single layer's dimension is 4x16 and it has full mipmaps, then there are 5
...
uint32_t ComputeExtraArraySizeForIntelGen12(uint32_t width,
    ...
    constexpr uint32_t kTileSize = 64 * 1024;
    ...
    // Texture should be aligned on both width (512 bytes) and height (128 rows) on Intel Gen12 GPU
    const uint32_t tileHeight = 128;
    const uint32_t tileWidth = kTileSize / tileHeight;
    ...
    if (actualAuxSize < expectedAuxSize) {
        float actualMainSizePerLayer = actualMainSize / arrayLayerCount;
        return ceil((expectedMainSize - actualMainSize) / actualMainSizePerLayer);
    }
    return 0;
}
