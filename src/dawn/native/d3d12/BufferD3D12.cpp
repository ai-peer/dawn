...
MaybeError Buffer::InitializeHostMapped(const BufferHostMappedPointer* hostMappedDesc) {
    ...
    DAWN_TRY(CheckHRESULT(
        ToBackend(GetDevice())->GetD3D12Device()->QueryInterface(IID_PPV_ARGS(&d3d12Device3)),
    ...
    auto heap = std::make_unique<Heap>(std::move(d3d12Heap), MemorySegment::NonLocal, GetSize());

    // TODO: Are these heaps created as already resident?
    // ToBackend(GetDevice())->GetResidencyManager()->TrackResidentAllocation(heap.get());
    ...
    DAWN_TRY(ToBackend(GetDevice())->GetResidencyManager()->LockAllocation(heap.get()));

    ComPtr<ID3D12Resource> placedResource;
    DAWN_TRY(CheckOutOfMemoryHRESULT(
        ToBackend(GetDevice())
            ->GetD3D12Device()
            ->CreatePlacedResource(heap->GetD3D12Heap(), 0, &resourceDescriptor,
    ...
    ToBackend(GetDevice())->GetResidencyManager()->UnlockAllocation(heap.get());

    mResourceAllocation = {AllocationInfo{0, AllocationMethod::kExternal}, 0,
                           std::move(placedResource), heap.get()};
