...
ResultOrError<std::wstring> ConvertStringToWstring(const std::string& s) {
    size_t len = s.length();
    if (len == 0) {
        return std::wstring();
    }
    int numChars = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, s.c_str(), len, nullptr, 0);
    if (numChars == 0) {
    ...
    int numConvertedChars =
        MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, s.c_str(), len, &result[0], numChars);
