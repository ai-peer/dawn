...
ResultOrError<Ref<AdapterBase>> CreateAdapterFromIDXGIAdapter(Backend* backend,
    ...
    Ref<Adapter> adapter = AcquireRef(new Adapter(backend, std::move(dxgiAdapter3), adapterToggles));
...
ResultOrError<std::vector<Ref<AdapterBase>>> Backend::DiscoverAdapters(
    ...
    if (options->dxgiAdapter != nullptr) {
        ...
        DAWN_TRY_ASSIGN(adapter, CreateAdapterFromIDXGIAdapter(this, options->dxgiAdapter, adapterToggles));
    ...
    for (uint32_t adapterIndex = 0;; ++adapterIndex) {
        ...
        ResultOrError<Ref<AdapterBase>> adapter = CreateAdapterFromIDXGIAdapter(this, dxgiAdapter, adapterToggles);
        if (adapter.IsError()) {
...
TogglesState Backend::MakeAdapterToggles(const TogglesState& requiredAdapterToggle) const {
    ...
    // Check DXC for use_dxc toggle
    if (!IsDXCAvailable()) {
        adapterToggleStates.ForceSet(Toggle::UseDXC, false);
    }
