    ...
    if (!mFunctions->IsDXCBinaryAvailable()) {
        ...
        if (std::holds_alternative<DxcVersionInfo>(mDxcVersionInfo)) {
            ...
            if (dxcVersionInfo.DxcCompilerVersion >=
                ...
                // DXC is available only if DXC binaries available, and DXC version not lower than
                // pre-defined minimum.
...
void Backend::AcquireDxcVersionInformation() {
    // This method should bu called only once.
    ...
    if (dxcVersionInfoOrError.IsSuccess()) {
        ...
        mDxcVersionInfo = DxcVersionUnavailable{errorMessage};
    }
}
...
bool Backend::IsDXCAvailableAndVersionAtLeast(uint64_t minimumCompilerMajorVersion,
    ...
    // Check that mIsDXCAvailable is true, i.e. DXC binaries are avaliable, and the DXC version
    // is validated to be no older than a pre-defined minimum version.
    if (mIsDXCAvailable) {
        // The DXC version information should be validated to be valid when setting mIsDXCAvailable.
        DAWN_ASSERT(std::holds_alternative<DxcVersionInfo>(mDxcVersionInfo));
