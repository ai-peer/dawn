...
#include "third_party/IntelExtension/igdext.h"
...
bool FillDeviceID(DeviceInfo* deviceInfo, const Adapter& adapter) {
...
class IntelExtensionImpl : public IntelExtension {
  public:
    explicit IntelExtensionImpl(const Adapter& adapter);
  ...
  private:
    bool LoadInterfaces();
    bool CreateINTCExtensionContext(const Adapter& adapter);
...
IntelExtensionImpl::IntelExtensionImpl(const Adapter& adapter) {
    ASSERT(gpu_info::IsIntel(adapter.GetVendorId()));

    DeviceInfo deviceInfo;

    // Get the display device instance ID
    if (!FillDeviceID(&deviceInfo, adapter)) {
        return;
    }
    ...
    if (!CreateINTCExtensionContext(adapter)) {
        CleanUp();
    }
}
...
bool IntelExtensionImpl::CreateINTCExtensionContext(const Adapter& adapter) {
    ASSERT(mIntelExtensionInterfaces.GetSupportedVersions != nullptr);

    ID3D12Device* d3d12Device = adapter.GetDevice().Get();
...
std::unique_ptr<IntelExtension> IntelExtension::Create(const Adapter& adapter) {
    std::unique_ptr<IntelExtensionImpl> extension = std::make_unique<IntelExtensionImpl>(adapter);
