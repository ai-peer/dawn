...
void Texture::DestroyImpl() {
    ...
    // EndAccess() should be called before Destroy() - an ASSERT is fine since it doesn't crash
    // in release builds.
    ASSERT(!mSignalFenceValue.has_value());
}

std::optional<ExecutionSerial> Texture::EndAccess() {
    // Need to explicitly call reset() since std::move() on optional doesn't make it std::nullopt.
