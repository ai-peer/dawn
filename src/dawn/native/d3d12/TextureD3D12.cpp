...
ResultOrError<ExecutionSerial> Texture::EndAccess() {
    ...
    if (!mSignalFenceValue.has_value() && !mD3D11on12Resource) {
    ...
    // Use value_or() to gracefully return when fence synchronization is not being used.
    ExecutionSerial ret = mSignalFenceValue.value_or(ExecutionSerial());
