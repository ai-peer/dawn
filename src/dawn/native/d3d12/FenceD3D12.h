...
class Fence : public RefCounted {
  public:
    static Ref<Fence> Create(Device* device);
    static Ref<Fence> CreateFromHandle(Device* device,
                                       HANDLE unownedHandle,
                                       ExecutionSerial fenceValue);

    MaybeError Wait();

    ResultOrError<ExecutionSerial> IncrementAndSignal();
