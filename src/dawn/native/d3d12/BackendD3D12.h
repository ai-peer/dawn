...
// version, DxcVersionUnavailable indicates the version information being unavailable and holds the
// detailed error information.
struct DxcVersionUnavailable {
...
class Backend : public BackendConnection {
  public:
    ...
    // Return mIsDXCAvailable, which is set in `Initialize` and begin true if and only if DXC binary
    // is avaliable, and the DXC compiler and validator version are validated to be no older than a
    // specific minimium version, currently 1.6.
  ...
  private:
    ...
    // True if and only if DXC binary is avaliable, and the DXC compiler and validator version are
    // validated to be no older than a specific minimium version, currently 1.6. Set in Initialize
    // when starting up.
    bool mIsDXCAvailable;
    ...
    //      and therefore no DXC version information available, represented by DxcVersionUnavailable
    //      in which a error message is held;
    //   3. The DXC version information is acquired successfully, stored in DxcVersionInfo.
    // This cache is updated by AcquireDxcVersionInformation.
    std::variant<DxcVersionNotAcquired, DxcVersionUnavailable, DxcVersionInfo> mDxcVersionInfo;
};
