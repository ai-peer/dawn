    ...
    if (r.stage == SingleShaderStage::Compute) {
        ...
        DAWN_TRY_ASSIGN(
            _, ValidateComputeStageWorkgroupSize(
                   transformedProgram, remappedEntryPointName->data(),
                   r.limits.maxComputeWorkgroupSizeX, r.limits.maxComputeWorkgroupSizeY,
                   r.limits.maxComputeWorkgroupSizeZ, r.limits.maxComputeInvocationsPerWorkgroup,
...
ResultOrError<CompiledShader> ShaderModule::Compile(const ProgrammableStage& programmableStage,
    ...
    req.hlsl.limits = stream::LimitsForCompilationRequest::Create(limits.v1);
