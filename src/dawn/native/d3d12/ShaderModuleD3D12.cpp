...
ResultOrError<std::string> TranslateToHLSL(D3DCompilationRequest* req,
                                           std::string* remappedEntryPointName,
                                           bool* usesVertexOrInstanceIndex) {
    ASSERT(req);
    D3DCompilationRequest& r = *req;
...
ResultOrError<CompiledShader> ShaderModule::Compile(const ProgrammableStage& programmableStage,
    ...
    req.defineStrings = GetOverridableConstantsDefines(programmableStage.constants,
...
std::variant<std::pair<const void*, size_t>, Blob> CompiledShader::GetDataForCache() const {
    Blob blob = CreateBlob(mShaderBlob.Size() + sizeof(mUsesVertexOrInstanceIndex));
    uint8_t* ptr = blob.Data();
    memcpy(ptr, mShaderBlob.Data(), mShaderBlob.Size());
    memcpy(ptr + mShaderBlob.Size(), &mUsesVertexOrInstanceIndex,
           sizeof(mUsesVertexOrInstanceIndex));
    return blob;
}

// static
ResultOrError<CompiledShader> CompiledShader::FromBlob(Blob blob) {
    CompiledShader compiledShader;
    size_t blobSize = blob.Size();
    DAWN_INVALID_IF(blobSize < sizeof(mUsesVertexOrInstanceIndex),
                    "Failed to deserialize from blob.");
    size_t offset = blobSize - sizeof(mUsesVertexOrInstanceIndex);

    memcpy(&compiledShader.mUsesVertexOrInstanceIndex, blob.Data() + offset,
           sizeof(mUsesVertexOrInstanceIndex));

    compiledShader.mShaderBlob = Blob::Slice(std::move(blob), 0, offset);
    return compiledShader;
}
