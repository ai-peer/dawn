...
void DumpDXCCompiledShader(Device* device,
    ...
    if (!shaderBlob.Empty()) {
        ...
        HRESULT hr = device->GetDxcCompiler()->Disassemble(&dxcBuffer, IID_PPV_ARGS(&dxcResult));

        ComPtr<IDxcBlobEncoding> disassembly;
        if (SUCCEEDED(hr) && dxcResult->HasOutput(DXC_OUT_DISASSEMBLY) &&
            SUCCEEDED(
            ...
            if (SUCCEEDED(hr) && dxcResult->HasOutput(DXC_OUT_ERRORS) &&
