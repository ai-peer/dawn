...
// Define a list of members that will only be public on a helper struct
// `BytecodeCompilationRequest` which aliases `D3DCompilationRequest`.
// Another helper struct `HlslCompilationRequest` will make all these private
// so that the structs expose disjoint sets of information.
// To avoid copies, `D3DCompilationRequest` is passed mutably to helper functions
// which consume members. Making disjoint aliases helps ensure a member is not
// consumed twice.
#define BYTECODE_COMPILATION_REQUEST_INCLUDE_LIST  \
    using D3DCompilationRequest::compileFlags;     \
    using D3DCompilationRequest::compiler;         \
    using D3DCompilationRequest::fxcShaderProfile; \
    using D3DCompilationRequest::dxcShaderProfile; \
    using D3DCompilationRequest::d3dCompile;       \
    using D3DCompilationRequest::dxcLibrary;       \
    using D3DCompilationRequest::dxcCompiler;      \
    using D3DCompilationRequest::defineStrings;

struct BytecodeCompilationRequest : private D3DCompilationRequest {
    BYTECODE_COMPILATION_REQUEST_INCLUDE_LIST
}

struct HlslCompilationRequest
    : public D3DCompilationRequest{private : BYTECODE_COMPILATION_REQUEST_INCLUDE_LIST}
#undef BYTECODE_COMPILATION_REQUEST_INCLUDE_LIST

      std::vector<const wchar_t*>
      GetDXCArguments(uint32_t compileFlags, bool enable16BitTypes) {
...
ResultOrError<ComPtr<IDxcBlob>>
CompileShaderDXC(const BytecodeCompilationRequest& r,
...
ResultOrError<ComPtr<ID3DBlob>> CompileShaderFXC(const BytecodeCompilationRequest& r,
                                                 const std::string& entryPointName,
                                                 const std::string& hlslSource) {
...
ResultOrError<std::string> TranslateToHLSL(HlslCompilationRequest* req,
                                           std::string* remappedEntryPointName,
                                           bool* usesVertexOrInstanceIndex) {
    ASSERT(req);
    HlslCompilationRequest& r = *req;
...
ResultOrError<CompiledShader> CompileShader(D3DCompilationRequest r) {
    // Cast `r` to disjoint aliases so that it's clear TranslateToHLSL
    // and CompileShaderFXC/CompileShaderDXC operate on different inputs.
    HlslCompilationRequest& hlslRequest = r;
    const BytecodeCompilationRequest& bytecodeRequest = r;

    CompiledShader compiledShader;
    // Compile the source shader to HLSL.
    std::string remappedEntryPoint;
    DAWN_TRY_ASSIGN(compiledShader.hlslSource,
                    TranslateToHLSL(&hlslRequest, &remappedEntryPoint,
    ...
    switch (r.compiler) {
        case Compiler::DXC: {
            ...
            DAWN_TRY_ASSIGN(compiledDXCShader, CompileShaderDXC(bytecodeRequest, remappedEntryPoint,
        ...
        case Compiler::FXC: {
            ...
            DAWN_TRY_ASSIGN(compiledFXCShader, CompileShaderFXC(bytecodeRequest, remappedEntryPoint,
                                                                compiledShader.hlslSource));
...
ResultOrError<CompiledShader> ShaderModule::Compile(const ProgrammableStage& programmableStage,
    ...
    req.dumpShaders = device->IsToggleEnabled(Toggle::DumpShaders);
    ...
    req.remappedBindingPoints = std::move(remappedBindingPoints);
    req.remappedAccessControls = std::move(remappedAccessControls);
    req.newBindingsMap = BuildExternalTextureTransformBindings(layout);
