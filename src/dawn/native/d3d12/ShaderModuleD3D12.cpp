...
ResultOrError<ComPtr<IDxcBlob>> CompileShaderDXC(const D3DCompilationRequest& r,
...
ResultOrError<ComPtr<ID3DBlob>> CompileShaderFXC(const D3DCompilationRequest& r,
                                                 const std::string& entryPointName,
                                                 const std::string& hlslSource) {
...
ResultOrError<std::string> TranslateToHLSL(const D3DCompilationRequest& r,
    ...
    TRACE_EVENT0(r.tracePlatform.UnsafeGetValue(), General, "tint::writer::hlsl::Generate");
...
ResultOrError<CompiledShader> CompileShader(D3DCompilationRequest r) {
    ...
    DAWN_TRY_ASSIGN(hlslSource,
                    TranslateToHLSL(r, &remappedEntryPoint, &usesVertexOrInstanceIndex));

    switch (r.compiler) {
        case Compiler::DXC: {
            TRACE_EVENT0(r.tracePlatform.UnsafeGetValue(), General, "CompileShaderDXC");
            ComPtr<IDxcBlob> compiledDXCShader;
            DAWN_TRY_ASSIGN(compiledDXCShader, CompileShaderDXC(r, remappedEntryPoint, hlslSource));
            return CompiledShader::Create(std::move(compiledDXCShader), usesVertexOrInstanceIndex,
                                          std::move(hlslSource));
        }
        case Compiler::FXC: {
            TRACE_EVENT0(r.tracePlatform.UnsafeGetValue(), General, "CompileShaderFXC");
            ComPtr<ID3DBlob> compiledFXCShader;
            DAWN_TRY_ASSIGN(compiledFXCShader, CompileShaderFXC(r, remappedEntryPoint, hlslSource));
            return CompiledShader::Create(std::move(compiledFXCShader), usesVertexOrInstanceIndex,
                                          std::move(hlslSource));
        }
    }
}
...
ResultOrError<CompiledShader> ShaderModule::Compile(const ProgrammableStage& programmableStage,
    ...
    req.disableWorkgroupInit = device->IsToggleEnabled(Toggle::DisableWorkgroupInit);
    req.hasShaderFloat16Feature = device->IsFeatureEnabled(Feature::ShaderFloat16);
    req.tracePlatform = UnsafeUnkeyedValue(device->GetPlatform());

    req.shaderModel = device->GetDeviceInfo().shaderModel;
    req.compileFlags = compileFlags;
    if (device->IsToggleEnabled(Toggle::UseDXC)) {
        req.compiler = Compiler::DXC;
        req.dxcLibrary = device->GetDxcLibrary().Get();
        req.dxcCompiler = device->GetDxcCompiler().Get();
        DAWN_TRY_ASSIGN(req.compilerVersion,
                        ToBackend(device->GetAdapter())->GetBackend()->GetDXCompilerVersion());
        req.dxcShaderProfile = device->GetDeviceInfo().shaderProfiles[stage];
    } else {
        req.compiler = Compiler::FXC;
        req.d3dCompile = device->GetFunctions()->d3dCompile;
        req.compilerVersion = D3D_COMPILER_VERSION;
        switch (stage) {
            case SingleShaderStage::Vertex:
                req.fxcShaderProfile = "vs_5_1";
                break;
            case SingleShaderStage::Fragment:
                req.fxcShaderProfile = "ps_5_1";
                break;
            case SingleShaderStage::Compute:
                req.fxcShaderProfile = "cs_5_1";
                break;
        }
    }
    ...
    req.stage = stage;
    req.firstIndexOffsetShaderRegister = layout->GetFirstIndexOffsetShaderRegister();
    req.firstIndexOffsetRegisterSpace = layout->GetFirstIndexOffsetRegisterSpace();
    req.usesNumWorkgroups = entryPoint.usesNumWorkgroups;
    req.numWorkgroupsShaderRegister = layout->GetNumWorkgroupsShaderRegister();
    req.numWorkgroupsRegisterSpace = layout->GetNumWorkgroupsRegisterSpace();

    GetOverridableConstantsDefines(&req.defineStrings, programmableStage.constants,
                                   entryPoint.overridableConstants);

    req.newBindingsMap = BuildExternalTextureTransform(layout);
    req.arrayLengthFromUniform.ubo_binding = {
    ...
    for (BindGroupIndex group : IterateBitSet(layout->GetBindGroupLayoutsMask())) {
        ...
        for (const auto& [binding, bindingInfo] : groupBindingInfo) {
            ...
            if (srcBindingPoint != dstBindingPoint) {
                req.remappedBindingPoints.emplace(srcBindingPoint, dstBindingPoint);
            }
            ...
            if (forceStorageBufferAsUAV) {
                req.remappedAccessControls.emplace(srcBindingPoint, tint::ast::Access::kReadWrite);
            }
        }
        ...
        // Add arrayLengthFromUniform options
        {
            for (const auto& bindingAndRegisterOffset :
                ...
                auto it = req.remappedBindingPoints.find(bindingPoint);
                if (it != req.remappedBindingPoints.end()) {
                    bindingPoint = it->second;
                }

                req.arrayLengthFromUniform.bindpoint_to_size_index.emplace(bindingPoint,
                                                                           registerOffset);
            }
        }
    }
    ...
    if (device->IsToggleEnabled(Toggle::DumpShaders)) {
        ...
        if (!compiledShader.IsCached()) {
            dumpedMsg << "/* Dumped generated HLSL */" << std::endl
                      << compiledShader->GetHLSLSourceForLogging() << std::endl;
    ...
    return compiledShader.Acquire();
}

bool CompiledShader::UsesVertexOrInstanceIndex() const {
    return mUsesVertexOrInstanceIndex;
}

// static
CompiledShader CompiledShader::FromBlob(Blob blob) {
    CompiledShader compiledShader;

    ASSERT(blob.Size() >= sizeof(mUsesVertexOrInstanceIndex));
    memcpy(&compiledShader.mUsesVertexOrInstanceIndex, blob.Data(),
           sizeof(mUsesVertexOrInstanceIndex));
    compiledShader.mCachedBlob = std::move(blob);

    return compiledShader;
}

// static
CompiledShader CompiledShader::Create(ComPtr<ID3DBlob> shaderBlob,
                                      bool usesVertexOrInstanceIndex,
                                      std::string hlslSource) {
    CompiledShader compiledShader;
    compiledShader.mShaderBlob = CreateBlob(std::move(shaderBlob));
    compiledShader.mUsesVertexOrInstanceIndex = usesVertexOrInstanceIndex;
    compiledShader.mHlslSource = std::move(hlslSource);
    return compiledShader;
}

// static
CompiledShader CompiledShader::Create(ComPtr<IDxcBlob> shaderBlob,
                                      bool usesVertexOrInstanceIndex,
                                      std::string hlslSource) {
    CompiledShader compiledShader;
    compiledShader.mShaderBlob = CreateBlob(std::move(shaderBlob));
...
D3D12_SHADER_BYTECODE CompiledShader::GetD3D12ShaderBytecode() const {
    if (mCachedBlob.Empty()) {
        ASSERT(!mShaderBlob.Empty());
        return {mShaderBlob.Data(), mShaderBlob.Size()};
    } else {
        ASSERT(mCachedBlob.Size() >= sizeof(mUsesVertexOrInstanceIndex));
        return {mCachedBlob.Data(), mCachedBlob.Size() - sizeof(mUsesVertexOrInstanceIndex)};
    }
    UNREACHABLE();
}

const std::string& CompiledShader::GetHLSLSourceForLogging() const {
    return mHlslSource;
}
...
// static
template <>
void BlobCache::Store<d3d12::CompiledShader>(const CacheKey& key,
                                             const d3d12::CompiledShader& shader) {
    D3D12_SHADER_BYTECODE code = shader.GetD3D12ShaderBytecode();
    bool usesVertexOrInstanceIndex = shader.UsesVertexOrInstanceIndex();

    Blob blob = CreateBlob(code.BytecodeLength + sizeof(usesVertexOrInstanceIndex));
    memcpy(blob.Data(), code.pShaderBytecode, code.BytecodeLength);
    memcpy(blob.Data() + code.BytecodeLength, &usesVertexOrInstanceIndex,
           sizeof(usesVertexOrInstanceIndex));
    Store(key, std::move(blob));
}

// Define no-op serializations for IDxcLibrary and IDxcCompiler which are output-only interfaces
// used to generate bytecode.
template <>
void CacheKeySerializer<IDxcLibrary*>::Serialize(CacheKey*, IDxcLibrary*) {}

template <>
void CacheKeySerializer<IDxcCompiler*>::Serialize(CacheKey*, IDxcCompiler*) {}
