...
std::unique_ptr<ExternalImageDXGIImpl> Device::CreateExternalImageDXGIImpl(
    ...
    mExternalImageResourcesList.Append(impl.get());
    return impl;
}
...
void Device::DestroyImpl() {
    ...
    LinkNode<ExternalImageDXGIImpl>* node = mExternalImageResourcesList.head();
    while (node != mExternalImageResourcesList.end()) {
        ExternalImageDXGIImpl* resources = node->value();
        // ExternalImageDXGIImpl::Destroy() calls RemoveFromList() which invalidates |node|.
        node = node->next();
        resources->Destroy();
    }
