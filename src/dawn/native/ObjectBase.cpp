...
void ApiObjectList::Destroy() {
    ...
    // No new objects will be added here since `mMarkedDestroyed` has been set to true.
    ...
    while (!objects.empty()) {
        ...
        ASSERT(removed);
        // Set `mAlive` to false and destroy the object.
        head->value()->mAlive.store(false, std::memory_order_release);
        head->value()->DestroyImpl();

        if (objects.empty()) {
            // Move the objects again in case more were added to `mObjects` inside this loop.
            // This can happen if Destroy races with Track.
            mObjects.MoveInto(&objects);
        }
    }
}
...
void ApiObjectBase::Destroy() {
    // Set mAlive to false. If it was already false, return.
