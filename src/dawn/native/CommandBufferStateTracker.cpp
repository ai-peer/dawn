...
Return FindStorageBufferBindingAliasing(
    ...
    // There can at most be 8 storage buffer bindings (by default) per shader stage.
    ...
    // potentially have ranges overlap.
    // There can at most be 8 storage texture bindings (by default) per shader stage.
    ...
    for (size_t i = 0; i < storageTextureViewsToCheck->size(); i++) {
        ...
        for (size_t j = i + 1; j < storageTextureViewsToCheck->size(); j++) {
            ...
            if (HasRangesOverlap(baseArrayLayer0, baseArrayLayer0 + arrayLayerCount0 - 1,
