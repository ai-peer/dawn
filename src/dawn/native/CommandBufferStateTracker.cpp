...
namespace {
// Returns -1 is all buffers in unverifiedBufferSizes are at least large enough to satisfy the
// minimum listed in pipelineMinBufferSizes, or the index of the first detected failing buffer
// otherwise.
int32_t BufferSizesAtLeastAsBig(const ityp::span<uint32_t, uint64_t> unverifiedBufferSizes,
...
MaybeError CommandBufferStateTracker::CheckMissingAspects(ValidationAspects aspects) {
    ...
    if (DAWN_UNLIKELY(aspects[VALIDATION_ASPECT_BIND_GROUPS])) {
        for (BindGroupIndex i : IterateBitSet(mLastPipelineLayout->GetBindGroupLayoutsMask())) {
            ...
            int32_t packedIndex = BufferSizesAtLeastAsBig(
                mBindgroups[i]->GetUnverifiedBufferSizes(), (*mMinBufferSizes)[i]);
            if (packedIndex != -1) {
                uint64_t bufferSize = mBindgroups[i]->GetUnverifiedBufferSizes()[packedIndex];
                uint64_t minBufferSize = (*mMinBufferSizes)[i][packedIndex];
                return DAWN_VALIDATION_ERROR(
                    ...
                    "at least %u bytes. (Buffer bindings must be a multiple of 16 bytes, and as a "
