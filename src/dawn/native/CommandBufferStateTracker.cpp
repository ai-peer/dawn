...
struct WritableBindingAliasingResult {
    std::variant<BufferAliasing, TextureAliasing> aliasing;
};
...
Return FindStorageBufferBindingAliasing(
    ...
    // aliasing is found.
    constexpr bool kProduceDetails =
        std::is_same_v<Return, std::optional<WritableBindingAliasingResult>>;
...
MaybeError CommandBufferStateTracker::CheckMissingAspects(ValidationAspects aspects) {
    ...
    if (aspects[VALIDATION_ASPECT_BIND_GROUPS]) {
        ...
        if (result) {
            if (std::holds_alternative<BufferAliasing>(result->aliasing)) {
                const auto& a = std::get<BufferAliasing>(result->aliasing);
                return DAWN_VALIDATION_ERROR(
                ...
                ASSERT(std::holds_alternative<TextureAliasing>(result->aliasing));
                const auto& a = std::get<TextureAliasing>(result->aliasing);
                return DAWN_VALIDATION_ERROR(
                    ...
                    a.e0.baseArrayLayer, a.e0.arrayLayerCount, a.e1.baseMipLevel,
                    a.e1.mipLevelCount, a.e1.baseArrayLayer, a.e1.arrayLayerCount);
            }
        }
