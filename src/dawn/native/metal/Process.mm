...
// static
std::unique_ptr<Process> Process::MakeInternal(const char* const args[],
    ...
    pid_t pid = -1;
    result =
        posix_spawnp(&pid, args[0], fileActions.Get(), attr.Get(), const_cast<char* const*>(args),
                     /*envp=*/nullptr);
    if (result != 0) {
        return {};
    }
...
// static
std::unique_ptr<Process> Process::MakeWithStringInput(const char* const args[],
...
// static
std::unique_ptr<Process> Process::MakeWithProcessInput(const char* const args[],
...
bool Process::ReadFromStdout(std::vector<char>& buffer, size_t size) {
