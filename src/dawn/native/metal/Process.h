...
class Process : public NonCopyable {
  public:
    // Create a process and use the input string as the new process stdin.
    static std::unique_ptr<Process> MakeWithStringInput(const char* const args[],
                                                        const std::string& input);
    // Create a process and use the input process's stdout as the new process stdin.
    static std::unique_ptr<Process> MakeWithProcessInput(const char* const args[],
                                                         std::unique_ptr<Process> input);
    ~Process();

    bool WriteToStdin(const std::string& data);
    bool CloseStdin();
    bool ReadFromStdout(std::vector<char>& buffer, size_t size);
    bool CloseStdout();
  ...
  private:
    static std::unique_ptr<Process> MakeInternal(const char* const args[],
