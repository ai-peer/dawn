...
// static
ResultOrError<QueueWorkDoneFuture*> QueueWorkDoneFuture::Create(QueueBase* queue,
                                                                ExecutionSerial serial) {
    Device* device = ToBackend(queue->GetDevice());
    EventPipe eventPipe;
    DAWN_TRY_ASSIGN(eventPipe, EventPipe::Create());
    QueueWorkDoneFuture* future = new QueueWorkDoneFuture(queue, serial, std::move(eventPipe));
...
QueueWorkDoneFuture::QueueWorkDoneFuture(QueueBase* queue,
                                         ExecutionSerial serial,
                                         EventPipe&& eventPipe)
    : QueueWorkDoneFutureBase(queue, serial), mEventPipe(std::move(eventPipe)) {}
