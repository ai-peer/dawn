...
int QueueWorkDoneFuture::APIGetFd() {
    if (!mRequestedFd) {
        DAWN_UNUSED(ToBackend(GetDevice())->ConsumedError(DAWN_INTERNAL_ERROR("fd not requested")));
        return -1;
    }
    return int(mEventPipe.GetReceiverFd());
}

void QueueWorkDoneFuture::APIGetResult(QueueWorkDoneResult*) {
    ASSERT(false);
}
