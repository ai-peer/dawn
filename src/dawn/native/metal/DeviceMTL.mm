...
Device::Device(AdapterBase* adapter,
    ...
    : DeviceBase(adapter, descriptor, userProvidedToggles),
      ...
      mCompletedSerial(0) {
    // On mac OS < 11.0, we only can check whether counter sampling is supported, and the counter
...
void Device::InitTogglesFromDriver() {
    ...
    // "mtl_use_dummy_blit_encoder_for_timestamp_writes".
    if (@available(macos 11.0, iOS 14.0, *)) {
        SetToggle(Toggle::MTLUseDummyBlitEncoderForWriteTimestamp, true);
    }
}
