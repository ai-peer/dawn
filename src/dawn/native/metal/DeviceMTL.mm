...
ResultOrError<PosixFd> Device::CreateWorkDoneEventFd(ExecutionSerial serial) {
    printf("%s %p\n", __FUNCTION__, (void*)this);
    EventPipe eventPipe;
    DAWN_TRY_ASSIGN(eventPipe, EventPipe::Create());
    PosixFd ret = eventPipe.AcquireReceiverFd();

    {
        ...
        if (mCompletedSerial.load() >= uint64_t(serial)) {
            ...
            // executing. Just signal the EventPipe instead of storing it, so it won't have to wait
            // until there's another command buffer completion (which may be never!). This will
            // leave the lower bound lower than it needs to be, but that's fine. In this very rare
            // case, UpdateWaitingEvents will take the lock unnecessarily.
            DAWN_TRY(eventPipe.Signal());
            return ret;
        }

        mWaitingEvents.push_back(std::pair(serial, std::move(eventPipe)));
    }
...
void Device::UpdateWaitingEvents(ExecutionSerial completedSerial) {
    printf("%s %p %llu %llu\n", __FUNCTION__, (void*)this, mCompletedSerial.load(),
