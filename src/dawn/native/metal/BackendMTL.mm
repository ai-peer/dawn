...
class Adapter : public AdapterBase {
  public:
    Adapter(InstanceBase* instance,
        ...
        : AdapterBase(instance, wgpu::BackendType::Metal, requiredAdapterToggle, requiredToggles),
          mDevice(device) {
...
ResultOrError<std::vector<Ref<AdapterBase>>> Backend::DiscoverAdapters(
    ...
    TogglesState adapterToggles = MakeAdapterToggles(requiredToggles);
    ...
    for (id<MTLDevice> device in devices.Get()) {
        Ref<Adapter> adapter =
            AcquireRef(new Adapter(GetInstance(), device, adapterToggles, requiredToggles));
