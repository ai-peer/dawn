  ...
  private:
    ...
    TogglesState MakeDeviceTogglesImpl(
        ...
        if (gpu_info::IsIntel(vendorId)) {
            ...
            // Stencil to buffer copies don't capture contents written in a rendering stage. Copying
            // through an intermediate texture fixes this problem on Metal Intel. See
            // https://crbug.com/dawn/1389 for more details.
            deviceToggles.Default(Toggle::UseTempTextureInStencilTextureToBufferCopy, true);
        }
        // Stencil8 textures don't work correctly on some drivers. Workaround by allocating a
        // combined depth-stencil texture instead. See https://crbug.com/dawn/1389 for more details.
