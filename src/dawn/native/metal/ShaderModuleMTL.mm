...
#define MSL_COMPILATION_REQUEST_MEMBERS(X)                                                  \
    ...
    X(stream::LimitsForCompilationRequest, limits)                                          \
...
ResultOrError<CacheResult<MslCompilation>> TranslateToMSL(
    ...
    req.limits = stream::LimitsForCompilationRequest::Create(limits.v1);
    ...
    DAWN_TRY_LOAD_OR_RUN(
        ...
        [](MslCompilationRequest r) -> ResultOrError<MslCompilation> {
            ...
            wgpu::Extent3D localSize{0, 0, 0};
            if (r.stage == SingleShaderStage::Compute) {
                // Validate workgroup size after program runs transforms.
                DAWN_TRY_ASSIGN(
                    localSize,
                    ValidateComputeStageWorkgroupSize(
                        program, remappedEntryPointName.data(), r.limits.maxComputeWorkgroupSizeX,
                        r.limits.maxComputeWorkgroupSizeY, r.limits.maxComputeWorkgroupSizeZ,
            ...
            return MslCompilation{{
                ...
                MTLSizeMake(localSize.width, localSize.height, localSize.depthOrArrayLayers),
            }};
        });
...
MaybeError ShaderModule::CreateFunction(const char* entryPointName,
    ...
    out->localWorkgroupSize = mslCompilation->localWorkgroupSize;
