...
struct MslCompilation {
    ...
    static constexpr auto kMembers = std::make_tuple(&MslCompilation::msl,
                                                     &MslCompilation::remappedEntryPointName,
                                                     &MslCompilation::needsStorageBufferLength,
                                                     &MslCompilation::hasInvariantAttribute,
                                                     &MslCompilation::workgroupAllocations);
};
...
void BlobCache::Store<metal::MslCompilation>(const CacheKey& key, const metal::MslCompilation& c) {
    ...
    StreamIn(&sink, stream::StructMembers(c, metal::MslCompilation::kMembers));
...
ResultOrError<metal::MslCompilation> metal::MslCompilation::FromBlob(Blob blob) {
    ...
    DAWN_TRY(StreamOut(&source, stream::StructMembers(&c, metal::MslCompilation::kMembers)));
    return c;
}
