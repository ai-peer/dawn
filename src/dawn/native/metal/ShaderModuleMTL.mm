...
ResultOrError<CacheResult<MslCompilation>> TranslateToMSL(
    ...
    DAWN_TRY_LOAD_OR_RUN(
        mslCompilation, device, std::move(req),
        SCOPED_DAWN_CACHE_HIT_FROM_BLOB(device->GetPlatform(), "Metal.TintMSLGenerate",
                                        MslCompilation::FromBlob),
        [](MslCompilationRequest r) -> ResultOrError<MslCompilation> {
            ...
            TRACE_EVENT0(r.platform.UnsafeGetValue(), General, "tint::writer::msl::Generate");
            auto result = [&]() {
                dawn::platform::Platform* platform = r.platform.UnsafeGetValue();
                SCOPED_DAWN_CACHE_MISS_TIMER(platform, "Metal.TintMSLGenerate");
                return tint::writer::msl::Generate(&program, options);
            }();
