...
ResultOrError<Ref<Texture>> Texture::Create(Device* device, const TextureDescriptor* descriptor) {
    ...
    if (texture->GetFormat().IsMultiPlanar()) {
        ...
        DAWN_TRY(texture->ClearTextureOnCreation(device->GetPendingCommandContext(),
...
MaybeError Texture::InitializeAsInternalTexture(const TextureDescriptor* descriptor) {
    ...
    DAWN_TRY(ClearTextureOnCreation(device->GetPendingCommandContext(), GetAllSubresources()));
...
MaybeError Texture::ClearTextureOnCreation(CommandRecordingContext* commandContext,
                                           const SubresourceRange& range) {
    if (GetDevice()->IsToggleEnabled(Toggle::NonzeroClearResourcesOnCreationForTesting)) {
        DAWN_TRY(ClearTexture(commandContext, range, TextureBase::ClearValue::NonZero));
...
MTLBlitOption Texture::ComputeMTLBlitOption(Aspect aspect) const {
    ...
    MTLPixelFormat format = mMtlFormat;
