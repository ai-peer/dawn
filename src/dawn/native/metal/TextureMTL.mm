...
MaybeError Texture::InitializeAsInternalTexture(const TextureDescriptor* descriptor) {
    ...
    const bool isDepthStencil = format.HasDepth() && format.HasStencil();
    if (format.HasDepth() || format.HasStencil()) {
        // Depth stencil textures are always lazily cleared using a render pass.
        AddInternalUsage(wgpu::TextureUsage::RenderAttachment);
    }
    const bool hasTextureBindingOrCopyUsage =
        descriptor->usage & (wgpu::TextureUsage::TextureBinding | wgpu::TextureUsage::CopySrc |
                             wgpu::TextureUsage::CopyDst);
    const bool disjointDepthStencil =
        isDepthStencil && hasTextureBindingOrCopyUsage &&
        device->IsToggleEnabled(
            Toggle::MetalUseDisjointDepthStencilTexturesForTextureBindingOrCopy);

    NSRef<MTLTextureDescriptor> mtlDesc = CreateMetalTextureDescriptor();
    if (isDepthStencil && hasTextureBindingOrCopyUsage && !disjointDepthStencil) {
...
MTLBlitOption Texture::ComputeMTLBlitOption(Aspect aspect) {
    ASSERT(HasOneBit(aspect));
    ASSERT(GetFormat().aspects & aspect);

    if (GetDisjointAspects() == Aspect::CombinedDepthStencil &&
        IsSubset(Aspect::Depth | Aspect::Stencil, GetFormat().aspects)) {
...
MaybeError Texture::ClearTexture(CommandRecordingContext* commandContext,
    ...
    if ((mMtlUsage & MTLTextureUsageRenderTarget) != 0) {
        ...
        // All depth stencil textures are created with MTLTextureUsageRenderTarget,
        // so initialization should use a render pass.
        ASSERT(GetFormat().IsColor());
