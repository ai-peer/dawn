...
ResultOrError<Ref<SharedTextureMemory>> SharedTextureMemory::Create(
    ...
    properties.size = {static_cast<uint32_t>(width), static_cast<uint32_t>(width), 1};
...
void SharedTextureMemory::SynchronizeBeforeUse(CommandRecordingContext* commandContext) {
    if (!mBeginFences.empty()) {
    ...
    // Consume the wait events on the texture.
    for (const auto& fence : mBeginFences) {
...
MaybeError SharedTextureMemory::EndAccessImpl(TextureBase* texture) {
    DAWN_INVALID_IF(!GetDevice()->HasFeature(Feature::SharedFenceMTLSharedEvent),
    ...
    if (@available(macOS 10.14, iOS 12.0, *)) {
        ...
        mEndFences.push_back({std::move(fence), oldEndAccessDesc.signaledValue});
    }
    return {};
}
