...
#pragma clang diagnostic pop
        ...
        struct StorageBufferLengthTracker {
            ...
            // This also means that max binding sizes must be restricted to MAX_UINT32.
    ...
    MaybeError CommandBuffer::EncodeRenderPass(CommandRecordingContext* commandContext,
        ...
        if (device->IsToggleEnabled(Toggle::AlwaysResolveIntoZeroLevelAndLayer)) {
            ...
            for (uint32_t i = 0; i < kMaxColorAttachments; ++i) {
                ...
                trueResolveLevels[i] =
                    checked_cast<uint32_t>(mtlRenderPass.colorAttachments[i].resolveLevel);
                trueResolveSlices[i] =
                    checked_cast<uint32_t>(mtlRenderPass.colorAttachments[i].resolveSlice);
