...
ResultOrError<NSPRef<id<MTLTexture>>> CreateResolveTextureForWorkaround(Device* device,
    ...
    while (mCommands.NextCommandId(&type)) {
        switch (type) {
            ...
            case Command::BeginRenderPass: {
                ...
                DAWN_TRY(
                    EncodeRenderPass(commandContext, descriptor.Get(), cmd->width, cmd->height));
...
MaybeError CommandBuffer::EncodeRenderPass(CommandRecordingContext* commandContext,
    ...
    if (device->IsToggleEnabled(Toggle::AlwaysResolveIntoZeroLevelAndLayer)) {
        ...
        for (uint32_t i = 0; i < kMaxColorAttachments; ++i) {
            ...
            DAWN_TRY_ASSIGN(temporaryResolveTextures[i],
                            CreateResolveTextureForWorkaround(device, mtlFormat, width, height));

            mtlRenderPass.colorAttachments[i].resolveTexture = temporaryResolveTextures[i].Get();
