...
void SetSampleBufferAttachments(PassDescriptor* descriptor, BeginPass* cmd) {
    // Use @available instead of API_AVAILABLE becuase GetCounterSampleBuffer() also needs checking
    // API availability.
...
void EncodeEmptyBlitEncoderForWriteTimestamp(Device* device,
    ...
    if (cmd->querySet.Get() != nullptr) {
        ...
        if (device->IsToggleEnabled(Toggle::MTLUseDummyBlitEncoderForWriteTimestamp)) {
