...
uint64_t RingBufferAllocator::Allocate(uint64_t allocationSize,
    ...
    if (mUsedStartOffset <= mUsedEndOffset) {
        ...
        // wrapped).
        if (mUsedEndOffset + allocationSize + alignmentOffset <= mMaxBlockSize) {
            startOffset = mUsedEndOffset + alignmentOffset;
    ...
    } else if (mUsedEndOffset + allocationSize + alignmentOffset <=
