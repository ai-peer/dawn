...
#include "dawn/common/Error.h"
#include "dawn/common/TypedInteger.h"

namespace dawn::serde {

// Abstract interface for a serialization sink.
class Sink {
  public:
    virtual void* GetSpace(size_t) = 0;
};

// Abstract interface for a deserialization source.
class Source {
  public:
    virtual ResultOrError<const void*> Read(size_t) = 0;
};
...
template<typename T>
struct CheckSerde {
    template<typename Fn, Fn> struct CheckFn;
...
constexpr bool HasSerialize = decltype(detail::CheckSerde<T>::template HasSerialize<Delegate<T>>(0))::value;

// HasDeserialize is true if Delegate<T> has a static Deserialize method.
template <template<typename...> class Delegate, typename T>
constexpr bool HasDeserialize = decltype(detail::CheckSerde<T>::template HasDeserialize<Delegate<T>>(0))::value;
...
template <template<typename...> class Delegate, typename T, typename SFINAE = std::enable_if_t<HasSerialize<Delegate, T>>>
...
template <template<typename...> class Delegate, typename T, typename SFINAE = std::enable_if_t<HasDeserialize<Delegate, T>>>
...
template <template<typename...> class Delegate, typename T, size_t N, typename SFINAE = std::enable_if_t<
    HasSerialize<Delegate, T> && !std::is_fundamental_v<T>>>
...
template <template<typename...> class Delegate, typename T, size_t N, typename SFINAE = std::enable_if_t<
    HasDeserialize<Delegate, T> && !std::is_fundamental_v<T>>>
...
template <template<typename...> class Delegate, typename A, typename B,
    typename SFINAE = std::enable_if_t<HasSerialize<Delegate, A> && HasSerialize<Delegate, B>>>
...
class Serde<T, std::enable_if_t<std::is_enum_v<T>>> {
  ...
  public:
    static void Serialize(Sink* s, const T& v) {
        serde::Serialize<Serde>(s, static_cast<U>(v));
    }
    static MaybeError Deserialize(Source* s, T* v) {
...
template <size_t N>
class Serde<std::bitset<N>, std::enable_if_t<
    std::less<size_t>{}(sizeof(unsigned long), N) &&
...
template <size_t N>
class Serde<std::bitset<N>, std::enable_if_t<std::greater<size_t>{}(N, sizeof(unsigned long long))>> {
  public:
    static void Serialize(Sink* s, const std::bitset<N>& t) {
