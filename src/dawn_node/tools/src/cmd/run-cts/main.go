...
func (r *runner) streamResults(wg *sync.WaitGroup, results chan result) {
	...
	for _, s := range statuses {
		// These are the counts of tests that produced the status s, which
		// either matched the expected status (numExpected), or was an
		// unexpected status (numUnexpected).
		numStatus := numByExpectedStatus[expectedStatus{s, true}] + numByExpectedStatus[expectedStatus{s, false}]
		diffFromExpected := 0
		if numExpectedByStatus != nil {
			diffFromExpected = numStatus - numExpectedByStatus[s]
		}
		if numStatus == 0 && diffFromExpected == 0 {
			continue
		}
		...
		fmt.Print(alignLeft(numStatus, 10))
		fmt.Print(ansiReset)
		fmt.Print(alignRight("("+percentage(numStatus, numTests)+")", 6))
