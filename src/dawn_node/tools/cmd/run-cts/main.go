...
func (r *runner) run() error {
	...
	results := make([]result, len(r.testcases))
	...
	numTests, numByStatus := len(results), map[status]int{}
...
func (r *runner) runTestcase(idx int) result {
	...
	args := append([]string{
		...
		// start at 1, so just inject a dummy argument.
		"dummy-arg",
	}, "--verbose", r.testcases[idx]) // Actual arguments
	...
	case errors.Is(err, context.DeadlineExceeded):
		return result{status: timeout, message: msg}
	...
	case strings.Contains(msg, "[pass]"), err == nil:
		return result{status: pass, message: msg}
	}
	return result{status: fail, message: fmt.Sprint(msg, err), error: err}
}
