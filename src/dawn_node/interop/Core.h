...
namespace wgpu { namespace interop {
    ...
    class Serializer<Napi::Object> {
      public:
        static inline bool Unmarshal(Napi::Env, Napi::Value value, Napi::Object& out) {
            if (out.IsObject()) {
                out = value.ToObject();
                return true;
            }
    ...
    class Serializer<Napi::TypedArray> {
      public:
        static inline bool Unmarshal(Napi::Env, Napi::Value value, Napi::TypedArray& out) {
    ...
    class Serializer<Napi::TypedArrayOf<T>> {
      public:
        static inline bool Unmarshal(Napi::Env, Napi::Value value, Napi::TypedArrayOf<T>& out) {
            if (value.IsTypedArray()) {
    ...
    class Serializer<std::optional<T>> {
      public:
        static bool Unmarshal(Napi::Env env, Napi::Value value, std::optional<T>& out) {
    ...
    class Serializer<std::vector<T>> {
      public:
        static inline bool Unmarshal(Napi::Env env, Napi::Value value, std::vector<T>& out) {
            if (!value.IsArray()) {
                return false;
            }
    ...
    class Serializer<std::unordered_map<K, V>> {
      public:
        static inline bool Unmarshal(Napi::Env env,
                                     Napi::Value value,
        ...
        static inline Napi::Value Marshal(Napi::Env env, std::unordered_map<K, V> value) {
    ...
    class Serializer<std::variant<TYPES...>> {
        ...
        static inline bool TryUnmarshal(Napi::Env env,
                                        Napi::Value value,
      ...
      public:
        ...
        static inline Napi::Value Marshal(Napi::Env env, std::variant<TYPES...> value) {
            return std::visit(
                [&](auto&& v) {
    ...
    inline bool UnmarshalArgs(const Napi::CallbackInfo& info, PARAM_TYPES& args) {
        if constexpr (BASE_INDEX < std::tuple_size_v<PARAM_TYPES>) {
            ...
            if (!Unmarshal<T>(info.Env(), info[BASE_INDEX], std::get<BASE_INDEX>(args))) {
                return false;
            }
