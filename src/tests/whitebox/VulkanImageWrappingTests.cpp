...
TEST_P(VulkanImageWrappingUsageTests, ClearImageAcrossDevices) {
    ...
    close(dawn_native::vulkan::ExportSignalSemaphoreFd(device.Get(), nextWrappedTexture.Get()));
}
...
TEST_P(VulkanImageWrappingUsageTests, CopyTextureToTextureSrcSync) {
    ...
    close(dawn_native::vulkan::ExportSignalSemaphoreFd(device.Get(), deviceWrappedTexture.Get()));
}
...
TEST_P(VulkanImageWrappingUsageTests, CopyTextureToTextureDstSync) {
    ...
    dawn::Texture wrappedTexture = WrapVulkanImage(device, &defaultDescriptor, defaultFd, {});

    // Clear |wrappedTexture| on |device|
    ClearImage(device, wrappedTexture, {5 / 255.0f, 6 / 255.0f, 7 / 255.0f, 8 / 255.0f});

    int signalFd = dawn_native::vulkan::ExportSignalSemaphoreFd(device.Get(), wrappedTexture.Get());

    // Import the image to |secondDevice|, making sure we wait on |signalFd|
    int memoryFd = GetMemoryFd(deviceVk, defaultAllocation.GetMemory());
    dawn::Texture secondDeviceWrappedTexture =
        WrapVulkanImage(secondDevice, &defaultDescriptor, memoryFd, {signalFd});
    ...
    signalFd = dawn_native::vulkan::ExportSignalSemaphoreFd(secondDevice.Get(),
                                                            secondDeviceWrappedTexture.Get());
    memoryFd = GetMemoryFd(deviceVk, defaultAllocation.GetMemory());
...
TEST_P(VulkanImageWrappingUsageTests, ChainTextureCopy) {
    ...
    int signalFdTexBDevice3 =
        dawn_native::vulkan::ExportSignalSemaphoreFd(thirdDevice.Get(), wrappedTexBDevice3.Get());
    close(
        dawn_native::vulkan::ExportSignalSemaphoreFd(thirdDevice.Get(), wrappedTexADevice3.Get()));
    ...
    int signalFdTexCDevice2 =
        dawn_native::vulkan::ExportSignalSemaphoreFd(secondDevice.Get(), wrappedTexCDevice2.Get());
    close(
        dawn_native::vulkan::ExportSignalSemaphoreFd(secondDevice.Get(), wrappedTexBDevice2.Get()));
    ...
    close(dawn_native::vulkan::ExportSignalSemaphoreFd(device.Get(), wrappedTexCDevice1.Get()));
}
