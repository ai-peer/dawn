...
AdapterTestParam::AdapterTestParam(const BackendTestConfig& config, dawn_native::Adapter adapter)
    : BackendTestConfig(config), adapter(adapter) {
}
        ...
        for (size_t i = 0; i < numParams; ++i) {
            for (const dawn_native::Adapter& adapter : gTestEnv->GetAdapters()) {
                ...
                if (params[i].backendType == properties.backendType) {
                    backends.push_back(AdapterTestParam(params[i], adapter));
                }
            }
        }
