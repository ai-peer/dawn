    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (strncmp(argv[i], kEnableBackendValidationSwitch, argLen) == 0) {
            ...
            if (level[0] != '\0') {
                ...
                } else if (strcmp(level, "=performance") == 0) {
        ...
        if (strcmp("-h", argv[i]) == 0 || strcmp("--help", argv[i]) == 0) {
            dawn::InfoLog()
                ...
                << " [GTEST_FLAGS...] [-w] [-c]\n"
                   ...
                   "to enable all available backend validation. Set to 'performance' to enable a \n"
                   "subset of backend validation with less performance overhead.\n"
