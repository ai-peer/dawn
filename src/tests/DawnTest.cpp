...
void DawnTest::ImplementDawnTestOptions(const DawnTestOptions& options) {
    if (options.test(static_cast<size_t>(DawnTestOption::ForceEmulatingStoreAndMSAAResolve))) {
        mUseCustomWorkaround = true;
        mWorkarounds.emulateStoreAndMSAAResolve = true;
    }
}
...
void DawnTest::SetUp() {
    ...
    dawn_native::Adapter backendAdapter;
    {
        ...
        for (const dawn_native::Adapter& adapter : adapters) {
            if (adapter.GetBackendType() == backendType) {
                ...
                if (std::get<0>(GetParam()) != MetalBackend) {
                    break;
                }
            }
        }
    ...
    if (mUseCustomWorkaround) {
        backendDevice = backendAdapter.CreateDevice(&mWorkarounds);
    } else {
