    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (strstr(argv[i], "--adapter-vendor-id") != 0) {
            ...
            if (value != nullptr) {
                ...
                mVendorIdFilterSet = mVendorIdFilter != 0;
            }
            continue;
        }
...
void DawnTestEnvironment::SetUp() {
    ...
    for (const dawn_native::Adapter& adapter : mInstance->GetAdapters()) {
        ...
        std::cout << "   vendorId: 0x" << vendorId.str() << ", deviceId: 0x" << deviceId.str()
                  << (VendorIdFilterSet() && mVendorIdFilter == pci.vendorId ? " [Selected]" : "")
                  << "\n";
    }
    std::cout << std::endl;
}
...
bool DawnTestEnvironment::VendorIdFilterSet() const {
    return mVendorIdFilterSet;
}
...
bool DawnTest::VendorIdFilterSet() const {
    return gTestEnv->VendorIdFilterSet();
}
    ...
    const dawn_native::BackendType backendType = GetParam().backendType;
    {
        ...
        for (const dawn_native::Adapter& adapter : adapters) {
            if (adapter.GetBackendType() == backendType) {
                ...
                // https://bugs.chromium.org/p/dawn/issues/detail?id=184 resolved.
                if (VendorIdFilterSet() && backendType != dawn_native::BackendType::OpenGL) {
