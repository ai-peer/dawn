...
void DawnTestEnvironment::SetUp() {
    ...
    for (const dawn_native::Adapter& adapter : mInstance->GetAdapters()) {
        ...
        vendorId << std::setfill('0') << std::uppercase << std::internal
                 << std::hex << std::setw(4) << pci.vendorId;
        deviceId << std::setfill('0') << std::uppercase << std::internal
                 << std::hex << std::setw(4) << pci.deviceId;
