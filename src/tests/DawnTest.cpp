...
AdapterTestParam::AdapterTestParam(const BackendTestConfig& config,
                                   const wgpu::AdapterProperties& adapterProperties)
    : BackendTestConfig(config), adapterProperties(adapterProperties) {
}

std::ostream& operator<<(std::ostream& os, const AdapterTestParam& param) {
    const wgpu::AdapterProperties& properties = param.adapterProperties;
...
DawnTestEnvironment::DawnTestEnvironment(int argc, char** argv) {
    ...
    for (const dawn_native::Adapter& adapter : mInstance->GetAdapters()) {
        ...
        bool selected =
            ...
            // Always select the Null backend. There are few and fast tests here. This is
    ...
    // If backend validation is enabled, destroy the instance now. We'll recreate it again in
    // DawnTestEnvironment::SetUp.
    // The Vulkan validation layers behave badly when we create them in the environment constructor
    // because they use static global variables for locks. Chromium's test launcher forks muliple
    // processes, and bad things happen when they exit.
...
void DawnTestEnvironment::CreateInstanceAndAdapters() {
...
void DawnTestEnvironment::SetUp() {
    ASSERT_NE(mAdapterProperties.size(), 0u);
    if (!mInstance) {
        ASSERT(mEnableBackendValidation);
        // The instance is destroyed if backend validation is enabled because the Vulkan validation
        // layers behave badly. Create them again.
        CreateInstanceAndAdapters();
        ASSERT(mInstance);
    }
}
...
void DawnTestBase::SetUp() {
    ...
    for (const dawn_native::Adapter& adapter : gTestEnv->GetInstance()->GetAdapters()) {
        wgpu::AdapterProperties properties;
        adapter.GetProperties(&properties);

        if (properties.deviceID == mParam.adapterProperties.deviceID &&
            ...
            strcmp(properties.name, mParam.adapterProperties.name) == 0) {
...
namespace detail {
    ...
    std::vector<AdapterTestParam> GetAvailableAdapterTestParamsForBackends(
        ...
        for (size_t i = 0; i < numParams; ++i) {
            for (const auto& adapterProperties : gTestEnv->GetAdapterProperties()) {
                if (params[i].backendType == adapterProperties.backendType) {
                    backends.push_back(AdapterTestParam(params[i], adapterProperties));
                }
            }
        }
