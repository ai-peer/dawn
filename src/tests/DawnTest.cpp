...
std::ostream& operator<<(std::ostream& os, const AdapterTestParam& param) {
    os << ParamName(param.adapterProperties.backendType) << "_"
...
std::string DawnTestBase::PrintToStringParamName::SanitizedParamName(std::string paramName,
                                                                     size_t index) const {
    ...
    if (testFormat.length() > 220) {
        ...
        sanitizedName.resize(testFormat.length() - prefix.length() - 10);
        sanitizedName = sanitizedName + "___" + std::to_string(index);
    }
...
std::ostringstream& DawnTestBase::ExpectSampledDepthData(wgpu::Texture texture,
    ...
    // Note: need explicit array length. OpenGL Mesa driver returns 0 for the dynamic array length.
    shaderSource << R"(
        [[block]] struct Result {
            values : array<f32, )"
                 << width * height << R"(>;
