    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (strncmp(argv[i], kEnableBackendValidationSwitch, argLen) == 0) {
            ...
            if (level[0] != '\0') {
                ...
                } else if (strcmp(level, "=low-overhead") == 0) {
                    mBackendValidationLevel = dawn_native::BackendValidationLevel::LowOverhead;
        ...
        if (strcmp("-h", argv[i]) == 0 || strcmp("--help", argv[i]) == 0) {
            dawn::InfoLog()
                ...
                << " [GTEST_FLAGS...] [-w] [-c]\n"
                   ...
                   "    [--adapter-vendor-id=x] "
                   "[--enable-backend-validation[=full,low-overhead,disabled]]\n"
                   ...
                   "    to enable all available backend validation. Set to 'low-overhead' to\n"
...
void DawnTestEnvironment::PrintTestConfigurationAndAdapterInfo(
    ...
    switch (mBackendValidationLevel) {
        case dawn_native::BackendValidationLevel::Full:
            log << "full";
            break;
        case dawn_native::BackendValidationLevel::LowOverhead:
            log << "low-overhead";
            break;
        default:
            log << "disabled";
