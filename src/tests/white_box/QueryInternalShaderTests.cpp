...
// - The destination buffer contains the original timestamps resolved from query set (created by
...
//   rate.
// - The availability buffer passes the data of which slot in destination buffer is an initialized
//   timestamp.
...
TEST_P(QueryInternalShaderTests, TimestampComputeShader) {
    ...
    // Convert timestamps to destination buffer with offset 0
    {
        ...
        EXPECT_BUFFER(destinationBuffer, 0, kTimestampCount * sizeof(uint64_t),
