...
// Tests a larget image is preserved when importing
...
TEST_P(VulkanImageWrappingUsageTests, LargerImage) {
    ...
    // Fill memory with textures to trigger layout issues
    ...
    for (int row = 0; row < height; row++) {
        for (int col = 0; col < width; col++) {
            float norm_row = (float)row / height;
            float norm_col = (float)col / width;
            float dist = sqrt(norm_row * norm_row + norm_col * norm_col) * 3;
            dist = dist - int(dist);
            data[4 * (row * width + col)] = dist * 255;
            data[4 * (row * width + col) + 1] = dist * 255;
            data[4 * (row * width + col) + 2] = dist * 255;
    ...
    EXPECT_BUFFER_U32_RANGE_EQ((unsigned int*)data, copyDstBuffer, 0, size / 4);
