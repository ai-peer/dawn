    ...
    class VulkanImageWrappingTestBase : public DawnTest {
      public:
        ...
        dawn::Texture WrapVulkanImage(dawn::Device device,
            ...
            descriptor.memoryHandle = memoryFd;
...
TEST_P(VulkanImageWrappingUsageTests, ClearImageAcrossDevices) {
    ...
    close(
        dawn_native::vulkan::ExportSignalSemaphoreOpaqueFD(device.Get(), nextWrappedTexture.Get()));
}
...
TEST_P(VulkanImageWrappingUsageTests, CopyTextureToTextureSrcSync) {
    ...
    close(dawn_native::vulkan::ExportSignalSemaphoreOpaqueFD(device.Get(),
                                                             deviceWrappedTexture.Get()));
}
...
TEST_P(VulkanImageWrappingUsageTests, ChainTextureCopy) {
    ...
    close(dawn_native::vulkan::ExportSignalSemaphoreOpaqueFD(thirdDevice.Get(),
                                                             wrappedTexADevice3.Get()));
    ...
    close(dawn_native::vulkan::ExportSignalSemaphoreOpaqueFD(secondDevice.Get(),
                                                             wrappedTexBDevice2.Get()));
    ...
    close(
        dawn_native::vulkan::ExportSignalSemaphoreOpaqueFD(device.Get(), wrappedTexCDevice1.Get()));
}
