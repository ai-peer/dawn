...
TEST_P(D3D12ResidencyTests, SwitchedSamplerHeapResidency) {
    ...
    EXPECT_TRUE(allocator->GetShaderVisibleHeapIsLockedResidentForTesting());
    // Check that the previously bound ShaderVisibleHeap was unlocked and was placed in the LRU
    // cache.
    EXPECT_TRUE(allocator->GetLastShaderVisibleHeapIsInLRUForTesting());
    ...
    EXPECT_FALSE(allocator->GetLastShaderVisibleHeapIsInLRUForTesting());
}
