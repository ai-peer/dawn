...
class D3D12ResidencyTests : public DawnTest {
  protected:
    void TestSetUp() override {
        ...
        // buffers.
        mSourceBuffer = CreateBuffer(kSourceBufferSize,
                                     wgpu::BufferUsage::CopySrc | wgpu::BufferUsage::CopyDst);

        std::vector<uint8_t> data(kSourceBufferSize);
        ...
        encoder.CopyBufferToBuffer(stagingBuffer, 0, mSourceBuffer, 0, kSourceBufferSize);
...
TEST_P(D3D12ResidencyTests, OvercommitLargeResources) {
    ...
    for (uint32_t i = 0; i < bufferSet1.size(); i++) {
        ASSERT_TRUE(CheckIfBufferIsResident(bufferSet1[i]));
        ASSERT_TRUE(CheckAllocationMethod(bufferSet1[i], dawn_native::AllocationMethod::kDirect));
    }
...
TEST_P(D3D12ResidencyTests, AsyncMappedBufferRead) {
    ...
    // Unmap the buffer and touch enough resources such that the entire budget is used. This should
    // evict the mappable buffer.
    buffer.Unmap();
    TouchBuffers(0, bufferSet.size(), bufferSet);
    ASSERT_FALSE(CheckIfBufferIsResident(buffer));
}
