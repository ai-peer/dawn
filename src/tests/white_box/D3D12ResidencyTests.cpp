...
class D3D12ResidencyTests : public DawnTest {
  protected:
    void AllocateTextures(uint32_t textureSize, uint64_t bytesToAllocate) {
...
TEST_P(D3D12ResidencyTests, OvercommitSmallResources) {
    ...
    AllocateTextures(kSize, GetVideoMemoryInfo() * 0.75);
    TouchTextures(kSize, 0, 1);
    AllocateTextures(kSize, GetVideoMemoryInfo() * 0.75);
...
TEST_P(D3D12ResidencyTests, AsyncMappedBufferLockResidency) {
    // Create a mappable buffer.
    wgpu::BufferDescriptor descriptor;
    descriptor.size = 4;
    descriptor.usage = wgpu::BufferUsage::MapWrite | wgpu::BufferUsage::CopySrc;
    ...
    // Load all the textures into memory, to ensure the least recently used resources get evicted.
    // The residency manager should treat the mapped buffer as a special case, and keep it resident.
