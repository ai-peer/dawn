...
class D3D12ResidencyTests : public DawnTest {
  protected:
    void AllocateBuffers(uint32_t bufferSize, uint32_t numberOfBuffers, std::vector<wgpu::Buffer> *buffers) {
    ...
    void TouchBuffers(uint32_t bufferSize, uint32_t beginIndex, uint32_t numBuffers, std::vector<wgpu::Buffer> &bufferSet) {
...
TEST_P(D3D12ResidencyTests, OvercommitSmallResources) {
    ...
    // Allocate 50 buffers that are 1MB each. These will all fit into the 100MB budget, so they should all be resident.
    ...
    // Allocate 100 buffers that are 1MB each. This will cause everything currently resident to be evicted.
    ...
    // Touch everything in bufferSet2 again to evict the buffer made resident in the previous operation.
...
// Check that resources existing on directly allocated heaps are made resident and evicted correctly.
TEST_P(D3D12ResidencyTests, OvercommitLargeResources) {
    ...
    // Allocate 10 buffers that are 5MB each. These will all fit into the 100MB budget, so they should all be resident.
    std::vector<wgpu::Buffer> bufferSet1;
    ...
    // Allocate 20 buffers that are 5MB each. This will cause everything currently resident to be evicted.
    std::vector<wgpu::Buffer> bufferSet2;
...
TEST_P(D3D12ResidencyTests, AsyncMappedBufferRead) {
    // Dawn only manages LOCAL_MEMORY. Mappable buffers exist in NON_LOCAL_MEMORY on discrete devices.
    ...
    // Allocate and touch 20 buffers that are 5MB each. This will cause everything currently resident to be evicted.
    ...
    // Touch 20 5MB buffers. This will cause everything currently resident to be evicted, however the mapped buffer should be locked resident.
    TouchBuffers(kSize, 0, 20, bufferSet);
...
TEST_P(D3D12ResidencyTests, AsyncMappedBufferWrite) {
    ...
    // Touch 20 5MB buffers. This would usually cause everything currently resident to be evicted, however the mapped buffer should be locked resident.
    TouchBuffers(kSize, 0, 20, bufferSet);
    ASSERT_TRUE(CheckIfBufferIsResident(buffer));
...
DAWN_INSTANTIATE_TEST(D3D12ResidencyTests, D3D12Backend({"restrict_d3d12_residency_budget_for_testing"}));