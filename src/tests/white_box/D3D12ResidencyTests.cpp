...
class D3D12ResidencyTests : public DawnTest {
  protected:
    void TestSetUp() override {
        ...
        // Restrict Dawn's budget to create an artificial budget of ~100MB.
        dawn_native::d3d12::Device* d3dDevice =
            reinterpret_cast<dawn_native::d3d12::Device*>(device.Get());
        d3dDevice->GetResidencyManager()->RestrictBudgetForTesting(100000000);  // 100MB
    }
...
// Budget has been artificially restricted such that ~100MB can be made resident at once.
...
// correctly. Budget has been artificially restricted such that ~100MB can be made resident at once.
...
// Check that calling MapReadAsync makes the buffer resident and keeps it locked resident. Budget
// has been artificially restricted such that ~100MB can be made resident at once.
...
// Check that calling MapWriteAsync makes the buffer resident and keeps it locked resident. Budget
// has been artificially restricted such that ~100MB can be made resident at once.
