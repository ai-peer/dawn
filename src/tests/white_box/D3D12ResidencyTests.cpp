...
TEST_P(D3D12ResidencyTests, OvercommitInASingleSubmit) {
    ...
    // Ensure the remainder of the buffer set is resident. Skip checking the first buffer because
    // it's unlikely <kRestrictedBudgetSize / kDirectlyAllocatedResourceSize> buffers were able to
    // fit perfectly into the budget.
    for (uint32_t i = numberOfBuffersToOvercommit + 1; i < bufferSet2.size(); i++) {
        EXPECT_TRUE(CheckIfBufferIsResident(bufferSet2[i]));
    }
}
