...
TEST_P(D3D12DescriptorHeapTests, EncodeReuseUBOOverflow) {
    ...
    std::array<float, 4> redColor = {1, 0, 0, 1};
    wgpu::Buffer uniformBuffer1 = utils::CreateBufferFromData(device, &redColor, sizeof(redColor),
                                                              wgpu::BufferUsage::Uniform);

    wgpu::BindGroup firstBindGroup = utils::MakeBindGroup(
        device, pipeline.GetBindGroupLayout(0), {{0, uniformBuffer1, 0, sizeof(redColor)}});

    std::vector<wgpu::BindGroup> bindGroups = {firstBindGroup};

    std::array<float, 4> blackColor = {0, 0, 0, 1};
    wgpu::Buffer uniformBuffer2 = utils::CreateBufferFromData(
        device, &blackColor, sizeof(blackColor), wgpu::BufferUsage::Uniform);

    for (uint32_t i = 1; i < heapSize; i++) {
        bindGroups.push_back(utils::MakeBindGroup(device, pipeline.GetBindGroupLayout(0),
                                                  {{0, uniformBuffer2, 0, sizeof(blackColor)}}));
    }
