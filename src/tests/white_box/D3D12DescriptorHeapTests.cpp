...
// Creates two heaps worth of bindgroups per |kNumOfSubmits| submits.
TEST_P(D3D12DescriptorHeapTests, ReuseUBOMultipleSubmits) {
    DAWN_SKIP_TEST_IF(!mD3DDevice->IsToggleEnabled(
        dawn_native::Toggle::UseD3D12SmallShaderVisibleHeapForTesting));
    ...
    wgpu::BindGroupLayout bgl = renderPipeline.GetBindGroupLayout(0);
    ...
    for (uint32_t i = 0; i < kNumOfSubmits; i++) {
        wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
        {
            ...
            for (uint32_t j = 0; j < heapSize; j++) {
                ...
                pass.SetBindGroup(0, utils::MakeBindGroup(device, bgl, {{0, uniformBuffer}}));
                pass.Draw(3, 1, 0, 0);
            }

            {
                ...
                pass.SetBindGroup(0, utils::MakeBindGroup(device, bgl, {{0, lastUniformBuffer}}));
                pass.Draw(3, 1, 0, 0);
            }
            ...
            poolSizes.insert(backendBgl->GetCbvSrvUavPoolSizeForTesting());
        }
    }

    // Verify the pool size has not increased.
    EXPECT_EQ(poolSizes.size(), 1u);
}
