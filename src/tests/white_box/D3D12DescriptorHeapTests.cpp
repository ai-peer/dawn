...
TEST_P(D3D12DescriptorHeapTests, EncodeManyUBOAndSamplers) {
...

    {
        ...
        const uint32_t viewHeapSize = GetShaderVisibleHeapSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);

        // "Small" view heap is always 2 x |samperHeap| size and encodes 3x the descriptors per
        // group. This means the count of heaps switches is determined by the total number of views to
        // encode. Compute the number of bindgroups to encode by counting the required views for
