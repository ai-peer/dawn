...
TEST_P(D3D12DescriptorHeapTests, EncodeUBOOverflowMultipleSubmit) {
    ...
    // Encode a heap worth of descriptors.
    {
        ...
        for (uint32_t i = 0; i < heapSize - 1; i++) {
            std::array<float, 4> fillColor = GetSolidColor(i);
            wgpu::Buffer uniformBuffer = utils::CreateBufferFromData(
...
TEST_P(D3D12DescriptorHeapTests, EncodeReuseUBOOverflow) {
    ...
    for (uint32_t i = 1; i < heapSize + 1; i++) {
        const std::array<float, 4>& fillColor = GetSolidColor(i);
        wgpu::Buffer uniformBuffer = utils::CreateBufferFromData(
            device, &fillColor, sizeof(fillColor), wgpu::BufferUsage::Uniform);
    ...
    wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
    {
        ...
        // Encode the first bindgroup again.
...
// Verify encoding a heaps worth of bindgroups plus one more in the first submit then a different
// bindgroup in the second submit.
...
TEST_P(D3D12DescriptorHeapTests, EncodeReuseUBOMultipleSubmits) {
    ...
    // Encode a heap worth of descriptors along with the first bindgroup.
    {
        wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
        {
            wgpu::RenderPassEncoder pass = encoder.BeginRenderPass(&renderPass.renderPassInfo);
