...
// Allocates |kNumOfBuffers| twice using the same heaps.
...
TEST_P(D3D12ResourceHeapTests, PoolHeapsInPendingSubmit) {
    ...
    // Count by heap (vs number of buffers) to ensure we have exactly |kNumOfHeaps| worth of
    // buffers. Otherwise, the heap may be reused from sub-allocation if not full.
