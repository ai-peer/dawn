...
class QuerySetValidationTest : public ValidationTest {
  protected:
    void SetUp() override {
        ...
        deviceWithExtensions =
            CreateDeviceFromAdapter(adapter, {"pipeline_statistics_query", "timestamp_query"});
    }
...
TEST_F(QuerySetValidationTest, UnnecessaryPipelineStatistics) {
    ...
    {
        descriptor = CreateQuerySetDescriptor(wgpu::QueryType::Timestamp, 1, &pipelineStatistics);
