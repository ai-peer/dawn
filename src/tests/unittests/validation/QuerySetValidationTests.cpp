...
// This test is only valid when compiling for 32-bits mode.
// To check that the size of the resolved data overflow the buffer size, but it bypass the
// validation due to the result of queryCount * sizeof(uint64) overflow in 32-bits mode.
TEST_F(ResolveQuerySetValidationTest, OverflowBufferOn32Bits) {
    ...
    bool is32Bits =
        static_cast<uint64_t>(kQueryCount * sizeof(uint64_t)) / sizeof(uint64_t) != kQueryCount;
