...
TEST_F(BindGroupLayoutCompatibilityTest, RWStorageInBGLWithROStorageInShader) {
    ...
    // Set up the bind group.
    wgpu::Buffer storageBuffer1 = CreateBuffer(kBufferSize, wgpu::BufferUsage::Storage);
    wgpu::Buffer storageBuffer2 = CreateBuffer(kBufferSize, wgpu::BufferUsage::Storage);
    wgpu::BindGroup bindGroup = utils::MakeBindGroup(
        device, bindGroupLayout,
        {{0, storageBuffer1, 0, kBindingSize}, {1, storageBuffer2, 0, kBindingSize}});

    TestRenderPassBindGroup(bindGroup, &bindGroupLayout, true);

    TestComputePassBindGroup(bindGroup, &bindGroupLayout, true);
}
...
TEST_F(BindGroupLayoutCompatibilityTest, ROStorageInBGLWithRWStorageInShader) {
    ...
    TestRenderPassBindGroup(bindGroup, &bindGroupLayout, false);

    TestComputePassBindGroup(bindGroup, &bindGroupLayout, false);
}
