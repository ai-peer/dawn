    ...
    std::tuple<std::vector<dawn::BindGroupLayout>, dawn::RenderPipeline> SetUpLayoutsAndPipeline(
        ...
        for (const auto& layout : layouts) {
            ...
            for (const auto& binding : layout) {
                bindings[b] = {b, dawn::ShaderStage::Fragment, binding, false};
                b++;
            }
        ...
        for (const auto& layout : layouts) {
            ...
            for (const auto& binding : layout) {
                ...
                ss << l << " { vec2 set" << l << "_binding" << b << "; };\n";
                b++;
            }
            l++;
        }

        ss << "layout(location = 0) out uvec4 fragColor;\n";
        ss << "void main() {}\n";
...
TEST_F(SetBindGroupPersistenceValidationTest, NotVulkanInheritance) {
    ...
    dawn::BindGroup bindGroupB1 = utils::MakeBindGroup(
        device, bindGroupLayoutsB[1],
        {{0, uniformBuffer, 0, kBindingSize}, {1, storageBuffer, 0, kBindingSize}});
