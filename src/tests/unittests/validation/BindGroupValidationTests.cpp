...
TEST_F(BindGroupValidationTest, StorageTextureViewLayerCount) {
    ...
    wgpu::TextureDescriptor descriptorNoMip;
    descriptorNoMip.dimension = wgpu::TextureDimension::e2D;
    descriptorNoMip.size = {16, 16, 1};
    descriptorNoMip.sampleCount = 1;
    descriptorNoMip.mipLevelCount = 1;
    descriptorNoMip.usage = wgpu::TextureUsage::StorageBinding;
    descriptorNoMip.format = wgpu::TextureFormat::RGBA8Uint;

    wgpu::TextureDescriptor descriptorMip;
    descriptorMip.dimension = wgpu::TextureDimension::e2D;
    descriptorMip.size = {16, 16, 1};
    descriptorMip.sampleCount = 1;
    descriptorMip.mipLevelCount = 3;
    descriptorMip.usage = wgpu::TextureUsage::StorageBinding;
    descriptorMip.format = wgpu::TextureFormat::RGBA8Uint;

    wgpu::Texture textureNoMip = device.CreateTexture(&descriptorNoMip);
    wgpu::Texture textureMip = device.CreateTexture(&descriptorMip);
    ...
    // Setting a storage texture view with only one mip level on a texture of multi mip levels
    {
        wgpu::TextureViewDescriptor viewDesc = {};
        ...
        // Setting texture view with other lod is valid
        viewDesc.baseMipLevel = 2;
        view = textureMip.CreateView(&viewDesc);
        utils::MakeBindGroup(device, layout, {{0, view}});
    }

    // Texture view with more than one mip level is invalid
    {
        wgpu::TextureViewDescriptor viewDesc = {};
        viewDesc.aspect = wgpu::TextureAspect::All;
        viewDesc.dimension = wgpu::TextureViewDimension::e2D;
        ...
        // Setting texture view with lod 1 and 2 is valid
