...
TEST_F(BindGroupValidationTest, BufferUsageReadonlySSBO) {
    dawn::BindGroupLayout layout = utils::MakeBindGroupLayout(
        device, {{0, dawn::ShaderStage::Fragment, dawn::BindingType::ReadonlyStorageBuffer}});
...
TEST_F(BindGroupLayoutValidationTest, DynamicAndTypeCompatibility) {
    ...
    utils::MakeBindGroupLayout(
        device, {
                    {0, dawn::ShaderStage::Compute, dawn::BindingType::ReadonlyStorageBuffer, true},
                });
...
TEST_F(BindGroupLayoutValidationTest, DynamicBufferNumberLimit) {
    ...
    std::vector<dawn::BindGroupLayoutBinding> maxReadonlyStorageDB;
    ...
    for (uint32_t i = 0; i < kMaxDynamicStorageBufferCount; ++i) {
        maxReadonlyStorageDB.push_back(
            {i, dawn::ShaderStage::Compute, dawn::BindingType::ReadonlyStorageBuffer, true});
    }
    ...
    {
        maxReadonlyStorageDB.push_back({kMaxDynamicStorageBufferCount, dawn::ShaderStage::Compute,
                                        dawn::BindingType::ReadonlyStorageBuffer, true});
