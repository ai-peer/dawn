...
TEST_F(BindGroupValidationTest, WrongBindings) {
    ...
    ASSERT_DEVICE_ERROR(utils::MakeBindGroup(device, layout, {{390, mSampler}}));
}
...
TEST_F(BindGroupLayoutValidationTest, BindGroupLayoutEntryUnbounded) {
    // Checks that a large binding number is valid.
    utils::MakeBindGroupLayout(device, {{298, wgpu::ShaderStage::Vertex,
                                         wgpu::BindingType::UniformBuffer}});

    // Checks that uint32_t max is valid.
    utils::MakeBindGroupLayout(device, {{std::numeric_limits<uint32_t>::max(), wgpu::ShaderStage::Vertex,
                                         wgpu::BindingType::UniformBuffer}});
}

// Test that there can be an unbounded number of bindings per group.
TEST_F(BindGroupLayoutValidationTest, BindGroupLayoutManyBindings) {
    static constexpr uint32_t kNumBindings = 302;

    wgpu::BindGroupLayoutEntry entries[kNumBindings + 1];

    for (uint32_t i = 0; i < kNumBindings + 1; i++) {
...
TEST_F(BindGroupLayoutValidationTest, DynamicBufferNumberLimit) {
    ...
    {
        maxUniformDB.push_back({kMaxDynamicUniformBuffersPerPipelineLayout, wgpu::ShaderStage::Compute,
    ...
    {
        maxStorageDB.push_back({kMaxDynamicStorageBuffersPerPipelineLayout, wgpu::ShaderStage::Compute,
    ...
    {
        maxReadonlyStorageDB.push_back({kMaxDynamicStorageBuffersPerPipelineLayout, wgpu::ShaderStage::Compute,
