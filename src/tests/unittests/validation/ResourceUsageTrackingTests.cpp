...
namespace {
    ...
    TEST_F(ResourceUsageTrackingTest, TextureWithMultipleSetBindGroupsOnSameIndex) {
        // Test render pass
        {
            ...
            // Set bind group on the same index twice. The second one overwrites the first one.
            // No texture is used as both sampeld and output attachment in the same pass. But the
            ...
            // Set bind group on the same index twice. The second one overwrites the first one.
            // texture0 is used as both sampeld and output attachment in the same pass
            {
                wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
                wgpu::RenderPassEncoder pass = encoder.BeginRenderPass(&renderPass);
                pass.SetBindGroup(0, bg1);
