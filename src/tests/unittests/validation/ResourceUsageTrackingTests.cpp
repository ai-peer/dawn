...
namespace {
    ...
    // render pass. It is valid in a pass for compute, but it is invalid in a dispatch.
    ...
    TEST_F(ResourceUsageTrackingTest, TextureWithReadAndWriteUsage) {
        ...
        // Test compute pass
        {
            ...
            // It is valid to use the texture as both sampeld and writeonly storage in a single
            // compute pass
            {
                wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
                wgpu::ComputePassEncoder pass = encoder.BeginComputePass();
            ...
            // It is invalid to use the texture as both sampeld and writeonly storage in a single
            // dispatch
            {
                wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
                wgpu::ComputePassEncoder pass = encoder.BeginComputePass();
