...
namespace {
    ...
    TEST_F(ResourceUsageTrackingTest, TextureWithReadAndWriteUsageInDifferentPasses) {
        ...
        // Test compute pass
        {
            ...
            // Use the texture as both readonly and writeonly storage in different passes
            wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
        ...
        // Test compute pass and render pass mixed together with resource dependency
        {
            ...
            // Use the same texture as writeonly and readonly storage in compute pass and render
            // pass respectively
            wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
    ...
    // Test that using the same texture as both readable and writable in the different
    // draws/dispatches is disallowed
    TEST_F(ResourceUsageTrackingTest, TextureWithReadAndWriteUsageOnDifferentDrawsOrDispatches) {
        ...
        wgpu::TextureView view = texture.CreateView();
        // wgpu::Texture t1 = CreateTexture(wgpu::TextureUsage::Sampled |
        // wgpu::TextureUsage::Storage,
        //                                  wgpu::TextureFormat::RGBA8Unorm);
        // wgpu::TextureView v1 = t1.CreateView();

        // Use the texture as both sampled and writable storage in different draws in the same
        // render pass
        {
            ...
            // Create a passthrough render pipeline. Note that bind groups can have more bindings
            // than pipeline.
            wgpu::RenderPipeline rp = CreatePassthroughRenderPipeline();
        ...
        // compute pass
        {
            ...
            // Create a passthrough compute pipeline. Note that bind groups can have more bindings
            // than pipeline.
            wgpu::ComputePipeline cp = CreatePassthroughComputePipeline();
