...
class MinBufferSizeTestsBase : public ValidationTest {
  public:
    ...
    wgpu::ComputePipeline CreateComputePipeline(const wgpu::BindGroupLayout& layout,
        ...
        if (layout != nullptr) {
            csDesc.layout = utils::MakeBasicPipelineLayout(device, &layout);
        }
    ...
    wgpu::ComputePipeline CreateDefaultComputePipeline(const std::string& shader) {
        return CreateComputePipeline(nullptr, shader);
    }

    // Creates render pipeline give na layout and shaders
    wgpu::RenderPipeline CreateRenderPipeline(const wgpu::BindGroupLayout& layout,
        ...
        if (layout != nullptr) {
            pipelineDescriptor.layout = utils::MakeBasicPipelineLayout(device, &layout);
        }
    ...
    wgpu::RenderPipeline CreateDefaultRenderPipeline(const std::string& vertexShader,
                                                     const std::string& fragShader) {
        return CreateRenderPipeline(nullptr, vertexShader, fragShader);
    }
    ...
    // Create a bind group with given binding sizes for each entry
...
// Fail if layout given has minimum sizes smaller than shader requirements
