...
static void ToMockDevicePopErrorScopeCallback(DawnErrorType type, const char* message, void* userdata) {
...
TEST_F(ErrorScopeValidationTest, Success) {
    ...
    EXPECT_CALL(*mockDevicePopErrorScopeCallback,
                Call(DAWN_ERROR_TYPE_NO_ERROR, _, this)).Times(1);
...
TEST_F(ErrorScopeValidationTest, CatchesError) {
    ...
    EXPECT_CALL(*mockDevicePopErrorScopeCallback,
                Call(DAWN_ERROR_TYPE_VALIDATION, _, this)).Times(1);
    device.PopErrorScope(ToMockDevicePopErrorScopeCallback, this);
}
...
TEST_F(ErrorScopeValidationTest, ErrorBubbles) {
    ...
    EXPECT_CALL(*mockDevicePopErrorScopeCallback,
                Call(DAWN_ERROR_TYPE_VALIDATION, _, this + 1)).Times(1);
...
TEST_F(ErrorScopeValidationTest, HandledErrorsStopBubbling) {
    ...
    EXPECT_CALL(*mockDevicePopErrorScopeCallback,
                Call(DAWN_ERROR_TYPE_NO_ERROR, _, this + 1)).Times(1);
    device.PopErrorScope(ToMockDevicePopErrorScopeCallback, this + 1);
}
...
TEST_F(ErrorScopeValidationTest, PushPopBalanced) {
    ...
    {
        ASSERT_DEVICE_ERROR(device.PopErrorScope(ToMockDevicePopErrorScopeCallback, this));
    }
    ...
    // Too many pops
    {
        ...
        ASSERT_DEVICE_ERROR(device.PopErrorScope(ToMockDevicePopErrorScopeCallback, this + 2));
    }
}

// Test that error scopes do not call their callbacks until after an enclosed Queue::Submit completes
...
// Test that parent error scopes do not call their callbacks until after an enclosed Queue::Submit completes
...
TEST_F(ErrorScopeValidationTest, DeviceDestroyedBeforeCallback) {
    ...
    EXPECT_CALL(*mockDevicePopErrorScopeCallback,
                Call(DAWN_ERROR_TYPE_UNKNOWN, _, this)).Times(1);
    device = nullptr;
}
