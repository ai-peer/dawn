...
// causes validation error. WebGPU doesn't support using cube map texture views and cube map array
// texture views as storage textures.
...
// Verify that declaring a storage texture dimension that is not supported in WebGPU in shader
// causes validation error with SPVC. WebGPU doesn't support using cube map texture views and cube
// map array texture views as storage textures.
TEST_F(StorageTextureValidationTests, UnsupportedTextureViewDimensionInShader_SPVC) {
    const std::vector<const char*> forceEnabledToggles(1, "use_spvc");
    wgpu::Device deviceWithSPVC = CreateDeviceFromAdapter(adapter, {}, forceEnabledToggles);
    ...
    for (wgpu::BindingType bindingType : kSupportedStorageTextureBindingTypes) {
        for (wgpu::TextureViewDimension dimension : kUnsupportedTextureViewDimensions) {
            std::string computeShader =
                CreateComputeShaderWithStorageTexture(bindingType, kFormat, dimension);
            ASSERT_DEVICE_ERROR(utils::CreateShaderModule(
                deviceWithSPVC, utils::SingleShaderStage::Compute, computeShader.c_str()));
        }
    }
}
