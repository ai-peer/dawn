...
TEST_F(StorageTextureValidationTests, BindGroupLayoutTextureDimensionMatchesShaderDeclaration) {
    constexpr wgpu::TextureFormat kStorageTextureFormat = wgpu::TextureFormat::R32Float;
    const std::array<wgpu::TextureViewDimension, 6> kAllDimensions = {
...
TEST_F(StorageTextureValidationTests, StorageTextureCannotHaveDynamicOffsets) {
    for (wgpu::BindingType storageBindingType : kSupportedStorageTextureBindingTypes) {
        wgpu::BindGroupLayoutBinding bindGroupLayoutBinding = {0, wgpu::ShaderStage::Compute,
                                                               storageBindingType};
...
// Verify that only a texture can be used as a read-only or write-only storage texture in a bind
// group.
...
// created with wgpu::TextureUsage::STORAGE.
...
TEST_F(StorageTextureValidationTests, StorageTextureFormatInBindGroup) {
    for (wgpu::BindingType storageBindingType : kSupportedStorageTextureBindingTypes) {
        wgpu::BindGroupLayoutBinding defaultBindGroupLayoutBinding = {0, wgpu::ShaderStage::Compute,
                                                                      storageBindingType};
