...
TEST_F(ShaderModuleValidationTest, kMaximumShaderIOLocations) {
    auto generateShaderForTest = [](uint32_t maximumOutputLocation, wgpu::ShaderStage shaderStage) {
        ...
        switch (shaderStage) {
            case wgpu::ShaderStage::Vertex: {
                ...
                for (uint32_t location = 0; location <= maximumOutputLocation; ++location) {
                    stream << "shaderIO.var" << location << " = " << location << ".0;" << std::endl;
                }
                stream << "return shaderIO;}";
            } break;
