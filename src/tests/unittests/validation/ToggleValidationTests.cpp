...
TEST_F(ToggleValidationTest, QueryToggleInfo) {
    ...
    // Query with a nullptr
    {
        const dawn_native::ToggleInfo* toggleInfo = instance->GetToggleInfo(nullptr);
        ASSERT_EQ(nullptr, toggleInfo);
    }
}
...
TEST_F(ToggleValidationTest, OverrideToggleUsage) {
    ...
    // Use nullptr as toggle name when creating the device
    {
        dawn_native::DeviceDescriptor descriptor;
        descriptor.forceEnabledToggles.push_back(nullptr);
