...
[[stage(compute), workgroup_size(1)]] fn main() {
    ...
    buf.data[10] = u32(c10);
        ...
        // descriptor.size = static_cast<uint64_t>(11 * sizeof(uint32_t));
...
TEST_F(ComputePipelineOverridableConstantsValidationTest, ConstantsIdentifierLookUp) {
    ...
    }
    {
        ...
        std::vector<wgpu::ConstantEntry> constants;
        constants.push_back({nullptr, "1000", 0});
        TestCreatePipeline(constants);
    }
    {
        // Error: constant name not exits
        std::vector<wgpu::ConstantEntry> constants;
        constants.push_back({nullptr, "c99", 0});
        ASSERT_DEVICE_ERROR(TestCreatePipeline(constants));
    }
    {
        // Error: constant numeric id not explicitly specified
        std::vector<wgpu::ConstantEntry> constants;
        constants.push_back({nullptr, "9999", 0});
        ASSERT_DEVICE_ERROR(TestCreatePipeline(constants));
    }
    {
        // Error: constant numeric id not explicitly specified
        // But could be implicilty assigned to one of the constants
        std::vector<wgpu::ConstantEntry> constants;
        constants.push_back({nullptr, "1", 0});
        // TODO(tint:1155): tint needs ways to differentiate whether a pipeline constant id is
