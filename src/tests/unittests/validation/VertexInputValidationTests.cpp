...
// if vertex buffer stride is not zeor.
...
TEST_F(VertexInputTest, SetSameShaderLocation) {
    // Control case, setting different shader locations in two attribtes
    utils::ComboVertexInputDescriptor state;
    state.numBuffers = 1;
    state.cBuffers[0].numAttributes = 2;
    state.cAttributes[1].shaderLocation = 1;
    ...
    // Test same shader location in two attribtes in the same buffer
    ...
    // Test same shader location in two attribtes in different buffers
    state.numBuffers = 2;
    state.cBuffers[0].numAttributes = 1;
    state.cBuffers[1].numAttributes = 1;
