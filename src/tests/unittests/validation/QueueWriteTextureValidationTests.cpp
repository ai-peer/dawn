...
namespace {
    ...
    TEST_F(QueueWriteTextureValidationTest, Success) {
        ...
        // Different copies, including some that touch the OOB condition
        {
            ...
            TestWriteTexture(dataSize, dataSize - 4, 256, WGPU_STRIDE_UNDEFINED, destination, 0,
                             {0, 0, 0}, {1, 1, 1});
        }
        ...
        // Empty copies are valid
        {
            ...
            TestWriteTexture(dataSize, 0, 0, WGPU_STRIDE_UNDEFINED, destination, 0, {0, 0, 0},
                             {0, 0, 1});
            ...
            TestWriteTexture(dataSize, dataSize, 0, WGPU_STRIDE_UNDEFINED, destination, 0,
                             {0, 0, 0}, {0, 0, 1});
            // An empty copy touching the side of the texture
            TestWriteTexture(dataSize, 0, 0, 0, destination, 0, {16, 16, 0}, {0, 0, 1});
            TestWriteTexture(dataSize, 0, 0, WGPU_STRIDE_UNDEFINED, destination, 0, {16, 16, 0},
                             {0, 0, 1});
            // An empty copy with depth = 1 and bytesPerRow > 0
            TestWriteTexture(dataSize, 0, 256, 0, destination, 0, {0, 0, 0}, {0, 0, 1});
            TestWriteTexture(dataSize, 0, 256, WGPU_STRIDE_UNDEFINED, destination, 0, {0, 0, 0},
                             {0, 0, 1});
    ...
    TEST_F(QueueWriteTextureValidationTest, BytesPerRowConstraints) {
        ...
        // bytesPerRow = 0 or WGPU_STRIDE_UNDEFINED
        {
            // copyHeight > 1
            ASSERT_DEVICE_ERROR(
                TestWriteTexture(128, 0, 0, 7, destination, 0, {0, 0, 0}, {3, 7, 1}));
            TestWriteTexture(128, 0, 0, 7, destination, 0, {0, 0, 0}, {0, 7, 1});
            ASSERT_DEVICE_ERROR(TestWriteTexture(128, 0, WGPU_STRIDE_UNDEFINED, 7, destination, 0,
                                                 {0, 0, 0}, {0, 7, 1}));
            ...
            ASSERT_DEVICE_ERROR(TestWriteTexture(128, 0, WGPU_STRIDE_UNDEFINED, 1, destination, 0,
                                                 {0, 0, 0}, {0, 1, 2}));

            // copyHeight = 1 and copyDepth = 1
            EXPECT_DEPRECATION_WARNING(
                TestWriteTexture(128, 0, 0, 1, destination, 0, {0, 0, 0}, {3, 1, 1}));
            TestWriteTexture(128, 0, WGPU_STRIDE_UNDEFINED, 1, destination, 0, {0, 0, 0},
                             {3, 1, 1});
        }
    ...
    TEST_F(QueueWriteTextureValidationTest, RowsPerImageConstraints) {
        ...
        // rowsPerImage is WGPU_STRIDE_UNDEFINED
        TestWriteTexture(dataSize, 0, 256, WGPU_STRIDE_UNDEFINED, destination, 0, {0, 0, 0},
                         {4, 4, 1});
    ...
    TEST_F(QueueWriteTextureValidationTest, WriteToStencilAspect) {
        ...
        // It is valid to write into the stencil aspect of depth24plus-stencil8
        {
            ...
            TestWriteTexture(dataSize, 0, bytesPerRow, WGPU_STRIDE_UNDEFINED, destination, 0,
