...
class DrawIndexedValidationTest : public ValidationTest {
  protected:
    void SetUp() override {
        ...
        indexBuffer = utils::CreateBufferFromData<uint32_t>(device, wgpu::BufferUsage::Index,
                                                            {0, 1, 2, 3, 1, 2});
    }
...
TEST_F(DrawIndexedValidationTest, IndexOutOfBounds) {
    ...
    // firstIndex out of bounds but indexCount is 0, still valid, maybe no-op
    // Some agreements: https://github.com/gpuweb/gpuweb/issues/955
    TestDrawIndexedIndexBound(utils::Expectation::Success, 0, 6);
