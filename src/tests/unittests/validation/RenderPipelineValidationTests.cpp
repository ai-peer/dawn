...
TEST_F(InterStageVariableMatchingValidationTest, DifferentInterpolationAttributeAtSameLocation) {
    ...
    std::vector<InterpolationAttribute> validInterpolationAttributes;
    for (uint8_t interpolationTypeInt = static_cast<uint8_t>(InterpolationType::First);
         interpolationTypeInt < static_cast<uint8_t>(InterpolationType::Last);
         ++interpolationTypeInt) {
        InterpolationType interpolationType = static_cast<InterpolationType>(interpolationTypeInt);
        ...
        if (interpolationType == InterpolationType::None ||
            ...
            validInterpolationAttributes.push_back(
                {interpolationType, InterpolationSampling::None});
            continue;
        }
        for (uint8_t interpolationSamplingTypeInt =
                 static_cast<uint8_t>(InterpolationSampling::First);
             interpolationSamplingTypeInt < static_cast<uint8_t>(InterpolationSampling::Last);
             ++interpolationSamplingTypeInt) {
            validInterpolationAttributes.push_back(
                {interpolationType,
                 static_cast<InterpolationSampling>(interpolationSamplingTypeInt)});
        }
    }
    ...
    auto IsInterpolationAttributeMatch = [](const InterpolationAttribute& attribute1,
    ...
    for (uint32_t vertexModuleIndex = 0; vertexModuleIndex < validInterpolationAttributes.size();
        ...
        for (uint32_t fragmentModuleIndex = 0;
            ...
            wgpu::ShaderModule fragmentModule = fragmentModules[fragmentModuleIndex];
            bool shouldSuccess =
                IsInterpolationAttributeMatch(validInterpolationAttributes[vertexModuleIndex],
