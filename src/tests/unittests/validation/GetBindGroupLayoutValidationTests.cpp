...
TEST_F(GetBindGroupLayoutTests, Multisampled) {
    ...
    //     void main() {})");
    //     EXPECT_EQ(device.CreateBindGroupLayout(&desc).Get(),
    //     pipeline.GetBindGroupLayout(0).Get());
    // }
}
...
// Test it is invalid to have duplicate bindings in the shaders.
...
// Test it is an error to query a bind group that is not present in the shader.
TEST_F(GetBindGroupLayoutTests, UnusedIndex) {
    ...
    pipeline.GetBindGroupLayout(0);                       // Used
    ASSERT_DEVICE_ERROR(pipeline.GetBindGroupLayout(1));  // Not used.
    pipeline.GetBindGroupLayout(2);                       // Used.
    ASSERT_DEVICE_ERROR(pipeline.GetBindGroupLayout(3));  // Not used
}
