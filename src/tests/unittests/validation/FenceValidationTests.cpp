...
class FenceValidationTest : public ValidationTest {
    protected:
        ...
        void TestOnCompletion(dawn::Fence fence, uint64_t value, dawnFenceCompletionStatus status) {
            ...
            dawnCallbackUserdata userdata = static_cast<dawnCallbackUserdata>(reinterpret_cast<uintptr_t>(expectation));
...
TEST_F(FenceValidationTest, QueueWait) {
    dawn::FenceDescriptor descriptor;
    descriptor.initialValue = 1;
    dawn::Fence fence = device.CreateFence(&descriptor);

    // Can wait on current (initial) signaled value
    queue.Wait(fence, 1);

    // Cannot wait on value greater than signaled value
    ASSERT_DEVICE_ERROR(queue.Wait(fence, 2));

    queue.Signal(fence, 4);
    // Can wait on value less than signaled value
    queue.Wait(fence, 3);
    // Can wait on updated signaled value
    queue.Wait(fence, 4);
}
