...
TEST_F(RenderPassDescriptorValidationTest, MultisampledColorAttachmentAndResolveTarget) {
    ...
    // It is not allowed to use a resolve target whose size is different from the color attachment.
    {
...

        {
            ...
            dawn::TextureView resolveTextureView2 =
                resolveTexture2.CreateTextureView(&firstMipLevelDescriptor);
...

        {
            ...
            dawn::TextureView resolveTextureView2 =
                resolveTexture2.CreateTextureView(&secondMipLevelDescriptor);
