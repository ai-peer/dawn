...
namespace {
    ...
    class DrawVertexAndIndexBufferOOBValidationTests : public ValidationTest {
      public:
        ...
        // Parameters for testing vertex-step-mode vertex buffer
        struct VertexBufferParams {
            uint32_t vertexBufferStride;
            uint32_t vertexBufferSize;              // Size for creating vertex buffer
            uint32_t vertexBufferOffsetForEncoder;  // Offset for SetVertexBuffer in encoder
            uint32_t vertexBufferSizeForEncoder;    // Size for SetVertexBuffer in encoder
            uint32_t maxValidVertexNumber;  // max number of {vertexCount + firstVertex} for this
                                            // set of parameters
        };

        // Parameters for testing instance-step-mode vertex buffer
        struct InstanceBufferParams {
            uint32_t instanceBufferStride;
            uint32_t instanceBufferSize;              // Size for creating vertex buffer
            uint32_t instanceBufferOffsetForEncoder;  // Offset for SetVertexBuffer in encoder
            uint32_t instanceBufferSizeForEncoder;    // Size for SetVertexBuffer in encoder
            uint32_t maxValidInstanceNumber;  // max number of {instanceCount + firstInstance} for
        ...
        void SetUp() override {
            ...
            vertexParamsList = {
                ...
                // Non-zero size
                {(2 * kFloat32x4Stride), 4 * (2 * kFloat32x4Stride), 0, 3 * (2 * kFloat32x4Stride),
                 3},
                // Non-zero offset and size
            ...
            instanceParamsList = {
                ...
                // Non-zero size
                {(3 * kFloat32x2Stride), 6 * (3 * kFloat32x2Stride), 0, 5 * (3 * kFloat32x2Stride),
                 5},
                // Non-zero offset and size
        ...
        const std::vector<VertexBufferParams>& GetBasicVertexBufferParams() const {
            return vertexParamsList;
        }

        const std::vector<InstanceBufferParams>& GetBasicInstanceBufferParams() const {
            return instanceParamsList;
        }

        const std::vector<IndexBufferParams>& GetBasicIndexBufferParams() const {
            return indexParamsList;
        }
      ...
      private:
        ...
        std::vector<IndexBufferParams> indexParamsList;
        std::vector<VertexBufferParams> vertexParamsList;
        std::vector<InstanceBufferParams> instanceParamsList;
    };
    ...
    TEST_F(DrawVertexAndIndexBufferOOBValidationTests, DrawVertexBufferOutOfBoundWithoutInstance) {
        for (VertexBufferParams params : GetBasicVertexBufferParams()) {
            // Create a render pipeline without instance step mode buffer
            wgpu::RenderPipeline pipeline = CreateBasicRenderPipeline(params.vertexBufferStride);

            // Build vertex buffer for 3 vertices
            wgpu::Buffer vertexBuffer = CreateBuffer(params.vertexBufferSize);
            VertexBufferList vertexBufferList = {{0, vertexBuffer,
                                                  params.vertexBufferOffsetForEncoder,
                                                  params.vertexBufferSizeForEncoder}};

            uint32_t n = params.maxValidVertexNumber;
    ...
    TEST_F(DrawVertexAndIndexBufferOOBValidationTests, DrawVertexBufferOutOfBoundWithInstance) {
        for (VertexBufferParams vertexParams : GetBasicVertexBufferParams()) {
            for (InstanceBufferParams instanceParams : GetBasicInstanceBufferParams()) {
                // Create pipeline with given buffer stride
                wgpu::RenderPipeline pipeline = CreateBasicRenderPipelineWithInstance(
                    vertexParams.vertexBufferStride, instanceParams.instanceBufferStride);

                // Build vertex buffer
                wgpu::Buffer vertexBuffer = CreateBuffer(vertexParams.vertexBufferSize);
                wgpu::Buffer instanceBuffer = CreateBuffer(instanceParams.instanceBufferSize);

                VertexBufferList vertexBufferList = {
                    {0, vertexBuffer, vertexParams.vertexBufferOffsetForEncoder,
                     vertexParams.vertexBufferSizeForEncoder},
                    {1, instanceBuffer, instanceParams.instanceBufferOffsetForEncoder,
                     instanceParams.instanceBufferSizeForEncoder},
                };

                uint32_t vert = vertexParams.maxValidVertexNumber;
                uint32_t inst = instanceParams.maxValidInstanceNumber;
    ...
    TEST_F(DrawVertexAndIndexBufferOOBValidationTests, DrawIndexedIndexBufferOOB) {
        ...
        for (IndexBufferParams params : GetBasicIndexBufferParams()) {
