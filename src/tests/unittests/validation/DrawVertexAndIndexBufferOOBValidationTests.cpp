...
namespace {
    ...
    class DrawVertexAndIndexBufferOOBValidationTests : public ValidationTest {
      public:
        ...
        struct IndexBufferDesc {
            ...
            wgpu::IndexFormat indexFormat;
            uint64_t offset = 0;
            uint64_t size = wgpu::kLimitU64Undefined;
        };
        ...
        const std::vector<IndexBufferParams> kIndexParamsList = {
            {wgpu::IndexFormat::Uint32, 12 * sizeof(uint32_t), 0, wgpu::kLimitU64Undefined, 12},
            {wgpu::IndexFormat::Uint32, 13 * sizeof(uint32_t), sizeof(uint32_t),
             wgpu::kLimitU64Undefined, 12},
            ...
            {wgpu::IndexFormat::Uint16, 12 * sizeof(uint16_t), 0, wgpu::kLimitU64Undefined, 12},
            {wgpu::IndexFormat::Uint16, 13 * sizeof(uint16_t), sizeof(uint16_t),
             wgpu::kLimitU64Undefined, 12},
        ...
        const std::vector<VertexBufferParams> kVertexParamsList = {
            ...
            {kFloat32x4Stride, 3 * kFloat32x4Stride, 0, wgpu::kLimitU64Undefined, 3},
            // Non-zero offset
            {kFloat32x4Stride, 4 * kFloat32x4Stride, kFloat32x4Stride, wgpu::kLimitU64Undefined, 3},
            // Non-default size
            ...
            {(2 * kFloat32x4Stride), 3 * (2 * kFloat32x4Stride), 0, wgpu::kLimitU64Undefined, 3},
            // Non-zero offset
            {(2 * kFloat32x4Stride), 4 * (2 * kFloat32x4Stride), (2 * kFloat32x4Stride),
             wgpu::kLimitU64Undefined, 3},
            // Non-default size
        ...
        const std::vector<VertexBufferParams> kInstanceParamsList = {
            ...
            {kFloat32x2Stride, 5 * kFloat32x2Stride, 0, wgpu::kLimitU64Undefined, 5},
            // Non-zero offset
            {kFloat32x2Stride, 6 * kFloat32x2Stride, kFloat32x2Stride, wgpu::kLimitU64Undefined, 5},
            // Non-default size
            ...
            {(3 * kFloat32x2Stride), 5 * (3 * kFloat32x2Stride), 0, wgpu::kLimitU64Undefined, 5},
            // Non-zero offset
            {(3 * kFloat32x2Stride), 6 * (3 * kFloat32x2Stride), (3 * kFloat32x2Stride),
             wgpu::kLimitU64Undefined, 5},
            // Non-default size
    ...
    TEST_F(DrawVertexAndIndexBufferOOBValidationTests, DrawBasic) {
        ...
        {
            // Implicit size
            VertexBufferList vertexBufferList = {{0, vertexBuffer, 0, wgpu::kLimitU64Undefined}};
    ...
    TEST_F(DrawVertexAndIndexBufferOOBValidationTests, DrawIndexedIndexBufferOOB) {
        ...
        for (IndexBufferParams params : kIndexParamsList) {
            ...
            VertexBufferList vertexBufferList = {{0, vertexBuffer, 0, wgpu::kLimitU64Undefined},
                                                 {1, instanceBuffer, 0, wgpu::kLimitU64Undefined}};
    ...
    TEST_F(DrawVertexAndIndexBufferOOBValidationTests, ZeroArrayStrideVertexBufferOOB) {
        ...
        const std::vector<VertexBufferParams> kVertexParamsListForZeroStride = {
            // Control case
            {0, 28, 0, wgpu::kLimitU64Undefined, 1},
            // Non-zero offset
            {0, 28, 4, wgpu::kLimitU64Undefined, 0},
            {0, 28, 28, wgpu::kLimitU64Undefined, 0},
            // Non-default size
            {0, 28, 0, 28, 1},
            {0, 28, 0, 27, 0},
            // Non-zero offset and size
            {0, 32, 4, 28, 1},
            {0, 31, 4, 27, 0},
            {0, 31, 4, wgpu::kLimitU64Undefined, 0},
        };

        const std::vector<VertexBufferParams> kInstanceParamsListForZeroStride = {
            // Control case
            {0, 20, 0, wgpu::kLimitU64Undefined, 1},
            // Non-zero offset
            {0, 24, 4, wgpu::kLimitU64Undefined, 1},
            {0, 23, 4, wgpu::kLimitU64Undefined, 0},
            {0, 20, 4, wgpu::kLimitU64Undefined, 0},
            {0, 20, 20, wgpu::kLimitU64Undefined, 0},
            // Non-default size
            {0, 21, 0, 20, 1},
            {0, 20, 0, 19, 0},
            // Non-zero offset and size
    ...
    TEST_F(DrawVertexAndIndexBufferOOBValidationTests, SetBufferMultipleTime) {
        ...
        // Test for setting vertex buffer for multiple times
        {
            ...
            VertexBufferList vertexBufferList = {{0, vertexBuffer2, 0, wgpu::kLimitU64Undefined},
                                                 {1, instanceBuffer4, 0, wgpu::kLimitU64Undefined},
                                                 {1, instanceBuffer5, 0, wgpu::kLimitU64Undefined},
                                                 {0, vertexBuffer3, 0, wgpu::kLimitU64Undefined}};
            ...
            vertexBufferList = VertexBufferList{{0, vertexBuffer3, 0, wgpu::kLimitU64Undefined},
                                                {1, instanceBuffer5, 0, wgpu::kLimitU64Undefined},
                                                {0, vertexBuffer2, 0, wgpu::kLimitU64Undefined},
                                                {1, instanceBuffer4, 0, wgpu::kLimitU64Undefined}};
