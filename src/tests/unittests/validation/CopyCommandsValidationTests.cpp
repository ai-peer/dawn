...
namespace {
    bool IsValidDepthStencilTextureAspectCopyFromBuffer(wgpu::TextureFormat textureFormat,
                                                        wgpu::TextureAspect textureAspect) {
        // Currently wgpu::TextureFormat::Stencil8 is not supported
        if (textureFormat == wgpu::TextureFormat::Stencil8) {
            return false;
        }
        switch (textureFormat) {
            case wgpu::TextureFormat::Stencil8: {
                return (textureAspect == wgpu::TextureAspect::StencilOnly) ||
                       (textureAspect == wgpu::TextureAspect::All);
            }
...
TEST_F(CopyCommandTest_B2T, IncorrectBufferOffsetForDepthStencilTexture) {
    std::vector<wgpu::TextureFormat> depthStencilTextureFormats{
        wgpu::TextureFormat::Depth24Plus, wgpu::TextureFormat::Depth24PlusStencil8,
        wgpu::TextureFormat::Depth32Float};

    std::vector<wgpu::TextureAspect> depthStencilTextureAspect{
        wgpu::TextureAspect::StencilOnly, wgpu::TextureAspect::DepthOnly, wgpu::TextureAspect::All};

    uint64_t bufferSize = BufferSizeForTextureCopy(32, 32, 1);
    wgpu::Buffer source = CreateBuffer(bufferSize, wgpu::BufferUsage::CopySrc);

    for (auto textureFormat : depthStencilTextureFormats) {
        for (auto textureAspect : depthStencilTextureAspect) {
            wgpu::Texture destination =
            ...
            for (uint64_t srcOffset = 0; srcOffset < 8; srcOffset++) {
                utils::Expectation expectation =
                    ((srcOffset % 4 == 0) &&
                     (IsValidDepthStencilTextureAspectCopyFromBuffer(textureFormat, textureAspect)))
                        ? utils::Expectation::Success
                        : utils::Expectation::Failure;
...
namespace {
    bool IsValidDepthStencilTextureAspectCopyToBuffer(wgpu::TextureFormat textureFormat,
                                                      wgpu::TextureAspect textureAspect) {
        // Currently wgpu::TextureFormat::Stencil8 is not supported
        if (textureFormat == wgpu::TextureFormat::Stencil8) {
            return false;
        ...
        switch (textureFormat) {
            ...
            case wgpu::TextureFormat::Depth32Float: {
                return (textureAspect == wgpu::TextureAspect::DepthOnly) ||
                       (textureAspect == wgpu::TextureAspect::All);
            }
    ...
    for (auto textureFormat : depthStencilTextureFormats) {
        for (auto textureAspect : depthStencilTextureAspect) {
            ...
            for (uint64_t dstOffset = 0; dstOffset < 8; dstOffset++) {
                utils::Expectation expectation =
                    ((dstOffset % 4 == 0) &&
                     (IsValidDepthStencilTextureAspectCopyToBuffer(textureFormat, textureAspect)))
                        ? utils::Expectation::Success
                        : utils::Expectation::Failure;
