...
class CopyCommandTest_MultipleArrayLayers : public CopyCommandTest_CompressedTextureFormats {
  protected:
    void TestBothTBCopiesToMultipleArrayLayers(uint32_t bufferBytesPerRow,
        ...
        uint64_t bufferSize = utils::RequiredBytesInCopy(bufferBytesPerRow, rowsPerImage, extent3D,
...
TEST_F(CopyCommandTest_MultipleArrayLayers, UncompressedTextures) {
    ...
    TestBothTBCopiesToMultipleArrayLayers(256, 2, destination, 4, 1, 1, {0, 0, 0}, {4, 2, 5});

    // Copy to the highest array layer
    TestBothTBCopiesToMultipleArrayLayers(256, 2, destination, 4, 1, 1, {0, 0, 4}, {4, 2, 1});

    // Copy to array layers in the middle
    TestBothTBCopiesToMultipleArrayLayers(256, 2, destination, 4, 1, 1, {0, 0, 1}, {4, 2, 3});

    // Copy with rowsPerImage alignment
    TestBothTBCopiesToMultipleArrayLayers(256, 3, destination, 4, 1, 1, {0, 0, 0}, {4, 2, 5});

    // Copy with rowsPerImage alignment
    TestBothTBCopiesToMultipleArrayLayers(512, 4, destination, 4, 1, 1, {0, 0, 1}, {4, 2, 3});
}
...
TEST_F(CopyCommandTest_MultipleArrayLayers, CompressedTextures) {
    for (wgpu::TextureFormat bcFormat : utils::kBCFormats) {
        ...
        TestBothTBCopiesToMultipleArrayLayers(256, 8, texture,
                                              utils::CompressedFormatBlockSizeInBytes(bcFormat), 4,
                                              4, {0, 0, 0}, {4, 4, 4});
