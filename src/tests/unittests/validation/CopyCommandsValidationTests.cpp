...
TEST_F(CopyCommandTest_B2T, CopyToStencilAspect) {
    ...
    // A copy fails when using a depth/stencil output attachment, and the entire subresource isn't
    // copied
    {
        uint64_t bufferSize = BufferSizeForTextureCopy(16, 16, 1, wgpu::TextureFormat::R8Uint);
        wgpu::Buffer source = CreateBuffer(bufferSize, wgpu::BufferUsage::CopySrc);
    ...
    // Non-zero mip: A copy fails when using a depth/stencil output attachment, and the entire
    ...
    // Non-zero mip, non-pow-2: A copy fails when using a depth/stencil output attachment, and the
    // entire subresource isn't copied
    {
        ...
        wgpu::Texture nonRenderableDestination = Create2DTexture(
            17, 17, 2, 1, wgpu::TextureFormat::Depth24PlusStencil8, wgpu::TextureUsage::CopyDst);
        ...
        // Partial mip works if non renderable
        TestB2TCopy(utils::Expectation::Success, source, 0, 256, 0, nonRenderableDestination, 1,
...
TEST_F(CopyCommandTest_T2B, CopyFromStencilAspect) {
    ...
    // entire subresource isn't copied
    {
        ...
        wgpu::Texture nonRenderableSource = Create2DTexture(
        ...
        // Partial mip works if non renderable
        TestT2BCopy(utils::Expectation::Success, nonRenderableSource, 1, {0, 0, 0}, destination, 0,
                    256, 0, {7, 7, 1}, wgpu::TextureAspect::StencilOnly);

        TestT2BCopy(utils::Expectation::Success, nonRenderableSource, 1, {0, 0, 0}, destination, 0,
                    256, 0, {1, 1, 1}, wgpu::TextureAspect::StencilOnly);
    }
}
...
TEST_F(CopyCommandTest_T2T, 2DTextureDepthStencil) {
    ...
    // Success when depth stencil subresource is partially copied but non-renderable
    TestT2TCopy(utils::Expectation::Success, nonRenderableSource, 0, {0, 0, 0}, destination, 0,
                {0, 0, 0}, {15, 15, 1});
