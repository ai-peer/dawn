...
TEST_F(CopyCommandTest_T2T, CopyWithinSameTexture) {
    ...
    // destination.
    {
        ...
        // No overlaps in copy region is allowed.
        {
            ...
            for (uint32_t srcIndex = 0; srcIndex < kCopyOrigins.size(); ++srcIndex) {
                for (uint32_t dstIndex = 0; dstIndex < kCopyOrigins.size(); ++dstIndex) {
                    ...
                    TestT2TCopy(utils::Expectation::Success, texture, kMipmapLevel,
                                kMinBaseArrayLayer, kCopyOrigins[srcIndex], texture, kMipmapLevel,
                                kMaxBaseArrayLayer, kCopyOrigins[dstIndex], kCopyExtent);
                }
            }
        }

        // Copy region having overlaps is not allowed.
        {
            constexpr std::array<wgpu::Origin3D, 4> kCopyOrigins = {
            ...
            for (uint32_t srcIndex = 0; srcIndex < kCopyOrigins.size(); ++srcIndex) {
                for (uint32_t dstIndex = 0; dstIndex < kCopyOrigins.size(); ++dstIndex) {
                    ...
                    TestT2TCopy(utils::Expectation::Failure, texture, kMipmapLevel,
                                kMinBaseArrayLayer, kCopyOrigins[srcIndex], texture, kMipmapLevel,
                                kMaxBaseArrayLayer, kCopyOrigins[dstIndex], kCopyExtent);
                }
            }
