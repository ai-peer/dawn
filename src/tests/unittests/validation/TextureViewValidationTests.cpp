...
wgpu::Texture Create2DArrayTexture(wgpu::Device& device,
    ...
    if (sampleCount == 1) {
        return utils::Create2DArrayTexture(device, width, height, arrayLayerCount,
                                           kDefaultTextureFormat, wgpu::TextureUsage::Sampled,
                                           mipLevelCount);
    } else {
        return utils::Create2DMultisampledTexture(device, width, height, kDefaultTextureFormat,
                                                  wgpu::TextureUsage::Sampled, sampleCount);
    }
}
