...
TEST_F(WireFenceTests, QueueSignalSynchronousValidationError) {
    ...
    EXPECT_CALL(*mockDeviceErrorCallback, Call(_, userdata)).Times(1).RetiresOnSaturation();
    dawnQueueSignal(queue, fence, 0u);  // Error

    EXPECT_CALL(*mockDeviceErrorCallback, Call(_, userdata)).Times(1).RetiresOnSaturation();
    dawnQueueSignal(queue, fence, 1u);  // Error

    EXPECT_CALL(*mockDeviceErrorCallback, Call(_, userdata)).Times(0).RetiresOnSaturation();
