...
struct RangeTracker {
    ...
    void Track(const SubresourceRange& range) {
        ...
        mTracked.Update(range, [](const SubresourceRange&, int* counter) {
            ASSERT_EQ(*counter, 0);
            *counter += 1;
        });
    }
...
void FakeStorage<T>::CheckSameAs(const SubresourceStorage<T>& real) {
    ...
    tracker.CheckTrackedExactly(SubresourceRange::Full(mAspects, mArrayLayerCount, mMipLevelCount));
}
...
TEST(SubresourceStorageTest, DefaultValue) {
    ...
    // Test setting no default value for a type with a default constructor.
    {
        SubresourceStorage<SmallData> s(Aspect::Color, 3, 5, {007});
        EXPECT_EQ(s.Get(Aspect::Color, 1, 2).value, 007u);

        FakeStorage<SmallData> f(Aspect::Color, 3, 5, {007});
        f.CheckSameAs(s);
    }
}
...
TEST(SubresourceStorageTest, SingleSubresourceUpdateSingleAspect) {
    ...
    SubresourceRange range = SubresourceRange::Single(Aspect::Color, 3, 2);
...
TEST(SubresourceStorageTest, SingleSubresourceUpdateMultiAspect) {
    ...
    SubresourceRange range = SubresourceRange::Single(Aspect::Stencil, 1, 2);
    CallUpdateOnBoth(&s, &f, range, [](const SubresourceRange&, int* data) { *data += 1; });
...
TEST(SubresourceStorageTest, UpdateStipple) {
    ...
    for (uint32_t layer = 0; layer < kLayers; layer++) {
        for (uint32_t level = 0; level < kLevels; level++) {
            if ((layer + level) % 2 == 0) {
                SubresourceRange range = SubresourceRange::Single(Aspect::Depth, layer, level);
    ...
    // Update completely with a single value, recompression should happen!
    {
        SubresourceRange fullRange =
            SubresourceRange::Full(Aspect::Depth | Aspect::Stencil, kLayers, kLevels);
...
// Test updating as a crossign band pattern:
...
TEST(SubresourceStorageTest, UpdateTwoBand) {
    ...
    // The layers were fully updates so they should stay compressed.
    f.CheckSameAs(s);
    ...
    // Update completely, but without a single value recompression shouldn't happen.
    {
        SubresourceRange fullRange =
            SubresourceRange::Full(Aspect::Depth | Aspect::Stencil, kLayers, kLevels);
...
TEST(SubresourceStorageTest, UpdateExtremas) {
    ...
    // Update the two extremas
    {
        SubresourceRange range = SubresourceRange::Single(Aspect::Color, 0, kLevels - 1);
        CallUpdateOnBoth(&s, &f, range, [](const SubresourceRange&, int* data) { *data += 3; });
    }
    {
        SubresourceRange range = SubresourceRange::Single(Aspect::Color, kLayers - 1, 0);
        CallUpdateOnBoth(&s, &f, range, [](const SubresourceRange&, int* data) { *data *= 3; });
    }
    ...
    {
        SubresourceRange fullRange = SubresourceRange::Full(Aspect::Color, kLayers, kLevels);
...
//  - mLayersCompressed not initialized to 0.
//  - DecompressLayer setting Compressed to true.
