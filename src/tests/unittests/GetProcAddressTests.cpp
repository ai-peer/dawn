...
namespace {
    ...
    TEST_P(GetProcAddressTests, ValidExamples) {
        ASSERT_EQ(mProcs.getProcAddress(nullptr, "dawnDeviceCreateBuffer"), reinterpret_cast<DawnProc>(mProcs.deviceCreateBuffer));
        ASSERT_EQ(mProcs.getProcAddress(mDevice.Get(), "dawnDeviceCreateBuffer"), reinterpret_cast<DawnProc>(mProcs.deviceCreateBuffer));
        ASSERT_EQ(mProcs.getProcAddress(nullptr, "dawnQueueSubmit"), reinterpret_cast<DawnProc>(mProcs.queueSubmit));
        ASSERT_EQ(mProcs.getProcAddress(mDevice.Get(), "dawnQueueSubmit"), reinterpret_cast<DawnProc>(mProcs.queueSubmit));
    }
    ...
    TEST_P(GetProcAddressTests, GetProcAddressItself) {
        ASSERT_EQ(mProcs.getProcAddress(nullptr, "dawnGetProcAddress"), reinterpret_cast<DawnProc>(mProcs.getProcAddress));
        ASSERT_EQ(mProcs.getProcAddress(mDevice.Get(), "dawnGetProcAddress"), reinterpret_cast<DawnProc>(mProcs.getProcAddress));
    }
...
namespace dawn_native {
    std::vector<std::string> GetProcMapNamesForTesting();

    TEST(GetProcAddressInternalTests, CheckDawnNativeProcMapOrder) {
        std::vector<std::string> names = GetProcMapNamesForTesting();
