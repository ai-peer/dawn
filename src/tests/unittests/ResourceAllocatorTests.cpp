...
typedef BuddyPoolAllocator<Block, DummyAllocator, BuddyBlockAllocator<Block>> BuddyAllocator;
...
TEST_F(BuddyAllocatorTests, SingleBlock) {
    ...
    // Allocate the block.
    Block block = allocator.Allocate(sizeInBytes);
    CheckBlockValid(block, sizeInBytes, 0u);
...
TEST_F(BuddyAllocatorTests, SmallAllocationPool) {
    ...
    Block block1 = allocator.Allocate(resourceSizeInBytes);
    CheckBlockValid(block1, resourceSizeInBytes, 0);

    Block block2 = allocator.Allocate(resourceSizeInBytes);
