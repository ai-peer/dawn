...
TEST_F(BuddyAllocatorTests, SingleBlockAligned) {
    ...
    // No additional blocks should of been allocated.
    ASSERT_EQ(allocator.GetNumOfFreeBlocks(), 1u);
}
...
TEST_F(BuddyPoolAllocatorTests, SmallPool) {
    ...
    constexpr size_t maxSizeInBytes = 512;
    BuddyResourceAllocator allocator(maxSizeInBytes, DummyAllocator::MIN_BLOCK_SIZE,
                                     resourceSizeInBytes);
...
TEST_F(BuddyPoolAllocatorTests, SingleBlockAligned) {
    ...
    BuddyResourceAllocator allocator(allocatorSizeInBytes, MIN_BLOCK_SIZE,
                                     DummyResource::MIN_RESOURCE_SIZE);

    // Block requires alignment.
    {
        constexpr size_t blockSize = MIN_BLOCK_SIZE / 2;
        HeapSubAllocationBlock block = allocator.Allocate(blockSize);
        CheckBlockInvalid(block);
    }

    // Block does not require alignment.
    {
        constexpr size_t blockSize = MIN_BLOCK_SIZE * 2;
        HeapSubAllocationBlock block = allocator.Allocate(blockSize);
...
TEST_F(BuddyPoolAllocatorTests, SingleResourceAligned) {
    ...
    BuddyResourceAllocator allocatorA(allocatorSizeInBytes, DummyAllocator::MIN_BLOCK_SIZE,
                                      DummyResource::MIN_RESOURCE_SIZE / 2);
    ...
    BuddyResourceAllocator allocatorB(allocatorSizeInBytes, DummyAllocator::MIN_BLOCK_SIZE,
                                      DummyResource::MIN_RESOURCE_SIZE * 2);
...
TEST_F(BuddyPoolAllocatorTests, LargePool) {
    ...
    constexpr size_t allocatorSizeInBytes = 16 * 1024LL * 1024LL;  // 16GB
    BuddyResourceAllocator allocator(allocatorSizeInBytes, DummyAllocator::MIN_BLOCK_SIZE,
                                     resourceSizeInBytes);
...
TEST_F(ConditionalAllocatorTests, BasicTest) {
    ...
    ConditionalResourceAllocator allocator(allocatorSizeInBytes, DummyAllocator::MIN_BLOCK_SIZE,
                                           resourceSizeInBytes);
