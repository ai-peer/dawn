...
class DummyResource : public ResourceHeapBase {
  public:
    DummyResource(size_t size) : ResourceHeapBase(size, 0) {
    }
    ...
    static const size_t MIN_RESOURCE_SIZE = 64;
};

class DummyAllocator {
  public:
    ResourceHeapBase* Allocate(size_t heapSize) {
        return new DummyResource(heapSize);
    }
...
class ResourceAllocatorTests : public testing::Test {
  public:
    void CheckBlockValid(const Block& block, size_t expectedSize, size_t expectedOffset) {
        ASSERT_EQ(block.GetSize(), expectedSize);
        ASSERT_EQ(block.GetOffset(), expectedOffset);
    }

    void CheckBlockInvalid(const Block& block) {
        ASSERT_EQ(block.GetSize(), 0u);
        ASSERT_EQ(block.GetOffset(), 0u);
    }
};

typedef DirectAllocator<HeapSubAllocationBlock, DummyAllocator> DirectResourceAllocator;
...
TEST_F(DirectAllocatorTests, SingleBlock) {
    DirectResourceAllocator allocator;

    constexpr size_t sizeInBytes = 64;
    HeapSubAllocationBlock blockA = allocator.Allocate(sizeInBytes);
    CheckBlockValid(blockA, sizeInBytes, 0u);

    ASSERT_TRUE(blockA.GetResourceHeap() != nullptr);

    // Re-allocate from the same allocator.
    HeapSubAllocationBlock blockB = allocator.Allocate(sizeInBytes);
    CheckBlockValid(blockB, sizeInBytes, 0u);

    ASSERT_TRUE(blockB.GetResourceHeap() != nullptr);

    // Check blockA maps to resourceA.
    ResourceAllocation allocA = allocator.GetSubAllocation(blockA);
    ASSERT_TRUE(allocA.GetResourceHeap() != nullptr);
    ASSERT_EQ(allocA.GetOffset(), 0u);

    // Check blockB maps to resourceB.
    ResourceAllocation allocB = allocator.GetSubAllocation(blockB);
    ASSERT_TRUE(allocB.GetResourceHeap() != nullptr);
    ASSERT_EQ(allocB.GetOffset(), 0u);

    // Check both allocations have seperate resource heaps.
    ASSERT_TRUE(allocA.GetResourceHeap() != allocB.GetResourceHeap());
    ASSERT_TRUE(allocA.GetOffset() == allocB.GetOffset());

    allocator.Deallocate(blockA);
    allocator.Deallocate(blockB);
}

typedef BuddyBlockAllocator<HeapSubAllocationBlock> BuddyAllocator;
...
TEST_F(BuddyAllocatorTests, SingleBlock) {
    ...
    // Sub-allocated blocks are not directly allocated.
    ASSERT_TRUE(block.GetResourceHeap() == nullptr);
...
// Verify the allocated size of the block is aligned when required.
TEST_F(BuddyAllocatorTests, SingleBlockAligned) {
    constexpr size_t maxSizeInBytes = 16 * 1024LL;  // 16KB
    constexpr size_t minSizeInBytes = 512;
    BuddyAllocator allocator(maxSizeInBytes, minSizeInBytes);

    // Requested block size requires alignment.
    size_t requestedSizeTooSmall = minSizeInBytes / 2;
    HeapSubAllocationBlock blockA = allocator.Allocate(requestedSizeTooSmall);
    CheckBlockValid(blockA, minSizeInBytes, 0u);

    // Requested block size is too large
    size_t requestedSizeTooLarge = maxSizeInBytes * 2;
    HeapSubAllocationBlock blockB = allocator.Allocate(requestedSizeTooLarge);
    CheckBlockInvalid(blockB);

    // Request block size requires no alignment.
    size_t requestedSize = minSizeInBytes;
    HeapSubAllocationBlock blockC = allocator.Allocate(requestedSize);
    CheckBlockValid(blockC, requestedSize, minSizeInBytes);  // BlockA was aligned.

    // After 2x512B allocated blocks = 1x1KB + 1x2KB + 1x4KB + 1x8KB = 4 free blocks.
    ASSERT_EQ(allocator.GetNumOfFreeBlocks(), 4u);
}
...
TEST_F(BuddyAllocatorTests, SingleSplitBlock) {
    ...
    HeapSubAllocationBlock block = allocator.Allocate(8);
...
TEST_F(BuddyAllocatorTests, MultipleSplitBlocks) {
    ...
    HeapSubAllocationBlock blockA = allocator.Allocate(blockSizeInBytes);
    CheckBlockValid(blockA, blockSizeInBytes, 0u);
    ASSERT_EQ(allocator.GetNumOfFreeBlocks(), 2u);

    // Allocate "b" block.
    HeapSubAllocationBlock blockB = allocator.Allocate(blockSizeInBytes);
    CheckBlockValid(blockB, blockSizeInBytes, blockSizeInBytes);
    ASSERT_EQ(allocator.GetNumOfFreeBlocks(), 1u);

    // Allocate "c" block (three splits).
    HeapSubAllocationBlock blockC = allocator.Allocate(blockSizeInBytes);
    CheckBlockValid(blockC, blockSizeInBytes, blockB.GetOffset() + blockSizeInBytes);
    ASSERT_EQ(allocator.GetNumOfFreeBlocks(), 1u);

    // Allocate "d" block.
    HeapSubAllocationBlock blockD = allocator.Allocate(blockSizeInBytes);
...
TEST_F(BuddyAllocatorTests, MultipleSplitBlocksInterleaved) {
    ...
    std::vector<HeapSubAllocationBlock> blocks;
...
class BuddyPoolAllocatorTests : public ResourceAllocatorTests {};

typedef BuddyPoolAllocator<HeapSubAllocationBlock,
                           DummyAllocator,
                           BuddyBlockAllocator<HeapSubAllocationBlock>,
                           DummyAllocator::MIN_BLOCK_SIZE,
                           DummyResource::MIN_RESOURCE_SIZE>
    BuddyResourceAllocator;

// Verify allocation of a few blocks over multple resources.
TEST_F(BuddyPoolAllocatorTests, SmallPool) {
    constexpr size_t resourceSizeInBytes = 128;
    constexpr size_t maxSizeInBytes = 512;
    BuddyResourceAllocator allocator(maxSizeInBytes, resourceSizeInBytes);
    ...
    HeapSubAllocationBlock block1 = allocator.Allocate(resourceSizeInBytes);
    CheckBlockValid(block1, resourceSizeInBytes, 0);

    HeapSubAllocationBlock block2 = allocator.Allocate(resourceSizeInBytes);
    CheckBlockValid(block2, resourceSizeInBytes, resourceSizeInBytes);

    ASSERT_EQ(allocator.GetResourceCount(), 2u);

    // Check block1 maps to resource1.
    {
        ResourceAllocation alloc = allocator.GetSubAllocation(block1);
        ASSERT_TRUE(alloc.GetResourceHeap() != nullptr);
        ASSERT_EQ(alloc.GetOffset(), 0u);
    }

    // Check block2 maps to resource2.
    {
        ResourceAllocation alloc = allocator.GetSubAllocation(block2);
        ASSERT_TRUE(alloc.GetResourceHeap() != nullptr);
        ASSERT_EQ(alloc.GetOffset(), 0u);
    }
...
// Verify allocation of aligned blocks.
TEST_F(BuddyPoolAllocatorTests, SingleBlockAligned) {
    constexpr size_t MIN_BLOCK_SIZE = 4;

    typedef BuddyPoolAllocator<HeapSubAllocationBlock, DummyAllocator,
                               BuddyBlockAllocator<HeapSubAllocationBlock>, MIN_BLOCK_SIZE,
                               DummyResource::MIN_RESOURCE_SIZE>
        BuddyResourceAllocator;

    constexpr size_t allocatorSizeInBytes = 16 * 1024LL * 1024LL;  // 16GB
    BuddyResourceAllocator allocator(allocatorSizeInBytes, DummyResource::MIN_RESOURCE_SIZE);

    // First block requires alignment.
    {
        constexpr size_t blockSize = MIN_BLOCK_SIZE / 2;
        HeapSubAllocationBlock block = allocator.Allocate(blockSize);
        CheckBlockValid(block, MIN_BLOCK_SIZE, 0);
    }

    // Second block does not require alignment.
    {
        constexpr size_t blockSize = MIN_BLOCK_SIZE * 2;
        HeapSubAllocationBlock block = allocator.Allocate(blockSize);
        CheckBlockValid(block, blockSize, blockSize);
    }
}

// Verify allocation of blocks with aligned resources.
TEST_F(BuddyPoolAllocatorTests, SingleResourceAligned) {
    constexpr size_t allocatorSizeInBytes = 16 * 1024LL * 1024LL;  // 16GB

    // Create an allocator with a resource size that is below the min. resource size.
    BuddyResourceAllocator allocatorA(allocatorSizeInBytes, DummyResource::MIN_RESOURCE_SIZE / 2);

    constexpr size_t blockSize = 16;
    HeapSubAllocationBlock blockA = allocatorA.Allocate(blockSize);
    CheckBlockValid(blockA, blockSize, /*offset*/ 0);

    // Verify the newly created resource heap is at-least the min resource size.
    {
        ResourceAllocation alloc = allocatorA.GetSubAllocation(blockA);
        ASSERT_TRUE(alloc.GetResourceHeap() != nullptr);
        ASSERT_EQ(alloc.GetResourceHeap()->GetSize(), DummyResource::MIN_RESOURCE_SIZE);
    }

    // Create an allocator with a resource size that is above the min. resource size.
    BuddyResourceAllocator allocatorB(allocatorSizeInBytes, DummyResource::MIN_RESOURCE_SIZE * 2);

    HeapSubAllocationBlock blockB = allocatorB.Allocate(blockSize);
    CheckBlockValid(blockB, blockSize, /*offset*/ 0);

    // Verify the newly created resource heap is same size as specified.
    {
        ResourceAllocation alloc = allocatorB.GetSubAllocation(blockB);
        ASSERT_TRUE(alloc.GetResourceHeap() != nullptr);
        ASSERT_EQ(alloc.GetResourceHeap()->GetSize(), DummyResource::MIN_RESOURCE_SIZE * 2);
    }
}

// Verify allocation of many blocks over multiple resources.
TEST_F(BuddyPoolAllocatorTests, LargePool) {
    constexpr size_t resourceSizeInBytes = 64 * 1024LL;            // 64KB
    constexpr size_t allocatorSizeInBytes = 16 * 1024LL * 1024LL;  // 16GB
    BuddyResourceAllocator allocator(allocatorSizeInBytes, resourceSizeInBytes);
    ...
    for (size_t i = 0; i < allocatorSizeInBytes / blockSize; i++) {
        ...
        // Check block maps to resource.
        {
            ResourceAllocation alloc = allocator.GetSubAllocation(block);
            ASSERT_TRUE(alloc.GetResourceHeap() != nullptr);
            ASSERT_EQ(alloc.GetOffset(), block.GetOffset() % resourceSizeInBytes);
        }
    ...
    // Deallocate blocks and resources.
    for (HeapSubAllocationBlock& block : blocks) {
        allocator.Deallocate(block);
    }
...
class ConditionalAllocatorTests : public ResourceAllocatorTests {};

typedef ConditionalAllocator<HeapSubAllocationBlock,
                             BuddyResourceAllocator,
                             DirectResourceAllocator>
    ConditionalResourceAllocator;

// Verify the conditional resource allocator will sub-allocate resources or not.
TEST_F(ConditionalAllocatorTests, BasicTest) {
    constexpr size_t resourceSizeInBytes = 8;
    constexpr size_t allocatorSizeInBytes = 512;
    ConditionalResourceAllocator allocator(allocatorSizeInBytes, resourceSizeInBytes);

    // Direct allocation
    constexpr size_t blockSize = 4;
    HeapSubAllocationBlock block = allocator.Allocate(blockSize, /*isDirect*/ true);
    CheckBlockValid(block, blockSize, 0u);
    ASSERT_TRUE(block.GetResourceHeap() != nullptr);
    ...
    // Sub-allocation
    block = allocator.Allocate(blockSize, /*isDirect*/ false);
    CheckBlockValid(block, blockSize, 0u);
    // Resource must be mapped through the sub-allocator.
    ASSERT_TRUE(block.GetResourceHeap() == nullptr);
