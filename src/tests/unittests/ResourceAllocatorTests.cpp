...
class DummyAllocator {
  public:
    ...
    void Deallocate(ResourceHeapBase* heap){ delete heap; };
};

class ResourceAllocatorTests : public testing::Test {
  public:
    void CheckBlockValid(Block block, size_t expectedSize, size_t expectedOffset) {
        ASSERT_EQ(block.GetSize(), expectedSize);
        ASSERT_EQ(block.GetOffset(), expectedOffset);
    }

    void CheckBlockInvalid(Block block) {
...
TEST_F(BuddyAllocatorTests, SingleBlock) {
    ...
    BuddyAllocator allocator(/*maxSize*/ sizeInBytes, /*resourceSize*/ sizeInBytes);
