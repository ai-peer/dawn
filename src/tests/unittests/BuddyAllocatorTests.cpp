...
class BuddyAllocatorTests : public testing::Test {};
...
TEST_F(BuddyAllocatorTests, MultipleBlocks) {
    ...
    for (uint32_t order = 1; (1 << order) <= maxBlockSize; order++) {
        BuddyAllocator allocator(maxBlockSize);

        uint64_t blockSize = (1 << order);
...
TEST_F(BuddyAllocatorTests, SameSizeVariousAlignment) {
    ...
    // Allocate Ab (skip Aa buddy due to alignment and perform two splits).
...
TEST_F(BuddyAllocatorTests, VariousSizeSameAlignment) {
