...
class EnumBitSetIteratorTest : public testing::Test {
  protected:
    ...
    TypedBitset<TestEnum, kEnumCount> mStateBits;
};
...
TEST_F(EnumBitSetIteratorTest, NonLValueBitset) {
    TypedBitset<TestEnum, kEnumCount> otherBits;
...
class TypedBitSetIteratorTest : public testing::Test {
  protected:
    using IntegerT = TypedInteger<struct Foo, uint32_t>;
    TypedBitset<IntegerT, 40> mStateBits;
};

// Simple iterator test.
TEST_F(TypedBitSetIteratorTest, Iterator) {
...
// Test an empty iterator.
TEST_F(TypedBitSetIteratorTest, EmptySet) {
    // We don't use the FAIL gtest macro here since it returns immediately,
    // causing an unreachable code warning in MSVC
    bool sawBit = false;
    for (IntegerT bit : IterateBitSet(mStateBits)) {
...
// Test iterating a result of combining two bitsets.
TEST_F(TypedBitSetIteratorTest, NonLValueBitset) {
    TypedBitset<IntegerT, 40> otherBits;
