...
namespace {
    ...
    // A simple task
    class SimpleTask : public Closure {
      public:
    ...
    class Tracker : public NonCopyable {
      ...
      private:
        ...
        std::list<std::shared_ptr<WaitableEvent>> mTasksInFlight;
...
TEST_F(WorkerThreadTest, Basic) {
    std::shared_ptr<WorkerTaskPool> pool = WorkerTaskPool::Create();
    Tracker tracker;
    ...
    tracker.WaitAll();
    EXPECT_EQ(0u, pool->GetRunningTasksCount());
