...
namespace {
    ...
    class Tracker : public NonCopyable {
      public:
        explicit Tracker(std::shared_ptr<dawn_platform::WorkerTaskPool> pool) : mPool(pool) {
        }
...
TEST_F(WorkerThreadTest, Basic) {
    ...
    std::shared_ptr<dawn_platform::WorkerTaskPool> pool(platform.CreateWorkerTaskPool());
    Tracker tracker(pool);
