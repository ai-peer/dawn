...
class DummyResourceMemoryAllocator : public ResourceMemoryAllocator {
  public:
    ResultOrError<std::unique_ptr<HeapBase>> Allocate(uint64_t size, int memoryFlags = 0) override {
        return std::make_unique<HeapBase>();
    }
    void Deallocate(std::unique_ptr<HeapBase> allocation) override {
    }
};
...
TEST(BuddyResourceHeapAllocatorTests, SameSizeVariousAlignment) {
    ...
    ASSERT_EQ(allocationB.GetInfo().mBlockOffset, 128u);
    ASSERT_EQ(allocationB.GetInfo().mMethod, AllocationMethod::kSubAllocated);

    ASSERT_EQ(allocator.ComputeTotalNumOfHeapsForTesting(), 2u);

    ResourceMemoryAllocation allocationC = allocator.Allocate(128, 128);
    ASSERT_EQ(allocationC.GetInfo().mBlockOffset, 256u);
    ASSERT_EQ(allocationC.GetInfo().mMethod, AllocationMethod::kSubAllocated);

    ASSERT_EQ(allocator.ComputeTotalNumOfHeapsForTesting(), 3u);

    ResourceMemoryAllocation allocationD = allocator.Allocate(64, 128);
    ASSERT_EQ(allocationD.GetInfo().mBlockOffset, 384u);
    ASSERT_EQ(allocationD.GetInfo().mMethod, AllocationMethod::kSubAllocated);
...
TEST(BuddyResourceHeapAllocatorTests, VariousSizeSameAlignment) {
    ...
    ASSERT_EQ(allocationB.GetInfo().mBlockOffset, 64u);
    ASSERT_EQ(allocationB.GetInfo().mMethod, AllocationMethod::kSubAllocated);
    ...
    ASSERT_EQ(allocationC.GetInfo().mBlockOffset, 128u);
    ASSERT_EQ(allocationC.GetInfo().mMethod, AllocationMethod::kSubAllocated);

    ASSERT_EQ(allocator.ComputeTotalNumOfHeapsForTesting(), 2u);

    ResourceMemoryAllocation allocationD = allocator.Allocate(128, alignment);
    ASSERT_EQ(allocationD.GetInfo().mBlockOffset, 256u);
    ASSERT_EQ(allocationD.GetInfo().mMethod, AllocationMethod::kSubAllocated);
