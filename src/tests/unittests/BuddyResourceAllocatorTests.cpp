...
TEST(BuddyResourceHeapAllocatorTests, SingleHeap) {
    //
    // After one 64 byte resource allocation:
    //
    // max block size -> ---------------------------
    //                   |          A1/H0          |       Hi - Heap at index i
    // max heap size  -> ---------------------------       Rn - Resource allocation n
    //
    ...
    ASSERT_EQ(invalidAllocation.GetOffset(), kInvalidOffset);
    ASSERT_EQ(invalidAllocation.GetAllocationMethod(), AllocationMethod::kInvalid);

    // Allocate one 128 byte allocation.
    ...
    // Allocation creates the one and only heap.
    ASSERT_EQ(allocator.ComputeTotalNumOfHeapsForTesting(), 1u);

    // Cannot allocate when allocator is full.
    ASSERT_EQ(allocator.Allocate(heapSize).GetOffset(), kInvalidOffset);

    // Deallocate the one and only allocation and heap.
...
// Verify multiple sub-allocations can re-use a heap.
TEST(BuddyResourceHeapAllocatorTests, MultipleSplitHeaps) {
    ...
    // Deallocate all allocations (reverse order).
    allocator.Deallocate(allocation1);
...
TEST(BuddyResourceHeapAllocatorTests, SameSizeVariousAlignment) {
    ...
    ASSERT_EQ(allocator.ComputeTotalNumOfHeapsForTesting(), 1u);  // H0
    ...
    ASSERT_EQ(allocator.ComputeTotalNumOfHeapsForTesting(), 2u);  // H1
    ...
    ASSERT_EQ(allocator.ComputeTotalNumOfHeapsForTesting(), 3u);  // H2
    ...
    ASSERT_EQ(allocator.ComputeTotalNumOfHeapsForTesting(), 4u);  // H3
...
TEST(BuddyResourceHeapAllocatorTests, VariousSizeSameAlignment) {
    ...
    ASSERT_EQ(allocator.ComputeTotalNumOfHeapsForTesting(), 1u);  // Reuse H0
