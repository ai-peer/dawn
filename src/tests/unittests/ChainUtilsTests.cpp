...
TEST(ChainUtilsTests, ValidateSingletonEmptyChain) {
    dawn_native::MaybeError result = dawn_native::ValidateSingletonSType(nullptr,
        wgpu::SType::ShaderModuleSPIRVDescriptor);
    ASSERT_TRUE(result.IsSuccess());

    result = dawn_native::ValidateSingletonSType(nullptr,
...
TEST(ChainUtilsTests, ValidateSingletonMultiChain) {
    dawn_native::PrimitiveDepthClampingState chain1;
    dawn_native::ShaderModuleSPIRVDescriptor chain2;
    chain1.nextInChain = &chain2;

    dawn_native::MaybeError result = dawn_native::ValidateSingletonSType(&chain1,
        wgpu::SType::PrimitiveDepthClampingState);
    ASSERT_TRUE(result.IsError());
    result.AcquireError();

    result = dawn_native::ValidateSingletonSType(&chain1,
...
TEST(ChainUtilsTests, ValidateSingletonSatisfied) {
...
TEST(ChainUtilsTests, ValidateSingletonUnsatisfied) {
    dawn_native::PrimitiveDepthClampingState chain1;
