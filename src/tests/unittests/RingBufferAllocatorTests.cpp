...
TEST(RingBufferAllocatorTests, BasicTest) {
    ...
    ASSERT_EQ(allocator.Allocate(sizeInBytes + 1), kInvalidOffset);

    // Fill the entire buffer with two requests of equal size.
    ASSERT_EQ(allocator.Allocate(sizeInBytes / 2), 0u);
    ASSERT_EQ(allocator.Allocate(sizeInBytes / 2), 32000u);

    // Ensure the buffer is full.
    ASSERT_EQ(allocator.Allocate(1), kInvalidOffset);
}
...
TEST(RingBufferAllocatorTests, RingBufferManyAlloc) {
    ...
    for (size_t i = 0; i < maxNumOfFrames; ++i) {
        offset = allocator.Allocate(frameSizeInBytes);
...
TEST(RingBufferAllocatorTests, RingBufferSubAlloc) {
    ...
    for (size_t i = 0; i < 8; ++i) {
        allocator.Allocate(frameSizeInBytes, i + 1);
    }
    ...
    ASSERT_EQ(allocator.Allocate(frameSizeInBytes * 3), kInvalidOffset);
    ...
    size_t offset = allocator.Allocate(frameSizeInBytes * 3);
    ...
    // Ensure we are full.
    ASSERT_EQ(allocator.Allocate(frameSizeInBytes), kInvalidOffset);
