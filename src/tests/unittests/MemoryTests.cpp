...
class MemoryTests : public testing::Test {
  protected:
    void SetUp() override {
        mDevice = std::make_unique<Device>(/*adapter*/ nullptr);
    }
    ...
    dawn_native::RingBuffer* CreateRingBuffer(size_t size) {
        return mDevice->GetDynamicUploader(size)->GetLargestBuffer();
    }
...
TEST_F(MemoryTests, BasicTest) {
    ...
    dawn_native::RingBuffer* buffer = CreateRingBuffer(sizeInBytes);
...
TEST_F(MemoryTests, RingBufferManyAlloc) {
    ...
    dawn_native::RingBuffer* buffer = CreateRingBuffer(maxNumOfFrames * frameSizeInBytes);
...
TEST_F(MemoryTests, AllocInSameFrame) {
...
TEST_F(MemoryTests, RingBufferSubAlloc) {
