...
namespace {
    ...
    std::ostream& operator<<(std::ostream& os, const Event& event) {
        switch (event.action) {
            case Action::kReference:
                os << "Reference " << event.this_id;
                break;
            case Action::kRelease:
                os << "Release " << event.this_id;
                break;
            case Action::kAssign:
                os << "Assign " << event.this_id << " <- " << event.other_id;
                break;
            case Action::kMarker:
                os << "Marker " << event.this_id;
                break;
        }
        return os;
    }
    ...
    bool operator==(const Event& a, const Event& b) {
        return a.action == b.action && a.this_id == b.this_id && a.other_id == b.other_id;
    }
...
TEST(RefBase, Acquire) {
    Events events;
    RefTracker trackerA(1, &events);
    RefTracker trackerB(2, &events);
    Ref ref(trackerA);

    events.clear();
    { ref.Acquire(trackerB); }
    EXPECT_THAT(events, testing::ElementsAre(Event{Action::kRelease, 1},   // release ref
                                             Event{Action::kAssign, 1, 2}  // acquire trackerB
                                             ));
}
...
TEST(RefBase, RefCopyAssignment) {
    ...
    EXPECT_THAT(events, testing::ElementsAre(Event{Action::kMarker, 10},    //
                                             Event{Action::kReference, 1},  // reference trackerA
                                             Event{Action::kAssign, 0, 1},  // copy trackerA
                                             Event{Action::kMarker, 20},    //
                                             Event{Action::kReference, 2},  // reference trackerB
                                             Event{Action::kRelease, 1},    // release trackerA
                                             Event{Action::kAssign, 1, 2},  // copy trackerB
                                             Event{Action::kMarker, 30},    //
                                             Event{Action::kReference, 1},  // reference trackerA
                                             Event{Action::kRelease, 2},    // release trackerB
                                             Event{Action::kAssign, 2, 1},  // copy trackerA
...
TEST(RefBase, TCopyAssignmentAlternate) {
    ...
    EXPECT_THAT(events, testing::ElementsAre(Event{Action::kReference, 1},  // reference trackerA
                                             Event{Action::kAssign, 0, 1},  // copy trackerA
                                             Event{Action::kMarker, 10},    //
                                             Event{Action::kReference, 2},  // reference trackerB
                                             Event{Action::kRelease, 1},    // release trackerA
