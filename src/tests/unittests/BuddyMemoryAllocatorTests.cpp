...
class DummyMemoryAllocator : public MemoryAllocator {
  public:
    ResultOrError<std::unique_ptr<MemoryBase>> Allocate(uint64_t size,
                                                        int memoryFlags = 0) override {
        return std::make_unique<MemoryBase>();
    }
    void Deallocate(std::unique_ptr<MemoryBase> allocation) override {
    }
};
...
class DummyBuddyResourceAllocator {
  public:
    ...
    ResourceMemoryAllocationBase Allocate(uint64_t allocationSize, uint64_t alignment = 1) {
        ResultOrError<ResourceMemoryAllocationBase> result =
            mAllocator.Allocate(allocationSize, alignment);
        return (result.IsSuccess()) ? result.AcquireSuccess() : ResourceMemoryAllocationBase{};
    }

    void Deallocate(ResourceMemoryAllocationBase& allocation) {
...
TEST(BuddyMemoryAllocatorTests, SingleHeap) {
    ...
    ResourceMemoryAllocationBase invalidAllocation = allocator.Allocate(heapSize * 2);
    ASSERT_EQ(invalidAllocation.GetInfo().mMethod, AllocationMethod::kInvalid);

    // Allocate one 128 byte allocation (same size as heap).
    ResourceMemoryAllocationBase allocation1 = allocator.Allocate(128);
...
TEST(BuddyMemoryAllocatorTests, MultipleHeaps) {
    ...
    ResourceMemoryAllocationBase allocation1 = allocator.Allocate(heapSize);
    ASSERT_EQ(allocation1.GetInfo().mBlockOffset, 0u);
    ASSERT_EQ(allocation1.GetInfo().mMethod, AllocationMethod::kSubAllocated);
    ...
    ResourceMemoryAllocationBase allocation2 = allocator.Allocate(heapSize);
...
TEST(BuddyMemoryAllocatorTests, MultipleSplitHeaps) {
    ...
    ResourceMemoryAllocationBase allocation1 = allocator.Allocate(heapSize / 2);
    ASSERT_EQ(allocation1.GetInfo().mBlockOffset, 0u);
    ASSERT_EQ(allocation1.GetInfo().mMethod, AllocationMethod::kSubAllocated);
    ...
    ResourceMemoryAllocationBase allocation2 = allocator.Allocate(heapSize / 2);
    ...
    ResourceMemoryAllocationBase allocation3 = allocator.Allocate(heapSize / 2);
...
TEST(BuddyMemoryAllocatorTests, MultiplSplitHeapsVariableSizes) {
    ...
    ResourceMemoryAllocationBase allocation1 = allocator.Allocate(64);
    ASSERT_EQ(allocation1.GetInfo().mBlockOffset, 0u);
    ASSERT_EQ(allocation1.GetOffset(), 0u);
    ASSERT_EQ(allocation1.GetInfo().mMethod, AllocationMethod::kSubAllocated);

    ResourceMemoryAllocationBase allocation2 = allocator.Allocate(64);
    ...
    ResourceMemoryAllocationBase allocation3 = allocator.Allocate(128);
    ...
    ResourceMemoryAllocationBase allocation4 = allocator.Allocate(64);
    ...
    ResourceMemoryAllocationBase allocation5 = allocator.Allocate(128);
...
TEST(BuddyMemoryAllocatorTests, SameSizeVariousAlignment) {
    ...
    ResourceMemoryAllocationBase allocation1 = allocator.Allocate(64, 128);
    ASSERT_EQ(allocation1.GetInfo().mBlockOffset, 0u);
    ASSERT_EQ(allocation1.GetOffset(), 0u);
    ASSERT_EQ(allocation1.GetInfo().mMethod, AllocationMethod::kSubAllocated);
    ...
    ResourceMemoryAllocationBase allocation2 = allocator.Allocate(64, 128);
    ...
    ResourceMemoryAllocationBase allocation3 = allocator.Allocate(64, 128);
    ...
    ResourceMemoryAllocationBase allocation4 = allocator.Allocate(64, 64);
...
TEST(BuddyMemoryAllocatorTests, VariousSizeSameAlignment) {
    ...
    ResourceMemoryAllocationBase allocation1 = allocator.Allocate(64, alignment);
    ASSERT_EQ(allocation1.GetInfo().mBlockOffset, 0u);
    ASSERT_EQ(allocation1.GetInfo().mMethod, AllocationMethod::kSubAllocated);

    ASSERT_EQ(allocator.ComputeTotalNumOfHeapsForTesting(), 1u);

    ResourceMemoryAllocationBase allocation2 = allocator.Allocate(64, alignment);
    ASSERT_EQ(allocation2.GetInfo().mBlockOffset, 64u);
    ASSERT_EQ(allocation2.GetOffset(), 64u);
    ASSERT_EQ(allocation2.GetInfo().mMethod, AllocationMethod::kSubAllocated);
    ...
    ResourceMemoryAllocationBase allocation3 = allocator.Allocate(128, alignment);
    ASSERT_EQ(allocation3.GetInfo().mBlockOffset, 128u);
    ASSERT_EQ(allocation3.GetOffset(), 0u);
    ASSERT_EQ(allocation3.GetInfo().mMethod, AllocationMethod::kSubAllocated);
    ...
    ResourceMemoryAllocationBase allocation4 = allocator.Allocate(128, alignment);
    ASSERT_EQ(allocation4.GetInfo().mBlockOffset, 256u);
    ASSERT_EQ(allocation4.GetOffset(), 0u);
    ASSERT_EQ(allocation4.GetInfo().mMethod, AllocationMethod::kSubAllocated);
