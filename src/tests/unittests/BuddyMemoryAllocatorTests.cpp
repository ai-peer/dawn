...
TEST_F(BuddyMemoryAllocatorTests, PoolHeapsMultipleSubmits) {
    ...
    PooledResourceMemoryAllocator poolAllocator(mDevice, &heapAllocator);
    DummyBuddyResourceAllocator allocator(maxBlockSize, heapSize, &poolAllocator);
...
TEST_F(BuddyMemoryAllocatorTests, PoolHeapsInPendingAndMultipleSubmits) {
    ...
    // Make sure we can't destroy the remaining heaps.
    poolAllocator.DestroyPool();
    ASSERT_EQ(poolAllocator.GetPoolSizeForTesting(), 0u);
}