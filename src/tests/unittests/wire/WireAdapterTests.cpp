...
TEST_F(WireAdapterTests, RequestDevicePassesDescriptor) {
    ...
    EXPECT_CALL(cb, Call(WGPURequestDeviceStatus_Unknown, nullptr, NotNull(), this)).Times(2);
    adapter = nullptr;
}
...
TEST_F(WireAdapterTests, RequestDeviceSuccess) {
    ...
    EXPECT_CALL(api, OnDeviceSetDeviceLostCallback(apiDevice, nullptr, nullptr)).Times(Exactly(1));
}
...
TEST_F(WireAdapterTests, RequestDevice_AdapterDestroyedBeforeCallback) {
    MockCallback<WGPURequestDeviceCallback> cb;
    auto* userdata = cb.MakeUserdata(this);
...
// before the callback happens.
TEST_F(WireAdapterTests, RequestDevice_WireDisconnectedBeforeCallback) {
    MockCallback<WGPURequestDeviceCallback> cb;
    auto* userdata = cb.MakeUserdata(this);
