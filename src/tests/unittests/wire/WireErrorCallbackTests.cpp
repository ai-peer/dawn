...
namespace {
    ...
    void ToMockDeviceLoggingCallback(const char* message, void* userdata) {
        mockDeviceLoggingCallback->Call(message, userdata);
    }
...
TEST_F(WireErrorCallbackTests, DeviceLoggingCallback) {
    ...
    // Calling the callback on the server side will result in the callback being called on the
    // client side
    api.CallDeviceSetLoggingCallbackCallback(apiDevice, "Some error message");

    EXPECT_CALL(*mockDeviceLoggingCallback, Call(StrEq("Some error message"), this)).Times(1);
