...
TEST_F(WireExtensionTests, ChainedStruct) {
    WGPUSamplerDescriptorAnisotropicFiltering clientExt = {};
    clientExt.chain.sType = WGPUSType_SamplerDescriptorAnisotropicFiltering;
    ...
    EXPECT_CALL(api, DeviceCreateSampler(apiDevice, NotNull()))
        .WillOnce(Invoke([&](Unused, const WGPUSamplerDescriptor* serverDesc) -> WGPUSampler {
            ...
            const auto* ext = reinterpret_cast<const WGPUSamplerDescriptorAnisotropicFiltering*>(
...
TEST_F(WireExtensionTests, MutlipleChainedStructs) {
    WGPUSamplerDescriptorAnisotropicFiltering clientExt2 = {};
    clientExt2.chain.sType = WGPUSType_SamplerDescriptorAnisotropicFiltering;
    clientExt2.chain.next = nullptr;
    clientExt2.maxAnisotropy = 2.71828;

    WGPUSamplerDescriptorAnisotropicFiltering clientExt1 = {};
    clientExt1.chain.sType = WGPUSType_SamplerDescriptorAnisotropicFiltering;
    ...
    EXPECT_CALL(api, DeviceCreateSampler(apiDevice, NotNull()))
        .WillOnce(Invoke([&](Unused, const WGPUSamplerDescriptor* serverDesc) -> WGPUSampler {
            ...
            const auto* ext1 = reinterpret_cast<const WGPUSamplerDescriptorAnisotropicFiltering*>(
                serverDesc->nextInChain);
            EXPECT_EQ(ext1->chain.sType, clientExt1.chain.sType);
            EXPECT_EQ(ext1->maxAnisotropy, clientExt1.maxAnisotropy);

            const auto* ext2 = reinterpret_cast<const WGPUSamplerDescriptorAnisotropicFiltering*>(
                ext1->chain.next);
