...
class WireMemoryTransferServiceTests : public WireTest {
  ...
  protected:
    ...
    void ExpectClientWriteHandleGetData(ClientWriteHandle* handle, uint32_t* mappedData) {
        EXPECT_CALL(clientMemoryTransferService, OnWriteHandleGetData(handle))
            .WillOnce(Return(mappedData));
    }

    void MockClientWriteHandleGetDataFailure(ClientWriteHandle* handle) {
        EXPECT_CALL(clientMemoryTransferService, OnWriteHandleGetData(handle))
...
TEST_F(WireMemoryTransferServiceTests, BufferMapWriteSuccess) {
    ...
    // Since the mapping succeeds, the client gets mapped data pointer from the WriteHandle.
    ExpectClientWriteHandleGetData(clientHandle, &mMappedBufferContent);
...
TEST_F(WireMemoryTransferServiceTests, BufferMapWriteHandleOpenFailure) {
    ...
    // Since the mapping succeeds, the client gets mapped data pointer from the WriteHandle.
    // Mock a failure.
    MockClientWriteHandleGetDataFailure(clientHandle);
...
TEST_F(WireMemoryTransferServiceTests, MappedAtCreationSuccess) {
    ...
    // Staging data is immediately available so the mapped data pointer is acquired.
    ExpectClientWriteHandleGetData(clientHandle, &mMappedBufferContent);
