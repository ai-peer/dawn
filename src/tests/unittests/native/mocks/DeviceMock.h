...
namespace dawn_native {
    ...
    class DeviceMock : public DeviceBase {
      public:
        ~DeviceMock() override {
            // Device mock currently only deletes API objects (not other normal device destruction
            // calls) because the mock object is only the bare minimum for testing.
            DestroyApiObjects();
        }
        ...
        MOCK_METHOD(ResultOrError<Ref<QuerySetBase>>,
                    ...
                    (override));
        MOCK_METHOD(ResultOrError<Ref<RenderPipelineBase>>,
        ...
        MOCK_METHOD(void, DestroyDeviceImpl, (), (override));
