...
class DawnTestPlatform : public dawn_platform::Platform {
  public:
    void setPersistentCacheFuncs(dawn_platform::StorePersistentCacheCB set,
                                 dawn_platform::LoadPersistentCacheCB get,
                                 void* userCache) override {
        mStorePersistentCacheCB = set;
        mLoadPersistentCacheCB = get;
  ...
  private:
    ...
    // PersistentCache API
    size_t loadData(const void* key, size_t keySize, void* valueOut, size_t valueSize) override {
        return mLoadPersistentCacheCB(key, keySize, valueOut, valueSize, mUserCache);
    }

    void storeData(const void* key, size_t keySize, const void* value, size_t valueSize) override {
        mStorePersistentCacheCB(key, keySize, value, valueSize, mUserCache);
    }

    virtual bool arePersistentCacheFuncsSet() override {
        return mLoadPersistentCacheCB != nullptr && mStorePersistentCacheCB != nullptr;
    }

  private:
    dawn_platform::StorePersistentCacheCB mStorePersistentCacheCB = nullptr;
    dawn_platform::LoadPersistentCacheCB mLoadPersistentCacheCB = nullptr;

    void* mUserCache = nullptr;
};
