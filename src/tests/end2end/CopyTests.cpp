...
class CopyTests : public DawnTest {
    protected:
      ...
      struct TextureSpec {
        ...
        static BufferSpec GetBufferSpec(uint32_t width,
                                        uint32_t rowsPerImage,
                                        uint32_t arrayLayer,
                                        bool applyZeroRowsPerImage = true) {
            ...
            uint32_t appliedRowsPerImage = applyZeroRowsPerImage ? 0 : rowsPerImage;
        ...
        static BufferSpec MinimumBufferSpec(uint32_t width,
            ...
            return GetBufferSpec(width, height, arrayLayer);
        }
...
TEST_P(CopyTests_T2B, Texture2DArrayRegionNonzeroRowsPerImage) {
    ...
    DoTest(textureSpec, GetBufferSpec(kWidth, kRowsPerImage, kCopyLayers, false),
           {kWidth, kHeight, kCopyLayers});
}
