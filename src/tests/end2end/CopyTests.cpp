...
// Test copying multiple contiguous texture slices within the same texture with overlapped slices
// and non-overlapped copy regions.
TEST_P(CopyTests_T2T, CopyWithinSameTextureOverlappedSlicesNonOverlappedCopyRegions) {
    // TODO(jiawei.shao@intel.com): investigate why this test fails with swiftshader.
    DAWN_SKIP_TEST_IF(IsSwiftshader());
    ...
    constexpr std::array<wgpu::Origin3D, 4> kCopyOrigins = {
        {{0, 0, 0}, {0, kHeight / 2, 0}, {kWidth / 2, 0, 0}, {kWidth / 2, kHeight / 2, 0}}};

    // srcBaseLayer < dstBaseLayer
    {
        constexpr uint32_t kSrcBaseLayer = 0u;
        constexpr uint32_t kDstBaseLayer = 1u;

        for (uint32_t srcIndex = 0; srcIndex < kCopyOrigins.size(); ++srcIndex) {
            for (uint32_t dstIndex = 0; dstIndex < kCopyOrigins.size(); ++dstIndex) {
                if (srcIndex == dstIndex) {
                    continue;
                }

                const wgpu::Origin3D kSrcOrigin = kCopyOrigins[srcIndex];
                const wgpu::Origin3D kDstOrigin = kCopyOrigins[dstIndex];

                DoTest({kWidth, kHeight, kSrcOrigin.x, kSrcOrigin.y, 0, kLayers, kSrcBaseLayer},
                       {kWidth, kHeight, kDstOrigin.x, kDstOrigin.y, 0, kLayers, kDstBaseLayer},
                       {kWidth / 2, kHeight / 2, kCopyArrayLayerCount}, true);
            }
        }
    }
    ...
    {
        constexpr uint32_t kSrcBaseLayer = 1u;
        constexpr uint32_t kDstBaseLayer = 0u;
        for (uint32_t srcIndex = 0; srcIndex < kCopyOrigins.size(); ++srcIndex) {
            for (uint32_t dstIndex = 0; dstIndex < kCopyOrigins.size(); ++dstIndex) {
                if (srcIndex == dstIndex) {
                    continue;
