...
class CopyTests_T2B : public CopyTests {
    protected:
      void DoTest(const TextureSpec& textureSpec,
          ...
          const uint32_t rowsPerImage = textureSpec.height >> textureSpec.level;
          const utils::BufferTextureCopyLayout copyLayout =
              utils::GetBufferTextureCopyLayoutForTextureAtLevel(
                  kTextureFormat, textureSpec.width, textureSpec.height, textureSpec.arraySize,
                  textureSpec.level, rowsPerImage);
          ...
          const uint32_t texelCountInCopyRegion =
              bufferSpec.bytesPerRow / bytesPerTexel * (testCopySize.height - 1) +
              testCopySize.width;
          ...
          for (uint32_t slice = textureSpec.baseArrayLayer; slice < maxArrayLayer; ++slice) {
              ...
              const uint32_t expectedTexelArrayDataStartIndex =
                  texelIndexOffset + (textureSpec.x + textureSpec.y * copyLayout.texelBlocksPerRow);

              PackTextureData(&textureArrayData[expectedTexelArrayDataStartIndex],
                              testCopySize.width, testCopySize.height, copyLayout.texelBlocksPerRow,
...
protected:
    ...
    void DoTest(const TextureSpec& textureSpec,
        ...
        for (uint32_t slice = textureSpec.baseArrayLayer; slice < maxArrayLayer; ++slice) {
            ...
            PackTextureData(&bufferData[bufferOffset / bytesPerTexel], testCopySize.width,
                            testCopySize.height, bufferSpec.bytesPerRow / bytesPerTexel,
...
class CopyTests_T2T : public CopyTests {
  protected:
    void DoTest(const TextureSpec& srcSpec,
        ...
        // `level` mip level
        const uint32_t rowsPerImage = srcSpec.height >> srcSpec.level;
        const utils::BufferTextureCopyLayout copyLayout =
            utils::GetBufferTextureCopyLayoutForTextureAtLevel(kTextureFormat, srcSpec.width,
                                                               srcSpec.height, copySize.depth,
        ...
        for (uint32_t slice = 0; slice < copySize.depth; ++slice) {
            ...
            const uint32_t texelIndexOffset = copyLayout.texelBlocksPerImage * slice;
            const uint32_t expectedTexelArrayDataStartIndex =
                texelIndexOffset + (srcSpec.x + srcSpec.y * copyLayout.texelBlocksPerRow);
