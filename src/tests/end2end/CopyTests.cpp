  ...
  protected:
    void DoTest(const TextureSpec& textureSpec, const BufferSpec& bufferSpec) {
        ...
        for (uint32_t slice = 0; slice < textureSpec.arraySize; ++slice) {
            ...
            FillTextureData(width, height, rowPitch / kBytesPerTexel,
                            textureArrayData[slice].data(), slice);
        ...
        // Create a buffer of size `size * textureSpec.arrayLayer` and populate it with empty data
        // (0,0,0,0) Note: Prepopulating the buffer with empty data ensures that there is not random
        // data in the expectation and helps ensure that the padding due to the row pitch is not
        ...
        buffer.SetSubData(0, static_cast<uint32_t>(emptyData.size() * sizeof(RGBA8)),
                          reinterpret_cast<const uint8_t*>(emptyData.data()));

        uint32_t bufferOffset = bufferSpec.offset;
        for (uint32_t slice = 0; slice < textureSpec.arraySize; ++slice) {
            // Copy the region [(`x`, `y`), (`x + copyWidth, `y + copyWidth`)] from the `level` mip
            // into the buffer at `offset + bufferSpec.size * slice` and `rowPitch`
        ...
        std::vector<RGBA8> expected(bufferSpec.rowPitch / kBytesPerTexel *
        ...
        for (uint32_t slice = 0; slice < textureSpec.arraySize; ++slice) {
            ...
            PackTextureData(&textureArrayData[slice][textureSpec.x +
                                                     textureSpec.y * (rowPitch / kBytesPerTexel)],
                            textureSpec.copyWidth, textureSpec.copyHeight,
                            rowPitch / kBytesPerTexel, expected.data(),
            ...
            EXPECT_BUFFER_U32_RANGE_EQ(reinterpret_cast<const uint32_t*>(expected.data()), buffer,
                ...
                << " layer " << slice << " to " << bufDescriptor.size << "-byte buffer with offset "
                << bufferOffset << " and row pitch " << bufferSpec.rowPitch << std::endl;
  ...
  protected:
    ...
    void DoTest(const TextureSpec& textureSpec, const BufferSpec& bufferSpec) {
        ...
        buffer.SetSubData(0, static_cast<uint32_t>(bufferData.size() * sizeof(RGBA8)),
                          reinterpret_cast<const uint8_t*>(bufferData.data()));
        ...
        // from the buffer at the specified `offset` and `rowPitch`
        {
            dawn::BufferCopyView bufferCopyView =
        ...
        uint32_t rowPitch =
            Align(kBytesPerTexel * textureSpec.copyWidth, kTextureRowPitchAlignment);
        std::vector<RGBA8> expected(rowPitch / kBytesPerTexel * (textureSpec.copyHeight - 1) +
                                    textureSpec.copyWidth);
        PackTextureData(&bufferData[bufferSpec.offset / kBytesPerTexel], textureSpec.copyWidth,
                        textureSpec.copyHeight, bufferSpec.rowPitch / kBytesPerTexel,
        ...
        EXPECT_TEXTURE_RGBA8_EQ(expected.data(), texture, textureSpec.x, textureSpec.y,
            ...
            << "-byte buffer with offset " << bufferSpec.offset << " and row pitch "
            << bufferSpec.rowPitch << " to [(" << textureSpec.x << ", " << textureSpec.y << "), ("
...
TEST_P(CopyTests_T2B, TextureMipAligned) {
    ...
    for (unsigned int i = 1; i < 4; ++i) {
        DoTest({kWidth, kHeight, 0, 0, kWidth >> i, kHeight >> i, i},
               MinimumBufferSpec(kWidth >> i, kHeight >> i));
    }
}
...
// Test that copying without a 512-byte aligned buffer offset that is greater than the row pitch
// works
...
// Test that copying with a greater row pitch than needed on a texture that is not 256-byte aligned
// works
...
TEST_P(CopyTests_T2B, Texture2DArrayRegion) {
    ...
    DoTest({kWidth, kHeight, 0, 0, kWidth, kHeight, 0, kLayers},
           MinimumBufferSpec(kWidth, kHeight));
}
...
TEST_P(CopyTests_T2B, Texture2DArrayMip) {
    ...
    constexpr uint32_t kLayers = 6u;
    for (unsigned int i = 1; i < 4; ++i) {
        DoTest({kWidth, kHeight, 0, 0, kWidth >> i, kHeight >> i, i, kLayers},
               MinimumBufferSpec(kWidth >> i, kHeight >> i));
    }
}
...
DAWN_INSTANTIATE_TEST(CopyTests_T2T, D3D12Backend, MetalBackend, VulkanBackend);