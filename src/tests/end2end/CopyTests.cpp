...
class CopyTests_T2B : public CopyTests {
    protected:
      void DoTest(const TextureSpec& textureSpec, const BufferSpec& bufferSpec) {
          ...
          descriptor.size.width = textureSpec.textureSize.width;
          descriptor.size.height = textureSpec.textureSize.height;
          descriptor.size.depth = 1;
          descriptor.arrayLayerCount = textureSpec.textureSize.depth;
          ...
          const utils::BufferTextureCopyLayout copyLayout =
              utils::GetBufferTextureCopyLayoutForTextureAtLevel(
          ...
          for (uint32_t slice = 0; slice < textureSpec.textureSize.depth; ++slice) {
              ...
              wgpu::Extent3D copySize = {copyLayout.mipSize.width, copyLayout.mipSize.height, 1};
          ...
          const uint32_t maxArrayLayer = textureSpec.copyOrigin.z + textureSpec.copySize.depth;
          ...
          for (uint32_t slice = textureSpec.copyOrigin.z; slice < maxArrayLayer; ++slice) {
              ...
              wgpu::TextureCopyView textureCopyView = utils::CreateTextureCopyView(
                  texture, textureSpec.level, slice,
                  {textureSpec.copyOrigin.x, textureSpec.copyOrigin.y, 0});
              wgpu::BufferCopyView bufferCopyView =
                  utils::CreateBufferCopyView(buffer, bufferOffset, bufferSpec.bytesPerRow, 0);
              wgpu::Extent3D copySize = {textureSpec.copySize.width, textureSpec.copySize.height,
                                         1};
          ...
          const uint32_t texelCountInCopyRegion =
              bufferSpec.bytesPerRow / bytesPerTexel * (textureSpec.copySize.height - 1) +
              textureSpec.copySize.width;
          ...
          for (uint32_t slice = textureSpec.copyOrigin.z; slice < maxArrayLayer; ++slice) {
              ...
              PackTextureData(&textureArrayData[expectedTexelArrayDataStartIndex],
                              textureSpec.copySize.width, textureSpec.copySize.height,
                              copyLayout.texelBlocksPerRow, expected.data(),
              ...
              EXPECT_BUFFER_U32_RANGE_EQ(reinterpret_cast<const uint32_t*>(expected.data()), buffer,
                  ...
                  << textureSpec.copyOrigin.x + textureSpec.copySize.width << ", "
                  << textureSpec.copyOrigin.y + textureSpec.copySize.height << ")) from "
    ...
    void DoTest(const TextureSpec& textureSpec, const BufferSpec& bufferSpec) {
        ...
        for (uint32_t slice = textureSpec.copyOrigin.z; slice < maxArrayLayer; ++slice) {
            ...
            wgpu::TextureCopyView textureCopyView = utils::CreateTextureCopyView(
                texture, textureSpec.level, slice,
                {textureSpec.copyOrigin.x, textureSpec.copyOrigin.y, 0});
            wgpu::Extent3D copySize = {textureSpec.copySize.width, textureSpec.copySize.height, 1};
            encoder.CopyBufferToTexture(&bufferCopyView, &textureCopyView, &copySize);
            bufferOffset += copyLayout.bytesPerImage;
        }
        ...
        for (uint32_t slice = textureSpec.copyOrigin.z; slice < maxArrayLayer; ++slice) {
            ...
            PackTextureData(&bufferData[bufferOffset / bytesPerTexel], textureSpec.copySize.width,
                            textureSpec.copySize.height, bufferSpec.bytesPerRow / bytesPerTexel,
                            expected.data(), textureSpec.copySize.width);

            EXPECT_TEXTURE_RGBA8_EQ(expected.data(), texture, textureSpec.copyOrigin.x,
                                    textureSpec.copyOrigin.y, textureSpec.copySize.width,
                                    textureSpec.copySize.height, textureSpec.level, slice)
                ...
                << textureSpec.copyOrigin.y << "), ("
                << textureSpec.copyOrigin.x + textureSpec.copySize.width << ", "
                << textureSpec.copyOrigin.y + textureSpec.copySize.height << ")) region of "
                << textureSpec.textureSize.width << " x " << textureSpec.textureSize.height
...
class CopyTests_T2T : public CopyTests {
  protected:
    void DoTest(const TextureSpec& srcSpec,
        ...
        ASSERT(srcSpec.copySize.width == dstSpec.copySize.width &&
               srcSpec.copySize.height == dstSpec.copySize.height &&
               srcSpec.copySize.depth == dstSpec.copySize.depth);

        wgpu::TextureDescriptor srcDescriptor;
        srcDescriptor.dimension = wgpu::TextureDimension::e2D;
        srcDescriptor.size.width = srcSpec.textureSize.width;
        srcDescriptor.size.height = srcSpec.textureSize.height;
        srcDescriptor.size.depth = 1;
        srcDescriptor.arrayLayerCount = srcSpec.textureSize.depth;
        ...
        if (copyWithinSameTexture) {
            ...
            dstDescriptor.size.width = dstSpec.textureSize.width;
            dstDescriptor.size.height = dstSpec.textureSize.height;
            dstDescriptor.size.depth = 1;
            dstDescriptor.arrayLayerCount = dstSpec.textureSize.depth;
        ...
        const utils::BufferTextureCopyLayout copyLayout =
            utils::GetBufferTextureCopyLayoutForTextureAtLevel(
                kTextureFormat,
                {srcSpec.textureSize.width, srcSpec.textureSize.height, srcSpec.copySize.depth},
        ...
        uint64_t uploadBufferOffset = 0;
        for (uint32_t slice = 0; slice < srcSpec.copySize.depth; ++slice) {
            wgpu::BufferCopyView bufferCopyView = utils::CreateBufferCopyView(
                uploadBuffer, uploadBufferOffset, copyLayout.bytesPerRow, 0);
            wgpu::TextureCopyView textureCopyView = utils::CreateTextureCopyView(
                srcTexture, srcSpec.level, srcSpec.copyOrigin.z + slice, {0, 0, 0});
            wgpu::Extent3D copySize = {copyLayout.mipSize.width, copyLayout.mipSize.height, 1};
            encoder.CopyBufferToTexture(&bufferCopyView, &textureCopyView, &copySize);
            uploadBufferOffset += copyLayout.bytesPerImage;
        }
        ...
        wgpu::TextureCopyView srcTextureCopyView =
            utils::CreateTextureCopyView(srcTexture, srcSpec.level, srcSpec.copyOrigin.z,
                                         {srcSpec.copyOrigin.x, srcSpec.copyOrigin.y, 0});
        wgpu::TextureCopyView dstTextureCopyView =
            utils::CreateTextureCopyView(dstTexture, dstSpec.level, dstSpec.copyOrigin.z,
                                         {dstSpec.copyOrigin.x, dstSpec.copyOrigin.y, 0});
        encoder.CopyTextureToTexture(&srcTextureCopyView, &dstTextureCopyView, &srcSpec.copySize);

        wgpu::CommandBuffer commands = encoder.Finish();
        queue.Submit(1, &commands);

        const uint32_t texelCountInCopyRegion =
            copyLayout.texelBlocksPerRow * (srcSpec.copySize.height - 1) + srcSpec.copySize.width;
        std::vector<RGBA8> expected(texelCountInCopyRegion);
        for (uint32_t slice = 0; slice < srcSpec.copySize.depth; ++slice) {
            std::fill(expected.begin(), expected.end(), RGBA8());
            const uint32_t texelIndexOffset = copyLayout.texelBlocksPerImage * slice;
            ...
            PackTextureData(&textureArrayCopyData[expectedTexelArrayDataStartIndex],
                            srcSpec.copySize.width, srcSpec.copySize.height,
                            copyLayout.texelBlocksPerRow, expected.data(), srcSpec.copySize.width);

            EXPECT_TEXTURE_RGBA8_EQ(expected.data(), dstTexture, dstSpec.copyOrigin.x,
                                    dstSpec.copyOrigin.y, srcSpec.copySize.width,
                                    srcSpec.copySize.height, dstSpec.level,
                                    dstSpec.copyOrigin.z + slice)
                << "Texture to Texture copy failed copying region [(" << srcSpec.copyOrigin.x
                << ", " << srcSpec.copyOrigin.y << "), ("
                << srcSpec.copyOrigin.x + srcSpec.copySize.width << ", "
                << srcSpec.copyOrigin.y + srcSpec.copySize.height << ")) from "
                ...
                << dstSpec.copyOrigin.y << "), (" << dstSpec.copyOrigin.x + srcSpec.copySize.width
                << ", " << dstSpec.copyOrigin.y + srcSpec.copySize.height << ")) region of "
...
TEST_P(CopyTests_T2B, FullTextureAligned) {
    ...
    textureSpec.copySize = {kWidth, kHeight, 1};
    textureSpec.level = 0;

    DoTest(textureSpec, MinimumBufferSpec(kWidth, kHeight));
}
...
TEST_P(CopyTests_T2B, TextureRegionAligned) {
    ...
    for (unsigned int w : {64, 128, 256}) {
        for (unsigned int h : { 16, 32, 48 }) {
            TextureSpec textureSpec;
            textureSpec.copyOrigin = {0, 0, 0};
            textureSpec.copySize = {w, h, 1};
            textureSpec.level = 0;
            textureSpec.textureSize = {kWidth, kHeight, 1};
            DoTest(textureSpec, MinimumBufferSpec(w, h));
        }
    }
}
...
TEST_P(CopyTests_T2B, TextureMipAligned) {
    ...
    for (unsigned int i = 1; i < 4; ++i) {
        TextureSpec textureSpec = defaultTextureSpec;
        textureSpec.level = i;
        textureSpec.copySize = {kWidth >> i, kHeight >> i, 1};
...
TEST_P(CopyTests_T2B, Texture2DArrayRegion) {
    ...
    TextureSpec textureSpec;
    textureSpec.copyOrigin = {0, 0, 0};
    textureSpec.copySize = {kWidth, kHeight, kLayers};
    textureSpec.textureSize = {kWidth, kHeight, kLayers};
    textureSpec.level = 0;

    DoTest(textureSpec, MinimumBufferSpec(kWidth, kHeight, kLayers));
}
...
TEST_P(CopyTests_T2B, Texture2DArraySubRegion) {
    ...
    textureSpec.copySize = {kWidth, kHeight, kCopyLayers};
    textureSpec.textureSize = {kWidth, kHeight, kLayers};
    textureSpec.level = 0;
...
TEST_P(CopyTests_T2B, Texture2DArrayMip) {
    ...
    for (unsigned int i = 1; i < 4; ++i) {
        TextureSpec textureSpec = defaultTextureSpec;
        textureSpec.copySize = {kWidth >> i, kHeight >> i, kLayers};
        textureSpec.level = i;
...
TEST_P(CopyTests_T2T, Texture) {
    ...
    textureSpec.level = 0;
    textureSpec.textureSize = {kWidth, kHeight, 1};
    DoTest(textureSpec, textureSpec);
}
...
TEST_P(CopyTests_T2T, Texture2DArrayRegion) {
    ...
    for (unsigned int w : {64, 128, 256}) {
        for (unsigned int h : {16, 32, 48}) {
            TextureSpec textureSpec = defaultTextureSpec;
            textureSpec.copySize = {w, h, kLayers};
            DoTest(textureSpec, textureSpec);
        }
    }
}
...
TEST_P(CopyTests_T2T, Texture2DArrayCopyOneSlice) {
    ...
    defaultTextureSpec.textureSize = {kWidth, kHeight, kLayers};
    defaultTextureSpec.level = 0;
    defaultTextureSpec.copySize = {kWidth, kHeight, kCopyArrayLayerCount};
...
TEST_P(CopyTests_T2T, CopyWithinSameTextureOneSlice) {
    ...
    DoTest(srcTextureSpec, dstTextureSpec, true);
}
...
TEST_P(CopyTests_T2T, SingleMipSrcMultipleMipDst) {
    ...
    for (unsigned int i = 1; i < 4; ++i) {
        ...
        srcTextureSpec.copySize = {kWidth >> i, kHeight >> i, 1};

        TextureSpec dstTextureSpec = defaultTextureSpec;
        dstTextureSpec.textureSize = {kWidth, kHeight, 1};
        dstTextureSpec.level = i;
        dstTextureSpec.copySize = {kWidth >> i, kHeight >> i, 1};
