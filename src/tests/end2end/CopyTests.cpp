    ...
    protected:
        ...
        void DoTest(const TextureSpec& textureSpec, const BufferSpec& bufferSpec) {
            ...
            for (uint32_t slice = 0; slice < textureSpec.arrayLayer; ++slice) {
                ...
                dawn::BufferCopyView bufferCopyView = utils::CreateBufferCopyView(uploadBuffer, 0, rowPitch, 0);
                dawn::TextureCopyView textureCopyView = utils::CreateTextureCopyView(texture, textureSpec.level, slice, 0, 0, 0, dawn::TextureAspect::Color);
                dawn::Extent3D copySize = utils::CreateExtent3D(width, height, 1);
            ...
            for (uint32_t slice = 0; slice < textureSpec.arrayLayer; ++slice) {
                ...
                dawn::TextureCopyView textureCopyView = utils::CreateTextureCopyView(texture, textureSpec.level, slice, textureSpec.x, textureSpec.y, 0, dawn::TextureAspect::Color);
                dawn::BufferCopyView bufferCopyView = utils::CreateBufferCopyView(buffer, bufferOffset, bufferSpec.rowPitch, 0);
                dawn::Extent3D copySize = utils::CreateExtent3D(textureSpec.copyWidth, textureSpec.copyHeight, 1);
...
protected:
    ...
    void DoTest(const TextureSpec& textureSpec, const BufferSpec& bufferSpec) {
        ...
        // and helps ensure that the padding due to the row pitch is not modified by the copy
        {
            ...
            dawn::BufferCopyView bufferCopyView = utils::CreateBufferCopyView(uploadBuffer, 0, rowPitch, 0);
            dawn::TextureCopyView textureCopyView = utils::CreateTextureCopyView(texture, textureSpec.level, 0, 0, 0, 0, dawn::TextureAspect::Color);
            dawn::Extent3D copySize = utils::CreateExtent3D(width, height, 1);
            cmdBuilder.CopyBufferToTexture(&bufferCopyView, &textureCopyView, &copySize);
        }
        ...
        {
            dawn::BufferCopyView bufferCopyView = utils::CreateBufferCopyView(buffer, bufferSpec.offset, bufferSpec.rowPitch, 0);
            dawn::TextureCopyView textureCopyView = utils::CreateTextureCopyView(texture, textureSpec.level, 0, textureSpec.x, textureSpec.y, 0, dawn::TextureAspect::Color);
            dawn::Extent3D copySize = utils::CreateExtent3D(textureSpec.copyWidth, textureSpec.copyHeight, 1);
            cmdBuilder.CopyBufferToTexture(&bufferCopyView, &textureCopyView, &copySize);
        }
