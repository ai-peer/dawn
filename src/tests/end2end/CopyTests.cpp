...
class CopyTests : public DawnTest {
  protected:
    ...
    static BufferSpec MinimumBufferSpec(uint32_t width, uint32_t height, uint32_t depth = 1) {
        return MinimumBufferSpec({width, height, depth}, STRIDE_COMPUTE_DEFAULT,
                                 depth == 1 ? WGPU_STRIDE_UNDEFINED : STRIDE_COMPUTE_DEFAULT);
    }
...
TEST_P(CopyTests_T2B, BytesPerRowWithOneRowCopy) {
...

    {
        ...
        bufferSpec.bytesPerRow = WGPU_STRIDE_UNDEFINED;
...
TEST_P(CopyTests_T2B, StrideSpecialCases) {
    ...
    for (const wgpu::Extent3D copyExtent :
        ...
        DoTest(textureSpec, MinimumBufferSpec(copyExtent, WGPU_STRIDE_UNDEFINED, 2), copyExtent);
    }
    ...
    for (const wgpu::Extent3D copyExtent : {wgpu::Extent3D{2, 2, 1}, {2, 2, 0}}) {
        DoTest(textureSpec, MinimumBufferSpec(copyExtent, 256, WGPU_STRIDE_UNDEFINED), copyExtent);
    }
}
