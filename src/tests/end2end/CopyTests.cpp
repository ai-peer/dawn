...
class CopyTests_B2B : public DawnTest {
  protected:
    ...
    // only their size.
    void DoTest(uint64_t sourceSize, uint64_t sourceOffset, uint64_t destinationSize, uint64_t destinationOffset, uint64_t copySize) {
        ...
        wgpu::Buffer destination = utils::CreateBufferFromData(device, zeroes.data(), zeroes.size() * sizeof(uint32_t), wgpu::BufferUsage::CopyDst | wgpu::BufferUsage::CopySrc);

        std::vector<uint32_t> counts(static_cast<size_t>(sourceSize / sizeof(uint32_t)));
        for (size_t i = 0; i < counts.size(); i++) {
            counts[i] = i + 1;
        }
        wgpu::Buffer source = utils::CreateBufferFromData(device, counts.data(), counts.size() * sizeof(uint32_t), wgpu::BufferUsage::CopySrc);
        ...
        EXPECT_BUFFER_U32_RANGE_EQ(zeroes.data(), destination, 0, destinationOffset / sizeof(uint32_t));
        EXPECT_BUFFER_U32_RANGE_EQ(counts.data(), destination, destinationOffset, copySize / sizeof(uint32_t));
        uint64_t copyEnd = destinationOffset + copySize;
        EXPECT_BUFFER_U32_RANGE_EQ(zeroes.data(), destination, copyEnd, (destinationSize - copyEnd) / sizeof(uint32_t));
    }
};
...
DAWN_INSTANTIATE_TEST(CopyTests_B2B, D3D12Backend(), MetalBackend(), OpenGLBackend(), VulkanBackend());
