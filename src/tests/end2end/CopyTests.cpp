...
class CopyTests_B2B : public DawnTest {
  protected:
    ...
    void DoTest(uint64_t sourceSize,
        ...
        wgpu::Buffer source = utils::CreateBufferFromData(
            device, counts.data(), counts.size() * sizeof(uint32_t), wgpu::BufferUsage::CopySrc);
        ...
        EXPECT_BUFFER_U32_RANGE_EQ(counts.data() + sourceOffset / sizeof(uint32_t), destination,
