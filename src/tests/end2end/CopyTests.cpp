...
class CopyTests : public DawnTest {
    protected:
        ...
        static std::vector<RGBA8> GetExpectedTextureData(uint32_t width,
                                                         uint32_t height,
                                                         uint32_t texelsPerRow,
                                                         uint32_t arrayLayerCount) {
            const uint32_t texelCountPerCompleteLayer = texelsPerRow * height;
            const uint32_t texelCountLastLayer = texelsPerRow * (height - 1) + width;
            const uint32_t totalTexelCount =
                texelCountPerCompleteLayer * arrayLayerCount + texelCountLastLayer;
            std::vector<RGBA8> textureData(totalTexelCount);
            for (uint32_t layer = 0; layer < arrayLayerCount; ++layer) {
                const uint32_t texelIndexOffsetPerSlice = texelCountPerCompleteLayer * layer;
        ...
        BufferSpec MinimumBufferSpec(uint32_t width, uint32_t height, uint32_t arrayLayer = 1) {
            const uint32_t bytesPerRow =
                Align(width * kBytesPerTexel, kTextureBytesPerRowAlignment);
            const uint32_t totalBufferSize =
                bytesPerRow * (height * arrayLayer - 1) + width * kBytesPerTexel;
...
class CopyTests_T2B : public CopyTests {
    protected:
      void DoTest(const TextureSpec& textureSpec,
                  ...
                  const wgpu::Extent3D& testCopySize) {
          ...
          const uint32_t widthAtLevel = textureSpec.width >> textureSpec.level;
          const uint32_t heightAtLevel = textureSpec.height >> textureSpec.level;
          const uint32_t bytesPerRow =
              Align(kBytesPerTexel * widthAtLevel, kTextureBytesPerRowAlignment);
          const uint32_t texelsPerRow = bytesPerRow / kBytesPerTexel;
          const uint32_t texelCountPerCompleteLayer = texelsPerRow * heightAtLevel;
          const uint32_t texelCountLastLayer = texelsPerRow * (heightAtLevel - 1) + widthAtLevel;
          const uint32_t totalTexelCount =
              texelCountPerCompleteLayer * textureSpec.arraySize + texelCountLastLayer;

          wgpu::CommandEncoder encoder = device.CreateCommandEncoder();

          // Initialize the source texture
          std::vector<RGBA8> textureArrayData = GetExpectedTextureData(
              widthAtLevel, heightAtLevel, texelsPerRow, textureSpec.arraySize);

          // TODO(jiawei.shao@intel.com): copy into multiple texture array layers in one
          // buffer-to-texture copy command.
          const uint32_t bytesPerCompleteLayer = sizeof(RGBA8) * texelCountPerCompleteLayer;
          wgpu::Buffer uploadBuffer = utils::CreateBufferFromData(device, textureArrayData.data(),
                                                                  sizeof(RGBA8) * totalTexelCount,
                                                                  wgpu::BufferUsage::CopySrc);
          uint64_t uploadBufferOffset = 0;
          for (uint32_t slice = 0; slice < textureSpec.arraySize; ++slice) {
              wgpu::BufferCopyView bufferCopyView =
                  utils::CreateBufferCopyView(uploadBuffer, uploadBufferOffset, bytesPerRow, 0);
              wgpu::TextureCopyView textureCopyView =
                  utils::CreateTextureCopyView(texture, textureSpec.level, slice, {0, 0, 0});
              wgpu::Extent3D copySize = {widthAtLevel, heightAtLevel, 1};
              encoder.CopyBufferToTexture(&bufferCopyView, &textureCopyView, &copySize);
              uploadBufferOffset += bytesPerCompleteLayer;
          }
          ...
          const std::vector<RGBA8> emptyData(bufferSpec.size / kBytesPerTexel, RGBA8());
          wgpu::Buffer buffer =
              utils::CreateBufferFromData(device, emptyData.data(), bufferSpec.size,
                                          wgpu::BufferUsage::CopySrc | wgpu::BufferUsage::CopyDst);

          const uint32_t maxArrayLayer = textureSpec.baseArrayLayer + testCopySize.depth;
          ...
          for (uint32_t slice = textureSpec.baseArrayLayer; slice < maxArrayLayer; ++slice) {
              ...
              wgpu::Extent3D copySize = {testCopySize.width, testCopySize.height, 1};
              encoder.CopyTextureToBuffer(&textureCopyView, &bufferCopyView, &copySize);
              bufferOffset += bytesPerCompleteLayer;
          }
          ...
          const uint32_t texelCountInCopyRegion =
              bufferSpec.bytesPerRow / kBytesPerTexel * (testCopySize.height - 1) +
              testCopySize.width;
          ...
          for (uint32_t slice = textureSpec.baseArrayLayer; slice < maxArrayLayer; ++slice) {
              ...
              const uint32_t texelIndexOffset = texelCountPerCompleteLayer * slice;
              const uint32_t expectedTexelArrayDataStartIndex =
                  texelIndexOffset + (textureSpec.x + textureSpec.y * texelsPerRow);

              PackTextureData(&textureArrayData[expectedTexelArrayDataStartIndex],
                              testCopySize.width, testCopySize.height, texelsPerRow,
              ...
              EXPECT_BUFFER_U32_RANGE_EQ(reinterpret_cast<const uint32_t*>(expected.data()), buffer,
                  ...
                  << textureSpec.y << "), (" << textureSpec.x + testCopySize.width << ", "
                  << textureSpec.y + testCopySize.height << ")) from " << textureSpec.width << " x "
                  << textureSpec.height << " texture at mip level " << textureSpec.level
                  << " layer " << slice << " to " << bufferSpec.size << "-byte buffer with offset "
                  << bufferOffset << " and bytes per row " << bufferSpec.bytesPerRow << std::endl;
...
protected:
    ...
    void DoTest(const TextureSpec& textureSpec,
        ...
        const uint32_t texelsPerRow = bytesPerRow / kBytesPerTexel;
        const uint32_t texelCountPerCompleteLayer = texelsPerRow * heightAtLevel;
        const uint32_t texelCounLastLayer = texelsPerRow * (heightAtLevel - 1) + widthAtLevel;

        const uint32_t maxArrayLayer = textureSpec.baseArrayLayer + testCopySize.depth;
        const uint32_t bytesPerCompleteLayer = kBytesPerTexel * texelCountPerCompleteLayer;
        ...
        for (uint32_t slice = textureSpec.baseArrayLayer; slice < maxArrayLayer; ++slice) {
            ...
            std::vector<RGBA8> expected(texelCounLastLayer);
            PackTextureData(&bufferData[bufferOffset / kBytesPerTexel], testCopySize.width,
                            testCopySize.height, bufferSpec.bytesPerRow / kBytesPerTexel,
                            expected.data(), testCopySize.width);

            EXPECT_TEXTURE_RGBA8_EQ(expected.data(), texture, textureSpec.x, textureSpec.y,
                                    testCopySize.width, testCopySize.height, textureSpec.level,
                                    slice)
                ...
                << "), (" << textureSpec.x + testCopySize.width << ", "
                << textureSpec.y + testCopySize.height << ")) region of " << textureSpec.width
                << " x " << textureSpec.height << " texture at mip level " << textureSpec.level
                << " layer " << slice << std::endl;
            bufferOffset += bytesPerCompleteLayer;
        }
    }
};
...
class CopyTests_T2T : public CopyTests {
  protected:
    void DoTest(const TextureSpec& srcSpec,
        ...
        // `level` mip level
        uint32_t widthAtLevel = srcSpec.width >> srcSpec.level;
        uint32_t heightAtLevel = srcSpec.height >> srcSpec.level;
        uint32_t bytesPerRow = Align(kBytesPerTexel * widthAtLevel, kTextureBytesPerRowAlignment);
        uint32_t texelsPerRow = bytesPerRow / kBytesPerTexel;
        const uint32_t texelCountPerCompleteLayer = texelsPerRow * heightAtLevel;
        const uint32_t texelCountLastLayer = texelsPerRow * (heightAtLevel - 1) + widthAtLevel;
        const uint32_t totalTexelCount =
            texelCountPerCompleteLayer * copySize.depth + texelCountLastLayer;

        std::vector<RGBA8> textureArrayCopyData =
            GetExpectedTextureData(widthAtLevel, heightAtLevel, texelsPerRow, copySize.depth);

        // TODO(jiawei.shao@intel.com): support copying into multiple contiguous array layers in one
        // copyBufferToTexture() call.
        const uint32_t bytesPerCompleteLayer = sizeof(RGBA8) * texelCountPerCompleteLayer;
        wgpu::Buffer uploadBuffer = utils::CreateBufferFromData(device, textureArrayCopyData.data(),
                                                                sizeof(RGBA8) * totalTexelCount,
                                                                wgpu::BufferUsage::CopySrc);
        uint64_t uploadBufferOffset = 0;
        ...
        const uint32_t texelCountInCopyRegion =
            bytesPerRow / kBytesPerTexel * (copySize.height - 1) + copySize.width;
        std::vector<RGBA8> expected(texelCountInCopyRegion);
        for (uint32_t slice = 0; slice < copySize.depth; ++slice) {
            std::fill(expected.begin(), expected.end(), RGBA8());
            const uint32_t texelIndexOffset = texelCountPerCompleteLayer * slice;
            const uint32_t expectedTexelArrayDataStartIndex =
                texelIndexOffset + (srcSpec.x + srcSpec.y * texelsPerRow);
            PackTextureData(&textureArrayCopyData[expectedTexelArrayDataStartIndex], copySize.width,
                            copySize.height, texelsPerRow, expected.data(), copySize.width);

            EXPECT_TEXTURE_RGBA8_EQ(expected.data(), dstTexture, dstSpec.x, dstSpec.y,
                                    copySize.width, copySize.height, dstSpec.level,
                                    dstSpec.baseArrayLayer + slice)
                << "Texture to Texture copy failed copying region [(" << srcSpec.x << ", "
                << srcSpec.y << "), (" << srcSpec.x + copySize.width << ", "
                << srcSpec.y + copySize.height << ")) from " << srcSpec.width << " x "
                << srcSpec.height << " texture at mip level " << srcSpec.level << " layer "
                << srcSpec.baseArrayLayer + slice << " to [(" << dstSpec.x << ", " << dstSpec.y
                << "), (" << dstSpec.x + copySize.width << ", " << dstSpec.y + copySize.height
...
TEST_P(CopyTests_T2B, FullTextureAligned) {
    ...
    DoTest({kWidth, kHeight, 0, 0, 0}, MinimumBufferSpec(kWidth, kHeight), {kWidth, kHeight, 1});
}
...
TEST_P(CopyTests_T2B, PixelReadAligned) {
    ...
    DoTest({kWidth, kHeight, 0, 0, 0}, pixelBuffer, kCopySize);
    DoTest({kWidth, kHeight, kWidth - 1, 0, 0}, pixelBuffer, kCopySize);
    DoTest({kWidth, kHeight, 0, kHeight - 1, 0}, pixelBuffer, kCopySize);
    DoTest({kWidth, kHeight, kWidth - 1, kHeight - 1, 0}, pixelBuffer, kCopySize);
    DoTest({kWidth, kHeight, kWidth / 3, kHeight / 7, 0}, pixelBuffer, kCopySize);
    DoTest({kWidth, kHeight, kWidth / 7, kHeight / 3, 0}, pixelBuffer, kCopySize);
}
...
TEST_P(CopyTests_T2B, TextureRegionAligned) {
    ...
    for (unsigned int w : {64, 128, 256}) {
        for (unsigned int h : { 16, 32, 48 }) {
            DoTest({kWidth, kHeight, 0, 0, 0}, MinimumBufferSpec(w, h), {w, h, 1});
        }
    }
}
...
TEST_P(CopyTests_T2B, TextureMipAligned) {
    ...
    for (unsigned int i = 1; i < 4; ++i) {
        DoTest({kWidth, kHeight, 0, 0, i}, MinimumBufferSpec(kWidth >> i, kHeight >> i),
...
TEST_P(CopyTests_T2B, OffsetBufferAligned) {
    ...
    for (unsigned int i = 0; i < 3; ++i) {
        ...
        DoTest({kWidth, kHeight, 0, 0, 0}, bufferSpec, {kWidth, kHeight, 1});
    }
}
...
TEST_P(CopyTests_T2B, Texture2DArrayRegion) {
    ...
    DoTest({kWidth, kHeight, 0, 0, 0, kLayers}, MinimumBufferSpec(kWidth, kHeight, kLayers),
...
TEST_P(CopyTests_T2B, Texture2DArraySubRegion) {
    ...
    DoTest({kWidth, kHeight, 0, 0, 0, kLayers, kBaseLayer},
           MinimumBufferSpec(kWidth, kHeight, kCopyLayers), {kWidth, kHeight, kCopyLayers});
}
...
TEST_P(CopyTests_T2B, Texture2DArrayMip) {
    ...
    constexpr uint32_t kLayers = 6u;
    for (unsigned int i = 1; i < 4; ++i) {
        DoTest({kWidth, kHeight, 0, 0, i, kLayers},
               MinimumBufferSpec(kWidth >> i, kHeight >> i, kLayers),
...
TEST_P(CopyTests_T2T, Texture) {
    constexpr uint32_t kWidth = 256;
    constexpr uint32_t kHeight = 128;
    DoTest({kWidth, kHeight, 0, 0, 0}, {kWidth, kHeight, 0, 0, 0}, {kWidth, kHeight, 1});
}
...
TEST_P(CopyTests_T2T, TextureRegion) {
    ...
    for (unsigned int w : {64, 128, 256}) {
        for (unsigned int h : {16, 32, 48}) {
            DoTest({kWidth, kHeight, 0, 0, 0, 1}, {kWidth, kHeight, 0, 0, 0, 1}, {w, h, 1});
        }
    }
}
...
TEST_P(CopyTests_T2T, TextureMip) {
    ...
    constexpr uint32_t kHeight = 128;
    for (unsigned int i = 1; i < 4; ++i) {
        DoTest({kWidth, kHeight, 0, 0, i}, {kWidth, kHeight, 0, 0, i},
               {kWidth >> i, kHeight >> i, 1});
    }
}
