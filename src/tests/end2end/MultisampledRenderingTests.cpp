...
class MultisampledRenderingTest : public DawnTest {
  protected:
    ...
    wgpu::RenderPipeline CreateRenderPipelineWithOneOutputForTest(bool testDepth, uint32_t sampleMask = 0xFFFFFFFF) {
    ...
    wgpu::RenderPipeline CreateRenderPipelineWithTwoOutputsForTest(uint32_t sampleMask = 0xFFFFFFFF) {
    ...
    void VerifyResolveTarget(const wgpu::Color& inputColor,
                             ...
                             const float kMSAACoverage = 0.5f) {
...
TEST_P(MultisampledRenderingTest, ResolveInto2DTextureWithSampleMask) {
    ...
    constexpr uint32_t kSampleMask = 0x00000006;
    constexpr float kMSAACoverage = 0.25f;
    wgpu::CommandEncoder commandEncoder = device.CreateCommandEncoder();
    wgpu::RenderPipeline pipeline = CreateRenderPipelineWithOneOutputForTest(kTestDepth, kSampleMask);
...
// Test doing MSAA resolve into multiple resolve targets works correctly with a non-default sample mask.
TEST_P(MultisampledRenderingTest, ResolveIntoMultipleResolveTargetsWithSampleMask) {
    ...
    constexpr uint32_t kSampleMask = 0x00000009;
    constexpr float kMSAACoverage = 0.25f;
...
TEST_P(MultisampledRenderingTest, MultisampledRenderingWithDepthTestAndSampleMask) {
    ...
    constexpr bool kTestDepth = true;
    // The second sample is included and it's covered by the triangle.
    constexpr uint32_t kSampleMask = 0x00000002;
    constexpr float kMSAACoverage = 0.25f;
