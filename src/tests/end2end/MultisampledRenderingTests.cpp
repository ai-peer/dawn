...
// Test doing MSAA resolve with multisampled 2D array textures works correctly.
TEST_P(MultisampledRenderingTest, ResolveFromMultisampled2DArrayTexture) {
    dawn::Texture multisampled2DArrayTexture =
        CreateTextureForOutputAttachment(kColorFormat, kSampleCount, 1, 2);
    ...
    textureViewDescriptor.baseArrayLayer = 1;
    dawn::TextureView colorView =
        multisampled2DArrayTexture.CreateTextureView(&textureViewDescriptor);

    dawn::CommandEncoder commandEncoder = device.CreateCommandEncoder();

    // Draw a green triangle and do MSAA resolve on a layer of a multisampled 2D array texture.
    {
        utils::ComboRenderPassDescriptor renderPass = CreateComboRenderPassDescriptorForTest(
            {colorView}, {mResolveView}, dawn::LoadOp::Clear, dawn::LoadOp::Clear);
