...
class MultisampledRenderingTest : public DawnTest {
  protected:
    ...
    wgpu::Texture CreateTextureForOutputAttachment(wgpu::TextureFormat format,
        ...
        const uint32_t width = kWidth << (mipLevelCount - 1);
        const uint32_t height = kHeight << (mipLevelCount - 1);
        constexpr wgpu::TextureUsage kUsage =
            wgpu::TextureUsage::OutputAttachment | wgpu::TextureUsage::CopySrc;
        if (sampleCount > 1) {
            ASSERT(mipLevelCount == 1 && arrayLayerCount == 1);
            return utils::Create2DMultisampledTexture(device, width, height, format, kUsage,
                                                      sampleCount);
        } else {
            return utils::Create2DArrayTexture(device, width, height, arrayLayerCount, format,
                                               kUsage, mipLevelCount);
        }
    }
