...
class DepthClampingTest : public DawnTest {
  protected:
    ...
    void DoTest(const std::vector<TestSpec>& testParams, const RGBA8& expected) {
        ...
        EXPECT_PIXEL_RGBA8_EQ(expected, renderTarget, kRTSize / 4, kRTSize / 2)
            << "Pixel check failed";
    }
...
TEST_P(DepthClampingTest, ClampOnBeyondFarPlane) {
    ...
    clampingState.sType = wgpu::SType::PrimitiveDepthClampingState;
...
TEST_P(DepthClampingTest, MultipleRenderPipelines) {
    ...
    clippingState.sType = wgpu::SType::PrimitiveDepthClampingState;
