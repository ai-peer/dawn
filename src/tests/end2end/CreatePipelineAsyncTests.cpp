...
TEST_P(CreatePipelineAsyncTest, ReleaseEntryPointsAfterCreateRenderPipelineAsync) {
    ...
    std::unique_ptr<std::string> vertexEntryPoint = std::make_unique<std::string>("main");
    std::unique_ptr<std::string> fragmentEntryPoint = std::make_unique<std::string>("main");
    ...
    vertexEntryPoint.reset();
    fragmentEntryPoint.reset();
    ...
    utils::ComboRenderPassDescriptor renderPass({renderTargetView});
    {
        ...
        renderPipelineDescriptor.cFragment.module = utils::CreateShaderModule(device, R"(
         struct FragmentInput {
            [[location(0)]] fragColorIn : vec4<f32>;
        };

        [[stage(fragment)]]
        fn main(fragmentInput : FragmentInput) -> [[location(0)]] vec4<f32> {
            return fragmentInput.fragColorIn;
        })");
