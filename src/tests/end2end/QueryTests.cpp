...
TEST_P(OcclusionQueryTests, Rewrite) {
    ...
    // Rewrite same query index in same query set with draw call
...
TEST_P(TimestampQueryTests, TimestampOnCommandEncoder) {
    ...
    // Write timestamp on different query index
    {
    ...
    // Write timestamp on same query index outside pass on same encoder
    {
        wgpu::QuerySet querySet = CreateQuerySetForTimestamp(kQueryCount);
        wgpu::Buffer destination = CreateResolveBuffer(kQueryCount * sizeof(uint64_t));
...
TEST_P(TimestampQueryTests, TimestampOnRenderPass) {
    ...
    // Write timestamp on same query index, not need test rewrite inside render pass due to it's
    // not allowed
    {
        wgpu::QuerySet querySet = CreateQuerySetForTimestamp(kQueryCount);
        wgpu::Buffer destination = CreateResolveBuffer(kQueryCount * sizeof(uint64_t));
...
TEST_P(TimestampQueryTests, TimestampOnComputePass) {
    ...
    // Write timestamp on same query index on both the outside and the inside of the compute pass
    {
        wgpu::QuerySet querySet = CreateQuerySetForTimestamp(kQueryCount);
        wgpu::Buffer destination = CreateResolveBuffer(kQueryCount * sizeof(uint64_t));
    ...
    // Write timestamp on same query index inside compute pass
    {
        wgpu::QuerySet querySet = CreateQuerySetForTimestamp(kQueryCount);
        wgpu::Buffer destination = CreateResolveBuffer(kQueryCount * sizeof(uint64_t));
