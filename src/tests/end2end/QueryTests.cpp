...
constexpr uint64_t kMiniDestinationOffset = 256;
constexpr uint64_t kMiniCount = kMiniDestinationOffset / sizeof(uint64_t);
...
TEST_P(OcclusionQueryTests, ResolveToBufferWithOffset) {
    ...
    constexpr uint64_t kBufferSize = kQueryCount * sizeof(uint64_t) + kMiniDestinationOffset;
    constexpr uint64_t kCount = kQueryCount + kMiniCount;
    ...
    // Resolve the query result to second slot in the buffer, the first one should not be written.
    {
        ...
        wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
        encoder.ResolveQuerySet(querySet, 0, 1, destination, kMiniDestinationOffset);
        wgpu::CommandBuffer commands = encoder.Finish();
        queue.Submit(1, &commands);

        EXPECT_BUFFER_U64_RANGE_EQ(sentinelValues.data(), destination, 0, kMiniCount);
        EXPECT_BUFFER(destination, kMiniDestinationOffset, sizeof(uint64_t),
                      new OcclusionExpectation(OcclusionExpectation::Result::NonZero));
    }
}
...
TEST_P(TimestampQueryTests, ResolveToBufferWithOffset) {
    ...
    // should not be written
    {
        ...
        std::vector<uint64_t> zeros(kMiniCount, kZero);
        EXPECT_BUFFER_U64_RANGE_EQ(zeros.data(), destination, 0, kMiniCount);
        EXPECT_BUFFER(destination, kMiniDestinationOffset, sizeof(uint64_t),
...
TEST_P(TimestampQueryTests, ResolveTwiceToSameBuffer) {
    ...
    constexpr uint32_t kQueryCount = kMiniCount + 2;
    ...
    encoder.ResolveQuerySet(querySet, 0, kMiniCount + 1, destination, 0);
    encoder.ResolveQuerySet(querySet, kMiniCount, 2, destination, kMiniDestinationOffset);
    wgpu::CommandBuffer commands = encoder.Finish();
    queue.Submit(1, &commands);
