...
class SwapChainTests : public DawnTest {
  public:
    void TestSetUp() override {
        ...
        baseDescriptor.presentMode = wgpu::PresentMode::VSync;
    }
...
// Test switching between VSync on and off.
TEST_P(SwapChainTests, SwitchVSync) {
    for (int i = 0; i < 3; i++) {
        wgpu::SwapChainDescriptor desc = baseDescriptor;
        if (i % 2 == 0) {
            desc.presentMode = wgpu::PresentMode::VSync;
        } else {
            desc.presentMode = wgpu::PresentMode::NoVSync;
        }
