...
class StorageTextureTests : public DawnTest {
  public:
    ...
    wgpu::Texture CreateTextureWithTestData(const std::vector<uint32_t>& initialTextureData,
        ...
        const size_t kUploadBufferSize =
            kTextureBytesPerRowAlignment * (kHeight * kArrayLayerCount - 1) + kWidth * texelSize;

        std::vector<uint32_t> uploadBufferData(kUploadBufferSize / texelSize);
        const size_t kTexelCountPerRow = kTextureBytesPerRowAlignment / texelSize;
        for (uint32_t layer = 0; layer < kArrayLayerCount; ++layer) {
            const size_t kInitialDataOffset = kWidth * kHeight * layer;
            for (size_t y = 0; y < kHeight; ++y) {
                for (size_t x = 0; x < kWidth; ++x) {
                    uint32_t data = initialTextureData[kInitialDataOffset + kWidth * y + x];
                    size_t indexInUploadBuffer = (kHeight * layer + y) * kTexelCountPerRow + x;
                    uploadBufferData[indexInUploadBuffer] = data;
                }
            }
        }
        wgpu::Buffer uploadBuffer =
            utils::CreateBufferFromData(device, uploadBufferData.data(), kUploadBufferSize,
    ...
    void CheckOutputStorageTexture(wgpu::Texture writeonlyStorageTexture,
        ...
        wgpu::Buffer resultBuffer = CreateEmptyBufferForTextureCopy(texelSize, kArrayLayerCount);
        ...
        for (uint32_t layer = 0; layer < kArrayLayerCount; ++layer) {
            ...
            const uint64_t kBufferOffset = kTextureBytesPerRowAlignment * kHeight * layer;
            wgpu::BufferCopyView bufferCopyView = utils::CreateBufferCopyView(
                resultBuffer, kBufferOffset, kTextureBytesPerRowAlignment, 0);
        ...
        // Check if the contents in the result buffer are what we expect.
        for (size_t layer = 0; layer < kArrayLayerCount; ++layer) {
            for (size_t y = 0; y < kHeight; ++y) {
                const size_t kResultBufferOffset =
                    kTextureBytesPerRowAlignment * (kHeight * layer + y);
                const size_t kExpectedDataOffset = kWidth * (kHeight * layer + y);
