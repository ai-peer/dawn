...
class StorageTextureTests : public DawnTest {
  public:
    ...
    std::string CommonReadWriteTestCode(wgpu::TextureFormat format, bool is2DArray = false) {
        ...
        ostream
            << R"(           imageStore(storageImage, texcoord, imageLoad(storageImage1, texcoord));
                             }
                         }
                     }
                 })";
...
// Test that read from read-only storage texture then write into write-only storage textures are
