...
TEST_P(StorageTextureTests, WriteonlyStorageTextureInComputeShader) {
    ...
    // Check if the content in the result buffer is as expected.
    constexpr size_t kTexelCountPerRow = kTextureRowPitchAlignment / kTexelSizeR32Uint;
    for (size_t y = 0; y < kHeight; ++y) {
        const size_t resultBufferOffset = kTexelCountPerRow * y;
