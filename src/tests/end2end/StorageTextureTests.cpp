...
class StorageTextureTests : public DawnTest {
  public:
    ...
    wgpu::Buffer CreateEmptyBufferForTextureCopy(uint32_t texelSize, uint32_t arrayLayerCount = 1) {
        ...
        // TODO(jiawei.shao@intel.com): remove this when we support buffer lazy-initialization.
        std::vector<uint8_t> emptyVector(uploadBufferSize, 0);
        return utils::CreateBufferFromData(device, emptyVector.data(), uploadBufferSize,
...
TEST_P(StorageTextureTests, Writeonly2DArrayStorageTexture) {
    ...
    for (wgpu::TextureFormat format : utils::kAllTextureFormats) {
        ...
        // Prepare the write-only storage texture.
        wgpu::Texture writeonlyStorageTexture =
            CreateTexture(format, wgpu::TextureUsage::Storage | wgpu::TextureUsage::CopySrc, kWidth,
                          kHeight, kArrayLayerCount);

        // Write the expected pixel values into the write-only storage texture.
        const std::string computeShader = CommonWriteOnlyTestCode(format, true);
        WriteIntoStorageTextureInComputePass(writeonlyStorageTexture, computeShader.c_str());

        // Verify the pixel data in the write-only storage texture is expected.
        CheckOutputStorageTexture(writeonlyStorageTexture, format, kArrayLayerCount);
    }
}
