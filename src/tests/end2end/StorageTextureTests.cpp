...
class StorageTextureTests : public DawnTest {
  public:
    ...
    void ValidateOutputBuffer(wgpu::CommandEncoder encoder,
                              wgpu::ComputePipeline pipeline,
                              wgpu::BindGroup inputBindGroup,
                              const std::vector<uint32_t>& resultData) {
...
TEST_P(StorageTextureTests, ReadonlyStorageTextureInComputeShader) {
    ...
    EncodeUploadingDataIntoTexture(encoder, readonlyStorageTexture, initialTextureData,
                                   kTexelSizeR32Uint);
    ...
    std::vector<uint32_t> resultData(kWidth * kHeight, 1);
    ValidateOutputBuffer(encoder, pipeline, inputBindGroup, resultData);
}
