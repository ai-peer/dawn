...
class VertexFormatDeprecationTests : public DeprecationTests {
  protected:
    ...
    void DoTest(const wgpu::VertexFormat vertexFormat, bool deprecated) {
        ...
        attribute += utils::GetWGSLVertexFormatType(vertexFormat);
        attribute += ";";

        std::string attribAccess = utils::VertexFormatNumComponents(vertexFormat) > 1
...
TEST_P(VertexFormatDeprecationTests, NewVertexFormats) {
    ...
    for (auto& format : utils::kAllVertexFormats) {
        DoTest(format, false);
    }
}
...
TEST_P(VertexFormatDeprecationTests, DeprecatedVertexFormats) {
    ...
    for (auto& format : utils::kAllDeprecatedVertexFormats) {
        DoTest(format, true);
    }
}
