...
#include "dawn_native/VertexFormatUtils.h"
...
class VertexFormatDeprecationTests : public DeprecationTests {
  protected:
    ...
    void DoTest(const wgpu::VertexFormat vertexFormat, bool deprecated) {
        ...
        attribute += dawn_native::GetWGSLVertexFormatType(vertexFormat);
        attribute += ";";

        std::string attribAccess = dawn_native::VertexFormatNumComponents(vertexFormat) > 1
...
TEST_P(VertexFormatDeprecationTests, NewVertexFormats) {
    ...
    for (auto& format : dawn_native::kAllVertexFormats) {
        DoTest(format, false);
    }
}
...
TEST_P(VertexFormatDeprecationTests, DeprecatedVertexFormats) {
    ...
    for (auto& format : dawn_native::kAllDeprecatedVertexFormats) {
        DoTest(format, true);
    }
}
