...
protected:
    void SetUp() override {
        ...
        descriptor.cColorAttachments[0]->format = kFormat;
...
TEST_P(RenderPassTest, TwoRenderPassesInOneCommandBuffer) {
    ...
    {
        // In the first render pass we clear renderTarget1 to green and draw a blue triangle in the
    ...
    EXPECT_PIXEL_RGBA8_EQ(kBlue, renderTarget1, 1, 3);
    EXPECT_PIXEL_RGBA8_EQ(kRed, renderTarget1, 3, 1);

    EXPECT_PIXEL_RGBA8_EQ(kBlue, renderTarget2, 1, 3);
    EXPECT_PIXEL_RGBA8_EQ(kGreen, renderTarget2, 3, 1);
}
