...
// Field holds test parameters for ComputeLayoutStorageBufferTests.Fields
struct Field {
...
// StorageClass is an enumerator of storage classes used by ComputeLayoutStorageBufferTests.Fields
...
DAWN_TEST_PARAM_STRUCT(ComputeLayoutStorageBufferTestParams, StorageClass, Field)

class ComputeLayoutStorageBufferTests
    : public DawnTestWithParams<ComputeLayoutStorageBufferTestParams> {};

TEST_P(ComputeLayoutStorageBufferTests, Fields) {
    // Magic codes
    ...
    constexpr uint32_t kStatusOk = 300u;
...
fn main() {
    ...
    {
        auto push_u32 = [&inputData](uint32_t u32) {
        ...
        auto align_to = [&inputData](uint32_t alignment, uint8_t code) {
        ...
        push_u32(kInputHeaderCode);                  // Input.header
        align_to(dataAlign, kDataAlignPaddingCode);  // Input.data
        {
            push_u32(kDataHeaderCode);                      // Input.data.header
            align_to(field.align, kFieldAlignPaddingCode);  // Input.data.field
            ...
            push_u32(kDataFooterCode);                    // Input.data.footer
            align_to(field.align, kDataSizePaddingCode);  // Input.data padding
        }
        align_to(footerAlign, kInputFooterAlignPaddingCode);  // Input.footer [[align]]
        push_u32(kInputFooterCode);                           // Input.footer
        align_to(256, kInputTailPaddingCode);                 // Input padding
    }

    // Set up input storage buffer
    wgpu::BufferDescriptor inputDesc;
    inputDesc.size = inputData.size();
    inputDesc.usage = wgpu::BufferUsage::CopySrc | wgpu::BufferUsage::CopyDst |
                      (isUniform ? wgpu::BufferUsage::Uniform : wgpu::BufferUsage::Storage);

    wgpu::Buffer inputBuf = device.CreateBuffer(&inputDesc);

    queue.WriteBuffer(inputBuf, 0, inputData.data(), inputData.size());
    ...
    {
        std::vector<uint32_t> zero(field.size);
        queue.WriteBuffer(outputBuf, 0, zero.data(), field.size);
    }
    ...
    {
        uint32_t zero = 0;
        queue.WriteBuffer(statusBuf, 0, &zero, 4);
    }

    // Set up bind group and issue dispatch
    wgpu::BindGroup bindGroup = utils::MakeBindGroup(device, pipeline.GetBindGroupLayout(0),
                                                     {
                                                         {0, inputBuf, 0, inputData.size()},
                                                         {1, outputBuf, 0, field.size},
                                                         {2, statusBuf, 0, 4},
                                                     });
...
namespace {
    ...
    auto GenerateParams() {
        auto params = MakeParamGenerator<ComputeLayoutStorageBufferTestParams>(
            {
        ...
        std::vector<ComputeLayoutStorageBufferTestParams> filtered;
    ...
    INSTANTIATE_TEST_SUITE_P(
        ,
        ComputeLayoutStorageBufferTests,
        ::testing::ValuesIn(GenerateParams()),
        DawnTestBase::PrintToStringParamName("ComputeLayoutStorageBufferTests"));
    GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(ComputeLayoutStorageBufferTests);
