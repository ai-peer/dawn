...
// Tests WriteBufferInternal commands on CommandEncoder.
TEST_P(CommandEncoderTests, WriteBufferInternal) {
    ...
    encoder.WriteBufferInternal(bufferA, 0, reinterpret_cast<const uint8_t*>(&kData1),
                                sizeof(kData1));
    encoder.CopyBufferToBuffer(bufferA, 0, bufferB, sizeof(uint32_t), 3 * sizeof(uint32_t));

    const uint32_t kData2 = 2;
    encoder.WriteBufferInternal(bufferB, 0, reinterpret_cast<const uint8_t*>(&kData2),
                                sizeof(kData2));
