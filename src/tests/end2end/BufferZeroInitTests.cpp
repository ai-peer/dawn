...
// Test that the buffer will be lazy initialized correctly when its first use is to be partly bound
// as a uniform buffer with an offset.
TEST_P(BufferZeroInitTest, BoundAsUniformBufferWithOffset) {
    const char* computeShader = R"(
        #version 450
        layout(set = 0, binding = 0, std140) uniform UBO {
            uvec4 value;
...
// Test that the buffer will be lazy initialized correctly when its first use is to be bound as a
// read-only storage buffer as a whole.
...
// Test that the buffer will be lazy initialized correctly when its first use is to be partly bound
// as a read-only storage buffer with an offset.
TEST_P(BufferZeroInitTest, BoundAsReadonlyStorageBufferWithOffset) {
    const char* computeShader = R"(
        #version 450
        layout(set = 0, binding = 0, std140) readonly buffer SSBO {
            uvec4 value;
...
// Test that the buffer will be lazy initialized correctly when its first use is to be partly bound
// as a storage buffer with an offset.
TEST_P(BufferZeroInitTest, BoundAsStorageBufferWithOffset) {
    const char* computeShader = R"(
        #version 450
        layout(set = 0, binding = 0, std140) buffer SSBO {
            uvec4 value[2];
    ...
    constexpr uint32_t kOffset = 256u;
    constexpr uint32_t kExtraBytes = 16u;
    std::vector<uint32_t> expected((kBoundBufferSize + kOffset + kExtraBytes) / sizeof(uint32_t),
                                   0u);
