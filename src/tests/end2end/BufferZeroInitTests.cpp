...
class BufferZeroInitTest : public DawnTest {
  public:
    ...
    void ValidateBufferAndColorAttachment(wgpu::CommandEncoder encoder,
                                          wgpu::Buffer buffer,
                                          uint64_t bufferSize,
        ...
        const std::vector<float> expectedVertexBufferData(bufferSize / sizeof(float), 0);
    ...
    void TestBufferZeroInitAsVertexBuffer(uint64_t vertexBufferOffset) {
        ...
        ValidateBufferAndColorAttachment(encoder, vertexBuffer, vertexBufferSize, colorAttachment);
    }
    ...
    void TestBufferZeroInitAsIndexBuffer(uint64_t indexBufferOffset) {
        ...
        ValidateBufferAndColorAttachment(encoder, indexBuffer, indexBufferSize, colorAttachment);
    }

    void TestBufferZeroInitAsIndirectBufferForDrawIndirect(uint64_t indirectBufferOffset) {
        constexpr wgpu::TextureFormat kColorAttachmentFormat = wgpu::TextureFormat::RGBA8Unorm;
        constexpr wgpu::Color kClearColorGreen = {0, 1, 0, 1};
        ...
        ValidateBufferAndColorAttachment(encoder, indirectBuffer, bufferSize, colorAttachment);
    }
    ...
    void TestBufferZeroInitAsIndirectBufferForDrawIndexedIndirect(uint64_t indirectBufferOffset) {
        ...
        constexpr std::array<float, 4> kVertexAndIndexBufferData = {0, 0, 0, 0};
        constexpr uint64_t kVertexAndIndexBufferDataSize = sizeof(kVertexAndIndexBufferData);
        wgpu::Buffer vertexBuffer =
            utils::CreateBufferFromData(device, kVertexAndIndexBufferData.data(),
                                        kVertexAndIndexBufferDataSize, wgpu::BufferUsage::Vertex);
        wgpu::Buffer indexBuffer =
            utils::CreateBufferFromData(device, kVertexAndIndexBufferData.data(),
                                        kVertexAndIndexBufferDataSize, wgpu::BufferUsage ::Index);
    ...
    void TestBufferZeroInitAsIndirectBufferForDispatchIndirect(uint64_t indirectBufferOffset) {
        ...
        ValidateBufferAndColorAttachment(encoder, indirectBuffer, bufferSize, outputTexture);
    }
};
