...
TEST_P(BufferZeroInitTest, PaddingInitialized) {
    constexpr wgpu::TextureFormat kColorAttachmentFormat = wgpu::TextureFormat::RGBA8Unorm;
    constexpr wgpu::VertexFormat kVertexFormat = wgpu::VertexFormat::Float16x4;
    ...
    wgpu::RenderPipeline renderPipeline =
        ...
        [[stage(vertex)]] fn main([[location(0)]] pos : vec4<f32>) -> VertexOut {
            ...
            return output;
                                    ...
                                    /* vertexBufferCount */ 1u, kVertexFormat);
    ...
    const uint32_t vertexFormatSize = utils::VertexFormatSize(kVertexFormat);
    ...
    for (uint32_t extraBytes : {1, 2, 3}) {
        // Create a vertex buffer to hold a single vertex attribute.
        // Uniform usage is added to force more even padding on D3D12.
        ...
        // "Fully" initialize the buffer with a copy from an 8-bit texture, touching everything
        // except the padding.
        // From the point-of-view of the API, all |vertexBufferSize| bytes are initialized.
        // Note: Uses CopyTextureToBuffer because it does not require 4-byte alignment.
        {
            wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
