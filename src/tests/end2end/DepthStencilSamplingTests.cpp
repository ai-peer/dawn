  ...
  protected:
    ...
    void DoSamplingTestImpl(TestAspect aspect,
        ...
        for (size_t i = 0; i < textureValues.size(); ++i) {
            ...
            {
                wgpu::TextureDescriptor dummyTextureDesc = {};
                dummyTextureDesc.size = {1, 1};
                dummyTextureDesc.format = wgpu::TextureFormat::RGBA8Unorm;
                dummyTextureDesc.usage = wgpu::TextureUsage::RenderAttachment;

                utils::ComboRenderPassDescriptor passDescriptor(
                    {device.CreateTexture(&dummyTextureDesc).CreateView()});
                wgpu::RenderPassEncoder pass = commandEncoder.BeginRenderPass(&passDescriptor);
