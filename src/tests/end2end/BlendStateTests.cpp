...
TEST_P(BlendStateTest, IndependentBlendState) {
    ...
    utils::ComboBlendStateDescriptor descriptor4(device);
    descriptor4.blendEnabled = true;
    descriptor4.colorBlend = blend3;
    descriptor4.alphaBlend = blend3;

    std::array<dawn::BlendState, 4> blendStates = {
        {device.CreateBlendState(&descriptor1), device.CreateBlendState(&descriptor2),
         device.CreateBlendState(&descriptor3), device.CreateBlendState(&descriptor4)}};
