...
TEST_P(ObjectCachingTest, ComputePipelineDeduplicationOnLayout) {
    ...
    dawn::ComputePipelineDescriptor desc;
    desc.computeStage.entryPoint = "main";
    desc.computeStage.module = utils::CreateShaderModule(device, utils::ShaderStage::Compute, R"(
            #version 450
            void main() {
                int i = 0;
            })");
...
TEST_P(ObjectCachingTest, RenderPipelineDeduplicationOnLayout) {
    ...
    desc.vertexStage.module = utils::CreateShaderModule(device, utils::ShaderStage::Vertex, R"(
            #version 450
            void main() {
