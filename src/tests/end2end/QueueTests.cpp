...
TEST_P(QueueWriteTextureTests, BytesPerRowNotDivisibleBy256) {
    ...
    const wgpu::Extent3D copyExtent = {17, 19, 1};
    const uint32_t baseBytesPerRow = utils::GetMinimumBytesPerRow(kTextureFormat, copyExtent.width);

    // Test with bytesPerRow divisible by blockWidth
    for (uint32_t b : {4, 8, 12, 16}) {
        BufferSpec bufferSpec;
        bufferSpec.offset = 0;
        bufferSpec.bytesPerRow = baseBytesPerRow + b;
        bufferSpec.rowsPerImage = 21;
        bufferSpec.size = utils::RequiredBytesInCopy(
            bufferSpec.bytesPerRow, bufferSpec.rowsPerImage, copyExtent, kTextureFormat);
        DoTest(textureSpec, bufferSpec, copyExtent);
    }
}
