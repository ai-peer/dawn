...
// Test using WriteBuffer for super large data block
TEST_P(QueueWriteBufferTests, TwoDifferentBuffers) {
    for (int i = 0; i < 1000; i++) {
        printf("i = %d\n", i);
        wgpu::BufferDescriptor descriptor;
        descriptor.size = i * sizeof(uint32_t);
        descriptor.usage = wgpu::BufferUsage::CopySrc | wgpu::BufferUsage::CopyDst;

        wgpu::Buffer buffer = device.CreateBuffer(&descriptor);

        std::vector<uint32_t> valuess;
        for (int j = 0; j < i; j++) {
            valuess.push_back(i-j);
        }

        queue.WriteBuffer(buffer, 0, valuess.data(), i * sizeof(uint32_t));

        EXPECT_BUFFER_U32_RANGE_EQ(valuess.data(), buffer, 0, i);
    }
}
...
TEST_P(QueueWriteTextureTests, TextureRegionAligned) {
    constexpr uint32_t kWidth = 256;
    constexpr uint32_t kHeight = 128;
    /*for (unsigned int w : {256, 128, 64}) {
        for (unsigned int h : {48, 32, 16}) {
            printf("%u %u\n", w, h);
            TextureSpec textureSpec;
            textureSpec.copyOrigin = {0, 0, 0};
            textureSpec.level = 0;
            ...
            textureSpec.textureSize = {kWidth, kHeight, 1};
    {
            DoTest(textureSpec, MinimumBufferSpec(64, 16), {64, 16, 1});
    } 
    {
            DoTest(textureSpec, MinimumBufferSpec(64, 32), {64, 32, 1});
    }
}
