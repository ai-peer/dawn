...
// Test the B10RG11Float format
// TODO(cwallez@chromium.org): This is actually RG11B10 in WebGPU but Vulkan doesn't support that
// format. Do all platforms have it reversed?
TEST_P(TextureFormatTest, B10RG11Float) {
    ...
    auto MakeB10RG11 = [] (uint32_t r, uint32_t g, uint32_t b) {
        ASSERT((r & 0x7FF) == r);
        ASSERT((g & 0x7FF) == g);
        ASSERT((b & 0x3FF) == b);
        return b << 22 | g << 11 | r;
    ...
    // Test each of (0, 1, INFINITY, NaN) for each component but never two with the same value at a time.
    std::vector<uint32_t> textureData = {
        MakeB10RG11(kFloat11Zero, kFloat11Infinity, kFloat10Nan),
        MakeB10RG11(kFloat11Infinity, kFloat11Nan, kFloat10One),
        MakeB10RG11(kFloat11Nan, kFloat11One, kFloat10Zero),
        MakeB10RG11(kFloat11One, kFloat11Zero, kFloat10Infinity),
    };
    ...
    DoSampleTest({dawn::TextureFormat::B10RG11Float, 4, Float, 4}, textureData, expectedData);
}

// TODO(cwallez@chromium.org): Add tests for depth-stencil formats when we know if they are copyable in WebGPU.
