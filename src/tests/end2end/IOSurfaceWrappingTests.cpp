...
namespace {
    ...
    class IOSurfaceTestBase : public DawnTest {
      public:
        dawn::Texture WrapIOSurface(const dawn::TextureDescriptor* descriptor, IOSurfaceRef ioSurface, uint32_t plane) {
            dawnTexture texture = dawn_native::metal::WrapIOSurface(device.Get(), reinterpret_cast<const dawnTextureDescriptor*>(descriptor), ioSurface, plane);
...
TEST_P(IOSurfaceValidationTests, InvalidTextureDescriptor) {
    ...
    ASSERT_DEVICE_ERROR(dawn::Texture texture = WrapIOSurface(&descriptor, defaultIOSurface.get(), 0));
    ASSERT_EQ(texture.Get(), nullptr);
}

// Test an error occurs if the plane is too large
TEST_P(IOSurfaceValidationTests, PlaneTooLarge) {
    ASSERT_DEVICE_ERROR(dawn::Texture texture = WrapIOSurface(&descriptor, defaultIOSurface.get(), 1));
    ASSERT_EQ(texture.Get(), nullptr);
}
...
class IOSurfaceUsageTests : public IOSurfaceTestBase {
  public:
    ...
    void DoSampleTest(IOSurfaceRef ioSurface, dawn::TextureFormat format, void* data, size_t dataSize,
                      RGBA8 expectedColor) {
        ...
        dawn::RenderPipeline pipeline;
        {
            ...
            dawn::ShaderModule fs = utils::CreateShaderModule(device, dawn::ShaderStage::Fragment, R"(
                #version 450
    ...
    void DoClearTest(IOSurfaceRef ioSurface, dawn::TextureFormat format, void* data, size_t dataSize) {
        ...
        colorAttachment.attachment = ioSurfaceView;
        colorAttachment.resolveTarget = nullptr;
        colorAttachment.clearColor = {1 / 255.0f, 2 / 255.0f, 3 / 255.0f, 4 / 255.0f};
        colorAttachment.loadOp = dawn::LoadOp::Clear;
        colorAttachment.storeOp = dawn::StoreOp::Store;
        dawn::RenderPassDescriptor renderPassDescriptor = device.CreateRenderPassDescriptorBuilder()
...
TEST_P(IOSurfaceUsageTests, SampleFromR8IOSurface)
{
    ...
    uint8_t data = 0x01;
    DoSampleTest(ioSurface.get(), dawn::TextureFormat::R8Unorm, &data, sizeof(data), RGBA8(1, 0, 0, 255));
}
...
TEST_P(IOSurfaceUsageTests, SampleFromRG8IOSurface)
{
    ...
    DoSampleTest(ioSurface.get(), dawn::TextureFormat::R8G8Unorm, &data, sizeof(data), RGBA8(2, 1, 0, 255));
}
...
TEST_P(IOSurfaceUsageTests, SampleFromBGRA8888IOSurface)
{
    ...
    DoSampleTest(ioSurface.get(), dawn::TextureFormat::B8G8R8A8Unorm, &data, sizeof(data), RGBA8(2, 3, 4, 1));
}
