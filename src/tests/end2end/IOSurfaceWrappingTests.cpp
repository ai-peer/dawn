...
TEST_P(IOSurfaceValidationTests, Success) {
    ...
    dawn::Texture texture = WrapIOSurface(&descriptor, BGRAIOSurface.get(), 0);
...
TEST_P(IOSurfaceValidationTests, SuccessCompatible) {
    ...
    {
        descriptor.format = dawn::TextureFormat::BGRA8Unorm;
        dawn::Texture texture = WrapIOSurface(&descriptor, RGBAIOSurface.get(), 0);
        ASSERT_NE(texture.Get(), nullptr);
    }
}
...
TEST_P(IOSurfaceValidationTests, InvalidTextureDescriptor) {
    ...
    ASSERT_DEVICE_ERROR(dawn::Texture texture = WrapIOSurface(&descriptor, BGRAIOSurface.get(), 0));
...
TEST_P(IOSurfaceValidationTests, PlaneTooLarge) {
    DAWN_SKIP_TEST_IF(UsesWire());
    ASSERT_DEVICE_ERROR(dawn::Texture texture = WrapIOSurface(&descriptor, BGRAIOSurface.get(), 1));
    ASSERT_EQ(texture.Get(), nullptr);
}
