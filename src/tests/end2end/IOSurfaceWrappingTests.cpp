...
TEST_P(IOSurfaceUsageTests, UnclearedTextureIsCleared) {
    ...
    ScopedIOSurfaceRef ioSurface = CreateSinglePlaneIOSurface(1, 1, 'RGBA', 4);
    IOSurfaceRef ioSurfaceRef = ioSurface.get();
    ...
    {
        wgpu::Texture ioSurfaceTexture = WrapIOSurface(&textureDescriptor, ioSurfaceRef, 0, true);

        wgpu::TextureView ioSurfaceView = ioSurfaceTexture.CreateView();

        utils::ComboRenderPassDescriptor renderPassDescriptor({ioSurfaceView}, {});
        renderPassDescriptor.cColorAttachments[0].clearColor = {1.0f, 0.0f, 0.0f, 1.0f};
        ...
        // Wait for the commands touching the IOSurface to be scheduled
        dawn_native::metal::WaitForCommandsToBeScheduled(device.Get());
        EXPECT_PIXEL_RGBA8_EQ(RGBA8(255, 0, 0, 255), ioSurfaceTexture, 0, 0);
    }

    // Check the correct data was written
    IOSurfaceLock(ioSurfaceRef, kIOSurfaceLockReadOnly, nullptr);
    ASSERT_EQ(0, memcmp(IOSurfaceGetBaseAddress(ioSurfaceRef), data, sizeof(data)));
    IOSurfaceUnlock(ioSurfaceRef, kIOSurfaceLockReadOnly, nullptr);

    // rewrap ioSurface and ensure color is not visible when isCleared set to false
    {
        wgpu::Texture ioSurfaceTexture = WrapIOSurface(&textureDescriptor, ioSurfaceRef, 0, false);
        EXPECT_PIXEL_RGBA8_EQ(RGBA8(0, 0, 0, 0), ioSurfaceTexture, 0, 0);
    }

    // Check the correct data was written
    IOSurfaceLock(ioSurfaceRef, kIOSurfaceLockReadOnly, nullptr);
    ASSERT_EQ(0, memcmp(IOSurfaceGetBaseAddress(ioSurfaceRef), 0, sizeof(data)));
    IOSurfaceUnlock(ioSurfaceRef, kIOSurfaceLockReadOnly, nullptr);
}
