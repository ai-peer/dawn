...
class TextureSubresourceTest : public DawnTest {
  public:
    ...
    void VerifyPixels(wgpu::Texture& texture, uint32_t level, uint32_t layer, uint32_t size) {
        // The bottom-left half should be red, while other pixels should be background black
        ASSERT_TRUE(size >= 2);

        std::vector<RGBA8> topLeft = {RGBA8::kBlack, RGBA8::kBlack,
                                      RGBA8::kRed, RGBA8::kBlack};
        EXPECT_TEXTURE_RGBA8_EQ(topLeft.data(), texture, 0, 0, 2, 2, level, layer);

        if (size >= 4) {
            std::vector<RGBA8> topRight = {RGBA8::kBlack, RGBA8::kBlack,
                                           RGBA8::kBlack, RGBA8::kBlack};
            std::vector<RGBA8> bottomLeft = {RGBA8::kRed, RGBA8::kRed,
                                             RGBA8::kRed, RGBA8::kRed};
            std::vector<RGBA8> bottomRight = {RGBA8::kBlack, RGBA8::kBlack,
                                              RGBA8::kRed, RGBA8::kBlack};
            EXPECT_TEXTURE_RGBA8_EQ(topRight.data(), texture, size - 2, 0, 2, 2, level, layer);
            EXPECT_TEXTURE_RGBA8_EQ(bottomLeft.data(), texture, 0, size - 2, 2, 2, level, layer);
            EXPECT_TEXTURE_RGBA8_EQ(bottomRight.data(), texture, size - 2, size - 2, 2, 2,
                                    level, layer);
        }
    }
};
...
TEST_P(TextureSubresourceTest, MipmapLevelsTest) {
    ...
    // Verify that pixels at bottom-left half are red
    VerifyPixels(texture, 1, 0, kSize / 2);
}
...
TEST_P(TextureSubresourceTest, ArrayLayersTest) {
    ...
    // Verify that pixels at bottom-left half are red
    VerifyPixels(texture, 0, 1, kSize);
}
