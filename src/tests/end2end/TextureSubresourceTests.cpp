...
class TextureSubresourceTest : public DawnTest {
  public:
    ...
    void SampleAndDraw(wgpu::Texture& texture,
        ...
        wgpu::ShaderModule fsModule =
            utils::CreateShaderModule(device, utils::SingleShaderStage::Fragment, R"(
                ...
                layout (location = 0) out vec4 fragColor;
                void main() {
                    // fragColor = texture(tex, gl_FragCoord.xy * 2);
    ...
    void VerifyPixels(wgpu::Texture& texture, uint32_t level, uint32_t layer) {
        for (uint32_t i = 0; i < kSize; ++i) {
            for (uint32_t j = 0; j < kSize; ++j) {
                std::vector<RGBA8> expected(1, i < j ? RGBA8::kRed : RGBA8::kBlack);
                EXPECT_TEXTURE_RGBA8_EQ(expected.data(), texture, i, j, 1, 1, level, layer);
            }
        }
    }

    void TestRenderPass(wgpu::Texture& texture,
                        const wgpu::TextureView& samplerView,
                        const wgpu::TextureView& renderView) {
        // Create bind group

        // Draw a triangle at the bottom-left into samplerView
        DrawTriangle(samplerView);
        VerifyPixels(texture, 0, 0);

        // Sample from sampleView, and draw into renderView
        SampleAndDraw(texture, samplerView, renderView);
        VerifyPixels(texture, 0, 1);
    }
};
...
/* TEST_P(TextureSubresourceTest, MipmapLevelsTest) {
    ...
    TestRenderPass(texture, samplerView, renderView);
} */
