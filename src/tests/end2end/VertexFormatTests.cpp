...
class VertexFormatTest : public DawnTest {
  protected:
    ...
    dawn::RenderPipeline MakeTestPipeline(dawn::VertexFormat format, std::vector<T>& expectedData) {
        ...
        vs << "layout (std140, set = 0, binding = 0) uniform uBuffer {\n";
        vs << "    " << variableType << " expected[" << kVertexNum << "];\n";
        vs << "};\n";
        ...
        vs << "    " << variableType << " expectedTry[" + std::to_string(kVertexNum) + "];\n";

        // Assign each elements in expected values
        // e.g. expected[0] = uvec2(1, 2);
        for (uint32_t i = 0; i < kVertexNum; ++i) {
            vs << "    expectedTry[" + std::to_string(i) + "] = " << variableType << "(";
        ...
        // The threshold is for float data comparation.
        vs << "    float threshold = 1.0 / ";
        // The loss of precision of two float data is related to the number of bits.
        if (isUnsigned) {
            switch(bytesPerComponents) {
                case 1:
                    vs << std::to_string(std::numeric_limits<uint8_t>::max());
                    break;
                case 2:
                    vs << std::to_string(std::numeric_limits<uint16_t>::max());
                    break;
                case 4:
                    vs << std::to_string(std::numeric_limits<uint32_t>::max());
                    break;
                default:
                    DAWN_UNREACHABLE();
            }
            ...
            switch(bytesPerComponents) {
                case 1:
                    vs << std::to_string(std::numeric_limits<int8_t>::max());
                    break;
                case 2:
                    vs << std::to_string(std::numeric_limits<int16_t>::max());
                    break;
                case 4:
                    vs << std::to_string(std::numeric_limits<int32_t>::max());
                    break;
                default:
                    DAWN_UNREACHABLE();
            }
        ...
        vs << "    success = success";
        if (!isNormalized && !isFloat) {  // For integer/unsigned integer, they need to equal perfectly.
            if (componentCount > 1) {
                for (uint32_t component = 0; component < componentCount; ++component) {
                    vs << " && test[" << component << "] == expectedTry[gl_VertexIndex][" << component// " && test[" << component << "] == expectedTry[gl_VertexIndex][" << component
                       << "]";
                }
            } else {
                vs << " && test == expectedTry[gl_VertexIndex]";
            }
        } else {
            if (componentCount > 1) {
                for (uint32_t component = 0; component < componentCount; ++component) {
                    vs << " && abs(test[" << component << "] - expectedTry[gl_VertexIndex][" << component
                       << "]) < threshold ";
                }
            } else {
                vs << " && abs(test - expectedTry[gl_VertexIndex]) < threshold";
            }
        }
        vs << ";\n";
        vs << "    if (success) {\n";
        vs << "        color = vec4(0.0f, 1.0f, 0.0f, 1.0f);\n";
        // vs << "        color = vec4(expectedTry[0][0] / 255.0f, expectedTry[0][1] / 255.0f, expectedTry[1][0] / 255.0f, expectedTry[1][1] / 255.0f);\n";
        vs << "    } else {\n";
    ...
    void DoVertexFormatTest(dawn::VertexFormat format,
        ...
        uint32_t uniformBufferSize = expectedData.size() * sizeof(ExpectedType);
        dawn::Buffer uniformBuffer =
            utils::CreateBufferFromData(device, expectedData.data(), uniformBufferSize,
                                        dawn::BufferUsageBit::Uniform);
        dawn::BindGroup bindGroup =
            utils::MakeBindGroup(device, mBindGroupLayout,
                                 {{0, uniformBuffer, 0, uniformBufferSize}});
...
TEST_P(VertexFormatTest, UChar2) {
    ...
    std::vector<uint8_t> expectedData = vertexData;
...
TEST_P(VertexFormatTest, UChar4) {
    ...
    DoVertexFormatTest(dawn::VertexFormat::UChar4, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, Char2) {
    ...
    std::vector<int8_t> expectedData = vertexData;
...
TEST_P(VertexFormatTest, Char4) {
    ...
    DoVertexFormatTest(dawn::VertexFormat::Char4, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, UChar2Norm) {
    ...
    std::vector<float> expectedData = {Normalize(vertexData[0]), Normalize(vertexData[1]),
                                       Normalize(vertexData[2]), Normalize(vertexData[3]),
                                       Normalize(vertexData[4]), Normalize(vertexData[5])};
...
TEST_P(VertexFormatTest, UChar4Norm) {
    ...
    std::vector<float> expectedData = {
        Normalize(vertexData[0]), Normalize(vertexData[1]),  Normalize(vertexData[2]),
        Normalize(vertexData[3]), Normalize(vertexData[4]),  Normalize(vertexData[5]),
        Normalize(vertexData[6]), Normalize(vertexData[7]),  Normalize(vertexData[8]),
        Normalize(vertexData[9]), Normalize(vertexData[10]), Normalize(vertexData[11]),
    };

    DoVertexFormatTest(dawn::VertexFormat::UChar4Norm, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, Char4Norm) {
    ...
    DoVertexFormatTest(dawn::VertexFormat::Char4Norm, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, UShort2) {
    ...
    std::vector<uint16_t> expectedData = vertexData;

    dawn::RenderPipeline pipeline =
        MakeTestPipeline<uint16_t>(dawn::VertexFormat::UShort2, vertexData);

    DoVertexFormatTest(dawn::VertexFormat::UShort2, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, UShort4) {
    ...
    DoVertexFormatTest(dawn::VertexFormat::UShort4, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, Short2) {
    ...
    std::vector<int16_t> expectedData = vertexData;

    DoVertexFormatTest(dawn::VertexFormat::Short2, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, Short4) {
    ...
    DoVertexFormatTest(dawn::VertexFormat::Short4, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, UShort2Norm) {
    ...
    DoVertexFormatTest(dawn::VertexFormat::UShort2Norm, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, UShort4Norm) {
    ...
    DoVertexFormatTest(dawn::VertexFormat::UShort4Norm, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, Short2Norm) {
    ...
    DoVertexFormatTest(dawn::VertexFormat::Short2Norm, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, Short4Norm) {
    ...
    DoVertexFormatTest(dawn::VertexFormat::Short4Norm, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, Half2) {
    ...
    std::vector<float> expectedData = {
        float16ToFloat32(vertexData[0]), float16ToFloat32(vertexData[1]), float16ToFloat32(vertexData[2]),
        float16ToFloat32(vertexData[3]), float16ToFloat32(vertexData[4]), float16ToFloat32(vertexData[5])
    };

    DoVertexFormatTest(dawn::VertexFormat::Half2, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, Half4) {
    ...
    std::vector<float> expectedData = {
        float16ToFloat32(vertexData[0]), float16ToFloat32(vertexData[1]), float16ToFloat32(vertexData[2]),
        float16ToFloat32(vertexData[3]), float16ToFloat32(vertexData[4]), float16ToFloat32(vertexData[5]),
        float16ToFloat32(vertexData[6]), float16ToFloat32(vertexData[7]), float16ToFloat32(vertexData[8]),
        float16ToFloat32(vertexData[9]), float16ToFloat32(vertexData[10]), float16ToFloat32(vertexData[11])
    };

    DoVertexFormatTest(dawn::VertexFormat::Half4, vertexData, expectedData);
}

TEST_P(VertexFormatTest, Float) {
    std::vector<float> vertexData = {1.0f, -1.0f, 1.0f};

    std::vector<float> expectedData = vertexData;

    DoVertexFormatTest(dawn::VertexFormat::Float, vertexData, expectedData);
}

TEST_P(VertexFormatTest, Float2) {
    std::vector<float> vertexData = {1.0f, -1.0f, 0.0f, 1.0f, -1.0f, 1.6f};
    std::vector<float> expectedData = vertexData;

    DoVertexFormatTest(dawn::VertexFormat::Float2, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, Float3) {
    ...
    DoVertexFormatTest(dawn::VertexFormat::Float3, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, Float4) {
    ...
    DoVertexFormatTest(dawn::VertexFormat::Float4, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, UInt) {
    std::vector<uint32_t> vertexData = {std::numeric_limits<uint32_t>::max(),
                                        std::numeric_limits<uint16_t>::max(),
                                        std::numeric_limits<uint8_t>::max()};
    std::vector<uint32_t> expectedData = vertexData;

    DoVertexFormatTest(dawn::VertexFormat::UInt, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, UInt2) {
    ...
    DoVertexFormatTest(dawn::VertexFormat::UInt2, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, UInt3) {
    ...
    DoVertexFormatTest(dawn::VertexFormat::UInt3, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, UInt4) {
    ...
    DoVertexFormatTest(dawn::VertexFormat::UInt4, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, Int) {
    std::vector<int32_t> vertexData = {std::numeric_limits<int32_t>::max(),
                                       ...
                                       std::numeric_limits<int8_t>::max()};
    std::vector<int32_t> expectedData = vertexData;

    DoVertexFormatTest(dawn::VertexFormat::Int, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, Int2) {
    ...
    DoVertexFormatTest(dawn::VertexFormat::Int2, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, Int3) {
    ...
    DoVertexFormatTest(dawn::VertexFormat::Int3, vertexData, expectedData);
}
...
TEST_P(VertexFormatTest, Int4) {
    ...
    DoVertexFormatTest(dawn::VertexFormat::Int4, vertexData, expectedData);
}
