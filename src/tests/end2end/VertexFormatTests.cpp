...
class VertexFormatTest : public DawnTest {
  protected:
    ...
    dawn::RenderPipeline MakeTestPipeline(dawn::VertexFormat format, std::vector<T>& expectedData) {
        ...
        const uint32_t NegativeZeroInHalf = 0x8000;
        ...
        vs << "layout(std140, set = 0, binding = 0) buffer TestR {\n";
        vs << "  uint result[16];} testR;\n";
        ...
        for (uint32_t i = 0; i < kVertexNum; ++i) {
            for (uint32_t j = 0; j < componentCount; ++j) {
                ...
                if (isInputTypeFloat &&
                    ...
                    if (expectedData[i * componentCount + j] == NegativeZeroInHalf) {
                        vs << "-0.0f);\n";
                    } else {
                        vs << "Float16ToFloat32("
        ...
        for (uint32_t component = 0; component < componentCount; ++component) {
            vs << "testR.result[gl_VertexIndex * 2 + " << component << "] = floatBitsToUint(test";
            vs << (componentCount == 1 ? "" : "[" + std::to_string(component) + "]);\n");
            vs << "testR.result[(gl_VertexIndex + 4) * 2 + " << component << "] = floatBitsToUint(expected[gl_VertexIndex]";
            vs << (componentCount == 1 ? "" : "[" + std::to_string(component) + "]);\n");
        }
        ...
        for (uint32_t component = 0; component < componentCount; ++component) {
            ...
            if (!isInputTypeFloat) {  // Integer / unsigned integer need to match exactly.
                ...
                vs << "        - min(testValFloatToUint, expectedValFloatToUint) < uint(4);\n";
                vs << "    }\n";
            }
        }
        vs << "    if (success) {\n";
        ...
        dawn::PipelineLayout pl = utils::MakeBasicPipelineLayout(device, &mBindGroupLayout);
    ...
    static void MapReadCallback(DawnBufferMapAsyncStatus status,
        ...
        auto test = reinterpret_cast<VertexFormatTest*>(static_cast<uintptr_t>(userdata));
        test->mappedData = data;
    }

    const uint32_t* MapReadAsyncAndWait(const dawn::Buffer& buffer) {
        ...
        return reinterpret_cast<const uint32_t*>(mappedData);
    }
    ...
    void DoVertexFormatTest(dawn::VertexFormat format,
        ...
        dawn::BufferDescriptor bDescriptor;
        bDescriptor.size = sizeof(uint32_t) * 4 * 4 * 2 * 4;
        bDescriptor.usage = dawn::BufferUsageBit::TransferSrc | dawn::BufferUsageBit::Storage | dawn::BufferUsageBit::TransferDst;
        dawn::Buffer testRBuffer = device.CreateBuffer(&bDescriptor);
        dawn::BindGroup bg = utils::MakeBindGroup(device, mBindGroupLayout,
            {{0, testRBuffer, 0, sizeof(uint32_t) * 4 * 4 * 2 * 4}});
        
        dawn::BufferDescriptor rDescriptor;
        rDescriptor.size = sizeof(uint32_t) * 4 * 4 * 2 * 4;
        rDescriptor.usage = dawn::BufferUsageBit::TransferDst | dawn::BufferUsageBit::MapRead;
        dawn::Buffer testReBuffer = device.CreateBuffer(&rDescriptor);
        ...
        encoder.CopyBufferToBuffer(testRBuffer, 0, testReBuffer, 0, sizeof(uint32_t) * 4 * 4 * 2 * 4);

        dawn::CommandBuffer transCommands = encoder.Finish();
        queue.Submit(1, &transCommands);

        const uint32_t* result = MapReadAsyncAndWait(testReBuffer);

        for (uint32_t i = 0; i < 128; ++i) {
            printf("Test index %i = %u;\n", i, result[i]);
        }
