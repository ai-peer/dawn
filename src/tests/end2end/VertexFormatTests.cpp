...
class VertexFormatTest : public DawnTest {
  protected:
    ...
    dawn::RenderPipeline MakeTestPipeline(dawn::VertexFormat format, std::vector<T>& expectedData) {
        ...
        vs << "layout(location = 0) in " << assignment << " test;\n";
        ...
        vs << "    " << assignment << " expected[" + std::to_string(kVertexNum) + "];";

        // Assign each elements in expected values
        // e.g. expected[0] = uvec2(1, 2);
        for (uint32_t i = 0; i < kVertexNum; ++i) {
            vs << "    expected[" + std::to_string(i) + "] = " << assignment << "(";
            for (uint32_t j = 0, commaPos = componentCount - 1; j < componentCount;
                 ++j, --commaPos) {
                // For uint8_t or char, output stream will take it as ASCII.
                if (bytesPerComponents == 1 && !isNormalized) {
                    isUnsigned ? vs << static_cast<uint32_t>(expectedData[i * componentCount + j])
                               : vs << static_cast<int>(expectedData[i * componentCount + j]);
                } else {
                    vs << expectedData[i * componentCount + j];
                }
                vs << (commaPos == 0 ? "" : ", ");
            }
            vs << ");\n";
...
TEST_P(VertexFormatTest, Half2) {
    ...
    std::vector<float> expectedData = {32.1f, -16.8f, 18.2f, -24.7f, 12.5f, -18.2f};
