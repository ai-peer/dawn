...
class MultisampledSamplingTest : public DawnTest {
  protected:
    ...
    void InitializePipelines(uint32_t sampleMask) {
        {
    ...
    void DoTest(uint32_t sampleMask) {
        InitializePipelines(sampleMask);
        ...
        for (uint32_t s = 0; s < kSampleCount; ++s) {
            expectedData = {0, 0, 0, 0, 0, 0, 0, 0};
            if ((1 << s) & sampleMask) {
                expectedData[s] = 1;
                expectedData[kSampleCount + s] = 0.7;
            }
            EXPECT_BUFFER_FLOAT_RANGE_EQ(expectedData.data(), outputBuffer, s * alignedResultSize,
                                         8)
                << "vertical sample " << s;
            EXPECT_BUFFER_FLOAT_RANGE_EQ(expectedData.data(), outputBuffer,
                                         (kSampleCount + s) * alignedResultSize, 8)
                << "horizontal sample " << s;
        }
    }
};
...
// Test a subset of the samples
TEST_P(MultisampledSamplingTest, SamplePositionsWithSampleMask) {
    // TODO(tommek@google.com): Remove this condition after enabling sampleMask usage in those
    // backends.
    DAWN_SKIP_TEST_IF(IsOpenGL() || IsMetal() || IsD3D12());

    // None of the samples is included
    DoTest(0x00000000);
    // Only the second sample position is included
    DoTest(0x00000002);
    // The first three sample positions are included
    DoTest(0x00000007);
    // The last three sample positions are included
    DoTest(0x0000000E);
}
