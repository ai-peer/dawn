...
class MultisampledSamplingTest : public DawnTest {
  protected:
    ...
    // D3D11 but it was left out of the documentation. {0.375, 0.125}, {0.875, 0.375}, {0.125,
    // 0.625}, {0.625, 0.875} In this test, we store them in -1 to 1 space because it makes it
    // simpler to upload vertex data.
    ...
    // Make a vertex buffer for drawing a quad centered at a point
    // This uses triangle strip topology, and coordinates are in (-1, 1) space.
    wgpu::Buffer MakeQuadVertexBuffer(float x, float y, float width, float height) {
...
// After drawing, a compute shader fetches all of the samples, and we check that only the
// one covered has data.
TEST_P(MultisampledSamplingTest, SamplePositions) {
    ...
    static constexpr float kQuadWidth = 0.01;
    std::vector<float> vBufferData = {{
        // clang-format off

        // A vertical quad covering the 1st sample position
        kSamplePositions[0][0] - kQuadWidth, -1.0,
        kSamplePositions[0][0] - kQuadWidth,  1.0,
        kSamplePositions[0][0] + kQuadWidth, -1.0,
        kSamplePositions[0][0] + kQuadWidth,  1.0,

        // A vertical quad covering the 2nd sample position
        kSamplePositions[1][0] - kQuadWidth, -1.0,
        kSamplePositions[1][0] - kQuadWidth,  1.0,
        kSamplePositions[1][0] + kQuadWidth, -1.0,
        kSamplePositions[1][0] + kQuadWidth,  1.0,

        // A vertical quad covering the 3rd sample position
        kSamplePositions[2][0] - kQuadWidth, -1.0,
        kSamplePositions[2][0] - kQuadWidth,  1.0,
        kSamplePositions[2][0] + kQuadWidth, -1.0,
        kSamplePositions[2][0] + kQuadWidth,  1.0,

        // A vertical quad covering the 4th sample position
        kSamplePositions[3][0] - kQuadWidth, -1.0,
        kSamplePositions[3][0] - kQuadWidth,  1.0,
        kSamplePositions[3][0] + kQuadWidth, -1.0,
        kSamplePositions[3][0] + kQuadWidth,  1.0,

        // A horizontal quad covering the 1st sample position
        -1.0, kSamplePositions[0][1] - kQuadWidth,
        -1.0, kSamplePositions[0][1] + kQuadWidth,
         1.0, kSamplePositions[0][1] - kQuadWidth,
         1.0, kSamplePositions[0][1] + kQuadWidth,

        // A horizontal quad covering the 2nd sample position
        -1.0, kSamplePositions[1][1] - kQuadWidth,
        -1.0, kSamplePositions[1][1] + kQuadWidth,
         1.0, kSamplePositions[1][1] - kQuadWidth,
         1.0, kSamplePositions[1][1] + kQuadWidth,

        // A horizontal quad covering the 3rd sample position
        -1.0, kSamplePositions[2][1] - kQuadWidth,
        -1.0, kSamplePositions[2][1] + kQuadWidth,
         1.0, kSamplePositions[2][1] - kQuadWidth,
         1.0, kSamplePositions[2][1] + kQuadWidth,

        // A horizontal quad covering the 4th sample position
        -1.0, kSamplePositions[3][1] - kQuadWidth,
        -1.0, kSamplePositions[3][1] + kQuadWidth,
         1.0, kSamplePositions[3][1] - kQuadWidth,
         1.0, kSamplePositions[3][1] + kQuadWidth,
    ...
    static constexpr uint64_t kResultSize = 4 * 4 + 4 * 4;
