  ...
  protected:
    ...
    void DoTest(uint32_t sampleMask) {
        ...
        for (uint32_t s = 0; s < kSampleCount; ++s) {
            ...
            EXPECT_BUFFER_FLOAT_RANGE_EQ(expectedData.data(), outputBuffer, s * alignedResultSize, 8)
               << "vertical sample " << s;
            EXPECT_BUFFER_FLOAT_RANGE_EQ(expectedData.data(), outputBuffer, (kSampleCount + s) * alignedResultSize, 8)
               << "horizontal sample " << s;
        }
    }
};
