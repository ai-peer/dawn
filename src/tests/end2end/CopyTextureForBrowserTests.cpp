        ...
        wgpu::ShaderModule csModule = utils::CreateShaderModule(device, R"(
            ...
            [[stage(compute), workgroup_size(1, 1, 1)]] fn main() -> void {
                ...
                if (dstTexCoord.x < uniforms.dstCopyOrigin.x ||
                    ...
                    // Note that |flipY| equals copy from src subrect to dst subrect firstly
                    // and then do flip option. We need to calculate the expect y coord based
                    // on this rule.
                    if (uniforms.dstTextureFlipY == 1u) {
                        srcTexCoord.y = (uniforms.srcCopyOrigin.y + uniforms.copySize.y) -
                                        (srcTexCoord.y - uniforms.srcCopyOrigin.y) - 1u;
                    }
