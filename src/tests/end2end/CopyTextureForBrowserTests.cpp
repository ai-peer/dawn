...
class CopyTextureForBrowserTests : public DawnTest {
  protected:
    ...
    void DoTest(const TextureSpec& srcSpec,
        ...
        // `level` mip level for all platforms except OpenGL, which doesn't support writeTexture
        // now. Create uploadBuffer and use it to populate the current slice of texture for OpenGL
        // backend.
        ...
        if (IsOpenGL()) {
            wgpu::Buffer uploadBuffer =
                utils::CreateBufferFromData(device, textureArrayCopyData.data(),
            ...
            wgpu::TextureCopyView textureCopyView = utils::CreateTextureCopyView(
                srcTexture, srcSpec.level, {0, 0, srcSpec.copyOrigin.z});
...
TEST_P(CopyTextureForBrowserTests, PassthroughCopy) {
    // This test fails due to crbug.com/tint/278.
