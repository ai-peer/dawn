...
class CopyTextureForBrowserTests : public DawnTest {
  protected:
    ...
    // The source data can fill a texture in default widht and height.
    ...
    wgpu::ComputePipeline MakeTestPipeline() {
        wgpu::ShaderModule csModule = utils::CreateShaderModuleFromWGSL(device, R"(
            [[block]] struct Uniforms {
                ...
                [[offset(4)]] isSwizzled : u32;
                [[offset(8)]] channelCount : u32;
            };
            ...
            fn isEqual(value : f32, expect : f32) -> bool {
                return max(value, expect) - min(value, expect) < 0.001;
            }
            [[stage(compute), workgroup_size(1, 1, 1)]] fn main() -> void {
                ...
                if (uniforms.isSwizzled == 1) { // BGRA8Unorm check
                  // RGBA8Unorm to BGRA8Unorm 
                  success = success &&
                            isEqual(dstColor[0], srcColor.b) &&
                            isEqual(dstColor[1], srcColor.g) &&
                            isEqual(dstColor[2], srcColor.r) &&
                            isEqual(dstColor[3], srcColor.a);
                } else { // Other format converts shouldn't have precision difference
                    if (uniforms.channelCount == 2) { // All have rg components.
                        success = success && 
                                  isEqual(dstColor.r, srcColor.r) &&
                                  isEqual(dstColor.g, srcColor.g);
                    } else {
                        success = success &&
                                  isEqual(dstColor.r, srcColor.r) &&
                                  isEqual(dstColor.g, srcColor.g) &&
                                  isEqual(dstColor.b, srcColor.b) &&
                                  isEqual(dstColor.a, srcColor.a);
                    }
                }
