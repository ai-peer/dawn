...
void ComputeIndirectTests::BasicTest(std::initializer_list<uint32_t> bufferList,
    ...
    // multiple of the max alignment of each member. Here the base alignment of uvec3 is 16 bytes on
    // Metal, so the required size of inputBuf and outputBuf is 16 bytes.
    ...
    dawn::Buffer expectedBuffer =
        utils::CreateBufferFromData(device, &indirectBufferData[indirectOffset / sizeof(uint32_t)],
                                    4 * sizeof(uint32_t), dawn::BufferUsageBit::Uniform);
