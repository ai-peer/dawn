...
// Test various viewport depth ranges that are inverted
TEST_P(ViewportTest, ViewportDepthInverted) {
    // It isn't clear if this will be supported in WebGPU, and it fails on a couple systems.
    DAWN_SKIP_TEST_IF(IsD3D12() && IsIntel());
    DAWN_SKIP_TEST_IF(IsMetal() && IsAMD());

    TestViewportDepth(1.0, 0.0);
    TestViewportDepth(0.5, 0.0);
    TestViewportDepth(1.0, 0.5);
}
