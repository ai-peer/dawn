...
class ViewportTest : public DawnTest {
  protected:
    ...
    void DoTest(const TestInfo& info) {
        ...
        constexpr RGBA8 kColor[EnumCount] = {
...
TEST_P(ViewportTest, Basic) {
    ...
    TestInfo info = {viewport, TopLeftTriangleColor, BottomRightTriangleColor, 1.0, true};
    DoTest(info);
}
...
TEST_P(ViewportTest, ShiftToTopLeft) {
    ...
    TestInfo info = {viewport, BottomRightTriangleColor, BackgroundColor, 1.0, true};
    DoTest(info);
}
...
TEST_P(ViewportTest, ShiftToBottomRight) {
    ViewportParams viewport = {2.0, 2.0, 4.0, 4.0, 0.0, 1.0};
    TestInfo info = {viewport, BackgroundColor, TopLeftTriangleColor, 1.0, true};
    DoTest(info);
}

// After apply the minDepth/maxDepth value in viewport and project to framebuffer coordinate, depth
// values of the top-left triangle are >= 0.25. They are greater than the depth values in depth
// buffer, so it is not drawn at all. As a result, point(0, 0) is not covered by any triangles.
// But the bottom-right triangle is drawn as usual.
TEST_P(ViewportTest, DepthOnly) {
    ViewportParams viewport = {0.0, 0.0, 4.0, 4.0, 0.0, 0.5};
    TestInfo info = {viewport, BackgroundColor, BottomRightTriangleColor, 0.25, true};
    DoTest(info);
}
...
TEST_P(ViewportTest, Combined) {
    ...
    TestInfo info = {viewport, BackgroundColor, BackgroundColor, 0.25, true};
    DoTest(info);
}
...
TEST_P(ViewportTest, EnlargeViewport) {
    ...
    TestInfo info = {viewport, TopLeftTriangleColor, TopLeftTriangleColor, 1.0, true};
    DoTest(info);
}
...
TEST_P(ViewportTest, ShrinkViewport) {
    ...
    TestInfo info = {viewport, TopLeftTriangleColor, BackgroundColor, 1.0, true};
    DoTest(info);
}
...
TEST_P(ViewportTest, ShrinkViewportAndShiftToTopLeftAndApplyDepth) {
    ...
    TestInfo info = {viewport, BottomRightTriangleColor, BackgroundColor, 0.25, true};
    DoTest(info);
}
