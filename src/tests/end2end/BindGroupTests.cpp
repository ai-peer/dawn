...
TEST_P(BindGroupTests, ReusedUBO) {
    ...
    wgpu::ShaderModule vsModule =
        ...
        #version 450
        layout (std140, set = 0, binding = 0) buffer vertexUniformBuffer {
            mat2 transform;
        };
        void main() {
            const vec2 pos[3] = vec2[3](vec2(-1.f, 1.f), vec2(1.f, 1.f), vec2(-1.f, -1.f));
    ...
    wgpu::ShaderModule fsModule =
        ...
        #version 450
        layout (std140, set = 0, binding = 1) buffer fragmentUniformBuffer {
            vec4 color;
        };
        layout(location = 0) out vec4 fragColor;
        void main() {
...
TEST_P(BindGroupTests, MultipleBindLayouts) {
    ...
    wgpu::ShaderModule vsModule =
        utils::CreateShaderModule(device, utils::SingleShaderStage::Vertex, R"(
        #version 450
        layout (std140, set = 0, binding = 0) buffer vertexUniformBuffer1 {
            mat2 transform1;
        };
        layout (std140, set = 1, binding = 0) buffer vertexUniformBuffer2 {
            mat2 transform2;
        };
        void main() {
            const vec2 pos[3] = vec2[3](vec2(-1.f, 1.f), vec2(1.f, 1.f), vec2(-1.f, -1.f));
