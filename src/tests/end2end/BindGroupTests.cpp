...
// Test a shader module that contains multiple entry points, using non-zero binding groups.
// This has the potential to cause problems when we only remap binding for one entry point, as the
// remaining unmapped binding numbers may be invalid for certain backends.
// This test passes by not asserting or crashing.
