...
// This test reproduces an out-of-bound bug when iterating bindGroups with 4 bind group sets twice
// in one render pass on D3D12 backends
TEST_P(BindGroupTests, DrawTwiceWithFourBindGroupSets)
{
    utils::BasicRenderPass renderPass = utils::CreateBasicRenderPass(device, kRTSize, kRTSize);
    ...
    dawn::RenderPassEncoder pass = builder.BeginRenderPass(renderPass.renderPassInfo);
    {
        ...
        std::array<dawn::Buffer, 4> uniformBuffers;
        std::array<dawn::BindGroup, 4> bindGroups;

        for (int i = 0; i < 4; ++i) {
            dawn::Buffer buffer = utils::CreateBufferFromData(
                device, &color, sizeof(color), dawn::BufferUsageBit::Uniform);
            uniformBuffers[i] = buffer;
            bindGroups[i] = utils::MakeBindGroup(
                device, layout, { { 0, uniformBuffers[i], 0, sizeof(color) } });
        }

        pass.SetPipeline(pipeline);
        pass.SetBindGroup(0, bindGroups[0]);
        pass.SetBindGroup(1, bindGroups[1]);
        pass.SetBindGroup(2, bindGroups[2]);
        pass.SetBindGroup(3, bindGroups[3]);
        pass.Draw(3, 1, 0, 0);
    }

    {
        std::array<float, 4> color = { 0.25, 0.25, 0, 0.25 };
        std::array<dawn::Buffer, 4> uniformBuffers;
        std::array<dawn::BindGroup, 4> bindGroups;
