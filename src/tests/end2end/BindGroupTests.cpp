        ...
        for (size_t i = 0; i < bindingTypes.size(); ++i) {
            switch (bindingTypes[i]) {
                ...
                case dawn::BindingType::StorageBuffer:
                    fs << "layout (std430, set = " << i << ", binding = 0) readonly buffer StorageBuffer" << i << R"( {
                        vec4 color;
                    } buffer)" << i << ";\n";
                    break;
                default:
...
TEST_P(BindGroupTests, SetBindGroupBeforePipeline) {
    ...
    dawn::RenderPipeline pipeline = MakeTestPipeline(renderPass, {dawn::BindingType::UniformBuffer}, {layout});
...
TEST_P(BindGroupTests, DrawThenChangePipelineAndBindGroup) {
    ...
    std::array<float, 4> color0 = {1, 0, 0, 0};
    ...
    std::vector<uint8_t> data(color3Offset + sizeof(color3));
    memcpy(data.data(), color0.data(), sizeof(color0));
    memcpy(data.data() + color1Offset, color1.data(), sizeof(color1));
    ...
    auto RecordFirstDraw = [&](dawn::RenderPassEncoder pass) {
        // uniform, uniform, storage
        pass.SetPipeline(pipeline0);
    ...
    // Run the base case then switch pipelines. The first and third bind groups persist.
    // The second bind group is changed.
    {
        dawn::CommandEncoder encoder = device.CreateCommandEncoder();
        dawn::RenderPassEncoder pass = encoder.BeginRenderPass(&renderPass.renderPassInfo);
        ...
        uint64_t dynamicOffset = color3Offset;
