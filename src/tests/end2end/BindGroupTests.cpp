...
TEST_P(BindGroupTests, MultipleBindLayouts) {
    ...
    std::vector<dawn::BindGroupLayout> layouts;

    layouts.push_back(utils::MakeBindGroupLayout(
        device, {
                    {0, dawn::ShaderStageBit::Vertex, dawn::BindingType::UniformBuffer},
                    {1, dawn::ShaderStageBit::Fragment, dawn::BindingType::UniformBuffer},
                }));
    ...
    dawn::PipelineLayout pipelineLayout = MakeBasicPipelineLayout(device, layouts);
    ...
    data.push_back(
        {{1.f, 0.f, dummy, dummy, dummy, dummy, dummy, dummy}, {0}, {0.f, 1.f, 0.f, 1.f}});

    data.push_back(
        {{dummy, dummy, dummy, dummy, 0.f, 1.0f, dummy, dummy}, {0}, {1.f, 0.f, 0.f, 1.f}});

    for (int i = 0; i < 2; i++) {
        dawn::Buffer buffer = utils::CreateBufferFromData(device, &data[i], sizeof(Data),
                                                          dawn::BufferUsageBit::Uniform);
        buffers.push_back(buffer);
        bindGroups.push_back(utils::MakeBindGroup(device, layouts[i],
