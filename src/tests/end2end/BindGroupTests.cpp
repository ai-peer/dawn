...
TEST_P(BindGroupTests, UBOSamplerAndTexture) {
    ...
    int widthInBytes = width * sizeof(uint32_t);
    widthInBytes = (widthInBytes + 255) & ~255;
    int sizeInBytes = widthInBytes * height;
    int size = sizeInBytes / sizeof(uint32_t);
    uint32_t* data = new uint32_t[size];
    for (int i = 0; i < size; i++) {
        data[i] = 0xFF00FF00;
    }
    dawn::Buffer stagingBuffer = utils::CreateBufferFromData(device, data, sizeInBytes, dawn::BufferUsageBit::TransferSrc);
    dawn::BindGroup bindGroup = device.CreateBindGroupBuilder()
        .SetLayout(bgl)
        .SetBufferViews(0, 1, &vertUBOBufferView)
        .SetSamplers(2, 1, &sampler)
        .SetTextureViews(3, 1, &textureView)
        .GetResult();

    dawn::CommandBufferBuilder builder = device.CreateCommandBufferBuilder();
    builder.CopyBufferToTexture(stagingBuffer, 0, kTextureRowPitchAlignment, texture, 0, 0, 0, width, height, 1, 0, 0);
    dawn::RenderPassEncoder pass = builder.BeginRenderPass(renderPass.renderPassInfo);
    pass.SetRenderPipeline(pipeline);
    pass.SetBindGroup(0, bindGroup);
    pass.DrawArrays(3, 1, 0, 0);
