...
TEST_P(MultipleWriteThenMultipleReadTests, SeparateBuffers) {
    ...
    wgpu::Buffer vertexBuffer =
        CreateBuffer(sizeof(float) * 4 * 4, wgpu::BufferUsage::Vertex | wgpu::BufferUsage::Storage);
    wgpu::Buffer indexBuffer =
        CreateBuffer(sizeof(int) * 4 * 2, wgpu::BufferUsage::Index | wgpu::BufferUsage::Storage);
    wgpu::Buffer uniformBuffer0 =
        CreateBuffer(sizeof(float), wgpu::BufferUsage::Uniform | wgpu::BufferUsage::Storage);
    wgpu::Buffer uniformBuffer1 =
        CreateBuffer(sizeof(float), wgpu::BufferUsage::Uniform | wgpu::BufferUsage::Storage);
    wgpu::BindGroup bindGroup0 = utils::MakeBindGroup(device, bgl0,
                                                      {{0, vertexBuffer, 0, sizeof(float) * 4 * 4},
                                                       {1, indexBuffer, 0, sizeof(int) * 4 * 2},
                                                       {2, uniformBuffer0, 0, sizeof(float)},
                                                       {3, uniformBuffer1, 0, sizeof(float)}});
    ...
    rpDesc.cVertexState.cVertexBuffers[0].arrayStride = 4 * sizeof(float);
    ...
    wgpu::BindGroup bindGroup1 = utils::MakeBindGroup(
        device, bgl1,
        {{0, uniformBuffer0, 0, sizeof(float)}, {1, uniformBuffer1, 0, sizeof(float)}});
...
TEST_P(MultipleWriteThenMultipleReadTests, OneBuffer) {
    ...
    wgpu::Buffer buffer = CreateBuffer(sizeof(struct Data),
    ...
    wgpu::BindGroup bindGroup1 = utils::MakeBindGroup(
        device, bgl1, {{0, buffer, 256 * 2, sizeof(float)}, {1, buffer, 256 * 3, sizeof(float)}});

    // Read various data in the buffer in render pass.
    wgpu::RenderPassEncoder pass1 = encoder.BeginRenderPass(&renderPass.renderPassInfo);
    pass1.SetPipeline(rp);
    pass1.SetVertexBuffer(0, buffer);
    pass1.SetIndexBuffer(buffer, 256);
    pass1.SetBindGroup(0, bindGroup1);
    pass1.DrawIndexed(6, 1, 0, 0, 0);
    pass1.EndPass();
