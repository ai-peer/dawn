...
TEST_P(MultipleWriteThenMultipleReadTests, SeparateBuffers) {
    ...
    wgpu::ShaderModule csModule = utils::CreateShaderModuleFromWGSL(device, R"(
        ...
        [[set(0), binding(2)]] var<storage_buffer> uniformContents : [[access(read_write)]] ColorContents;
        [[set(0), binding(3)]] var<storage_buffer> storageContents : [[access(read_write)]] ColorContents;
    ...
    RGBA8 expected(255, 128, 0, 255);
    EXPECT_PIXEL_RGBA8_EQ(expected, renderPass.color, min, min);
    EXPECT_PIXEL_RGBA8_EQ(expected, renderPass.color, max, min);
    EXPECT_PIXEL_RGBA8_EQ(expected, renderPass.color, min, max);
    EXPECT_PIXEL_RGBA8_EQ(expected, renderPass.color, max, max);
}
