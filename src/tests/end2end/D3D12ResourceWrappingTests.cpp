...
TEST_P(D3D12SharedHandleUsageTests, ClearInD3D11CopyAndReadbackInD3D12) {
    ...
    EXPECT_EQ(dawn_native::d3d12::GetAcquireMutexKeyForTesting(
                  reinterpret_cast<DawnTexture>(dawnSrcTexture.Get())),
              1u);
...
TEST_P(D3D12SharedHandleUsageTests, ClearInD3D11ReadbackInD3D12) {
    ...
    WrapAndClearD3D11Texture(&dawnDescriptor, &d3dDescriptor, &dawnTexture, clearColor,
                             &d3d11Texture, &dxgiKeyedMutex);
    EXPECT_EQ(dawn_native::d3d12::GetAcquireMutexKeyForTesting(
                  reinterpret_cast<DawnTexture>(dawnTexture.Get())),
              1u);
...
// 4. Verify that:
//    a. Ensure we receive the color we cleared with Dawn.
//    b. ExportSharedTexture returns the correct acquire key.
//    c. Multiple calls to ExportSharedTexture return an error key.
TEST_P(D3D12SharedHandleUsageTests, ClearInD3D12ReadbackInD3D11) {
    ...
    EXPECT_EQ(
        dawn_native::d3d12::ExportSharedTexture(reinterpret_cast<DawnTexture>(dawnTexture.Get())),
        2ull);

    // Since ExportSharedTexture destroys the texture, multiple export calls with the same texture
    // should return the error key: -1ull.
    EXPECT_EQ(
        dawn_native::d3d12::ExportSharedTexture(reinterpret_cast<DawnTexture>(dawnTexture.Get())),
        -1ull);
}
...
// 3. Readback the texture with D3D11.
// 4. Verify
//    a. The readback color was the final color cleared.
//    b. Each submission to the Dawn command queue increments the acquire key by 1.
//    c. ExportSharedTexture returns the expected acquire key.
//    d. Multiple calls to ExportSharedTexture return an error.
TEST_P(D3D12SharedHandleUsageTests, ClearTwiceInD3D12ReadbackInD3D11) {
    ...
    ExpectPixelRGBA8EQ(3, d3d11Texture.Get(), dxgiKeyedMutex.Get(), d3d12ClearColor2);
