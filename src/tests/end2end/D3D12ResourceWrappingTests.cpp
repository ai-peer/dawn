...
class D3D12SharedHandleUsageTests : public D3D12ResourceTestBase {
  protected:
    ...
    void WrapAndClearD3D11Texture(const dawn::TextureDescriptor* dawnDescriptor,
        ...
        dawn::Texture dawnTexture;
        ComPtr<ID3D11Texture2D> d3d11Texture;
        WrapSharedHandle(dawnDescriptor, d3dDescriptor, &dawnTexture, &d3d11Texture);
        EXPECT_EQ(dawn_native::d3d12::GetAcquireMutexKey(
        ...
        const float colorBGRA[] = {clearColor.r, clearColor.g, clearColor.b, clearColor.a};
        mD3d11DeviceContext->ClearRenderTargetView(d3d11RTV.Get(), colorBGRA);

        hr = dxgiKeyedMutex->ReleaseSync(1);
        ASSERT_EQ(hr, S_OK);

        dawn_native::d3d12::SetAcquireMutexKey(reinterpret_cast<DawnTexture>(dawnTexture.Get()),
                                               1u);

        *dawnTextureOut = dawnTexture;
        *d3d11TextureOut = d3d11Texture.Detach();
    ...
    void ExpectPixelRGBA8EQ(UINT64 acquireKey,
        ...
        Microsoft::WRL::ComPtr<ID3D11Texture2D> spD3DTextureStaging;
...
TEST_P(D3D12SharedHandleUsageTests, ClearInD3D11CopyAndReadbackInD3D12) {
    ...
    EXPECT_EQ(
        dawn_native::d3d12::GetAcquireMutexKey(reinterpret_cast<DawnTexture>(dawnSrcTexture.Get())),
        1u);
...
TEST_P(D3D12SharedHandleUsageTests, ClearInD3D11ReadbackInD3D12) {
    ...
    WrapAndClearD3D11Texture(&dawnDescriptor, &d3dDescriptor, &dawnTexture, clearColor,
                             &d3d11Texture, &dxgiKeyedMutex);
    EXPECT_EQ(
        dawn_native::d3d12::GetAcquireMutexKey(reinterpret_cast<DawnTexture>(dawnTexture.Get())),
        1u);
