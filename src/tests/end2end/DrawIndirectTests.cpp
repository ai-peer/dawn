...
TEST_P(DrawIndirectTest, Uint32) {
    ...
    dawn::Buffer indirectBuffer =
        utils::CreateBufferFromData<uint32_t>(device, dawn::BufferUsageBit::Indirect, {0, 0, 0, 0});
    Test(indirectBuffer, 0, notFilled, notFilled);

    // Test a draw with only the first 3 indices (bottom left triangle)
    indirectBuffer =
        utils::CreateBufferFromData<uint32_t>(device, dawn::BufferUsageBit::Indirect, {3, 1, 0, 0});
    Test(indirectBuffer, 0, filled, notFilled);

    // Test a draw with only the last 3 indices (top right triangle)
    indirectBuffer =
        utils::CreateBufferFromData<uint32_t>(device, dawn::BufferUsageBit::Indirect, {3, 1, 3, 0});
    Test(indirectBuffer, 0, notFilled, filled);

    // Test a draw with all 6 indices (both triangles).
    indirectBuffer =
        utils::CreateBufferFromData<uint32_t>(device, dawn::BufferUsageBit::Indirect, {6, 1, 0, 0});
    Test(indirectBuffer, 0, filled, filled);
}
...
TEST_P(DrawIndirectTest, IndirectOffset) {
    ...
    dawn::Buffer indirectBuffer = utils::CreateBufferFromData<uint32_t>(
        device, dawn::BufferUsageBit::Indirect, {3, 1, 0, 0, 3, 1, 3, 0});

    // Test #1 (no offset)
    Test(indirectBuffer, 0, filled, notFilled);

    // Offset to draw #2
    Test(indirectBuffer, 4 * sizeof(uint32_t), notFilled, filled);
}
