  ...
  protected:
    ...
    void Test(uint32_t vertexCount,
        ...
        dawn::Buffer indirectBuffer = utils::CreateBufferFromData<uint32_t>(
            ...
            {vertexCount, instanceCount, firstIndex, firstInstance,
             // For offset test, a second draw call (only the last 3 indices (top right triangle))
             3, 1, 3, 0});
...
TEST_P(DrawIndirectTest, IndirectOffset) {
    ...
    Test(3, 1, 0, 0, 4 * sizeof(uint32_t), notFilled, filled);
}
