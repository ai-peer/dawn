...
// A small fixture used to initialize default data for the validation tests.
...
class VulkanImageWrappingValidationTests : public VulkanImageWrappingTestBase {
  public:
    void SetUp() override {
        ...
        CreateBindExportImage(deviceVk, 10, 10, VK_FORMAT_R8G8B8A8_UNORM, &defaultImage,
...
// Test a successful wrapping in a texture
TEST_P(VulkanImageWrappingValidationTests, Success) {
    DAWN_SKIP_TEST_IF(UsesWire());

    dawn::Texture texture = WrapVulkanImage(device, &defaultDescriptor, defaultFd, {});
...
class VulkanImageWrappingUsageTests : public VulkanImageWrappingTestBase {
  public:
    void SetUp() override {
        ...
        mDeviceDescriptor.forceEnabledToggles = GetParam().forceEnabledWorkarounds;
        mDeviceDescriptor.forceDisabledToggles = GetParam().forceDisabledWorkarounds;

        secondDeviceVk = reinterpret_cast<dawn_native::vulkan::Device*>(
            backendAdapter->CreateDevice(&mDeviceDescriptor));
  ...
  protected:
    ...
    dawn_native::DeviceDescriptor mDeviceDescriptor;
...
// Clear an image in |mSecondaryDevice|
