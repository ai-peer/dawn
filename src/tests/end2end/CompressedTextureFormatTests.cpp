...
class CompressedTextureBCFormatTest : public DawnTest {
  protected:
    void CopyDataIntoCompressedTexture(dawn::Texture bcCompressedTexture,
        ...
        uint32_t actualHeightAtLevel =
            copyConfig.textureHeightLevel0 >> copyConfig.baseMipmapLevel;
        uint32_t copyWidthAtLevel = (actualWidthAtLevel + kBCBlockWidthInTexels - 1) /
                                    kBCBlockWidthInTexels * kBCBlockWidthInTexels;
        uint32_t copyHeightAtLevel = (actualHeightAtLevel + kBCBlockHeightInTexels - 1) /
        ...
        if (copyConfig.rowPitchAlignment != 0) {
            ...
            bufferRowPitchInBytes = (copyWidthAtLevel / kBCBlockWidthInTexels) *
                                    CompressedFormatBlockSizeInBytes(copyConfig.format);
        }

        uint32_t uploadBufferSize =
            bufferRowPitchInBytes * (copyHeightAtLevel / kBCBlockHeightInTexels);
        ...
        for (uint32_t h = 0; h < copyHeightAtLevel / kBCBlockHeightInTexels; ++h) {
            for (uint32_t w = 0; w < copyWidthAtLevel / kBCBlockWidthInTexels; ++w) {
    ...
    void InitPipelineAndBindGroup(dawn::Texture bcCompressedTexture,
    ...
    void Verify(uint32_t renderTargetWidth,
		...
		EXPECT_TEXTURE_RGBA8_EQ(expected.data(), renderPass.color, expectedOrigin.x, expectedOrigin.y,
    ...
    void VerifyCopyRegion(const CopyConfig& config) {
        ...
        Verify(
            noPaddingWidthAtLevel, noPaddingHeightAtLevel, config.copyOrigin3D, noPaddingExtent3D,
            expectedData);
    }
    ...
    static std::vector<RGBA8> FillExpectedDataWithPureRedAndPureGreen(uint32_t textureWidth,
        ...
        for (uint32_t y = 0; y < textureHeight; ++y) {
            for (uint32_t x = 0; x < textureWidth; ++x) {
                if (x % kBCBlockWidthInTexels > kBCBlockWidthInTexels / 2 - 1) {
