...
class CompressedTextureBCFormatTest : public DawnTest {
  protected:
    ...
    // Compute the upload data size with bytesPerRowAlignment and the copy region.
...
class WriteBCTextureTest : public CompressedTextureBCFormatTest {
  protected:
...
TEST_P(WriteBCTextureTest, WriteMultiple2DArrayLayers) {
    ...
    CopyConfig config;
    config.textureDescriptor.usage = kDefaultBCFormatTextureUsage;
    config.textureDescriptor.size = {20, 24, 25};

    config.copyOrigin3D = {4, 8, 12};
    config.copyExtent3D = {12, 16, 13};
    config.bytesPerRowAlignment = 511;
    config.rowsPerImage = 20;
