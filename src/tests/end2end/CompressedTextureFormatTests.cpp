  ...
  protected:
    ...
    static std::vector<uint8_t> GetOneBlockBCFormatTextureData(dawn::TextureFormat bcFormat) {
        switch (bcFormat) {
            ...
            // right side dark green, but the pixel values in the output attachments are different
            // compared with the tests using non-SRGB formats to the ones using SRGB formats.
            case dawn::TextureFormat::BC1RGBAUnorm:
            case dawn::TextureFormat::BC1RGBAUnormSrgb:
            ...
            // The expected data represents 4x4 pixel images with the left side dark red and the
            // right side dark green, but the pixel values in the output attachments are different
            // compared with the tests using non-SRGB formats to the ones using SRGB formats. The
            // pixels in the left side of the block all have an alpha value equal to 0x88.
            case dawn::TextureFormat::BC2RGBAUnorm:
            case dawn::TextureFormat::BC2RGBAUnormSrgb:
