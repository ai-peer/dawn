...
class CompressedTextureFormatSpecificTest : public DawnTest {
  protected:
    std::vector<const char*> GetRequiredExtensions() override {
        ...
        mIsETC2FormatSupported = SupportsExtensions({"texture-compression-etc2"});
        mIsASTCFormatSupported = SupportsExtensions({"texture-compression-astc"});

        std::vector<const char*> extensions;
        if (mIsBCFormatSupported) {
            extensions.emplace_back("texture_compression_bc");
        }
        if (mIsETC2FormatSupported) {
            extensions.emplace_back("texture-compression-etc2");
        }
        if (mIsASTCFormatSupported) {
            extensions.emplace_back("texture-compression-astc");
        }
        return extensions;
    }
    ...
    bool IsETC2FormatSupported() const {
        return mIsETC2FormatSupported;
    }
    bool IsASTCFormatSupported() const {
        return mIsASTCFormatSupported;
    }

    bool mIsBCFormatSupported = false;
    bool mIsETC2FormatSupported = false;
    bool mIsASTCFormatSupported = false;
};
