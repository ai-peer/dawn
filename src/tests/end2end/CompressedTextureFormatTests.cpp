...
class CompressedTextureBCFormatTest : public DawnTest {
  protected:
    ...
    std::vector<const char*> GetRequiredExtensions() override {
        const std::vector<const char*> kRequiredExtensions(1, "texture_compression_bc");
        mIsBCFormatSupported = SupportAllRequiredExtensions(kRequiredExtensions);
        if (!mIsBCFormatSupported) {
            return DawnTest::GetRequiredExtensions();
        }
