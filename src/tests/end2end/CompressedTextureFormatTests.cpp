...
class CompressedTextureBCFormatTest : public DawnTest {
  protected:
    ...
    void InitializeDataInCompressedTexture(dawn::Texture bcCompressedTexture,
        ...
        uint32_t actualWidthAtLevel = textureSize.width >> copyConfig.baseMipmapLevel;
        uint32_t actualHeightAtLevel = textureSize.height >> copyConfig.baseMipmapLevel;
    ...
    void TestCopyRegionIntoBCFormatTextures(const CopyConfig& config) {
        ...
        dawn::BindGroup bindGroup =
            CreateBindGroupForTest(bcTexture, config.textureDescriptor.format,
                                   config.baseArrayLayer, config.baseMipmapLevel);
    ...
    static dawn::Extent3D GetVirtualSizeAtLevel(const CopyConfig& config) {
        return {config.textureDescriptor.size.width >> config.baseMipmapLevel,
                config.textureDescriptor.size.height >> config.baseMipmapLevel, 1};
    }
...
TEST_P(CompressedTextureBCFormatTest, CopyBufferIntoNonZeroMipmapLevel) {
    ...
    const uint32_t kActualWidthAtLevel = textureSizeLevel0.width >> config.baseMipmapLevel;
    const uint32_t kActualHeightAtLevel = textureSizeLevel0.height >> config.baseMipmapLevel;
...
TEST_P(CompressedTextureBCFormatTest, CopyWholeTextureSubResourceIntoNonZeroMipmapLevel) {
    ...
    for (dawn::TextureFormat format : kBCFormats) {
        ...
        dawn::BindGroup bindGroup = CreateBindGroupForTest(
            bcTextureDst, format, config.baseArrayLayer, config.baseMipmapLevel);
        dawn::RenderPipeline renderPipeline = CreateRenderPipelineForTest();
...
TEST_P(CompressedTextureBCFormatTest, CopyPartofTextureSubResourceIntoNonZeroMipmapLevel) {
    ...
    srcConfig.baseMipmapLevel = srcConfig.textureDescriptor.mipLevelCount - 1;
    ...
    for (dawn::TextureFormat format : kBCFormats) {
        ...
        // Verify if we can use bcTexture as sampled textures correctly.
        dawn::BindGroup bindGroup = CreateBindGroupForTest(
            bcTextureDst, format, dstConfig.baseArrayLayer, dstConfig.baseMipmapLevel);
        dawn::RenderPipeline renderPipeline = CreateRenderPipelineForTest();
