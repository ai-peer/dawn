...
TEST_P(ExternalTextureTests, SampleExternalTexture) {
    // TODO (brandon1.jones@intel.com)
    // Remove this after Tint rolls.
    DAWN_SKIP_TEST_IF(IsD3D12() && IsUsingTintGenerator());
    ...
    const wgpu::ShaderModule fsModule = utils::CreateShaderModule(device, R"(
        ...
        [[stage(fragment)]] fn main() -> void {
            FragColor = textureSampleLevel(myTexture, mySampler, FragCoord.xy / vec2<f32>(4.0, 4.0));
            return;
        })");
    ...
    wgpu::Buffer vertexBuffer = utils::CreateBufferFromData(device, vertexData, sizeof(vertexData),
