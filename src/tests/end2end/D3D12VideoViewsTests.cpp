        ...
        wgpu::ShaderModule GetTestVertexShaderModule() const {
            return utils::CreateShaderModuleFromWGSL(device, R"(
                ...
                [[stage(vertex)]] fn main() -> void {
                    ...
                    texCoord = vec2<f32>(Position.xy * 0.5);
                    return;
            })");
        }
...
// Test copying from one multi-plane texture into another fails.
...
// Test copying from one multi-plane texture into another per plane fails.
TEST_P(D3D12VideoViewsValidation, T2TCopyPlaneAspectFails) {
    ...
    srcCopyView =
        utils::CreateTextureCopyView(srcTexture, 0, {0, 0, 0}, wgpu::TextureAspect::Plane1);
...
// Test copying from a multi-plane texture to a buffer fails.
TEST_P(D3D12VideoViewsValidation, T2BCopyAllAspectsFails) {
    wgpu::Texture srcTexture;
    CreateVideoTextureForTest(wgpu::TextureFormat::R8BG82plane420Unorm, wgpu::TextureUsage::CopySrc,
                              &srcTexture);
...
// Test copying from multi-plane texture per plane to a buffer fails.
TEST_P(D3D12VideoViewsValidation, T2BCopyPlaneAspectsFails) {
    wgpu::Texture srcTexture;
    CreateVideoTextureForTest(wgpu::TextureFormat::R8BG82plane420Unorm, wgpu::TextureUsage::CopySrc,
                              &srcTexture);
...
// Test copying from a buffer to a multi-plane texture fails.
TEST_P(D3D12VideoViewsValidation, B2TCopyAllAspectsFails) {
    std::vector<uint8_t> dummyData(4, 0);

    wgpu::Buffer srcBuffer = utils::CreateBufferFromData(device, dummyData.data(), dummyData.size(),
...
// Test copying from a buffer to a multi-plane texture per plane fails.
TEST_P(D3D12VideoViewsValidation, B2TCopyPlaneAspectsFails) {
    ...
    wgpu::TextureCopyView dstCopyView =
        utils::CreateTextureCopyView(dstTexture, 0, {0, 0, 0}, wgpu::TextureAspect::Plane0);
...
// Tests which multi-plane formats are allowed to be sampled.
...
// Tests writing from one multi-plane texture into another fails.
TEST_P(D3D12VideoViewsValidation, WriteTextureAllAspectsFails) {
    wgpu::Texture texture;
    CreateVideoTextureForTest(wgpu::TextureFormat::R8BG82plane420Unorm,
                              wgpu::TextureUsage::Sampled | wgpu::TextureUsage::CopyDst, &texture);
...
// Tests writing from one multi-plane texture into another per plane fails.
TEST_P(D3D12VideoViewsValidation, WriteTexturePlaneAspectsFails) {
    wgpu::Texture texture;
    CreateVideoTextureForTest(wgpu::TextureFormat::R8BG82plane420Unorm,
                              wgpu::TextureUsage::Sampled | wgpu::TextureUsage::CopyDst, &texture);
    ...
    wgpu::TextureCopyView textureCopyView =
        utils::CreateTextureCopyView(texture, 0, {0, 0, 0}, wgpu::TextureAspect::Plane0);

    std::vector<uint8_t> dummmyData(4, 0);
    wgpu::Extent3D writeSize = {1, 1, 1};

    ASSERT_DEVICE_ERROR(queue.WriteTexture(&textureCopyView, dummmyData.data(), dummmyData.size(),
                                           &textureDataLayout, &writeSize));
}
...
TEST_P(D3D12VideoViewsUsageTests, NV12SampleYUVtoRGB) {
    wgpu::Texture wgpuTexture;
    CreateVideoTextureForTest(wgpu::TextureFormat::R8BG82plane420Unorm, wgpu::TextureUsage::Sampled,
                              &wgpuTexture, true);
