    ...
    class D3D12VideoViewsTests : public DawnTest {
      protected:
        ...
        std::vector<const char*> GetRequiredExtensions() override {
            mIsMultiPlaneTexturesSupported = SupportsExtensions({"multiplane_textures"});
            if (!mIsMultiPlaneTexturesSupported) {
                return {};
            }

            return {"multiplane_textures"};
        }

        bool IsMultiPlaneTexturesSupported() const {
            return mIsMultiPlaneTexturesSupported;
        }
        ...
        bool mIsMultiPlaneTexturesSupported = false;
    };
}  // namespace
...
TEST_P(D3D12VideoViewsTests, NV12SampleYtoR) {
    DAWN_SKIP_TEST_IF(!IsMultiPlaneTexturesSupported());
