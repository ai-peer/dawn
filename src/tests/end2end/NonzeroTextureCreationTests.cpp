...
namespace {
    ...
    DAWN_TEST_PARAM_STRUCT(Params, Format, Usage, Dimension, DepthOrArrayLayers)
    ...
    class NonzeroTextureCreationTests : public DawnTestWithParams<Params> {
      protected:
        ...
        void Run() {
            ...
            for (uint32_t mip = 0; mip < descriptor.mipLevelCount; ++mip) {
                uint32_t mipSize = std::max(kSize >> mip, 1u);
                uint32_t depthOrArrayLayers =
                    GetParam().mDimension == wgpu::TextureDimension::e3D
                ...
                switch (GetParam().mFormat) {
                    case wgpu::TextureFormat::RGBA8Unorm: {
                        if ((GetParam().mUsage & wgpu::TextureUsage::RenderAttachment) != 0 ||
                            ...
                            std::vector<RGBA8> expected(texelCount, RGBA8(255, 255, 255, 255));
                    ...
                    case wgpu::TextureFormat::BC1RGBAUnorm: {
                        ...
                        uint32_t blockHeight =
                            utils::GetTextureFormatBlockHeight(GetParam().mFormat);
                        ...
                        uint64_t bufferSize = utils::RequiredBytesInCopy(
...
DAWN_INSTANTIATE_TEST_P(NonzeroTextureCreationTests,
