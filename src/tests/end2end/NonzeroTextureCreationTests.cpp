...
TEST_P(NonzeroTextureCreationTests, MipMapClears) {
    ...
    EXPECT_2D_TEXTURE_RGBA8_EQ(expected.data(), texture, 0, 0, mipSize, mipSize, 2, 0);
}
...
TEST_P(NonzeroTextureCreationTests, ArrayLayerClears) {
    ...
    EXPECT_2D_TEXTURE_RGBA8_EQ(expected.data(), texture, 0, 0, kSize, kSize, 0, 2);
}
...
TEST_P(NonzeroTextureCreationTests, AllSubresourcesFilled) {
...

    {
        ...
        for (uint32_t i = 0; i < descriptor.size.depthOrArrayLayers; ++i) {
            EXPECT_2D_TEXTURE_RGBA8_EQ(&filled, texture, 0, 0, 1, 1, 0, i);
        }
    }

    {
        ...
        for (uint32_t i = 0; i < descriptor.mipLevelCount; ++i) {
            EXPECT_2D_TEXTURE_RGBA8_EQ(&filled, texture, 0, 0, 1, 1, i, 0);
        }
    }

    {
        ...
        for (uint32_t i = 0; i < descriptor.size.depthOrArrayLayers; ++i) {
            for (uint32_t j = 0; j < descriptor.mipLevelCount; ++j) {
                EXPECT_2D_TEXTURE_RGBA8_EQ(&filled, texture, 0, 0, 1, 1, j, i);
            }
        }
    }
}
