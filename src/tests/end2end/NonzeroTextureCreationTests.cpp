...
TEST_P(NonzeroTextureCreationTests, TextureCreationClearsOneBits) {
    device.SetToggle(dawn::Toggle::NonzeroClearResourcesOnCreationForTesting, true);
...
// does not get lazy cleard to 0's and old memory is getting recycled.
// Test that texture clears to 0's because toggle is enabled.
TEST_P(NonzeroTextureCreationTests, TextureCreationClearsZeroBits) {
    device.SetToggle(dawn::Toggle::NonzeroClearResourcesOnCreationForTesting, false);
...
DAWN_INSTANTIATE_TEST(NonzeroTextureCreationTests, VulkanBackend);