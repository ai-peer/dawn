...
TEST_P(NonzeroTextureCreationTests, Depth32TextureCreationDepthClears) {
    ...
    if (IsMetal()) {
        // Metal initializes this with a copy, and just for the sake of testing, it would be
        // excessive to create two buffers: One to clear depth with 0x000803F (1.0f), and one to
        // clear stencil with 0xFF.
        // Instead, we just check the values as if they were unsigned integers in this test.
        // TODO(crbug.com/dawn/145): Update this if the Metal backend uses multiple render passes to
...
// Test that nonrenderable texture formats clear 0xFF because toggle is enabled
...
TEST_P(NonzeroTextureCreationTests, MemsetClearsBuffer) {
    uint32_t bufferSize = 4 * kSize * kSize;
    std::vector<uint8_t> data(bufferSize, 100);
    wgpu::Buffer bufferDst = utils::CreateBufferFromData(
        device, data.data(), static_cast<uint32_t>(data.size()), wgpu::BufferUsage::CopySrc);
    ...
    auto result = device.CreateBufferMapped(&bufferDesc);
    wgpu::Buffer bufferSrc = result.buffer;

    memset(result.data, 0xFF, bufferSize);
    bufferSrc.Unmap();

    wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
    encoder.CopyBufferToBuffer(bufferSrc, 0, bufferDst, 0, bufferSize);
    wgpu::CommandBuffer commands = encoder.Finish();
    queue.Submit(1, &commands);
...
// Test that textures with more than 1 array layers and nonrenderable texture formats clear 0xFF
