...
// Test out of bounds accesses to dynamic buffer bindings are clamped.
TEST_P(OOBDynamicBufferOffsetTests, CheckOOBAccess) {
    ...
    wgpu::ComputePipeline pipeline;
    {
        ...
        if (GetParam().mOOBRead) {
            shader << "let kReadOffset: u32 = " << std::to_string(kArrayLength) << "u;\n";
        } else {
            shader << "let kReadOffset: u32 = 0u;\n";
        }

        if (GetParam().mOOBWrite) {
            shader << "let kWriteOffset: u32 = " << std::to_string(kArrayLength) << "u;\n";
        } else {
    ...
    for (uint32_t i = 0; i < kArrayLength; ++i) {
        uint32_t readIndex = GetParam().mOOBRead ? kArrayLength - 1 : i;
        uint32_t writeIndex = GetParam().mOOBWrite ? kArrayLength - 1 : i;
    ...
    for (uint32_t i = 0; i < kArrayLength; ++i) {
        for (uint32_t c = 0; c < 4; ++c) {
            uint32_t value = GetParam().mOOBRead ? 0 : srcData[srcBufferByteOffset / 4 + 4 * i + c];
            if (!GetParam().mOOBWrite) {
                expectedDst1[dstBufferByteOffset / 4 + 4 * i + c] = value;
            }
        }
    }
