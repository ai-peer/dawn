...
class DynamicBufferOffsetTests : public DawnTest {
  protected:
    ...
    dawn::RenderPipeline CreateRenderPipeline(bool isInheritedPipeline = false) {
        ...
        fs << "#version 450\n";
        fs << " layout(std140, set = 0, binding = 0) uniform uBufferNotDynamic {\n";
        fs << "     uvec2 notDynamicValue;\n";
        fs << " };\n";
        fs << " layout(std140, set = 0, binding = 1) buffer sBufferNotDynamic {\n";
        fs << "     uvec2 notDynamicResult;\n";
        fs << " } mid;\n";
        fs << " layout(std140, set = 0, binding = 3) uniform uBuffer {\n";
        fs << "      uvec2 value;\n";
        fs << " };\n";
        fs << " layout(std140, set = 0, binding = 4) buffer SBuffer {\n";
        fs << "     uvec2 result;\n";
        fs << " } sBuffer;\n";

        if (isInheritedPipeline) {
            fs << " layout(std140, set = 1, binding = 0) uniform paddingBlock {\n";
            fs << "     uvec2 padding;\n";
            fs << " };\n";
        }
    ...
    dawn::ComputePipeline CreateComputePipeline(bool isInheritedPipeline = false) {
        ...
        std::string multipleNumber = isInheritedPipeline ? "2" : "1";
        cs << "#version 450\n";
        cs << " layout(std140, set = 0, binding = 0) uniform uBufferNotDynamic {\n";
        cs << "     uvec2 notDynamicValue;\n";
        cs << " };\n";
        cs << " layout(std140, set = 0, binding = 1) buffer sBufferNotDynamic {\n";
        cs << "     uvec2 notDynamicResult;\n";
        cs << " } mid;\n";
        cs << " layout(std140, set = 0, binding = 3) uniform uBuffer {\n";
        cs << "      uvec2 value;\n";
        cs << " };\n";
        cs << " layout(std140, set = 0, binding = 4) buffer SBuffer {\n";
        cs << "     uvec2 result;\n";
        cs << " } sBuffer;\n";

        if (isInheritedPipeline) {
            cs << " layout(std140, set = 1, binding = 0) uniform paddingBlock {\n";
            cs << "     uvec2 padding;\n";
            cs << " };\n";
        }
...
TEST_P(DynamicBufferOffsetTests, DISABLED_InheritDynamicOffestsComputePipeline) {
    dawn::ComputePipeline pipeline = CreateComputePipeline();
