  ...
  protected:
    ...
    wgpu::RenderPipeline CreateRenderPipeline(bool isInheritedPipeline = false) {
        ...
        fs << R"(
            ...
            [[set(0), binding(0)]] var<uniform> uBufferNotDynamic : Buffer;
            [[set(0), binding(1)]] var<storage_buffer> sBufferNotDynamic : [[access(read_write)]] Buffer;
            [[set(0), binding(2)]] var<uniform> uBuffer : Buffer;
            [[set(0), binding(3)]] var<storage_buffer> sBuffer : [[access(read_write)]] Buffer;
        )";

        if (isInheritedPipeline) {
            fs << R"(
                [[set(1), binding(0)]] var<uniform> paddingBlock : Buffer;
            )";
        }

        fs << "[[location(0)]] var<out> fragColor : vec4<f32>;\n";

        fs << "const multipleNumber : u32 = " << multipleNumber << ";\n";
        fs << R"(
            [[stage(fragment)]]
            fn main() -> void {
                sBufferNotDynamic.value.xy = uBufferNotDynamic.value.xy;
                sBuffer.value.xy = multipleNumber * (uBuffer.value.xy + sBufferNotDynamic.value.xy);
                fragColor = vec4<f32>(uBuffer.value.x / 255.0, uBuffer.value.y / 255.0,
                                      1.0, 1.0);
            }
        )";
    ...
    wgpu::ComputePipeline CreateComputePipeline(bool isInheritedPipeline = false) {
        ...
        cs << "const multipleNumber : u32 = " << multipleNumber << ";\n";
        cs << R"(
            [[stage(compute)]]
            fn main() -> void {
                sBufferNotDynamic.value.xy = uBufferNotDynamic.value.xy;
