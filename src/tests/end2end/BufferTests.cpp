...
TEST_P(BufferMappingTests, MapRead_Large) {
    ...
    ASSERT_EQ(0, memcmp(buffer.GetConstMappedRange(), myData.data(), kByteSize));
    ASSERT_EQ(0, memcmp(buffer.GetConstMappedRange(8), myData.data() + 2, kByteSize - 8));
    ASSERT_EQ(
    ...
    ASSERT_EQ(nullptr, buffer.GetConstMappedRange());
    ASSERT_EQ(nullptr, buffer.GetConstMappedRange(8));
    ASSERT_EQ(0, memcmp(buffer.GetConstMappedRange(12), myData.data() + 3, kByteSize - 12));
    ASSERT_EQ(0, memcmp(buffer.GetConstMappedRange(20), myData.data() + 5, kByteSize - 20));
    ASSERT_EQ(0, memcmp(buffer.GetConstMappedRange(20, kByteSize - 24), myData.data() + 5,
                        kByteSize - 44));
    buffer.Unmap();
}
    ...
    buffer.MapAsync(
        ...
        [](WGPUBufferMapAsyncStatus status, void* userdata) {
            ...
            if (status == WGPUBufferMapAsyncStatus_Success) {
                ...
                CheckMapping(user->buffer.GetConstMappedRange(4, 4), &zero, sizeof(zero));
