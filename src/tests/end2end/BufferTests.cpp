...
TEST_P(BufferMapReadTests, MapTwice) {
    // TODO(http://crbug.com/dawn/278): the second read cann't get updated data
    // on D3D12, Metal and Vulkan.
    DAWN_SKIP_TEST_IF(!IsOpenGL() || UsesWire());
    wgpu::BufferDescriptor descriptor;
    descriptor.size = 4;
    descriptor.usage = wgpu::BufferUsage::MapRead | wgpu::BufferUsage::CopyDst;
    wgpu::Buffer buffer = device.CreateBuffer(&descriptor);
...
// Call SetSubData at offset 0 via a u32 twice. Test that data is updated acoordingly.
