...
TEST_P(TextureViewTest, RG8UnormComputeShaderSampling) {
    ...
    queue.WriteTexture(&textureCopyView, textureData.data(), textureData.size(), &textureDataLayout, &writeSize);
    ...
    EXPECT_BUFFER_FLOAT_RANGE_EQ(
        expected.data(), resultBuffer, 0, mipWidth * mipHeight);
}
