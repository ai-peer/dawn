...
protected:
    ...
    void initTexture(uint32_t layerCount) {
        ...
        for (uint32_t layer = 0; layer < layerCount; ++layer) {
            ...
            data[0] = data[rowPixels + 1] = RGBA8(0, 0, 0, pixelValue);
            data[1] = data[rowPixels] = RGBA8(pixelValue, pixelValue, pixelValue, pixelValue);;
...
TEST_P(TextureViewTest, Default2DArrayTexture) {
    ...
    Test(textureView, fragmentShader, kLayers * (kLayers - 1) / 2);
}
