    ...
    wgpu::ShaderModule CreateDefaultVertexShaderModule(wgpu::Device device) {
        return utils::CreateShaderModuleFromWGSL(device, R"(
            ...
            [[stage(vertex)]] fn main() -> void {
                ...
                o_texCoord = texCoords[VertexIndex];
            }
        )");
    }
}  // anonymous namespace
  ...
  protected:
    ...
    void Texture2DViewTest(uint32_t textureArrayLayers,
        ...
        const char* fragmentShader = R"(
            [[group(0), location(0)]] var sampler0 : sampler;
            [[group(0), location(1)]] var texture0 : texture_2d<f32>;
    ...
    void Texture2DArrayViewTest(uint32_t textureArrayLayers,
        ...
        const char* fragmentShader = R"(
            [[group(0), location(0)]] var sampler0 : sampler;
            [[group(0), location(1)]] var texture0 : texture_2d_array<f32>;
            [[location(0)]] var<in> texCoord : vec2<f32>;
            [[location(0)]] var<out> FragColor : vec4<f32>;
    ...
    std::string CreateFragmentShaderForCubeMapFace(uint32_t layer, bool isCubeMapArray) {
        ...
        const std::array<std::string, 6> kCoordsToCubeMapFace = {{
            " 1.0,   tc,  -sc",  // Positive X
            "-1.0,   tc,   sc",  // Negative X
            "  sc,  1.0,  -tc",  // Positive Y
            "  sc, -1.0,   tc",  // Negative Y
            "  sc,   tc,  1.0",  // Positive Z
            " -sc,   tc, -1.0",  // Negative Z
        }};
        ...
        stream << R"(
            [[group(0), location(0)]] var sampler0 : sampler;
            [[group(0), location(1)]] var texture0 : )"
               << textureType << R"(<f32>;
            [[location(0)]] var<in> texCoord : vec2<f32>;
            [[location(0)]] var<out> FragColor : vec4<f32>;

            [[stage(fragment)]] fn main() -> void {
                const sc : f32 = 2.0 * texCoord.x - 1.0;
                const tc : f32 = 2.0 * texCoord.y - 1.0;
                FragColor = textureSample(texture0, sampler0, )";

        if (isCubeMapArray) {
            stream << "vec4<f32>(" << coordToCubeMapFace << ", " << cubeMapArrayIndex << "));\n";
        } else {
            stream << "vec3<f32>(" << coordToCubeMapFace << "));\n";
        }
        stream << "}";
