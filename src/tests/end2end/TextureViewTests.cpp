    ...
    void initTexture(uint32_t layerCount, uint32_t levelCount) {
        ...
        for (uint32_t layer = 0; layer < layerCount; ++layer) {
            for (uint32_t level = 0; level < levelCount; ++level) {
                ...
                dawn::BufferCopyView bufferCopyView = utils::CreateBufferCopyView(stagingBuffer, 0, kTextureRowPitchAlignment, 0);
                dawn::TextureCopyView textureCopyView = utils::CreateTextureCopyView(mTexture, level, layer, 0, 0, 0, dawn::TextureAspect::Color);
                dawn::Extent3D copySize = utils::CreateExtent3D(texWidth, texHeight, 1);
