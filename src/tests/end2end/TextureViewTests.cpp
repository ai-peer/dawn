...
protected:
    static int GetPixelValue(uint32_t layer, uint32_t level) {
    ...
    void initTexture(uint32_t layerCount, uint32_t levelCount) {
        ...
        // Create a texture with pixel = (0, 0, 0, level * 10 + layer + 1) at level `level` and layer `layer`.
        ...
        for (uint32_t layer = 0; layer < layerCount; ++layer) {
            for (uint32_t level = 0; level < levelCount; ++level) {
                ...
                const int pixelValue = GetPixelValue(layer, level);
                std::vector<RGBA8> data(kRowPixels * texHeight, RGBA8(0, 0, 0, pixelValue));
                dawn::Buffer stagingBuffer = utils::CreateBufferFromData(
                    device, data.data(), data.size() * sizeof(RGBA8), dawn::BufferUsageBit::TransferSrc);
                builder.CopyBufferToTexture(
                    stagingBuffer, 0, kTextureRowPitchAlignment, mTexture, 0, 0, 0, texWidth, texHeight, 1, level, layer);
            }
        }
    ...
    void Verify(const dawn::TextureView &textureView, const char* fragmentShader, int expected) {
        ...
        EXPECT_PIXEL_RGBA8_EQ(expectedPixel, mRenderPass.color, mRenderPass.width - 1, mRenderPass.height - 1);
    ...
    void Texture2DViewTest(uint32_t textureArrayLayers,
        ...
        // TODO(jiawei.shao@intel.com): support creating texture view by a texture view descriptor
        ...
        Verify(textureView, fragmentShader, GetPixelValue(textureViewBaseLayer, textureViewBaseMipLevel));
    }
    ...
    void Texture2DArrayViewTest(uint32_t textureArrayLayers,
        ...
        // We always set the layer count of the texture view to be 3 to match the fragment shader in this test.
        ...
        for (int i = 0; i < static_cast<int>(kTextureViewLayerCount); ++i) {
            expected += GetPixelValue(textureViewBaseLayer + i, textureViewBaseMipLevel);
        }
...
TEST_P(TextureViewTest, Default2DArrayTexture) {
    ...
    const int expected = GetPixelValue(0, 0) + GetPixelValue(1, 0) + GetPixelValue(2, 0);
    Verify(textureView, fragmentShader, expected);
}
