...
protected:
    ...
    void initTexture(uint32_t layerCount) {
        ...
        for (uint32_t layer = 0; layer < layerCount; ++layer) {
            ...
            builder.CopyBufferToTexture(stagingBuffer, 0, 256, mTexture, 0, 0, 0, 2, 2, 1, 0, layer);
        }
    ...
    void Test(const dawn::TextureView &textureView, const char* fragmentShader) {
        ...
        auto fsModule = utils::CreateShaderModule(device, dawn::ShaderStage::Fragment, fragmentShader);
...
TEST_P(TextureViewTest, Default2DArrayTexture) {
    ...
    const char* fragmentShader = R"(
            ...
            void main() {
                fragColor = texture(sampler2DArray(texture0, sampler0), vec3(gl_FragCoord.xy / 2.0, 0)) +
