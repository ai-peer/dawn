...
class DrawIndexedIndirectTest : public DawnTest {
  protected:
    ...
    void Test(dawn::Buffer& indirectBuffer,
...
TEST_P(DrawIndexedIndirectTest, Uint32) {
    ...
    dawn::Buffer indirectBuffer = utils::CreateBufferFromData<uint32_t>(
        device, dawn::BufferUsageBit::Indirect, {0, 0, 0, 0, 0});
    Test(indirectBuffer, 0, 0, notFilled, notFilled);

    // Test a draw with only the first 3 indices of the first quad (bottom left triangle)
    indirectBuffer = utils::CreateBufferFromData<uint32_t>(device, dawn::BufferUsageBit::Indirect,
                                                           {3, 1, 0, 0, 0});
    Test(indirectBuffer, 0, 0, filled, notFilled);

    // Test a draw with only the last 3 indices of the first quad (top right triangle)
    indirectBuffer = utils::CreateBufferFromData<uint32_t>(device, dawn::BufferUsageBit::Indirect,
                                                           {3, 1, 3, 0, 0});
    Test(indirectBuffer, 0, 0, notFilled, filled);

    // Test a draw with all 6 indices (both triangles).
    indirectBuffer = utils::CreateBufferFromData<uint32_t>(device, dawn::BufferUsageBit::Indirect,
                                                           {6, 1, 0, 0, 0});
    Test(indirectBuffer, 0, 0, filled, filled);
}

// Test the parameter 'baseVertex' of DrawIndexed() works.
TEST_P(DrawIndexedIndirectTest, BaseVertex) {
    // TODO(idanr@google.com): add workaround for OpenGL index buffer offset (could be compute
    ...
    dawn::Buffer indirectBuffer = utils::CreateBufferFromData<uint32_t>(
        device, dawn::BufferUsageBit::Indirect, {3, 1, 0, 4, 0});
    Test(indirectBuffer, 0, 0, notFilled, filled);
    ...
    indirectBuffer = utils::CreateBufferFromData<uint32_t>(device, dawn::BufferUsageBit::Indirect,
                                                           {3, 1, 0, -4, 0});
    Test(indirectBuffer, 6 * sizeof(uint32_t), 0, filled, notFilled);

    // Test a draw with only the last 3 indices of the first quad (top right triangle)
    indirectBuffer = utils::CreateBufferFromData<uint32_t>(device, dawn::BufferUsageBit::Indirect,
                                                           {3, 1, 3, -4, 0});
    Test(indirectBuffer, 6 * sizeof(uint32_t), 0, notFilled, filled);
}
...
TEST_P(DrawIndexedIndirectTest, IndirectOffset) {
    ...
    dawn::Buffer indirectBuffer = utils::CreateBufferFromData<uint32_t>(
        device, dawn::BufferUsageBit::Indirect, {3, 1, 0, 4, 0, 3, 1, 3, 4, 0});

    // Test #1 (no offset)
    Test(indirectBuffer, 0, 0, notFilled, filled);

    // Offset to draw #2
    Test(indirectBuffer, 0, 5 * sizeof(uint32_t), filled, notFilled);
}
