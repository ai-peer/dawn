    ...
    void Test(uint32_t indexCount,
        ...
        dawn::Buffer indirectBuffer = utils::CreateBufferFromData<uint32_t>(
            ...
            {indexCount, instanceCount, firstIndex, baseVertex, firstInstance,
             // For offset test, a second draw call (only the last 3 indices of the second quad)
             3, 1, 3, 4, 0});
...
TEST_P(DrawIndexedIndirectTest, IndirectOffset) {
    ...
    // Test an offset draw call, with indirect buffer containing:
    // 1) first 3 indices of the second quad (top right triangle)
    // 2) last 3 indices of the second quad

    // Test #1 (no offset) (function arguments are #1)
    Test(3, 1, 0, 4, 0, 0, 0, notFilled, filled);

    // Offset to draw only #2
    Test(3, 1, 0, 4, 0, 0, 5 * sizeof(uint32_t), filled, notFilled);
}
