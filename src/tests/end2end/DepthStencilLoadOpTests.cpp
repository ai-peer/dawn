      ...
      protected:
        ...
        void CheckMipLevel(uint32_t mipLevel) {
            ...
            switch (GetParam().mCheck) {
                ...
                case Check::DepthTest: {
                    std::vector<float> expectedDepth(mipSize * mipSize, kDepthValues[mipLevel]);
                    ExpectAttachmentDepthData(texture, GetParam().mFormat, mipSize, mipSize, 0,
                ...
                case Check::StencilTest: {
                    ExpectAttachmentStencilData(texture, GetParam().mFormat, mipSize, mipSize, 0,
...
TEST_P(DepthStencilLoadOpTests, ClearFirstMip) {
...
TEST_P(DepthStencilLoadOpTests, ClearSecondMip) {
    wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
    encoder.BeginRenderPass(&renderPassDescriptors[0]).EndPass();
    wgpu::CommandBuffer commandBuffer = encoder.Finish();
    queue.Submit(1, &commandBuffer);
...
// Clear first mip then the second mip.
TEST_P(DepthStencilLoadOpTests, ClearFirstThenSecondMip) {
    wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
    encoder.BeginRenderPass(&renderPassDescriptors[0]).EndPass();
...
// Clear second mip then the first mip.
TEST_P(DepthStencilLoadOpTests, ClearSecondThenFirstMip) {
    wgpu::CommandEncoder encoder = device.CreateCommandEncoder();
    encoder.BeginRenderPass(&renderPassDescriptors[1]).EndPass();
    encoder.BeginRenderPass(&renderPassDescriptors[0]).EndPass();
    wgpu::CommandBuffer commandBuffer = encoder.Finish();
