...
class ReadOnlyDepthStencilAttachmentTests : public DawnTest {
  protected:
    ...
    void TestDepth(wgpu::Texture colorTexture) {
        ...
        {
            // Create a render pass with readonly depth attachment.
...
TEST_P(ReadOnlyDepthStencilAttachmentTests, Depth) {
    ...
    // The bottome part is rendered, whose red channel is sampled from depth attachment, which
    // is initialized into 0.5 (128).
...
DAWN_INSTANTIATE_TEST(ReadOnlyDepthStencilAttachmentTests, D3D12Backend());
