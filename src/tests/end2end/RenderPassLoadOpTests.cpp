...
TEST_P(RenderPassLoadOpTests, ColorClearThenLoadAndDraw) {
    ...
    auto clearZeroPass = commandsClearZeroEncoder.BeginRenderPass(&renderPassClearZero.desc);
    ...
    renderPassClearGreen.colorAttachmentsInfoPtr[0]->clearColor = {0.0f, 1.0f, 0.0f, 1.0f};
    auto commandsClearGreenEncoder = device.CreateCommandEncoder();
    auto clearGreenPass = commandsClearGreenEncoder.BeginRenderPass(&renderPassClearGreen.desc);
    ...
    renderPassLoad.colorAttachmentsInfoPtr[0]->loadOp = dawn::LoadOp::Load;
    dawn::CommandBuffer commandsLoad;
    {
        auto encoder = device.CreateCommandEncoder();
        auto pass = encoder.BeginRenderPass(&renderPassLoad.desc);
        blueQuad.Draw(&pass);
        pass.EndPass();
