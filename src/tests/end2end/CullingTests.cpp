...
class CullingTest : public DawnTest {
  protected:
    ...
    void DoTest(dawn::FrontFace frontFace,
        ...
        if (isCCWTriangleCulled) {
            EXPECT_PIXEL_RGBA8_EQ(RGBA8(0, 255, 0, 255), colorTexture, 0, 0);
        } else {
            EXPECT_PIXEL_RGBA8_EQ(RGBA8(255, 0, 0, 255), colorTexture, 0, 0);
        }

        if (isCWTriangleCulled) {
            EXPECT_PIXEL_RGBA8_EQ(RGBA8(0, 255, 0, 255), colorTexture, kSize - 1, kSize - 1);
        } else {
...
DAWN_INSTANTIATE_TEST(CullingTest, MetalBackend);
