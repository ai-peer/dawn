...
TEST_P(TextureZeroInitTest, CopyBufferToTexture) {
    ...
    EXPECT_TEXTURE_RGBA8_EQ(expected.data(), texture, (0, 0, 0), (kSize, kSize), 0);
...
TEST_P(TextureZeroInitTest, CopyBufferToTextureHalf) {
    ...
    EXPECT_TEXTURE_RGBA8_EQ(expected100.data(), texture, (0, 0, 0), (kSize / 2, kSize), 0);
    // second half should be cleared
    EXPECT_TEXTURE_RGBA8_EQ(expectedZeros.data(), texture, (kSize / 2, 0, 0), (kSize / 2, kSize),
                            0);
...
TEST_P(TextureZeroInitTest, CopyTextureToTexture) {
    ...
    EXPECT_TEXTURE_RGBA8_EQ(expected.data(), srcTexture, (0, 0, 0), (kSize, kSize), 0);
    EXPECT_TEXTURE_RGBA8_EQ(expected.data(), dstTexture, (0, 0, 0), (kSize, kSize), 0);
...
TEST_P(TextureZeroInitTest, CopyTextureToTextureHalf) {
    ...
    EXPECT_TEXTURE_RGBA8_EQ(expectedWith100.data(), srcTexture, (0, 0, 0), (kSize, kSize), 0);
    EXPECT_TEXTURE_RGBA8_EQ(expectedWith100.data(), dstTexture, (0, 0, 0), (kSize / 2, kSize), 0);
    EXPECT_TEXTURE_RGBA8_EQ(expectedWithZeros.data(), dstTexture, (kSize / 2, 0, 0),
                            (kSize / 2, kSize), 0);
...
TEST_P(TextureZeroInitTest, IndependentDepthStencilLoadAfterDiscard) {
    ...
    // Uninitialize only depth
    {
        ...
        // at 2.
        {
            ...
            // and stencil was 2.
            std::vector<RGBA8> expected(kSize * kSize, {255, 0, 0, 255});
            EXPECT_TEXTURE_RGBA8_EQ(expected.data(), colorTexture, (0, 0, 0), (kSize, kSize), 0);
        }
...
TEST_P(TextureZeroInitTest, ColorAttachmentsClear) {
    ...
    std::vector<RGBA8> expected(kSize * kSize, {0, 0, 0, 0});
    EXPECT_TEXTURE_RGBA8_EQ(expected.data(), renderPass.color, (0, 0, 0), (kSize, kSize), 0);
...
TEST_P(TextureZeroInitTest, RenderPassSampledTextureClear) {
    ...
    EXPECT_TEXTURE_RGBA8_EQ(expectedWithZeros.data(), renderTexture, (0, 0, 0), (kSize, kSize), 0);
...
TEST_P(TextureZeroInitTest, RenderingLoadingDepthStencilStoreOpClear) {
    ...
    renderPassDescriptor.cDepthStencilAttachmentInfo.stencilStoreOp = wgpu::StoreOp::Clear;
    {
        ...
        EXPECT_TEXTURE_RGBA8_EQ(expectedBlack.data(), srcTexture, (0, 0, 0), (kSize, kSize), 0);
    ...
    // because storeOp clear sets the subresource as uninitialized
    {
        ...
        EXPECT_TEXTURE_RGBA8_EQ(expectedRed.data(), srcTexture, (0, 0, 0), (kSize, kSize), 0);
...
TEST_P(TextureZeroInitTest, PreservesInitializedMip) {
    ...
    EXPECT_LAZY_CLEAR(0u, EXPECT_TEXTURE_RGBA8_EQ(expectedWithTwos.data(), sampleTexture, (0, 0, 0),
                                                  (mipSize, mipSize), 1));
...
TEST_P(TextureZeroInitTest, WriteWholeTexture) {
    ...
    EXPECT_TEXTURE_RGBA8_EQ(data.data(), texture, (0, 0, 0), (kSize, kSize), 0);
}
...
TEST_P(TextureZeroInitTest, WriteTextureHalf) {
    ...
    EXPECT_TEXTURE_RGBA8_EQ(data.data(), texture, (0, 0, 0), (kSize / 2, kSize), 0);
    // second half should be cleared
    EXPECT_TEXTURE_RGBA8_EQ(expectedZeros.data(), texture, (kSize / 2, 0, 0), (kSize / 2, kSize),
                            0);
}
...
TEST_P(TextureZeroInitTest, WriteWholeTextureAtMipLevel) {
    ...
    EXPECT_TEXTURE_RGBA8_EQ(data.data(), texture, (0, 0, 0), (kMipSize, kMipSize), kMipLevel);
}
...
TEST_P(TextureZeroInitTest, WriteTextureHalfAtMipLevel) {
    ...
    // first half filled with 100, by the data
    EXPECT_TEXTURE_RGBA8_EQ(data.data(), texture, (0, 0, 0), (kMipSize / 2, kMipSize), kMipLevel);
    // second half should be cleared
    EXPECT_TEXTURE_RGBA8_EQ(expectedZeros.data(), texture, (kMipSize / 2, 0, 0),
  ...
  protected:
    ...
    void SampleCompressedTextureAndVerifyColor(wgpu::Texture bcTexture,
        ...
        std::vector<RGBA8> expected(nonPaddedCopyExtent.width * nonPaddedCopyExtent.height,
                                    {0x00, 0x20, 0x08, 0xFF});
        EXPECT_TEXTURE_RGBA8_EQ(expected.data(), renderPass.color, (0, 0, 0),
        ...
        if (halfCopyTest) {
            std::vector<RGBA8> expectBlack(nonPaddedCopyExtent.width * nonPaddedCopyExtent.height,
                                           {0x00, 0x00, 0x00, 0xFF});
            EXPECT_TEXTURE_RGBA8_EQ(expectBlack.data(), renderPass.color,
                                    (copyExtent3D.width, 0, 0),
                                    (nonPaddedCopyExtent.width, nonPaddedCopyExtent.height), 0);
        }
    }
