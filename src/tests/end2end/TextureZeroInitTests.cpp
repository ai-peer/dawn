...
TEST_P(TextureZeroInitTest, CopyMultipleTextureArrayLayersToBufferSource) {
    ...
    for (uint32_t layer = 0; layer < kArrayLayers; ++layer) {
        EXPECT_TEXTURE_RGBA8_EQ(kExpectedAllZero.data(), texture, {0, 0, layer}, {kSize, kSize});
    }
}
...
TEST_P(TextureZeroInitTest, CopyBufferToTexture) {
    ...
    EXPECT_TEXTURE_RGBA8_EQ(expected.data(), texture, {0, 0}, {kSize, kSize});
...
TEST_P(TextureZeroInitTest, CopyBufferToTextureHalf) {
    ...
    EXPECT_TEXTURE_RGBA8_EQ(expected100.data(), texture, {0, 0}, {kSize / 2, kSize});
...
TEST_P(TextureZeroInitTest, CopyBufferToTextureMultipleArrayLayers) {
    ...
    for (uint32_t layer = kBaseArrayLayer; layer < kBaseArrayLayer + kCopyLayerCount; ++layer) {
        EXPECT_TEXTURE_RGBA8_EQ(expected100.data(), texture, {0, 0, layer}, {kSize, kSize});
    }
}
...
TEST_P(TextureZeroInitTest, CopyTextureToTexture) {
    ...
    EXPECT_TEXTURE_RGBA8_EQ(expected.data(), srcTexture, {0, 0}, {kSize, kSize});
    EXPECT_TEXTURE_RGBA8_EQ(expected.data(), dstTexture, {0, 0}, {kSize, kSize});
...
TEST_P(TextureZeroInitTest, CopyTextureToTextureHalf) {
    ...
    EXPECT_TEXTURE_RGBA8_EQ(expectedWith100.data(), srcTexture, {0, 0}, {kSize, kSize});
    EXPECT_TEXTURE_RGBA8_EQ(expectedWith100.data(), dstTexture, {0, 0}, {kSize / 2, kSize});
...
TEST_P(TextureZeroInitTest, IndependentDepthStencilLoadAfterDiscard) {
    ...
    // Uninitialize only depth
    {
        ...
        // at 2.
        {
            ...
            // and stencil was 2.
            std::vector<RGBA8> expected(kSize * kSize, {255, 0, 0, 255});
            EXPECT_TEXTURE_RGBA8_EQ(expected.data(), colorTexture, {0, 0}, {kSize, kSize});
        }
...
TEST_P(TextureZeroInitTest, ColorAttachmentsClear) {
    ...
    std::vector<RGBA8> expected(kSize * kSize, {0, 0, 0, 0});
    EXPECT_TEXTURE_RGBA8_EQ(expected.data(), renderPass.color, {0, 0}, {kSize, kSize});
...
TEST_P(TextureZeroInitTest, RenderPassSampledTextureClear) {
    ...
    EXPECT_TEXTURE_RGBA8_EQ(expectedWithZeros.data(), renderTexture, {0, 0}, {kSize, kSize});
...
TEST_P(TextureZeroInitTest, TextureBothSampledAndAttachmentClear) {
    ...
    EXPECT_TEXTURE_RGBA8_EQ(&RGBA8::kZero, texture, {0, 0, 0}, {1, 1});
    EXPECT_TEXTURE_RGBA8_EQ(&RGBA8::kZero, texture, {0, 0, 1}, {1, 1});
...
TEST_P(TextureZeroInitTest, RenderingLoadingDepthStencilStoreOpClear) {
    ...
    renderPassDescriptor.cDepthStencilAttachmentInfo.stencilStoreOp = wgpu::StoreOp::Clear;
    {
        ...
        EXPECT_TEXTURE_RGBA8_EQ(expectedBlack.data(), srcTexture, {0, 0}, {kSize, kSize});
    ...
    // because storeOp clear sets the subresource as uninitialized
    {
        ...
        EXPECT_TEXTURE_RGBA8_EQ(expectedRed.data(), srcTexture, {0, 0}, {kSize, kSize});
...
TEST_P(TextureZeroInitTest, WriteWholeTexture) {
    ...
    EXPECT_TEXTURE_RGBA8_EQ(data.data(), texture, {0, 0}, {kSize, kSize});
}
...
TEST_P(TextureZeroInitTest, WriteTextureHalf) {
    ...
    EXPECT_TEXTURE_RGBA8_EQ(data.data(), texture, {0, 0}, {kSize / 2, kSize});
    // second half should be cleared
    EXPECT_TEXTURE_RGBA8_EQ(expectedZeros.data(), texture, {kSize / 2, 0}, {kSize / 2, kSize});
}
...
TEST_P(TextureZeroInitTest, WriteWholeTextureArray) {
    ...
    for (uint32_t layer = kBaseArrayLayer; layer < kBaseArrayLayer + kCopyLayerCount; ++layer) {
        EXPECT_TEXTURE_RGBA8_EQ(data.data(), texture, {0, 0, layer}, {kSize, kSize});
    }
}
...
TEST_P(TextureZeroInitTest, WriteTextureArrayHalf) {
    ...
    for (uint32_t layer = kBaseArrayLayer; layer < kBaseArrayLayer + kCopyLayerCount; ++layer) {
        // first half filled with 100, by the data
        EXPECT_TEXTURE_RGBA8_EQ(data.data(), texture, {0, 0, layer}, {kSize / 2, kSize});
        // second half should be cleared
