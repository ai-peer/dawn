...
TEST_P(TextureZeroInitTest, WriteWholeTextureAtMipLevel) {
    ...
    wgpu::TextureCopyView textureCopyView = utils::CreateTextureCopyView(texture, 2, {0, 0, 0});
    wgpu::Extent3D copySize = {kSize, kSize, 1};
    ...
    // Expect texture initialized to be true
    EXPECT_TRUE(dawn_native::IsTextureSubresourceInitialized(texture.Get(), 2, 1, 0, 1));

    EXPECT_TEXTURE_RGBA8_EQ(data.data(), texture, 0, 0, kSize, kSize, 2, 0);
}
...
TEST_P(TextureZeroInitTest, WriteTextureHalfAtMipLevel) {
    ...
    // Expect texture initialized to be true
    EXPECT_EQ(true, dawn_native::IsTextureSubresourceInitialized(texture.Get(), 2, 1, 0, 1));

    std::vector<RGBA8> expectedZeros((kSize / 2) * kSize, {0, 0, 0, 0});
    // first half filled with 100, by the data
    EXPECT_TEXTURE_RGBA8_EQ(data.data(), texture, 0, 0, kSize / 2, kSize, 2, 0);
    // second half should be cleared
    EXPECT_TEXTURE_RGBA8_EQ(expectedZeros.data(), texture, kSize / 2, 0, kSize / 2, kSize, 2, 0);
}
