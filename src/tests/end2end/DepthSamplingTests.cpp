  ...
  protected:
    ...
    wgpu::RenderPipeline CreateSamplingRenderPipeline(std::vector<TestAspect> aspects) {
        ...
        for (TestAspect aspect : aspects) {
            ...
            shaderSource << "layout(set = 0, binding = " << index + 1 << ") uniform " << prefix
                         << "texture2D tex" << index << ";\n";

            shaderSource << "layout(location = " << index << ") out " << resultType << " result"
                         << index << ";\n";
            ...
            switch (aspect) {
                ...
                case TestAspect::Stencil:
                    shaderBody << "result" << index << " = texelFetch(usampler2D(tex" << index
                               << ", samp), ivec2(0, 0), 0).r;\n";
                    pipelineDescriptor.cColorStates[index].format = wgpu::TextureFormat::R32Uint;
                    break;
            }
    ...
    wgpu::ComputePipeline CreateSamplingComputePipeline(TestAspect aspect) {
        const char* prefix = aspect == TestAspect::Depth ? "" : "u";
        const char* resultType = aspect == TestAspect::Depth ? "float" : "uint";
        const char* getResult = aspect == TestAspect::Depth
                                    ? "texture(sampler2D(tex, samp), vec2(0.5, 0.5)).r;"
                                    : "texelFetch(usampler2D(tex, samp), ivec2(0, 0), 0).r;";
    ...
    void DoSamplingTest(TestAspect aspect,
        ...
        switch (aspect) {
            case TestAspect::Depth:
                ...
                break;
            case TestAspect::Stencil:
                outputTexture = CreateOutputTexture(wgpu::TextureFormat::R32Uint);
                break;
        }
...
TEST_P(DepthStencilSamplingTest, SampleDepthAndStencilRender) {
    wgpu::RenderPipeline pipeline =
        CreateSamplingRenderPipeline({TestAspect::Depth, TestAspect::Stencil});
    ...
    wgpu::Texture stencilOutput = CreateOutputTexture(wgpu::TextureFormat::R32Uint);
    ...
    EXPECT_TEXTURE_EQ(0.43f, depthOutput, 0, 0);
    EXPECT_TEXTURE_EQ(uint8_t(31), stencilOutput, 0, 0);
}
