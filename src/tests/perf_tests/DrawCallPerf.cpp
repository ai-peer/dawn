...
namespace {
    ...
    constexpr uint32_t kIndexData[3] = {0, 1, 2};
...
void DrawCallPerf::TestSetUp() {
    ...
    mAlignedVertexDataSize = Align(sizeof(kVertexData), kMinDynamicBufferOffsetAlignment);
    ...
    mIndexBuffer = utils::CreateBufferFromData(device, kIndexData, sizeof(kIndexData),
    ...
    switch (GetParam().vertexBufferType) {
        ...
        case VertexBuffer::Dynamic: {
            char* data = reinterpret_cast<char*>(malloc(mAlignedVertexDataSize * kNumDraws));
            for (uint32_t i = 0; i < kNumDraws; ++i) {
                memcpy(data + mAlignedVertexDataSize * i, kVertexData, sizeof(kVertexData));
            }

            mVertexBuffers[0] = utils::CreateBufferFromData(
                device, data, mAlignedVertexDataSize * kNumDraws, wgpu::BufferUsage::Vertex);
            free(data);
        } break;
    }
...
void DrawCallPerf::RecordRenderCommands(Encoder pass) {
    ...
    pass.SetIndexBuffer(mIndexBuffer);
...
DAWN_INSTANTIATE_PERF_TEST_SUITE_P(
    ...
    {
        // Baseline
        DrawCallParam{Pipeline::Static, VertexBuffer::NoChange, BindGroup::NoChange, UniformData::Static,  RenderBundle::No},  // Baseline

        // Change vertex buffer binding
        DrawCallParam{Pipeline::Static, VertexBuffer::Multiple, BindGroup::NoChange, UniformData::Static,  RenderBundle::No},  // Multiple vertex buffers
        DrawCallParam{Pipeline::Static, VertexBuffer::Dynamic,  BindGroup::NoChange, UniformData::Static,  RenderBundle::No},  // Dynamic vertex buffer

        // Change bind group binding
        DrawCallParam{Pipeline::Static, VertexBuffer::NoChange, BindGroup::Multiple, UniformData::Static,  RenderBundle::No},  // Multiple bind groups
        DrawCallParam{Pipeline::Static, VertexBuffer::NoChange, BindGroup::Dynamic,  UniformData::Static,  RenderBundle::No},  // Dynamic bind groups
        DrawCallParam{Pipeline::Static, VertexBuffer::NoChange, BindGroup::NoReuse,  UniformData::Static,  RenderBundle::No},  // New bind group per-draw

         // Switch the pipeline every draw to test state tracking and updates to binding points
        DrawCallParam{Pipeline::Dynamic, VertexBuffer::NoChange, BindGroup::Multiple, UniformData::Static,  RenderBundle::No},  // Multiple bind groups w/ dynamic pipeline
        DrawCallParam{Pipeline::Dynamic, VertexBuffer::NoChange, BindGroup::Dynamic,  UniformData::Static,  RenderBundle::No},  // Dynamic bind groups w/ dynamic pipeline
        ...
        DrawCallParam{Pipeline::Static, VertexBuffer::Multiple, BindGroup::NoChange, UniformData::Static,  RenderBundle::Yes},  // Multiple vertex buffers w/ render bundle
        DrawCallParam{Pipeline::Static, VertexBuffer::Dynamic,  BindGroup::NoChange, UniformData::Static,  RenderBundle::Yes},  // Dynamic vertex buffer w/ render bundle

        // Use render bundles with varying bind group binding
        DrawCallParam{Pipeline::Static, VertexBuffer::NoChange, BindGroup::Multiple, UniformData::Static,  RenderBundle::Yes},  // Multiple bind groups w/ render bundle
        DrawCallParam{Pipeline::Static, VertexBuffer::NoChange, BindGroup::Dynamic,  UniformData::Static,  RenderBundle::Yes},  // Dynamic bind groups w/ render bundle

        // Use render bundles with dynamic pipeline
        DrawCallParam{Pipeline::Dynamic, VertexBuffer::NoChange, BindGroup::Multiple, UniformData::Static,  RenderBundle::Yes},  // Multiple bind groups w/ dynamic pipeline w/ render bundle
        DrawCallParam{Pipeline::Dynamic, VertexBuffer::NoChange, BindGroup::Dynamic,  UniformData::Static,  RenderBundle::Yes},  // Dynamic bind groups w/ dynamic pipeline w/ render bundle
        // --------------------------------------

        // Update per-draw data in the bind group(s). This will cause resource transitions between updating and drawing.
        DrawCallParam{Pipeline::Static, VertexBuffer::NoChange, BindGroup::Multiple, UniformData::Dynamic, RenderBundle::No},   // Update per-draw data: Multiple bind groups
        DrawCallParam{Pipeline::Static, VertexBuffer::NoChange, BindGroup::Dynamic,  UniformData::Dynamic, RenderBundle::No},   // Update per-draw data: Dynamic bind groups
    });
