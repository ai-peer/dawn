...
class DawnPerfTestPlatform : public dawn_platform::Platform {
  public:
    ...
    struct TraceEvent final {
        ...
        TraceEvent(char phaseIn, const char* categoryNameIn, const char* nameIn, uint64_t idIn, double timestampIn)
            : phase(phaseIn), categoryName(categoryNameIn), name(nameIn), id(idIn), timestamp(timestampIn) {
        }
