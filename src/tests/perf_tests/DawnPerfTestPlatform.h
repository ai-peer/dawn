...
class DawnPerfTestPlatform : public dawn_platform::Platform {
  public:
    explicit DawnPerfTestPlatform(DawnPerfTestEnvironment* testEnv);
    ~DawnPerfTestPlatform() override;

    void DumpTraceEventsToJSONFile(const char* outputFileName);
  ...
  private:
    ...
    DawnPerfTestEnvironment* mTestEnv;
