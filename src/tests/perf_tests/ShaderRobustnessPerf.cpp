...
namespace {
    ...
    struct ShaderRobustnessParamss : AdapterTestParam {
        ShaderRobustnessParamss(const AdapterTestParam& param,
  ...
  public:
    ShaderRobustnessPerf()
        : DawnPerfTestWithParams(kNumIterations, 1),
          mDimAOuter(GetParam().m_DimAOuter),
          mDimInner(GetParam().m_DimInner),
          mDimBOuter(GetParam().m_DimBOuter) {
    }
...
void ShaderRobustnessPerf::SetUp() {
    ...
    switch (GetParam().m_MatMulMethod) {
        case MatMulMethod::MatMulFloatOneDimSharedArray: {
            module =
