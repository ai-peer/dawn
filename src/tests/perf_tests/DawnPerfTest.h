...
class DawnPerfTestBase {
  ...
  public:
    ...
    // A test deriving from |DawnPerfTestBase| must call the base contructor with |interationsPerStep|
    // appropriately to reflect the amount of work performed.
    // |maxStepsInFlight| may be used to mimic having multiple frames or workloads in flight which is common
    // with double or triple buffered applications.
    DawnPerfTestBase(DawnTestBase* test, unsigned int iterationsPerStep, unsigned int maxStepsInFlight);
...
class DawnPerfTestWithParams : public DawnTestWithParams<Params>, public DawnPerfTestBase {
  protected:
    DawnPerfTestWithParams(unsigned int iterationsPerStep, unsigned int maxStepsInFlight)
        : DawnTestWithParams<Params>(), DawnPerfTestBase(this, iterationsPerStep, maxStepsInFlight) {
    }
