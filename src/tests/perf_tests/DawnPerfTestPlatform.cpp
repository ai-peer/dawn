...
uint64_t DawnPerfTestPlatform::AddTraceEvent(char phase,
    ...
    // thread id will be populated later in AcquireTraceEventBuffer so we can defer string
    // conversion to minimize overhead of trace events.
    buffer->emplace_back(phase, info->category, name, id, "", timestamp);

    std::thread::id tid = std::this_thread::get_id();
    return static_cast<uint64_t>(std::hash<size_t>()(buffer->size()) ^
