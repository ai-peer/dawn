...
DawnPerfTestPlatform::DawnPerfTestPlatform(DawnPerfTestEnvironment* testEnv)
    : dawn_platform::Platform(), mTestEnv(testEnv), mTimer(utils::CreateTimer()) {
}
...
void DawnPerfTestPlatform::DumpTraceEventsToJSONFile(const char* outputFileName) {
    if (!mTestEnv->IsTracingEnabled()) {
        return;
    }
    ...
    for (const TraceEvent& traceEvent : mTraceEventBuffer) {
