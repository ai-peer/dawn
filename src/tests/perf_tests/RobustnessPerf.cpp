...
namespace {
    ...
    // The vec4 version requires that dimInner and dimOuter are divisible by 4.
...
void RobustnessPerf::SetUp() {
    ...
    for (uint32_t i = 0; i < static_cast<uint32_t>(dataASize); i++) {
        dataA.push_back((float)rand() / float(RAND_MAX));
    }
    ...
    for (uint32_t i = 0; i < static_cast<uint32_t>(dataBSize); i++) {
        dataB.push_back((float)rand() / float(RAND_MAX));
    }
...
DAWN_INSTANTIATE_PERF_TEST_SUITE_P(RobustnessPerf,
