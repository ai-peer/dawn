...
DawnPerfTestBase::DawnPerfTestBase(DawnTestBase* test, unsigned int iterationsPerStep, unsigned int maxStepsInFlight)
    : mTest(test), mIterationsPerStep(iterationsPerStep), mMaxStepsInFlight(maxStepsInFlight), mTimer(utils::CreateTimer()) {
}
...
void DawnPerfTestBase::DoRunLoop(double maxRunTime) {
    ...
    while (mRunning) {
        while (signaledFenceValue - fence.GetCompletedValue() + 1 > mMaxStepsInFlight) {
            mTest->WaitABit();
        }
        Step();
