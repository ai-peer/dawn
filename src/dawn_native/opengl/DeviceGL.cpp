...
namespace dawn_native { namespace opengl {
    ...
    MaybeError Device::TickImpl() {
        if (GetCompletedCommandSerial() == GetLastSubmittedCommandSerial() &&
            GetCompletedCommandSerial() < GetFutureCallbackSerial()) {
            // There is no work in the GPU and there is a future callback serial greater than the
            // current completed serial, assume commands complete to move the completed command
            // serial forward
