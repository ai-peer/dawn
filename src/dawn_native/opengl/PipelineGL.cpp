            ...
            for (const auto& it : bgl->GetBindingMap()) {
                ...
                switch (bgl->GetBindingInfo(bindingIndex).type) {
                    ...
                    case wgpu::BindingType::ReadonlyStorageBuffer: {
                        ...
                        if (location != GL_INVALID_INDEX && !gl.GetVersion().IsES()) {
                            // OpenGL ES doesn't have this call, but the ES backend has already
                            // handled this by modifying the binding decorations in the shader.
