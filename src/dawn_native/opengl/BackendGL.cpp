...
namespace dawn_native { namespace opengl {
    ...
    class Adapter : public AdapterBase {
      ...
      private:
        ResultOrError<DeviceBase*> CreateDeviceImpl() override {
            // There is no limit on the number of device created from this adapter because they can
    ...
    ResultOrError<std::vector<std::unique_ptr<AdapterBase>>> Backend::DiscoverAdapters(
        ...
        // static variables to store the pointers to OpenGL procs. Also if we could create multiple
        // adapters we would need to figure out what to do about MakeCurrent.
        if (mCreatedAdapter) {
