...
namespace dawn_native { namespace opengl {
    ...
    ResultOrError<std::string> ShaderModule::TranslateToGLSL(const char* entryPointName,
        ...
        if (GetDevice()->IsToggleEnabled(Toggle::UseTintGenerator)) {
            ...
            DAWN_TRY(ValidateSpirv(GetDevice(), spirv,
            ...
            // To workaround this, we must inject a dummy sampler which can be used to form a
            // sampler2D at the call-site of texelFetch as necessary.
            spirv_cross::VariableID dummySamplerId =
                compiler.build_dummy_sampler_for_combined_images();

            // Extract bindings names so that it can be used to get its location in program.
            // Now translate the separate sampler / textures into combined ones and store their
            // info. We need to do this before removing the set and binding decorations.
            ...
            // for the implicit pipeline layout and pipeline/layout validation, but bindingInfo is
            // set to mGLEntryPoints which is the SPIRV-Cross reflection. Tint reflects bindings
            // used more precisely than SPIRV-Cross so some bindings in bindingInfo might not exist
            // in the layout and querying the layout for them would cause an ASSERT. That's why we
            // defensively check that bindings are in the layout before modifying them. This slight
            // hack is ok because in the long term we will use Tint to produce GLSL.
            for (BindGroupIndex group : IterateBitSet(layout->GetBindGroupLayoutsMask())) {
                for (const auto& it : bindingInfo[group]) {
                    ...
                    // Remove the name of the base type. This works around an issue where if the
                    // SPIRV has two uniform/storage interface variables that point to the same base
                    // type, then SPIRV-Cross would emit two bindings with type names that conflict:
                    //
