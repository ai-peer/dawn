...
namespace dawn_native { namespace opengl {
    ...
    MaybeError Texture::ClearTexture(GLint baseMipLevel,
        ...
        if (GetFormat().isRenderable) {
            ...
            descriptor.size = rowPitch * (GetSize().height / GetFormat().blockHeight) * GetFormat().blockByteSize;
            ...
            // Call release here to prevent memory leak since CreateBuffer will up the ref count to 1,
            // then assigning to Ref<Buffer> ups the ref count to 2. Release will reduce the ref count
            // and ensure it to reach 0 when out of use.
            srcBuffer->Release();
            ...
            for (GLint level = baseMipLevel; level < baseMipLevel + levelCount; ++level) {
                ...
                gl.PixelStorei(GL_UNPACK_ROW_LENGTH, GetSize().width);
                ...
                switch (GetDimension()) {
                    case dawn::TextureDimension::e2D:
                        if (layerCount > 1) {
                            // TODO(natlee@microsoft.com): This will break when layerCount is 
                            // greater than 1, because buffer is sized only for one layer.
                            gl.TexSubImage3D(GetGLTarget(), level, 0, 0, layerCount, size.width,
