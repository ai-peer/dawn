...
namespace dawn_native { namespace opengl {
    ...
    void RenderPipeline::CreateVAOForVertexInput(const VertexInputDescriptor* vertexInput) {
        ...
        device->gl.GenVertexArrays(1, &mVertexArrayObject);
        device->gl.BindVertexArray(mVertexArrayObject);

        for (uint32_t location : IterateBitSet(GetAttributesSetMask())) {
            const auto& attribute = GetAttribute(location);
            device->gl.EnableVertexAttribArray(location);
            ...
            if (input.stride == 0) {
                ...
                device->gl.VertexAttribDivisor(location, 0xffffffff);
            } else {
                switch (input.stepMode) {
                    case dawn::InputStepMode::Vertex:
                        break;
                    case dawn::InputStepMode::Instance:
                        device->gl.VertexAttribDivisor(location, 1);
                        break;
                    default:
                        UNREACHABLE();
                }
