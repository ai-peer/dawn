...
namespace dawn_native { namespace opengl {
    ...
    MaybeError Queue::WriteTextureImpl(const TextureCopyView& destination,
        ...
        Extent3D extent = texture->GetMipLevelPhysicalSize(destination.mipLevel);
        if (extent.width == writeSizePixel.width && extent.height == writeSizePixel.height) {
            const_cast<Texture*>(texture)->SetIsSubresourceContentInitialized(
                true, SubresourceRange(Aspect::Color, {destination.origin.z, writeSizePixel.depth},
