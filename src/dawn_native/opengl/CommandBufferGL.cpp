...
namespace dawn_native { namespace opengl {
    ...
    void CommandBuffer::ExecuteRenderPass(BeginRenderPassCmd* renderPassCmd) {
        GLuint fbo = 0;
        ...
        // Create the framebuffer used for this render pass and calls the correct glDrawBuffers
        {
            ...
            for (uint32_t i : IterateBitSet(renderPassCmd->colorAttachmentsSet)) {
                TextureViewBase* textureView = renderPassCmd->colorAttachments[i].view.Get();
            ...
            if (HasDepthStencilAttachmentInRenderPass(renderPassCmd)) {
                TextureViewBase* textureView = renderPassCmd->depthStencilAttachment.view.Get();
                GLuint texture = ToBackend(textureView->GetTexture())->GetHandle();
                dawn::TextureFormat format = textureView->GetTexture()->GetFormat();
        ...
        {
            for (uint32_t i : IterateBitSet(renderPassCmd->colorAttachmentsSet)) {
                const auto& attachmentInfo = renderPassCmd->colorAttachments[i];
            ...
            if (HasDepthStencilAttachmentInRenderPass(renderPassCmd)) {
                const auto& attachmentInfo = renderPassCmd->depthStencilAttachment;
        ...
        glViewport(0, 0, renderPassCmd->width, renderPassCmd->height);
        glScissor(0, 0, renderPassCmd->width, renderPassCmd->height);
