            ...
            for (ColorAttachmentIndex i :
                ...
                if (textureView->GetTexture()->GetArrayLayers() == 1) {
                    ...
                    gl.FramebufferTexture2D(GL_DRAW_FRAMEBUFFER,
                                            GL_COLOR_ATTACHMENT0 + static_cast<uint8_t>(i), target,
                                            texture, textureView->GetBaseMipLevel());
                } else {
                    gl.FramebufferTextureLayer(
                        GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + static_cast<uint8_t>(i),
                        texture, textureView->GetBaseMipLevel(), textureView->GetBaseArrayLayer());
                }
                drawBuffers[i] = GL_COLOR_ATTACHMENT0 + static_cast<uint8_t>(i);
                attachmentCount = i;
                attachmentCount++;
            }
