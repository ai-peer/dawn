        ...
        while (mCommands.NextCommandId(&type)) {
            switch (type) {
                    ...
                    switch (texture->GetDimension()) {
                        case dawn::TextureDimension::e2D:
                            if (texture->GetArrayLayers() > 1) {
                                glTexSubImage3D(
                                    target, dst.level, dst.origin.x, dst.origin.y, dst.slice, copySize.width,
                                    copySize.height, 1, format.format, format.type,
                                    reinterpret_cast<void*>(static_cast<uintptr_t>(src.offset)));
                            } else {
                                glTexSubImage2D(
                                    target, dst.level, dst.origin.x, dst.origin.y, copySize.width, copySize.height,
                ...
                case Command::CopyTextureToBuffer: {
                    ...
                    glReadPixels(src.origin.x, src.origin.y, copySize.width, copySize.height, format.format, format.type,
                                 offset);
