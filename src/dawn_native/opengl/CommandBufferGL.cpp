        ...
        auto TransitionForPass = [](const PassResourceUsage& usages) {
            for (size_t i = 0; i < usages.textures.size(); i++) {
                ...
                texture->EnsureSubresourceContentInitialized(
                    texture->GetHandle(), 0, texture->GetNumMipLevels(), 0,
            ...
            switch (type) {
                ...
                case Command::CopyBufferToTexture: {
                    ...
                    if (IsCompleteSubresourceCopiedTo(texture, copySize, dst.level)) {
                        ...
                        texture->EnsureSubresourceContentInitialized(
                            texture->GetHandle(), dst.level, 1, dst.slice, 1, dst.origin,
                            dst.texture->GetSize());
                    }
                ...
                case Command::CopyTextureToBuffer: {
                    ...
                    texture->EnsureSubresourceContentInitialized(
                        texture->GetHandle(), src.level, 1, src.slice, 1, src.origin, copySize);
                ...
                case Command::CopyTextureToTexture: {
                    ...
                    srcTexture->EnsureSubresourceContentInitialized(
                        srcTexture->GetHandle(), src.level, 1, src.slice, 1, src.origin, copySize);
                    if (IsCompleteSubresourceCopiedTo(dstTexture, copySize, dst.level)) {
                        ...
                        dstTexture->EnsureSubresourceContentInitialized(
                            dstTexture->GetHandle(), dst.level, 1, dst.slice, 1, dst.origin,
                            dst.texture->GetSize());
                    }
        ...
        // Clear framebuffer attachments as needed
        {
            for (uint32_t i : IterateBitSet(renderPass->colorAttachmentsSet)) {
                ...
                if (attachmentInfo.loadOp == dawn::LoadOp::Clear) {
                    ...
                    texture->EnsureSubresourceContentInitialized(
                        texture->GetHandle(), view->GetBaseMipLevel(), view->GetLevelCount(),
            ...
            if (renderPass->hasDepthStencilAttachment) {
                ...
                if ((texture->GetFormat().HasDepth() &&
                    ...
                    texture->EnsureSubresourceContentInitialized(
                        texture->GetGLTarget(), attachmentInfo.view->GetBaseMipLevel(),
