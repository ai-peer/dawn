...
namespace dawn_native {
    ...
    // Defines the memory properties used by the resource allocator.
    enum class AllocatorHeapType {
        Readback,  // CPU visible, write-back pages.
        Upload,    // CPU visible, write-through pages.
        None,      // GPU visbile.
    };

    // Depending on the expected resource usage pattern, optimal allocators may be used.
    enum class AllocatorResourceUsage {
        // For uploading resources from CPU to GPU, where resources are are expected to be modified
        // frequently. Used for immediate buffer uploads (ie. SetSubData).
        Dynamic,

        // For uploading resources from CPU to GPU, where resources must be CPU-visible but not
        // "dynamic".
        // Used for mapped buffer uploads (ie. Map[Read|Write]).
        Upload,

        // For reading back resources from GPU to CPU, where resources are expected to written by
        // GPU then immediately read by CPU.
        // Used for buffer downloads.
        Readback,
    ...
    // Creates a pool of resources on-demand using a sub-allocation allocator.
    // Block: allocator block type.
    // ResourceAllocator: allocator for resource heap.
    // BlockAllocator: Sub-allocation allocator of blocks.
    // minBlockSize: Smallest block size sub-allocated, as specified by back-end resource alloactor.
    ...
    // Allocator which could sub-allocate from a pool or not.
