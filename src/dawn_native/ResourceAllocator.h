...
namespace dawn_native {
    ...
    class Block {
      public:
        Block() : mSize(0), mOffset(0), mAlignment(0) {
        }
        Block(size_t size, size_t offset, size_t alignment)
            : mSize(size), mOffset(offset), mAlignment(alignment){};
    ...
    struct BuddyBlock : public HeapSubAllocationBlock {
        BuddyBlock(size_t size, size_t offset, size_t alignment)
            : HeapSubAllocationBlock(size, offset, alignment), mState(BlockState::Free) {
        }
    ...
    class ConditionalAllocator {
      public:
        // maxBlockSize: Max block size or memory range of allocator.
        // minBlockSize: Min size block or alignment of the offset.
        template <typename... ResourceAllocatorArgs>
        ConditionalAllocator(size_t maxBlockSize,
                             size_t minBlockSize,
                             size_t resourceSize,
                             ResourceAllocatorArgs&&... resourceArgs);
