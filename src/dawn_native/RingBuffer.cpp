...
namespace dawn_native {
    ...
    RingBuffer::RingBuffer(DeviceBase* device) : mDevice(device) {
    }

    MaybeError RingBuffer::Initialize(size_t size) {
        mBufferSize = size;
        StagingBufferBase* stagingBuffer = nullptr;
        DAWN_TRY_ASSIGN(stagingBuffer, mDevice->CreateStagingBuffer(size));
        DAWN_TRY(stagingBuffer->Initialize());
        mStagingBuffer = std::unique_ptr<StagingBufferBase>(stagingBuffer);
        return {};
    }
