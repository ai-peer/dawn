...
namespace dawn_native {
    ...
    const char* TogglesInfo::ToggleEnumToName(Toggle toggle) {
        ASSERT(toggle != Toggle::InvalidEnum);
    ...
    std::vector<const char*> TogglesInfo::GetEnabledToggleNamesFromTogglesSet(
        TogglesSet togglesSet) {
        std::vector<const char*> togglesNameInUse(togglesSet.toggleBitset.count());

        uint32_t index = 0;
        for (uint32_t i : IterateBitSet(togglesSet.toggleBitset)) {
