...
namespace dawn_native {
    ...
    CommandBlock* CommandBlockAllocator::Allocate(size_t minimumSize, CommandBlock* previousBlock) {
        ...
        if (block == nullptr) {
            static constexpr size_t kMaxBlockGrowthSize = 64 * 1024;
            mLastBlockSize =
                std::max(minimumSize, std::min(mLastBlockSize * 2, kMaxBlockGrowthSize));

            void* ptr = malloc(mLastBlockSize + sizeof(CommandBlock));
            block = new (ptr) CommandBlock(mLastBlockSize);
        }
