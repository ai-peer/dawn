...
namespace dawn_native {
    ...
    namespace {
        ...
        static const char sConvertTimestampsToNanoseconds[] = R"(
            ...
            fn main() -> void {
                ...
                if (inputIndex >= params.count + params.inputByteOffset / sizeofTimestamp) {
                    return;
                }

                var timestamp : Timestamp = input.t[inputIndex];

                # Skip the unavailable value at the slot.
    ...
    void EncodeConvertTimestampsToNanoseconds(CommandEncoder* encoder,
        ...
        pass->Dispatch(static_cast<uint32_t>(ceil(input->GetSize() / sizeof(uint64_t) / 8.0f)));
        pass->EndPass();
    }
