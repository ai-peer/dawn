...
namespace dawn_native {
    ...
    namespace {
        ...
        static const char sConvertTimestampsToNanoseconds[] = R"(
            ...
            [[set(0), binding(0)]]
                var<storage_buffer> destination : [[access(read_write)]] TimestampArr;
            [[set(0), binding(1)]]
            ...
            fn main() -> void {
                ...
                var timestamp : Timestamp = destination.t[index];

                # Return 0 for the unavailable value.
                if (availability.v[index] == 0u) {
                    destination.t[index].low = 0u;
                    destination.t[index].high = 0u;
                    return;
                }

                # Multiply the values in destination by the period.
                ...
                if (timestamp.low <= u32(f32(0xFFFFFFFFu) / period)) {
                    destination.t[index].low = u32(round(f32(timestamp.low) * period));
                } else {
                    ...
                    destination.t[index].low = result;
                }
                ...
                destination.t[index].high = u32(round(f32(timestamp.high) * period)) + w;
            }
        )";
    ...
    void EncodeConvertTimestampsToNanoseconds(CommandEncoder* encoder,
        ...
        bindGroupEntries[0].buffer = destination;
        bindGroupEntries[0].size = destination->GetSize();
        ...
        pass->Dispatch(static_cast<uint32_t>((destination->GetSize() / sizeof(uint64_t) + 7) / 8));
        pass->EndPass();
    }
