            ...
            fn main() -> void {
                ...
                for (var i : u32 = 0u; i <= index; i = i + 1) {
                    if (input.t[i].x != 0 || input.t[i].y != 0) {
                        baseline.x = input.t[i].x;
                        baseline.y = input.t[i].y;
                        break;
                    }
                }
                ...
                for (var i : u32 = 1u; i <= index; i = i + 1) {
                    # Record the previous timestamp. For example,
                    # if the src data is [0, t0, 0, t1], the previous timestamp should be t0.
                    if (previous.x == 0 && previous.y == 0 && input.t[i].x != 0 && input.t[i].y != 0 ) {
                        previous.x = input.t[i].x;
                        previous.y = input.t[i].y;
                    }

                    # Skip the zero value checking which is not a timestamp.
                    if (input.t[i].x == 0 && input.t[i].y == 0) { continue; }

                    if (input.t[i].y < previous.y) {
                        return;
                    }
                    if (input.t[i].y == previous.y && input.t[i].x < previous.x) {
                        return;
                    }

                    # Record the current timestamp as the previous.
                    previous.x = input.t[i].x;
                    previous.y = input.t[i].y;
                }
