...
namespace dawn_native {
    ...
    namespace {
        ...
        static const char sTimestampCompute[] = R"(
            ...
            fn main() -> void {
                ...
                var current: vec2<u32> = input.t[index];

                # Skip the zero value which is not a timestamp.
                if (current.x == 0u && current.y == 0u) { return; }
                ...
                # Multiply input values by the period
                var w: u32 = 0u;
                ...
                if (current.x <= u32(f32(0xFFFFFFFFu) / period)) {
                    output.t[index].x = u32(round(f32(current.x) * period));
                } else {
                ...
                output.t[index].y = u32(round(f32(current.y) * period)) + w;
            }
        )";
    ...
    void DoTimestampCompute(CommandEncoder* encoder,
        ...
        bindGroupEntries[1].size = output->GetSize();
