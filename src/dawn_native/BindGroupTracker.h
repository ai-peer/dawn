...
namespace dawn_native {
    ...
    // |BindGroup| is a template parameter so a backend may provide it's backend-specific
    // type or handle.
    // |DynamicOffset| is a template parameter as a temporary workaround for Vulkan where
    // we don't support 64-bit dynamic offsets yet.
    template <typename BindGroup, bool CanInheritBindGroups, typename DynamicOffset = uint64_t>
    class BindGroupTrackerBase {
      public:
        using Base = BindGroupTrackerBase<BindGroup, CanInheritBindGroups, DynamicOffset>;
