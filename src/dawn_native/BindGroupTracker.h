...
namespace dawn_native {
    ...
    // |DynamicOffset| is a template parameter as a temporary workaround for Vulkan where
    // we don't support uint64_t dynamic offsets yet.
    ...
    class BindGroupTrackerBase {
      public:
        void OnSetBindGroup(uint32_t index,
            ...
            if (mBindGroupLayoutsMask[index]) {
                ...
                if (mBindGroups[index] != bindGroup) {
                    ...
                    mDirtyOrDynamicBindGroups.set(index);
                }
        ...
        void OnSetPipeline(PipelineBase* pipeline) {
            ...
            if (CanInheritBindGroups && mLastAppliedPipelineLayout != nullptr) {
                ...
                mDirtyOrDynamicBindGroups |= dirtiedGroups;

                // Clear any bind groups not in the mask.
                mDirtyBindGroups &= mBindGroupLayoutsMask;
                mDirtyOrDynamicBindGroups &= mBindGroupLayoutsMask;
            } else {
                mDirtyBindGroups = mBindGroupLayoutsMask;
                mDirtyOrDynamicBindGroups = mBindGroupLayoutsMask;
            }
      ...
      protected:
        ...
        std::bitset<kMaxBindGroups> mDirtyOrDynamicBindGroups = 0;
