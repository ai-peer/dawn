...
namespace dawn_native {
    ...
    // returning the starting offset whose size is guarenteed to be greater than or equal to the
    ...
    // the size of the block be used to satisfy the request. The first level (index=0) represents
    ...
    class BuddyAllocator {
      public:
        ...
        // Required methods.
        uint64_t Allocate(uint64_t allocationSize, uint64_t alignment = 0);
        void Deallocate(uint64_t offset);

        // For testing purposes only.
        uint64_t GetNumOfFreeBlocks() const;

      private:
        size_t ComputeLevelFromBlockSize(uint64_t blockSize) const;
        ...
        struct BuddyBlock {
            ...
            uint64_t mOffset = INVALID_OFFSET;
            uint64_t mSize = 0;
