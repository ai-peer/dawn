        ...
        for (auto usedSlotVertex : IterateBitSet(vertexBufferSlotsUsedAsVertexBuffer)) {
            ...
            if (arrayStride == 0) {
                DAWN_INVALID_IF(vertexBuffer.usedBytesInStride > bufferSize,
                                "Stride (%u) of slot %u does not fit in vertex buffer size (%u).",
                                vertexBuffer.usedBytesInStride, usedSlotVertex, bufferSize);
            } else {
                ...
                DAWN_INVALID_IF(
                    ...
                    "with stride (%u) of slot %u.",
                    firstVertex, vertexCount, bufferSize, arrayStride, usedSlotVertex);
            }
        }
    ...
    MaybeError CommandBufferStateTracker::ValidateBufferInRangeForInstanceBuffer(
        ...
        for (auto usedSlotInstance : IterateBitSet(vertexBufferSlotsUsedAsInstanceBuffer)) {
            ...
            if (arrayStride == 0) {
                DAWN_INVALID_IF(vertexBuffer.usedBytesInStride > bufferSize,
                                "Stride (%u) of slot %u does not fit in vertex buffer size (%u).",
                                vertexBuffer.usedBytesInStride, usedSlotInstance, bufferSize);
            } else {
                ...
                DAWN_INVALID_IF(
                    ...
                    "with stride (%u) of slot %u.",
                    firstInstance, instanceCount, bufferSize, arrayStride, usedSlotInstance);
            }
        }
