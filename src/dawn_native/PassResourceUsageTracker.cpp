...
namespace dawn_native {
    ...
    void ComputePassResourceUsageTracker::AddUnusedBindGroup(BindGroupBase* group) {
        for (BindingIndex index{0}; index < group->GetLayout()->GetBindingCount(); ++index) {
            ...
            switch (bindingInfo.bindingType) {
                case BindingInfoType::Buffer: {
                    mUsage.unusedBuffers.insert(group->GetBindingAsBufferBinding(index).buffer);
                    break;
                }

                case BindingInfoType::Texture: {
                    mUsage.unusedTextures.insert(
