...
namespace dawn_native {
    ...
    // Manages a ref to the underlying buffer.
    struct CachedBlob {
    ...
    class PersistentCache {
      public:
        ...
        // Encapsulates load/store operations into a single call.
        // If the load was successfull, the caller owned blob is returned back.
        // Else, the creation call-back |createFn| gets invoked with a call-back
        // |doCache| to store the newly created blob.
        ResultOrError<CachedBlob> LoadFromCacheOrCreate(
            const std::string& keyStr,
            std::function<MaybeError(DoCache doCache)> createFn) {
            PersistentCacheKey key(keyStr.begin(), keyStr.end());
            CachedBlob blob = {};
            ...
            // Cannot expect the |buffer| blob to be CachedBlob.
            // TODO: do partial application here?
            DoCache doCacheFn = [&](const void* buffer, size_t size) {
                return storeData(key, buffer, size);
            };
        ...
        static PersistentCacheKey CreateKey(const std::string& keyStr);

      protected:
        DeviceBase* mDevice = nullptr;
        dawn_platform::CachingInterface* mCachingInterface;
    };
}  // namespace dawn_native
