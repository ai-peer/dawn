...
namespace dawn_native {
    ...
    RenderPipelineBase* DeviceBase::GetCachedRenderPipeline(
    ...
    ResultOrError<Ref<RenderPipelineBase>> DeviceBase::CreateRenderPipeline(
        ...
        RenderPipelineBase* cachedRenderPipeline =
            GetCachedRenderPipeline(uninitializedRenderPipeline.Get());
        if (cachedRenderPipeline != nullptr) {
            Ref<RenderPipelineBase> result = cachedRenderPipeline;
            return result;
        }
    ...
    MaybeError DeviceBase::CreateRenderPipelineAsync(const RenderPipelineDescriptor* descriptor,
        ...
        if (cachedRenderPipeline != nullptr) {
            callback(WGPUCreatePipelineAsyncStatus_Success,
                     reinterpret_cast<WGPURenderPipeline>(cachedRenderPipeline), "", userdata);
        } else {
    ...
    Ref<RenderPipelineBase> DeviceBase::CreateUninitializedRenderPipeline(
        const RenderPipelineDescriptor* descriptor) {
        Ref<RenderPipelineBase> uninitializedRenderPipeline =
            CreateUninitializedRenderPipelineImpl(descriptor);
        size_t blueprintHash = uninitializedRenderPipeline->ComputeContentHash();
        uninitializedRenderPipeline->SetContentHash(blueprintHash);
        return uninitializedRenderPipeline;
    }
