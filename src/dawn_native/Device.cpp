...
namespace dawn_native {
    ...
    void DeviceBase::PopErrorScope(dawn::ErrorCallback callback, void* userdata) {
        if (DAWN_UNLIKELY(mCurrentErrorScope.Get() == &mRootErrorScope)) {
            ConsumedError(DAWN_VALIDATION_ERROR("No error scopes to pop"));
            return;
        }
