    ...
    bool IsTextureSubresourceInitialized(WGPUTexture texture,
        ...
        switch (aspect) {
            ...
            case WGPUTextureAspect_DepthOnly:
                ASSERT(textureBase->GetFormat().aspectMask[TextureAspect::Depth]);
                aspectMask = 1u << static_cast<uint8_t>(TextureAspect::Depth);
                break;
            case WGPUTextureAspect_StencilOnly:
                ASSERT(textureBase->GetFormat().aspectMask[TextureAspect::Stencil]);
                aspectMask = 1u << static_cast<uint8_t>(TextureAspect::Stencil);
                break;
            default:
                UNREACHABLE();
                break;
