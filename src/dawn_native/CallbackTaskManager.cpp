...
namespace dawn_native {
    ...
    std::vector<std::unique_ptr<CallbackTask>>
    CallbackTaskManager::FetchAndClearAllCallbackTasks() {
        std::lock_guard<std::mutex> lock(mCallbackTaskQueueMutex);

        std::vector<std::unique_ptr<CallbackTask>> allTasks(mCallbackTaskQueue.size());
        for (size_t i = 0; i < mCallbackTaskQueue.size(); ++i) {
            allTasks[i] = std::move(mCallbackTaskQueue[i]);
        }

        mCallbackTaskQueue.clear();
        return allTasks;
    }
