...
namespace dawn_native {
    ...
    class CommandIterator {
      ...
      private:
        static constexpr uint32_t EndOfBlock = UINT_MAX;  // std::numeric_limits<uint32_t>::max();
        static constexpr uint32_t AdditionalData =
    ...
    class CommandAllocator {
      ...
      private:
        ...
        uint8_t* Allocate(uint32_t commandId, size_t commandSize, size_t commandAlignment) {
            ...
            // When there is not enough space, we signal the EndOfBlock, so that the iterator knows
            // to move to the next one. EndOfBlock on the last block means the end of the commands.
            uint32_t* idAlloc = reinterpret_cast<uint32_t*>(mCurrentPtr);
            *idAlloc = EndOfBlock;
        ...
        uint8_t* AllocateData(size_t commandSize, size_t commandAlignment) {
