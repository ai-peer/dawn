    ...
    void CreateReadyPipelineTask::Finish() {
        ...
        if (mCreateReadyComputePipelineCallback != nullptr) {
            mCreateReadyComputePipelineCallback(
                WGPUCreateReadyPipelineStatus_Success,
                reinterpret_cast<WGPUComputePipeline>(
                    reinterpret_cast<ComputePipelineBase*>(mPipeline)),
                "", mUserData);
        } else {
            ASSERT(mCreateReadyRenderPipelineCallback != nullptr);
            mCreateReadyRenderPipelineCallback(
                WGPUCreateReadyPipelineStatus_Success,
                reinterpret_cast<WGPURenderPipeline>(
                    reinterpret_cast<RenderPipelineBase*>(mPipeline)),
                "", mUserData);
        }
    }
