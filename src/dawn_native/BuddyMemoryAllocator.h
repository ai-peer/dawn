...
namespace dawn_native {
    ...
    class BuddyMemoryAllocator {
      public:
        ...
        ResultOrError<ResourceMemoryAllocationBase> Allocate(uint64_t allocationSize,
                                                             uint64_t alignment,
                                                             int memoryFlags = 0);
        void Deallocate(const ResourceMemoryAllocationBase& allocation);
      ...
      private:
        ...
        struct TrackedSubAllocations {
            size_t refcount = 0;
            std::unique_ptr<MemoryBase> mMemoryAllocation;
        };
