...
namespace dawn_native {
    ...
    MaybeError ValidateExternalTexturePlane(const TextureViewBase* textureView,
        ...
        if (textureView->GetTexture()->GetTextureState() == TextureBase::TextureState::Destroyed) {
    ...
    MaybeError ValidateExternalTextureDescriptor(const DeviceBase* device,
        ...
        if (!descriptor->plane0) {
            return DAWN_VALIDATION_ERROR(
                "The first plane in the external texture descriptor is null.");
        }
        ...
        if (format->IsMultiPlanar()) {
            return DAWN_VALIDATION_ERROR("Multiplanar formats are not yet supported.");
        } else {
    ...
    std::array<Ref<TextureViewBase>, kMaxExternalTexturePlanes>
