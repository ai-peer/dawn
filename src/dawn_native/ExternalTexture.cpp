...
namespace dawn_native {
    MaybeError ValidateExternalTextureDescriptor(const DeviceBase* device,
        ...
        if (!descriptor->externalTextureView) {
            return DAWN_VALIDATION_ERROR(
                "The external texture descriptor contains a null texture view.");
        }

        if (descriptor->externalTextureView->GetTexture()->GetTextureState() ==
    ...
    ExternalTextureBase::ExternalTextureBase(DeviceBase* device,
                                             const ExternalTextureDescriptor* descriptor)
        : ObjectBase(device), textureView(descriptor->externalTextureView) {
    }
    ...
    TextureViewBase* ExternalTextureBase::GetTextureView() const {
        return textureView;
    }

    void ExternalTextureBase::Destroy() {
