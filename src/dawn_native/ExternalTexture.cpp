...
namespace dawn_native {
    ...
    MaybeError ValidateExternalTexturePlane(const TextureViewBase* textureView,
        ...
        DAWN_INVALID_IF(
            ...
            "The external texture plane (%s) doesn't have the required usage (%s).", textureView,
            wgpu::TextureUsage::TextureBinding);

        DAWN_INVALID_IF(textureView->GetDimension() != wgpu::TextureViewDimension::e2D,
                        "The external texture plane (%s) is a view that's not 2D.", textureView);

        DAWN_INVALID_IF(textureView->GetLevelCount() > 1,
                        "The external texture plane (%s) has more than on mip level.", textureView);

        DAWN_INVALID_IF(textureView->GetTexture()->GetSampleCount() != 1,
                        "The external texture plane (%s) is multisampled.", textureView);
    ...
    MaybeError ValidateExternalTextureDescriptor(const DeviceBase* device,
        ...
        switch (descriptor->format) {
            ...
            case wgpu::TextureFormat::RGBA16Float:
                ...
                break;
            default:
                DAWN_INVALID_IF(true, "Format (%s) is not a supported external texture format",
                                descriptor->format);
        }
