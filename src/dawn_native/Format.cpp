...
namespace dawn_native {
    ...
    static_assert(decltype(std::declval<Format>().aspectMask)().size() == kTextureAspectCount, "");
    ...
    bool Format::IsColor() const {
        return aspectMask == (1u << static_cast<uint8_t>(TextureAspect::Color));
    }

    bool Format::HasDepth() const {
        return aspectMask[TextureAspect::Depth];
    }

    bool Format::HasStencil() const {
        return aspectMask[TextureAspect::Stencil];
    }

    bool Format::HasDepthOrStencil() const {
        return aspectMask[TextureAspect::Depth] || aspectMask[TextureAspect::Stencil];
    }
    ...
    FormatTable BuildFormatTable(const DeviceBase* device) {
        ...
        auto AddColorFormat = [&AddFormat](wgpu::TextureFormat format, bool renderable,
            ...
            internalFormat.aspectMask = 1u << static_cast<uint8_t>(TextureAspect::Color);
        ...
        auto AddDepthStencilFormat = [&AddFormat](wgpu::TextureFormat format, uint32_t aspectMask,
                                                  uint32_t byteSize) {
            Format internalFormat;
            internalFormat.format = format;
        ...
        auto AddDepthFormat = [&AddFormat](wgpu::TextureFormat format, uint32_t byteSize,
            ...
            internalFormat.isSupported = true;
            internalFormat.supportsStorageUsage = false;
            internalFormat.aspectMask = 1u << static_cast<uint8_t>(TextureAspect::Depth);
            internalFormat.type = type;
            internalFormat.blockByteSize = byteSize;
            internalFormat.blockWidth = 1;
            internalFormat.blockHeight = 1;
        ...
        AddDepthStencilFormat(wgpu::TextureFormat::Depth24Plus, 1u << static_cast<uint8_t>(TextureAspect::Depth), 4);
        ...
        AddDepthStencilFormat(wgpu::TextureFormat::Depth24PlusStencil8,
                              (1u << static_cast<uint8_t>(TextureAspect::Depth)) |
                              (1u << static_cast<uint8_t>(TextureAspect::Stencil)), 4);
