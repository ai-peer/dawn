...
namespace dawn_native {
    ...
    AspectInfo GetSinglePlaneAspectInfo(wgpu::TextureFormat format, FormatTable& table) {
        const size_t index = ComputeFormatIndex(format);
        ...
        auto AddFormat = [&table, &formatsSet](Format format) {
            ...
            if (hasMultipleAspects) {
                for (Aspect aspect : IterateEnumMask(format.aspects)) {
                    const size_t aspectIndex = GetAspectIndex(aspect);
                    ASSERT(aspectIndex < kMaxPlanesPerFormat);

                    format.aspectInfo[aspectIndex] =
                        GetSinglePlaneAspectInfo(format.aspectInfo[aspectIndex].format, table);
                    ASSERT(formatsSet[aspectIndex]);

                    // Parent format should never be the same as the planar format.
                    ASSERT(format.format != format.aspectInfo[aspectIndex].format);
                }
            }
