...
namespace dawn_native {
    ...
    const AspectInfo& Format::GetAspectInfo(wgpu::TextureAspect aspect) const {
        ASSERT(HasDepth() || aspect != wgpu::TextureAspect::DepthOnly);
        ASSERT(HasStencil() || aspect != wgpu::TextureAspect::StencilOnly);

        // The stencil aspect is the only aspec that's not the first aspect. Since it is alwaus the
