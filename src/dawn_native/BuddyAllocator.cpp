...
namespace dawn_native {
    ...
    uint64_t BuddyAllocator::GetNumOfFreeBlocks() const {
    ...
    size_t BuddyAllocator::ComputeLevelFromBlockSize(uint64_t blockSize) const {
    ...
    uint64_t BuddyAllocator::GetNextFreeAlignedBlock(size_t allocationBlockLevel,
        ...
        for (size_t currLevel = allocationBlockLevel; currLevel >= 0; currLevel--) {
            ...
            if (freeBlock && (alignment == 0 || freeBlock->mOffset % alignment == 0)) {
                return currLevel;
            }
    ...
    uint64_t BuddyAllocator::Allocate(uint64_t allocationSize, uint64_t alignment) {
        // Allocation size must be non-zero nor larger than max block size.
