...
namespace dawn_native {
    ...
    MaybeError ValidateBufferDescriptor(DeviceBase*, const BufferDescriptor* descriptor) {
        ...
        DAWN_INVALID_IF(
            ...
            "Buffer usages (%s) contains %s but are not a subset of %s.", usage,
    ...
    MaybeError BufferBase::ValidateCanUseOnQueueNow() const {
        ...
        switch (mState) {
            case BufferState::Destroyed:
                DAWN_INVALID_IF(true, "%s used in submit while destroyed.", this);
            case BufferState::Mapped:
            case BufferState::MappedAtCreation:
                DAWN_INVALID_IF(true, "%s used in submit while mapped.", this);
    ...
    void BufferBase::APIMapAsync(wgpu::MapMode mode,
        ...
        if (GetDevice()->ConsumedError(ValidateMapAsync(mode, offset, size, &status),
                                       "calling %s.MapAsync(%s, %u, %u, ...)", mode, offset,
                                       size)) {
            if (callback) {
