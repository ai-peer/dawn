...
namespace dawn_native {
    ...
    void BufferBase::Unmap() {
        ...
        if (mStagingBuffer != nullptr) {
            GetDevice()->ConsumedError(GetDevice()->CopyFromStagingToBuffer(
                mStagingBuffer.release(), 0, this, 0, GetSize()));
        } else {
    ...
    MaybeError BufferBase::ValidateUnmap() const {
        ...
        switch (mState) {
            case BufferState::Mapped:
                // A buffer may be in the Mapped state if it was created with CreateBufferMapped even
                // if it did not have a mappable usage.
                return {};
            case BufferState::Unmapped:
