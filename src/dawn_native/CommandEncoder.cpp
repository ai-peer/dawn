...
namespace dawn_native {
    ...
    namespace {
        ...
        MaybeError ValidateB2BCopySizeAlignment(uint32_t dataSize, uint32_t srcOffset, uint32_t dstOffset) {
            ...
            if (srcOffset % 4 != 0 || dstOffset % 4 != 0) {
                return DAWN_VALIDATION_ERROR("Source offset and destination offset must be multiples of 4 bytes");
            }
        ...
        while (mIterator.NextCommandId(&type)) {
            switch (type) {
                ...
                case Command::CopyBufferToBuffer: {
                    ...
                    DAWN_TRY(ValidateB2BCopySizeAlignment(copy->size, copy->source.offset, copy->destination.offset));
