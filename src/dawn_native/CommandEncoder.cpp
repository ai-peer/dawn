...
namespace dawn_native {
    ...
    void CommandEncoderBase::CopyBufferToBuffer(BufferBase* source,
                            ...
                            uint32_t size) {
        return mBuilder->CopyBufferToBuffer(source, sourceOffset, destination, destinationOffset, size);
    }
    ...
    MaybeError CommandEncoderBase::ValidateFinish() {
        ...
        // This could have been triggered if a validation error happened in the builder or its
        // encoders before "ValidateGetResult".
        if (mGotError) {
    ...
    // static
    void CommandEncoderBase::HandleBuilderError(dawnBuilderErrorStatus status, const char* message, dawnCallbackUserdata userdata1, dawnCallbackUserdata) {
        CommandEncoderBase* self = reinterpret_cast<CommandEncoderBase*>(static_cast<uintptr_t>(userdata1));
