...
namespace dawn_native {
    ...
    void CommandEncoder::AddQueryStatesMap(QueryStatesMap& queryStatesMap) {
        for (auto srcIter : queryStatesMap) {
            ...
            QueryStatesMap::iterator dstIter = mQueryStatesMap.find(srcIter.first);
            if (dstIter != mQueryStatesMap.end()) {
                std::vector<QueryState>& queryStates = srcIter.second;
                for (uint32_t i = 0; i < queryStates.size(); i++) {
                    // Only update the used queries.
                    if (queryStates[i] == QueryState::Available) {
                        dstIter->second[i] = QueryState::Available;
                    }
                }
            } else {
                mQueryStatesMap.insert({srcIter.first, std::move(srcIter.second)});
            }
        }
    }
