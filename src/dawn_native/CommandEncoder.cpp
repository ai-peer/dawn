...
namespace dawn_native {
    ...
    namespace {
        ...
        MaybeError CheckArrayLayersAndLevelCountForAttachment(const TextureViewBase* attachment) {
        ...
        MaybeError CheckOrSetAttachmentSize(const TextureViewBase* attachment, uint32_t* width,
                                            uint32_t* height) {
        ...
        MaybeError ValidateRenderPassColorAttachment(
            ...
            if (colorAttachment->attachment == nullptr) {
                return DAWN_VALIDATION_ERROR("Texture view cannot be nullptr");
            }
            ...
            if (!IsColorRenderableTextureFormat(attachment->GetFormat())) {
                return DAWN_VALIDATION_ERROR("The format of the texture view used as color"
                                             "attachment is not color renderable");
            }

            DAWN_TRY(CheckArrayLayersAndLevelCountForAttachment(attachment));
            DAWN_TRY(CheckOrSetAttachmentSize(attachment, width, height));
        ...
        MaybeError ValidateRenderPassDepthStencilAttachment(
            ...
            if (!TextureFormatHasDepthOrStencil(attachment->GetFormat())) {
                return DAWN_VALIDATION_ERROR("The format of the texture view used as depth stencil"
                                             "attachment is not a depth stencil format");
            }
        ...
        MaybeError ValidateRenderPassDescriptorAndSetSize(const RenderPassDescriptor* renderPass,
                                                          uint32_t* width,
                                                          uint32_t* height) {
            if (renderPass == nullptr) {
                return DAWN_VALIDATION_ERROR("RenderPassDescriptor cannot be nullptr");
            }
            ...
            for (uint32_t i = 0; i < renderPass->colorAttachmentCount; ++i) {
                if (renderPass->colorAttachments[i] != nullptr) {
                    DAWN_TRY(ValidateRenderPassColorAttachment(renderPass->colorAttachments[i],
                                                               width, height));
                }
            }
            ...
            if (renderPass->depthStencilAttachment != nullptr) {
                DAWN_TRY(ValidateRenderPassDepthStencilAttachment(
                    renderPass->depthStencilAttachment, width, height));
            }
    ...
    RenderPassEncoderBase* CommandEncoderBase::BeginRenderPass(const RenderPassDescriptor* info) {
        ...
        uint32_t width = 0;
        uint32_t height = 0;
        if (ConsumedError(ValidateRenderPassDescriptorAndSetSize(info, &width, &height))) {
            return nullptr;
        }
        ...
        for (uint32_t i = 0; i < info->colorAttachmentCount; ++i) {
            if (info->colorAttachments[i] != nullptr) {
                ...
                cmd->colorAttachments[i].clearColor[0] = info->colorAttachments[i]->clearColor.r;
                cmd->colorAttachments[i].clearColor[1] = info->colorAttachments[i]->clearColor.g;
                cmd->colorAttachments[i].clearColor[2] = info->colorAttachments[i]->clearColor.b;
                cmd->colorAttachments[i].clearColor[3] = info->colorAttachments[i]->clearColor.a;
            }
        }
        ...
        if (cmd->hasDepthStencilAttachment) {
            ...
            cmd->depthStencilAttachment.stencilStoreOp = info->depthStencilAttachment->stencilStoreOp;
        }
