...
namespace dawn_native {
    ...
    namespace {
        ...
        MaybeError ValidateResolveTarget(
            ...
            dawn::TextureFormat resolveTargetFormat = colorAttachment->resolveTarget->GetFormat().format;
        ...
        MaybeError ValidateRenderPassDepthStencilAttachment(
            ...
            if (!attachment->GetFormat().HasDepthOrStencil() || !attachment->GetFormat().isRenderable) {
                return DAWN_VALIDATION_ERROR(
    ...
    void CommandEncoderBase::CopyBufferToTexture(const BufferCopyView* source,
        ...
        if (source->rowPitch == 0) {
            copy->source.rowPitch = ComputeDefaultRowPitch(destination->texture->GetFormat(), copySize->width);
        } else {
    ...
    void CommandEncoderBase::CopyTextureToBuffer(const TextureCopyView* source,
        ...
        if (destination->rowPitch == 0) {
            copy->destination.rowPitch = ComputeDefaultRowPitch(source->texture->GetFormat(), copySize->width);
        } else {
        ...
        while (mIterator.NextCommandId(&type)) {
            switch (type) {
                ...
                case Command::CopyBufferToTexture: {
                    ...
                    DAWN_TRY(ValidateTexelBufferOffset(copy->source, copy->destination.texture->GetFormat()));
                ...
                case Command::CopyTextureToBuffer: {
                    ...
                    DAWN_TRY(ValidateTexelBufferOffset(copy->destination, copy->source.texture->GetFormat()));
