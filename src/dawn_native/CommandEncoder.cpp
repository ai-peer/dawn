...
namespace dawn_native {
    ...
    CommandEncoderBase::CommandEncoderBase(DeviceBase* device, const CommandEncoderDescriptor*)
        : ObjectBase(device),
          mEncodingContext(new EncodingContext(device, this)),
          mErrorEncodingContext(nullptr) {
    }
    ...
    EncodingContext* CommandEncoderBase::GetErrorEncodingContext() {
        if (!mErrorEncodingContext) {
            mErrorEncodingContext =
                std::unique_ptr<EncodingContext>(EncodingContext::MakeError(GetDevice()));
        }
        return mErrorEncodingContext.get();
    }
    ...
    ComputePassEncoderBase* CommandEncoderBase::BeginComputePass(
        ...
        ComputePassEncoderBase* passEncoder = nullptr;
        bool errored =
            mEncodingContext->TryEncode(this, [&](CommandAllocator* allocator) -> MaybeError {
                DAWN_TRY(ValidateComputePassDescriptor(device, descriptor));

                allocator->Allocate<BeginComputePassCmd>(Command::BeginComputePass);
                passEncoder = new ComputePassEncoderBase(device, this, mEncodingContext.get());
                mEncodingContext->PushEncoder(passEncoder);
        ...
        return ComputePassEncoderBase::MakeError(device, this, GetErrorEncodingContext());
    }
    ...
    RenderPassEncoderBase* CommandEncoderBase::BeginRenderPass(
        ...
        RenderPassEncoderBase* passEncoder = nullptr;
        bool errored =
            mEncodingContext->TryEncode(this, [&](CommandAllocator* allocator) -> MaybeError {
                uint32_t width = 0;
                uint32_t height = 0;
                ...
                passEncoder = new RenderPassEncoderBase(device, this, mEncodingContext.get());
                mEncodingContext->PushEncoder(passEncoder);
        ...
        return RenderPassEncoderBase::MakeError(device, this, GetErrorEncodingContext());
    }
    ...
    MaybeError CommandEncoderBase::ValidateFinish(const CommandBufferDescriptor*) {
        ...
        mEncodingContext->MoveToIterator();
        CommandIterator* iterator = mEncodingContext->GetIterator();
        iterator->Reset();
        ...
        while (iterator->NextCommandId(&type)) {
            switch (type) {
                ...
                case Command::CopyTextureToTexture: {
                    CopyTextureToTextureCmd* copy =
                        iterator->NextCommand<CopyTextureToTextureCmd>();
        ...
        while (iterator->NextCommandId(&type)) {
            switch (type) {
                ...
                case Command::DispatchIndirect: {
                    DispatchIndirectCmd* cmd = iterator->NextCommand<DispatchIndirectCmd>();
                    DAWN_TRY(persistentState.ValidateCanDispatch());
        ...
        while (iterator->NextCommandId(&type)) {
            switch (type) {
                ...
                case Command::ExecuteBundles: {
                    ExecuteBundlesCmd* cmd = iterator->NextCommand<ExecuteBundlesCmd>();
                    auto bundles = iterator->NextData<Ref<RenderBundleBase>>(cmd->count);
                ...
                case Command::SetViewport: {
                    iterator->NextCommand<SetViewportCmd>();
                } break;

                case Command::SetScissorRect: {
                    iterator->NextCommand<SetScissorRectCmd>();
                } break;

                default:
                    DAWN_TRY(ValidateRenderBundleCommand(
                        iterator, type, "Command disallowed inside a render pass"));
            }
        }
