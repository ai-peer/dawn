...
namespace dawn_native {
    ...
    class ObjectBase : public RefCounted {
      public:
        ...
        // We should not, for example, create an object, use it in a command buffer (pass validation),
        // and then mark is as an error.
        // This is useful to mark objects which fail native backend initialization as errors.
        // It may also help in the future if we have asynchronous command buffer validation or
        // asynchronous pipeline creation.
        // (Declared as a template so we don't need to include Error.h)
