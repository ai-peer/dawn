...
#define HEADER \
    R"(
        ...
        static const char sCopyTextureForBrowserFragment[] = HEADER R"(
            ...
            [[stage(fragment)]] fn main(
                ...
                if (uniforms.u_alphaOp != 0u) {
                    if (uniforms.u_alphaOp == 1u) { // premultiply alpha
                        srcColor = vec4<f32>(srcColor.rgb * srcColor.a , srcColor.a);
                    } else { // unpremultiply alpha, value is 2u
                         // Noop for 0 alpha
                        if (srcColor.a != 0.0) {
                             srcColor = vec4<f32>(srcColor.rgb / srcColor.a, srcColor.a);
                        }
        ...
        MaybeError ValidateCopyTextureForBrowserOptions(
            ...
            if (options->alphaOp != wgpu::AlphaOp::DontChange &&
                options->alphaOp != wgpu::AlphaOp::Premultiply &&
                options->alphaOp != wgpu::AlphaOp::Unpremultiply) {
                return DAWN_VALIDATION_ERROR("CopyTextureForBrowserOptions: Invalid AlphaOp value");
            }
    ...
    MaybeError DoCopyTextureForBrowser(DeviceBase* device,
        ...
        Uniform uniformData = {
            ...
            0  // alphaOp default value: DontChange
        };
        ...
        // Handle alpha. Alpha here helps on the source content and dst content have different
        // alpha config. There are three possible ops: DontChange, Premultiply and Unpremultiply.
        if (options && options->alphaOp != wgpu::AlphaOp::DontChange) {
            uniformData.alphaOp = static_cast<uint32_t>(options->alphaOp);
        }
