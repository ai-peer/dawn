...
namespace dawn_native {
    ...
    PooledResourceMemoryAllocator::PooledResourceMemoryAllocator(
        ...
        : mDevice(device), mHeapAllocator(heapAllocator) {
    }

    void PooledResourceMemoryAllocator::DestroyPool() {
        for (auto& resourceHeap : mPool) {
            ASSERT(resourceHeap.memory != nullptr);
            mHeapAllocator->DeallocateResourceHeap(std::move(resourceHeap.memory));
        }
    ...
    PooledResourceMemoryAllocator::AllocateResourceHeap(uint64_t size) {
        ...
        if (!mPool.empty() && mPool.front().memorySerial <= mDevice->GetCompletedCommandSerial()) {
            memory = std::move(mPool.front().memory);
            mPool.pop_front();
        }
    ...
    void PooledResourceMemoryAllocator::DeallocateResourceHeap(
        ...
        mPool.push_front({mDevice->GetPendingCommandSerial(), std::move(allocation)});
    }
