...
// clang-format off
// TODO(crbug.com/dawn/685): Further investigation needed to set these tiers.
...
#define LIMITS_BINDING_SPACE(X)                                      \
    ...
    X(Higher,                 maxSamplersPerShaderStage, 16, 32, 64) \
...
#define LIMITS_EACH_GROUP(X) X(LIMITS_MEMORY_SIZE) X(LIMITS_BINDING_SPACE) X(LIMITS_OTHER)
#define LIMITS(X) LIMITS_MEMORY_SIZE(X) LIMITS_BINDING_SPACE(X) LIMITS_OTHER(X)
