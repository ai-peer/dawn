...
#define LIMITS(X)                                           \
    ...
    X(Higher, maxTextureArrayLayers, 2048)                  \
    ...
    X(Higher, maxStorageBuffersPerShaderStage, 4)           \
...
#undef X
    ...
    void AssignLimits(wgpu::Limits* currentLimits, const wgpu::Limits& updatedLimits) {
#define X(Better, limitName, defaultValue)                  \
    if (!IsLimitUndefined(updatedLimits.limitName)) {       \
        currentLimits->limitName = updatedLimits.limitName; \
    }
        LIMITS(X)
#undef X
        ASSERT(currentLimits != nullptr);
    }
