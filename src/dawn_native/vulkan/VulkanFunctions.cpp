...
#define GET_GLOBAL_PROC(name)                                                          \
    ...
    if (name == nullptr) {                                                             \
        return DAWN_CONTEXT_LOST_ERROR(std::string("Couldn't get vk") + #name);          \
    }
...
#define GET_DEVICE_PROC(name)                                                       \
    ...
    MaybeError VulkanFunctions::LoadDeviceProcs(VkDevice device, const VulkanDeviceKnobs& usedKnobs) {
