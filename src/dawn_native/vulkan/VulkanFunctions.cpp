...
#define GET_DEVICE_PROC(name)                                                       \
    ...
    MaybeError VulkanFunctions::LoadDeviceProcs(VkDevice device,
        ...
        if (deviceInfo.getMemoryRequirements2 ||
            deviceInfo.properties.apiVersion >= VK_MAKE_VERSION(1, 1, 0)) {
            GET_DEVICE_PROC(GetImageMemoryRequirements2KHR);
            GET_DEVICE_PROC(GetBufferMemoryRequirements2KHR);
            GET_DEVICE_PROC(GetImageSparseMemoryRequirements2KHR);
        }
