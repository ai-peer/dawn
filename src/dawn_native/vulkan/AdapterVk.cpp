...
#define CHECK_AND_SET_V1_MIN_LIMIT(vulkanName, webgpuName) \
    CHECK_AND_SET_V1_LIMIT_IMPL(vulkanName, webgpuName, >, "most")
        ...
        if (vkLimits.maxVertexInputBindingStride < limits->v1.maxVertexBufferArrayStride ||
            vkLimits.maxVertexInputAttributeOffset < limits->v1.maxVertexBufferArrayStride - 1) {
            return DAWN_INTERNAL_ERROR("Insufficient Vulkan limits for maxVertexBufferArrayStride");
        }
        limits->v1.maxVertexBufferArrayStride =
            std::min(vkLimits.maxVertexInputBindingStride, vkLimits.maxVertexInputAttributeOffset);

        if (vkLimits.maxVertexOutputComponents < limits->v1.maxInterStageShaderComponents ||
            vkLimits.maxFragmentInputComponents < limits->v1.maxInterStageShaderComponents) {
            return DAWN_INTERNAL_ERROR(
        ...
        if (!gpu_info::IsAMD(vendorId) && !gpu_info::IsIntel(vendorId) &&
            ...
            if (vkLimits.maxFragmentCombinedOutputResources < maxFragmentCombinedOutputResources) {
                uint32_t count = maxFragmentCombinedOutputResources -
                                 vkLimits.maxFragmentCombinedOutputResources;
                limits->v1.maxStorageTexturesPerShaderStage -= count / 2;
                limits->v1.maxStorageBuffersPerShaderStage -= count - (count / 2);
            }
        }
