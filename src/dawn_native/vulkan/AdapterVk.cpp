...
namespace dawn_native { namespace vulkan {
    ...
    MaybeError Adapter::CheckCoreWebGPUSupport() {
        ...
        if (limits.maxDescriptorSetStorageBuffersDynamic < kMaxSampledTexturesPerShaderStage) {
            return DAWN_INTERNAL_ERROR(
                "Insufficient Vulkan limits for maxDynamicStorageBuffersPerPipelineLayout");
        }
        ...
        if (limits.maxComputeWorkGroupInvocations < kMaxComputeWorkgroupInvocations ||
            limits.maxComputeWorkGroupCount[0] < kMaxComputeWorkgroupInvocations ||
            limits.maxComputeWorkGroupCount[1] < kMaxComputeWorkgroupInvocations ||
            limits.maxComputeWorkGroupCount[2] < kMaxComputeWorkgroupInvocations) {
            return DAWN_INTERNAL_ERROR(
                "Insufficient Vulkan limits for maxComputeWorkgroupInvocations");
        }
        if (limits.maxComputeWorkGroupSize[0] < kMaxComputePerDimensionDispatchSize ||
            limits.maxComputeWorkGroupSize[1] < kMaxComputePerDimensionDispatchSize ||
            limits.maxComputeWorkGroupSize[2] < kMaxComputePerDimensionDispatchSize) {
            return DAWN_INTERNAL_ERROR(
