...
namespace dawn_native { namespace vulkan {
    ...
    RenderPipeline::RenderPipeline(DeviceBase* device, const RenderPipelineDescriptor* descriptor)
        ...
        for (uint32_t i = 0; i < descriptor->numOfRenderStages; ++i) {
            switch (descriptor->stages[i]) {
                case dawn::ShaderStage::Vertex:
                    shaderStages[0].module = ToBackend(descriptor->modules[i])->GetHandle();
                    shaderStages[0].pName = descriptor->entryPoint;
                    break;
                case dawn::ShaderStage::Fragment:
                    shaderStages[1].module = ToBackend(descriptor->modules[i])->GetHandle();
                    shaderStages[1].pName = descriptor->entryPoint;
                    break;
                default:
                    UNREACHABLE();
            }
        ...
        VkRenderPass renderPass = VK_NULL_HANDLE;
        {
            ...
            renderPass = ToBackend(GetDevice())->GetRenderPassCache()->GetRenderPass(query);
        }
        ...
        if (ToBackend(GetDevice())->fn.CreateGraphicsPipelines(ToBackend(GetDevice())->GetVkDevice(), VK_NULL_HANDLE, 1,
