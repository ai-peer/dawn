...
namespace dawn_native { namespace vulkan {
    ...
    ResultOrError<ResourceMemoryAllocation> ResourceMemoryAllocator::Allocate(
        ...
        if (requirements.size < kMaxSizeForSubAllocation && kind != MemoryKind::LinearMappable) {
            ...
            // least this alignment. TODO(cwallez@chromium.org): this is suboptimal because multiple
            // linear (resp. opaque) resources can coexist in the same page. Revisit with a more
            // efficient algorithm later.
            uint64_t alignment =
