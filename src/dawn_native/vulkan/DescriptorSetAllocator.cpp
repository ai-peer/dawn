...
namespace dawn_native { namespace vulkan {
    ...
    ResultOrError<DescriptorSetAllocation> DescriptorSetAllocator::Allocate() {
        ...
        if (pool.sets[setIndex] == VK_NULL_HANDLE) {
            VkDescriptorSetAllocateInfo allocateInfo;
            allocateInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
            allocateInfo.pNext = nullptr;
            allocateInfo.descriptorPool = pool.vkPool;
            allocateInfo.descriptorSetCount = 1;
            allocateInfo.pSetLayouts = &*mLayout->GetHandle();

            Device* device = ToBackend(mLayout->GetDevice());
            DAWN_TRY(CheckVkSuccess(
                device->fn.AllocateDescriptorSets(device->GetVkDevice(), &allocateInfo,
                                                  AsVkArray(pool.sets.data()) + setIndex),
    ...
    MaybeError DescriptorSetAllocator::AllocateDescriptorPool() {
        ...
        mDescriptorPools.emplace_back(
            DescriptorPool{descriptorPool, std::vector<VkDescriptorSet>(mMaxSets, VK_NULL_HANDLE),
