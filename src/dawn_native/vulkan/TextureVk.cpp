...
namespace dawn_native { namespace vulkan {
    ...
    void Texture::DoTransitionSpecialCases(CommandRecordingContext* recordingContext,
    ...
    void Texture::TransitionUsageForPass(CommandRecordingContext* recordingContext,
        ...
        wgpu::TextureUsage allUsage = wgpu::TextureUsage::None;
        wgpu::TextureUsage allLastUsage = wgpu::TextureUsage::None;
        ...
        for (uint32_t i = 0; i < GetArrayLayers(); ++i) {
            for (uint32_t j = 0; j < GetNumMipLevels(); ++j, ++index) {
                ...
                barriers.push_back(BuildMemoryBarrier(format, mHandle,
                                                      mLastSubresourceUsages[index],
                                                      subresourceUsages[index], j, i));

                allUsage |= subresourceUsages[index];
                allLastUsage |= mLastSubresourceUsages[index];
        ...
        VkPipelineStageFlags srcStages = VulkanPipelineStage(allLastUsage, format);
        VkPipelineStageFlags dstStages = VulkanPipelineStage(allUsage, format);
        ToBackend(GetDevice())
        ...
        for (uint32_t i = 0; i < layerCount; ++i) {
            for (uint32_t j = 0; j < levelCount; ++j) {
                ...
                barriers.push_back(BuildMemoryBarrier(format, mHandle, lastUsage, usage,
                                                      mipLevel + j, arrayLayer + i));
                allLastUsage |= lastUsage;
