...
namespace dawn_native { namespace vulkan {
    ...
    namespace {
        ...
        VkImageAspectFlags VulkanAspectMask(const AspectMask& aspectMask) {
            ...
            for (TextureAspect aspect : IterateBitSet(aspectMask)) {
                switch (aspect) {
        ...
        // TODO(enga): In Vulkan 1.1 without VK_KHR_separate_depth_stencil_layouts, barriers must
        // set both depth and stencil aspect bits together. In Vulkan 1.2, or when the extension
        // is present, we should be able to perform layout transitions independently.
        VkImageAspectFlags VulkanAspectMaskForBarrier(const AspectMask& aspectMask) {
            VkImageAspectFlags flags = 0;
            for (TextureAspect aspect : IterateBitSet(aspectMask)) {
                switch (aspect) {
                    case TextureAspect::Color:
        ...
        VkImageMemoryBarrier BuildMemoryBarrier(const VkImage& image,
            ...
            barrier.image = image;
            barrier.subresourceRange.aspectMask = VulkanAspectMaskForBarrier(range.aspectMask);
    ...
    VkImageAspectFlags Texture::GetVkAspectMask(wgpu::TextureAspect aspect) const {
        switch (aspect) {
            ...
            case wgpu::TextureAspect::DepthOnly:
                ASSERT(GetFormat().aspectMask[TextureAspect::Depth]);
                return VulkanAspectMask(SingleAspectMask(TextureAspect::Depth));
            case wgpu::TextureAspect::StencilOnly:
                ASSERT(GetFormat().aspectMask[TextureAspect::Stencil]);
                return VulkanAspectMask(SingleAspectMask(TextureAspect::Stencil));
            default:
                UNREACHABLE();
                return 0;
        }
    ...
    void Texture::TransitionUsageForPass(CommandRecordingContext* recordingContext,
        ...
        if (textureUsages.sameUsagesAcrossSubresources && mSameLastUsagesAcrossSubresources) {
            ...
            for (TextureAspect aspect : IterateBitSet(GetFormat().aspectMask)) {
                for (uint32_t arrayLayer = 0; arrayLayer < GetArrayLayers(); ++arrayLayer) {
                    for (uint32_t mipLevel = 0; mipLevel < GetNumMipLevels(); ++mipLevel) {
                        ...
                        imageBarriers->push_back(BuildMemoryBarrier(
                            mHandle, mSubresourceLastUsages[index],
                            textureUsages.subresourceUsages[index],
                            SubresourceRange::SingleSubresource(mipLevel, arrayLayer, aspect)));
                ...
                for (uint32_t layer = range.baseArrayLayer;
                    ...
                    for (uint32_t level = range.baseMipLevel;
                        ...
                        barriers.push_back(BuildMemoryBarrier(
                            mHandle, mSubresourceLastUsages[index], usage,
