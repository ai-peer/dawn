...
namespace dawn_native { namespace vulkan {
    ...
    void Texture::TransitionUsageNow(CommandRecordingContext* recordingContext,
        ...
        if (levelCount > 0 && layerCount > 0) {
            mLastUsage = mLastSubresourceUsages[GetSubresourceIndex(mipLevel, arrayLayer)];
        }
        ...
        VkImageMemoryBarrier barrier;
        if (levelCount > 0 && layerCount > 0) {
            barrier = BuildMemoryBarrier(mLastUsage, usage, format, mipLevel, levelCount,
                                         arrayLayer, layerCount);
        } else {
            barrier = BuildMemoryBarrier(mLastUsage, usage, format, 0, GetNumMipLevels(), 0,
                                         GetArrayLayers());
        }
