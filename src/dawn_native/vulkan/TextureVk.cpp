...
namespace dawn_native { namespace vulkan {
    ...
    MaybeError Texture::InitializeAsInternalTexture() {
        ...
        if (device->IsToggleEnabled(Toggle::NonzeroClearResourcesOnCreationForTesting)) {
            SubresourceRange range = {0, GetNumMipLevels(), 0, GetArrayLayers()};
            DAWN_TRY(ClearTexture(ToBackend(GetDevice())->GetPendingRecordingContext(), range,
        ...
        if (GetFormat().isRenderable) {
            ...
            for (uint32_t level = range.baseMipLevel; level < range.baseMipLevel + range.levelCount;
                ...
                for (uint32_t layer = range.baseArrayLayer;
                    ...
                    SubresourceRange subresource = {level, 1, layer, 1};
                    if (clearValue == TextureBase::ClearValue::Zero &&
                        IsSubresourceContentInitialized(subresource)) {
