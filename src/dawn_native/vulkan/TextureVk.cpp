...
namespace dawn_native { namespace vulkan {
    ...
    void Texture::TransitionUsageNow(CommandRecordingContext* recordingContext,
        ...
        // Note that if the texture has only one mip level and one array slice, it will fall into
        // this category.
        // On Vulkan, depth/stencil aspects are the same subresource so we need to also check the
        // aspect mask is full.
        bool areAllSubresourcesCovered =
            range.levelCount * range.layerCount * range.aspectMask.count() == subresourceCount &&
            range.aspectMask == GetFormat().aspectMask;
            ...
            for (uint32_t layer = range.baseArrayLayer;
                ...
                for (uint32_t level = range.baseMipLevel;
                    ...
                    barriers.push_back(BuildMemoryBarrier(
                        mHandle, lastUsage, usage, {level, 1, layer, 1, GetFormat().aspectMask}));
                    allLastUsages |= lastUsage;
                }
            }
        }
