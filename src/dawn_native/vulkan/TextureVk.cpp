...
namespace dawn_native { namespace vulkan {
    ...
    void Texture::ClearTexture(CommandRecordingContext* recordingContext,
        ...
        if (GetFormat().HasDepthOrStencil()) {
            ...
            // TODO(natlee@microsoft.com): clear compressed textures
            return;
        } else {
            // create temp buffer with clear color to clear the texture image
            ...
            descriptor.size = GetSize().width * GetSize().height * GetFormat().blockByteSize;
            ...
            std::vector<uint32_t> clearBuffer(descriptor.size, clearColor);
            srcBuffer->SetSubData(0, clearBuffer.size(), clearBuffer.data());

            // create the buffer image copy to set the clear region
            VkBufferImageCopy region;
            region.bufferOffset = 0;
            region.bufferRowLength = 0;
            region.bufferImageHeight = 0;

            region.imageSubresource.aspectMask = GetVkAspectMask();
            ...
            region.imageExtent.width = GetSize().width;
            region.imageExtent.height = GetSize().height;
