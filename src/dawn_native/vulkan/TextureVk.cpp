...
namespace dawn_native { namespace vulkan {
    ...
    void Texture::TransitionUsageForPass(CommandRecordingContext* recordingContext,
        ...
        // single plane in a new SubresourceStorage<TextureUsage>. Then we modify the generated
        // VkImageMemoryBarriers to replace the aspect of just Depth with Depth | Stencil.
        if (ShouldCombineDepthStencilBarriers()) {
            SubresourceStorage<wgpu::TextureUsage> combinedUsages(Aspect::Depth, GetArrayLayers(),
                                                                  GetNumMipLevels());
            textureUsages.subresourceUsages.Iterate([&](const SubresourceRange& range,
                                                        wgpu::TextureUsage usage) {
                SubresourceRange updateRange = range;
                updateRange.aspects = Aspect::Depth;
            ...
            TransitionUsageForPassImpl(recordingContext, combinedUsages, &newBarriers, srcStages,
                                       dstStages);

            for (auto& barrier : newBarriers) {
                ASSERT(barrier.subresourceRange.aspectMask == VK_IMAGE_ASPECT_DEPTH_BIT);
                barrier.subresourceRange.aspectMask =
                    VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT;
            }

            imageBarriers->insert(imageBarriers->end(), newBarriers.begin(), newBarriers.end());
        } else {
    ...
    void Texture::TransitionUsageForPassImpl(
        ...
        mSubresourceLastUsages.Merge<wgpu::TextureUsage>(
    ...
    void Texture::TransitionUsageAndGetResourceBarrier(
        ...
        VkPipelineStageFlags* dstStages) {
        // Base Vulkan doesn't support transitioning depth and stencil separately. We work around
        // this limitation by modifying the range to be only on Depth. Then we modify the generated
        // VkImageMemoryBarriers to replace the aspect of just Depth with Depth | Stencil.
        if (ShouldCombineDepthStencilBarriers()) {
            SubresourceRange updatedRange = range;
            updatedRange.aspects = Aspect::Depth;

            std::vector<VkImageMemoryBarrier> newBarriers;
            TransitionUsageAndGetResourceBarrierImpl(usage, updatedRange, &newBarriers, srcStages,
                                                     dstStages);
