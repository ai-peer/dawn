...
namespace dawn_native { namespace vulkan {
    ...
    MaybeError Texture::ExportExternalTexture(VkImageLayout desiredLayout,
        ...
        barrier.newLayout =
            desiredLayout == VK_IMAGE_LAYOUT_UNDEFINED
                ? barrier.oldLayout
                : desiredLayout;  // VK_IMAGE_LAYOUT_UNDEFINED is invalid here. We use it as a
        ...
        VkPipelineStageFlags dstStages =
            VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;  // We don't know when the importing queue will need
                                                // the texture, so pass
                                                // VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT to ensure any
    ...
    void Texture::TweakTransitionForExternalUsage(CommandRecordingContext* recordingContext,
        ...
        if (mExternalState == ExternalState::PendingAcquire) {
            ...
            barrier->oldLayout =
                isInitialized ? mPendingAcquireOldLayout : VK_IMAGE_LAYOUT_UNDEFINED;
            barrier->newLayout = isInitialized ? mPendingAcquireNewLayout : desiredLayout;
