...
namespace dawn_native { namespace vulkan {
    ...
    Texture::Texture(Device* device, const TextureDescriptor* descriptor, TextureState state)
        : TextureBase(device, descriptor, state),
          mSubresourceLastUsages(wgpu::TextureUsage::None, this) {
    }
    ...
    void Texture::TransitionUsageForPass(CommandRecordingContext* recordingContext,
        ...
        // TODO depth-stencil is a bit more effy without the extension to handle them separately
        mSubresourceLastUsages.Merge<wgpu::TextureUsage>(
            textureUsages.subresourceUsages,
            [&](const SubresourceRange& range, const wgpu::TextureUsage& lastUsage,
                const wgpu::TextureUsage& newUsage) -> wgpu::TextureUsage {
    ...
    void Texture::TransitionUsageAndGetResourceBarrier(
        ...
        mSubresourceLastUsages.Update(
            range,
            [&](const SubresourceRange& range,
                const wgpu::TextureUsage& lastUsage) -> wgpu::TextureUsage {
                if (CanReuseWithoutBarrier(lastUsage, usage)) {
                    return lastUsage;
                }

                imageBarriers->push_back(
                    BuildMemoryBarrier(format, mHandle, lastUsage, usage, range));
