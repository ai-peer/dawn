...
namespace dawn_native { namespace vulkan {
    ...
    ResultOrError<VulkanGlobalKnobs> Backend::CreateInstance() {
        ...
        // Register the debug messenger for instance creation so we receive message for any errors
        // (validation or other).
        ...
        if (mGlobalInfo.HasExt(InstanceExt::DebugUtils)) {
            ...
            utilsMessengerCreateInfo.pfnUserCallback = Backend::OnDebugUtilsCallback;
            utilsMessengerCreateInfo.pUserData = nullptr;

            createInfoChain.Add(&utilsMessengerCreateInfo, VK_STRUCTURE_TYPE_DEBUG_UTILS_MESSENGER_CREATE_INFO_EXT);
        }

        // Try to turn on synchronization validation if the instance was created with backend validation enabled.
        ...
        if (GetInstance()->IsBackendValidationEnabled() && mGlobalInfo.HasExt(InstanceExt::ValidationFeatures)) {
    ...
    MaybeError Backend::RegisterDebugUtils() {
        ...
        createInfo.messageType = VK_DEBUG_UTILS_MESSAGE_TYPE_GENERAL_BIT_EXT |
                                 VK_DEBUG_UTILS_MESSAGE_TYPE_VALIDATION_BIT_EXT;
        createInfo.pfnUserCallback = Backend::OnDebugUtilsCallback;
