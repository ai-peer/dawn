...
namespace dawn_native { namespace vulkan {
    ...
    ResultOrError<VulkanGlobalKnobs> Backend::CreateInstance() {
        ...
        auto UseLayerIfAvailable = [&](InstanceLayer layer) {
            if (mGlobalInfo.layers[layer]) {
                layerNames.push_back(GetInstanceLayerInfo(layer).name);
        ...
        UseLayerIfAvailable(InstanceLayer::LunargVkTrace);
#endif
        ...
        UseLayerIfAvailable(InstanceLayer::RenderDocCapture);
#endif

        if (GetInstance()->IsBackendValidationEnabled()) {
            UseLayerIfAvailable(InstanceLayer::Validation);
        }

        // Always use the Fuchsia swapchain layer if available.
        UseLayerIfAvailable(InstanceLayer::FuchsiaImagePipeSwapchain);
