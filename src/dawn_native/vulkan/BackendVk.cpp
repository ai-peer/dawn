...
namespace dawn_native { namespace vulkan {
    ...
    MaybeError Backend::Initialize(bool useSwiftshader) {
        ...
        if (useSwiftshader) {
            ...
            vkICDFilenames =
                ScopedEnvironmentVar("VK_ICD_FILENAMES", fullSwiftshaderICDPath.c_str());
            if (!vkICDFilenames.IsSet()) {
        ...
        if (GetInstance()->IsBackendValidationEnabled()) {
            ...
            vkLayerPath = ScopedEnvironmentVar("VK_LAYER_PATH", vkDataDir.c_str());
            if (!vkLayerPath.IsSet()) {
