...
#    if defined(DAWN_SWIFTSHADER_VK_ICD_JSON)
                MaybeError result = LoadVulkan(kVulkanLibName);
                if (result.IsSuccess()) {
                    result = SetSwiftShaderICD();
                    if (result.IsSuccess()) {
                        // Succesfully loaded driver and set VK_ICD_FILENAMES.
                        break;
                    }
                }
                auto prevErrorData = result.AcquireError();
                result = LoadVulkan(kSwiftshaderLibName);
                if (result.IsSuccess()) {
                    // Succesfully loaded driver; break.
                    break;
                }
                auto errorData = result.AcquireError();
                return DAWN_FORMAT_INTERNAL_ERROR("Failed to load SwiftShader.\n%s\n%s",
