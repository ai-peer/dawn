...
namespace dawn_native { namespace vulkan {
    ...
    const uint32_t VulkanVersion_1_1 = VK_MAKE_VERSION(1, 1, 0);
    const uint32_t NeverPromoted = VK_MAKE_VERSION(10, 0, 0);
    ...
    static constexpr std::array<InstanceExtInfo, static_cast<size_t>(InstanceExt::EnumCount)>
        sInstanceExtInfos{
        ...
        for (uint32_t i = 0; i < sInstanceExtInfos.size(); i++) {
            ...
            switch (ext) {
                ...
                case InstanceExt::ExternalSemaphoreCapabilities:
                    ASSERT(visitedSet.Has(InstanceExt::GetPhysicalDeviceProperties2));
                    hasDependencies = trimmedSet.Has(InstanceExt::GetPhysicalDeviceProperties2);
                    break;
                ...
                case InstanceExt::XlibSurface:
                    ASSERT(visitedSet.Has(InstanceExt::Surface));
                    hasDependencies = trimmedSet.Has(InstanceExt::Surface);
                    break;
            ...
            trimmedSet.Set(ext, hasDependencies && availableSet.Has(ext));
