...
namespace dawn_native { namespace vulkan {
    ...
    namespace {
        ...
        uint32_t ImageCountForPresentMode(VkPresentModeKHR mode) {
            switch (mode) {
    ...
    MaybeError SwapChain::Initialize(NewSwapChainBase* previousSwapChain) {
        ...
        createInfo.minImageCount = mConfig.imageCount;
    ...
    ResultOrError<SwapChain::Config> SwapChain::ChooseConfig(
        ...
        // Only the identity transform with opaque alpha is supports for now.
        ...
        uint32_t targetCount = ImageCountForPresentMode(config.presentMode);
