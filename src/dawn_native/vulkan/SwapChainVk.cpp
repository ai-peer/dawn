...
namespace dawn_native { namespace vulkan {
    ...
    namespace {
        ...
        ResultOrError<VkSurfaceKHR> CreateVulkanSurface(Backend* backend, Surface* surface) {
            ...
            return DAWN_UNIMPLEMENTED_ERROR("Unsupported surface type for Vulkan");
        }
    ...
    MaybeError SwapChain::Initialize(NewSwapChainBase* previousSwapChain) {
        ...
        // TODO(cwallez@chromium.org): Remove the need for this by tagging the texture as
        // uninitialized so it gets initialized the first time it is used.
        CommandRecordingContext* recordingContext = device->GetPendingRecordingContext();
    ...
    MaybeError SwapChain::PresentImpl() {
        ...
        CommandRecordingContext* recordingContext = device->GetPendingRecordingContext();
        mTexture->TransitionUsageNow(recordingContext, wgpu::TextureUsage::Present);
        DAWN_TRY(device->SubmitPendingCommands());
    ...
    ResultOrError<TextureViewBase*> SwapChain::GetCurrentTextureViewImpl() {
        ...
        mTexture = AcquireRef(new Texture(device, &textureDesc, currentImage));
