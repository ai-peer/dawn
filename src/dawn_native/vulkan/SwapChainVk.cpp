...
namespace dawn_native { namespace vulkan {
    ...
    MaybeError OldSwapChain::OnBeforePresent(TextureBase* texture) {
    ...
    MaybeError SwapChain::Initialize(NewSwapChainBase* previousSwapChain) {
        ...
        if (previousSwapChain != nullptr) {
            ...
            // TODO(cwallez@chromium.org): check they have the same instance.
    ...
    ResultOrError<SwapChain::Config> SwapChain::ChooseConfig(const VulkanSurfaceInfo& surfaceInfo) const {
        Config config;
        ...
        switch (GetPresentMode()) {
            case wgpu::PresentMode::Immediate:
                if (HasPresentMode(surfaceInfo.presentModes, VK_PRESENT_MODE_IMMEDIATE_KHR)) {
                    config.presentMode = VK_PRESENT_MODE_IMMEDIATE_KHR;
                    break;
                }
                // fallthrough
            case wgpu::PresentMode::Mailbox:
                if (HasPresentMode(surfaceInfo.presentModes, VK_PRESENT_MODE_MAILBOX_KHR)) {
                    config.presentMode = VK_PRESENT_MODE_MAILBOX_KHR;
                    break;
                }
                // fallthrough
            case wgpu::PresentMode::Fifo:
                if (HasPresentMode(surfaceInfo.presentModes, VK_PRESENT_MODE_FIFO_KHR)) {
                    config.presentMode = VK_PRESENT_MODE_FIFO_KHR;
                    break;
                }
                // fallthrough
            default:
                return DAWN_VALIDATION_ERROR("present mode not supported");
       }
