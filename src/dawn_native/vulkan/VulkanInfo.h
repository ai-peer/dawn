...
namespace dawn_native { namespace vulkan {
    ...
    struct VulkanGlobalKnobs {
        ...
        ityp::array<InstanceLayer, InstanceExtSet, static_cast<uint32_t>(InstanceLayer::EnumCount)>
            layerExtensions;

        // During information gathering `extensions` only contains the instance's extensions but
        // after the layers we want to use have been selected it becomes the OR of the instance's
        // extensions and the selected layers' extensions.
