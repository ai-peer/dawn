...
namespace dawn_native { namespace vulkan { namespace external_memory {
    ...
    Service::Service(Device* device) : mDevice(device) {
        ...
        mSupportedFirstPass = globalInfo.getPhysicalDeviceProperties2 &&
    ...
    bool Service::Supported(VkFormat format,
        ...
        // Find the VkExternalImageFormatProperties in the pNext chain
        const VkBaseInStructure* curStruct =
            reinterpret_cast<const VkBaseInStructure*>(formatProperties.pNext);
        const VkExternalImageFormatProperties* externalFormatProperties = nullptr;
        while (curStruct != nullptr) {
            if (curStruct->sType == VK_STRUCTURE_TYPE_EXTERNAL_IMAGE_FORMAT_PROPERTIES_KHR) {
                externalFormatProperties =
                    reinterpret_cast<const VkExternalImageFormatProperties*>(curStruct);
                break;
            }
            curStruct = curStruct->pNext;
        }

        if (externalFormatProperties == nullptr) {
            return false;
        }

        // TODO: Decide if we need to support dedicated images - bug?
        VkFlags memoryFlags =
