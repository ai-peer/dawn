...
namespace dawn_native { namespace vulkan { namespace external_semaphore {
    ...
    class Service {
      public:
        ...
        ResultOrError<VkSemaphore> ImportSemaphore(Device* device, ExternalSemaphoreHandle handle);
        ResultOrError<VkSemaphore> CreateExportableSemaphore(Device* device);
        ResultOrError<ExternalSemaphoreHandle> ExportSemaphore(Device* device,
