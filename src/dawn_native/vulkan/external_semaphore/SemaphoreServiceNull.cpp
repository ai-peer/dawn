...
namespace dawn_native { namespace vulkan { namespace external_semaphore {
    ...
    ResultOrError<VkSemaphore> Service::ImportSemaphore(Device* device, ExternalHandle handle) {
        return NULL_ERROR;
    }
    ...
    ResultOrError<ExternalHandle> Service::ExportSemaphore(Device* device, VkSemaphore semaphore) {
        return NULL_ERROR;
    }
