...
namespace dawn_native { namespace vulkan {
    ...
    MaybeError StagingBuffer::Flush(size_t size, size_t offset) const {
        VkMappedMemoryRange stagingRange;
        stagingRange.sType = VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE;
        stagingRange.pNext = nullptr;
        stagingRange.memory = mAllocation.GetMemory();
        stagingRange.offset = offset;
        stagingRange.size = size;

        if (mDevice->fn.FlushMappedMemoryRanges(mDevice->GetVkDevice(), 1, &stagingRange) !=
            VK_SUCCESS) {
            return DAWN_CONTEXT_LOST_ERROR("Unable to flush staging memory.");
        }
