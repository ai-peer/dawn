...
namespace dawn_native { namespace vulkan {
    ...
    ResultOrError<VulkanGlobalInfo> GatherGlobalInfo(const Backend& backend) {
        ...
        // Gather the info about the instance layers
        {
            ...
            std::unordered_map<std::string, InstanceLayer> knownLayers =
                CreateInstanceLayerNameMap();
        ...
        // Gather the info about the instance extensions
        {
            ...
            for (InstanceLayer layer : IterateBitSet(info.layers)) {
                DAWN_TRY_ASSIGN(info.layerExtensions[layer],
                                GatherInstanceExtensions(GetInstanceLayerInfo(layer).name,
