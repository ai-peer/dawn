...
namespace {
    ...
    bool EnumerateInstanceExtensions(const char* layer_name,
        ...
        uint32_t count = 0;
        VkResult result =
            vkFunctions.EnumerateInstanceExtensionProperties(layer_name, &count, nullptr);
        if (result != VK_SUCCESS && result != VK_INCOMPLETE) {
            return false;
        }
        extensions->resize(count);
        result =
            vkFunctions.EnumerateInstanceExtensionProperties(layer_name, &count, extensions->data());
