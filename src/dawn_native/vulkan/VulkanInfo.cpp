...
namespace dawn_native { namespace vulkan {
    ...
    ResultOrError<VulkanDeviceInfo> GatherDeviceInfo(const Adapter& adapter) {
        ...
        {
            uint32_t count = 0;
            VkResult result = vkFunctions.EnumerateDeviceExtensionProperties(physicalDevice, nullptr,
                                                                           &count, nullptr);
            if (result != VK_SUCCESS && result != VK_INCOMPLETE) {
    ...
    MaybeError GatherSurfaceInfo(const Adapter& adapter,
        ...
        // Query which queue families support presenting this surface
        {
            ...
            for (uint32_t i = 0; i < nQueueFamilies; ++i) {
                ...
                VkResult result = vkFunctions.GetPhysicalDeviceSurfaceSupportKHR(physicalDevice, i,
                                                                               surface, &supported);
        ...
        {
            uint32_t count = 0;
            VkResult result = vkFunctions.GetPhysicalDeviceSurfaceFormatsKHR(physicalDevice, surface,
                                                                           &count, nullptr);
            if (result != VK_SUCCESS && result != VK_INCOMPLETE) {
