...
namespace dawn_native { namespace vulkan {
    ...
    void Device::CheckAndHandleDeviceLost(wgpu::ErrorType type) {
        if (type == wgpu::ErrorType::DeviceLost) {
            ...
            VkResult waitIdleResult = fn.QueueWaitIdle(mQueue);
            // Ignore the result of QueueWaitIdle: it can return OOM which we can't really do
            // anything about, Device lost, which means workloads running on the GPU are no longer
            // accessible (so they are as good as waited on) or success.
            DAWN_UNUSED(waitIdleResult);
            ...
            // The VkRenderPasses in the cache can be destroyed immediately since all commands
            // referring to them are guaranteed to be finished executing.
            mRenderPassCache = nullptr;
