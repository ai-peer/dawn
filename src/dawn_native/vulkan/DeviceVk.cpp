...
namespace dawn_native { namespace vulkan {
    ...
    MaybeError Device::CopyFromStagingToBuffer(StagingBufferBase* source,
                                               ...
                                               uint32_t size) {
        // Note: Memory barrier for the transfer from host to staging buffer is implicit upon
        // submission. However, these writes to the staging buffer are not guarenteed to be made
        // visible to the device.
        // TODO(b-brber): Avoid unnecessary flushes or consider
        // VK_MEMORY_PROPERTY_HOST_COHERENT_BIT.
        DAWN_TRY(ToBackend(source)->Flush(sourceOffset, size));

        // Insert pipeline barrier for copy into destination buffer.
