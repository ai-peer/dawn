...
namespace dawn_native { namespace vulkan {
    ...
    MaybeError Device::TickImpl() {
        ...
        for (auto& allocator :
             mDescriptorSetAllocatorsPendingDeallocation.IterateUpTo(mCompletedSerial)) {
            allocator->Tick(mCompletedSerial);
        }
        mDescriptorSetAllocatorsPendingDeallocation.ClearUpTo(mCompletedSerial);
    ...
    void Device::EnqueueDeferredDeallocation(DescriptorSetAllocator* allocator) {
        mDescriptorSetAllocatorsPendingDeallocation.Enqueue(allocator, GetPendingCommandSerial());
    }
