...
namespace dawn_native { namespace vulkan {
    ...
    void Device::WaitForSerial(Serial serial) {
        ...
        Serial currentFenceSerial = mCompletedSerial;
        while (!mFencesInFlight.empty()) {
            VkFence fence = mFencesInFlight.front().first;
            Serial fenceSerial = mFencesInFlight.front().second;
            ...
            ASSERT(fenceSerial > currentFenceSerial);
            currentFenceSerial = fenceSerial;
        }
        ...
        bool signaled = false;
        do {
            VKResult result = fn.WaitForFences(
                mVkDevice, fences.size(), fences.data(), true, UINT64_MAX);
        ...
        mCompletedSerial = currentFenceSerial;
    }
