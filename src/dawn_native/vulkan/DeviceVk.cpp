...
namespace dawn_native { namespace vulkan {
    ...
    MaybeError Device::Initialize() {
        ...
        *functions = ToBackend(GetAdapter())->GetBackend()->GetFunctions();
        ...
        // these two are set up, so that a failed initialization doesn't cause a crash in Destroy()
        {
    ...
    void Device::Destroy() {
        ...
        // The deleter is the second thing we initialize. If it is not present, it means the
