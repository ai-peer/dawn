...
namespace dawn_native { namespace vulkan {
    ...
    void CommandBuffer::RecordCopyImageWithTemporaryBuffer(
        ...
        Buffer* tempBuffer = ToBackend(device->CreateBuffer(&tempBufferDescriptor));
        ...
        // When *tempBuffer is created, its ref count is 1, and before the execution of the last
        // line the ref count of *tempBuffer is 3 after the pointer tempBuffer is assigned to
        // tempBufferCopy.buffer and the last member of recordingContext->tempBuffers. To prevent
        // memory leak, we must reduce the ref count before the return of the funciton to ensure the
        // ref count of *tempBuffer is 0 after both tempBufferCopy.buffer and
        // recordingContext->tempBuffers are destroyed.
        tempBuffer->Release();
    }
