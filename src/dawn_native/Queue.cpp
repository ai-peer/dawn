...
namespace dawn_native {
    ...
    namespace {
        ...
        struct CommandBufferSubmitDone : QueueBase::TaskInFlight {
            CommandBufferSubmitDone(CommandBufferBase* commandBuffer)
                : mCommandBuffer(commandBuffer) {
            }
            void Finish() override {
                ASSERT(mCommandBuffer != nullptr);
                // TODO: This is where we should actually read back the execution times.
                mCommandBuffer->OnExecutionTimeResolved(WGPUExecutionTimeRequestStatus_Success,
                                                        1.0);
          ...
          private:
            CommandBufferBase* mCommandBuffer = nullptr;
        };
    ...
    void QueueBase::SubmitInternal(uint32_t commandCount, CommandBufferBase* const* commands) {
        ...
        for (uint32_t i = 0; i < commandCount; ++i) {
            if (commands[i]->MeasureExecutionTime()) {
                // TODO: How do we manage command buffer lifetime here?
                std::unique_ptr<CommandBufferSubmitDone> task =
                    std::make_unique<CommandBufferSubmitDone>(commands[i]);
                TrackTask(std::move(task), GetDevice()->GetPendingCommandSerial());
            }
        }
    }
