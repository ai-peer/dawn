...
namespace dawn_native { namespace metal {
    ...
    namespace {
        ...
        shaderc_spvc_execution_model SpvcExecutionModelForStage(SingleShaderStage stage) {
            switch (stage) {
                case SingleShaderStage::Vertex:
    ...
    MaybeError ShaderModule::GetFunction(const char* functionName,
            ...
            for (uint32_t binding : IterateBitSet(bgInfo.mask)) {
                for (auto stage : IterateStages(bgInfo.visibilities[binding])) {
                    ...
                    if (GetDevice()->IsToggleEnabled(Toggle::UseSpvc)) {
                        ...
                        mslBinding.stage = SpvcExecutionModelForStage(stage);
        ...
        {
            if (GetDevice()->IsToggleEnabled(Toggle::UseSpvc)) {
                shaderc_spvc_execution_model executionModel =
                    SpvcExecutionModelForStage(functionStage);
