    ...
    namespace {
        ...
        class BindGroupTracker {
          public:
            ...
            template <typename Encoder, typename Pipeline>
            void Apply(Encoder encoder, Pipeline* pipeline) {
                PipelineLayout* layout = ToBackend(pipeline->GetLayout());
                std::bitset<kMaxBindGroups> bindGroupLayoutsMask =
                    layout->GetBindGroupLayoutsMask();

                if (mLastAppledPipeline != pipeline) {
                    // If the pipline has changed, all of the bind groups in the pipeline are dirty.
                    mDirtyBindGroups |= layout->GetBindGroupLayoutsMask();
                }

                std::bitset<kMaxBindGroups> bindGroupsToApply =
                    bindGroupLayoutsMask & mDirtyBindGroups;

                for (uint32_t index : IterateBitSet(bindGroupsToApply)) {
                    ApplyBindGroup(encoder, index, mBindGroups[index], mDynamicOffsetCounts[index],
                                   mDynamicOffsets[index].data(), layout);
                }

                mLengthTracker.Apply(encoder, pipeline);

                mDirtyBindGroups &= ~bindGroupsToApply;
                mLastAppledPipeline = pipeline;
            }
          ...
          private:
            ...
            PipelineBase* mLastAppledPipeline = nullptr;
        ...
        while (mCommands.NextCommandId(&type)) {
            switch (type) {
                ...
                case Command::Dispatch: {
                    ...
                    bindGroups.Apply(encoder, lastPipeline);
