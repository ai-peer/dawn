    ...
    namespace {
        ...
        MTLRenderPassDescriptor* CreateMTLRenderPassDescriptor(RenderPassDescriptorBase* desc) {
            ...
            for (uint32_t i : IterateBitSet(desc->GetColorAttachmentMask())) {
                ...
                // TODO(jiawei.shao@intel.com): support rendering into a layer or a mipmap level of a texture.
            ...
            for (size_t binding = 0; binding < layout.mask.size(); ++binding) {
                ...
                switch (layout.types[binding]) {
                    ...
                    case dawn::BindingType::SampledTexture: {
                        ...
                        if (hasVertStage) {
                            [render setVertexTexture:textureView->GetMTLTextureView() atIndex:vertIndex];
                        }
                        if (hasFragStage) {
                            [render setFragmentTexture:textureView->GetMTLTextureView() atIndex:fragIndex];
                        }
                        if (hasComputeStage) {
                            [compute setTexture:textureView->GetMTLTextureView() atIndex:computeIndex];
                        }
                    } break;
                }
            }
