            ...
            switch (type) {
                ...
                case Command::CopyBufferToTexture: {
                    ...
                    for (uint32_t i = 0; i < splitCopies.count; ++i) {
                        ...
                        const uint32_t copyBaseSlice = copyInfo.textureOrigin.z;
                        ...
                        switch (texture->GetDimension()) {
                            case wgpu::TextureDimension::e2D: {
                                ...
                                for (uint32_t copyLayer = copyBaseSlice;
                                     copyLayer <
                                     copyBaseSlice + copyInfo.copyExtent.depthOrArrayLayers;
                                     ++copyLayer) {
                                    [commandContext->EnsureBlit()
                ...
                case Command::CopyTextureToTexture: {
                    ...
                    uint32_t* sourceSlicePtr;
                    if (srcTexture->GetDimension() == wgpu::TextureDimension::e2D) {
                        sourceSlicePtr = &sourceLayer;
                    } else {
                        sourceSlicePtr = &sourceOriginZ;
                    }

                    uint32_t* destinationSlicePtr;
                    if (dstTexture->GetDimension() == wgpu::TextureDimension::e2D) {
                        destinationSlicePtr = &destinationLayer;
                    } else {
                        destinationSlicePtr = &destinationOriginZ;
                    }

                    for (uint32_t slice = 0; slice < copy->copySize.depthOrArrayLayers; ++slice) {
                        *sourceSlicePtr = copy->source.origin.z + slice;
                        *destinationSlicePtr = copy->destination.origin.z + slice;
