        ...
        while (mCommands.NextCommandId(&type)) {
            switch (type) {
                ...
                case Command::CopyBufferToTexture: {
                    ...
                    // When uploading textures from an unpack buffer, he Metal validation device doesn't compute
                    // the correct range when checking if the buffer is big enough to contain the data for the
                    // whole copy. Instead of looking at the position of the last texet in the buffer, it computes
                    // the volumw of the 3D box with rowPitch * imageHeight * copySize.depth.
                    // For example considering the pixel buffer below where in memory, each row data (D) of the
                    // texture is followed by some padding data (P):
                    //     |DDDDDDD|PP|
                    //     |DDDDDDD|PP|
                    //     |DDDDDDD|PP|
                    //     |DDDDDDD|PP|
                    //     |DDA|PPPPPP|
                    // The last pixel read will be A, but the driver will think it is the whole last padding row,
                    // causing it to generate an error when the pixel buffer is just big enough.

                    // We work around this limitation by detecting when Metal would complain and copy the last
                    // image and row separately using tight sourceBytesPerRow or sourceBytesPerImage.
                    uint32_t bytesPerImage = src.rowPitch * src.imageHeight;

                    // Check whether buffer size is unaligned and not big enough
                    bool needWorkaround = (buffer->GetSize() - src.offset < bytesPerImage * size.depth);
                    ...
                    if (size.depth > 1) {
                        ...
                        // Update origin, offset and size on z to copy to the last image.
                        origin.z += copySize.depth - 1;
                    ...
                    if (size.height > 1) {
                        ...
                        // Update origin, offset and size on y to copy to the last row.
                        origin.y += copySize.height - 1;
                ...
                case Command::CopyTextureToBuffer: {
                    ...
                    // When Copy textures to an unpack buffer, he Metal validation device doesn't compute
                    // the correct range when checking if the buffer is big enough to contain the data for the
                    // whole copy. Instead of looking at the position of the last texet in the buffer, it computes
                    // the volumw of the 3D box with rowPitch * imageHeight * copySize.depth.
                    // For example considering the texture below where in memory, each row data (D) of the
                    // texture is followed by some padding data (P):
                    //     |DDDDDDD|PP|
                    //     |DDDDDDD|PP|
                    //     |DDDDDDD|PP|
                    //     |DDDDDDD|PP|
                    //     |DDA|PPPPPP|
                    // The last valid pixel read will be A, but the driver will think it needs the whole last
                    // padding row, causing it to generate an error when the buffer is just big enough.

                    // We work around this limitation by detecting when Metal would complain and copy the last
                    // image and row separately using tight destinationBytesPerRow or destinationBytesPerImage.
                    uint32_t bytesPerImage = dst.rowPitch * dst.imageHeight;

                    // Check whether buffer size is unaligned and not big enough
                    bool needWorkaround = (buffer->GetSize() - dst.offset < bytesPerImage * size.depth);
                    ...
                    if (size.depth > 1) {
                        ...
                        // Update origin, offset and size on z to copy from the last image.
                        origin.z += copySize.depth - 1;
                        offset += (copySize.depth - 1) * bytesPerImage;
                        size.depth = 1;
                    }
                    ...
                    if (size.height > 1) {
                        ...
                        // Update origin, offset and size on y to copy from the last row.
                        origin.y += copySize.height - 1;
