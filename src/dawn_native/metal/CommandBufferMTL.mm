...
namespace dawn_native { namespace metal {
    ...
    namespace {
        ...
        MTLRenderPassDescriptor* CreateMTLRenderPassDescriptor(
            ...
            std::bitset<kMaxColorAttachments> forceUnsetResolveTargetsSet;
            for (uint32_t i : IterateBitSet(renderPass->colorAttachmentsSet)) {
                auto& attachmentInfo = renderPass->colorAttachments[i];
                TextureViewBase* resolveTarget = attachmentInfo.resolveTarget.Get();
                if (resolveTarget == nil) {
                ...
                if (shouldEmulateStoreAndMSAAResolve &&
                    ...
                    forceUnsetResolveTargetsSet.set(i);
                }
            }

            if (!forceUnsetResolveTargetsSet.any()) {
                return CreateMTLRenderPassDescriptorImpl(
                    renderPass, !shouldEmulateStoreAndMSAAResolve, resolveTexturesForWorkaround);
            }

            // Create a new BeginRenderPassCmd from the original one except the resolve target is
            // not set if the store operation of the color attachment is StoreOp::Store.
            BeginRenderPassCmd appliedRenderPass = *renderPass;
            for (uint32_t i : IterateBitSet(renderPass->colorAttachmentsSet)) {
                if (forceUnsetResolveTargetsSet.test(i)) {
        ...
        void ResolveInAnotherRenderPass(
            ...
            if (renderPassForResolve.colorAttachmentsSet.any()) {
                // All the members in BeginRenderPassCmd have not been initialized, so we do the
