...
#    error "Unsupported Apple platform."
    ...
    NSRef<MTLTextureDescriptor> CreateMetalTextureDescriptor(DeviceBase* device,
        ...
        [*mtlDesc setWidth:descriptor->size.width];
        [*mtlDesc setHeight:descriptor->size.height];
        [*mtlDesc setSampleCount:descriptor->sampleCount];
        // TODO: add MTLTextureUsagePixelFormatView when needed when we support format
        // reinterpretation.
        [*mtlDesc setUsage:MetalTextureUsage(device->GetValidInternalFormat(descriptor->format),
                                             descriptor->usage)];
        [*mtlDesc setPixelFormat:MetalPixelFormat(descriptor->format)];
        [*mtlDesc setMipmapLevelCount:descriptor->mipLevelCount];
        ...
        // is specified.
        [*mtlDesc setDepth:descriptor->size.depth];
        [*mtlDesc setArrayLength:1];
        switch (descriptor->dimension) {
            case wgpu::TextureDimension::e2D:
                if ([*mtlDesc depth] > 1) {
                    ASSERT([*mtlDesc sampleCount] == 1);
                    [*mtlDesc setTextureType:MTLTextureType2DArray];
                    [*mtlDesc setArrayLength:[*mtlDesc depth]];
                    [*mtlDesc setDepth:1];
                } else {
                    if ([*mtlDesc sampleCount] > 1) {
                        [*mtlDesc setTextureType:MTLTextureType2DMultisample];
                    } else {
