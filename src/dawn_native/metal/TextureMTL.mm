...
namespace dawn_native { namespace metal {
    ...
    Texture::Texture(Device* device,
        ...
        MTLTextureType textureViewType =  MTLTextureType2D;
        auto mipLevelRange = NSMakeRange(0, 1);
        auto arrayLayerRange = NSMakeRange(0, 1);
        mMtlTexture = [mMtlTexture newTextureViewWithPixelFormat:format
    ...
    TextureView::TextureView(TextureBase* texture, const TextureViewDescriptor* descriptor)
        ...
        if (!UsageNeedsTextureView(texture->GetUsage())) {
            ...
            MTLPixelFormat format = MetalPixelFormat(descriptor->format);
            if (descriptor->format == dawn::TextureFormat::RGBA8Unorm) {
                format = MetalPixelFormat(dawn::TextureFormat::BGRA8Unorm);
            } else {
                format = MetalPixelFormat(dawn::TextureFormat::RGBA8Unorm);
            }
            // MTLPixelFormat format = MetalPixelFormat(descriptor->format);
