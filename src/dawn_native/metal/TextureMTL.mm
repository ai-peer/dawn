...
namespace dawn_native { namespace metal {
    ...
    namespace {
        ...
        // linear space and sRGB without setting MTLTextureUsagePixelFormatView flag. For example,
        // create bgra8Unorm texture view on rgba8Unorm texture or create rgba8Unorm_srgb texture
        // view on rgab8Unorm texture.
...
#    error "Unsupported Apple platform."
    ...
    id<MTLTexture> Texture::GetCompatibleMTLTexture(wgpu::TextureFormat format) {
        ASSERT(GetFormat().format == format ||
               AllowFormatReinterpreterWithoutFlag(MetalPixelFormat(GetFormat().format),
