    ...
    MaybeError Queue::WriteTextureImpl(const TextureCopyView* destination,
        ...
        if (dataLayout->bytesPerRow % destination->texture->GetFormat().blockByteSize != 0) {
            return DAWN_VALIDATION_ERROR(
                "In Metal bytesPerRow must be a multiple of texel block size");
        }
        ...
        return ToBackend(device)->CopyFromStagingToTexture(
            uploadHandle.stagingBuffer, passDataLayout, &textureCopy, *writeSize);
    }
