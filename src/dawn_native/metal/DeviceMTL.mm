...
namespace dawn_native { namespace metal {
    ...
    float Device::GetTimestampPeriodInNS() const {
        if (@available(macos 10.15, iOS 14.0, *)) {
            // CPU timestamps are measured in nanosenconds, we can calculate a conversion factor
            ...
            if (cpuTimestamp > mCpuTimestamp && gpuTimestamp > mGpuTimestamp) {
                const double cpuDelta = cpuTimestamp - mCpuTimestamp;
                const double gpuDelta = gpuTimestamp - mGpuTimestamp;

                // Average with the previous result to converge to the accurate value.
                return mTimestampPeriod == 1.0f ? cpuDelta / gpuDelta
