...
namespace dawn_native { namespace metal {
    ...
    namespace {
        // Since CGDisplayIOServicePort was deprecated in macOS 10.9, we need a substitute function.
        io_service_t IOServicePortFromCGDisplayID(CGDirectDisplayID displayID) {
            // The matching service port (or 0 if none can be found)
            io_service_t serv, servicePort = 0;
            io_iterator_t iter;

            // Get matching display services
            ...
            // Create the iterator with all matching devices
            ...
            while ((serv = IOIteratorNext(iter))) {
                ...
                // Create information for the service
                ...
                // Check if vendor id and product id match with current display
    ...
    void Device::CollectPCIInfo() {
        io_registry_entry_t entry = IOServicePortFromCGDisplayID(kCGDirectMainDisplay);
        if (entry) {
