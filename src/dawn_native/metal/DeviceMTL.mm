...
namespace dawn_native { namespace metal {
    ...
    MaybeError Device::CopyFromStagingToTexture(StagingBufferBase* source,
        ...
        uint32_t clampedCopyExtentWidth =
            (dst->origin.x + copySize.width > virtualSizeAtLevel.width)
                ? (virtualSizeAtLevel.width - dst->origin.x)
                : copySize.width;
        uint32_t clampedCopyExtentHeight =
            (dst->origin.y + copySize.height > virtualSizeAtLevel.height)
                ? (virtualSizeAtLevel.height - dst->origin.y)
                : copySize.height;
        ...
        for (uint32_t copyLayer = copyBaseLayer; copyLayer < copyBaseLayer + copyLayerCount;
            ...
            [GetPendingCommandContext()->EnsureBlit()
                ...
                sourceBytesPerImage:bytesPerImage
                         sourceSize:MTLSizeMake(clampedCopyExtentWidth, clampedCopyExtentHeight, 1)
