...
namespace dawn_native { namespace metal {
    ...
    namespace {
        ...
        ResultOrError<PCIIDs> GetDeviceIORegistryPCIInfo(id<MTLDevice> device) {
            // MTLDevice.registryID was introduced in the macOS 10.13 SDK but Chromium still builds
            // against the 10.12 SDK, so we use ObjectiveC reflection data to extract the property.
            // TODO(cwallez@chromium.org): Remove this hack when Chromium rolls the 10.13 SDK
            NSNumber* registryIDProperty = [reinterpret_cast<NSObject*>(device) valueForKey:@"registryID"];
            uint64_t registryID = static_cast<uint64_t>(registryIDProperty.unsignedLongLongValue);
            [registryIDProperty release];
