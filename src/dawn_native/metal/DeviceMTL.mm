...
namespace dawn_native { namespace metal {
    ...
    MaybeError Device::TickImpl() {
        if (mCommandContext.GetCommands() != nil) {
            ...
            // there is a new callback that we need to address or we wait forever
    ...
    MaybeError Device::WaitForIdleForDestruction() {
        ...
        // If there was a pending callback, TickImpl would have submitted command context, we need to assume commands complete again
        if (device->HasNewCallback()) {
            AssumeCommandsComplete();
        }
