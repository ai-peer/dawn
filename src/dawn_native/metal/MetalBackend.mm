...
namespace dawn_native { namespace metal {
    ...
    dawnTexture WrapIOSurface(dawnDevice cDevice, const dawnTextureDescriptor* cDescriptor, IOSurfaceRef ioSurface, uint32_t plane) {
        Device* device = reinterpret_cast<Device*>(cDevice);
        const TextureDescriptor* descriptor = reinterpret_cast<const TextureDescriptor*>(cDescriptor);
