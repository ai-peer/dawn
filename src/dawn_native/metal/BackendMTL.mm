...
namespace dawn_native { namespace metal {
    ...
    std::vector<std::unique_ptr<AdapterBase>> Backend::DiscoverDefaultAdapters() {
        ...
        // If there is a single device in the system then we can get its PCI info through the
        // display in the IORegistry.
        // TODO(cwallez@chromium.org): Once Chromium has the macOS 10.13 SDK rolled, we should
        // use MTLDevice.registryID to gather the information.
        if (adapters.size() == 1) {
            ToBackend(adapters[0].get())->GetPCIInfoFromDisplay();
        }
