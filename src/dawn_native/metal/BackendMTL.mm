...
#define FEATURE_INFO(X, ...)                                                                                                              \
    X(__VA_ARGS__, uint32_t,          maxVertexAttribsPerDescriptor,    31,    31,    31,    31,    31,    31,    31,     31,    31 )  \
    X(__VA_ARGS__, uint32_t,        maxBufferArgumentEntriesPerFunc,    31,    31,    31,    31,    31,    31,    31,     31,    31 )  \
    X(__VA_ARGS__, uint32_t,       maxTextureArgumentEntriesPerFunc,    31,    31,    31,    96,    96,   128,   128,    128,   128 )  \
    X(__VA_ARGS__, uint32_t,  maxSamplerStateArgumentEntriesPerFunc,    16,    16,    16,    16,    16,    16,    16,     16,    16 )  \
    X(__VA_ARGS__, uint32_t,               maxThreadsPerThreadgroup,   512,   512,   512,  1024,  1024,  1024,  1024,   1024,  1024 )  \
    X(__VA_ARGS__, uint32_t,              maxTotalThreadgroupMemory, 16352, 16352, 16384, 32768, 32768, 32768, 32768,  32768, 32768 )  \
    X(__VA_ARGS__, uint32_t,             maxFragmentInputComponents,    60,    60,    60,   124,   124,   124,   124,    124,   124 )  \
    X(__VA_ARGS__, uint32_t,                       max1DTextureSize,  8192,  8192, 16384, 16384, 16384, 16384, 16384,  16384, 16384 )  \
    X(__VA_ARGS__, uint32_t,                       max2DTextureSize,  8192,  8192, 16384, 16384, 16384, 16384, 16384,  16384, 16384 )  \
    X(__VA_ARGS__, uint32_t,                       max3DTextureSize,  2048,  2048,  2048,  2048,  2048,  2048,  2048,   2048,  2048 )  \
    X(__VA_ARGS__, uint32_t,                  maxTextureArrayLayers,  2048,  2048,  2048,  2048,  2048,  2048,  2048,   2048,  2048 )  \
    X(__VA_ARGS__, uint32_t,               minBufferOffsetAlignment,     4,     4,     4,     4,     4,     4,     4,    256,   256 )

// Sets |name| to the first column value.
#define POPULATE_FEATURE_INFO_1(out, type, name, val, ...) \
    out.name = static_cast<type>(val);

// Sets |name| to the ith column value by consuming the first argument and forwarding to the (i-1)th macro.
#define POPULATE_FEATURE_INFO_2(out, type, name, v, ...) POPULATE_FEATURE_INFO_1(out, type, name, __VA_ARGS__, dummy)
#define POPULATE_FEATURE_INFO_3(out, type, name, v, ...) POPULATE_FEATURE_INFO_2(out, type, name, __VA_ARGS__, dummy)
#define POPULATE_FEATURE_INFO_4(out, type, name, v, ...) POPULATE_FEATURE_INFO_3(out, type, name, __VA_ARGS__, dummy)
#define POPULATE_FEATURE_INFO_5(out, type, name, v, ...) POPULATE_FEATURE_INFO_4(out, type, name, __VA_ARGS__, dummy)
#define POPULATE_FEATURE_INFO_6(out, type, name, v, ...) POPULATE_FEATURE_INFO_5(out, type, name, __VA_ARGS__, dummy)
#define POPULATE_FEATURE_INFO_7(out, type, name, v, ...) POPULATE_FEATURE_INFO_6(out, type, name, __VA_ARGS__, dummy)
#define POPULATE_FEATURE_INFO_8(out, type, name, v, ...) POPULATE_FEATURE_INFO_7(out, type, name, __VA_ARGS__, dummy)
#define POPULATE_FEATURE_INFO_9(out, type, name, v, ...) POPULATE_FEATURE_INFO_8(out, type, name, __VA_ARGS__, dummy)
// clang-format on
...
#define MTL_FEATURE_INFO_MEMBER(dummy, type, name, ...) type name;
...
#define SWITCH_CASE(Family, Col)                                  \
    case MTLGPUFamily::Family:                                    \
        FEATURE_INFO(POPULATE_FEATURE_INFO_##Col, mtlFeatureInfo) \
        break;
...
#undef SWITCH_CASE
            ...
            limits->v1.maxInterStageShaderComponents = mtlFeatureInfo.maxFragmentInputComponents;

            limits->v1.maxComputeWorkgroupStorageSize = mtlFeatureInfo.maxTotalThreadgroupMemory;
            limits->v1.maxComputeInvocationsPerWorkgroup = mtlFeatureInfo.maxThreadsPerThreadgroup;
            ...
            limits->v1.minUniformBufferOffsetAlignment = mtlFeatureInfo.minBufferOffsetAlignment;
            limits->v1.minStorageBufferOffsetAlignment = mtlFeatureInfo.minBufferOffsetAlignment;

            // Metal has no limit on the size of a binding.
            limits->v1.maxUniformBufferBindingSize = std::numeric_limits<uint64_t>::max();
            limits->v1.maxStorageBufferBindingSize = std::numeric_limits<uint64_t>::max();
