...
namespace dawn_native {
    ...
    namespace {
        ...
        MaybeError ValidateVertexBufferLayoutDescriptor(
            ...
            if (buffer->arrayStride > kMaxVertexBufferStride) {
                return DAWN_VALIDATION_ERROR("Setting input arrayStride out of bounds");
            }
            ...
            for (uint32_t i = 0; i < buffer->attributeCount; ++i) {
                DAWN_TRY(ValidateVertexAttributeDescriptor(&buffer->attributes[i], buffer->arrayStride,
                                                           attributesSetMask));
            }
    ...
    RenderPipelineBase::RenderPipelineBase(DeviceBase* device,
        ...
        for (uint32_t slot = 0; slot < mVertexState.vertexBufferCount; ++slot) {
            ...
            mInputInfos[slot].arrayStride = mVertexState.vertexBuffers[slot].arrayStride;
            mInputInfos[slot].stepMode = mVertexState.vertexBuffers[slot].stepMode;
            ...
            for (uint32_t i = 0; i < mVertexState.vertexBuffers[slot].attributeCount; ++i) {
                ...
                mAttributeInfos[location].offset = mVertexState.vertexBuffers[slot].attributes[i].offset;
                mAttributeInfos[location].format = mVertexState.vertexBuffers[slot].attributes[i].format;
            }
        }
