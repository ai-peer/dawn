...
namespace dawn_native {
    ...
    ResultOrError<ShaderModuleBase*> RenderPipelineBase::GetEmptyFragmentShaderModule(
        DeviceBase* device) {
        InternalPipelineStore* store = device->GetInternalPipelineStore();
        if (store->emptyFragmentShader == nullptr) {
            ShaderModuleDescriptor descriptor;
            ShaderModuleWGSLDescriptor wgslDesc;
            wgslDesc.source = sEmptyFragmentShader;
            descriptor.nextInChain = reinterpret_cast<ChainedStruct*>(&wgslDesc);

            DAWN_TRY_ASSIGN(store->emptyFragmentShader, device->CreateShaderModule(&descriptor));
        }

        ShaderModuleBase* shaderModule = store->emptyFragmentShader.Get();
