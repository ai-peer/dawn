...
namespace dawn_native {
    ...
    RenderPipelineBase::RenderPipelineBase(DeviceBase* device,
        ...
        if (descriptor->vertexState != nullptr) {
            ...
            mBufferCount = vertexState.vertexBufferCount;
            mPrimitive.stripIndexFormat = vertexState.indexFormat;

            for (uint8_t slot = 0; slot < mBufferCount; ++slot) {
    ...
    size_t RenderPipelineBase::ComputeContentHash() {
        ...
        for (ColorAttachmentIndex i : IterateBitSet(mAttachmentState->GetColorAttachmentsMask())) {
            ...
            if (desc.blend != nullptr) {
                ...
                recorder.Record(wgpu::BlendOperation::Add, wgpu::BlendFactor::One,
        ...
        recorder.Record(mPrimitive.stripIndexFormat);

        // Record rasterization state
        {
            // const RasterizationStateDescriptor& desc = mRasterizationState;
            recorder.Record(mPrimitive.frontFace, mPrimitive.cullMode);
            recorder.Record(mDepthStencil.depthBias, mDepthStencil.depthBiasSlopeScale,
                            mDepthStencil.depthBiasClamp);
        }

        // Record other state
        recorder.Record(mPrimitive.topology, mMultisample.mask,
                        mMultisample.alphaToCoverageEnabled);
    ...
    bool RenderPipelineBase::EqualityFunc::operator()(const RenderPipelineBase* a,
        ...
        for (ColorAttachmentIndex i :
            ...
            } else if ((descA.blend == nullptr && descB.blend != nullptr) ||
                       (descA.blend != nullptr && descB.blend == nullptr)) {
                return false;
            }
        }
