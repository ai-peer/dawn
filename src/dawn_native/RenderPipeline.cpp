...
namespace dawn_native {
    ...
    namespace {
        ...
        MaybeError ValidateVertexBufferDescriptor(
            ...
            if (buffer->numAttributes > kMaxVertexAttributes) {
                return DAWN_VALIDATION_ERROR("Vertex Attributes number exceeds maximum");
            }

            for (uint32_t i = 0; i < buffer->numAttributes; ++i) {
                DAWN_TRY(
                    ValidateVertexAttributeDescriptor(&buffer->attributes[i], attributesSetMask));
            }
    ...
    RenderPipelineBase::RenderPipelineBase(DeviceBase* device,
        ...
        for (uint32_t i = 0; i < mVertexInput.numBuffers; ++i) {
            ...
            mInputInfos[slot].stride = mVertexInput.buffers[i].stride;
            mInputInfos[slot].stepMode = mVertexInput.buffers[i].stepMode;

            uint32_t location = 0;
            for (uint32_t j = 0; j < mVertexInput.buffers[i].numAttributes; ++j) {
                location = mVertexInput.buffers[i].attributes[j].shaderLocation;
                mAttributesSetMask.set(location);
                mAttributeInfos[location].shaderLocation = location;
                mAttributeInfos[location].inputSlot = i;
                mAttributeInfos[location].offset = mVertexInput.buffers[i].attributes[j].offset;
                mAttributeInfos[location].format = mVertexInput.buffers[i].attributes[j].format;
            }
        }
