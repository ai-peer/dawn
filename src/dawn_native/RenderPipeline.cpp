...
namespace dawn_native {
    ...
    namespace {
        ...
        MaybeError ValidateColorRenderableTextureFormat(dawn::TextureFormat format) {
            switch (format) {
                ...
                case dawn::TextureFormat::D32FloatS8Uint:
                    return DAWN_VALIDATION_ERROR("Texture format not renderable");

                default:
                    return DAWN_VALIDATION_ERROR("Invalid value for dawnTextureFormat");
            }
        }

        MaybeError ValidateDepthStencilTextureFormat(dawn::TextureFormat format) {
            switch (format) {
                case dawn::TextureFormat::B8G8R8A8Unorm:
                case dawn::TextureFormat::R8G8B8A8Uint:
                case dawn::TextureFormat::R8G8B8A8Unorm:
                ...
                case dawn::TextureFormat::R8Uint:
                case dawn::TextureFormat::R8Unorm:
                    return DAWN_VALIDATION_ERROR("Texture format not a depth stencil format");
        ...
        MaybeError ValidateAttachmentsStateDescriptor(
            ...
            if (descriptor->hasDepthStencilAttachment) {
                DAWN_TRY(ValidateDepthStencilTextureFormat(descriptor->depthStencilAttachment->format));
            }

            for (uint32_t i = 0; i < descriptor->numColorAttachments; ++i) {
                DAWN_TRY(ValidateColorRenderableTextureFormat(descriptor->colorAttachments[i].format));
            }
            return {};
        }
    ...
    RenderPipelineBase::RenderPipelineBase(DeviceBase* device,
        ...
        : PipelineBase(device,
          ...
          mHasDepthStencilAttachment(
              descriptor->attachmentsState->hasDepthStencilAttachment) {
          if (mHasDepthStencilAttachment) {
              mDepthStencilFormat =
                  descriptor->attachmentsState->depthStencilAttachment->format;
        }
        ...
        for (uint32_t i = 0; i < descriptor->attachmentsState->numColorAttachments; ++i) {
            ...
            mColorAttachmentFormats[i] =
                descriptor->attachmentsState->colorAttachments[i].format;
        }
