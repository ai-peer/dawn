...
namespace dawn_native {
    ...
    namespace {
        ...
        MaybeError ValidateVertexAttribute(
            ...
            ASSERT(kMaxVertexBufferArrayStride >= formatInfo.size);
            if (attribute->offset > kMaxVertexBufferArrayStride - formatInfo.size) {
            ...
            if (vertexBufferStride > 0 &&
                attribute->offset + formatInfo.size > vertexBufferStride) {
                return DAWN_VALIDATION_ERROR("Setting attribute offset out of bounds");
            }

            if (attribute->offset % formatInfo.componentSize != 0) {
                return DAWN_VALIDATION_ERROR(
                    "Attribute offset needs to be a multiple of the size format's components");
            }

            if (formatInfo.baseType != metadata.vertexInputBaseTypes[location]) {
                return DAWN_VALIDATION_ERROR(
