...
namespace dawn_native {
    ...
    namespace {
        ...
        BufferSizesArray ComputeBufferSizes(const RenderPipelineDescriptor* descriptor) {
            BufferSizesArray bufferSizes =
                descriptor->vertexStage.module->GetPipelineMinBufferSizes(descriptor->layout);

            // Merge the two buffer size requirements by taking the larger element from each
            if (descriptor->fragmentStage != nullptr) {
                BufferSizesArray fragmentSizes =
                    descriptor->fragmentStage->module->GetPipelineMinBufferSizes(
                        descriptor->layout);
