...
namespace dawn_native {
    ...
    wgpu::VertexFormat NormalizeVertexFormat(wgpu::VertexFormat format) {
        ...
        uint32_t firstDeprecatedFormat = static_cast<uint32_t>(wgpu::VertexFormat::UChar2);
        if (formatValue >= firstDeprecatedFormat) {
            return static_cast<wgpu::VertexFormat>(formatValue - firstDeprecatedFormat);
        }
        return format;
    }
    ...
    RenderPipelineBase::RenderPipelineBase(DeviceBase* device,
        ...
        for (uint8_t slot = 0; slot < mVertexState.vertexBufferCount; ++slot) {
            ...
            for (uint32_t i = 0; i < mVertexState.vertexBuffers[slot].attributeCount; ++i) {
                ...
                if (mVertexState.vertexBuffers[slot].attributes[i].format >
                ...
                mAttributeInfos[location].format =
                    NormalizeVertexFormat(mVertexState.vertexBuffers[slot].attributes[i].format);
            }
        }
