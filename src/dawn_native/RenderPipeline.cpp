...
#include "utils/VertexFormatUtils.h"
...
namespace dawn_native {
    ...
    namespace {
        ...
        MaybeError ValidateVertexAttributeDescriptor(
            ...
            if (utils::IsDeprecatedVertexFormat(attribute->format)) {
            ...
            ASSERT(kMaxVertexBufferStride >= utils::VertexFormatSize(attribute->format));
            if (attribute->offset >
                kMaxVertexBufferStride - utils::VertexFormatSize(attribute->format)) {
            ...
            if (vertexBufferStride > 0 &&
                attribute->offset + utils::VertexFormatSize(attribute->format) >
                    vertexBufferStride) {
                return DAWN_VALIDATION_ERROR("Setting attribute offset out of bounds");
            }
    ...
    RenderPipelineBase::RenderPipelineBase(DeviceBase* device,
        ...
        for (uint8_t slot = 0; slot < mVertexState.vertexBufferCount; ++slot) {
            ...
            for (uint32_t i = 0; i < mVertexState.vertexBuffers[slot].attributeCount; ++i) {
                ...
                mAttributeInfos[location].format = utils::NormalizeVertexFormat(
