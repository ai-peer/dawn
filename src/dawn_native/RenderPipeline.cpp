...
namespace dawn_native {
    ...
    namespace {
        MaybeError ValidatePipelineStageDescriptor(const PipelineStageDescriptor* descriptor,
            ...
            if (descriptor->entryPoint != std::string("main")) {
                return DAWN_VALIDATION_ERROR("We only support main() as entry point now");
            }
        ...
        MaybeError ValidateAttachmentsStateDescriptor(const AttachmentsStateDescriptor* descriptor) {
            ...
            if (descriptor->numColorAttachments == 0 &&
            ...
            if (descriptor->hasDepthStencilAttachment) {
                DAWN_TRY(ValidateTextureFormat(
                    descriptor->depthStencilAttachment->format));
            }

            for (uint32_t i = 0; i < descriptor->numColorAttachments; ++i) {
                DAWN_TRY(ValidateTextureFormat(
                    descriptor->colorAttachments[i].format));             
            }
            return {};
        }
    ...
    MaybeError ValidateRenderPipelineDescriptor(DeviceBase* device,
        ...
        DAWN_TRY(ValidatePipelineStageDescriptor(descriptor->vertexStage,
                                                 descriptor->layout,
                                                 dawn::ShaderStage::Vertex));
        DAWN_TRY(ValidatePipelineStageDescriptor(descriptor->fragmentStage,
                                                 descriptor->layout,
                                                 dawn::ShaderStage::Fragment));
        DAWN_TRY(ValidateAttachmentsStateDescriptor(descriptor->renderAttachmentsState));
    ...
    dawn::TextureFormat RenderPipelineBase::GetDepthStencilFormat() const {
        // If no depth stencil format attachment, return our default value.
        if (!mHasDepthStencilAttachment) {
            return dawn::TextureFormat::InvalidFormat;
        }
