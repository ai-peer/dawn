    ...
    RenderPipelineBase::RenderPipelineBase(DeviceBase* device,
        ...
        for (uint8_t slot = 0; slot < mVertexBufferCount; ++slot) {
            ...
            for (uint32_t i = 0; i < buffers[slot].attributeCount; ++i) {
                ...
                // Compute the access boundary of this attribute. offset is already in uint64_t, but
                // it must be no larger than maxVertexBufferArrayStride, which is promised by the
