    ...
    namespace {
        ...
        MaybeError ValidateVertexStateDescriptor(
            ...
            if (descriptor->indexFormat == wgpu::IndexFormat::Undefined) {
                ...
                // TODO(bajones@chromium.org): Once setIndexBuffer is always required to pass the
                // index format this should be updated to require that the index format is
                // undefined for non-strip topologies. Until then we must require that it's non
                return DAWN_VALIDATION_ERROR("indexFormat must not be undefined");
            }
