...
namespace dawn_native {
    ...
    namespace {
        ...
        std::array<std::vector<uint64_t>, kMaxBindGroups> ComputeBufferSizes(
            const RenderPipelineDescriptor* descriptor) {
            std::array<std::vector<uint64_t>, kMaxBindGroups> bufferSizes =
                descriptor->vertexStage.module->GetPipelineBufferSizes(descriptor->layout);

            // Merge the two buffer size requirements by taking the larger element from each
            if (descriptor->fragmentStage != nullptr) {
                std::array<std::vector<uint64_t>, kMaxBindGroups> fragmentSizes =
                    descriptor->fragmentStage->module->GetPipelineBufferSizes(descriptor->layout);

                for (size_t group = 0; group < bufferSizes.size(); ++group) {
