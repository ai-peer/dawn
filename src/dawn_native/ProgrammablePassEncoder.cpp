...
namespace dawn_native {
    ...
    void ProgrammablePassEncoder::SetBindGroup(uint32_t groupIndex,
        ...
        mEncodingContext->TryEncode(this, [&](CommandAllocator* allocator) -> MaybeError {
            ...
            DAWN_VALIDATE_FALSE(groupIndex >= kMaxBindGroups, "Setting bind group over the max");

            // Dynamic offsets count must match the number required by the layout perfectly.
            DAWN_VALIDATE_FALSE(layout->GetDynamicBufferCount() != dynamicOffsetCount,
                                "dynamicOffset count mismatch");

            for (uint32_t i = 0; i < dynamicOffsetCount; ++i) {
                DAWN_VALIDATE_FALSE(dynamicOffsets[i] % kMinDynamicBufferOffsetAlignment != 0,
                                    "Dynamic Buffer Offset need to be aligned");
