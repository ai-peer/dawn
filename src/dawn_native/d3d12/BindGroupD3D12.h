...
namespace dawn_native { namespace d3d12 {
    ...
    class BindGroup : public BindGroupBase {
      public:
        ...
        ResultOrError<bool> Populate(DescriptorAllocatorManager* allocator);

        D3D12_GPU_DESCRIPTOR_HANDLE GetBaseCbvUavSrvDescriptor() const;
        D3D12_GPU_DESCRIPTOR_HANDLE GetBaseSamplerDescriptor() const;

      private:
        bool IsAllocationStillValid(Serial completedSerial, Serial heapSerial) const;

        Serial mPendingSerial = 0;
