...
namespace dawn_native { namespace d3d12 {
    ...
    MaybeError RenderPipeline::Initialize(const RenderPipelineDescriptor* descriptor) {
        ...
        for (auto stage : IterateStages(renderStages)) {
            ...
            DAWN_TRY_ASSIGN(hlslSource, modules[stage]->GetHLSL(GetStage(stage).entryPoint.c_str(),
                                                                stage, ToBackend(GetLayout())));

            if (device->IsToggleEnabled(Toggle::UseDXC)) {
                DAWN_TRY_ASSIGN(
                    compiledDXCShader[stage],
                    CompileShaderDXC(device, stage, hlslSource, entryPoints[stage], compileFlags));
                ...
                DAWN_TRY_ASSIGN(
                    ...
                    CompileShaderFXC(device, stage, hlslSource, entryPoints[stage], compileFlags));
