...
namespace dawn_native { namespace d3d12 {
    ...
    class ResidencyManager {
      public:
        ...
        MaybeError ProcessResidency(
            const std::vector<ResourceHeapAllocation*>& pendingResidentList);

        void TrackResidentAllocation(ResourceHeapAllocation* resourceHeapAllocation);
        void TrackResidentAllocation(Heap* heap);

      private:
        LRUEntry* GetLRUEntry(ResourceHeapAllocation* resourceHeapAllocation);
