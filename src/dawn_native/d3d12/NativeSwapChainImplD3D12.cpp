...
namespace dawn_native { namespace d3d12 {
    ...
    DawnSwapChainError NativeSwapChainImpl::Configure(WGPUTextureFormat format,
        ...
        if (mSwapChain == nullptr) {
            InitializeDXGISwapChain(format, usage, width, height);
        } else {
            for (uint32_t i = 0; i < kFrameCount; ++i) {
                mBuffers[i] = nullptr;
            }
            HRESULT success = mSwapChain->ResizeBuffers(kFrameCount, width, height,
                                      D3D12TextureFormat(GetPreferredFormat()), 0);
    ...
    void NativeSwapChainImpl::InitializeDXGISwapChain(WGPUTextureFormat format,
                                                      WGPUTextureUsage usage,
                                                      uint32_t width,
                                                      uint32_t height) {
        ...
        // Create the D3D12 swapchain, assuming only three buffers for now
