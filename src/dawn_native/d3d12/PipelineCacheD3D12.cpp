...
namespace dawn_native { namespace d3d12 {
...
#if defined(_DEBUG)
            // Pipelines created from regenerated shaders using debug options will never match the
            // ones previously stored. Since we do not allow debug builds to persistently store
            // pipelines, this error can be ignored so testing can still work.
            if (result == E_INVALIDARG) {
                return {};
            }
#endif
    ...
    ResultOrError<Ref<PipelineCache>> SharedPipelineCaches::GetOrCreate(
        ...
        // Try to use an existing pipeline cache from this adapter.
        dawn_platform::ScopedCachedData pipelineCacheData = LoadPipelineCacheData(persistentCache);
        ...
        // Pipeline cache data doesn't exist, initialize an empty cached blob.
        if (pipelineCacheData.Get() == nullptr) {
            StorePipelineCacheData(nullptr, 0, persistentCache);
            pipelineCacheData = LoadPipelineCacheData(persistentCache);
        }
        ...
        // If the pipeline cache data cannot be stored in the persistent cache, do not share
        // this pipeline cache from this adapter. This is because pipelines added
        // from one device's cache could overwrite another device cache.
        if (pipelineCacheData.Get() != nullptr) {
    ...
    void PipelineCache::StorePipelineCacheData(PersistentCache* persistentCache) {
        ...
        // Debug builds use regenerated shaders which cannot be loaded back from the persistent
        // cache nor does the library allow duplicate pipelines to be stored.
#if defined(_DEBUG)
        if (mIsLibraryBlobDirty) {
            return;
        }
