...
namespace dawn_native { namespace d3d12 {
    ...
    dawn_platform::ScopedCachedBlob SharedPipelineCaches::LoadPipelineCacheData(
        PersistentCache* persistentCache) {
    ...
    ResultOrError<Ref<PipelineCache>> SharedPipelineCaches::GetOrCreate(
        ...
        PipelineCache blueprint(/*device*/ nullptr, this, pipelineCacheData);
        ...
        DAWN_TRY_ASSIGN(pipelineCache,
                        PipelineCache::Create(std::move(device), this, pipelineCacheData));

        // Ensure the pipeline cache data is saved as cached blob in the persistent cache.
        // If the pipeline cache data cannot be retireved from the persistent cache,
        // do not cache the pipeline cache on this adapter. This is because pipelines added
    ...
    MaybeError PipelineCache::Initialize() {
        ...
        // CreatePipelineLibrary. If no cached blob exists, CreatePipelineLibrary(data=nullptr,
        // size=0) initializes an empty library.
        ...
        // Creating a library from a cached blob does not require this pipeline
        // cache to be considered dirtied since it will be loaded with the same pipelines
    ...
    bool PipelineCache::SyncPipelineCacheData(PersistentCache* persistentCache) {
        // Library support is required to store pipelines back to the persistent cache.
        if (mLibrary == nullptr) {
            return false;
        }

        // Check if library data is already up-to-date. This happens if the pipeline cache data was
        // initialized from a cached blob or SyncPipelineCacheData was called without adding any new
        // pipelines to the pipeline cache.
        if (mLibraryBlob != nullptr && !mIsLibraryBlobDirty) {
            return true;
        }

        // Pipeline cache that contains debug built shaders does not need to be persistently stored
        // since debug shaders will be always re-compiled with new debug metadata unique to
        // compilation. If we call StorePipelines() using the same key but with a different
        // pipeline, Serialize() will fail since storing duplicates is not allowed. To avoid such
        // error but allow pipeline caching to work with debug builds for testing, we must always
        // forbid syncing after the pipeline cache gets initialized.
#if defined(_DEBUG)
        ...
        // If there is no ref, the pipeline cache cannot be synced.
        if (mLibraryBlob == nullptr) {
