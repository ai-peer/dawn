...
namespace dawn_native { namespace d3d12 {
    ...
    MaybeError Adapter::InitializeSupportedLimitsImpl(CombinedLimits* limits) {
        ...
        DAWN_TRY(CheckHRESULT(mD3d12Device->CheckFeatureSupport(
            D3D12_FEATURE_D3D12_OPTIONS, &featureData, sizeof(featureData))));
        ...
        uint32_t availableRootSignatureSlots =
            kMaxRootSignatureSize - kReservedSlots - 2 * limits->maxBindGroups -
            2 * (limits->maxDynamicUniformBuffersPerPipelineLayout +
        ...
        // Distribute evenly.
        limits->maxBindGroups += availableDynamicBufferOrBindGroup / 3;
        limits->maxDynamicUniformBuffersPerPipelineLayout += availableDynamicBufferOrBindGroup / 3;
        limits->maxDynamicStorageBuffersPerPipelineLayout +=
            (availableDynamicBufferOrBindGroup - 2 * (availableDynamicBufferOrBindGroup / 3));

        ASSERT(2 * limits->maxBindGroups +
                   2 * (limits->maxDynamicUniformBuffersPerPipelineLayout +
                        limits->maxDynamicStorageBuffersPerPipelineLayout) ==
