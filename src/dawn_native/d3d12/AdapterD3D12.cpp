...
Adapter::Adapter(Backend* backend, ComPtr<IDXGIAdapter1> hardwareAdapter)
            : AdapterBase(backend->GetInstance(), BackendType::D3D12), mHardwareAdapter(hardwareAdapter), mBackend(backend) {
        ...
        ResultOrError<DeviceBase*> Adapter::CreateDeviceImpl() {
            ...
            if (FAILED(mBackend->GetFunctions()->d3d12CreateDevice(mHardwareAdapter.Get(), D3D_FEATURE_LEVEL_11_0,
