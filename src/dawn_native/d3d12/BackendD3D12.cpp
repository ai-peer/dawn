...
namespace dawn_native { namespace d3d12 {
    ...
    namespace {
        ...
        ResultOrError<ComPtr<IDxcLibrary>> CreateDxcLibrary(const PlatformFunctions* functions) {
            ComPtr<IDxcLibrary> library;
            if (FAILED(functions->dxcCreateInstance(CLSID_DxcLibrary, IID_PPV_ARGS(&library)))) {
        ...
        ResultOrError<ComPtr<IDxcCompiler>> CreateDxcCompiler(const PlatformFunctions* functions) {
            ComPtr<IDxcCompiler> compiler;
            if (FAILED(functions->dxcCreateInstance(CLSID_DxcCompiler, IID_PPV_ARGS(&compiler)))) {
    ...
    ResultOrError<IDxcLibrary*> Backend::GetOrCreateDxcLibrary() {
        if (mDxcLibrary == nullptr) {
            DAWN_TRY_ASSIGN(mDxcLibrary, CreateDxcLibrary(mFunctions.get()));
        }
    ...
    ResultOrError<IDxcCompiler*> Backend::GetOrCreateDxcCompiler() {
        if (mDxcCompiler == nullptr) {
            DAWN_TRY_ASSIGN(mDxcCompiler, CreateDxcCompiler(mFunctions.get()));
        }
