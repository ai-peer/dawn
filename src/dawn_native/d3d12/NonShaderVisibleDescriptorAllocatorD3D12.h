...
// allocator. The SLOB allocator only needs the offset and index of the heap in the pool. The
// SLOB is backed by a list of blocks (free-list). The heap is in one of two states:
// AVAILABLE or not. To allocate, a block of the range [base + start, base + end] is removed from
// the free-list. The AVAILABLE heap always has room for at-least one block. If no available heap
// exists, a new heap is created and inserted back to the pool to be immediately used. To
// deallocate, the allocation is AVAILABLE by inserting a block back into the free-list.
//
    ...
    class NonShaderVisibleDescriptorAllocator {
      ...
      private:
        struct NonShaderVisibleBuffer {
            ...
            std::vector<uint64_t> freeAllocationIndicies;
        };

        MaybeError AllocateCPUHeap();

        std::vector<uint32_t> mAvailableHeaps;
