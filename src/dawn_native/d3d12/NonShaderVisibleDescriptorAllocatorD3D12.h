...
// "freed" by inserting a block back into the free-list. Each block represents the interval [base +
// start, base + end] in the heap which makes free-list insertion/removal trivial and fast
// operations (initially).
//
// The SLOB allocator uses a first-first algorithm. If it's a new heap, the block start is bumped-up
// by the block size; otherwise, the entire fix-sized block is allocated from the heap. The downside
// to this simple strategy is blocks can become heavily fragmented after heap space is exhausted and
// more sparse de-allocation occurs.
