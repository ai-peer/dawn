...
namespace dawn_native { namespace d3d12 {
    ...
    ResultOrError<ResourceHeapAllocation> ResourceAllocatorManager::CreatePlacedResource(
        D3D12_HEAP_TYPE heapType,
        const D3D12_RESOURCE_DESC& requestedDescriptor,
        D3D12_RESOURCE_STATES initialUsage) {
        const size_t resourceHeapKindIndex =
            GetResourceHeapKind(requestedDescriptor.Dimension, heapType, requestedDescriptor.Flags);
        ...
        D3D12_RESOURCE_DESC resourceDescriptor = requestedDescriptor;
        resourceDescriptor.Alignment =
            (resourceHeapKindIndex == Default_OnlyNonRenderableOrDepthTextures)
                ? D3D12_SMALL_RESOURCE_PLACEMENT_ALIGNMENT
                : requestedDescriptor.Alignment;

        D3D12_RESOURCE_ALLOCATION_INFO resourceInfo =
            mDevice->GetD3D12Device()->GetResourceAllocationInfo(0, 1, &resourceDescriptor);

        // If the request for small alignment was rejected, let D3D tell us what the required
        // alignment is for this resource.
