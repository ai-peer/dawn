...
namespace dawn_native { namespace d3d12 {
    ...
    MaybeError CommandBuffer::RecordCommands(CommandRecordingContext* commandContext,
        ...
        auto TransitionForPass = [](CommandRecordingContext* commandContext,
            ...
            bool passHasUAV = (bufferUsages & wgpu::BufferUsage::Storage ||
    ...
    void CommandBuffer::SetupRenderPass(CommandRecordingContext* commandContext,
        ...
        for (uint32_t i : IterateBitSet(renderPass->attachmentState->GetColorAttachmentsMask())) {
            ...
            bool requiresInitialization = attachmentInfo.storeOp == wgpu::StoreOp::Store;
            texture->SetIsSubresourceContentInitialized(
                requiresInitialization, view->GetBaseMipLevel(), 1, view->GetBaseArrayLayer(), 1);
        }
        ...
        if (renderPass->attachmentState->HasDepthStencilAttachment()) {
            ...
            if (attachmentInfo.depthStoreOp == wgpu::StoreOp::Clear) {
                if (hasStencil) {
                    ASSERT(attachmentInfo.stencilStoreOp == wgpu::StoreOp::Clear);
                }
                ...
                if (hasStencil) {
                    ASSERT(attachmentInfo.stencilStoreOp == wgpu::StoreOp::Store);
                }
    ...
    void CommandBuffer::RecordRenderPass(
        ...
        // We use D3D12's native render pass API if it's available, otherwise we emulate the
