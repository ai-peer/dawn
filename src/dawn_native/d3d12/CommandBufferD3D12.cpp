...
namespace dawn_native { namespace d3d12 {
    ...
    namespace {
        ...
        bool IsFullTextureToTextureCopy(const Extent3D& srcSize,
                                        const Extent3D& dstSize,
                                        const Extent3D& copySize) {
            if (srcSize.width == dstSize.width && srcSize.height == dstSize.height &&
                srcSize.depth == 1 && dstSize.depth == 1 && srcSize.width == copySize.width &&
        ...
        while (mCommands.NextCommandId(&type)) {
            switch (type) {
                ...
                case Command::CopyTextureToTexture: {
                    ...
                    if (IsFullTextureToTextureCopy(source->GetSize(), destination->GetSize(),
                                                   copy->copySize)) {
