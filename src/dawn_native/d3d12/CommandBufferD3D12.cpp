    ...
    class BindGroupStateTracker {
      public:
        ...
        void SetBindGroup(ComPtr<ID3D12GraphicsCommandList> commandList,
                          ...
                          bool force = false) {
            // Basically, application won't set the same offsets many times,
            // so always try to apply dynamic offsets even if the bind group stays the same
      ...
      private:
        ...
        std::array<std::array<uint64_t, kMaxDynamicOffsetPerGroup>, kMaxBindGroups>
            mLastDynamicOffsets;
