    ...
    class BindGroupStateTracker : public BindGroupAndStorageBarrierTrackerBase<false, uint64_t> {
      public:
        ...
        MaybeError Apply(CommandRecordingContext* commandContext) {
            ...
            bool boundHeapsChanged = false;
            for (uint32_t index : IterateBitSet(mDirtyBindGroupsObjectChangedOrIsDynamic)) {
                BindGroup* group = ToBackend(mBindGroups[index]);
                DAWN_TRY(ApplyBindGroup(commandList, ToBackend(mPipelineLayout), index, group,
                                        mDynamicOffsetCounts[index], mDynamicOffsets[index].data(),
                                        false, boundHeapsChanged));

                // If the recently allocated bind group belongs to a new heap, only the previously
                // applied groups must be re-applied on the new heap. Only one pair of GPU
                // heaps can be bound and both must contain all applied bind groups.
                if (boundHeapsChanged && mBindGroupsAllocated.count() > 0) {
                    reapplyBindGroups = true;
                }

                // Maintain a bitmask which tracks groups allocated on the same heap. Should the
                // bounded heaps change, only these groups will be re-applied on the new heap.
                if (!reapplyBindGroups) {
                    mBindGroupsAllocated.set(index);
                }
            }

            // Append the bind groups from the old heap to the newly bound heap.
            // This is inefficient as it re-applies both GPU heaps even if only one group spilled.
            // However, this only occurs when heap space is exhausted and unable to be reclaimed.
            // TODO(bryan.bernhart@intel.com): Consider optimizing this further.
            if (reapplyBindGroups) {
                // Only re-apply groups used by the layout.
                for (uint32_t index : IterateBitSet(mBindGroupsAllocated & mBindGroupLayoutsMask)) {
                    DAWN_TRY(
                        ApplyBindGroup(commandList, ToBackend(mPipelineLayout), index,
                                       ToBackend(mBindGroups[index]), mDynamicOffsetCounts[index],
                                       mDynamicOffsets[index].data(), true, boundHeapsChanged));
                }

                // Update the bitmask to count all groups upon being
                // re-applied. This is needed in-case another re-apply occurs.
                mBindGroupsAllocated = mDirtyBindGroupsObjectChangedOrIsDynamic;
            }
        ...
        // Returns true if the currently bound descriptor heaps changed.
        bool SetDescriptorHeaps(ID3D12GraphicsCommandList* commandList) {
            ...
            ID3D12DescriptorHeap* cbvSrvUavHeap =
                allocator->GetGPUDescriptorHeapInfo(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV)
                    .heap.Get();
            ID3D12DescriptorHeap* samplerHeap =
                allocator->GetGPUDescriptorHeapInfo(D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER).heap.Get();
      ...
      private:
        MaybeError ApplyBindGroup(ID3D12GraphicsCommandList* commandList,
            ...
            if (requiresAllocation ||
                !group->TestAndSetCounted(mDevice->GetPendingCommandSerial(), mIndexInSubmit)) {
                DAWN_TRY(group->AllocateDescriptors());
            }

            ASSERT(!requiresAllocation || !boundHeapsChanged);

            // Must be called after AllocateDescriptors, which lazily
            // allocates heaps but before the root table is set.
            boundHeapsChanged = SetDescriptorHeaps(commandList);
            ...
            if (!mDirtyBindGroups[index] && !requiresAllocation) {
                return {};
            }
        ...
        // Keep track of the allocated bind groups should they overflow the heap between draws so
        // these groups can be re-applied on the new heap.
        std::bitset<kMaxBindGroups> mBindGroupsAllocated = 0;
    };
