      ...
      public:
        ...
        MaybeError Apply(CommandRecordingContext* commandContext) {
            ...
            for (uint32_t index : IterateBitSet(mDirtyBindGroupsObjectChangedOrIsDynamic)) {
                ...
                // If the recently allocated bind group belongs to a new heap, previously applied
                // groups must be re-applied on the new heap since only one pair of GPU heaps can be
                // bound.
