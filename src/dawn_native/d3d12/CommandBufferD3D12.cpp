...
namespace dawn_native { namespace d3d12 {
    ...
    namespace {
        ...
        bool ShouldCopyUsingTemporaryBuffer(DeviceBase* device,
            ...
            if (srcCopy.mipLevel <= dstCopy.mipLevel) {
                return false;
            }

            const dawn_native::Format& format = srcCopy.texture->GetFormat();
            ASSERT(format.format == dstCopy.texture->GetFormat().format);

            return format.GetAspectInfo(srcCopy.aspect).block.byteSize < 4u;
        }
