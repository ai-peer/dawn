    ...
    class BindGroupStateTracker : public BindGroupAndStorageBarrierTrackerBase<false, uint64_t> {
      public:
        ...
        MaybeError Apply(CommandRecordingContext* commandContext) {
            ...
            DAWN_TRY_ASSIGN(didReallocation,
                            mDevice->AllocateBindGroups(mDirtyBindGroups, mBindGroupLayoutsMask,
                                                        mBindGroups, commandList));
            ...
            const bool didDescriptorHeapChange = SetID3D12DescriptorHeaps(commandList);
        ...
        bool SetID3D12DescriptorHeaps(ID3D12GraphicsCommandList* commandList) {
            ...
            std::array<ID3D12DescriptorHeap*, 2> descriptorHeaps = mDevice->GetShaderVisibleHeaps();
            if (descriptorHeaps[0] != mDescriptorHeaps[0] ||
                ...
                mDescriptorHeaps = mDevice->GetShaderVisibleHeaps();
                return true;
            }
            return false;
        }
        ...
        void ApplyBindGroup(ID3D12GraphicsCommandList* commandList,
            ...
            if (cbvUavSrvCount > 0) {
                ...
                const D3D12_GPU_DESCRIPTOR_HANDLE baseDescriptor =
                    group->GetCbvUavSrvHeapAllocation().GetGPUHandle(0);
                if (mInCompute) {
            ...
            if (samplerCount > 0) {
                ...
                const D3D12_GPU_DESCRIPTOR_HANDLE baseDescriptor =
                    group->GetSamplerHeapAllocation().GetGPUHandle(0);
                if (mInCompute) {
                    commandList->SetComputeRootDescriptorTable(parameterIndex, baseDescriptor);
                } else {
                    commandList->SetGraphicsRootDescriptorTable(parameterIndex, baseDescriptor);
