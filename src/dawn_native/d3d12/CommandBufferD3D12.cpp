    ...
    class BindGroupStateTracker : public BindGroupAndStorageBarrierTrackerBase<false, uint64_t> {
      public:
        ...
        MaybeError Apply(CommandRecordingContext* commandContext) {
            ...
            const Serial serial = mDevice->GetPendingCommandSerial();
            ...
            if (mDirtyBindGroups.any()) {
                ...
                if (reapplyBindGroups) {
                    ...
                    // If bindgroup allocations belong to a new heap, dirty all bindgroups so they
                    // may be re-populated + bound to the new heap offsets.
            ...
            for (uint32_t index : IterateBitSet(mDirtyBindGroupsObjectChangedOrIsDynamic)) {
                ...
                if (mDirtyBindGroups[index] && !group->TestCounted(serial)) {
                    // Bindgroups allocations in the descriptor heap do not get invalidated upon
                    // Tick(). To determine if a bindgroup still exists in the heap and does
                    // required to be encoded, the same |serial| when allocated is remembered.
                    // This works because the bindgroup allocation cannot outlive this serial while
                    // ensuring the bindgroup allocation can be uniquely identified to check if it
                    // already exists.
                    group->PopulateDescriptors(
                        serial, cbvSrvUavDescriptorHeapAllocation, &cbvSrvUavDescriptorIndex,
                        samplerDescriptorHeapAllocation, &samplerDescriptorIndex);
                }
