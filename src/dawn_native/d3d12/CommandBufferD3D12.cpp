...
namespace dawn_native { namespace d3d12 {
    ...
    namespace {
        ...
        OMSetRenderTargetArgs GetSubpassOMSetRenderTargetArgs(RenderPassDescriptorBase* renderPass) {
            OMSetRenderTargetArgs args = {};

            unsigned int rtvIndex = 0;
            for (uint32_t i : IterateBitSet(renderPass->GetColorAttachmentMask())) {
                args.RTVs[rtvIndex] = ToBackend(renderPass->GetColorAttachment(i).view)->GetRTVDescriptorHandle();
                rtvIndex++;
            }
            args.numRTVs = rtvIndex;

            if (renderPass->HasDepthStencilAttachment()) {
                args.dsv = ToBackend(renderPass->GetDepthStencilAttachment().view)->GetDSVDescriptorHandle();
            }
        ...
        // Clear framebuffer attachments as needed and transition to render target
        {
            for (uint32_t i : IterateBitSet(renderPass->GetColorAttachmentMask())) {
                ...
                if (attachmentInfo.loadOp == dawn::LoadOp::Clear) {
                    D3D12_CPU_DESCRIPTOR_HANDLE handle = ToBackend(attachmentInfo.view)->GetRTVDescriptorHandle();
            ...
            if (renderPass->HasDepthStencilAttachment()) {
                ...
                if (clearFlags) {
                    auto handle = ToBackend(renderPass->GetDepthStencilAttachment().view)->GetDSVDescriptorHandle();
