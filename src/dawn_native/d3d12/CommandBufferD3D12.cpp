...
namespace dawn_native { namespace d3d12 {
    ...
    void CommandBuffer::SetupNativeRenderPass(ComPtr<ID3D12GraphicsCommandList4> commandList,
        ...
        for (uint32_t i : IterateBitSet(renderPass->colorAttachmentsSet)) {
            ...
            if (attachmentInfo.loadOp == dawn::LoadOp::Clear) {
                D3D12_RENDER_PASS_BEGINNING_ACCESS renderPassBeginningAccess;

                renderPassBeginningAccess.Type = D3D12_RENDER_PASS_BEGINNING_ACCESS_TYPE_CLEAR;
                renderPassBeginningAccess.Clear.ClearValue.Format =
                    D3D12TextureFormat(texture->GetFormat());
                renderPassBeginningAccess.Clear.ClearValue.Color[0] = attachmentInfo.clearColor[0];
                renderPassBeginningAccess.Clear.ClearValue.Color[1] = attachmentInfo.clearColor[1];
                renderPassBeginningAccess.Clear.ClearValue.Color[2] = attachmentInfo.clearColor[2];
                renderPassBeginningAccess.Clear.ClearValue.Color[3] = attachmentInfo.clearColor[3];

                renderPassRenderTargetDesc.BeginningAccess = renderPassBeginningAccess;
        ...
        D3D12_RENDER_PASS_DEPTH_STENCIL_DESC* renderPassDepthStencilDescPtr = nullptr;
        ...
        if (renderPass->hasDepthStencilAttachment) {
            ...
            renderPassDepthStencilDescPtr = &renderPassDepthStencilDesc;
        ...
        commandList->BeginRenderPass(renderPassRenderTargetDescriptors.size(),
                                     renderPassRenderTargetDescriptors.data(),
                                     renderPassDepthStencilDescPtr, D3D12_RENDER_PASS_FLAG_NONE);
    }
    ...
    void CommandBuffer::RecordRenderPass(ComPtr<ID3D12GraphicsCommandList> commandList,
                                         ...
                                         RenderPassDescriptorHeapTracker* renderPassTracker,
                                         BeginRenderPassCmd* renderPass) {
        // Try to cat to ID3D12GraphicsCommandList4. This allows us to use the D3D12 Render Pass
        // APIs introduced in Windows build 1809. Versions of Windows prior to 1809 will fail to
        // cast to ID3D12GraphicsCommandList4.
        ...
        commandList4 = nullptr;
        if (commandList4 != nullptr) {
            SetupNativeRenderPass(commandList4, bindingTracker, renderPassTracker, renderPass,
                                  &args);
        } else {
            // If ID3D12GraphicsCommandList4 isn't available, we must emulate a render pass
            SetupEmulatedRenderPass(commandList, bindingTracker, renderPassTracker, renderPass,
                                    &args);
        }
