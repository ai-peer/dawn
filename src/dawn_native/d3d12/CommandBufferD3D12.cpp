...
namespace dawn_native { namespace d3d12 {
    ...
    namespace {
        ...
        MaybeError ClearBufferToValue(Device* device,
                                      Buffer* destination,
                                      uint64_t destinationOffset,
                                      uint64_t size,
                                      uint8_t value) {
            ...
            memset(uploadHandle.mappedBuffer, value, size);
            ...
            switch (type) {
                ...
                case Command::FillBuffer: {
                    ...
                    DAWN_TRY_ASSIGN(clearedToZero,
                                    dstBuffer->EnsureDataInitializedAsDestination(
                                        commandContext, cmd->destinationOffset, cmd->size));

                    if (!clearedToZero || cmd->value != 0u) {
                        DAWN_TRY(ClearBufferToValue(device, dstBuffer, cmd->destinationOffset,
                                                    cmd->size, cmd->value));
                    }
                ...
                case Command::ResolveQuerySet: {
                    ...
                    if (hasUnavailableQueries) {
                        DAWN_TRY(ClearBufferToValue(device, destination, destinationOffset,
                                                    queryCount * sizeof(uint64_t), 0u));
                    }
