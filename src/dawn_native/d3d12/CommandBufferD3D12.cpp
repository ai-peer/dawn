...
namespace dawn_native { namespace d3d12 {
    ...
    namespace {
        ...
        bool ShouldCopyUsingTemporaryBuffer(DeviceBase* device,
            ...
            if (srcCopy.mipLevel < 2u || dstCopy.mipLevel >= 2u) {
                return false;
            }

            wgpu::TextureFormat format = srcCopy.texture->GetFormat().format;
            ASSERT(format == dstCopy.texture->GetFormat().format);
        ...
        void RecordCopyTextureWithTemporaryBuffer(CommandRecordingContext* recordingContext,
            ...
            recordingContext->AddToTempBuffers(tempBuffer);
        }
    }  // anonymous namespace
