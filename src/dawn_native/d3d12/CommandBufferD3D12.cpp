        ...
        void ResolveMultisampledRenderPass(ComPtr<ID3D12GraphicsCommandList> commandList,
            ...
            for (uint32_t i : IterateBitSet(renderPass->colorAttachmentsSet)) {
                ...
                if (resolveTarget != nullptr) {
                    ...
                    D3D12_RESOURCE_STATES resolveTextureState = resolveTexture->GetLastResourceState();

                    const uint32_t resolveTextureSubresourceIndex =
                        GetSubresourceIndex(resolveTexture->GetNumMipLevels(),
                    ...
                    commandList->ResolveSubresource(resolveTextureHandle,
                                                    resolveTextureSubresourceIndex,
                                                    colorTextureHandle,
                                                    kColorTextureSubresourceIndex,
                                                    colorTexture->GetD3D12Format());

                    // TODO(jiawei.shao@intel.com): set these barriers only when we need to do so.
