...
namespace dawn_native { namespace d3d12 {
    ...
    class RenderPassDescriptorHeapTracker {
      public:
          RenderPassDescriptorHeapTracker(Device* device)
              : mDevice(device)
          {
          }
        ...
        OMSetRenderTargetArgs GetSubpassOMSetRenderTargetArgs(RenderPassDescriptor* renderPass) {
            ...
            for (uint32_t i : IterateBitSet(renderPass->GetColorAttachmentMask())) {
                auto &view = ToBackend(renderPass->GetColorAttachment(i).view);
            ...
            if (renderPass->HasDepthStencilAttachment()) {
                ...
                auto &view = ToBackend(renderPass->GetDepthStencilAttachment().view);
    ...
    void CommandBuffer::RecordCommands(ComPtr<ID3D12GraphicsCommandList> commandList,
        ...
        {
            AllocateAndSetDescriptorHeaps(
                device, &bindingTracker, &renderPassTracker, &mCommands, indexInSubmit);
