    ...
    void CommandBuffer::SetVertexBuffers(ComPtr<ID3D12GraphicsCommandList> commandList,
        ...
        if (vertexBuffersInfo->lastInputState != inputState) {
            ...
            for (uint32_t slot : IterateBitSet(inputsMask)) {
                ...
                vertexBuffersInfo->bufferViews[slot].StrideInBytes = inputState->GetInput(slot).stride;
            }
        }
        ...
        while (mCommands.NextCommandId(&type)) {
            switch (type) {
                ...
                case Command::SetVertexBuffers: {
                    ...
                    vertexBuffersInfo.startSlot = std::min(vertexBuffersInfo.startSlot, cmd->startSlot);
                    vertexBuffersInfo.endSlot = std::max(vertexBuffersInfo.endSlot, cmd->startSlot + cmd->count);
                    ...
                    for (uint32_t i = 0; i < cmd->count; ++i) {
                        ...
                        auto* bufferView = &vertexBuffersInfo.bufferViews[cmd->startSlot + i];
