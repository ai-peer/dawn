...
#include "dawn_native/BindGroupAndUsageTracker.h"
    ...
    class BindGroupStateTracker : public BindGroupTrackerBase<false> {
      public:
        ...
        void Apply(CommandRecordingContext* commandContext) {
            ...
            for (uint32_t bgl : IterateBitSet(mBindGroupLayoutsMask)) {
                for (uint32_t binding : IterateBitSet(mBuffersNeedingTransition[bgl])) {
                    dawn::BindingType bindingType = mBindingTypes[bgl][binding];
                    switch (bindingType) {
                        case dawn::BindingType::UniformBuffer:
                            ToBackend(mBuffers[bgl][binding])
                                ->TransitionUsageNow(commandContext, dawn::BufferUsage::Uniform);
                            break;

                        case dawn::BindingType::StorageBuffer:
                            ToBackend(mBuffers[bgl][binding])
                                ->TransitionUsageNow(commandContext, dawn::BufferUsage::Storage);
                            break;
