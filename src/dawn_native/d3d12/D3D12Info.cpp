    ...
    ResultOrError<D3D12DeviceInfo> GatherDeviceInfo(const Adapter& adapter) {
        ...
        for (D3D12_FEATURE_DATA_SHADER_MODEL shaderModel : knownShaderModels) {
            if (SUCCEEDED(adapter.GetDevice()->CheckFeatureSupport(
                ...
                info.shaderModel = shaderModel.HighestShaderModel;
                break;
            }
        }

        if (info.shaderModel < D3D_SHADER_MODEL_5_1) {
        ...
        ASSERT(info.shaderModel <= 0xFF);
        uint32_t shaderModelMajor = (info.shaderModel & 0xF0) >> 4;
        uint32_t shaderModelMinor = (info.shaderModel & 0xF);
