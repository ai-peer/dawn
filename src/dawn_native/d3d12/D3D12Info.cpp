...
namespace dawn_native { namespace d3d12 {
    ...
    ResultOrError<D3D12DeviceInfo> GatherDeviceInfo(const Adapter& adapter)
    {
        ...
        if (FAILED(functions->d3d12CreateDevice(adapter.GetHardwareAdapter(), D3D_FEATURE_LEVEL_11_0,
        ...
        // Gather info about device memory
        {
            ...
            // D3D_FEATURE_DATA_ARCHITECTURE1 is only enabled on newer Win10 builds
            // https://docs.microsoft.com/en-us/windows/desktop/api/d3d12/ne-d3d12-d3d12_feature 
            D3D12_FEATURE_DATA_ARCHITECTURE1 arch1 = {};
            if (FAILED(device->CheckFeatureSupport(D3D12_FEATURE_ARCHITECTURE1, &arch1, sizeof(arch1))))
            {
                D3D12_FEATURE_DATA_ARCHITECTURE arch = {};
                if (FAILED(device->CheckFeatureSupport(D3D12_FEATURE_ARCHITECTURE, &arch, sizeof(arch))))
                {
