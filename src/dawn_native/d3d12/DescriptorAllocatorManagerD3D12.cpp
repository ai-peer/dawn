...
namespace dawn_native { namespace d3d12 {
    ...
    MaybeError DescriptorAllocatorManager::AllocateAndSwitchShaderVisibleHeaps() {
    ...
    ResultOrError<DescriptorHeapAllocation> DescriptorAllocatorManager::AllocateGPUDescriptors(
        uint32_t descriptorCount,
        D3D12_DESCRIPTOR_HEAP_TYPE heapType) {
        ASSERT(heapType == D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV ||
               heapType == D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER);
        ASSERT(mShaderVisibleBuffers[heapType].heap != nullptr);
        // Check if heap has no more space. Caller will replace the heaps by calling
        // AllocateAndSwitchShaderVisibleHeaps() before calling AllocateGPUDescriptors() again.
        if (mShaderVisibleBuffers[heapType].remainingSize < descriptorCount) {
            return DescriptorHeapAllocation{};  // Invalid
        }
        // Linearly allocate from the heap.
        const uint32_t startOffset = mShaderVisibleBuffers[heapType].totalSize -
                                     mShaderVisibleBuffers[heapType].remainingSize;

        mShaderVisibleBuffers[heapType].remainingSize -= descriptorCount;
    ...
    MaybeError DescriptorAllocatorManager::AllocateGPUHeap(D3D12_DESCRIPTOR_HEAP_TYPE heapType,
        ...
        DAWN_TRY_ASSIGN(heap, CreateDescriptorHeap(descriptorCount, heapFlags, heapType));

        mShaderVisibleBuffers[heapType].heap = std::move(heap);
        mShaderVisibleBuffers[heapType].remainingSize = descriptorCount;
        mShaderVisibleBuffers[heapType].totalSize = descriptorCount;
    ...
    Serial DescriptorAllocatorManager::GetShaderVisibleHeapsSerial() const {
        return mShaderVisibleHeapsSerial;
    }

    uint64_t DescriptorAllocatorManager::GetShaderVisibleHeapSizeForTesting(
        D3D12_DESCRIPTOR_HEAP_TYPE heapType) const {
        ASSERT(heapType == D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV ||
               heapType == D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER);
        return mShaderVisibleBuffers[heapType].totalSize;
    }
