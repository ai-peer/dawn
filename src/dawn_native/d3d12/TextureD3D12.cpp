    ...
    MaybeError Texture::ClearTexture(CommandRecordingContext* commandContext,
        ...
        if (GetFormat().isRenderable) {
            if (GetFormat().HasDepthOrStencil()) {
                ...
                StagingDescriptorAllocator* rtvAllocator = device->GetDepthStencilViewAllocator();
                ...
                for (uint32_t level = baseMipLevel; level < baseMipLevel + levelCount; ++level) {
                    for (uint32_t layer = baseArrayLayer; layer < baseArrayLayer + layerCount;
                        ...
                        DAWN_TRY_ASSIGN(dsvHandle, rtvAllocator->AllocateCPUDescriptors());
                        ...
                        rtvAllocator->DeferredDeallocate(&dsvHandle);
                    }
                }
            } else {
                ...
                StagingDescriptorAllocator* dsvAllocator = device->GetRenderTargetViewAllocator();
                ...
                for (uint32_t level = baseMipLevel; level < baseMipLevel + levelCount; ++level) {
                    for (uint32_t layer = baseArrayLayer; layer < baseArrayLayer + layerCount;
                        ...
                        DAWN_TRY_ASSIGN(rtvHeap, dsvAllocator->AllocateCPUDescriptors());
                        ...
                        dsvAllocator->DeferredDeallocate(&rtvHeap);
                    }
                }
            }
        } else {
