...
namespace dawn_native { namespace d3d12 {
    ...
    MaybeError Texture::ClearTexture(ComPtr<ID3D12GraphicsCommandList> commandList,
        ...
        if (GetFormat().isRenderable) {
            ...
            uint32_t bufferSize = (GetSize().width / GetFormat().blockWidth) *
            ...
            DAWN_TRY_ASSIGN(uploadHandle,
                            uploader->Allocate(bufferSize, GetFormat().blockByteSize));
            ...
            TextureCopySplit copySplit =
                ComputeTextureCopySplit({0, 0, 0}, copySize, GetFormat(), 0, 0, 0);
            ...
            for (uint32_t i = 0; i < copySplit.count; ++i) {
                ...
                D3D12_TEXTURE_COPY_LOCATION bufferLocation =
                    ComputeBufferLocationForCopyTextureRegion(
                        ...
                        copySplit.offset,
                        Align(GetSize().width * GetFormat().blockByteSize,
