...
namespace dawn_native { namespace d3d12 {
    ...
    bool Texture::TransitionOneSubresource(std::vector<D3D12_RESOURCE_BARRIER>* barriers,
        ...
        if (subresourceValidToDecay && pendingCommandSerial > subresourceLastDecaySerial) {
            subresourceLastState = D3D12_RESOURCE_STATE_COMMON;
        }
        ...
        // PIXEL_SHADER_RESOURCE, COPY_SRC, COPY_DEST.
        {
            ...
            if (subresourceLastState == D3D12_RESOURCE_STATE_COMMON) {
                if (subresourceNewState == (subresourceNewState & kD3D12TextureReadOnlyStates)) {
                    ...
                    // read-only state.
                    subresourceLastDecaySerial = pendingCommandSerial;
                    subresourceLastState = subresourceNewState;
                    return true;
                } else if (subresourceNewState == D3D12_RESOURCE_STATE_COPY_DEST) {
                    subresourceLastState = subresourceNewState;
                    return false;
                }
            }
        ...
        barrier.Transition.StateBefore = subresourceLastState;
        barrier.Transition.StateAfter = subresourceNewState;
    ...
    void Texture::TransitionUsageAndGetResourceBarrier(
        ...
        for (uint32_t arrayLayer = 0; arrayLayer < layerCount; ++arrayLayer) {
            for (uint32_t mipLevel = 0; mipLevel < levelCount; ++mipLevel) {
                ...
                mSubresourceValidToDecay[index] = TransitionOneSubresource(
                    barriers, mSubresourceLastDecaySerial[index], mSubresourceLastStates[index],
                    mSubresourceValidToDecay[index], newState, index);
            }
        }
    }
