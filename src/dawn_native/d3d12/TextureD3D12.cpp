...
namespace dawn_native { namespace d3d12 {
    ...
    namespace {
        ...
        DXGI_FORMAT D3D12AllocationFormat(const D3D12Format& d3d12Format, wgpu::TextureUsage usage) {
            // Currently this only returns the base format if a Depth32Float texture has Sampled usage.
            // More format reinterpretation will be needed when WebGPU has texture view compatibility rules.
            if ((usage & wgpu::TextureUsage::Sampled) != 0 && d3d12Format.srvFormat != d3d12Format.format) {
                return d3d12Format.baseFormat;
            }
            return d3d12Format.format;
        }
    ...
    MaybeError ValidateD3D12TextureCanBeWrapped(const Device* device,
        ...
        const DXGI_FORMAT dxgiFormatFromDescriptor =
            device->GetD3D12Format(device->GetValidInternalFormat(dawnDescriptor->format)).format;
    ...
    MaybeError Texture::InitializeAsInternalTexture() {
        ...
        resourceDescriptor.Format = D3D12AllocationFormat(GetD3D12Format(), GetUsage());
    ...
    TextureView::TextureView(TextureBase* texture, const TextureViewDescriptor* descriptor)
        ...
        mSrvDesc.Format =
            device->GetD3D12Format(device->GetValidInternalFormat(descriptor->format)).srvFormat;
