...
namespace dawn_native { namespace d3d12 {
    ...
    bool Texture::SkipTransitionForReadonlyAndUncompressedTexture(uint32_t baseMipLevel,
                                                                  uint32_t levelCount,
                                                                  uint32_t baseArrayLayer,
                                                                  uint32_t layerCount) {
    ...
    void Texture::TransitionUsageAndGetResourceBarrier(
        ...
        if (SkipTransitionForReadonlyAndUncompressedTexture(baseMipLevel, levelCount, baseArrayLayer, layerCount) {
                SetStateForReadonlyAndUncompressedTexture(
                    D3D12TextureUsage(allUsages, GetFormat()));
                return;
	}
    ...
    void Texture::TrackUsageAndGetResourceBarrierForPass(
        ...
        bool readOnly = (allUsages & kReadOnlyTextureUsages) == allUsages;
        bool singleUse = wgpu::HasZeroOrOneBits(allUsages);
        if (readOnly || singleUse) {
            if (SkipTransitionForReadonlyAndUncompressedTexture(0, GetNumMipLevels(), 0, GetArrayLayers()) {
                    SetStateForReadonlyAndUncompressedTexture(
                        D3D12TextureUsage(allUsages, GetFormat()));
                    return;
	    }
