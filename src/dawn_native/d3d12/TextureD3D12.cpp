...
namespace dawn_native { namespace d3d12 {
    ...
    bool Texture::TransitionUsageAndGetResourceBarrier(D3D12_RESOURCE_BARRIER* barrier,
        ...
        // implicitly decay to the COMMON state when 1) the texture is accessed on a command list,
        // 2) the ExecuteCommandLists call that uses that command list has ended, and 3) the texture
        // was promoted implicitly to a read-only state and is still in that state.
        ...
        // common state.
        if (mValidToDecay && ToBackend(GetDevice())->GetPendingCommandSerial() > mNextDecaySerial) {
        ...
        // Destination states that qualify for an implicit promotion for a non-simulataneous-access
        // texture: NON_PIXEL_SHADER_RESOURCE, PIXEL_SHADER_RESOURCE, COPY_SRC, COPY_DEST.
        {
            ...
            if (lastState == D3D12_RESOURCE_STATE_COMMON) {
                if (newState == (newState & kD3D12TextureReadOnlyStates)) {
                    ...
                    SetNextDecaySerial(ToBackend(GetDevice())->GetPendingCommandSerial());
                    return false;
