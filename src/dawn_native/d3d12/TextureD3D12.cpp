...
namespace dawn_native { namespace d3d12 {
    ...
    Texture::Texture(Device* device, const TextureDescriptor* descriptor)
        ...
        if (device->IsToggleEnabled(Toggle::NonzeroClearResourcesOnCreationForTesting)) {
            ...
            D3D12_RENDER_TARGET_VIEW_DESC rtvDesc = GetRTVDescriptor(0);
    ...
    D3D12_RENDER_TARGET_VIEW_DESC Texture::GetRTVDescriptor(uint32_t mipSlice,
        ...
        if (IsMultisampledTexture()) {
            ASSERT(GetArrayLayers() == 1 && GetNumMipLevels() == 1 && baseArrayLayer == 0 &&
                   mipSlice == 0);
            ...
            rtvDesc.Texture2DArray.ArraySize = GetArrayLayers();
    ...
    D3D12_RENDER_TARGET_VIEW_DESC TextureView::GetRTVDescriptor() const {
        return ToBackend(GetTexture())->GetRTVDescriptor(GetBaseMipLevel(), GetBaseArrayLayer());
    }
