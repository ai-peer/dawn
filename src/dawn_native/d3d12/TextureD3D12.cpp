...
namespace dawn_native { namespace d3d12 {
    ...
    void Texture::TrackFullUsageAndTransitionNow(CommandRecordingContext* commandContext,
                                                 wgpu::TextureUsage usage) {
    ...
    bool Texture::SkipTransitionForReadonlyAndUncompressedTexture() {
        ...
        if (mValidToDecay && mLastUsedSerial < ToBackend(GetDevice())->GetPendingCommandSerial()) {
            return true;
        }
        return false;
    }

    void Texture::SetStateForReadonlyAndUncompressedTexture(D3D12_RESOURCE_STATES newState) {
        ...
        if (newState == (newState & kD3D12TextureReadOnlyStates)) {
            ...
            mValidToDecay = true;
            mLastUsedSerial = ToBackend(GetDevice())->GetPendingCommandSerial();
        } else {
            mValidToDecay = false;
        }
    }
    ...
    void Texture::TransitionUsageAndGetResourceBarrier(
        ...
        // Skip explicit transition barrier if the texture is uncompressed and its last usages are
        // readonly usages and the new usage(s) are readonly usages or single write usage.
        bool skipTransitionBarrier = SkipTransitionForReadonlyAndUncompressedTexture();
        ...
        for (uint32_t arrayLayer = 0; arrayLayer < layerCount; ++arrayLayer) {
            for (uint32_t mipLevel = 0; mipLevel < levelCount; ++mipLevel) {
                ...
                if (skipTransitionBarrier) {
                    mLastSubresourceStates[index] = newState;
                    continue;
                }

                // Build transition barrier.
        ...
        // Inspect the texture and set the state for readonly and uncompressed usages.
        D3D12_RESOURCE_STATES allStates = D3D12_RESOURCE_STATE_COMMON;
        for (uint32_t index = 0; index < GetSubresourceCount(); ++index) {
            allStates |= mLastSubresourceStates[index];
        }
        SetStateForReadonlyAndUncompressedTexture(allStates);
    }
    ...
    void Texture::TrackUsageAndGetResourceBarrierForPass(
        ...
        // Skip explicit transition barrier if the texture is uncompressed and its last usages are
        // readonly usages and the new usage(s) are readonly usages or single write usage.
        bool skipTransitionBarrier = false;
        if (((allUsages & kReadOnlyTextureUsages) == allUsages) ||
            wgpu::HasZeroOrOneBits(allUsages)) {
            skipTransitionBarrier = SkipTransitionForReadonlyAndUncompressedTexture();
        }

        // D3D12_RESOURCE_STATES allNewStates = D3D12_RESOURCE_STATE_COMMON;
        for (uint32_t arrayLayer = 0; arrayLayer < GetArrayLayers(); ++arrayLayer) {
            for (uint32_t mipLevel = 0; mipLevel < GetNumMipLevels(); ++mipLevel) {
                ...
                // Avoid transitioning the texture when it isn't needed.
                // TODO(cwallez@chromium.org): Need some form of UAV barriers at some point.
                if (newState == D3D12_RESOURCE_STATE_COMMON || lastState == newState) {
                    continue;
                }
                if (skipTransitionBarrier) {
                    mLastSubresourceStates[index] = newState;
        ...
        // Inspect the texture and set the state for readonly and uncompressed usages.
        SetStateForReadonlyAndUncompressedTexture(D3D12TextureUsage(allUsages, GetFormat()));
    }
