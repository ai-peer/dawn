...
namespace dawn_native { namespace d3d12 {
    ...
    MaybeError CommandRecordingContext::ExecuteCommandList(ID3D12CommandQueue* d3d12CommandQueue) {
        if (IsOpen()) {
            ...
            DAWN_TRY(mDevice->GetResidencyManager()->ProcessResidency(
                mResourceHeapAllocationsPendingUsage));
            ...
            mResourceHeapAllocationsPendingUsage.clear();
        }
        return {};
    }

    void CommandRecordingContext::TrackResourceHeapUsage(ResourceHeapAllocation* heapAllocation) {
        if (heapAllocation->GetLRUEntry()->GetAllocationType() != AllocationType::kExternal) {
            mResourceHeapAllocationsPendingUsage.push_back(heapAllocation);
        }
    }
