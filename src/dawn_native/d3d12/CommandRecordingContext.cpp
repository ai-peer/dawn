...
namespace dawn_native { namespace d3d12 {
    ...
    MaybeError CommandRecordingContext::Open(ID3D12Device* d3d12Device,
        ...
        if (mD3d12CommandList != nullptr) {
            DAWN_TRY(CheckHRESULT(mD3d12CommandList->Reset(commandAllocator, nullptr),
    ...
    ResultOrError<ID3D12GraphicsCommandList*> CommandRecordingContext::Close() {
        ASSERT(IsOpen());
        mIsOpen = false;
        DAWN_TRY(CheckHRESULT(mD3d12CommandList->Close(), "D3D12 closing pending command list"));
