...
namespace dawn_native { namespace d3d12 {
    ...
    class Texture final : public TextureBase {
      ...
      private:
        ...
        bool SkipTransitionForReadonlyAndUncompressedTexture(uint32_t baseMipLevel,
                                                             uint32_t levelCount,
                                                             uint32_t baseArrayLayer,
                                                             uint32_t layerCount);
        void SetStateForReadonlyAndUncompressedTexture(D3D12_RESOURCE_STATES newState);

        ResourceHeapAllocation mResourceAllocation;
        std::vector<D3D12_RESOURCE_STATES> mLastSubresourceStates =
            std::vector<D3D12_RESOURCE_STATES>(GetSubresourceCount(),
