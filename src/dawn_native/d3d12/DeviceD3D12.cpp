...
namespace dawn_native { namespace d3d12 {
    ...
    size_t Device::GetD3D12HeapTypeToIndex(D3D12_HEAP_TYPE heapType) const {
        ASSERT(heapType > 0 && heapType <= kNumHeapTypes);
        return heapType - 1;
    }

    void Device::DeallocateMemory(std::unique_ptr<ResourceBase> allocation) {
        CommittedResourceAllocator* allocator = nullptr;
        D3D12_HEAP_PROPERTIES heapProp;
        ToBackend(allocation.get())->GetD3D12Resource()->GetHeapProperties(&heapProp, nullptr);
        ...
        allocator->Deallocate(std::move(allocation));
    }

    ResultOrError<std::unique_ptr<ResourceBase>> Device::AllocateMemory(
        ...
        std::unique_ptr<ResourceBase> allocation;
