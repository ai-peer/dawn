...
namespace dawn_native { namespace d3d12 {
    ...
    void Device::DeallocateMemory(ResourceMemoryAllocation& allocation) {
        ...
        if (allocation.IsDirect()) {
            ...
            allocator =
                mResourceAllocators[heapInfo.Flags][heapInfo.Properties.Type][heapLevel].get();
        }
    ...
    ResultOrError<ResourceMemoryAllocation> Device::AllocateMemory(
        ...
        // and would otherwise increase internal fragementation (due to power-of-two alignment).
        //
        // For very small resources, it is inefficent to sub-allocate them since the min. heap size
        // or page-size in D3D is 64KB.
        bool isDirect = true;
        ...
        if (isDirect) {
            ...
            // PlacedResourceAllocator requires heap flags to be explicitly
            // specified. Not all GPUs allow mixed resource types to co-exist on the same
            // heap nor does PlacedResourceAllocator handle sub-allocation with multiple
            // heap options. Instead, a seperate set of allocators (per heap flag) is required.
            ...
            // Sub-allocations from PlacedResourceAllocator must be power-of-two aligned.
