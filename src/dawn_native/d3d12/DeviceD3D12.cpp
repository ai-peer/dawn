...
namespace dawn_native { namespace d3d12 {
    ...
    Ref<TextureBase> Device::CreateExternalTexture(const TextureDescriptor* descriptor,
        ...
        if (ConsumedError(Texture::CreateExternalImage(
                              this, descriptor, std::move(d3d12Texture), std::move(dxgiKeyedMutex),
                              acquireMutexKey, releaseMutexKey, isSwapChainTexture, isInitialized),
                          &dawnTexture)) {
            return nullptr;
        }
        return {dawnTexture};
    ...
    void Device::ShutDownImpl() {
        ...
        // ExternalImage may outlive the Dawn device so we must ensure it cannot leak should the
        // device go away.
