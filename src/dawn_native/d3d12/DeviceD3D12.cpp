...
namespace dawn_native { namespace d3d12 {
    ...
    void Device::DeviceLostImpl() {
        ...
        DAWN_UNUSED(NextSerial()).AcquireError();
        // Wait for all in-flight commands to finish executing
        DAWN_UNUSED(WaitForSerial(mLastSubmittedSerial)).AcquireError();
        ...
        DAWN_UNUSED(TickImpl()).AcquireError();
    ...
    MaybeError Device::CheckAndHandleDeviceLost() {
        if (IsDeviceLost()) {
            return DAWN_DEVICE_LOST_ERROR("D3D12 device lost");
        }
        MaybeError err = CheckHRESULT(GetD3D12Device()->GetDeviceRemovedReason(),
                                      "D3D12 get device removed reason");
        if (err.IsError()) {
            LoseDevice("D3D12 device lost");
        }
        return err;
    }
