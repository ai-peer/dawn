...
namespace dawn_native { namespace d3d12 {
    ...
    MaybeError Device::CheckDebugLayerAndGenerateErrors() {
        ...
        if (FAILED(infoQueue->GetMessageW(0, nullptr, &messageLength))) {
            return DAWN_INTERNAL_ERROR("D3D12 Debug layer message occurred");
        }
        static std::unique_ptr<uint8_t[]> messageData(new uint8_t[messageLength]);

        D3D12_MESSAGE* message = reinterpret_cast<D3D12_MESSAGE*>(messageData.get());
        if (FAILED(infoQueue->GetMessageW(0, message, &messageLength))) {
            return DAWN_INTERNAL_ERROR("D3D12 Debug layer message occurred");
        }
