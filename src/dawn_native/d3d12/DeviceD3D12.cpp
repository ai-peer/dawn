...
namespace dawn_native { namespace d3d12 {
    ...
    MaybeError Device::ExecuteCommandContext(CommandRecordingContext* commandContext) {
        ...
        ComPtr<ID3D12GraphicsCommandList> d3d12CommandListPending;
        ComPtr<ID3D12GraphicsCommandList> d3d12CommandListIn;

        // If there are pending commands, prepend them to ExecuteCommandLists
        if (mPendingCommands.IsOpen()) {
            DAWN_TRY_ASSIGN(d3d12CommandListPending, mPendingCommands.Close());
            d3d12CommandLists[numLists++] = d3d12CommandListPending.Get();
        }
