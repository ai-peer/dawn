...
namespace dawn_native { namespace d3d12 {
    ...
    Device::~Device() {
        ...
        // Cannot ASSERT(mUsedComObjectRefs.empty()) as the allocator could of enqueued
    ...
    void Device::DeallocateMemory(const ResourceMemoryAllocation& allocation) {
        ...
        ASSERT(heapType <= kMaxHeapTypes);
        allocator = mDirectResourceAllocators[heapType - 1].get();
    ...
    ResultOrError<ResourceMemoryAllocation> Device::AllocateMemory(
        ...
        CommittedResourceAllocator* allocator = mDirectResourceAllocators[heapType - 1].get();
        if (allocator == nullptr) {
            mDirectResourceAllocators[heapType - 1] =
