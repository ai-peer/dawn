...
namespace dawn_native { namespace d3d12 {
    ...
    ResultOrError<DescriptorHeapAllocation> Device::AllocateDescriptors(
        uint32_t descriptorCount,
        D3D12_DESCRIPTOR_HEAP_TYPE heapType) {
        return mDescriptorAllocatorManager->AllocateDescriptors(
            descriptorCount, GetPendingCommandSerial(), heapType);
    }
    ...
    bool Device::IsBindGroupAllocationValid(const BindGroup* group) const {
        return mDescriptorAllocatorManager->IsAllocationValid(group);
    }
