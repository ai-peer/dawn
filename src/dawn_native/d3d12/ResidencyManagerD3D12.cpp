...
namespace dawn_native { namespace d3d12 {
    ...
    MaybeError ResidencyManager::LockHeap(Pageable* pageable) {
        ...
        if (!pageable->IsInResidencyLRUCache() && !pageable->IsResidencyLocked()) {
            ...
            ID3D12Pageable* d3d12Pageable = pageable->GetD3D12Pageable().Get();
    ...
    void ResidencyManager::UnlockHeap(Pageable* pageable) {
        if (!mResidencyManagementEnabled) {
            return;
        }
    ...
    // Removes a pageable allocation from the LRU and returns it when possible. Returns
    ...
    MaybeError ResidencyManager::EnsureCanMakeResident(uint64_t sizeToMakeResident,
        ...
        while (sizeEvicted < sizeNeededToBeUnderBudget) {
            ...
            resourcesToEvict.push_back(pageable->GetD3D12Pageable().Get());
        }
    ...
    // Inserts a pageable allocation at the bottom of the LRU. The pageable allocation must be
    // resident or scheduled to become resident within the current serial.
