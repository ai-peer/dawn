...
namespace dawn_native { namespace d3d12 {
    ...
    LRUEntry* ResidencyManager::Evict() {
    ...
    MaybeError ResidencyManager::ProcessResidency(const std::vector<Heap*>& heapsPendingUsage) {
        if (mResidencyManagementEnabled) {
            std::vector<ID3D12Pageable*> heapsToMakeResident;
            uint64_t sizeToMakeResident = 0;
            for (uint32_t i = 0; i < heapsPendingUsage.size(); i++) {
                Heap* heap = heapsPendingUsage[i];
                LRUEntry* entry = heap->GetLRUEntry();
