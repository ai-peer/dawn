...
namespace dawn::native::d3d12 {
    ...
    template <BufferTextureCopyDirection Direction>
    void RecordBufferTextureCopyFromSplits(ID3D12GraphicsCommandList* commandList,
        ...
        for (uint32_t i = 0; i < baseCopySplit.count; ++i) {
            ...
            if constexpr (Direction == BufferTextureCopyDirection::B2T) {
                ...
                static_assert(Direction == BufferTextureCopyDirection::T2B);
                const D3D12_BOX sourceRegion =
    ...
    template <BufferTextureCopyDirection Direction>
    void Record2DBufferTextureCopyWithSplit(ID3D12GraphicsCommandList* commandList,
                                            ID3D12Resource* bufferResource,
                                            const uint64_t offset,
        ...
        for (uint32_t copyLayer = 0; copyLayer < copySize.depthOrArrayLayers; ++copyLayer) {
            ...
            RecordBufferTextureCopyFromSplits<Direction>(
                commandList, copySplitPerLayerBase, bufferResource, bufferOffsetForNextLayer,
                bytesPerRow, textureCopy.texture.Get(), textureCopy.mipLevel, copyTextureLayer,
                textureCopy.aspect);
    ...
    template <BufferTextureCopyDirection Direction>
    void RecordBufferTextureCopyWithBufferHandle(ID3D12GraphicsCommandList* commandList,
                                                 ID3D12Resource* bufferResource,
                                                 const uint64_t offset,
                                                 const uint32_t bytesPerRow,
                                                 const uint32_t rowsPerImage,
        ...
        switch (texture->GetDimension()) {
            ...
            case wgpu::TextureDimension::e3D: {
                ...
                RecordBufferTextureCopyFromSplits<Direction>(
                    commandList, copyRegions, bufferResource, 0, bytesPerRow, texture,
    ...
    template <BufferTextureCopyDirection Direction>
    void RecordBufferTextureCopy(ID3D12GraphicsCommandList* commandList,
                                 const BufferCopy& bufferCopy,
                                 const TextureCopy& textureCopy,
                                 const Extent3D& copySize) {
        RecordBufferTextureCopyWithBufferHandle<Direction>(
            commandList, ToBackend(bufferCopy.buffer)->GetD3D12Resource(), bufferCopy.offset,
            bufferCopy.bytesPerRow, bufferCopy.rowsPerImage, textureCopy, copySize);
    }

    // extern template instanciations for buffer texture copy recording.
    template void RecordBufferTextureCopyWithBufferHandle<BufferTextureCopyDirection::B2T>(
    ...
    template void RecordBufferTextureCopy<BufferTextureCopyDirection::B2T>(
        ID3D12GraphicsCommandList* commandList,
        const BufferCopy& bufferCopy,
        const TextureCopy& textureCopy,
        const Extent3D& copySize);

    template void RecordBufferTextureCopy<BufferTextureCopyDirection::T2B>(
        ID3D12GraphicsCommandList* commandList,
        const BufferCopy& bufferCopy,
        const TextureCopy& textureCopy,
        const Extent3D& copySize);
