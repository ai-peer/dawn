...
namespace dawn_native { namespace d3d12 {
    ...
    NonShaderVisibleDescriptorAllocator::~NonShaderVisibleDescriptorAllocator() {
        const Index freeBlockIndiciesSize = GetFreeBlockIndiciesSize();
        for (auto& buffer : mPool) {
            ASSERT(buffer.freeBlockIndicies.size() == freeBlockIndiciesSize);
        }
    ...
    NonShaderVisibleDescriptorAllocator::AllocateCPUDescriptors() {
        ...
        ASSERT(!buffer.freeBlockIndicies.empty());

        const Index blockIndex = buffer.freeBlockIndicies.back();

        buffer.freeBlockIndicies.pop_back();

        if (buffer.freeBlockIndicies.empty()) {
    ...
    MaybeError NonShaderVisibleDescriptorAllocator::AllocateCPUHeap() {
        ...
        const Index freeBlockIndiciesSize = GetFreeBlockIndiciesSize();
        newBuffer.freeBlockIndicies.reserve(freeBlockIndiciesSize);

        for (Index blockIndex = 0; blockIndex < freeBlockIndiciesSize; blockIndex++) {
            newBuffer.freeBlockIndicies.push_back(blockIndex);
        }
    ...
    void NonShaderVisibleDescriptorAllocator::Deallocate(CPUDescriptorHeapAllocation* allocation) {
        ...
        std::vector<Index>& freeBlockIndicies = mPool[heapIndex].freeBlockIndicies;
        if (freeBlockIndicies.empty()) {
        ...
        freeBlockIndicies.emplace_back(blockIndex);
    }
    ...
    NonShaderVisibleDescriptorAllocator::GetFreeBlockIndiciesSize() const {
