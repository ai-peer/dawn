...
namespace dawn_native { namespace d3d12 {
    ...
    std::vector<Sampler*>& SamplerHeapCacheEntry::GetSamplers() {
        return mSamplers;
    }
    ...
    bool SamplerHeapCacheEntry::Populate(Device* device,
        ...
        if (descriptorCount == 0 || allocator->IsAllocationStillValid(mGPUAllocation)) {
            return true;
        }
    ...
    ResultOrError<Ref<SamplerHeapCacheEntry>> SamplerHeapCache::GetOrCreate(
        ...
        // real entry below.
        samplers = std::move(blueprint.GetSamplers());
