...
namespace dawn_native { namespace d3d12 {
    ...
    class GPUDescriptorHeapCacheEntry : public RefCounted {
      public:
        GPUDescriptorHeapCacheEntry() = default;
        GPUDescriptorHeapCacheEntry(BindingInfoKey hash);
        ~GPUDescriptorHeapCacheEntry() override;

        // Memory is owned by the cache and must no-op.
        void DeleteThis() override {
        }

        void ReleaseEntry(GPUDescriptorHeapCache* cache);

        GPUDescriptorHeapAllocation* GetAllocation();

      private:
        friend GPUDescriptorHeapCache;

        GPUDescriptorHeapAllocation mAllocation;

        BindingInfoKey mHash = 0;  // Lookup key used to remove entry from cache.
    };
    ...
    class GPUDescriptorHeapCache {
      public:
        ...
        ~GPUDescriptorHeapCache();

        GPUDescriptorHeapCacheEntry* Acquire(BindingInfoKey hash);
        void DestoryCacheEntry(GPUDescriptorHeapCacheEntry* entry);
