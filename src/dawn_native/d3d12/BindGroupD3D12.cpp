...
namespace dawn_native { namespace d3d12 {
    ...
    ResultOrError<bool> BindGroup::Populate(DescriptorAllocatorManager* allocator) {
        Device* device = ToBackend(GetDevice());

        const Serial heapSerial = allocator->GetShaderVisibleHeapsSerial();
        if (IsAllocationStillValid(heapSerial)) {
            return true;
        }
        ...
        if (cbvUavSrvDescriptorCount > 0) {
            DAWN_TRY_ASSIGN(cbvSrvUavDescriptorHeapAllocation,
                            allocator->AllocateGPUDescriptors(
                                cbvUavSrvDescriptorCount, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV));
            if (cbvSrvUavDescriptorHeapAllocation.IsNull()) {
                return false;
            }
        ...
        if (samplerDescriptorCount > 0) {
            DAWN_TRY_ASSIGN(samplerDescriptorHeapAllocation,
                            allocator->AllocateGPUDescriptors(samplerDescriptorCount,
                                                              D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER));
            if (samplerDescriptorHeapAllocation.IsNull()) {
                return false;
            }
    ...
    bool BindGroup::IsAllocationStillValid(Serial heapSerial) const {
        // Allocation is only considered valid if the bounded heaps
        // have not switched over.
        return mHeapSerial == heapSerial;
    }
