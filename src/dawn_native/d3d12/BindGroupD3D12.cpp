...
namespace dawn_native { namespace d3d12 {
    ...
    ResultOrError<bool> BindGroup::Create() {
        ...
        // This only occurs when the currently bound heaps do not switch.
        if (device->IsBindGroupAllocationValid(this)) {
            return true;
        }
        ...
        DAWN_TRY_ASSIGN(cbvSrvUavDescriptorHeapAllocation,
                        device->AllocateDescriptors(bgl->GetCbvUavSrvDescriptorCount(),
                                                    D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV));

        DescriptorHeapAllocation samplerDescriptorHeapAllocation;
        DAWN_TRY_ASSIGN(samplerDescriptorHeapAllocation,
                        device->AllocateDescriptors(bgl->GetSamplerDescriptorCount(),
    ...
    void BindGroup::Invalidate() {
        mCbvSrvUavHeapAllocation = {};
        mSamplerHeapAllocation = {};
    }
