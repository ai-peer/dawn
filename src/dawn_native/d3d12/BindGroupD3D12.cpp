...
namespace dawn_native { namespace d3d12 {
    ...
    ResultOrError<bool> BindGroup::Populate(DescriptorAllocatorManager* allocator) {
        ...
        if (IsAllocationStillValid(device->GetCompletedCommandSerial(), heapSerial)) {
            return true;
        }
    ...
    bool BindGroup::IsAllocationStillValid(Serial completedSerial, Serial heapSerial) const {
        // Allocation is only considered valid if memory was not reclaimed and the bounded heaps
        // have not switched over.
        return (mPendingSerial > completedSerial && mHeapSerial == heapSerial);
    }
