...
namespace dawn_native { namespace d3d12 {
    ...
    ResultOrError<bool> BindGroup::Populate(DescriptorAllocatorManager* allocator) {
        ...
        // Skip if not reclaimed upon submission nor heaps have been switched.
        const Serial heapSerial = allocator->GetShaderVisibleHeapsSerial();
        if (mPendingSerial > device->GetCompletedCommandSerial() && mHeapSerial == heapSerial) {
            return true;
        }
        ...
        if (cbvUavSrvDescriptorCount > 0) {
            DAWN_TRY_ASSIGN(cbvSrvUavDescriptorHeapAllocation,
                            allocator->AllocateDescriptors(cbvUavSrvDescriptorCount, pendingSerial,
        ...
        if (samplerDescriptorCount > 0) {
            DAWN_TRY_ASSIGN(samplerDescriptorHeapAllocation,
                            allocator->AllocateDescriptors(samplerDescriptorCount, pendingSerial,
