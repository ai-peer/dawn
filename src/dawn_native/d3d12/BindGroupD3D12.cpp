...
namespace dawn_native { namespace d3d12 {
    ...
    ResultOrError<bool> BindGroup::Populate() {
        ...
        // Skip if not reclaimed upon submission.
        if (mPendingSerial > device->GetCompletedCommandSerial()) {
            return true;
        }

        DescriptorAllocatorManager* allocatorManager = device->GetDescriptorAllocatorManager();

        // Skip if remains on currently bound heaps.
        const Serial heapSerial = allocatorManager->GetShaderVisibleHeapsSerial();
        if (mHeapSerial == heapSerial) {
            return true;
        }
        ...
        // If either allocation failed, return early to re-allocate the heaps.
        // This is because bindgroups cannot be populated until both shader visible heaps are valid
        // as it uses both allocations to create GPU descriptors for both heaps.
