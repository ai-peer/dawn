...
namespace dawn_native { namespace d3d12 {
    ...
    void Recompute3DTextureCopyRegionsForBlockWithEmptyFirstRow(Origin3D origin,
        ...
        // to all depth slices, the last copy block will be row (N + 9) to row (N + 12).
        // row (N + 11) might be the last row of the entire buffer, and the rest row(s) might
    ...
    void Compute3DTextureCopySubresourceForSpecialCases(Origin3D origin,
        ...
        if (copy.count == 1) {
            ...
            if (i == 0) {
                ...
                Recompute3DTextureCopyRegionsForBlockWithEmptyFirstRow(
                    origin, copySize, blockInfo, bytesPerRow, bytesPerRow, rowsPerImage, copy, i);
            }
        }
    }
