    ...
    TextureCopySubresource Compute3DTextureCopySplits(Origin3D origin,
        ...
        for (uint32_t i = 0; i < originalCopyCount; ++i) {
            ...
            if (rowDelta == 2) {
                ...
                originalInfo->bufferSize.height = copySize.height;
                originalInfo->copySize.height = copySize.height;
                continue;
            }

            // Allocate a new copy that starts as a duplicate of the original copy.
            ...
            *newInfo = *originalInfo;
            ...
            if (copySize.height > 1) {
                ...
                newInfo->bufferSize.height = copySize.height;
                newInfo->copySize.height = blockInfo.height;
                newInfo->textureOffset.y += copySize.height - blockInfo.height;

                // Advance the buffer two rows to make sure we stay 512 aligned.
                newInfo->alignedOffset += 2 * bytesPerRow;
                ...
                newInfo->bufferOffset.y = copySize.height - 2 * blockInfo.height;
            } else {
                // XXX different split
                UNREACHABLE();
            }
