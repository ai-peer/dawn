...
namespace dawn_native { namespace d3d12 {
    ...
    namespace {
        Origin3D ComputeTexelOffsets(Format format,
            ...
            texelOffset.x = byteOffsetX / format.blockByteSize * format.blockWidth;
            texelOffset.y = byteOffsetY / rowPitch * format.blockHeight;
    ...
    TextureCopySplit ComputeTextureCopySplit(Origin3D origin,
        ...
        Origin3D texelOffset =
            ComputeTexelOffsets(format, static_cast<uint32_t>(offset - alignedOffset), rowPitch, slicePitch);

        uint32_t copyBytesPerRowPitch = copySize.width / format.blockWidth * format.blockByteSize;
        uint32_t byteOffsetPerRowPitch = texelOffset.x / format.blockWidth * format.blockByteSize;
        if (copyBytesPerRowPitch + byteOffsetPerRowPitch <= rowPitch) {
        ...
        ASSERT(rowPitch > byteOffsetPerRowPitch);
        uint32_t texelsPerRowPitch = rowPitch / format.blockByteSize * format.blockWidth;
        copy.copies[0].copySize.width = texelsPerRowPitch - texelOffset.x;
        copy.copies[0].copySize.height = copySize.height;
        copy.copies[0].copySize.depth = copySize.depth;

        copy.copies[0].bufferOffset = texelOffset;
        copy.copies[0].bufferSize.width = texelsPerRowPitch;
        copy.copies[0].bufferSize.height = imageHeight + texelOffset.y;
        copy.copies[0].bufferSize.depth = copySize.depth + texelOffset.z;
