...
namespace dawn_native { namespace d3d12 {
    ...
    void Recompute3DTextureCopyRegionsForBlockWithEmptyFirstRowAndOddCopyHeight(
        Origin3D origin,
        Extent3D copySize,
        const TexelBlockInfo& blockInfo,
        uint32_t bytesPerRow,
        ...
        TextureCopySubresource& copy,
        uint32_t i) {
        // Read the comments of Recompute3DTextureCopyRegionsForBlockWithEmptyFirstRow() for
        ...
        //
        // If the copy height is even, we can use two copies to make it correct:
        ...
        *copy1 = copy0;
        ...
        copy1->textureOffset = {copy0.textureOffset.x, 0, 1};
        copy1->bufferOffset = {copy0.bufferOffset.x, 0, 0};
    ...
    TextureCopySubresource Compute3DTextureCopySplits(Origin3D origin,
        ...
        for (uint32_t i = 0; i < originalCopyCount; ++i) {
            ...
            if (bufferSize.height > rowsPerImageInTexels) {
                ...
                // Otherwise, there won't be an empty row at the beginning of this copy region.
                ASSERT(bytesPerRow == D3D12_TEXTURE_DATA_PITCH_ALIGNMENT);
                ASSERT(copySie.height == rowsPerImageInTexels);
                if (copySize.height % 2 == 0) {
                    ...
                    // without an empty row. This is an easier case relative to even copy height
                    // cases.
                    Recompute3DTextureCopyRegionsForBlockWithEmptyFirstRowAndOddCopyHeight(
                        origin, copySize, blockInfo, bytesPerRow, rowsPerImage, copySubresource, i);
                }
            } else {
