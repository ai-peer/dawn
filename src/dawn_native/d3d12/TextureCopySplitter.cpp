...
namespace dawn_native { namespace d3d12 {
    ...
    namespace {
        ...
        uint64_t ComputeBufferLinearOffset(const TexelBlockInfo& blockInfo,
                                           uint32_t bytesPerRow,
    ...
    void Recompute3DTextureCopyRegionsForBlockWithEmptyFirstRow(Origin3D origin,
        ...
        // Note that alignedOffset is at the beginning of row (N - 1), while buffer offset is at
        ...
        // Note that we recompute alignedOffset and buffer offset for copy region of each image.
        //   - copied rows for image 0: row N to row (N + 3). Row (N - 1) is in this copy region
        //     but it is skipped via parameter bufferOffset.
        //   - copied rows for image 1: row (N + 4) to row (N + 7). Row (N + 3) is in this copy
        //     region but it is skipped. It won't be copied twice.
        // Likewise, the rest images are copied. So all data will be copied correctly for a 2D
        // texture.
        // However, if we expand the copy region of image 0 to all depth ranges of a 3D texture,
        // we have no chance to recompute alignedOffset and buffer offset for each depth slice.
        // And the entire 3D copy region shares one single alignedOffset and buffer offset for
        // all depth images. So we will copy 5 rows every time, and every first row of each slice
        // will be skipped. As a result,
        //   - the copied data for image 0: row N to row (N + 3), which is correct. Row (N - 1)
        //     is skipped.
        //   - the copied data for image 1: row (N + 5) to row (N + 8) because row (N + 4) is
        //     skipped. It is incorrect.
        ...
        // Copy 1: copy the last row on image 0, and expand to copySize.depthOrArrayLayers - 1
        // depth slices. Note that if we expand it to all depth slices, the last copy block will
        // be row (N + 11) to row (N + 14). row (N + 11) might be the last row of the entire
        // buffer, and the rest rows might be out-of-bound. Then we will get a validation error.
        // So we need one more copy for the last row of the last depth slice.
        //                _____________________
        //               /                    /|
        //              /                    / |
        //              |-------------------|  |
        ...
        // Copy 1: copy the last row on image 0 and expand to all depth slices but the last one.
        uint64_t offsetForCopy0 = ComputeBufferLinearOffset(
            blockInfo, bytesPerRow, copy0.alignedOffset, copy0.bufferOffset);
        uint64_t offsetForLastRow = offsetForCopy0 + bytesPerRow * copy0.copySize.height;
        ...
        copy1.copySize = {copy0.copySize.width, blockInfo.height, copySize.depthOrArrayLayers - 1};
    ...
    void Compute3DTextureCopySubresourceForSpecialCases(Origin3D origin,
        ...
        // adjustment, we need to recompute/modify the copy region. The empty first row case
        // can be divided into two scenarios:
        //   - If copySize.height is greater than 1: we can reuse this copy region and modify
        //     a couple arguments, and add some copy regions to fix the empty first row issue.
        //   - If copySize.height is 1. This is an even more special case. We will abandon
        //     all copy regions and recompute copy regions in a different approach.

        ASSERT(bytesPerRow == D3D12_TEXTURE_DATA_PITCH_ALIGNMENT);
        Recompute3DTextureCopyRegionsForBlockWithEmptyFirstRow(origin, copySize, blockInfo,
                                                               bytesPerRow, rowsPerImage, copy, i);
        // TODO(yunchao.he@intel.com): recompute copy regions when copySize.height is 1.
    }
    ...
    TextureCopySubresource Compute3DTextureCopySplits(Origin3D origin,
        ...
        int32_t indexOfCopyRegionForRecompute = -1;
        for (uint32_t i = 0; i < copySubresource.count; ++i) {
            ...
            if (bufferSize.height > rowsPerImageInTexels) {
                // This is set at most once because we assert copy region count is not greater
                // than 2 and one copy region at most has more rows than rowsPerImageInTexels.
                // Note that if the copy region's bufferSize.height equals to rowsPerImageInTexels,
                // we can use this copy region without any modification.
                indexOfCopyRegionForRecompute = i;
        ...
        if (indexOfCopyRegionForRecompute == -1) {
            return copySubresource;
        }

        ASSERT(bytesPerRow == D3D12_TEXTURE_DATA_PITCH_ALIGNMENT);
        Compute3DTextureCopySubresourceForSpecialCases(origin, copySize, blockInfo, offset,
                                                       bytesPerRow, rowsPerImage, copySubresource,
                                                       indexOfCopyRegionForRecompute);
        return copySubresource;
    }
