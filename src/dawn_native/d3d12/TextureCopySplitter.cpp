...
namespace dawn_native { namespace d3d12 {
    ...
    void Recompute3DTextureCopyRegionsForBlockWithEmptyFirstRow(Origin3D origin,
        ...
        // It is incorrect. And all other image followed will be incorrect.
        //              |-------------------|
        //  row (N - 1) |                   |
        //  row N       |                 ++|
        //  row (N + 1) |~~~~~~~~~~~~~~~~~++|
        ...
        // So we need one more copy for the last row of the entire copy block.
        //                _____________________
        //               /                    /|
        //              /                    / |
        //              |-------------------|  |
    ...
    void Compute3DTextureCopySubresourceForSpecialCases(Origin3D origin,
        ...
        // adjustment, we need to recompute/modify all copy regions. These empty first row cases
        // can be divided into two scenarios:
        //     - If copySize.height is greater than 1, data in one row in buffer may straddle
        //       two rows (copy.count is 2) or not (copy.count is 1). We may reuse some copy
        //       region, and add a couple more copy regions to handle the empty first row issue.
        //     - If copySize.height is 1. This is an even more special case. We will abandon
        //       all copy regions and recompute copy regions in different approach.
        ...
        if (copy.count == 1) {
            ...
            // Then either copy.copies[0] or copy.copies[1] has an empty row. And the other block
            // has no empty first row issue, which means that its copy region is correct.
            copy.count = 4;
            if (i == 0) {
                // If i is 0, it means copy.copies[0] has an empty first row. And copy.copies[1] is
                // correct, we can move it to copy.copies[3] because we need 2 more copy regions in
                // order to handle the empty first row issue via function
                // Recompute3DTextureCopyRegionsForBlockWithEmptyFirstRow().
                copy.copies[3] = copy.copies[1];
                Recompute3DTextureCopyRegionsForBlockWithEmptyFirstRow(
                    origin, copySize, blockInfo, offset, bytesPerRow, rowsPerImage, copy, i);
            } else {
                // If i is 1, it means that copy.copies[1] has an empty first row. We can call
                // function Recompute3DTextureCopyRegionsForBlockWithEmptyFirstRow() to handle it.
                Recompute3DTextureCopyRegionsForBlockWithEmptyFirstRow(
