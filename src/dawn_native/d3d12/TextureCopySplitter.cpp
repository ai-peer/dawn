...
namespace dawn_native { namespace d3d12 {
    ...
    void Compute3DTextureCopyRegionsForBlockWithEmptyFirstRow(Origin3D origin,
        ...
        // In short, if any dimension of any 2D copy regions is out-of-bound, say copy.copies[i].
        // bufferSize.height is greater than rowsPerImage, the copied data will be in a mess if
        // we expand that copy region to a 3D texture. Because we will simply duplicate that big
        // copy region on every depth slice without any change.
        //              |-------------------|
        //  row (N - 1) |                   |
        //  row N       |                 ++|
        //  row (N + 1) |~~~~~~~~~~~~~~~~~++|
        ...
        // rows = 4 rows, which is the height of buffer size (rowsPerImage). Then copy the last row
        // in copy 1, and expand to copySize.depthOrArrayLayers - 1 depth slices. And copy the last
        // row of the last depth slice in copy 2.
        ...
        //  TODO(yunchao.he@intel.com): combine copy 1 and copy 2 for at least some cases.
        //              |-------------------|
        //  row (N + 11)|                 ++|
        //              |-------------------|
