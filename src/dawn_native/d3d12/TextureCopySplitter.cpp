...
namespace dawn_native { namespace d3d12 {
    ...
    TextureCopySubresource Compute3DTextureCopySplits(Origin3D origin,
        ...
        using CopyInfo = TextureCopySubresource::CopyInfo;
        auto AllocateCopy = [&](const CopyInfo* blueprint) -> CopyInfo* {
            ASSERT(copySubresource.count < TextureCopySubresource::kMaxTextureCopyRegions);
            CopyInfo* newInfo = &copySubresource.copies[copySubresource.count];
            copySubresource.count++;
            *newInfo = *blueprint;
            return newInfo;
        };
        ...
        for (uint32_t i = 0; i < originalCopyCount; ++i) {
            CopyInfo* originalInfo = &copySubresource.copies[i];

            ASSERT(copySubresource.copies[i].bufferSize.height >= copySize.height);
            if (copySubresource.copies[i].bufferSize.height == copySize.height) {
                continue;
            }
            uint32_t rowDelta =
                (copySubresource.copies[i].bufferSize.height - copySize.height) / blockInfo.height;
            ASSERT(rowDelta == 1 || rowDelta == 2);

            // When we are offset by an even number of rows, we can always offset the start of the
            // buffer directly as it will be ok for D3D12 placement offset alignement constraint.
            if (rowDelta == 2) {
                originalInfo->alignedOffset += 2 * bytesPerRow;
                originalInfo->bufferOffset.y = 0;
                originalInfo->bufferSize.height = rowsPerImage;
                originalInfo->copySize.height = copySize.height;
                continue;
            }

            if (copySize.height > 1) {
                // Modify the original copy to not contain the last row.
                originalInfo->bufferSize.height = rowsPerImage;
                originalInfo->copySize.height = copySize.height - blockInfo.height;

                CopyInfo* lastRowsButOne = AllocateCopy(originalInfo);

                // Make newInfo do the copy of the last row only.
                lastRowsButOne->copySize.height = blockInfo.height;
                lastRowsButOne->textureOffset.y += copySize.height - blockInfo.height;

                // Advance the buffer two rows to make sure we stay 512 aligned.
                lastRowsButOne->alignedOffset += 2 * bytesPerRow;
                // Remove two rows from the copy height for the bufferOffset:
                //   - one for the row we advanced in the buffer (not counting the first empty row)
                //   - one for the last row we want to copy.
                lastRowsButOne->bufferOffset.y = copySize.height - 2 * blockInfo.height;

                // Skip the very last row.
                lastRowsButOne->copySize.depthOrArrayLayers--;
                lastRowsButOne->bufferSize.depthOrArrayLayers--;

                CopyInfo* veryLastRow = AllocateCopy(lastRowsButOne);
                veryLastRow->copySize.depthOrArrayLayers = 1;
                veryLastRow->bufferSize.depthOrArrayLayers = 1;
                veryLastRow->bufferOffset.z = 0;
                veryLastRow->textureOffset.z += copySize.depthOrArrayLayers - 1;



                uint64_t unalignedOffsetToLastRow =
                    originalInfo->alignedOffset + bytesPerRow * (rowsPerImage * copySize.depthOrArrayLayers - 1 + originalInfo->bufferOffset.y);

                // XXX add uint64_t overload of IsAligned
                if (IsAligned(unalignedOffsetToLastRow, D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT)) {
                    veryLastRow->alignedOffset = unalignedOffsetToLastRow;
                    veryLastRow->bufferSize.height = 1;
                    veryLastRow->bufferOffset.y = 0;
                } else {
                    veryLastRow->alignedOffset = unalignedOffsetToLastRow - bytesPerRow;
                    veryLastRow->bufferSize.height = 2;
                    veryLastRow->bufferOffset.y = 1;
                }

            } else {
                // XXX different split. Need to think about how it works with rowsPerImage != 1.
                UNREACHABLE();
            }
        }
