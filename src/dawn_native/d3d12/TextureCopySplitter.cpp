...
namespace dawn_native { namespace d3d12 {
    ...
    Texture2DCopySplit Compute3DTextureCopySplit(Origin3D origin,
        ...
        if (copyBytesPerRowPitch + byteOffsetInRowPitch <= bytesPerRow) {
            ...
            // only one alignedOffset for all depth ranges in 3D texture copy. The copied data
            // will be in a mess if we put the last row into copy.copies[0] and make
            // copy.copies[0].bufferSize bigger than the real buffer size of each depth slice.
