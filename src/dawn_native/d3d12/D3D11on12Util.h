...
namespace dawn_native { namespace d3d12 {
    ...
    class D3D11on12ResourceCacheEntry {
      public:
        ...
        struct HashFunc {
            size_t operator()(const std::shared_ptr<D3D11on12ResourceCacheEntry> a) const;
        };

        struct EqualityFunc {
            bool operator()(const std::shared_ptr<D3D11on12ResourceCacheEntry> a,
                            const std::shared_ptr<D3D11on12ResourceCacheEntry> b) const;
        };
    ...
    class D3D11on12ResourceCache {
      public:
        ...
        std::shared_ptr<D3D11on12ResourceCacheEntry> GetOrCreateD3D11on12Resource(
            WGPUDevice device,
      ...
      private:
        ...
        using Cache = std::unordered_set<std::shared_ptr<D3D11on12ResourceCacheEntry>,
