...
namespace dawn_native { namespace d3d12 {
    ...
    class D3D11on12ResourceCacheEntry {
      public:
        ...
        struct HashFunc {
            size_t operator()(const D3D11on12ResourceCacheEntry& a) const;
        };

        struct EqualityFunc {
            bool operator()(const D3D11on12ResourceCacheEntry& a,
                            const D3D11on12ResourceCacheEntry& b) const;
        };
    ...
    class D3D11on12ResourceCache {
      ...
      private:
        ...
        using Cache = std::unordered_set<D3D11on12ResourceCacheEntry,
