...
namespace dawn_native { namespace d3d12 {
    ...
    D3D11on12ResourceCacheEntry::~D3D11on12ResourceCacheEntry() {
        ...
        // Until the Dawn device destructs and calls Flush11On12DeviceToAvoidLeaks(),
        // it cannot properly released. To ensure 11on12 does not leak any resources,
        // flush the 11on12 device in both places.
        Flush11On12DeviceToAvoidLeaks(std::move(mD3D11on12Device));
    }

    ComPtr<IDXGIKeyedMutex> D3D11on12ResourceCacheEntry::GetDXGIKeyedMutex() {
        ASSERT(mDXGIKeyedMutex != nullptr);
        return mDXGIKeyedMutex;
    }

    size_t D3D11on12ResourceCacheEntry::HashFunc::operator()(
        const D3D11on12ResourceCacheEntry& a) const {
        size_t hash = 0;
    ...
    bool D3D11on12ResourceCacheEntry::EqualityFunc::operator()(
        const D3D11on12ResourceCacheEntry& a,
    ...
    ComPtr<IDXGIKeyedMutex> D3D11on12ResourceCache::GetOrCreateDXGIKeyedMutex(
        ...
        if (iter != mCache.end()) {
            ComPtr<IDXGIKeyedMutex> keyedMutex =
                const_cast<D3D11on12ResourceCacheEntry&>(*iter).GetDXGIKeyedMutex();
            ASSERT(keyedMutex != nullptr);
            return keyedMutex;
        }
        ...
        mCache.emplace(dxgiKeyedMutex, std::move(d3d11on12Device));
