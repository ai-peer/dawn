...
namespace dawn_native { namespace d3d12 {
    ...
    ResultOrError<std::unique_ptr<ResourceHeapBase>> HeapAllocator::AllocateResourceHeap(
        ...
        std::unique_ptr<ResourceHeapBase> heapBase = std::make_unique<Heap>(std::move(d3d12Heap));

        mDevice->GetResidencyManager()->TrackResidentAllocation(static_cast<Heap*>(heapBase.get()));
        return heapBase;
    }

    void HeapAllocator::DeallocateResourceHeap(std::unique_ptr<ResourceHeapBase> resourceHeap) {
        Heap* heap = static_cast<Heap*>(resourceHeap.get());
        mDevice->ReferenceUntilUnused(heap->GetD3D12Heap());
    }
