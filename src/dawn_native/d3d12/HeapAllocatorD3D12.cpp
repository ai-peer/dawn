...
namespace dawn_native { namespace d3d12 {
    ...
    ResultOrError<std::unique_ptr<ResourceHeapBase>> HeapAllocator::AllocateResourceHeap(
        ...
        mDevice->GetResidencyManager()->EnsureCanMakeResident(size);
        ...
        mDevice->GetResidencyManager()->TrackAllocation(static_cast<Heap*>(heapBase.get()));
        return heapBase;
    }

    void HeapAllocator::DeallocateResourceHeap(std::unique_ptr<ResourceHeapBase> heap) {
        mDevice->GetResidencyManager()->EvictUponDeallocation(static_cast<Heap*>(heap.get()));
