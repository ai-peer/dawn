...
namespace dawn_native { namespace d3d12 {
    ...
    MaybeError Buffer::ClearBuffer(CommandRecordingContext* commandContext,
        ...
        if (D3D12HeapType(GetUsage()) == D3D12_HEAP_TYPE_UPLOAD) {
            ...
            const Buffer* zeroBuffer = device->GetZeroBuffer(commandContext);
            if (clearValue == 0u && this != zeroBuffer) {
                uint64_t zeroBufferSize = zeroBuffer->GetAllocatedSize();
                TrackUsageAndTransitionNow(commandContext, wgpu::BufferUsage::CopyDst);
                ...
                // The zeroBuffer must only be cleared to zero.
                ASSERT(this != zeroBuffer || clearValue == 0u);
