...
namespace dawn_native { namespace d3d12 {
    ...
    MaybeError Buffer::Initialize() {
        ...
        DAWN_TRY_ASSIGN(mResource, ToBackend(GetDevice())
                                       ->AllocateMemory(heapType, resourceDescriptor, bufferUsage,
    ...
    ComPtr<ID3D12Resource> Buffer::GetD3D12Resource() const {
        return ToBackend(mResource.get())->GetD3D12Resource();
    }
    ...
    D3D12_GPU_VIRTUAL_ADDRESS Buffer::GetVA() const {
        return ToBackend(mResource.get())->GetGPUPointer();
    }
    ...
    void Buffer::DestroyImpl() {
        ToBackend(GetDevice())->DeallocateMemory(std::move(mResource));
    }
