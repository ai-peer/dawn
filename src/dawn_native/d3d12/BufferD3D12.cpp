...
namespace dawn_native { namespace d3d12 {
    ...
    MaybeError Buffer::Initialize() {
        ...
        DAWN_TRY_ASSIGN(
            mAllocation,
            ToBackend(GetDevice())->AllocateMemory(heapType, resourceDescriptor, heapFlags));
        return {};
    }
    ...
    void Buffer::TransitionUsageNow(ComPtr<ID3D12GraphicsCommandList> commandList,
        ...
        if (TransitionUsageAndGetResourceBarrier(&transitionBarrier, usage)) {
    ...
    void Buffer::DestroyImpl() {
        ToBackend(GetDevice())->DeallocateMemory(mAllocation);
