...
namespace dawn_native { namespace d3d12 {
    ...
    // Uses a d3d pipeline library to cache d3d pipelines in-memory then save them back to disk.
    // If no d3d pipeline library support, the cache is in passthrough mode.
    class PipelineCache : public RefCounted {
      ...
      private:
        ...
        // The |mLibrary| cannot tell us when it's contents get modified so a dirty bit is needed
        // to determine when to persistent back updates.
    ...
    class SharedPipelineCaches {
      public:
        ...
        dawn_platform::ScopedCachedBlob LoadPipelineCacheData(PersistentCache* persistentCache);
