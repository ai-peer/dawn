...
namespace dawn_native { namespace d3d12 {
    ...
    MaybeError SwapChain::Initialize(NewSwapChainBase* previousSwapChain) {
        ...
        mConfig.bufferCount = BufferCountForPresentMode(GetPresentMode());
        mConfig.format = D3D12TextureFormat(GetFormat());
        mConfig.swapChainFlags = SwapChainFlagsForPresentMode(GetPresentMode());
        ...
        if (!canReuseSwapChain) {
            DAWN_TRY(previousD3D12SwapChain->DetachAndWaitDeallocation());
            return InitializeSwapChainFromScratch();
        }
        ...
        if (canReuseBuffers) {
            ...
            std::swap(mBufferLastUsedSerials, previousD3D12SwapChain->mBufferLastUsedSerials);
    ...
    MaybeError SwapChain::InitializeSwapChainFromScratch() {
        ...
        factory2->CreateSwapChainForHwnd(device->GetCommandQueue().Get(),
                                         static_cast<HWND>(GetSurface()->GetHWND()), &swapChainDesc,
    ...
    ResultOrError<TextureViewBase*> SwapChain::GetCurrentTextureViewImpl() {
        ...
        // Synchrounously wait until previous operations on the next swapchain buffer are finished.
    ...
    MaybeError SwapChain::DetachAndWaitDeallocation() {
        DetachFromSurface();
