...
namespace dawn_native { namespace d3d12 {
    ...
    ResultOrError<BindGroup*> BindGroupLayout::AllocateBindGroup(
        ...
        SamplerHeapCacheEntry* samplerAllocationEntry = nullptr;
        if (GetSamplerDescriptorCount() > 0) {
            DAWN_TRY_ASSIGN(samplerAllocationEntry, device->GetSamplerHeapCache()->GetOrCreate(
                                                        descriptor, mSamplerAllocator));
        }

        return mBindGroupAllocator.Allocate(device, descriptor, viewSizeIncrement, viewAllocation,
                                            samplerAllocationEntry);
    }
