...
namespace dawn_native { namespace d3d12 {
    ...
    ResultOrError<ComPtr<IDxcBlob>> ShaderModule::CompileShaderDXC(SingleShaderStage stage,
        ...
        static const PerStage<const wchar_t*> compileTargets = [&]() {
            PerStage<const wchar_t*> compileTargets;
            compileTargets[SingleShaderStage::Vertex] = L"vs_6_0";
            compileTargets[SingleShaderStage::Fragment] = L"ps_6_0";
            compileTargets[SingleShaderStage::Compute] = L"cs_6_0";
        ...
        DAWN_TRY(CheckHRESULT(dxcLibrary->CreateBlobWithEncodingOnHeapCopy(
            hlslSource.c_str(), hlslSource.length(), CP_UTF8, &sourceBlob), "DXC create blob"));
        ...
        DAWN_TRY(CheckHRESULT(dxcCompiler->Compile(sourceBlob.Get(), nullptr, entryPointW.c_str(),
                                                   compileTargets[stage], arguments.data(),
        ...
        if (FAILED(hr)) {
            ...
            return DAWN_INTERNAL_ERROR(reinterpret_cast<char*>(errors->GetBufferPointer()));
        }
    ...
    ResultOrError<ComPtr<ID3DBlob>> ShaderModule::CompileShaderFXC(SingleShaderStage stage,
                                                                   ...
                                                                   const char* entryPoint,
                                                                   uint32_t compileFlags) {
        static const PerStage<const char*> targetProfiles = [&]() {
            PerStage<const char*> targetProfiles;
            targetProfiles[SingleShaderStage::Vertex] = "vs_5_1";
            targetProfiles[SingleShaderStage::Fragment] = "ps_5_1";
            targetProfiles[SingleShaderStage::Compute] = "cs_5_1";
        ...
        if (FAILED(functions->d3dCompile(hlslSource.c_str(), hlslSource.length(), nullptr, nullptr,
                                         nullptr, entryPoint, targetProfiles[stage], compileFlags,
                                         0, &compiledFXCShader, &errors))) {
            return DAWN_INTERNAL_ERROR(reinterpret_cast<char*>(errors->GetBufferPointer()));
        }
