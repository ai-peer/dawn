...
namespace dawn_native { namespace d3d12 {
    ...
    MaybeError ShaderModule::Initialize() {
        ...
        if (GetDevice()->IsToggleEnabled(Toggle::UseSpvc)) {
            ...
            if (GetDevice()->IsExtensionEnabled(Extension::ShaderFloat16)) {
                options.SetHLSLShaderModel(ToBackend(GetDevice())->GetDeviceInfo().dxcShaderModel);
    ...
    ResultOrError<std::string> ShaderModule::GetHLSLSource(PipelineLayout* layout) {
        ...
        if (!GetDevice()->IsToggleEnabled(Toggle::UseSpvc)) {
            ...
            if (GetDevice()->IsExtensionEnabled(Extension::ShaderFloat16)) {
                options_hlsl.shader_model = ToBackend(GetDevice())->GetDeviceInfo().dxcShaderModel;
    ...
    ResultOrError<ComPtr<IDxcBlob>> ShaderModule::CompileShaderDXC(SingleShaderStage stage,
        ...
        DAWN_TRY(CheckHRESULT(
            dxcCompiler->Compile(
                ...
                ToBackend(GetDevice())->GetDeviceInfo().dxcShaderProfiles[stage].c_str(),
