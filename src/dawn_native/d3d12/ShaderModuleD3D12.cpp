...
namespace dawn_native { namespace d3d12 {
    ...
    namespace {
        ...
        template <typename T>
        void Serialize(std::stringstream& output, const T& t) {
            output << t;
        }
    ...
    ResultOrError<CompiledShader> ShaderModule::Compile(const char* entryPointName,
        ...
        if (device->IsToggleEnabled(Toggle::UseMesa)) {
            backend = ShaderCompilationRequest::Compiler::Mesa;
        ...
        ShaderCompilationRequest request{
            ...
            backend == ShaderCompilationRequest::Compiler::Mesa ? mesaVersion : 0,
            layout->GetFirstIndexOffsetRegisterSpace(),
            layout->GetFirstIndexOffsetShaderRegister(),
        };
