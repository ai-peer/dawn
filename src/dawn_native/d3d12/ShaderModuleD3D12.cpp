...
namespace dawn_native { namespace d3d12 {
    ...
    ResultOrError<tint::Program> ShaderModule::TranslateToTintProgram(
        bool mayCollide,
    ...
    ResultOrError<std::string> ShaderModule::TranslateToHLSLWithTint(
        ...
        tint::Program program;
        DAWN_TRY_ASSIGN(program, TranslateToTintProgram(mayCollide, entryPointName, stage, layout,
                                                        remappedEntryPointName, firstOffsetInfo));
    ...
    ResultOrError<ScopedCachedBlob> ShaderModule::TranslateToDXILWithMesa(
        Device* device,
        const char* entryPointName,
        SingleShaderStage stage,
        PipelineLayout* layout,
        ...
        if (!functions->spirvToDxil(result.spirv.data(), result.spirv.size(), NULL, 0, dxStage,
                                    remappedEntryPointName->c_str(),
                                    reinterpret_cast<void**>(&dxil), &dxilCount)) {
        ...
        return ScopedCachedBlob{std::move(data), dxilCount};
    }
    ...
    ResultOrError<CompiledShader> ShaderModule::Compile(const char* entryPointName,
        ...
        if (device->IsToggleEnabled(Toggle::UseMesa)) {
            ...
            DAWN_TRY_ASSIGN(shaderCacheKey,
                            CreateWGSLKey(entryPointName, stage, layout, compileFlags));
                            ...
                            device->GetPersistentCache()->GetOrCreate(
                                shaderCacheKey, [&](auto doCache) -> MaybeError {
                                    DAWN_TRY_ASSIGN(
                                        ...
                                        TranslateToDXILWithMesa(device, entryPointName, stage,
                                                                layout, &remappedEntryPoint,
                                                                &compiledShader.firstOffsetInfo));

                                    doCache(compiledShader.compiledMesaShader.buffer,
                                            compiledShader.compiledMesaShader.bufferSize);
                                    return {};
                                }));
    ...
    ResultOrError<PersistentCacheKey> ShaderModule::CreateWGSLKey(const char* entryPointName,
        ...
        if (GetDevice()->IsToggleEnabled(Toggle::UseTintGenerator)) {
            auto result =
                tint::writer::wgsl::Generate(GetTintProgram(), tint::writer::wgsl::Options{});
            if (!result.success) {
                std::ostringstream errorStream;
            ...
            if (GetDevice()->IsToggleEnabled(Toggle::UseMesa)) {
                ...
                ASSERT(functions->IsSPIRVToDXILAvailable());
                uint64_t version = functions->spirvToDxilGetVersion();
                ASSERT(version != 0);

                stream << "spirv_to_dxil" << version << " ";
            } else {
                // todo include tint version?
            }

            stream << static_cast<uint32_t>(stage);
            stream << entryPointName;

            // Include the pipeline layout as a part of the key
            stream << layout->ComputeContentHash();

            return PersistentCacheKey(std::istreambuf_iterator<char>{stream},
                                      std::istreambuf_iterator<char>{});
        } else {
            return DAWN_UNIMPLEMENTED_ERROR("WGSL key not implemented for SPIRV-cross");
        }
    }
