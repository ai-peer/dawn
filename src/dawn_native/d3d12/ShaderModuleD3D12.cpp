...
namespace dawn_native {
    ...
    // template <typename T>
    // std::string to_string_with_precision(const float a_value, const int n = 8) {
    ...
    // std::string GetHLSLValueString(EntryPointMetadata::OverridableConstant::Type dawnType, const OverridableConstantScalar* entry,
    //                               double value = 0) {
    ...
    //                     return to_string_with_precision(entry ? entry->f32 : static_cast<float>(value));
    ...
    //     static StringType toStringFunc(T v) {
    //         return std::is_same<StringType, std::string>::value ? std::to_string(v) : std::to_wstring(v);
    //     }
    // };
    // template<typename StringType>
    // static constexpr std::enable_if<std::is_same<StringType, std::string>::value, std::string>::type
    // struct NumberToString {
    //     template<typename T>
    //     static StringType toStringFunc(T v) {
    //         return std::is_same<StringType, std::string>::value ? std::to_string(v) : std::to_wstring(v);
    ...
    // template<typename T>
    // std::string NumberToString<std::string, T>::toStringFunc(T v) {
    ...
    struct NumberToString<std::string, T> {
        static std::string toStringFunc(T v) {
            return std::to_string(v);
        }
    };
    ...
    struct NumberToString<std::string, float> {
        static std::string toStringFunc(float v) {
            std::ostringstream out;
            // 32 bit float has 7 decimal digits of precision so setting n to 8 should be enough
            out.precision(8);
            out << std::fixed << v;
    ...
    struct NumberToString<std::string, uint32_t> {
        static std::string toStringFunc(uint32_t v) {
    ...
    struct NumberToString<std::wstring, T> {
        static std::wstring toStringFunc(T v) {
    ...
    // template<typename StringType, typename T>
    // struct NumberToString {
    //     static StringType toStringFunc(typename std::enable_if_t<std::is_same<uint32_t, T>::value> v);
    // };
    ...
    // template<>
    // std::string NumberToString<std::string, uint32_t>::toStringFunc(uint32_t v) {
    //     return std::to_string(v);
    // }
    // template<>
    // std::wstring NumberToString<std::wstring, uint32_t>::toStringFunc(uint32_t v) {
    //     return std::to_wstring(v);
    // }

//     template<typename StringType, typename T>
// typename std::enable_if_t<std::is_same<StringType, std::string>::value, std::string>
// toStringFunc(T v) {
//     return std::to_string(v);
// }

// template<typename StringType, typename T>
// typename std::enable_if_t<std::is_same<StringType, std::wstring>::value, std::wstring>
// toStringFunc(T v) {
//     return std::to_wstring(v);
// }
    ...
    // template<typename T>
    // std::string NumberToString<std::string, T>::toStringFunc(typename std::enable_if_t<std::is_same<uint32_t, T>::value> v) {
    //     return std::to_string<T>(v);
    // }

    // template <
    //     typename StringType = std::string,
    //     typename toStringFunc = (std::is_same<StringType, std::string> ? std::to_string : std::to_wstring)
    // >
    template <typename StringType = std::string>
    StringType GetHLSLValueString(EntryPointMetadata::OverridableConstant::Type dawnType, const OverridableConstantScalar* entry,
                                  double value = 0) {
            // constexpr auto toStringFunc = std::is_same<StringType, std::string>::value ? std::to_string : std::to_wstring;
            // constexpr auto toStringFunc = std::is_same<StringType, std::string>::value ? decltype(std::to_string) : decltype(std::to_wstring);
            switch (dawnType) {
                    case EntryPointMetadata::OverridableConstant::Type::Boolean:
                        // return toStringFunc<std::string, int32_t>(entry ? entry->b : static_cast<int32_t>(value));
                        return NumberToString<StringType, int32_t>::toStringFunc(entry ? entry->b : static_cast<int32_t>(value));
                    case EntryPointMetadata::OverridableConstant::Type::Float32:
                        // return to_string_with_precision(entry ? entry->f32 : static_cast<float>(value));
                        return NumberToString<StringType, float>::toStringFunc(entry ? entry->f32 : static_cast<float>(value));
                    case EntryPointMetadata::OverridableConstant::Type::Int32:
                        // return toStringFunc<std::string, int32_t>(entry ? entry->i32 : static_cast<int32_t>(value));
                        return NumberToString<StringType, int32_t>::toStringFunc(entry ? entry->i32 : static_cast<int32_t>(value));
                    case EntryPointMetadata::OverridableConstant::Type::Uint32:
                        // return toStringFunc<std::string, uint32_t>(entry ? entry->u32 : static_cast<uint32_t>(value)) + "u";
                        return NumberToString<StringType, uint32_t>::toStringFunc(entry ? entry->u32 : static_cast<uint32_t>(value));
                        // return NumberToString<StringType, uint32_t>::toStringFunc(entry ? entry->u32 : static_cast<uint32_t>(value)) + "u";
                        // return NumberToString<std::string>::toStringFunc(entry ? entry->u32 : static_cast<uint32_t>(value)) + "u";
                    default:
                        UNREACHABLE();
                }
            }
...
namespace dawn_native { namespace d3d12 {
    ...
    namespace {
        ...
        template <typename DefineType, typename stringType>
        void GetOverridableConstantsDefines(const ProgrammableStage& programmableStage, std::vector<DefineType>* defines, std::vector<std::pair<stringType, stringType>>* defineStrings) {
            // // Build defines for overridable constants
            // const D3D_SHADER_MACRO* pDefines = nullptr;
            // std::vector<D3D_SHADER_MACRO> fxcDefines;

            // // Store define strings
            // // Make sure live until it's passed to D3D API
            ...
            const EntryPointMetadata& entryPointMetadata =
                ...
                for (const auto& pipelineConstant : programmableStage.constants) {
                    ...
                    defineStrings->emplace_back(kSpecConstantPrefix + std::to_string(moduleConstant.id), GetHLSLValueString(moduleConstant.type, nullptr, value));
                }

                // Set shader initialized default values
                // D3D12 HLSL shaders use defines so we should define every constants regardless whether they are reference by current entryPoint or not to pass shader compilation
                const EntryPointMetadataTable& entryPoints = programmableStage.module->GetAllEntryPoints();
                for (const auto& it : entryPoints) {
                    // printf("\n\n!!!!!!!!!!! %s \n\n", it.first.c_str());
                    const EntryPointMetadata* curEntryPointMetadata = it.second.get();
                    for (const auto& iter : curEntryPointMetadata->overridableConstants) {
                        ...
                        const auto& moduleConstant = curEntryPointMetadata->overridableConstants.at(name);

                        // uninitialized default values are okay since they are only defined to pass compilation but not used
                        defineStrings->emplace_back(kSpecConstantPrefix + std::to_string(moduleConstant.id), GetHLSLValueString(moduleConstant.type, &moduleConstant.defaultValue));
                    }
                }

                // Needs to populate defines at the end to avoid vector reallocation messing up with char pointers
                // for (size_t i = 0; i < defineStrings->size(); i++) {
                    // const auto& d = defineStrings->at(i);
                for (const auto& d : *defineStrings) {
                    defines->push_back({d.first.c_str(), d.second.c_str()});
                }

                // if (defines.size() > 0) {
                //     // d3dCompile D3D_SHADER_MACRO* pDefines is a nullptr terminated array
                //     defines.push_back({nullptr});
                //     pDefines = defines.data();
                // }
                


                // for (size_t i = 0; i < defines.size() - 1; i++) {
        ...
        ResultOrError<ComPtr<ID3DBlob>> CompileShaderFXC(const PlatformFunctions* functions,
            ...
            GetOverridableConstantsDefines(programmableStage, &fxcDefines, &defineStrings);

            if (fxcDefines.size() > 0) {
                // d3dCompile D3D_SHADER_MACRO* pDefines is a nullptr terminated array
