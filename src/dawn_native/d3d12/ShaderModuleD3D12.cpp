    ...
    namespace {
        ...
        ResultOrError<ComPtr<IDxcBlob>> TranslateToDxilThroughSPIRV(
            ...
            switch (request.stage) {
                ...
                case SingleShaderStage::Compute:
                    if (request.usesNumWorkgroups) {
                        conf.runtime_data_cbv.register_space = request.numWorkgroupsRegisterSpace;
                        conf.runtime_data_cbv.base_shader_register =
                            request.numWorkgroupsShaderRegister;
                    }
                    break;
            }
        ...
        MaybeError CompileShader(const PlatformFunctions* functions,
            ...
            if (dumpShaders && request.compiler == ShaderCompilationRequest::Compiler::FXC) {
                ...
                if (FAILED(functions->d3dDisassemble(
                        compiledShader->compiledDXBCShader->GetBufferPointer(),
                        compiledShader->compiledDXBCShader->GetBufferSize(), 0, nullptr,
                        &disassembly))) {
