    ...
    namespace {
        ...
        struct ShaderCompilationRequest {
            ...
            static ResultOrError<ShaderCompilationRequest> Create(
                ...
                request.programmableStage = &programmableStage;
            ...
            ResultOrError<PersistentCacheKey> CreateCacheKey() const {
                ...
                const EntryPointMetadata& entryPointMetadata =
                    programmableStage->module->GetEntryPoint(entryPointName);
                ...
                for (const auto& it : entryPoints) {
                    ...
                    for (const auto& iter : curEntryPointMetadata->overridableConstants) {
                        ...
                        // Either the constant has default value or not
                        // it is already settled down by the shader source code
                        // so we only store its identifier for cache keys
                        stream << " <" << name << ">";
                    }
                }
                stream << " }";
        ...
        ResultOrError<ComPtr<IDxcBlob>> CompileShaderDXC(IDxcLibrary* dxcLibrary,
            ...
            const auto& defineStrings =
                GetOverridableConstantsDefines<std::wstring>(*request.programmableStage);
        ...
        ResultOrError<ComPtr<ID3DBlob>> CompileShaderFXC(const PlatformFunctions* functions,
            ...
            // Build defines for overridable constants
            const auto& defineStrings =
                GetOverridableConstantsDefines<std::string>(*request.programmableStage);
