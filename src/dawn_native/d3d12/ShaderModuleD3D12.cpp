    ...
    ResultOrError<std::string> ShaderModule::GetHLSLSource(PipelineLayout* layout) {
        ...
        for (uint32_t group : IterateBitSet(layout->GetBindGroupLayoutsMask())) {
            ...
            for (const auto& it : groupBindingInfo) {
                ...
                // the BGL produces the wrong output. If this happens, force these SRV bindings to
                // be treated as UAV.
                const bool do_force_uav =
                ...
                if (GetDevice()->IsToggleEnabled(Toggle::UseSpvc)) {
                    DAWN_TRY(CheckSpvcSuccess(
                        ...
                        "spvc"));
                    // TODO(dawn:410): Add support for force UAVs when available.
                } else {
