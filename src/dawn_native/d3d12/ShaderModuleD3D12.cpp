...
namespace dawn_native { namespace d3d12 {
    ...
    namespace {
        ...
        ResultOrError<ComPtr<ID3DBlob>> CompileShaderFXC(const PlatformFunctions* functions,
            ...
            // Make sure what LPCSTR points to live until passed to d3dCompile
            // TODO: or use new and delete?
            ...
            // if (entryPointMetadata.overridableConstants.size() > 0) {
                ...
                for (const auto& pipelineConstant : programmableStage.constants) {
                    ...
                    defines.emplace_back(kSpecConstantPrefix + std::to_string(moduleConstant.id), GetHLSLValueString(moduleConstant.type, nullptr, value));
                    // fxcDefines.push_back({defines.back().first.c_str(), defines.back().second.c_str()});
                ...
                // D3D12 HLSL shaders use defines so we should define every constants regardless whether they are reference by current entryPoint
                ...
                for (const auto& it : entryPoints) {
                    ...
                    // for (const std::string& name : curEntryPointMetadata->initializedOverridableConstants) {
                    for (const auto& iter : curEntryPointMetadata->overridableConstants) {
                        ...
                        // uninitialized entries for other entry points are defined but not used, but needed to pass hlsl compile
                        defines.emplace_back(kSpecConstantPrefix + std::to_string(moduleConstant.id), GetHLSLValueString(moduleConstant.type, &moduleConstant.defaultValue));
                        // fxcDefines.push_back({defines.back().first.c_str(), defines.back().second.c_str()});
                    }
                }

                // Needs to populate at the end to avoid reallocation mess up with pointers
