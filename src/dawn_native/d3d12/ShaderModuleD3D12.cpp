...
namespace dawn_native { namespace d3d12 {
    ...
    std::string ShaderModule::GetHLSLSource(PipelineLayout* layout) const {
        ...
        if (GetDevice()->IsToggleEnabled(Toggle::UseSpvc)) {
            ...
            spirv_cross::CompilerHLSL* compiler = compiler_impl.get();
        ...
        if (GetDevice()->IsToggleEnabled(Toggle::UseSpvc)) {
            mSpvcContext.CompileShader(&mSpvcResult);
            // TODO(rharrison): Check status & have some sort of meaningful error path
            return mSpvcResult.GetStringOutput();
        } else {
