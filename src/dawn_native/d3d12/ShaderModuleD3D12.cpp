...
namespace dawn_native { namespace d3d12 {
    ...
    ResultOrError<ScopedCachedBlob> ShaderModule::TranslateToDXILWithMesa(
        ...
        std::unique_ptr<uint8_t[]> data(new uint8_t[dxil.binary.size]);
        const uint8_t* dxil_buffer = static_cast<uint8_t*>(dxil.binary.buffer);
        std::copy(dxil_buffer, dxil_buffer + dxil.binary.size, data.get());
        functions->spirvToDxilFree(&dxil);

        return ScopedCachedBlob{std::move(data), dxil.binary.size};
    }
                ...
                shaderCacheKey, [&](auto doCache) -> MaybeError {
                    if (device->IsToggleEnabled(Toggle::UseMesa)) {
                        ...
                        DAWN_TRY_ASSIGN(
                            ...
                            TranslateToDXILWithMesa(device, entryPointName, stage, layout));
                    } else {
                        // Compile the source shader to DXIL via HLSL.
                        std::string hlslSource;
                        std::string remappedEntryPoint;
                        DAWN_TRY_ASSIGN(hlslSource, TranslateToHLSL(entryPointName, stage, layout,
                                                                    &remappedEntryPoint));
    ...
    ResultOrError<PersistentCacheKey> ShaderModule::CreateWGSLKey(const char* entryPointName,
        ...
        // If the source contains multiple entry points, ensure they are cached seperately per stage
        // since DX shader code can only be compiled per stage using the same entry point.
