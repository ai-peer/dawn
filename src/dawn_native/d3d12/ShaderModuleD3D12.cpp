...
namespace dawn_native { namespace d3d12 {
    ...
    namespace {
        ...
        struct ShaderCompilationRequest {
            ...
            static ResultOrError<ShaderCompilationRequest> Create(
                ...
                if (compiler == Compiler::DXC || compiler == Compiler::Mesa) {
                    // The Mesa backend uses dxil.dll to sign its output, so we need to include the
                    // DXC version.
                ...
                request.mesaVersion = compiler == Compiler::Mesa ? mesaVersion : 0;
        ...
        ResultOrError<ComPtr<IDxcBlob>> CompileShaderMesa(const PlatformFunctions* functions,
            ...
            // Free dxil with a unique_ptr custom deleter to ensure it is cleaned up.
            size_t dataLen = dxil.binary.size;
            ...
            ComPtr<IDxcBlobEncoding> dxilUnvalidated;
            DAWN_TRY(CheckHRESULT(dxcLibrary->CreateBlobWithEncodingFromPinned(
                                      data.get(), dataLen, CP_ACP, &dxilUnvalidated),
                                  "DXC create blob"));

            ComPtr<IDxcOperationResult> validationResult;
            DAWN_TRY(
                CheckHRESULT(dxcValidator->Validate(dxilUnvalidated.Get(),
                                                    DxcValidatorFlags_Default, &validationResult),
                             "DXC validate dxil"));
            ...
            switch (request.compiler) {
                case ShaderCompilationRequest::Compiler::Mesa:
                    ...
                    DAWN_TRY_ASSIGN(
                        ...
                        CompileShaderMesa(functions, dxcLibrary, dxcValidator, request));
                    break;
                ...
                shaderCacheKey, [&](auto doCache) -> MaybeError {
                    DAWN_TRY(CompileShader(
                        ...
                        device->IsToggleEnabled(Toggle::UseDXC) ? device->GetDxcCompiler().Get()
                                                                : nullptr,
                        device->IsToggleEnabled(Toggle::UseMesa) ? device->GetDxcValidator().Get()
                                                                 : nullptr,
    ...
    D3D12_SHADER_BYTECODE CompiledShader::GetD3D12ShaderBytecode() const {
        ...
        } else if (compiledMesaShader != nullptr) {
            return {compiledMesaShader->GetBufferPointer(), compiledMesaShader->GetBufferSize()};
        }
        UNREACHABLE();
        return {};
    }
