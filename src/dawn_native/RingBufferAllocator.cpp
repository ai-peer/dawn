...
namespace dawn_native {
    ...
    RingBufferAllocator::RingBufferAllocator(DeviceBase* device, StagingBufferBase* buffer)
        : mStagingBuffer(buffer), mDevice(device) {
        mBufferSize = mStagingBuffer->GetSize();
    }

    MaybeError RingBufferAllocator::Initialize() {
    ...
    void RingBufferAllocator::Track() {
    ...
    void RingBufferAllocator::Tick(Serial lastCompletedSerial) {
        Track();
    ...
    StagingBufferBase* RingBufferAllocator::GetStagingBuffer() const {
    ...
    size_t RingBufferAllocator::Allocate(size_t allocationSize) {
        ASSERT(mStagingBuffer != nullptr);
        ...
        if (mUsedStartOffset <= mUsedEndOffset) {
            ...
            // wrapped).
            if (mUsedEndOffset + allocationSize <= mBufferSize) {
                ...
                const size_t requestSize = (mBufferSize - mUsedEndOffset) + allocationSize;
