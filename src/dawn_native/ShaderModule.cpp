...
namespace dawn_native {
    ...
    ResultOrError<ShaderModuleBase*> ShaderModuleBase::ConvertToPulling(
        const VertexStateDescriptor& vertexState,
        const std::string& entryPoint) {
        std::vector<uint32_t> pullingSpirv;
        DAWN_TRY_ASSIGN(pullingSpirv,
                        ConvertWGSLToSPIRVWithPulling(mSpirv, vertexState, entryPoint));

        ShaderModuleSPIRVDescriptor spirvDesc;
        spirvDesc.codeSize = static_cast<uint32_t>(pullingSpirv.size());
        spirvDesc.code = pullingSpirv.data();

        ShaderModuleDescriptor descriptor;
        descriptor.nextInChain = &spirvDesc;

        return GetDevice()->CreateShaderModule(&descriptor);
    }
    ...
    ResultOrError<std::vector<uint32_t>> ShaderModuleBase::GeneratePullingSpirv(
        ...
        return ConvertWGSLToSPIRVWithPulling(mSpirv, vertexState, entryPoint);
    }
