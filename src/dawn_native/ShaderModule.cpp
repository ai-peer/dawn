...
namespace dawn_native {
    ...
    std::vector<uint64_t> ShaderModuleBase::GetBindGroupLayoutBufferSizes(
        ...
        for (const auto& it : layout->GetBindingMap()) {
            ...
            switch (layout->GetBindingInfo(bindingIndex).type) {
                case wgpu::BindingType::UniformBuffer:
                case wgpu::BindingType::StorageBuffer:
                case wgpu::BindingType::ReadonlyStorageBuffer: {
                    // Shader does not require this buffer, so 0 minimum size allows any
                    if (indexToSize.find(bindingIndex) == indexToSize.end()) {
