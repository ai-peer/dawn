                ...
                if (metadata->stage == SingleShaderStage::Fragment) {
                    ...
                    for (const auto& output_var : entryPoint.output_variables) {
                        if (!output_var.has_location_decoration) {
                            return DAWN_VALIDATION_ERROR(
                                "Need location qualifier on fragment output");
                        }
        ...
        MaybeError PopulateMetadataUsingSPIRVCross(DeviceBase* device,
            ...
            for (auto& crossMember : crossTable) {
                ...
                if (tintEntry->stage == SingleShaderStage::Fragment) {
                    ...
                    // variables in the written bitset. Instead testing that SPIRV-Cross's bitset is
                    // a superset of Tint's.
                    for (uint8_t index = 0; index < kMaxColorAttachments; index++) {
                        auto ca_index = ColorAttachmentIndex(index);
                        if (tintEntry->fragmentOutputsWritten[ca_index]) {
                            if (tintEntry->fragmentOutputFormatBaseTypes[ca_index] !=
                                crossEntry->fragmentOutputFormatBaseTypes[ca_index]) {
                                return DAWN_VALIDATION_ERROR(
                                    "Tint and SPIRV-Cross returned different values for used "
                                    "fragment "
                                    "output base type");
                            }
                        }
