...
namespace dawn_native {
    ...
    MaybeError ShaderModuleBase::ValidateCompatibilityWithPipelilneLayout(
        ...
        for (const auto& it : mBindingInfo[group]) {
            ...
            if (bindingInfo.type != moduleInfo.type) {
                ...
                if (!validBindingConversion) {
                    return DAWN_VALIDATION_ERROR(
                        "The binding type of the bind group layout entry mismatches " +
                        GetShaderDeclarationString(group, bindingNumber));
                }
            }
            ...
            if ((bindingInfo.visibility & StageBit(mExecutionModel)) == 0) {
                return DAWN_VALIDATION_ERROR(
                    "The bind group layout entry is not visible to the shader execution model");
            }
