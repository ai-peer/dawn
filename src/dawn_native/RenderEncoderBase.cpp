...
namespace dawn_native {
    ...
    void RenderEncoderBase::APIDrawIndexedIndirect(BufferBase* indirectBuffer,
        ...
        mEncodingContext->TryEncode(this, [&](CommandAllocator* allocator) -> MaybeError {
            ...
            cmd->indirectBufferRef = new DeferredBufferRef();
            if (IsValidationEnabled()) {
                // `cmd` will be populated when the EncodingContext encodes a validation pass to
                // precede our enclosing render pass.
                mValidationMetadata.AddIndexedIndirectDraw(&mCommandBufferState, indirectBuffer,
                                                           indirectOffset,
                                                           cmd->indirectBufferRef.Get());
            } else {
                cmd->indirectBufferRef->SetBuffer(indirectBuffer, indirectOffset);
            }
