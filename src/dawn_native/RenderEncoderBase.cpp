    ...
    void RenderEncoderBase::APIDraw(uint32_t vertexCount,
        ...
        mEncodingContext->TryEncode(this, [&](CommandAllocator* allocator) -> MaybeError {
            if (IsValidationEnabled()) {
                ...
                DAWN_VALIDATION_ERROR_IF(mDisableBaseInstance && firstInstance != 0,
    ...
    void RenderEncoderBase::APIDrawIndexed(uint32_t indexCount,
        ...
        mEncodingContext->TryEncode(this, [&](CommandAllocator* allocator) -> MaybeError {
            if (IsValidationEnabled()) {
                ...
                DAWN_VALIDATION_ERROR_IF(mDisableBaseVertex && baseVertex != 0,
    ...
    void RenderEncoderBase::APIDrawIndirect(BufferBase* indirectBuffer, uint64_t indirectOffset) {
        mEncodingContext->TryEncode(this, [&](CommandAllocator* allocator) -> MaybeError {
            if (IsValidationEnabled()) {
                ...
                DAWN_VALIDATION_ERROR_IF(indirectOffset % 4 != 0,
                                         "Indirect offset (%u) was not a multiple of 4",
                                         indirectOffset);

                DAWN_VALIDATION_ERROR_IF(
                    indirectOffset >= indirectBuffer->GetSize() ||
                        kDrawIndirectSize > indirectBuffer->GetSize() - indirectOffset,
                    "Indirect offset (%u) out of bounds of indirect buffer %s size (%u).",
    ...
    void RenderEncoderBase::APISetIndexBuffer(BufferBase* buffer,
        ...
        mEncodingContext->TryEncode(this, [&](CommandAllocator* allocator) -> MaybeError {
            if (IsValidationEnabled()) {
                ...
                DAWN_VALIDATION_ERROR_IF(format == wgpu::IndexFormat::Undefined,
                                         "Index format must be specified");

                DAWN_VALIDATION_ERROR_IF(offset % uint64_t(IndexFormatSize(format)) != 0,
                                         "Index buffer %s offset (%u) was not a multiple of the "
                                         "index format (%s) size (%u).",
                                         buffer, offset, format, IndexFormatSize(format));

                uint64_t bufferSize = buffer->GetSize();
                DAWN_VALIDATION_ERROR_IF(
                    offset > bufferSize,
                    "Index buffer %s offset (%u) was larger than the buffer size (%u),", buffer,
                    offset, bufferSize);
                ...
                if (size == 0) {
                    ...
                    DAWN_VALIDATION_ERROR_IF(size > remainingSize,
                                             "Size + offset larger than the buffer size");
                }
            } else {
                if (size == 0) {
        ...
        mEncodingContext->TryEncode(this, [&](CommandAllocator* allocator) -> MaybeError {
            if (IsValidationEnabled()) {
                ...
                DAWN_VALIDATION_ERROR_IF(
                    ...
                    "Vertex buffer %s slot (%u) was not less than the maximum (%u)", buffer, slot,
                    kMaxVertexBuffers);

                DAWN_VALIDATION_ERROR_IF(offset % 4 != 0,
                                         "Vertex buffer %s offset (%u) was not a multiple of 4",
                                         buffer, offset);

                uint64_t bufferSize = buffer->GetSize();
                DAWN_VALIDATION_ERROR_IF(
                    offset > bufferSize,
                    "Vertex buffer %s offset (%u) was larger than the buffer size (%u),", buffer,
                    offset, bufferSize);
