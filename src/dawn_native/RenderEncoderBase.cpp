...
namespace dawn_native {
    ...
    void RenderEncoderBase::APIDrawIndirect(BufferBase* indirectBuffer, uint64_t indirectOffset) {
        TRY_ENCODE_CONTEXT(
            ...
            "encoding DrawIndirect with %s", indirectBuffer);
    }
    ...
    void RenderEncoderBase::APIDrawIndexedIndirect(BufferBase* indirectBuffer,
        ...
        TRY_ENCODE_CONTEXT(
            ...
            "encoding DrawIndexedIndirect with %s", indirectBuffer);
    }
    ...
    void RenderEncoderBase::APISetIndexBuffer(BufferBase* buffer,
        ...
        TRY_ENCODE_CONTEXT(
            ...
            [&](CommandAllocator* allocator) -> MaybeError {
                if (IsValidationEnabled()) {
                    ...
                    DAWN_INVALID_IF(offset % uint64_t(IndexFormatSize(format)) != 0,
                                    "Index buffer offset (%u) was not a multiple of the size (%u)"
                                    "of %s.",
                                    offset, IndexFormatSize(format), format);

                    uint64_t bufferSize = buffer->GetSize();
                    DAWN_INVALID_IF(offset > bufferSize,
                                    "Index buffer offset (%u) was larger than the size (%u) of %s.",
            ...
            "encoding SetIndexBuffer with %s", buffer);
    }
    ...
    void RenderEncoderBase::APISetVertexBuffer(uint32_t slot,
        ...
        TRY_ENCODE_CONTEXT(
            ...
            [&](CommandAllocator* allocator) -> MaybeError {
                if (IsValidationEnabled()) {
                    ...
                    DAWN_INVALID_IF(slot >= kMaxVertexBuffers,
                                    "Vertex buffer slot (%u) was not less than the maximum (%u)",
                                    slot, kMaxVertexBuffers);

                    DAWN_INVALID_IF(offset % 4 != 0,
                                    "Vertex buffer offset (%u) was not a multiple of 4", offset);
            ...
            "encoding SetVertexBuffer with %s at slot %u", buffer, slot);
    }
