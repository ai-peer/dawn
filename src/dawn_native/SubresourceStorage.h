...
namespace dawn_native {
    ...
    // subresources inside a render pass: the vast majority of texture view will select the whole
    ...
    // same time allow to have per-subresource data when needed.
    //
    ...
    // container uses a more functional approach of calling a closer on the interesting ranges.
    ...
    class SubresourceStorage {
      public:
        ...
        // Given an iterateFunc convertible to void(const SubresourceRange& range, const T& data),
        ...
        //
        //   bool isError = false;
        //   subresources.Iterate([&isError](const SubresourceRange& range, const T& data) {
        //       if (data == kBadStuff) isError = true;
        //   });
        template <typename F>
        void Iterate(F&& iterateFunc) const;

        // Given an updateFunc convertible to void(const SubresourceRange& range, T* data),
