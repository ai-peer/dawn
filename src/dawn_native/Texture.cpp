...
namespace dawn_native {
    ...
    TextureAspect SingleAspect(const Format& format, wgpu::TextureAspect aspect) {
        switch (aspect) {
            case wgpu::TextureAspect::All: {
                ASSERT(format.aspectMask.count() == 1);
                if (format.aspectMask[TextureAspect::Color]) {
                    return TextureAspect::Color;
                } else if (format.aspectMask[TextureAspect::Depth]) {
                    return TextureAspect::Depth;
                } else if (format.aspectMask[TextureAspect::Stencil]) {
            ...
            case wgpu::TextureAspect::DepthOnly:
                ASSERT(format.aspectMask[TextureAspect::Depth]);
                return TextureAspect::Depth;
                break;
            case wgpu::TextureAspect::StencilOnly:
                ASSERT(format.aspectMask[TextureAspect::Stencil]);
                return TextureAspect::Stencil;
                break;
            default:
                UNREACHABLE();
    ...
    AspectMask SingleAspectMask(const Format& format, wgpu::TextureAspect aspect) {
        return SingleAspectMask(SingleAspect(format, aspect));
    }
    ...
    SubresourceRange SubresourceRange::SingleSubresource(uint32_t baseMipLevel,
        ...
        return {baseMipLevel, 1, baseArrayLayer, 1, SingleAspectMask(aspect)};
    }
    ...
    TextureBase::TextureBase(DeviceBase* device,
        ...
        : ObjectBase(device),
          ...
          mSubresourcePlaneCount(device->GetSubresourcePlaneCount(mFormat)),
          mSize(descriptor->size),
          mMipLevelCount(descriptor->mipLevelCount),
          mSampleCount(descriptor->sampleCount),
          mUsage(descriptor->usage),
          mState(state) {
        for (TextureAspect aspect : IterateBitSet(mFormat.aspectMask)) {
            mPlaneIndices[aspect] = device->GetSubresourcePlaneIndex(mFormat, aspect);
        }
    ...
    SubresourceRange TextureBase::GetAllSubresources() const {
        ASSERT(!IsError());
        return {0, mMipLevelCount, 0, GetArrayLayers(), mFormat.aspectMask};
    }
    ...
    uint32_t TextureBase::GetSubresourceCount() const {
        ASSERT(!IsError());
        return mMipLevelCount * mSize.depth * mSubresourcePlaneCount;
    }
    ...
    uint32_t TextureBase::GetSubresourceIndex(uint32_t mipLevel,
        ...
        ASSERT(mFormat.aspectMask[aspect]);
        ...
        return mipLevel + arraySlice * GetNumMipLevels() +
               mPlaneIndices[aspect] * GetNumMipLevels() * GetArrayLayers();
    }
    ...
    TextureViewBase::TextureViewBase(TextureBase* texture, const TextureViewDescriptor* descriptor)
        ...
        switch (descriptor->aspect) {
            case wgpu::TextureAspect::All:
                mRange.aspectMask = GetFormat().aspectMask;
                break;
            case wgpu::TextureAspect::DepthOnly:
                ASSERT(mFormat.aspectMask[TextureAspect::Depth]);
                mRange.aspectMask = SingleAspectMask(TextureAspect::Depth);
                break;
            case wgpu::TextureAspect::StencilOnly:
                ASSERT(mFormat.aspectMask[TextureAspect::Stencil]);
                mRange.aspectMask = SingleAspectMask(TextureAspect::Stencil);
                break;
            default:
                UNREACHABLE();
                break;
