    ...
    namespace {
        ...
        MaybeError ValidateSampleCount(const TextureDescriptor* descriptor,
            ...
            if (descriptor->sampleCount > 1) {
                DAWN_INVALID_IF(descriptor->mipLevelCount > 1,
                                "The mipmap level count (%u) of a multisampled texture is not 1.",
        ...
        MaybeError ValidateTextureViewDimensionCompatibility(
            ...
            DAWN_INVALID_IF(
                ...
                "(%u).",
                descriptor->dimension, descriptor->arrayLayerCount);
            ...
            DAWN_INVALID_IF(
                !IsTextureSizeValidForTextureViewDimension(descriptor->dimension,
                                                           texture->GetSize()),
                "The dimension (%s) of the texture view is not compatible with the size (%s) of "
                "the original texture.",
                descriptor->dimension, &texture->GetSize());
