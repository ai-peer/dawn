...
namespace dawn_native {
    ...
    // TODO(jiawei.shao@intel.com): support validation on all texture view dimensions
    bool IsValidArrayLayerToTextureViewDimension(dawn::TextureViewDimension textureViewDimension,
    ...
    MaybeError ValidateTextureViewDimensionCompatibility(const TextureBase* texture,
                                                         const TextureViewDescriptor* descriptor) {
        if (!IsValidArrayLayerToTextureViewDimension(descriptor->dimension,
                                                     descriptor->layerCount)) {
            return DAWN_VALIDATION_ERROR(
                "The dimension of the texture view is not compatible to the layer count");
        }

        if (!IsTextureViewDimensionCompatibleToTextureDimension(descriptor->dimension,
