...
namespace dawn_native {
    namespace {
        ...
        Format ConvertFormatInternal(dawn::TextureFormat format) {
            auto MakeColorFormat = [](dawn::TextureFormat format, bool renderable, uint32_t byteSize) -> Format {
                Format result;
            ...
            auto MakeDepthStencilFormat = [](dawn::TextureFormat format, Format::Aspect aspect, uint32_t byteSize) -> Format {
                Format result;
                result.format = format;
            ...
            auto MakeCompressedFormat = [](dawn::TextureFormat format, uint32_t byteSize, uint32_t width, uint32_t height) -> Format {
                Format result;
                result.format = format;
        ...
        MaybeError ValidateTextureSize(const TextureDescriptor* descriptor, const Format& format) {
            ...
            if (format.isCompressed && (descriptor->size.width % format.blockWidth != 0 || descriptor->size.height % format.blockHeight != 0)) {
                return DAWN_VALIDATION_ERROR(
