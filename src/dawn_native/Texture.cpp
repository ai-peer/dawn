    ...
    namespace {
        ...
        MaybeError ValidateSampleCount(const TextureDescriptor* descriptor, const Format* format) {
            ...
            if (descriptor->sampleCount > 1) {
                ...
                if (descriptor->usage &
                    (wgpu::TextureUsage::Storage | wgpu::TextureUsage::ReadonlyStorage)) {
                    return DAWN_VALIDATION_ERROR(
                        "The sample counts of the storage textures and read-only storage textures "
                        "must be 1.");
                }
            }
        ...
        MaybeError ValidateTextureUsage(const TextureDescriptor* descriptor, const Format* format) {
            ...
            if (!format->supportStorageUsage &&
                descriptor->usage &
                    (wgpu::TextureUsage::Storage | wgpu::TextureUsage::ReadonlyStorage)) {
                return DAWN_VALIDATION_ERROR(
                    "Format cannot be used in storage or read-only storage textures");
            }
