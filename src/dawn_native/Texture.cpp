...
namespace dawn_native {
    namespace {
        ...
        MaybeError ValidateSampleCount(const TextureDescriptor* descriptor,
                                       const DawnTextureInternalUsageDescriptor* internalUsageDesc,
                                       const Format* format) {
    ...
    MaybeError ValidateTextureDescriptor(const DeviceBase* device,
        ...
        if (internalUsageDesc && !device->IsExtensionEnabled(Extension::DawnInternalUsages)) {
        ...
        DAWN_TRY(ValidateTextureUsage(descriptor, internalUsageDesc, format));
        DAWN_TRY(ValidateTextureDimension(descriptor->dimension));
        DAWN_TRY(ValidateSampleCount(descriptor, internalUsageDesc, format));
