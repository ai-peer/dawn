...
namespace dawn_native {
    namespace {
        ...
        uint8_t GetPlaneIndex(Aspect aspect) {
            ...
            switch (aspect) {
                case Aspect::Color:
                    return ConstexprLog2(static_cast<uint8_t>(Aspect::Color));
                case Aspect::Depth:
                    return ConstexprLog2(static_cast<uint8_t>(Aspect::Depth));
                case Aspect::Stencil:
                    return ConstexprLog2(static_cast<uint8_t>(Aspect::Stencil));
                default:
                    UNREACHABLE();
                    return 0;
            }
    ...
    Aspect SingleAspect(const Format& format, wgpu::TextureAspect aspect) {
        Aspect aspectMask = GetAspects(format, aspect);
    ...
    uint32_t TextureBase::GetSubresourceIndex(uint32_t mipLevel,
        ...
        return mipLevel + arraySlice * GetNumMipLevels() +
               mPlaneIndices[GetPlaneIndex(aspect)] * GetNumMipLevels() * GetArrayLayers();
    }
    ...
    TextureViewBase::TextureViewBase(TextureBase* texture, const TextureViewDescriptor* descriptor)
        : ObjectBase(texture->GetDevice()),
          ...
          mRange({descriptor->baseMipLevel, descriptor->mipLevelCount, descriptor->baseArrayLayer,
                  descriptor->arrayLayerCount, GetAspects(mFormat, mAspect)}) {
