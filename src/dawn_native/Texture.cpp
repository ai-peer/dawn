...
namespace dawn_native {
    ...
    ResultOrError<Ref<CommandBufferBase>> TextureBase::GenerateClearTextureCommands(
        ...
        if ((GetUsage() & wgpu::TextureUsage::RenderAttachment) != 0 &&
            ...
            if (GetFormat().HasDepthOrStencil()) {
                ...
                for (uint32_t level = range.baseMipLevel;
                    ...
                    for (uint32_t arrayLayer = range.baseArrayLayer;
                        ...
                        attachment.depthLoadOp = (aspectsToClear & Aspect::Depth) == 0
                                                     ? wgpu::LoadOp::Load
                                                     : wgpu::LoadOp::Clear;
                        attachment.depthStoreOp = wgpu::StoreOp::Store;
                        attachment.clearDepth = clearDepth;
                        attachment.stencilLoadOp = (aspectsToClear & Aspect::Stencil) == 0
                                                       ? wgpu::LoadOp::Load
                                                       : wgpu::LoadOp::Clear;
            ...
            const uint32_t clearColor = (clearValue == TextureBase::ClearValue::Zero) ? 0x00 : 0x01;
            ...
            for (Aspect aspect : IterateEnumMask(range.aspects)) {
                ...
                auto AllocateStagingBuffer = [&](uint32_t level) -> MaybeError {
                    ...
                    uint64_t alignment =
                        std::max(uint64_t(std::max(4u, blockInfo.byteSize)),
