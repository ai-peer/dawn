...
namespace dawn_native {
    ...
    size_t ComputePipelineBase::CacheFuncs::operator()(const ComputePipelineBase* pipeline) const {
        size_t hash = 0;
        HashCombine(&hash, pipeline->mModule.Get(), pipeline->mEntryPoint, pipeline->GetLayout());
        return hash;
    }

    bool ComputePipelineBase::CacheFuncs::operator()(const ComputePipelineBase* a,
