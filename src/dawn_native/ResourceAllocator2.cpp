...
namespace dawn_native {
    ...
    // ResourceAllocationHandle

    ResourceAllocationHandle::ResourceAllocationHandle(
    ...
    size_t ResourceAllocationHandle::GetOffset() const {
        return mSubAllocationBlock.GetOffset();
    }

    void ResourceAllocationHandle::Reset() {
        mResourceHeap = nullptr;
    }

    ResultOrError<void*> ResourceAllocationHandle::Map() {
    ...
    void ResourceAllocationHandle::Unmap() {
        mResourceHeap->Unmap();
    }

    ResourceHeapBase* ResourceAllocationHandle::GetResourceHeap() const {
        return mResourceHeap;
    }

    const HeapSubAllocationBlock& ResourceAllocationHandle::GetSubAllocationBlock() const {
    ...
    LinearPoolAllocator<THeapBlock, TResourceAllocator, TBlockAllocator>::LinearPoolAllocator(
        size_t maxSize,
        size_t resourceSize,
    ...
    template <class THeapBlock, class TResourceAllocator, class TBlockAllocator>
    size_t LinearPoolAllocator<THeapBlock, TResourceAllocator, TBlockAllocator>::GetResourceIndex(
        size_t offset) const {
        return offset / mResourceSize;
    }

    template <class THeapBlock, class TResourceAllocator, class TBlockAllocator>
    THeapBlock LinearPoolAllocator<THeapBlock, TResourceAllocator, TBlockAllocator>::Allocate(
        size_t size) {
        // Allocation cannot exceed the specified allocator size.
        if (size > mBlockAllocator.GetMaxBlockSize()) {
            return THeapBlock{0, 0};  // Error/empty.
        }

        // Allocation cannot exceed the specified resource size.
        if (size > mResourceSize) {
            return THeapBlock{0, 0};
        }

        // Attempt to sub-allocate a block of the requested size.
        THeapBlock block = mBlockAllocator.Allocate(size);
        if (block.GetSize() != size) {  // Return empty block on failure.
            return block;
        }
        ...
        if (resourceIndex >= mTrackedResourceAllocations.size()) {
            ...
            for (size_t i = 0; i < numOfResourcesRequired; i++) {
                // Transfer ownership of the resource to this allocator.
                mTrackedResourceAllocations.push_back(
                    {/*refcount*/ 0, std::move(mResourceAllocator.Allocate(mResourceSize))});
            }
        }
    ...
    template <class THeapBlock, class TResourceAllocator, class TBlockAllocator>
    ResourceAllocationHandle
    LinearPoolAllocator<THeapBlock, TResourceAllocator, TBlockAllocator>::GetSubAllocation(
        const THeapBlock& block) const {
        const size_t resourceIndex = GetResourceIndex(block.GetOffset());
        return ResourceAllocationHandle(mTrackedResourceAllocations[resourceIndex].mResource,
                                        block);
    }

    template <class THeapBlock, class TResourceAllocator, class TBlockAllocator>
    void LinearPoolAllocator<THeapBlock, TResourceAllocator, TBlockAllocator>::Deallocate(
        THeapBlock block) {
        const size_t resourceIndex = GetResourceIndex(block.GetOffset());
    ...
    LinearPoolAllocator<THeapBlock, TResourceAllocator, TBlockAllocator>::GetBlockAllocator()
        const {
        return mBlockAllocator;
    }

    template <class THeapBlock, class TResourceAllocator, class TBlockAllocator>
    size_t LinearPoolAllocator<THeapBlock, TResourceAllocator, TBlockAllocator>::GetResourceCount()
        const {
        size_t allocatedCount = 0;
        for (auto& trackedResource : mTrackedResourceAllocations) {
    ...
    BuddyAllocator<TBlock>::BuddyAllocator(size_t size) : mMaxBlockSize(size) {
        ...
        BlockList blockList = {mRoot};
        mFreeLists[0] = blockList;
    }
    ...
    size_t BuddyAllocator<TBlock>::ComputeNumOfFreeBlocks(TBlock* block) const {
        if (block->mState == BlockNode::BlockState::Free)
            return 1;
        else if (block->mState == BlockNode::BlockState::Split) {
    ...
    size_t BuddyAllocator<TBlock>::ComputeLevelFromBlockSize(size_t blockSize) const {
        ...
        // TODO: Convert index by level to using order-n to eliminate Log2 calls.
        return Log2(mMaxBlockSize) - Log2(blockSize);  // TODO: Log2ceil?
    }
    ...
    void BuddyAllocator<TBlock>::InsertFreeBlock(TBlock* block, size_t level) {
        ASSERT(block->mState == BlockNode::BlockState::Free);

        // Child R gets inserted first then child L.
    ...
    TBlock BuddyAllocator<TBlock>::Allocate(size_t allocationSize) {
        ...
        while (currBlockLevel < allocationSizeToLevel) {
            ASSERT(currBlock->mState == BlockNode::BlockState::Free);
            ...
            currBlock->mState = BlockNode::BlockState::Split;
        ...
        RemoveFreeBlock(currBlock, currBlockLevel);
        currBlock->mState = BlockNode::BlockState::Allocated;
    ...
    void BuddyAllocator<TBlock>::Deallocate(TBlock block) {
        ...
        while (curr->mState == BlockNode::BlockState::Split) {
        ...
        curr->mState = BlockNode::BlockState::Free;

        // Merge the buddies (LevelN-to-Level0).
        while (currBlockLevel > 0 && curr->pBuddy->mState == BlockNode::BlockState::Free) {
            // Remove the buddy.
            ...
            // Parent is now free.
            parent->mState = BlockNode::BlockState::Free;
    ...
    void BuddyAllocator<TBlock>::DeleteBlock(TBlock* block) {
        ...
        if (block->mState == BlockNode::BlockState::Split) {
    ...
    // void ResourceAllocatorBase::Tick(uint64_t lastCompletedSerial) {
    //     // Each heap contains at-least one block. Each block has a serial which
    //     // is used to prevent erasing a heap contain allocations in-flight.
    //     for (size_t i = 0; i < mResourceHeaps.size(); ++i) {
    //         mResourceHeaps[i]->Tick(lastCompletedSerial);

    //         if (mResourceHeaps[i]->IsEmpty()) {
    //             mResourceHeaps.erase(mResourceHeaps.begin() + i);
    //         }
    //     }
    // }
