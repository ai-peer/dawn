        ...
        mEncodingContext->TryEncode(this, [&](CommandAllocator* allocator) -> MaybeError {
            if (IsValidationEnabled()) {
                ...
                DAWN_INVALID_IF(
                    ...
                    "Viewport bounds (x: %f, y: %f, width: %f, height: %f) contained a negative "
                    "value.",
                    x, y, width, height);

                DAWN_INVALID_IF(
                    x + width > mRenderTargetWidth || y + height > mRenderTargetHeight,
                    "Viewport bounds (x: %f, y: %f, width: %f, height: %f) were not contained in "
                ...
                DAWN_INVALID_IF(
                    ...
                    "Viewport minDepth (%f) and maxDepth (%f) were not in [0, 1] or minDepth was "
    ...
    void RenderPassEncoder::APIBeginOcclusionQuery(uint32_t queryIndex) {
        mEncodingContext->TryEncode(this, [&](CommandAllocator* allocator) -> MaybeError {
            if (IsValidationEnabled()) {
                ...
                DAWN_INVALID_IF(mOcclusionQueryActive, "An occlusion query (%u) is already active.",
