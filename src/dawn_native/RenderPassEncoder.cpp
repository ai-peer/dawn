...
namespace dawn_native {
    namespace {
        ...
        MaybeError ValidateQueryIndexOverwrite(QuerySetBase* querySet,
            ...
            DAWN_INVALID_IF(it != queryAvailabilityMap.end() && it->second[queryIndex],
                            "Wrote to the same index (%u) of %s twice in a single render pass.",
                            queryIndex, querySet);
            ...
            [&](CommandAllocator* allocator) -> MaybeError {
                if (IsValidationEnabled()) {
                    ...
                    DAWN_INVALID_IF(
                        ...
                        "Occlusion query (%u) of %s was not ended prior to ending the render pass.",
            ...
            [&](CommandAllocator* allocator) -> MaybeError {
                if (IsValidationEnabled()) {
                    DAWN_INVALID_IF(
                        ...
                        "minDepth: %f, maxDepth: %f) was NaN.",
                        x, y, width, height, minDepth, maxDepth);

                    DAWN_INVALID_IF(x < 0 || y < 0 || width < 0 || height < 0,
                                    "Viewport bounds (x: %f, y: %f, width: %f, height: %f) "
                    ...
                    DAWN_INVALID_IF(
                        ...
                        "Viewport bounds (x: %f, y: %f, width: %f, height: %f) were not contained "
                        "in "
                    ...
                    DAWN_INVALID_IF(minDepth < 0 || minDepth > maxDepth || maxDepth > 1,
                                    "Viewport minDepth (%f) and maxDepth (%f) were not in [0, 1] "
                                    "or minDepth was "
            ...
            [&](CommandAllocator* allocator) -> MaybeError {
                if (IsValidationEnabled()) {
                    DAWN_INVALID_IF(
                        ...
                        "Scissor rect (x: %u, y: %u, width: %u, height: %u) were not contained in "
                        "the render target dimensions (%u x %u).",
                        x, y, width, height, mRenderTargetWidth, mRenderTargetHeight);
                }
            ...
            [&](CommandAllocator* allocator) -> MaybeError {
                if (IsValidationEnabled()) {
                    DAWN_INVALID_IF(mOcclusionQuerySet.Get() == nullptr,
                                    "The occlusionQuerySet in RenderPassDescriptor was not set.");
                    ...
                    DAWN_TRY_CONTEXT(
                        ...
                        "validating the occlusion query index");
                }
    ...
    void RenderPassEncoder::APIEndOcclusionQuery() {
        TRY_ENCODE_CONTEXT(
            ...
            [&](CommandAllocator* allocator) -> MaybeError {
                if (IsValidationEnabled()) {
                    DAWN_INVALID_IF(!mOcclusionQueryActive, "No occlusion queries were active.");
                }
                ...
                if (IsValidationEnabled()) {
                    ...
                    DAWN_TRY_CONTEXT(
                        ValidateQueryIndexOverwrite(querySet, queryIndex,
                                                    mUsageTracker.GetQueryAvailabilityMap()),
                        "validating the timestamp query index of %s", querySet);
                }
