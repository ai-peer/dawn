...
namespace dawn_native {
    ...
    void IndirectDrawMetadata::AddBundle(uint32_t maxDrawCallsPerIndirectValidationBatch,
                                         uint32_t maxBatchOffsetRange,
        ...
        for (const auto& entry :
            ...
            if (it != mIndexedIndirectBufferValidationInfo.end() && it->first == config) {
                ...
                for (const IndexedIndirectValidationBatch& batch : entry.second.GetBatches()) {
                    it->second.AddBatch(maxDrawCallsPerIndirectValidationBatch, maxBatchOffsetRange,
                                        batch);
                }
            } else {
    ...
    void IndirectDrawMetadata::AddIndexedIndirectDraw(
        ...
        it->second.AddIndexedIndirectDraw(maxDrawCallsPerIndirectValidationBatch,
                                          maxBatchOffsetRange, std::move(draw));
    }
