...
namespace dawn_native {
        ...
        for (uint32_t i = 0; i < descriptor->entryCount; ++i) {
            ...
            if (entry.buffer.type != wgpu::BufferBindingType::Undefined) {
                ...
                if (texture.multisampled) {
                    if (texture.viewDimension != wgpu::TextureViewDimension::e2D) {
                ...
                // TODO: Raising this warning breaks a ton of validation tests.
    ...
    BindGroupLayoutBase::BindGroupLayoutBase(DeviceBase* device,
        ...
        for (BindingIndex i{0}; i < mBindingInfo.size(); ++i) {
            ...
            // TODO: This code currently converts the new-style BindGroupLayoutEntry definitions
            // into the older-style BindingInfo. This is to allow for a staggered conversion, but
            // it means that there's some combinations that the more expressive new style can handle
            // that will be lost in the translation. The solution is to update BindingInfo to only
            // reflect the new-style entry layout and convert the old-style entry layout into it
            // instead.
