...
namespace dawn_native {
    ...
    void IncrementBindingCounts(BindingCounts* bindingCounts, const BindGroupLayoutEntry& entry) {
        ...
        switch (entry.type) {
            case wgpu::BindingType::UniformBuffer:
                bindingCounts->bufferCount += 1;
                if (entry.hasDynamicOffset) {
                    bindingCounts->dynamicUniformBufferCount += 1;
                }
                if (entry.minBufferBindingSize == 0) {
                    bindingCounts->unverifiedBufferCount += 1;
                }
            ...
            case wgpu::BindingType::ReadonlyStorageBuffer:
                bindingCounts->bufferCount += 1;
                if (entry.hasDynamicOffset) {
                    bindingCounts->dynamicStorageBufferCount += 1;
                }
                if (entry.minBufferBindingSize == 0) {
                    bindingCounts->unverifiedBufferCount += 1;
                }
        ...
        for (SingleShaderStage stage : IterateStages(entry.visibility)) {
            bindingCounts->perStage[stage].*perStageBindingCountMember += 1;
        }
    }
    ...
    uint32_t BindGroupLayoutBase::GetDynamicUniformBufferCount() const {
        return mBindingCounts.dynamicUniformBufferCount;
    }

    uint32_t BindGroupLayoutBase::GetDynamicStorageBufferCount() const {
        return mBindingCounts.dynamicStorageBufferCount;
    }
    ...
    const PerStage<PerStageBindingCounts>& BindGroupLayoutBase::GetPerStageBindingCounts() const {
        return mBindingCounts.perStage;
    }
