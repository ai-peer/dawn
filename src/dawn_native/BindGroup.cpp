...
namespace dawn_native {
    ...
    namespace {
        ...
        MaybeError ValidateBufferBinding(const DeviceBase* device,
            ...
            DAWN_VALIDATION_ERROR_IF(bindingSize > bufferSize,
                                     "Binding size (%u) is larger than the buffer size (%u) of %s.",
                                     bindingSize, bufferSize, entry.buffer);

            DAWN_VALIDATION_ERROR_IF(bindingSize == 0, "Binding size is zero");
            ...
            DAWN_VALIDATION_ERROR_IF(!IsAligned(entry.offset, requiredBindingAlignment),
                                     "Offset (%u) does not satisfy the minimum alignment (%u).",
                                     entry.offset, requiredBindingAlignment);

            DAWN_VALIDATION_ERROR_IF(!(entry.buffer->GetUsage() & requiredUsage),
    ...
    MaybeError ValidateBindGroupDescriptor(DeviceBase* device,
        ...
        for (uint32_t i = 0; i < descriptor->entryCount; ++i) {
            ...
            DAWN_VALIDATION_ERROR_IF(
                bindingsSet[bindingIndex],
                "In entry %u, binding index %u already used by a previous entry", i, entry.binding);
