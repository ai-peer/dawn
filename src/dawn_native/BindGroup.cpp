...
namespace dawn_native {
    ...
    MaybeError ValidateBindGroupDescriptor(DeviceBase* device,
        ...
        const auto& bindingMap = descriptor->layout->GetBindingMap();
        ...
        for (uint32_t i = 0; i < descriptor->bindingCount; ++i) {
            ...
            auto it = bindingMap.find(binding.binding);
    ...
    BindGroupBase::BindGroupBase(DeviceBase* device,
        ...
        for (uint32_t i = 0; i < descriptor->bindingCount; ++i) {
            ...
            uint32_t bindingIndex = descriptor->layout->GetBindingIndex(binding.binding);
    ...
    BufferBinding BindGroupBase::GetBindingAsBufferBinding(uint32_t bindingIndex) {
        ASSERT(!IsError());
        ASSERT(bindingIndex < mLayout->GetBindingCount());
    ...
    SamplerBase* BindGroupBase::GetBindingAsSampler(uint32_t bindingIndex) {
        ASSERT(!IsError());
        ASSERT(bindingIndex < mLayout->GetBindingCount());
    ...
    TextureViewBase* BindGroupBase::GetBindingAsTextureView(uint32_t bindingIndex) {
        ASSERT(!IsError());
        ASSERT(bindingIndex < mLayout->GetBindingCount());
