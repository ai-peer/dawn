...
namespace dawn_native {
    ...
    namespace {
        ...
        MaybeError ValidateSamplerBinding(const DeviceBase* device, const BindGroupBinding& binding) {
        ...
        for (uint32_t i = 0; i < descriptor->numBindings; ++i) {
            ...
            switch (layoutInfo.types[bindingIndex]) {
                ...
                case dawn::BindingType::SampledTexture:
                    DAWN_TRY(ValidateTextureBinding(device, binding, dawn::TextureUsageBit::Sampled));
                    break;
    ...
    BindGroupBase::BindGroupBase(DeviceBase* device, ObjectBase::ErrorTag tag) : ObjectBase(device, tag) {
    }
