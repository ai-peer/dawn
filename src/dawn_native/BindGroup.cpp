...
namespace dawn_native {
    ...
    namespace {
        ...
        MaybeError ValidateBufferBinding(const DeviceBase* device,
            ...
            if (binding.size != kWholeSize && binding.size > bufferSize) {
            ...
            if ((binding.size == kWholeSize && binding.offset > 0) ||
    ...
    BindGroupBase::BindGroupBase(DeviceBase* device, const BindGroupDescriptor* descriptor)
        ...
        for (uint32_t i = 0; i < descriptor->bindingCount; ++i) {
            ...
            if (binding.buffer != nullptr) {
                ...
                uint64_t bufferSize =
                    (binding.size == kWholeSize) ? binding.buffer->GetSize() : binding.size;
