...
namespace dawn_native {
    ...
    BindGroupBase::BindGroupBase(DeviceBase* device,
        ...
        for (BindingIndex bindingIndex = 0, packed_i = 0;
             bindingIndex < descriptor->layout->GetBufferCount(); ++bindingIndex) {
            if (descriptor->layout->GetBindingInfo(bindingIndex).minimumBufferSize == 0) {
                mBindingData.unverifiedBufferSizes[packed_i] =
    ...
    const uint64_t* BindGroupBase::GetUnverifiedBufferSizes() const {
        ASSERT(!IsError());
