...
namespace dawn_native {
    ...
    namespace {
        ...
        MaybeError ValidateCopySizeFitsInTexture(const TextureCopy& textureCopy, const Extent3D& copySize) {
        ...
        MaybeError ValidateCopySizeFitsInBuffer(const BufferCopy& bufferCopy,
                                                uint32_t dataSize) {
        ...
        MaybeError ValidateRowPitch(dawn::TextureFormat format, const Extent3D& copySize, uint32_t rowPitch) {
        ...
        while (mIterator.NextCommandId(&type)) {
            switch (type) {
                ...
                case Command::CopyBufferToTexture: {
                    ...
                    DAWN_TRY(ValidateRowPitch(copy->destination.texture->GetFormat(), copy->copySize, copy->source.rowPitch));
                ...
                case Command::CopyTextureToBuffer: {
                    ...
                    uint32_t bufferCopySize = 0;
                    DAWN_TRY(ValidateRowPitch(copy->source.texture->GetFormat(), copy->copySize, copy->destination.rowPitch));
                    DAWN_TRY(ComputeTextureCopyBufferSize(copy->copySize, copy->destination.rowPitch,
                                                          &bufferCopySize));
    ...
    void CommandBufferBuilder::CopyBufferToTexture(const BufferCopyView* source,
        ...
        copy->destination.origin.x = destination->origin.x;
        copy->destination.origin.y = destination->origin.y;
        copy->destination.origin.z = destination->origin.z;
        copy->copySize.width = copySize->width;
        copy->copySize.height = copySize->height;
        copy->copySize.depth = copySize->depth;
    ...
    void CommandBufferBuilder::CopyTextureToBuffer(const TextureCopyView* source,
        ...
        copy->source.origin.x = source->origin.x;
        copy->source.origin.y = source->origin.y;
        copy->source.origin.z = source->origin.z;
        copy->copySize.width = copySize->width;
        copy->copySize.height = copySize->height;
        copy->copySize.depth = copySize->depth;
