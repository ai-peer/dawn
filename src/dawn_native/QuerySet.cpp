...
namespace dawn_native {
        ...
        switch (descriptor->type) {
            case wgpu::QueryType::PipelineStatistics: {
                ...
                if (descriptor->pipelineStatisticsCount == 0) {
                    return DAWN_VALIDATION_ERROR("Pipeline statistics count must be larger than 0");
                }

                if (descriptor->pipelineStatisticsCount > kMaxPipelineStatisticsCount) {
                    return DAWN_VALIDATION_ERROR("Pipeline statistics count exceeds maximum");
                }

                if (descriptor->pipelineStatistics == nullptr) {
                    return DAWN_VALIDATION_ERROR(
                        "The pipelineStatistics must be set if query type is PipelineStatistics");
                }
    ...
    QuerySetBase::QuerySetBase(DeviceBase* device, const QuerySetDescriptor* descriptor)
        ...
        for (uint32_t i = 0; i < descriptor->pipelineStatisticsCount; i++) {
            mPipelineStatistics[i] = descriptor->pipelineStatistics[i];
        }
    }
    ...
    PipelineStatisticsArray QuerySetBase::GetPipelineStatistics() const {
