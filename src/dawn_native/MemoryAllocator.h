...
namespace dawn_native {
    ...
    class MemoryAllocator {
      public:
        ...
        virtual ResultOrError<std::unique_ptr<MemoryBase>> Allocate(uint64_t size,
                                                                    int memoryFlags) = 0;
        virtual void Deallocate(std::unique_ptr<MemoryBase> allocation) = 0;
    };
}  // namespace dawn_native
