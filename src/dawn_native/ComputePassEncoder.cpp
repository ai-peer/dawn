        ...
        mEncodingContext->TryEncode(
            ...
            [&](CommandAllocator* allocator) -> MaybeError {
                ...
                if (IsValidationEnabled()) {
                    ...
                    scope.BufferUsedAs(validatedIndirectBuffer.Get(), wgpu::BufferUsage::Indirect);
                    // TODO(crbug.com/dawn/1166): Adding |indirectBuffer| is needed for correct
                    // usage validation, but it will unecessarily transition the |indirectBuffer| in
                    // the backend.
                    scope.BufferUsedAs(indirectBuffer, wgpu::BufferUsage::Indirect);
                    mUsageTracker.AddReferencedBuffer(indirectBuffer);
                    mUsageTracker.AddReferencedBuffer(validatedIndirectBuffer.Get());
