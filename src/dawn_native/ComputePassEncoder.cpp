...
namespace dawn_native {
    ...
    namespace {
        ...
        ResultOrError<ComputePipelineBase*> GetOrCreateIndirectDispatchValidationPipeline(
            ...
            std::string shaderSource =
                ...
                std::to_string(device->GetLimits().v1.maxComputeWorkgroupsPerDimension) + "u;\n";
    ...
    ComputePassEncoder::ValidateIndirectDispatch(BufferBase* indirectBuffer,
        ...
        BindGroupBase* previousBindGroup = mCommandBufferState.GetBindGroup(BindGroupIndex(0));
        ...
        if (previousPipeline) {
            ...
            // Clear out the state if there was no previous pipeline.
            mCommandBufferState.SetComputePipeline(nullptr);
        }
        if (previousBindGroup) {
            // Reset the previously-bound bind group.
            // TODO: dynamic offsets too
            APISetBindGroup(0, previousBindGroup);
        } else {
            // Clear out the state if there was no previous bind group.
