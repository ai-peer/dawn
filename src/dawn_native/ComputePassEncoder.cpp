...
namespace dawn_native {
    ...
    ComputePassEncoder::TransformIndirectDispatchBuffer(Ref<BufferBase> indirectBuffer,
        ...
        ComputePipelineBase* currentComputePipeline = mCommandBufferState.GetComputePipeline();

        if (!device->ShouldTransformIndirectBuffer(currentComputePipeline)) {
        ...
        Ref<BufferBase> uniformBuffer;
        {
            ...
            params.enableValidation = static_cast<uint32_t>(GetDevice()->IsValidationEnabled());
            params.duplicateNumWorkgroups =
                static_cast<uint32_t>(GetDevice()->ShouldDuplicateNumWorkgroupsForDispatchIndirect(
        ...
        const uint64_t scratchBufferSize =
            GetDevice()->GetDispatchIndirectScratchBufferSize(currentComputePipeline);
