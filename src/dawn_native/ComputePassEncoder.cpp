...
namespace dawn_native {
    ...
    namespace {
        ...
        MaybeError ValidatePerDimensionDispatchSizeLimit(const DeviceBase* device, uint32_t size) {
            DAWN_INVALID_IF(size > device->GetLimits().v1.maxComputeWorkgroupsPerDimension,
                            "Dispatch size (%u) exceeds max compute workgroups per dimension (%u).",
                            size, device->GetLimits().v1.maxComputeWorkgroupsPerDimension);
