...
namespace dawn_native {
    ...
    ErrorScope::ErrorScope(wgpu::ErrorFilter errorFilter) : mErrorFilter(errorFilter) {
    }
    ...
    bool ErrorScopeStack::HandleError(wgpu::ErrorType type, const char* message) {
        ...
        for (auto it = mScopes.rbegin(); it != mScopes.rend(); ++it) {
            if (ErrorFilterToErrorType(it->mErrorFilter) != type) {
            ...
            if (it->mErrorType == wgpu::ErrorType::NoError) {
                it->mErrorType = type;
                it->mErrorMessage = message;
            }

            if (type == wgpu::ErrorType::DeviceLost) {
                if (it->mErrorType != wgpu::ErrorType::DeviceLost) {
