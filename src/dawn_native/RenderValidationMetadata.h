      ...
      public:
        ...
        class IndexedIndirectBufferValidationInfo {
          public:
            ...
            // Adds draw calls from an already-computed pass, e.g. from a previously encoded
            // RenderBundle. The added pass is merged into an existing pass if possible, otherwise
            // it's added to mPasses.
            void AddPass(const IndexedIndirectValidationPass& pass);

            const std::vector<IndexedIndirectValidationPass>& GetPasses() const;
          ...
          private:
            ...
            // minOffset and may overlap iff the number of offsets in one pass would otherwise
