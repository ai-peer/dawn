...
namespace dawn_native {
    ...
    const uint64_t kMaxDrawCallsPerIndirectValidationBatch =
    ...
    MaybeError EncodeIndirectDrawValidationCommands(DeviceBase* device,
        ...
        struct Batch {
            ...
            size_t validatedParamsBufferIndex;
        ...
        struct Pass {
            ...
            std::unique_ptr<uint8_t[]> batchData;
        ...
        size_t currentValidatedParamsBufferIndex = 0;
        uint64_t currentValidatedParamsSize = 0;
        ...
        for (auto& entry : bufferInfoMap) {
            ...
            for (const IndirectDrawMetadata::IndexedIndirectValidationBatch& batch :
                ...
                uint64_t validatedParamsOffset =
                    Align(currentValidatedParamsSize, kMinStorageBufferOffsetAlignment);
                uint64_t newValidatedParamsSize =
                    validatedParamsOffset + newBatch.validatedParamsSize;
                if (newValidatedParamsSize <= kMaxStorageBufferBindingSize) {
                    currentValidatedParamsSize = newValidatedParamsSize;
                } else {
                    ++currentValidatedParamsBufferIndex;
                    validatedParamsOffset = 0;
                    currentValidatedParamsSize = newBatch.validatedParamsSize;
                }

                newBatch.validatedParamsOffset = validatedParamsOffset;
                newBatch.validatedParamsBufferIndex = currentValidatedParamsBufferIndex;
        ...
        ScratchBuffer& validatedParamsScratchBuffer = store->scratchIndirectStorage;
        ...
        std::vector<Ref<BufferBase>> validatedParamsBuffers;
        validatedParamsBuffers.reserve(currentValidatedParamsBufferIndex);
        if (currentValidatedParamsBufferIndex == 0) {
            // In the most common case, we only need one buffer for all validated param data, so we
            // can use the scratch buffer alone.
            DAWN_TRY(validatedParamsScratchBuffer.EnsureCapacity(currentValidatedParamsSize));
            validatedParamsBuffers.push_back(validatedParamsScratchBuffer.GetBuffer());
        } else {
            // Otherwise we max out the scratch buffer and allocate some auxilliary buffers for the
            // remaining data.
            //
            // TODO(rockot): In might be worth doing some more resource caching here if there turn
            // out to be interesting use cases for such a huge amount of validation, but that seems
            // unlikely.
            DAWN_TRY(validatedParamsScratchBuffer.EnsureCapacity(kMaxStorageBufferBindingSize));
            validatedParamsBuffers.push_back(validatedParamsScratchBuffer.GetBuffer());
            for (size_t i = 0; i < currentValidatedParamsBufferIndex; ++i) {
                BufferDescriptor desc;
                desc.usage = wgpu::BufferUsage::Indirect | wgpu::BufferUsage::Storage;
                if (i == currentValidatedParamsBufferIndex - 1) {
                    desc.size = currentValidatedParamsSize;
                } else {
                    desc.size = kMaxStorageBufferBindingSize;
                }

                Ref<BufferBase> newBuffer;
                DAWN_TRY_ASSIGN(newBuffer, device->CreateBuffer(&desc));
                validatedParamsBuffers.push_back(std::move(newBuffer));
            }
        }

        for (Ref<BufferBase>& validatedParamsBuffer : validatedParamsBuffers) {
            usageTracker->BufferUsedAs(validatedParamsBuffer.Get(), wgpu::BufferUsage::Indirect);
        }
        ...
        for (Pass& pass : passes) {
            pass.batchData = std::make_unique<uint8_t[]>(pass.batchDataSize);
            for (Batch& batch : pass.batches) {
                batch.batchInfo = new (&pass.batchData[batch.dataBufferOffset]) BatchInfo();
                ...
                for (size_t i = 0; i < batch.metadata->draws.size(); ++i) {
                    const auto& draw = batch.metadata->draws[i];
                    const uint64_t offsetFromAlignedBoundary =
                        draw.clientBufferOffset - batch.clientIndirectOffset;

                    // The shader uses this to index an array of u32, hence the division by 4 bytes.
                    indirectOffsets[i] = static_cast<uint32_t>(offsetFromAlignedBoundary) / 4;

                    DeferredBufferLocationUpdate deferredUpdate;
                    deferredUpdate.location = draw.bufferLocation;
                    deferredUpdate.buffer =
                        validatedParamsBuffers[batch.validatedParamsBufferIndex];
                    deferredUpdate.offset =
                        batch.validatedParamsOffset + i * kDrawIndexedIndirectSize;
        ...
        for (const Pass& pass : passes) {
            commandEncoder->APIWriteBuffer(batchDataBuffer.GetBuffer(), 0, pass.batchData.get(),
                                           pass.batchDataSize);
            ...
            for (const Batch& batch : pass.batches) {
                ...
                validatedParamsBinding.buffer =
                    validatedParamsBuffers[batch.validatedParamsBufferIndex].Get();
