...
#endif  // defined(DAWN_ENABLE_BACKEND_METAL)
#if defined(DAWN_ENABLE_BACKEND_VULKAN)
        // TODO(enga): When we can load SwiftShader in parallel with the system driver, we should
        // create the backend only once and expose SwiftShader as an additional adapter.
        // For now, we create two VkInstances, one from SwiftShader, and one from the system.
        // Note: If the Vulkan driver *is* SwiftShader, then this would load SwiftShader twice.
