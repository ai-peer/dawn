...
namespace dawn_native {
    ...
    MaybeError ResourceUsageTracker::ValidateResourceUsages() const {
        ...
        auto ValidateBufferUsage = [](BufferBase* buffer, wgpu::BufferUsage usage) -> MaybeError {
        ...
        auto ValidateTextureUsage = [](TextureBase* texture,
        ...
        for (const PassResourceUsage& pass : mResourceUsages.perPass) {
            for (size_t i = 0; i < pass.buffers.size(); ++i) {
                DAWN_TRY(ValidateBufferUsage(pass.buffers[i], pass.bufferUsages[i]));
            }

            for (size_t i = 0; i < pass.textures.size(); ++i) {
                DAWN_TRY(ValidateTextureUsage(pass.textures[i], pass.textureUsages[i]));
            }
        }
