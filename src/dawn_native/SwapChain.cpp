...
namespace dawn_native {
    ...
    MaybeError ValidateSwapChainDescriptor(const DeviceBase* device, const SwapChainDescriptor* descriptor) {
    ...
    SwapChainBase::SwapChainBase(DeviceBase* device, const SwapChainDescriptor* descriptor)
        : ObjectBase(device), mImplementation(*reinterpret_cast<dawnSwapChainImplementation*>(descriptor->implementation)) {
    }
    ...
    MaybeError SwapChainBase::ValidatePresent(TextureBase* texture) const {
        ...
        if (texture != mLastNextTexture) {
            return DAWN_VALIDATION_ERROR("Tried to present something other than the last NextTexture");
        }
