...
namespace dawn_native {
    ...
    bool ExtensionsSet::IsEnabled(Extension extension) const {
        ASSERT(extension != Extension::InvalidEnum);
        const size_t extensionIndex = static_cast<size_t>(extension);
        return extensionsBitSet.test(extensionIndex);
    }
    ...
    const ExtensionInfo* ExtensionsInfo::GetExtensionInfo(const char* extensionName) {
        ASSERT(extensionName);

        EnsureExtensionNameToEnumMapInitialized();
    ...
    Extension ExtensionsInfo::ExtensionNameToEnum(const char* extensionName) {
        ASSERT(extensionName);
    ...
    void ExtensionsInfo::EnsureExtensionNameToEnumMapInitialized() {
        if (mExtensionNameToEnumMapInitialized) {
            return;
        }
        ...
        mExtensionNameToEnumMapInitialized = true;
    }
