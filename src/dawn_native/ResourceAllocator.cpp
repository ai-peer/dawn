...
namespace dawn_native {
    ...
    template <class TBlock, class TResourceAllocator, class TBlockAllocator, size_t minBlockSize>
    template <typename... ResourceAllocatorArgs>
    BuddyPoolAllocator<TBlock, TResourceAllocator, TBlockAllocator, minBlockSize>::
    ...
    template <class TBlock, class TResourceAllocator, class TBlockAllocator, size_t minBlockSize>
    size_t
    BuddyPoolAllocator<TBlock, TResourceAllocator, TBlockAllocator, minBlockSize>::GetResourceIndex(
        size_t offset) const {
        return offset / mResourceHeapSize;
    }

    template <class TBlock, class TResourceAllocator, class TBlockAllocator, size_t minBlockSize>
    TBlock BuddyPoolAllocator<TBlock, TResourceAllocator, TBlockAllocator, minBlockSize>::Allocate(
        ...
        if (resourceIndex >= mTrackedResourceAllocations.size()) {
            ...
            for (uint32_t i = 0; i < numOfResourcesRequired; i++) {
                ...
                mTrackedResourceAllocations.push_back(
                    {/*refcount*/ 0, mResourceAllocator.Allocate(mResourceHeapSize)});
            }
        }
    ...
    template <class TBlock, class TResourceAllocator, class TBlockAllocator, size_t minBlockSize>
    ResourceAllocation
    BuddyPoolAllocator<TBlock, TResourceAllocator, TBlockAllocator, minBlockSize>::GetSubAllocation(
        const TBlock& block) const {
        const size_t resourceIndex = GetResourceIndex(block.GetOffset());
    ...
    template <class TBlock, class TResourceAllocator, class TBlockAllocator, size_t minBlockSize>
    void BuddyPoolAllocator<TBlock, TResourceAllocator, TBlockAllocator, minBlockSize>::Deallocate(
        TBlock block) {
        const size_t resourceIndex = GetResourceIndex(block.GetOffset());
    ...
    template <class TBlock, class TResourceAllocator, class TBlockAllocator, size_t minBlockSize>
    size_t BuddyPoolAllocator<TBlock, TResourceAllocator, TBlockAllocator, minBlockSize>::
