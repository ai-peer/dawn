...
namespace dawn_native {
    ...
    template <typename... ResourceAllocatorArgs>
    BuddyPoolAllocator<TBlock, TResourceAllocator, TBlockAllocator>::BuddyPoolAllocator(
        size_t maxSize,
        size_t resourceSize,
        ResourceAllocatorArgs&&... resourceArgs)
    ...
    template <class TBlock, class TResourceAllocator, class TBlockAllocator>
    size_t BuddyPoolAllocator<TBlock, TResourceAllocator, TBlockAllocator>::GetResourceIndex(
        size_t offset) const {
        return offset / mMaxResourceSize;
    }

    template <class TBlock, class TResourceAllocator, class TBlockAllocator>
    TBlock BuddyPoolAllocator<TBlock, TResourceAllocator, TBlockAllocator>::Allocate(
    ...
    template <class TBlock, class TResourceAllocator, class TBlockAllocator>
    ResourceAllocation
    BuddyPoolAllocator<TBlock, TResourceAllocator, TBlockAllocator>::GetSubAllocation(
        const TBlock& block) const {
        const size_t resourceIndex = GetResourceIndex(block.GetOffset());
        return ResourceAllocation(mTrackedResourceAllocations[resourceIndex].mResource.get(),
                                  block);
    }

    template <class TBlock, class TResourceAllocator, class TBlockAllocator>
    void BuddyPoolAllocator<TBlock, TResourceAllocator, TBlockAllocator>::Deallocate(TBlock block) {
    ...
    BuddyPoolAllocator<TBlock, TResourceAllocator, TBlockAllocator>::GetBlockAllocator() const {
        return mBlockAllocator;
    }

    template <class TBlock, class TResourceAllocator, class TBlockAllocator>
    size_t BuddyPoolAllocator<TBlock, TResourceAllocator, TBlockAllocator>::GetResourceCount()
        const {
    ...
    BuddyBlockAllocator<TBlock>::BuddyBlockAllocator(size_t size) : mMaxBlockSize(size) {
    ...
    template <class TBlock>
    size_t BuddyBlockAllocator<TBlock>::ComputeNumOfFreeBlocks(TBlock* block) const {
    ...
    template <class TBlock>
    void BuddyBlockAllocator<TBlock>::InsertFreeBlock(TBlock* block, size_t level) {
    ...
    template <class TBlock>
    void BuddyBlockAllocator<TBlock>::RemoveFreeBlock(TBlock* block, size_t level) {
        ASSERT(block->mState == BlockState::Free);
    ...
    template <class TBlock>
    void BuddyBlockAllocator<TBlock>::DeleteBlock(TBlock* block) {
    ...
    void ConditionalAllocator<TBlock, TPoolAllocator, TDirectAllocator>::Deallocate(TBlock block) {
        return (block.GetResourceHeap()) ? mDirectAllocator.Allocate(allocationSize)
                                         : mPoolAllocator.Allocate(allocationSize);
    }
