...
namespace dawn_native {
    ...
    size_t ResourceAllocation::GetOffset() const {
        ...
        ASSERT(mSubAllocationBlock.GetOffset() == 0);

        return 0;
    }

    size_t ResourceAllocation::GetAlignment() const {
        return mSubAllocationBlock.GetAlignment();
    }

    ResultOrError<void*> ResourceAllocation::Map() {
        void* mappedPointer = nullptr;
        DAWN_TRY_ASSIGN(mappedPointer, mResourceHeap->Map());
        return static_cast<uint8_t*>(mappedPointer) + GetOffset();
    }
    ...
    HeapSubAllocationBlock::HeapSubAllocationBlock(size_t size,
        ...
        : Block(size, offset, alignment), mResourceHeap(resourceHeap){};
    ...
    TBlock DirectAllocator<TBlock, TResourceAllocator>::Allocate(size_t allocationSize) {
        return TBlock(allocationSize, /*offset*/ 0, /*alignment*/ 0,
                      mResourceAllocator.Allocate(allocationSize));
    }
    ...
    template <typename... ResourceAllocatorArgs>
    BuddyPoolAllocator<TBlock, TResourceAllocator, TBlockAllocator, minResourceHeapSize>::
    ...
    template <class TBlock,
              ...
              class TBlockAllocator,
              size_t minResourceHeapSize>
    size_t BuddyPoolAllocator<TBlock, TResourceAllocator, TBlockAllocator, minResourceHeapSize>::
        GetResourceIndex(size_t offset) const {
    ...
    template <class TBlock,
              ...
              size_t minResourceHeapSize>
    TBlock
    BuddyPoolAllocator<TBlock, TResourceAllocator, TBlockAllocator, minResourceHeapSize>::Allocate(
        size_t allocationSize) {
        const TBlock empty = TBlock{0, 0, 0};

        // Specified resource size cannot be smaller then the resource heap min size.
    ...
    ResourceAllocation
    BuddyPoolAllocator<TBlock, TResourceAllocator, TBlockAllocator, minResourceHeapSize>::
        GetSubAllocation(const TBlock& block) const {
        const size_t resourceIndex = GetResourceIndex(block.GetOffset());
    ...
    template <class TBlock,
              ...
              class TBlockAllocator,
              size_t minResourceHeapSize>
    void BuddyPoolAllocator<TBlock, TResourceAllocator, TBlockAllocator, minResourceHeapSize>::
        Deallocate(TBlock block) {
        const size_t resourceIndex = GetResourceIndex(block.GetOffset());
    ...
    BuddyBlockAllocator<TBlock>::BuddyBlockAllocator(size_t maxSize, size_t minSize)
        ...
        mRoot = new BuddyBlock(maxSize, /*offset*/ 0, /*alignment*/ mMinBlockSize);
    ...
    TBlock BuddyBlockAllocator<TBlock>::Allocate(size_t allocationSize) {
        ...
        // Allocation size must be a power-of-two.
        if (!IsPowerOfTwo(allocationSize))
            return empty;

        // Allocation size cannot the max block size.
        if (allocationSize > mMaxBlockSize)
            return empty;

        // Allocation size must be evenly divisible by min block size.
        if (allocationSize % mMinBlockSize != 0)
            return empty;
        ...
        if (allocationSizeToLevel >= mFreeLists.size())
            return empty;
        ...
        while (currBlockLevel < allocationSizeToLevel) {
            ...
            BuddyBlock* leftChildBlock =
                new BuddyBlock(nextLevelSize, currBlock->GetOffset(), mMinBlockSize);
            BuddyBlock* rightChildBlock = new BuddyBlock(
                nextLevelSize, currBlock->GetOffset() + nextLevelSize, mMinBlockSize);
