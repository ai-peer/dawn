...
namespace dawn_native {
    ...
    MaybeError ValidatePassResourceUsage(const PassResourceUsage& pass) {
        ...
        for (size_t i = 0; i < pass.textures.size(); ++i) {
            ...
            constexpr wgpu::TextureUsage kReadOnlyTextureUsagesInPass =
                wgpu::TextureUsage::Sampled | kReadonlyStorageTexture;
            bool readOnly = (usage & kReadOnlyTextureUsagesInPass) == usage;
            bool singleUse = wgpu::HasZeroOrOneBits(usage);
            if (!readOnly && !singleUse) {
                return DAWN_VALIDATION_ERROR(
