...
namespace dawn_native {
    ...
    MaybeError ValidateTextureCopyView(DeviceBase const* device,
        ...
        if (texture->GetSampleCount() > 1 ||
            ((texture->GetUsage() & wgpu::TextureUsage::OutputAttachment) != 0 &&
             texture->GetFormat().HasDepthOrStencil())) {
            Extent3D subresourceSize = texture->GetMipLevelVirtualSize(textureCopy.mipLevel);
            ...
            if (textureCopy.origin.x != 0 || textureCopy.origin.y != 0 ||
                ...
                return DAWN_VALIDATION_ERROR(
                    "The entire subresource must be copied when using a depth/stencil texture with "
                    "OutputAttachment usage, or "
