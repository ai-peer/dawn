...
namespace dawn_native {
    ...
    MaybeError ValidatePassResourceUsage(const PassResourceUsage& pass) {
        ...
        for (size_t i = 0; i < pass.textures.size(); ++i) {
            ...
            // Because in most cases, a texture:
            // 1) doesn't have subresources (we can also say the texture has one subresource only),
            // then we can rely on the usage of the whole texture.
            // 2) or is used as single-write or multiple read even though the texture has
            // subresources, then we can skip iterating the subresources' usages.
