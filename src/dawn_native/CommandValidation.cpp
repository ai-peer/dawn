...
namespace dawn_native {
    ...
    ResultOrError<uint64_t> ComputeRequiredBytesInCopy(const TexelBlockInfo& blockInfo,
        ...
        ASSERT(copySize.depth <= 1 ||
               (bytesPerRow != WGPU_STRIDE_UNDEFINED && rowsPerImage != WGPU_STRIDE_UNDEFINED));
        ...
        if (heightInBlocks > 0) {
            ASSERT(heightInBlocks <= 1 || bytesPerRow != WGPU_STRIDE_UNDEFINED);
    ...
    TextureDataLayout FixUpDeprecatedTextureDataLayoutOptions(
        ...
        if (copyExtent.height != 0 && layout.rowsPerImage == 0) {
            if (copyExtent.depth > 1) {
                ...
                layout.rowsPerImage = WGPU_STRIDE_UNDEFINED;
            }
        }
        ...
        if (copyExtent.height == 1 && copyExtent.depth == 1 &&
            ...
            device->EmitDeprecationWarning(
                ...
                "or left unspecified.");
            layout.bytesPerRow = WGPU_STRIDE_UNDEFINED;
        }
        return layout;
    }

    // Replace WGPU_STRIDE_UNDEFINED with real values, so backends don't have to think about it.
    ...
    MaybeError ValidateLinearTextureData(const TextureDataLayout& layout,
        ...
        if (copyExtent.depth > 1 && (layout.bytesPerRow == WGPU_STRIDE_UNDEFINED ||
                                     layout.rowsPerImage == WGPU_STRIDE_UNDEFINED)) {
            return DAWN_VALIDATION_ERROR(
                "If copy depth > 1, bytesPerRow and rowsPerImage must be specified.");
        }
        if (heightInBlocks > 1 && layout.bytesPerRow == WGPU_STRIDE_UNDEFINED) {
        ...
        // These != WGPU_STRIDE_UNDEFINED checks are technically redundant with the > checks, but
        // they should get optimized out.
        if (layout.bytesPerRow != WGPU_STRIDE_UNDEFINED && bytesInLastRow > layout.bytesPerRow) {
            return DAWN_VALIDATION_ERROR("The byte size of each row must be <= bytesPerRow.");
        }
        if (layout.rowsPerImage != WGPU_STRIDE_UNDEFINED && heightInBlocks > layout.rowsPerImage) {
            return DAWN_VALIDATION_ERROR(
    ...
    MaybeError ValidateBufferCopyView(DeviceBase const* device,
        ...
        if (bufferCopyView.layout.bytesPerRow != WGPU_STRIDE_UNDEFINED) {
