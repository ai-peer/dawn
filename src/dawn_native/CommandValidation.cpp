    ...
    MaybeError ValidateTextureToTextureCopyRestrictions(const ImageCopyTexture& src,
        ...
        DAWN_INVALID_IF(
            src.texture->GetFormat().format != dst.texture->GetFormat().format &&
                !src.texture->GetFormat().CompatibleWith(dst.texture->GetFormat().format),
            "Source %s format (%s) and destination %s format (%s) do not match.", src.texture,
            src.texture->GetFormat().format, dst.texture, dst.texture->GetFormat().format);
