...
namespace dawn_native {
    ...
    MaybeError ValidateSyncScopeResourceUsage(const SyncScopeResourceUsage& scope) {
        ...
        for (size_t i = 0; i < scope.bufferUsages.size(); ++i) {
            ...
            DAWN_INVALID_IF(
                !readOnly && !singleUse,
                "%s used with writable usage and another usage in the same synchronization "
                "scope.",
                scope.buffers[i]);
        }
            ...
            textureUsage.Iterate([&](const SubresourceRange&, const wgpu::TextureUsage& usage) {
                ...
                if (!readOnly && !singleUse && !error.IsError()) {
                    error = DAWN_FORMAT_VALIDATION_ERROR(
                        "%s used with writable usage and another usage in the same "
    ...
    MaybeError ValidateLinearTextureData(const TextureDataLayout& layout,
        ...
        DAWN_INVALID_IF(
            !fitsInData,
            "Required size for texture data layout (%u) exceeds the linear data size (%u) and "
            "offset (%u).",
    ...
    MaybeError ValidateImageCopyTexture(DeviceBase const* device,
        ...
        if (texture->GetSampleCount() > 1 || texture->GetFormat().HasDepthOrStencil()) {
            ...
            DAWN_INVALID_IF(
                ...
                "[x: 0, y: 0], size: %s) of %s. The entire subresource must be copied the format "
                "(%s) is a depth/stencil format or the sample count (%u) is > 1.",
    ...
    MaybeError ValidateCopyTextureForBrowserRestrictions(const ImageCopyTexture& src,
                                                         const ImageCopyTexture& dst,
                                                         const Extent3D& copySize) {
        DAWN_INVALID_IF(!(src.texture->GetUsage() & wgpu::TextureUsage::TextureBinding),
                        "Source %s usage (%s) does not include %s.", src.texture,
                        src.texture->GetUsage(), wgpu::TextureUsage::TextureBinding);

        DAWN_INVALID_IF(!(dst.texture->GetUsage() & wgpu::TextureUsage::RenderAttachment),
                        "Dest %s usage (%s) does not include %s.", dst.texture,
                        dst.texture->GetUsage(), wgpu::TextureUsage::RenderAttachment);
