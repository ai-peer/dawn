...
namespace dawn_native {
    ...
    void DynamicUploader::Tick(Serial lastCompletedSerial) {
        ...
        // in-flight). Then delete all buffers in sequence from the start, [0th, ith] where the ith
        // buffer corresponds to the ith frame. This ensures the property of increasing buffer sizes
        // holds and given the allocation pattern for "uploads" do not have arbitrary lives, they
        // should be removed in the same order they were allocated.
        ...
        for (size_t i = 0; i < mRingBuffers.size(); ++i) {
            ...
            // again.
            if (mRingBuffers[i]->Empty() && i == bufferCount && i < mRingBuffers.size() - 1)
                bufferCount++;
        }

        if (bufferCount > 0) {
            ASSERT(bufferCount < mRingBuffers.size());
            mRingBuffers.erase(mRingBuffers.begin(), mRingBuffers.begin() + bufferCount);
        }
    }
}  // namespace dawn_native