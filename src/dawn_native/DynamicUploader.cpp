...
namespace dawn_native {
    ...
    MaybeError DynamicUploader::CreateAndAppendBuffer(size_t size) {
        ...
        std::unique_ptr<RingBufferAllocator> ringBuffer =
            std::make_unique<RingBufferAllocator>(mDevice, stagingBuffer.release());
    ...
    ResultOrError<UploadHandle> DynamicUploader::Allocate(uint32_t size, uint32_t alignment) {
        ...
        RingBufferAllocator* largestRingBuffer = GetLargestBuffer();
        size_t startOffset = largestRingBuffer->Allocate(alignedSize);
        ...
        if (startOffset == INVALID_OFFSET) {
            ...
            startOffset = largestRingBuffer->Allocate(alignedSize);
        }
    ...
    RingBufferAllocator* DynamicUploader::GetLargestBuffer() {
