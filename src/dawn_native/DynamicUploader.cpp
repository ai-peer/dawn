...
namespace dawn_native {
    ...
    void DynamicUploader::CreateAndAppendBuffer(size_t size) {
        std::unique_ptr<RingBuffer> ringBuffer = std::make_unique<RingBuffer>(mDevice);
        if (ringBuffer->Initialize(size).IsSuccess()) {
    ...
    UploadHandle DynamicUploader::Allocate(uint32_t size, uint32_t alignment) {
        ...
        if (uploadHandle.mappedBuffer == nullptr) {
            ...
            CreateAndAppendBuffer(newMaxSize);
