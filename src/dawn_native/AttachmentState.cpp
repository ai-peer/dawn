...
namespace dawn_native {
    ...
    AttachmentStateBlueprint::AttachmentStateBlueprint(const RenderPassDescriptor* descriptor) {
        for (uint8_t i = 0; i < descriptor->colorAttachmentCount; ++i) {
            TextureViewBase* attachment = descriptor->colorAttachments[i].attachment;
            mColorAttachmentsSet.set(ColorAttachmentIndex(i));
            mColorFormats[ColorAttachmentIndex(i)] = attachment->GetFormat().format;
            if (mSampleCount == 0) {
