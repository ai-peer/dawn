...
namespace dawn_native {
    ...
    AttachmentStateBlueprint::AttachmentStateBlueprint(const RenderPassDescriptor* descriptor)
        ...
        for (uint32_t i = 0; i < descriptor->colorAttachmentCount; ++i) {
            const RenderPassColorAttachmentDescriptor* attachmentInfo =
                descriptor->colorAttachments[i];
            ASSERT(attachmentInfo != nullptr);
            mColorAttachmentsSet.set(i);
            mColorFormats[i] = attachmentInfo->attachment->GetFormat().format;
            if (!sampleCountInitialized) {
                mSampleCount = attachmentInfo->attachment->GetTexture()->GetSampleCount();
                sampleCountInitialized = true;
            }
        }
        if (mHasDepthStencilAttachment) {
            mDepthStencilFormat =
                descriptor->depthStencilAttachment->attachment->GetFormat().format;
            if (!sampleCountInitialized) {
                mSampleCount =
                    descriptor->depthStencilAttachment->attachment->GetTexture()->GetSampleCount();
                sampleCountInitialized = true;
            }
        }
        ASSERT(sampleCountInitialized);
    }
