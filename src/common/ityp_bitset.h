...
namespace ityp {
    ...
    class bitset : private std::bitset<N> {
      ...
      public:
        ...
        bitset operator<<(Index i) const noexcept {
            return bitset(Base::operator<<(static_cast<I>(i)));
        }

        bitset& operator<<=(Index i) noexcept {
            return static_cast<bitset&>(Base::operator<<=(static_cast<I>(i)));
        }
        ...
        // Should be used sparingly. This is needed to cast to the base type to
        // implement std::hash.
        const Base& AsUntyped() const {
