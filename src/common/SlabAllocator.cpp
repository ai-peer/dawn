...
SlabAllocatorImpl::SlabAllocatorImpl(Index blocksPerSlab,
    ...
    : mAllocationAlignment(allocationAlignment),
      ...
      mTotalAllocationSize(
          ...
          // If the alignment is greater than the default alignment, pad the allocation size by
          // mAllocationAlignment so that the aligned result of malloc() still fulfills the
          // required size.
          (mAllocationAlignment > alignof(max_align_t) ? mAllocationAlignment : 0)) {
