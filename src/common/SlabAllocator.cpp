...
SlabAllocatorImpl::BlockInfo::BlockInfo(Index index, Index nextIndex)
    : index(index), nextIndex(nextIndex) {
}

// Slab

SlabAllocatorImpl::Slab::Slab(std::unique_ptr<char[]> allocation, BlockInfo* head)
...
SlabAllocatorImpl::SlabAllocatorImpl(Index count,
    ...
    : mAllocationAlignment(allocationAlignment),
      ...
      mBlockInfoOffset(blockInfoOffset),
      mCount(count) {
    GetNewSlab();
}
...
SlabAllocatorImpl::BlockInfo* SlabAllocatorImpl::OffsetFrom(BlockInfo* info,
                                                            std::make_signed_t<Index> offset) {
    return reinterpret_cast<BlockInfo*>(reinterpret_cast<char*>(info) +
                                        static_cast<intptr_t>(mBlockSize) * offset);
}

SlabAllocatorImpl::BlockInfo* SlabAllocatorImpl::InfoFromObject(void* object) {
    return reinterpret_cast<SlabAllocatorImpl::BlockInfo*>(static_cast<char*>(object) +
                                                           mBlockInfoOffset);
}

void* SlabAllocatorImpl::ObjectFromInfo(SlabAllocatorImpl::BlockInfo* info) {
    return static_cast<void*>(reinterpret_cast<char*>(info) - mBlockInfoOffset);
}

void SlabAllocatorImpl::PushFront(Slab* slab, BlockInfo* info) {
    BlockInfo* head = slab->freeList;
    if (head == nullptr) {
...
SlabAllocatorImpl::BlockInfo* SlabAllocatorImpl::PopFront(Slab* slab) {
...
void* SlabAllocatorImpl::Allocate() {
    ...
    return ObjectFromInfo(info);
}

void SlabAllocatorImpl::Deallocate(void* ptr) {
    BlockInfo* info = InfoFromObject(ptr);

    ASSERT(info->index >= 0);
    void* firstAllocation = ObjectFromInfo(OffsetFrom(info, -info->index));
...
void SlabAllocatorImpl::GetNewSlab() {
    ...
    // requested size.
    size_t requiredSize = static_cast<size_t>(mDataOffset) + mCount * mBlockSize;
    ...
    BlockInfo* info = InfoFromObject(dataStart);
    for (uint32_t i = 0; i < mCount; ++i) {
        new (OffsetFrom(info, i)) BlockInfo(i, i + 1);
    }

    BlockInfo* lastInfo = OffsetFrom(info, mCount - 1);
    lastInfo->nextIndex = kInvalidIndex;

    mAvailableSlabs.Prepend(new (alignedPtr) Slab(std::move(allocation), info));
}
